...
static void get_strength_ver(Macroblock *MbQ, int edge, int mvlimit,
                             StorablePicture *p);
static void get_strength_hor(Macroblock *MbQ, int edge, int mvlimit,
                             StorablePicture *p);
static void edge_loop_luma_ver(ColorPlane pl, imgpel **Img, byte *Strength,
                               Macroblock *MbQ, int edge);
static void edge_loop_luma_hor(ColorPlane pl, imgpel **Img, byte *Strength,
                               Macroblock *MbQ, int edge, StorablePicture *p);
static void edge_loop_chroma_ver(imgpel **Img, byte *Strength, Macroblock *MbQ,
                                 int edge, int uv, StorablePicture *p);
static void edge_loop_chroma_hor(imgpel **Img, byte *Strength, Macroblock *MbQ,
                                 int edge, int uv, StorablePicture *p);
  ...
  if ((currSlice->slice_type == SP_SLICE) ||
    ...
    if (MbQ->is_intra_block == FALSE) {
      ...
      if (edge || MbP->is_intra_block == FALSE) {
        if (edge &&
            (currSlice->slice_type == P_SLICE && MbQ->mb_type == PSKIP)) {
          ...
          for (idx = 0; idx < MB_BLOCK_SIZE; idx += BLOCK_SIZE) {
            ...
            if ((MbQ->s_cbp[0].blk & (i64_power2(blkQ) | i64_power2(blkP))) !=
                0)
              StrValue = 2;
            else
              StrValue = 0;  // if internal edge of certain types, then we
                             // already know StrValue should be 0
          ...
          for (idx = 0; idx < MB_BLOCK_SIZE; idx += BLOCK_SIZE) {
            ...
            if (((MbQ->s_cbp[0].blk & i64_power2(blkQ)) != 0) ||
              ...
              StrValue = 2;
            else  // for everything else, if no coefs, but vector difference >=
                  // 1 set Strength=1
            {
        ...
        if (edge &&
          ...
          for (idx = 0; idx < BLOCK_SIZE; idx++) {
            ...
            if ((MbQ->s_cbp[0].blk & (i64_power2(blkQ) | i64_power2(blkP))) !=
              ...
              StrValue = 0;  // if internal edge of certain types, we already
                             // know StrValue should be 0
 ...
 *****************************************************************************************
 */
static void luma_ver_deblock_strong(imgpel **cur_img, int pos_x1, int Alpha,
                                    int Beta) {
  int i;
 ...
 *****************************************************************************************
 */
static void luma_ver_deblock_normal(imgpel **cur_img, int pos_x1, int Alpha,
                                    int Beta, int C0, int max_imgpel_value) {
  int i;
  ...
  if (MbP || (MbQ->DFDisableIdc == 0)) {
    ...
    if ((Alpha | Beta) != 0) {
      ...
      for (pel = 0; pel < MB_BLOCK_SIZE; pel += 4) {
        ...
        {
          luma_ver_deblock_normal(cur_img, pos_x1, Alpha, Beta,
                                  ClipTab[*Strength] * bitdepth_scale,
                                  max_imgpel_value);
        }
        cur_img += 4;
        Strength++;
 ...
 *****************************************************************************************
 */
static void luma_hor_deblock_strong(imgpel *imgP, imgpel *imgQ, int width,
                                    int Alpha, int Beta) {
  int pixel;
 ...
 *****************************************************************************************
 */
static void luma_hor_deblock_normal(imgpel *imgP, imgpel *imgQ, int width,
                                    int Alpha, int Beta, int C0,
  ...
  if (C0 == 0) {
    for (i = 0; i < BLOCK_SIZE; ++i) {
      ...
      if (iabs(edge_diff) < Alpha) {
        ...
        if ((iabs(*imgQ - *SrcPtrQ1) < Beta) &&
            (iabs(*imgP - *SrcPtrP1) < Beta)) {
  ...
  if (MbP || (MbQ->DFDisableIdc == 0)) {
    ...
    if ((Alpha | Beta) != 0) {
      ...
      for (pel = 0; pel < BLOCK_SIZE; pel++) {
        ...
        {
          luma_hor_deblock_normal(imgP, imgQ, width, Alpha, Beta,
                                  ClipTab[*Strength] * bitdepth_scale,
                                  max_imgpel_value);
        }
        imgP += 4;
...
static void perform_db_dep_normal(Macroblock *MbQ, StorablePicture *p) {
  ...
  if (MbQ->DFDisableIdc == 2) {
    ...
    // if this the bottom of a frame macroblock pair then always filter the top
    // edge
  ...
  if (MbQ->luma_transform_size_8x8_flag) {
    ...
    for (edge = 0; edge < 4; edge += 2) {
      ...
      if (MbQ->cbp == 0 && (currSlice->slice_type == P_SLICE ||
      ...
      if (edge || filterLeftMbEdgeFlag) {
        ...
        if (Strength[0] != 0 || Strength[1] != 0 || Strength[2] != 0 ||
            Strength[3] !=
                0)  // only if one of the first 4 Strength bytes is != 0
        {
    ...
    for (edge = 0; edge < 4; edge += 2) {
      ...
      if (edge || filterTopMbEdgeFlag) {
        ...
        if (Strength[0] != 0 || Strength[1] != 0 || Strength[2] != 0 ||
            Strength[3] != 0)  // only if one of the 16 Strength bytes is != 0
        {
          ...
          if (active_sps->chroma_format_idc == YUV420 ||
            ...
            if (imgUV != NULL) {
              ...
              edge_loop_chroma_hor(imgUV[1], Strength, MbQ, 0, 1, p);
               ...
               (MbQ->mb_type == BSKIP_DIRECT) &&
               (active_sps->direct_8x8_inference_flag) && (MbQ->cbp == 0)) {
      int edge, edge_cr;
      // Vertical deblocking
      for (edge = 0; edge < 4; edge += 2) {
        if (edge || filterLeftMbEdgeFlag) {
 ...
 *****************************************************************************************
 */
static void perform_db_normal(VideoParameters *p_Vid, StorablePicture *p,
                              int MbQAddr) {
 ...
 *****************************************************************************************
 */
static void get_db_strength_normal(VideoParameters *p_Vid, StorablePicture *p,
                                   int MbQAddr, int *piCnt) {
  Macroblock *MbQ = &(p_Vid->mb_data[MbQAddr]);  // current Mb
  ...
  if (MbQ->DFDisableIdc == 1) {
    ...
    *piCnt = (*piCnt < 0) ? MbQAddr : (*piCnt);
    if (MbQ->luma_transform_size_8x8_flag) {
      ...
      // Vertical deblocking
      if (filterLeftMbEdgeFlag) get_strength_ver(MbQ, 0, mvlimit, p);
      get_strength_ver(MbQ, 2, mvlimit, p);

      // horizontal deblocking
      if (filterTopMbEdgeFlag) get_strength_hor(MbQ, 0, mvlimit, p);
      ...
      if ((currSlice->slice_type == P_SLICE && MbQ->mb_type == PSKIP) ||
          ((MbQ->mb_type == P16x16) && (MbQ->cbp == 0))) {
        // Vertical deblocking
        if (filterLeftMbEdgeFlag) get_strength_ver(MbQ, 0, mvlimit, p);
