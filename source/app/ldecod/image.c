...
static void init_mvc_picture(Slice *currSlice) {
  ...
  if (currSlice->structure == FRAME) {
    for (i = 0; i < (int)p_Dpb->used_size /*size*/; i++) {
      ...
      if ((fs->frame->view_id == 0) &&
          (fs->frame->frame_poc == currSlice->framepoc)) {
        p_pic = fs->frame;
        break;
      }
    }
    ...
    for (i = 0; i < (int)p_Dpb->used_size /*size*/; i++) {
      FrameStore *fs = p_Dpb->fs[i];
      if ((fs->top_field->view_id == 0) &&
          (fs->top_field->top_poc == currSlice->toppoc)) {
        p_pic = fs->top_field;
        break;
      }
    }
    ...
    for (i = 0; i < (int)p_Dpb->used_size /*size*/; i++) {
      FrameStore *fs = p_Dpb->fs[i];
      if ((fs->bottom_field->view_id == 0) &&
          (fs->bottom_field->bottom_poc == currSlice->bottompoc)) {
  ...
  if (!p_pic) {
    ...
    store_proc_picture_in_dpb(currSlice->p_Dpb,
                              clone_storable_picture(p_Vid, p_pic));
  }
}
#endif
...
static void init_picture(VideoParameters *p_Vid, Slice *currSlice,
  ...
  if (p_Vid->recovery_point)
    p_Vid->recovery_frame_num =
        (currSlice->frame_num + p_Vid->recovery_frame_cnt) %
        p_Vid->max_frame_num;

  if (currSlice->idr_flag) p_Vid->recovery_frame_num = currSlice->frame_num;
  ...
  if (p_Vid->recovery_point == 0 &&
    ...
    if (p_Vid->conceal_mode == 0) fill_frame_num_gap(p_Vid, currSlice);
  }
  ...
  dec_picture = p_Vid->dec_picture = alloc_storable_picture(
      p_Vid, currSlice->structure, p_Vid->width, p_Vid->height, p_Vid->width_cr,
      p_Vid->height_cr, 1);
  ...
  switch (currSlice->structure) {
    ...
    default: error("p_Vid->structure not initialized", 235);
  }
  ...
  if ((p_Vid->separate_colour_plane_flag != 0)) {
    for (nplane = 0; nplane < MAX_PLANE; ++nplane) {
      ...
      fast_memset(
          p_Vid->ipredmode_JV[nplane][0], DC_PRED,
          16 * p_Vid->FrameHeightInMbs * p_Vid->PicWidthInMbs * sizeof(char));
...
#pragma omp parallel for
    ...
    for (i = 0; i < (int)p_Vid->PicSizeInMbs; ++i) reset_mbs(currMB++);
#endif
    if (p_Vid->active_pps->constrained_intra_pred_flag) {
      for (i = 0; i < (int)p_Vid->PicSizeInMbs; ++i) {
  ...
  if (dec_picture->frame_cropping_flag) {
    ...
    dec_picture->frame_crop_bottom_offset =
  ...
  if ((p_Vid->separate_colour_plane_flag != 0)) {
    ...
    p_Vid->dec_picture_JV[1] = alloc_storable_picture(
        ...
        p_Vid->height, p_Vid->width_cr, p_Vid->height_cr, 1);
...
static void init_picture_decoding(VideoParameters *p_Vid) {
  ...
  if (p_Vid->iSliceNumOfCurrPic >= MAX_NUM_SLICES) {
    error(
        ...
        "increased value for MAX_NUM_SLICES",
        200);
  }
  ...
  if (p_Vid->pNextPPS->Valid && (int)p_Vid->pNextPPS->pic_parameter_set_id ==
    ...
    memcpy(&tmpPPS, &(p_Vid->PicParSet[pSlice->pic_parameter_set_id]),
  ...
  if (pSlice->idr_flag) p_Vid->number = 0;
...
#if (MVC_EXTENSION_ENABLE)
  if ((pSlice->layer_id > 0) && (pSlice->svc_extension_flag == 0 &&
                                 pSlice->NaluHeaderMVCExt.non_idr_flag == 0)) {
    idr_memory_management(p_Vid->p_Dpb_layer[pSlice->layer_id],
                          p_Vid->dec_picture);
  }
  ...
  for (j = 0; j < p_Vid->iSliceNumOfCurrPic; j++) {
    if (p_Vid->ppSliceList[j]->DFDisableIdc != 1) iDeblockMode = 0;
#if (MVC_EXTENSION_ENABLE)
...
int decode_one_frame(DecoderParams *pDecoder) {
  ...
  if (p_Vid->newframe) {
    if (p_Vid->pNextPPS->Valid) {
      // assert((int) p_Vid->pNextPPS->pic_parameter_set_id ==
  ...
  if ((sizeof(char) == sizeof(imgpel)) &&
    ...
    if (testEndian()) {
      ...
      switch (symbol_size_in_bytes) {
        ...
        default: {
          error(
              "reading only from formats of 8, 16 or 32 bit allowed on big "
              "endian architecture",
              500);
          break;
        }
...
void find_snr(VideoParameters *p_Vid, StorablePicture *p, int *p_ref) {
  ...
  if (rgb_output) lseek(*p_ref, framesize_in_bytes / 3, SEEK_CUR);

  for (k = 0; k < ((p->chroma_format_idc != YUV400) ? 3 : 1); ++k) {
    if (rgb_output && k == 2) lseek(*p_ref, -framesize_in_bytes, SEEK_CUR);
  ...
  if (rgb_output) lseek(*p_ref, framesize_in_bytes * 2 / 3, SEEK_CUR);
  ...
  if ((currSlice->slice_type != I_SLICE) &&
    ...
    if (p_Vid->no_reference_picture ==
      ...
      if (p_Vid->non_conforming_stream)
        ...
        error(
            "RefPicList0[ num_ref_idx_l0_active_minus1 ] is equal to 'no "
  ...
  if (currSlice->slice_type == B_SLICE) {
    ...
    if (p_Vid->no_reference_picture ==
      ...
      if (p_Vid->non_conforming_stream)
        ...
        error(
            "RefPicList1[ num_ref_idx_l1_active_minus1 ] is equal to 'no "
            "reference picture', invalid bitstream",
            500);
    }
    // that's a definition
...
int read_new_slice(Slice *currSlice) {
  ...
  for (;;) {
...
#endif
    if (!pending_nalu) {
      if (0 == read_next_nalu(p_Vid, nalu)) return EOS;
    } else {
      nalu = pending_nalu;
      pending_nalu = NULL;
    }
    ...
    if (p_Inp->DecodeAllLayers == 1 &&
      ...
      currSlice->svc_extension_flag =
          read_u_1("svc_extension_flag", currStream, &p_Dec->UsedBits);
      ...
      if (nalu->nal_unit_type == NALU_TYPE_SLC_EXT) {
        if (currSlice->svc_extension_flag) {
          ...
          nalu->nal_unit_type =
              NALU_TYPE_SLICE;  // currSlice->NaluHeaderMVCExt.non_idr_flag==0?
                                // NALU_TYPE_IDR: NALU_TYPE_SLICE;
        }
      }
    }
#endif
  ...
  process_nalu:
    switch (nalu->nal_unit_type) {
      ...
      case NALU_TYPE_IDR:
        ...
        if (p_Vid->recovery_point_found == 0) break;
        ...
        if (currSlice->svc_extension_flag ==
          ...
          //}
        } else if (currSlice->svc_extension_flag ==
                   -1)  // SVC and the normal AVC;
        {
          if (p_Vid->active_subset_sps == NULL) {
            currSlice->view_id =
                GetBaseViewId(p_Vid, &p_Vid->active_subset_sps);
        ...
        currSlice->layer_id = currSlice->view_id =
            GetVOIdx(p_Vid, currSlice->view_id);
#endif
        ...
        currSlice->Transform8x8Mode =
            p_Vid->active_pps->transform_8x8_mode_flag;
        currSlice->chroma444_not_separate =
        ...
        // From here on, p_Vid->active_sps, p_Vid->active_pps and the slice
        // header are valid
        if (currSlice->mb_aff_frame_flag)
      ...
      case NALU_TYPE_DPA:
        ...
        slice_id_a =
            read_ue_v("NALU: DP_A slice_id", currStream, &p_Dec->UsedBits);

        if (p_Vid->active_pps->entropy_coding_mode_flag)
          error("received data partition with CABAC, this is not allowed", 500);

        // continue with reading next DP
        if (0 == read_next_nalu(p_Vid, nalu)) return current_header;
        ...
        if (NALU_TYPE_DPB == nalu->nal_unit_type) {
          ...
          slice_id_b =
              read_ue_v("NALU: DP_B slice_id", currStream, &p_Dec->UsedBits);

          currSlice->dpB_NotPresent = 0;

          if ((slice_id_b != slice_id_a) || (nalu->lost_packets)) {
            printf(
                "Waning: got a data partition B which does not match DP_A (DP "
                "loss!)\n");
            currSlice->dpB_NotPresent = 1;
            currSlice->dpC_NotPresent = 1;
          } else {
        ...
        if (NALU_TYPE_DPC == nalu->nal_unit_type) {
          ...
          slice_id_c =
              read_ue_v("NALU: DP_C slice_id", currStream, &p_Dec->UsedBits);
          if ((slice_id_c != slice_id_a) || (nalu->lost_packets)) {
            printf(
                "Warning: got a data partition C which does not match DP_A(DP "
                "loss!)\n");
            // currSlice->dpB_NotPresent =1;
            currSlice->dpC_NotPresent = 1;
          }
      ...
      case NALU_TYPE_EOSTREAM:
        //        printf ("read_new_slice: Found 'End of Stream' NAL unit, len
        //        %d, ignored\n", nalu->len);
        break;
      case NALU_TYPE_FILL:
        if (p_Inp->silent == FALSE) {
          // printf ("read_new_slice: Found NALU_TYPE_FILL, len %d\n", (int)
          // nalu->len); printf ("Skipping these filling bits, proceeding w/
          // next NALU\n");
        }
        break;
#if (MVC_EXTENSION_ENABLE)
      ...
      case NALU_TYPE_PREFIX:
        ...
        if (currSlice->svc_extension_flag == 1) prefix_nal_unit_svc();
        break;
      ...
      default: {
        if (p_Inp->silent == FALSE)
          printf(
              "Found NALU type %d, len %d undefined, ignore NALU, moving on\n",
...
#else
  int i;
  for (i = -iPadX; i < 0; i++) pImgBuf[i] = *pImgBuf;
  for (i = 0; i < iPadX; i++) pImgBuf[i + iWidth] = *(pImgBuf + iWidth - 1);

  for (j = -iPadY; j < 0; j++)
    memcpy(pLine0 + j * iStride, pLine0, iStride * sizeof(imgpel));
  for (j = 1; j < iHeight; j++) {
    pLine = pLine0 + j * iStride;
    for (i = 0; i < iPadX; i++) pLine[i] = pLine[iPadX];
    pLine += iPadX + iWidth - 1;
    for (i = 1; i < iPadX + 1; i++) pLine[i] = *pLine;
  }
...
void exit_picture(VideoParameters *p_Vid, StorablePicture **dec_picture) {
  ...
  if ((*dec_picture)->mb_aff_frame_flag) MbAffPostProc(p_Vid);
...
#else
  if ((*dec_picture)->used_for_reference) pad_dec_picture(p_Vid, *dec_picture);
#endif
...
#if MVC_EXTENSION_ENABLE
  store_picture_in_dpb(p_Vid->p_Dpb_layer[(*dec_picture)->view_id],
                       *dec_picture);
#else
...
int GetVOIdx(VideoParameters *p_Vid, int iViewId) {
  ...
  if (p_Vid->active_subset_sps) {
    ...
    for (iVOIdx = p_Vid->active_subset_sps->num_views_minus1; iVOIdx >= 0;
         iVOIdx--)
      if (piViewIdMap[iVOIdx] == iViewId) break;
  } else {
