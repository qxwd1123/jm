                            ...
                            int joff)            //!< index to 4x4 block
{
  ...
  if (currMB->ipmode_DPCM == VERT_PRED) {
    ...
    for (j = joff; j < joff + BLOCK_SIZE; ++j)
      for (i = ioff; i < ioff + BLOCK_SIZE; ++i) mb_rres[j][i] = cof[j][i];
  }
                              ...
                              ColorPlane pl)       //!< used color plane
{
  ...
  if (currMB->ipmode_DPCM == VERT_PRED_16) {
    ...
    for (i = 0; i < MB_BLOCK_SIZE; ++i) {
      for (j = 0; j < MB_BLOCK_SIZE; j++) mb_rres[j][i] = temp[j][i];
    }
    ...
    for (j = 0; j < MB_BLOCK_SIZE; ++j) {
      for (i = 0; i < MB_BLOCK_SIZE; ++i) mb_rres[j][i] = temp[j][i];
    }
  } else {
    for (j = 0; j < MB_BLOCK_SIZE; ++j)
      for (i = 0; i < MB_BLOCK_SIZE; ++i) mb_rres[j][i] = cof[j][i];
  }
...
void Inv_Residual_trans_Chroma(Macroblock *currMB, int uv) {
  ...
  if (currMB->c_ipred_mode == VERT_PRED_8) {
    for (i = 0; i < width; i++) {
      temp[0][i] = cof[0][i];
      for (j = 1; j < height; j++) temp[j][i] = temp[j - 1][i] + cof[j][i];
    }
    for (i = 0; i < width; i++) {
      for (j = 0; j < height; j++) mb_rres[j][i] = temp[j][i];
    }
  } else  // HOR_PRED_8
  {
    for (i = 0; i < height; i++) {
      temp[i][0] = cof[i][0];
      for (j = 1; j < width; j++) temp[i][j] = temp[i][j - 1] + cof[i][j];
    }
    for (i = 0; i < height; i++) {
      for (j = 0; j < width; j++) mb_rres[i][j] = temp[i][j];
    }
  }
}
      ...
      for (n1 = 0; n1 < p_Vid->mb_cr_size_x; n1 += BLOCK_SIZE) {
        for (j = 0; j < BLOCK_SIZE; ++j) {
          for (i = 0; i < BLOCK_SIZE; ++i) {
            ...
            ilev = rshift_rnd_sf(
                iabs(PBlock[n2 + j][n1 + i]) * quant_coef[qp_rem_sp][j][i],
                q_bits_sp);
...
void iTransform(Macroblock *currMB, ColorPlane pl, int smb) {
  ...
  if (smb) currSlice->is_reset_coeff = FALSE;
