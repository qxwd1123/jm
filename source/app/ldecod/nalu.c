...
int read_next_nalu(VideoParameters *p_Vid, NALU_t *nalu) {
  ...
  switch (p_Inp->FileFormat) {
    ...
    case PAR_OF_ANNEXB:
      ...
      break;
    case PAR_OF_RTP: ret = GetRTPNALU(p_Vid, nalu, p_Vid->BitStreamFile); break;
  }
  ...
  if (ret < 0) error("Invalid startcode emulation prevention found.", 602);
...
void CheckZeroByteNonVCL(VideoParameters *p_Vid, NALU_t *nalu) {
  ...
  if (nalu->nal_unit_type >= 1 && nalu->nal_unit_type <= 5) return;
  ...
  if (nalu->nal_unit_type == NALU_TYPE_AUD ||
    ...
    if (p_Vid->LastAccessUnitExists) {
      p_Vid->LastAccessUnitExists =
          0;  // deliver the last access unit to decoder
      p_Vid->NALUCount = 0;
    }
  }
  p_Vid->NALUCount++;
  // for the first NAL unit in an access unit, zero_byte shall exists
  if (p_Vid->NALUCount == 1) CheckZeroByte = 1;
...
void nal_unit_header_mvc_extension(NALUnitHeaderMVCExt_t *NaluHeaderMVCExt,
                                   Bitstream *s) {
  // to be implemented;
  NaluHeaderMVCExt->non_idr_flag =
      read_u_v(1, "non_idr_flag", s, &p_Dec->UsedBits);
  NaluHeaderMVCExt->priority_id =
      read_u_v(6, "priority_id", s, &p_Dec->UsedBits);
  NaluHeaderMVCExt->view_id = read_u_v(10, "view_id", s, &p_Dec->UsedBits);
  NaluHeaderMVCExt->temporal_id =
      read_u_v(3, "temporal_id", s, &p_Dec->UsedBits);
  NaluHeaderMVCExt->anchor_pic_flag =
      read_u_v(1, "anchor_pic_flag", s, &p_Dec->UsedBits);
  NaluHeaderMVCExt->inter_view_flag =
      read_u_v(1, "inter_view_flag", s, &p_Dec->UsedBits);
  NaluHeaderMVCExt->reserved_one_bit =
      read_u_v(1, "reserved_one_bit", s, &p_Dec->UsedBits);
