...
typedef struct macroblock_dec {
  ...
  void (*itrans_4x4)(struct macroblock_dec *currMB, ColorPlane pl, int ioff,
                     int joff);
  void (*itrans_8x8)(struct macroblock_dec *currMB, ColorPlane pl, int ioff,
                     int joff);
  ...
  char (*readRefPictureIdx)(struct macroblock_dec *currMB,
                            struct syntaxelement_dec *currSE,
                            struct datapartition_dec *dP, char b8mode,
                            int list);
  ...
  void (*read_comp_coeff_4x4_CAVLC)(struct macroblock_dec *currMB,
                                    ColorPlane pl, int (*InvLevelScale4x4)[4],
                                    int qp_per, int cbp, byte **nzcoeff);
  void (*read_comp_coeff_8x8_CAVLC)(struct macroblock_dec *currMB,
                                    ColorPlane pl, int (*InvLevelScale8x8)[8],
                                    int qp_per, int cbp, byte **nzcoeff);
} Macroblock;
...
#define TRACESTRING_SIZE 100           //!< size of trace string
  ...
  //! element type
  void (*reading)(struct macroblock_dec *currMB, struct syntaxelement_dec *,
...
typedef struct datapartition_dec {
  ...
  int (*readSyntaxElement)(struct macroblock_dec *currMB,
                           struct syntaxelement_dec *,
                           struct datapartition_dec *);
  /*!< virtual function;
...
typedef struct slice {
  ...
  Boolean chroma444_not_separate;  //!< indicates chroma 4:4:4 coding with
                                   //!< separate_colour_plane_flag equal to zero
  ...
  int (*intra_pred_4x4)(Macroblock *currMB, ColorPlane pl, int ioff, int joff,
                        int i4, int j4);
...
typedef struct coding_par {
  ...
  int max_vmv_r;  //!< maximum vertical motion vector range in luma quarter
                  //!< frame pixel units for the current level_idc
...
typedef struct video_par {
  ...
  void (*GetStrengthVer)(Macroblock *MbQ, int edge, int mvlimit,
                         struct storable_picture *p);
  void (*GetStrengthHor)(Macroblock *MbQ, int edge, int mvlimit,
                         struct storable_picture *p);
  void (*EdgeLoopLumaVer)(ColorPlane pl, imgpel **Img, byte *Strength,
                          Macroblock *MbQ, int edge);
  void (*EdgeLoopLumaHor)(ColorPlane pl, imgpel **Img, byte *Strength,
                          Macroblock *MbQ, int edge,
                          struct storable_picture *p);
  void (*EdgeLoopChromaVer)(imgpel **Img, byte *Strength, Macroblock *MbQ,
                            int edge, int uv, struct storable_picture *p);
  void (*EdgeLoopChromaHor)(imgpel **Img, byte *Strength, Macroblock *MbQ,
                            int edge, int uv, struct storable_picture *p);
  void (*img2buf)(imgpel **imgX, unsigned char *buf, int size_x, int size_y,
                  int symbol_size_in_bytes, int crop_left, int crop_right,
                  int crop_top, int crop_bottom, int iOutStride);
...
typedef struct decoder_params {
  ...
  int UsedBits;      // for internal statistics, is adjusted by read_se_v,
                     // read_ue_v, read_u_1
...
#if (MVC_EXTENSION_ENABLE)
extern void nal_unit_header_mvc_extension(
    NALUnitHeaderMVCExt_t *NaluHeaderMVCExt, struct bit_stream_dec *bitstream);
#endif

extern void FreeDecPicList(DecodedPicList *pDecPicList);
extern void ClearDecPicList(VideoParameters *p_Vid);
extern DecodedPicList *get_one_avail_dec_pic_from_list(
    DecodedPicList *pDecPicList, int b3D, int view_id);
...
extern void set_global_coding_par(VideoParameters *p_Vid,
...
static inline int is_HI_intra_only_profile(unsigned int profile_idc,
