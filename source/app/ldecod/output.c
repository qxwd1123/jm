  ...
  if ((crop_top || crop_bottom || crop_left || crop_right) || (size != 1)) {
...
#else
    {
      ...
      for (j = 0; j < size_y; j++) {
        ...
        for (i = 0; i < size_x; i++) *(pDst++) = (unsigned char)*(cur_pixel++);
      }
    }
#endif
  }
...
static void img2buf_endian(imgpel **imgX, unsigned char *buf, int size_x,
  ...
  switch (symbol_size_in_bytes) {
    ...
    default: {
      error(
          "writing only to formats of 8, 16 or 32 bit allowed on big endian "
          "architecture",
          500);
      break;
    }
...
void write_picture(VideoParameters *p_Vid, StorablePicture *p, int p_out,
  ...
  if (p_Vid->pending_output_state == FRAME) {
    ...
    if ((p_Vid->pending_output->size_x != p->size_x) ||
        ...
        (p_Vid->pending_output->frame_cropping_flag &&
         ((p_Vid->pending_output->frame_crop_left_offset !=
           p->frame_crop_left_offset) ||
          (p_Vid->pending_output->frame_crop_right_offset !=
           p->frame_crop_right_offset) ||
          (p_Vid->pending_output->frame_crop_top_offset !=
...
static void allocate_p_dec_pic(VideoParameters *p_Vid, DecodedPicList *pDecPic,
                               StorablePicture *p, int iLumaSize,
                               int iFrameSize, int iLumaSizeX, int iLumaSizeY,
                               int iChromaSizeX, int iChromaSizeY) {
  int symbol_size_in_bytes = ((p_Vid->pic_unit_bitsize_on_disk + 7) >> 3);

  if (pDecPic->pY) mem_free(pDecPic->pY);
...
static void write_out_picture(VideoParameters *p_Vid, StorablePicture *p,
  ...
  int symbol_size_in_bytes = ((p_Vid->pic_unit_bitsize_on_disk + 7) >> 3);
  int rgb_output =
      p_Vid->p_EncodePar[p->layer_id]
          ->rgb_output;  //(p_Vid->active_sps->vui_seq_parameters.matrix_coefficients==0);
  unsigned char *buf;
  ...
  iChromaSizeX =
      p->size_x_cr - p->frame_crop_left_offset - p->frame_crop_right_offset;
  iChromaSizeY = p->size_y_cr -
  ...
  if ((pDecPic->pY == NULL) || (pDecPic->iBufSize < iFrameSize))
    allocate_p_dec_pic(p_Vid, pDecPic, p, iLumaSize, iFrameSize, iLumaSizeX,
                       iLumaSizeY, iChromaSizeX, iChromaSizeY);
...
void write_stored_frame(VideoParameters *p_Vid, FrameStore *fs, int p_out) {
  ...
  if (fs->is_used < 3) {
    ...
    if (fs->recovery_frame) p_Vid->recovery_flag = 1;
...
void direct_output(VideoParameters *p_Vid, StorablePicture *p, int p_out) {
  ...
  if (p->structure == FRAME) {
    ...
    if (-1 != p_Vid->p_ref && !p_Inp->silent) find_snr(p_Vid, p, &p_Vid->p_ref);
    free_storable_picture(p);
    return;
  }

  if (p->structure == TOP_FIELD) {
    if (p_Vid->out_buffer->is_used & 1) flush_direct_output(p_Vid, p_out);
    p_Vid->out_buffer->top_field = p;
    p_Vid->out_buffer->is_used |= 1;
  }

  if (p->structure == BOTTOM_FIELD) {
    if (p_Vid->out_buffer->is_used & 2) flush_direct_output(p_Vid, p_out);
