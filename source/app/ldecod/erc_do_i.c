...
int ercCollectColumnBlocks(int predBlocks[], int currRow, int currColumn,
                           char *condition, int maxRow, int maxColumn,
                           int step) {
...
static void concealBlocks(VideoParameters *p_Vid, int lastColumn, int lastRow,
  ...
  int row, column, thr = ERC_BLOCK_CORRUPTED, lastCorruptedRow = -1,
                   firstCorruptedRow = -1, currRow = 0, areaHeight = 0, i = 0,
                   smoothColumn = 0;
      ...
      if (condition[row * lastColumn + column] <= thr) {
        ...
        if (lastCorruptedRow >= lastRow) {
          ...
          for (currRow = firstCorruptedRow; currRow < lastRow;
               currRow += step) {
            // srcCounter = ercCollect8PredBlocks( predBlocks, currRow, column,
            // condition, lastRow, lastColumn, step, 1 );
            ercCollect8PredBlocks(predBlocks, currRow, column, condition,
                                  lastRow, lastColumn, step, 1);

            switch (comp) {
              case 0:
                ercPixConcealIMB(p_Vid, recfr->yptr, currRow, column,
                                 predBlocks, picSizeX, 2);
                break;
              case 1:
                ercPixConcealIMB(p_Vid, recfr->uptr, currRow, column,
                                 predBlocks, (picSizeX >> 1), 1);
                break;
              case 2:
                ercPixConcealIMB(p_Vid, recfr->vptr, currRow, column,
                                 predBlocks, (picSizeX >> 1), 1);
                break;
            }
          ...
          for (i = 0; i < areaHeight; i += step) {
            ...
            if (smoothColumn > 0) {
              // srcCounter = ercCollectColumnBlocks( predBlocks, currRow,
              // column, condition, lastRow, lastColumn, step );
              ercCollectColumnBlocks(predBlocks, currRow, column, condition,
                                     lastRow, lastColumn, step);
            } else {
              // srcCounter = ercCollect8PredBlocks( predBlocks, currRow,
              // column, condition, lastRow, lastColumn, step, 1 );
              ercCollect8PredBlocks(predBlocks, currRow, column, condition,
                                    lastRow, lastColumn, step, 1);
            }
