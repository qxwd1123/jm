...
int EBSPtoRBSP(byte *streamBuffer, int end_bytepos, int begin_bytepos) {
  ...
  if (end_bytepos < begin_bytepos) return end_bytepos;
  ...
  for (i = begin_bytepos; i < end_bytepos;
    ...
    if (count == ZEROBYTES_SHORTSTARTCODE && streamBuffer[i] < 0x03) return -1;
    if (count == ZEROBYTES_SHORTSTARTCODE && streamBuffer[i] == 0x03) {
      ...
      if ((i < end_bytepos - 1) && (streamBuffer[i + 1] > 0x03)) return -1;
