                                   ...
                                   int joff)  //!< pixel offset Y within MB
{
  ...
  if (block_available_up) {
    memcpy(&PredPel[1], &imgY[pix_b.pos_y][pix_b.pos_x],
           BLOCK_SIZE_8x8 * sizeof(imgpel));
  } else {
#if (IMGTYPE == 0)
    memset(&PredPel[1], p_Vid->dc_pred_value_comp[pl],
           BLOCK_SIZE_8x8 * sizeof(imgpel));
#else
  ...
  if (block_available_up_right) {
    memcpy(&PredPel[9], &imgY[pix_c.pos_y][pix_c.pos_x],
           BLOCK_SIZE_8x8 * sizeof(imgpel));
  } else {
#if (IMGTYPE == 0)
  ...
  for (i = ioff; i < ioff + BLOCK_SIZE_8x8; i++) mpr[joff][i] = (imgpel)s0;
 ...
 ***********************************************************************
 */
static inline int intra8x8_vert_pred(
 ...
 ***********************************************************************
 */
static inline int intra8x8_diag_down_right_pred(
    Macroblock *currMB,  //!< current macroblock
    ColorPlane pl,       //!< current image plane
    int ioff,            //!< pixel offset X within MB
    int joff)            //!< pixel offset Y within MB
{
  ...
  if ((!block_available_up) || (!block_available_left) ||
      (!block_available_up_left))
    printf(
        "warning: Intra_8x8_Diagonal_Down_Right prediction mode not allowed at "
        "mb %d\n",
        (int)currSlice->current_mb_nr);
 ...
 ***********************************************************************
 */
static inline int intra8x8_diag_down_left_pred(
    Macroblock *currMB,  //!< current macroblock
    ColorPlane pl,       //!< current image plane
    int ioff,            //!< pixel offset X within MB
    int joff)            //!< pixel offset Y within MB
{
  ...
  if (!block_available_up)
    printf(
        "warning: Intra_8x8_Diagonal_Down_Left prediction mode not allowed at "
        "mb %d\n",
        (int)currSlice->current_mb_nr);
 ...
 ***********************************************************************
 */
static inline int intra8x8_vert_right_pred(
    Macroblock *currMB,  //!< current macroblock
    ColorPlane pl,       //!< current image plane
    int ioff,            //!< pixel offset X within MB
    int joff)            //!< pixel offset Y within MB
{
  ...
  if ((!block_available_up) || (!block_available_left) ||
      (!block_available_up_left))
    printf(
        "warning: Intra_8x8_Vertical_Right prediction mode not allowed at mb "
        "%d\n",
        (int)currSlice->current_mb_nr);
 ...
 ***********************************************************************
 */
static inline int intra8x8_vert_left_pred(
    Macroblock *currMB,  //!< current macroblock
    ColorPlane pl,       //!< current image plane
    int ioff,            //!< pixel offset X within MB
    int joff)            //!< pixel offset Y within MB
{
  ...
  if (!block_available_up)
    printf(
        "warning: Intra_4x4_Vertical_Left prediction mode not allowed at mb "
        "%d\n",
        (int)currSlice->current_mb_nr);
 ...
 ***********************************************************************
 */
static inline int intra8x8_hor_up_pred(
    Macroblock *currMB,  //!< current macroblock
    ColorPlane pl,       //!< current image plane
    int ioff,            //!< pixel offset X within MB
    int joff)            //!< pixel offset Y within MB
{
  ...
  if (!block_available_left)
    printf(
        "warning: Intra_8x8_Horizontal_Up prediction mode not allowed at mb "
        "%d\n",
        (int)currSlice->current_mb_nr);
 ...
 ***********************************************************************
 */
static inline int intra8x8_hor_down_pred(
    Macroblock *currMB,  //!< current macroblock
    ColorPlane pl,       //!< current image plane
    int ioff,            //!< pixel offset X within MB
    int joff)            //!< pixel offset Y within MB
{
  ...
  if ((!block_available_up) || (!block_available_left) ||
      (!block_available_up_left))
    printf(
        "warning: Intra_8x8_Horizontal_Down prediction mode not allowed at mb "
        "%d\n",
        (int)currSlice->current_mb_nr);
...

{
  ...
  switch (predmode) {
    case DC_PRED: return (intra8x8_dc_pred(currMB, pl, ioff, joff)); break;
    case VERT_PRED: return (intra8x8_vert_pred(currMB, pl, ioff, joff)); break;
    case HOR_PRED: return (intra8x8_hor_pred(currMB, pl, ioff, joff)); break;
    ...
    case HOR_UP_PRED:
      ...
      break;
    case HOR_DOWN_PRED: return (intra8x8_hor_down_pred(currMB, pl, ioff, joff));
    default:
