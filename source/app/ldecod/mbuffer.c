...
int getDpbSize(VideoParameters *p_Vid, seq_parameter_set_rbsp_t *active_sps) {
  ...
  if (active_sps->vui_parameters_present_flag &&
    ...
    if (size_vui < size) {
      printf(
          "Warning: max_dec_frame_buffering(%d) is less than DPB size(%d) "
...
void init_dpb(VideoParameters *p_Vid, DecodedPictureBuffer *p_Dpb, int type) {
  ...
  p_Dpb->size =
      getDpbSize(p_Vid, active_sps) + p_Vid->p_Inp->dpb_plus[type == 2 ? 1 : 0];
  ...
  if (type == 2) {
    ...
    // given that this is in a different buffer, do we even need proc_flag
    // anymore?
  } else
...
void re_init_dpb(VideoParameters *p_Vid, DecodedPictureBuffer *p_Dpb,
  ...
  if (iDpbSize > (int)p_Dpb->size) {
    ...
    if (NULL == p_Dpb->fs) no_mem_exit("re_init_dpb: p_Dpb->fs");

    p_Dpb->fs_ref = realloc(p_Dpb->fs_ref, iDpbSize * sizeof(FrameStore *));
    if (NULL == p_Dpb->fs_ref) no_mem_exit("re_init_dpb: p_Dpb->fs_ref");

    p_Dpb->fs_ltref = realloc(p_Dpb->fs_ltref, iDpbSize * sizeof(FrameStore *));
    if (NULL == p_Dpb->fs_ltref) no_mem_exit("re_init_dpb: p_Dpb->fs_ltref");
...
StorablePicture *alloc_storable_picture(VideoParameters *p_Vid,
  ...
  get_mem2Dpel_pad(&(s->imgY), size_y, size_x, p_Vid->iLumaPadY,
                   p_Vid->iLumaPadX);
  s->iLumaStride = size_x + 2 * p_Vid->iLumaPadX;
  s->iLumaExpandedHeight = size_y + 2 * p_Vid->iLumaPadY;

  if (active_sps->chroma_format_idc != YUV400) {
    get_mem3Dpel_pad(&(s->imgUV), 2, size_y_cr, size_x_cr, p_Vid->iChromaPadY,
                     p_Vid->iChromaPadX);
  }
  ...
  if (!p_Vid->active_sps->frame_mbs_only_flag && structure != FRAME) {
    ...
    for (j = 0; j < MAX_NUM_SLICES; j++) {
      for (i = 0; i < 2; i++) {
        s->listX[j][i] = calloc(
            MAX_LIST_SIZE, sizeof(StorablePicture *));  // +1 for reordering
        if (NULL == s->listX[j][i])
...
void free_storable_picture(StorablePicture *p) {
  ...
  if (p) {
    ...
    if (p->seiHasTone_mapping) free(p->tone_mapping_lut);
  ...
  if ((p_Vid->profile_idc == MVC_HIGH || p_Vid->profile_idc == STEREO_HIGH) &&
    ...
    if (currSlice->listXsize[0] > 0) {
      ...
      printf(" ** (CurViewID:%d %d) %s Ref Pic List 0 ****\n",
             currSlice->view_id, currSlice->ThisPOC,
...
void init_lists_b_slice(Slice *currSlice) {
...

  {
    ...
    if (currSlice->structure == FRAME) {
      ...
      // printf("listX[0] (PicNum): "); for (i=0; i<currSlice->listXsize[0];
      // i++){printf ("%d  ", currSlice->listX[0][i]->pic_num);} printf("\n");
      // printf("listX[1] (PicNum): "); for (i=0; i<currSlice->listXsize[1];
      // i++){printf ("%d  ", currSlice->listX[1][i]->pic_num);} printf("\n");
      // printf("currSlice->listX[0] currPoc=%d (Poc): ", p_Vid->framepoc); for
      // (i=0; i<currSlice->listXsize[0]; i++){printf ("%d  ",
      // currSlice->listX[0][i]->poc);} printf("\n");
      // printf("currSlice->listX[1] currPoc=%d (Poc): ", p_Vid->framepoc); for
      // (i=0; i<currSlice->listXsize[1]; i++){printf ("%d  ",
      ...
      // printf("fs_list0 currPoc=%d (Poc): ", currSlice->ThisPOC); for (i=0;
      // i<list0idx; i++){printf ("%d  ", fs_list0[i]->poc);} printf("\n");
      // printf("fs_list1 currPoc=%d (Poc): ", currSlice->ThisPOC); for (i=0;
      // i<list0idx; i++){printf ("%d  ", fs_list1[i]->poc);} printf("\n");
...
static void sliding_window_memory_management(DecodedPictureBuffer *p_Dpb,
  ...
  if (p_Dpb->ref_frames_in_buffer ==
      imax(1, p_Dpb->num_ref_frames) - p_Dpb->ltref_frames_in_buffer) {
    for (i = 0; i < p_Dpb->used_size; i++) {
...
void store_picture_in_dpb(DecodedPictureBuffer *p_Dpb, StorablePicture *p) {
  ...
  if (p_Vid->conceal_mode != 0) {
    for (i = 0; i < p_Dpb->size; i++)
      if (p_Dpb->fs[i]->is_reference) p_Dpb->fs[i]->concealment_reference = 1;
  }

  // first try to remove unused frames
  if (p_Dpb->used_size == p_Dpb->size) {
    // picture error concealment
    if (p_Vid->conceal_mode != 0) conceal_non_ref_pics(p_Dpb, 2);

    remove_unused_frame_from_dpb(p_Dpb);

    if (p_Vid->conceal_mode != 0) sliding_window_poc_management(p_Dpb, p);
  }
...
void flush_dpb(DecodedPictureBuffer *p_Dpb) {
  ...
  if (!p_Dpb->init_done) return;
  //  if(p_Vid->conceal_mode == 0)
  if (p_Vid->conceal_mode != 0) conceal_non_ref_pics(p_Dpb, 0);
...
void flush_dpbs(DecodedPictureBuffer **p_Dpb_layers, int nLayers) {
  ...
  while (used_size) {
    for (j = 0; j < nLayers; j++) {
      p_Dpb = p_Dpb_layers[j];
      if (p_Dpb->used_size) output_one_frame_from_dpb(p_Dpb);
    }
    used_size--;
  }
  for (j = 0; j < nLayers; j++) {
          ...
          if (frm_motion->mb_field[currentmb]) {
            ...
            if (fs_btm->mv_info[j][i].ref_idx[LIST_0] >= 0)
              fs_btm->mv_info[j][i].ref_pic[LIST_0] =
                  p_Vid->ppSliceList[frame->mv_info[jj4][i].slice_no]
            ...
            if (fs_top->mv_info[j][i].ref_idx[LIST_0] >= 0)
              fs_top->mv_info[j][i].ref_pic[LIST_0] =
                  p_Vid->ppSliceList[frame->mv_info[jj][i].slice_no]
      ...
      for (i = 0; i < (frame->size_x >> 2); i++) {
        ...
        if (!frame->mb_aff_frame_flag || !frame->motion.mb_field[currentmb]) {
          ...
          if (frame->mv_info[jj][ii].ref_idx[LIST_0] == -1) {
            ...
            fs_top->mv_info[j][i].ref_pic[LIST_0] =
                fs_btm->mv_info[j][i].ref_pic[LIST_0] =
                    p_Vid->ppSliceList[frame->mv_info[jj][ii].slice_no]
                        ->listX[LIST_0]
...
void dpb_combine_field_yuv(VideoParameters *p_Vid, FrameStore *fs) {
  ...
  if (!fs->frame) {
    fs->frame = alloc_storable_picture(
        ...
        fs->top_field->size_x_cr, fs->top_field->size_y_cr * 2, 1);
  }
  ...
  for (j = 0; j < (fs->top_field->size_y >> 2); j++) {
    ...
    for (i = 0; i < (fs->top_field->size_x >> 2); i++) {
      ...
      fs->frame->mv_info[jj4][i].ref_pic[LIST_0] =
          k >= 0 ? fs->bottom_field->listX[l][LIST_0][k] : NULL;
      k = fs->bottom_field->mv_info[j][i].ref_idx[LIST_1];
      fs->frame->mv_info[jj4][i].ref_pic[LIST_1] =
          k >= 0 ? fs->bottom_field->listX[l][LIST_1][k] : NULL;
    }
  }
}
    ...
    for (j = 0; j < 2 + (currSlice->mb_aff_frame_flag * 4); j += 2) {
      for (i = 0; i < currSlice->listXsize[j]; i++) {
        ...
        if (j == 0) {
          ...
          iTRb = iClip3(
              -128, 127,
              p_Vid->dec_picture->bottom_poc - listX[LIST_0 + j][i]->poc);
        }
...
static int is_view_id_in_ref_view_list(int view_id, int *ref_view_id,
  ...
  for (i = 0; i < num_ref_views; i++) {
    if (view_id == ref_view_id[i]) break;
  }
...
void append_interview_list(
  ...
  if (iVOIdx < 0) printf("Error: iVOIdx: %d is not less than 0\n", iVOIdx);
...
int init_img_data(VideoParameters *p_Vid, ImageData *p_ImgData,
  ...
  p_ImgData->top_stride[0] = p_ImgData->bot_stride[0] = p_ImgData->frm_stride[0]
                                                        << 1;
  p_ImgData->top_stride[1] = p_ImgData->top_stride[2] =
      p_ImgData->bot_stride[1] = p_ImgData->bot_stride[2] =
          p_ImgData->frm_stride[1] << 1;

  if (sps->separate_colour_plane_flag) {
    for (nplane = 0; nplane < MAX_PLANE; nplane++) {
      memory_size += get_mem2Dpel(&(p_ImgData->frm_data[nplane]), p_Vid->height,
                                  p_Vid->width);
    }
  } else {
    memory_size +=
        get_mem2Dpel(&(p_ImgData->frm_data[0]), p_Vid->height, p_Vid->width);

    if (p_Vid->yuv_format != YUV400) {
      int i, j, k;
      memory_size += get_mem2Dpel(&(p_ImgData->frm_data[1]), p_Vid->height_cr,
                                  p_Vid->width_cr);
      memory_size += get_mem2Dpel(&(p_ImgData->frm_data[2]), p_Vid->height_cr,
                                  p_Vid->width_cr);

      if (sizeof(imgpel) == sizeof(unsigned char)) {
        for (k = 1; k < 3; k++)
          fast_memset(p_ImgData->frm_data[k][0], 128,
  ...
  if (!p_Vid->active_sps->frame_mbs_only_flag) {
    ...
    memory_size += init_top_bot_planes(p_ImgData->frm_data[0], p_Vid->height,
                                       &(p_ImgData->top_data[0]),
                                       &(p_ImgData->bot_data[0]));

    if (p_Vid->yuv_format != YUV400) {
      memory_size += 4 * (sizeof(imgpel **));

      memory_size += init_top_bot_planes(
          p_ImgData->frm_data[1], p_Vid->height_cr, &(p_ImgData->top_data[1]),
          &(p_ImgData->bot_data[1]));
      memory_size += init_top_bot_planes(
          p_ImgData->frm_data[2], p_Vid->height_cr, &(p_ImgData->top_data[2]),
          &(p_ImgData->bot_data[2]));
    }
  }
...
static inline void copy_img_data(imgpel *out_img, imgpel *in_img, int ostride,
                                 int istride, unsigned int size_y,
                                 unsigned int size_x) {
  unsigned int i;
  for (i = 0; i < size_y; i++) {
 ...
 ************************************************************************
 * \brief
 *    Store a processed picture in DPB. This includes cheking for space in DPB
 *and flushing frames. If we received a frame, we need to check for a new store,
 *if we got a field, check if it's the second field of an already allocated
 *    store.
 *
 * \param p_Vid
 *    VideoParameters
 ...
 */
#if (MVC_EXTENSION_ENABLE)
void store_proc_picture_in_dpb(DecodedPictureBuffer *p_Dpb,
                               StorablePicture *p) {
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  FrameStore *fs = p_Dpb->fs_ilref[0];
 ...
 ************************************************************************
 */
StorablePicture *clone_storable_picture(VideoParameters *p_Vid,
  ...
  StorablePicture *p_stored_pic = alloc_storable_picture(
      p_Vid, (PictureStructure)p_Vid->structure, p_Vid->width, p_Vid->height,
      p_Vid->width_cr, p_Vid->height_cr, 0);
  ...
  p_stored_pic->no_output_of_prior_pics_flag =
      p_pic->no_output_of_prior_pics_flag;
  ...
  // memcpy((void *)p_stored_pic->imgY[0], (void
  // *)p_Vid->tempData3.frm_data[0][0], p_pic->size_x * p_pic->size_y *
  // sizeof(imgpel));
  ...
  copy_img_data(&p_stored_pic->imgY[0][0], &img_in[0][0][0], ostride[0],
                istride[0], p_pic->size_y, p_pic->size_x * sizeof(imgpel));

  pad_buf(*p_stored_pic->imgY, p_stored_pic->size_x, p_stored_pic->size_y,
          p_stored_pic->iLumaStride, p_Vid->iLumaPadX, p_Vid->iLumaPadY);

  if (p_Vid->active_sps->chroma_format_idc != YUV400) {
    // memcpy((void *)p_stored_pic->imgUV[0][0], (void
    // *)p_Vid->tempData3.frm_data[1][0], p_pic->size_x_cr * p_pic->size_y_cr *
    // sizeof(imgpel)); memcpy((void *)p_stored_pic->imgUV[1][0], (void
    // *)p_Vid->tempData3.frm_data[2][0], p_pic->size_x_cr * p_pic->size_y_cr *
    // sizeof(imgpel));
    copy_img_data(&p_stored_pic->imgUV[0][0][0], &img_in[1][0][0], ostride[1],
                  istride[1], p_pic->size_y_cr,
                  p_pic->size_x_cr * sizeof(imgpel));
    pad_buf(*p_stored_pic->imgUV[0], p_stored_pic->size_x_cr,
            p_stored_pic->size_y_cr, p_stored_pic->iChromaStride,
            p_Vid->iChromaPadX, p_Vid->iChromaPadY);
    copy_img_data(&p_stored_pic->imgUV[1][0][0], &img_in[2][0][0], ostride[1],
                  istride[2], p_pic->size_y_cr,
                  p_pic->size_x_cr * sizeof(imgpel));
    pad_buf(*p_stored_pic->imgUV[1], p_stored_pic->size_x_cr,
            p_stored_pic->size_y_cr, p_stored_pic->iChromaStride,
            p_Vid->iChromaPadX, p_Vid->iChromaPadY);
  }
