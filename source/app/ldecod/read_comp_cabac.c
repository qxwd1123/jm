...
************************************************************************
*/
static void read_comp_coeff_4x4_smb_CABAC(Macroblock *currMB,
                                          SyntaxElement *currSE, ColorPlane pl,
  ...
  const byte(*pos_scan4x4)[2] =
      ((currSlice->structure == FRAME) && (!currMB->mb_field)) ? SNGL_SCAN
                                                               : FIELD_SCAN;
...
************************************************************************
*/
static void read_comp_coeff_4x4_CABAC(Macroblock *currMB, SyntaxElement *currSE,
       ...
       block_y += BLOCK_SIZE_8x8) /* all modes */
  {
    ...
    for (block_x = 0; block_x < MB_BLOCK_SIZE; block_x += BLOCK_SIZE_8x8) {
      ...
      {
        read_comp_coeff_4x4_smb_CABAC(currMB, currSE, pl, block_y, block_x,
                                      start_scan, cbp_blk);
...
************************************************************************
*/
static void read_comp_coeff_4x4_CABAC_ls(Macroblock *currMB,
                                         SyntaxElement *currSE, ColorPlane pl,
...
static void readCompCoeff8x8_CABAC(Macroblock *currMB, SyntaxElement *currSE,
      ...
      (1 << b8))  // are there any coefficients in the current block
  {
    ...
    int64 cbp_mask =
        ...
        << (4 * b8 -
            2 * (b8 &
                 0x01));  // corresponds to 110011, as if all four 4x4 blocks
                          // contain coeff, shifted to block position
...
************************************************************************
*/
static void read_comp_coeff_8x8_MB_CABAC(Macroblock *currMB,
                                         SyntaxElement *currSE, ColorPlane pl) {
...
************************************************************************
*/
static void read_comp_coeff_8x8_MB_CABAC_ls(Macroblock *currMB,
                                            SyntaxElement *currSE,
                                            ColorPlane pl) {
  //======= 8x8 transform size & CABAC ========
      ...
      InvLevelScale4x4 =
          intra
              ...
              : currSlice->InvLevelScale4x4_Inter[currSlice->colour_plane_id]
                                                 [qp_rem];
      currMB->read_comp_coeff_4x4_CABAC(currMB, &currSE, PLANE_Y,
                                        InvLevelScale4x4, qp_per, cbp);
    }
  }
        ...
        for (b4 = 0; b4 < 4; ++b4) {
          ...
          for (k = 0; (k < 16) && (level != 0); ++k) {
            ...
            if (level != 0) {
              ...
              cof[(j << 2) + j0][(i << 2) + i0] = rshift_rnd_sf(
  ...
  for (uv = 0; uv < 2; ++uv) {
...

    {
      if (cbp) {
        if (currMB->luma_transform_size_8x8_flag) {
          //======= 8x8 transform size & CABAC ========
          currMB->read_comp_coeff_8x8_CABAC(currMB, &currSE,
                                            (ColorPlane)(PLANE_U + uv));
        } else  // 4x4
        {
          currMB->read_comp_coeff_4x4_CABAC(
              currMB, &currSE, (ColorPlane)(PLANE_U + uv), InvLevelScale4x4,
              qp_per_uv[uv], cbp);
        }
      }
    }
