 ...
 ***********************************************************************
 */
static int intra4x4_diag_down_right_pred(
    ...
    int joff)            //!< pixel offset Y within MB
{
  ...
  if ((!block_available_up) || (!block_available_left) ||
      (!block_available_up_left))
    printf(
        "warning: Intra_4x4_Diagonal_Down_Right prediction mode not allowed at "
        "mb %d\n",
        (int)currSlice->current_mb_nr);
  else {
    imgpel PredPixel[7];
    ...
    int joff)            //!< pixel offset Y within MB
{
  ...
  getNonAffNeighbour(currMB, ioff, joff - 1, p_Vid->mb_size[IS_LUMA], &pix_b);
  getNonAffNeighbour(currMB, ioff + 4, joff - 1, p_Vid->mb_size[IS_LUMA],
                     &pix_c);
  ...
  if (!block_available_up)
    printf(
        "warning: Intra_4x4_Diagonal_Down_Left prediction mode not allowed at "
        "mb %d\n",
        (int)currSlice->current_mb_nr);
  else {
    imgpel **imgY = (pl) ? currSlice->dec_picture->imgUV[pl - 1]
    ...
    if (block_available_up_right) {
      memcpy(&PredPel[5], &imgY[pix_c.pos_y][pix_c.pos_x],
             BLOCK_SIZE * sizeof(imgpel));
    } else {
#if (IMGTYPE == 0)
                                    ...
                                    int joff)  //!< pixel offset Y within MB
{
  ...
  if ((!block_available_up) || (!block_available_left) ||
      (!block_available_up_left))
    printf(
        "warning: Intra_4x4_Vertical_Right prediction mode not allowed at mb "
        "%d\n",
        (int)currSlice->current_mb_nr);
  {
    imgpel **imgY = (pl) ? currSlice->dec_picture->imgUV[pl - 1]
                                ...
                                int joff)  //!< pixel offset Y within MB
{
  ...
  if (!block_available_left)
    printf(
        "warning: Intra_4x4_Horizontal_Up prediction mode not allowed at mb "
        "%d\n",
        (int)currSlice->current_mb_nr);
  else {
    imgpel PredPixel[10];
                                  ...
                                  int joff)  //!< pixel offset Y within MB
{
  ...
  if ((!block_available_up) || (!block_available_left) ||
      (!block_available_up_left))
    printf(
        "warning: Intra_4x4_Horizontal_Down prediction mode not allowed at mb "
        "%d\n",
        (int)currSlice->current_mb_nr);
  else {
    imgpel PredPixel[10];
    ...
    int img_block_y)     //!< location of block Y, multiples of 4
{
  ...
  switch (predmode) {
    case DC_PRED: return (intra4x4_dc_pred(currMB, pl, ioff, joff)); break;
    case VERT_PRED: return (intra4x4_vert_pred(currMB, pl, ioff, joff)); break;
    case HOR_PRED: return (intra4x4_hor_pred(currMB, pl, ioff, joff)); break;
    ...
    case HOR_UP_PRED:
      ...
      break;
    case HOR_DOWN_PRED: return (intra4x4_hor_down_pred(currMB, pl, ioff, joff));
    default:
