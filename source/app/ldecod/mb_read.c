...
static void read_ipred_8x8_modes_mbaff(Macroblock *currMB) {
  ...
  for (b8 = 0; b8 < 4; ++b8)  // loop 8x8 blocks
  {
    ...
    get4x4Neighbour(currMB, (bx << 2) - 1, (by << 2), p_Vid->mb_size[IS_LUMA],
                    &left_block);
    get4x4Neighbour(currMB, (bx << 2), (by << 2) - 1, p_Vid->mb_size[IS_LUMA],
                    &top_block);
    ...
    if (p_Vid->active_pps->constrained_intra_pred_flag) {
      left_block.available =
          left_block.available ? currSlice->intra_block[left_block.mb_addr] : 0;
      top_block.available =
          top_block.available ? currSlice->intra_block[top_block.mb_addr] : 0;
    }
  ...
  for (b8 = 0; b8 < 4; ++b8)  // loop 8x8 blocks
  {
    for (j = 0; j < 2; j++)  // loop subblocks
    {
      ...
      for (i = 0; i < 2; i++) {
        ...
        if (p_Vid->active_pps->constrained_intra_pred_flag) {
          left_block.available =
              left_block.available ? currSlice->intra_block[left_block.mb_addr]
                                   : 0;
          top_block.available = top_block.available
                                    ? currSlice->intra_block[top_block.mb_addr]
                                    : 0;
        }
        ...
        currSlice->ipredmode[bj][bi] =
            (byte)((currSE.value1 == -1)
                       ? mostProbableIntraPredMode
                       : currSE.value1 +
...
static void init_macroblock_direct(Macroblock *currMB) {
  int slice_no = currMB->p_Slice->current_slice_nr;
  PicMotionParams **mv_info =
      &currMB->p_Slice->dec_picture->mv_info[currMB->block_y];
  int i, j;
...
static inline void SetB8Mode(Macroblock *currMB, int value, int i) {
  ...
  static const char b_v2b8[14] = {
    0, 4, 4, 4, 5, 6, 5, 6, 5, 6, 7, 7, 7, IBLOCK
  };
...
static inline void reset_coeffs(Macroblock *currMB) {
  ...
  // CAVLC
  if (p_Vid->active_pps->entropy_coding_mode_flag == (Boolean)CAVLC)
    fast_memset(p_Vid->nz_coeff[currMB->mbAddrX][0][0], 0,
 ...
 ************************************************************************
 */
static void read_intra4x4_macroblock_cavlc(Macroblock *currMB,
                                           const byte *partMap) {
  Slice *currSlice = currMB->p_Slice;
  //============= Transform Size Flag for INTRA MBs =============
  //-------------------------------------------------------------
 ...
 ************************************************************************
 */
static void read_intra4x4_macroblock_cabac(Macroblock *currMB,
                                           const byte *partMap) {
  Slice *currSlice = currMB->p_Slice;
  //============= Transform Size Flag for INTRA MBs =============
  //-------------------------------------------------------------
 ...
 ************************************************************************
 */
static void read_P8x8_macroblock(Macroblock *currMB, DataPartition *dP,
                                 SyntaxElement *currSE) {
  int i;
  Slice *currSlice = currMB->p_Slice;
...
static void read_one_macroblock_i_slice_cabac(Macroblock *currMB) {
  ...
  if (dP->bitstream->ei_flag) currSE.mapping = linfo_ue;
...
static void read_one_macroblock_p_slice_cabac(Macroblock *currMB) {
  ...
  if (currSlice->mb_aff_frame_flag == 0) {
    ...
    // read MB skip_flag
    if (((mb_nr & 0x01) == 0 || prevMbSkipped)) field_flag_inference(currMB);
...
static void read_one_macroblock_b_slice_cavlc(Macroblock *currMB) {
  ...
  if (currMB->mb_type == IPCM) {
    ...
    // init NoMbPartLessThan8x8Flag
    currMB->NoMbPartLessThan8x8Flag =
        (!(currSlice->active_sps->direct_8x8_inference_flag)) ? FALSE : TRUE;
...
static void read_one_macroblock_b_slice_cabac(Macroblock *currMB) {
  ...
  if (currSlice->mb_aff_frame_flag == 0) {
    ...
    if (currSE.value1 == 0 && currSE.value2 == 0) currSlice->cod_counter = 0;
