  ...
  if (errorVar && errorVar->concealment) {
    ...
    if (errorVar->nOfCorruptedSegments) {
      if (chroma_format_idc != YUV400)
        predMB =
            (imgpel *)malloc((256 + (p_Vid->mb_cr_size) * 2) * sizeof(imgpel));
      else
...
static int concealByTrial(frame *recfr, imgpel *predMB, int currMBNum,
  ...
  do { /* 4 blocks loop */
          ...
          if (isBlock(object_list, predMBNum, compSplit1, INTRA) ||
            ...
            for (predSplitted = isSplitted(object_list, predMBNum),
              ...
              if (currDist < minDist || !fInterNeighborExists) {
                ...
                for (k = 0; k < 3; k++) mvBest[k] = mvPred[k];
    ...
    for (i = 0; i < 3; i++) currRegion->mv[i] = mvBest[i];
...
static int edgeDistortion(int predBlocks[], int currYBlockNum, imgpel *predMB,
  ...
  do {
      ...
      if (predBlocks[j] >= threshold) {
        switch (j) {
          ...
          case 5:
            ...
            for (i = 0; i < regionSize; i++) {
              distortion +=
                  iabs((int)(predMB[i * 16] - neighbor[i * picSizeX]));
            }
            break;
          case 6:
    ...
    threshold--;
    if (threshold < ERC_BLOCK_CONCEALED) break;
...
static inline int compare_pic_by_lt_pic_num_asc(const void *arg1,
  ...
  if (long_term_pic_num1 < long_term_pic_num2) return -1;
...
static void CopyImgData(imgpel **inputY, imgpel ***inputUV, imgpel **outputY,
  ...
  for (y = 0; y < img_height; y++)
    for (x = 0; x < img_width; x++) outputY[y][x] = inputY[y][x];
...
static void copy_to_conceal(StorablePicture *src, StorablePicture *dst,
  ...
  if (p_Vid->conceal_mode == 2) {
    ...
    if (p_Vid->conceal_slice_type == B_SLICE) {
      init_lists_for_non_reference_loss(p_Vid->p_Dpb_layer[0], dst->slice_type,
                                        p_Vid->ppSliceList[0]->structure);
    } else
      p_Vid->ppSliceList[0]->init_lists(
          p_Vid->ppSliceList[0]);  // p_Vid->currentSlice);
    ...
    for (i = 0; i < mb_height * 4; i++) {
      ...
      for (j = 0; j < mb_width * 4; j++) {
        ...
        if ((mm % 16 == 0) && (nn % 16 == 0)) current_mb_nr++;
...
void update_ref_list_for_concealment(DecodedPictureBuffer *p_Dpb) {
  ...
  p_Dpb->ref_frames_in_buffer =
      p_Vid->active_pps->num_ref_idx_l0_default_active_minus1;
}
...
void init_lists_for_non_reference_loss(DecodedPictureBuffer *p_Dpb,
      ...
      if (p_Dpb->fs[i]->concealment_reference == 1) {
        if (p_Dpb->fs[i]->frame_num > p_Vid->frame_to_conceal)
          p_Dpb->fs_ref[i]->frame_num_wrap =
              p_Dpb->fs[i]->frame_num - max_frame_num;
        else
  ...
  if (currSliceType == B_SLICE) {
    if (currPicStructure == FRAME) {
      ...
      p_Vid->ppSliceList[0]->listXsize[0] =
          p_Vid->ppSliceList[0]->listXsize[1] = (char)list0idx;
  ...
  // set max size
  p_Vid->ppSliceList[0]->listXsize[0] = (char)imin(
      p_Vid->ppSliceList[0]->listXsize[0], (int)active_sps->num_ref_frames);
  p_Vid->ppSliceList[0]->listXsize[1] = (char)imin(
