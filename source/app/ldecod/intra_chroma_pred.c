...
static void intra_chroma_DC_all(imgpel **curr_img, int up_avail, int left_avail,
  ...
  if (left_avail) {
    imgpel **cur_pel = &(curr_img[left.pos_y + blk_y - 1]);
    int pos_x = left.pos_x;
    for (i = 0; i < 4; ++i) s1 += *((*cur_pel++) + pos_x);
  }
...
void intra_pred_chroma(Macroblock *currMB) {
  switch (currMB->c_ipred_mode) {
    case DC_PRED_8: intrapred_chroma_dc(currMB); break;
    case HOR_PRED_8: intrapred_chroma_hor(currMB); break;
    case VERT_PRED_8: intrapred_chroma_ver(currMB); break;
    case PLANE_8: intrapred_chroma_plane(currMB); break;
