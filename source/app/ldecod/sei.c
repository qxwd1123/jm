...
// SEI info #define PRINT_PICTURE_TIMING_INFO      // uncomment to print picture
...
// packing arrangement SEI info #define PRINT_GREEN_METADATA_INFO      //
// uncomment to print Green Metadata SEI info
  ...
  do {
    ...
    switch (payload_type)  // sei_payload( type, size );
    {
      ...
      case SEI_USER_DATA_REGISTERED_ITU_T_T35:
        interpret_user_data_registered_itu_t_t35_info(msg + offset,
                                                      payload_size, p_Vid);
        break;
      ...
      case SEI_DEC_REF_PIC_MARKING_REPETITION:
        interpret_dec_ref_pic_marking_repetition_info(
            msg + offset, payload_size, p_Vid, pSlice);
        break;
      case SEI_SPARE_PIC:
...
void interpret_spare_pic(byte *payload, int size, VideoParameters *p_Vid) {
  ...
  num_spare_pics =
      1 + read_ue_v("SEI: num_spare_pics_minus1", buf, &p_Dec->UsedBits);
  ...
  for (i = 0; i < num_spare_pics; i++) {
    ...
    delta_spare_frame_num =
        read_ue_v("SEI: delta_spare_frame_num", buf, &p_Dec->UsedBits);

    SpareFrameNum = CandidateSpareFrameNum - delta_spare_frame_num;
    if (SpareFrameNum < 0) SpareFrameNum = MAX_FN + SpareFrameNum;

    ref_area_indicator =
        read_ue_v("SEI: ref_area_indicator", buf, &p_Dec->UsedBits);

    switch (ref_area_indicator) {
      case 0:  // The whole frame can serve as spare picture
        for (y = 0; y < p_Vid->height >> 4; y++)
          for (x = 0; x < p_Vid->width >> 4; x++) map[i][y][x] = 0;
        break;
      case 1:  // The map is not compressed
        for (y = 0; y < p_Vid->height >> 4; y++)
          for (x = 0; x < p_Vid->width >> 4; x++) {
            map[i][y][x] =
                (byte)read_u_1("SEI: ref_mb_indicator", buf, &p_Dec->UsedBits);
          }
        break;
      case 2:  // The map is compressed
        ...
        for (m = 0; m < p_Vid->height >> 4; m++)
          for (n = 0; n < p_Vid->width >> 4; n++) {
            if (no_bit0 < 0) {
              no_bit0 =
                  read_ue_v("SEI: zero_run_length", buf, &p_Dec->UsedBits);
            }
            if (no_bit0 > 0)
              map[i][y][x] = (byte)bit0;
            else
    ...
    for (kk = 0; kk < num_spare_pics; kk++) {
      for (i = 0; i < p_Vid->height >> 4; i++)
        for (j = 0; j < p_Vid->width >> 4; j++) {
          ...
          for (i0 = 0; i0 < 16; i0++)
            for (j0 = 0; j0 < 16; j0++) Y[i * 16 + i0][j * 16 + j0] = tmp;
        }
...
void interpret_subsequence_info(byte *payload, int size,
  ...
  sub_seq_layer_num =
      read_ue_v("SEI: sub_seq_layer_num", buf, &p_Dec->UsedBits);
  sub_seq_id = read_ue_v("SEI: sub_seq_id", buf, &p_Dec->UsedBits);
  first_ref_pic_flag =
      read_u_1("SEI: first_ref_pic_flag", buf, &p_Dec->UsedBits);
  leading_non_ref_pic_flag =
      read_u_1("SEI: leading_non_ref_pic_flag", buf, &p_Dec->UsedBits);
  last_pic_flag = read_u_1("SEI: last_pic_flag", buf, &p_Dec->UsedBits);
  sub_seq_frame_num_flag =
      read_u_1("SEI: sub_seq_frame_num_flag", buf, &p_Dec->UsedBits);
  if (sub_seq_frame_num_flag) {
    sub_seq_frame_num =
        read_ue_v("SEI: sub_seq_frame_num", buf, &p_Dec->UsedBits);
  }
...
void interpret_subsequence_layer_characteristics_info(byte *payload, int size,
  ...
  num_sub_layers =
      1 + read_ue_v("SEI: num_sub_layers_minus1", buf, &p_Dec->UsedBits);
  ...
  for (i = 0; i < num_sub_layers; i++) {
    accurate_statistics_flag =
        read_u_1("SEI: accurate_statistics_flag", buf, &p_Dec->UsedBits);
    average_bit_rate =
        read_u_v(16, "SEI: average_bit_rate", buf, &p_Dec->UsedBits);
    average_frame_rate =
        read_u_v(16, "SEI: average_frame_rate", buf, &p_Dec->UsedBits);
...
void interpret_subsequence_characteristics_info(byte *payload, int size,
  ...
  if (duration_flag) {
    sub_seq_duration =
        read_u_v(32, "SEI: duration_flag", buf, &p_Dec->UsedBits);
#ifdef PRINT_SUBSEQUENCE_CHAR
  ...
  num_referenced_subseqs =
      read_ue_v("SEI: num_referenced_subseqs", buf, &p_Dec->UsedBits);
  ...
  for (i = 0; i < num_referenced_subseqs; i++) {
    ref_sub_seq_layer_num =
        read_ue_v("SEI: ref_sub_seq_layer_num", buf, &p_Dec->UsedBits);
    ref_sub_seq_id = read_ue_v("SEI: ref_sub_seq_id", buf, &p_Dec->UsedBits);
    ref_sub_seq_direction =
        read_u_1("SEI: ref_sub_seq_direction", buf, &p_Dec->UsedBits);
...
void interpret_scene_information(byte *payload, int size,
  ...
  scene_transition_type =
      read_ue_v("SEI: scene_transition_type", buf, &p_Dec->UsedBits);
...
void interpret_pan_scan_rect_info(byte *payload, int size,
  ...
  pan_scan_rect_cancel_flag =
      read_u_1("SEI: pan_scan_rect_cancel_flag", buf, &p_Dec->UsedBits);
  if (!pan_scan_rect_cancel_flag) {
    pan_scan_cnt_minus1 =
        read_ue_v("SEI: pan_scan_cnt_minus1", buf, &p_Dec->UsedBits);
    for (i = 0; i <= pan_scan_cnt_minus1; i++) {
      pan_scan_rect_left_offset =
          read_se_v("SEI: pan_scan_rect_left_offset", buf, &p_Dec->UsedBits);
      pan_scan_rect_right_offset =
          read_se_v("SEI: pan_scan_rect_right_offset", buf, &p_Dec->UsedBits);
      pan_scan_rect_top_offset =
          read_se_v("SEI: pan_scan_rect_top_offset", buf, &p_Dec->UsedBits);
      pan_scan_rect_bottom_offset =
          read_se_v("SEI: pan_scan_rect_bottom_offset", buf, &p_Dec->UsedBits);
    ...
    pan_scan_rect_repetition_period = read_ue_v(
        "SEI: pan_scan_rect_repetition_period", buf, &p_Dec->UsedBits);
  }
...
void interpret_recovery_point_info(byte *payload, int size,
  ...
  changing_slice_group_idc =
      read_u_v(2, "SEI: changing_slice_group_idc", buf, &p_Dec->UsedBits);
...
void interpret_dec_ref_pic_marking_repetition_info(byte *payload, int size,
  ...
  original_frame_num =
      read_ue_v("SEI: original_frame_num", buf, &p_Dec->UsedBits);

  if (!p_Vid->active_sps->frame_mbs_only_flag) {
    original_field_pic_flag =
        read_u_1("SEI: original_field_pic_flag", buf, &p_Dec->UsedBits);
    if (original_field_pic_flag) {
      original_bottom_field_flag =
          read_u_1("SEI: original_bottom_field_flag", buf, &p_Dec->UsedBits);
    }
  }
...
void interpret_full_frame_freeze_info(byte *payload, int size,
  ...
  full_frame_freeze_repetition_period = read_ue_v(
      "SEI: full_frame_freeze_repetition_period", buf, &p_Dec->UsedBits);
...
void interpret_progressive_refinement_start_info(byte *payload, int size,
  ...
  progressive_refinement_id =
      read_ue_v("SEI: progressive_refinement_id", buf, &p_Dec->UsedBits);
  num_refinement_steps_minus1 =
      read_ue_v("SEI: num_refinement_steps_minus1", buf, &p_Dec->UsedBits);
...
void interpret_motion_constrained_slice_group_set_info(byte *payload, int size,
  ...
  num_slice_groups_minus1 =
      read_ue_v("SEI: num_slice_groups_minus1", buf, &p_Dec->UsedBits);
  ...
  for (i = 0; i <= num_slice_groups_minus1; i++) {
    slice_group_id =
        read_u_v(sliceGroupSize, "SEI: slice_group_id", buf, &p_Dec->UsedBits);
#ifdef PRINT_MOTION_CONST_SLICE_GROUP_SET_INFO
  ...
  exact_match_flag = read_u_1("SEI: exact_match_flag", buf, &p_Dec->UsedBits);
  pan_scan_rect_flag =
      read_u_1("SEI: pan_scan_rect_flag", buf, &p_Dec->UsedBits);
  ...
  if (pan_scan_rect_flag) {
    pan_scan_rect_id =
        read_ue_v("SEI: pan_scan_rect_id", buf, &p_Dec->UsedBits);
#ifdef PRINT_MOTION_CONST_SLICE_GROUP_SET_INFO
    printf("pan_scan_rect_id         = %d\n", pan_scan_rect_id);
#endif
  }
...
void interpret_film_grain_characteristics_info(byte *payload, int size,
  ...
  film_grain_characteristics_cancel_flag = read_u_1(
      "SEI: film_grain_characteristics_cancel_flag", buf, &p_Dec->UsedBits);
  ...
  if (!film_grain_characteristics_cancel_flag) {
    ...
    separate_colour_description_present_flag = read_u_1(
        "SEI: separate_colour_description_present_flag", buf, &p_Dec->UsedBits);
#ifdef PRINT_FILM_GRAIN_CHARACTERISTICS_INFO
    printf("model_id = %d\n", model_id);
    ...
    if (separate_colour_description_present_flag) {
      film_grain_bit_depth_luma_minus8 = read_u_v(
          3, "SEI: film_grain_bit_depth_luma_minus8", buf, &p_Dec->UsedBits);
      film_grain_bit_depth_chroma_minus8 = read_u_v(
          3, "SEI: film_grain_bit_depth_chroma_minus8", buf, &p_Dec->UsedBits);
      film_grain_full_range_flag =
          read_u_v(1, "SEI: film_grain_full_range_flag", buf, &p_Dec->UsedBits);
      film_grain_colour_primaries = read_u_v(
          8, "SEI: film_grain_colour_primaries", buf, &p_Dec->UsedBits);
      film_grain_transfer_characteristics = read_u_v(
          8, "SEI: film_grain_transfer_characteristics", buf, &p_Dec->UsedBits);
      film_grain_matrix_coefficients = read_u_v(
          8, "SEI: film_grain_matrix_coefficients", buf, &p_Dec->UsedBits);
#ifdef PRINT_FILM_GRAIN_CHARACTERISTICS_INFO
    ...
    blending_mode_id =
        read_u_v(2, "SEI: blending_mode_id", buf, &p_Dec->UsedBits);
    log2_scale_factor =
        read_u_v(4, "SEI: log2_scale_factor", buf, &p_Dec->UsedBits);
#ifdef PRINT_FILM_GRAIN_CHARACTERISTICS_INFO
    ...
    for (c = 0; c < 3; c++) {
      comp_model_present_flag[c] =
          read_u_1("SEI: comp_model_present_flag", buf, &p_Dec->UsedBits);
#ifdef PRINT_FILM_GRAIN_CHARACTERISTICS_INFO
    ...
    for (c = 0; c < 3; c++)
      if (comp_model_present_flag[c]) {
        num_intensity_intervals_minus1 = read_u_v(
            8, "SEI: num_intensity_intervals_minus1", buf, &p_Dec->UsedBits);
        num_model_values_minus1 =
            read_u_v(3, "SEI: num_model_values_minus1", buf, &p_Dec->UsedBits);
#ifdef PRINT_FILM_GRAIN_CHARACTERISTICS_INFO
        ...
        for (i = 0; i <= num_intensity_intervals_minus1; i++) {
          intensity_interval_lower_bound = read_u_v(
              8, "SEI: intensity_interval_lower_bound", buf, &p_Dec->UsedBits);
          intensity_interval_upper_bound = read_u_v(
              8, "SEI: intensity_interval_upper_bound", buf, &p_Dec->UsedBits);
#ifdef PRINT_FILM_GRAIN_CHARACTERISTICS_INFO
          ...
          for (j = 0; j <= num_model_values_minus1; j++) {
            comp_model_value =
                read_se_v("SEI: comp_model_value", buf, &p_Dec->UsedBits);
#ifdef PRINT_FILM_GRAIN_CHARACTERISTICS_INFO
...
void interpret_deblocking_filter_display_preference_info(
  ...
  deblocking_display_preference_cancel_flag = read_u_1(
      "SEI: deblocking_display_preference_cancel_flag", buf, &p_Dec->UsedBits);
  ...
  if (!deblocking_display_preference_cancel_flag) {
    display_prior_to_deblocking_preferred_flag =
        read_u_1("SEI: display_prior_to_deblocking_preferred_flag", buf,
                 &p_Dec->UsedBits);
    dec_frame_buffering_constraint_flag = read_u_1(
        "SEI: dec_frame_buffering_constraint_flag", buf, &p_Dec->UsedBits);
    deblocking_display_preference_repetition_period =
        read_ue_v("SEI: deblocking_display_preference_repetition_period", buf,
                  &p_Dec->UsedBits);
#ifdef PRINT_DEBLOCKING_FILTER_DISPLAY_PREFERENCE_INFO
...
void interpret_stereo_video_info_info(byte *payload, int size,
  ...
  if (field_views_flags) {
    top_field_is_left_view_flag =
        read_u_1("SEI: top_field_is_left_view_flag", buf, &p_Dec->UsedBits);
#ifdef PRINT_STEREO_VIDEO_INFO_INFO
    printf("top_field_is_left_view_flag = %d\n", top_field_is_left_view_flag);
#endif
  } else {
    current_frame_is_left_view_flag =
        read_u_1("SEI: current_frame_is_left_view_flag", buf, &p_Dec->UsedBits);
    next_frame_is_second_view_flag =
        read_u_1("SEI: next_frame_is_second_view_flag", buf, &p_Dec->UsedBits);
#ifdef PRINT_STEREO_VIDEO_INFO_INFO
  ...
  left_view_self_contained_flag =
      read_u_1("SEI: left_view_self_contained_flag", buf, &p_Dec->UsedBits);
  right_view_self_contained_flag =
      read_u_1("SEI: right_view_self_contained_flag", buf, &p_Dec->UsedBits);
#ifdef PRINT_STEREO_VIDEO_INFO_INFO
...
void interpret_buffering_period_info(byte *payload, int size,
  ...
  seq_parameter_set_id =
      read_ue_v("SEI: seq_parameter_set_id", buf, &p_Dec->UsedBits);
  ...
  if (sps->vui_parameters_present_flag) {
    if (sps->vui_seq_parameters.nal_hrd_parameters_present_flag) {
      for (k = 0;
        ...
        initial_cpb_removal_delay =
            read_u_v(sps->vui_seq_parameters.nal_hrd_parameters
                             .initial_cpb_removal_delay_length_minus1 +
                         1,
                     "SEI: initial_cpb_removal_delay", buf, &p_Dec->UsedBits);
        initial_cpb_removal_delay_offset = read_u_v(
            sps->vui_seq_parameters.nal_hrd_parameters
                    .initial_cpb_removal_delay_length_minus1 +
                1,
            "SEI: initial_cpb_removal_delay_offset", buf, &p_Dec->UsedBits);
    ...
    if (sps->vui_seq_parameters.vcl_hrd_parameters_present_flag) {
      for (k = 0;
           ...
           k++) {
        initial_cpb_removal_delay =
            read_u_v(sps->vui_seq_parameters.vcl_hrd_parameters
                             .initial_cpb_removal_delay_length_minus1 +
                         1,
                     "SEI: initial_cpb_removal_delay", buf, &p_Dec->UsedBits);
        initial_cpb_removal_delay_offset = read_u_v(
            sps->vui_seq_parameters.vcl_hrd_parameters
                    .initial_cpb_removal_delay_length_minus1 +
                1,
            "SEI: initial_cpb_removal_delay_offset", buf, &p_Dec->UsedBits);
...
void interpret_picture_timing_info(byte *payload, int size,
  ...
  if (CpbDpbDelaysPresentFlag) {
    ...
    if ((active_sps->vui_seq_parameters.nal_hrd_parameters_present_flag) ||
      ...
      cpb_removal_delay = read_u_v(cpb_removal_len, "SEI: cpb_removal_delay",
                                   buf, &p_Dec->UsedBits);
  ...
  if (pic_struct_present_flag) {
    ...
    switch (pic_struct) {
      case 0:
      case 1:
      case 2: NumClockTs = 1; break;
      case 3:
      case 4:
      case 7: NumClockTs = 2; break;
      case 5:
      case 6:
      case 8: NumClockTs = 3; break;
      default:
        error("reserved pic_struct used (can't determine NumClockTs)", 500);
    }
    for (i = 0; i < NumClockTs; i++) {
      clock_timestamp_flag =
          read_u_1("SEI: clock_timestamp_flag", buf, &p_Dec->UsedBits);
#ifdef PRINT_PICTURE_TIMING_INFO
      ...
      if (clock_timestamp_flag) {
        ...
        full_timestamp_flag =
            read_u_1("SEI: full_timestamp_flag", buf, &p_Dec->UsedBits);
        discontinuity_flag =
...
void interpret_frame_packing_arrangement_info(byte *payload, int size,
  ...
  seiFramePackingArrangement.frame_packing_arrangement_id =
      (unsigned int)read_ue_v("SEI: frame_packing_arrangement_id", buf,
                              &p_Dec->UsedBits);
  seiFramePackingArrangement.frame_packing_arrangement_cancel_flag = read_u_1(
      "SEI: frame_packing_arrangement_cancel_flag", buf, &p_Dec->UsedBits);
#ifdef PRINT_FRAME_PACKING_ARRANGEMENT_INFO
  ...
  if (seiFramePackingArrangement.frame_packing_arrangement_cancel_flag ==
    ...
    seiFramePackingArrangement.quincunx_sampling_flag =
        read_u_1("SEI: quincunx_sampling_flag", buf, &p_Dec->UsedBits);
    ...
    seiFramePackingArrangement.spatial_flipping_flag =
        read_u_1("SEI: spatial_flipping_flag", buf, &p_Dec->UsedBits);
    seiFramePackingArrangement.frame0_flipped_flag =
        read_u_1("SEI: frame0_flipped_flag", buf, &p_Dec->UsedBits);
    seiFramePackingArrangement.field_views_flag =
        read_u_1("SEI: field_views_flag", buf, &p_Dec->UsedBits);
    seiFramePackingArrangement.current_frame_is_frame0_flag =
        read_u_1("SEI: current_frame_is_frame0_flag", buf, &p_Dec->UsedBits);
    seiFramePackingArrangement.frame0_self_contained_flag =
        read_u_1("SEI: frame0_self_contained_flag", buf, &p_Dec->UsedBits);
    seiFramePackingArrangement.frame1_self_contained_flag =
        read_u_1("SEI: frame1_self_contained_flag", buf, &p_Dec->UsedBits);
#ifdef PRINT_FRAME_PACKING_ARRANGEMENT_INFO
    ...
    seiFramePackingArrangement.frame_packing_arrangement_reserved_byte =
        (unsigned char)read_u_v(8,
                                "SEI: frame_packing_arrangement_reserved_byte",
                                buf, &p_Dec->UsedBits);
    seiFramePackingArrangement.frame_packing_arrangement_repetition_period =
        (unsigned int)read_ue_v(
            "SEI: frame_packing_arrangement_repetition_period", buf,
            &p_Dec->UsedBits);
#ifdef PRINT_FRAME_PACKING_ARRANGEMENT_INFO
  ...
  seiFramePackingArrangement.frame_packing_arrangement_extension_flag =
      read_u_1("SEI: frame_packing_arrangement_extension_flag", buf,
               &p_Dec->UsedBits);
#ifdef PRINT_FRAME_PACKING_ARRANGEMENT_INFO
...
void interpret_tone_mapping(byte *payload, int size, VideoParameters *p_Vid) {
  ...
  seiToneMappingTmp.tone_map_id =
      read_ue_v("SEI: tone_map_id", buf, &p_Dec->UsedBits);
  seiToneMappingTmp.tone_map_cancel_flag = (unsigned char)read_u_1(
      "SEI: tone_map_cancel_flag", buf, &p_Dec->UsedBits);
  ...
  if (seiToneMappingTmp.tone_map_id != 0)
    printf(
        "WARNING! Tone_map_id != 0, print the SEI message info only. The tone "
        "mapping is actually applied only when Tone_map_id==0\n\n");
  ...
  if (!seiToneMappingTmp.tone_map_cancel_flag) {
    seiToneMappingTmp.tone_map_repetition_period =
        read_ue_v("SEI: tone_map_repetition_period", buf, &p_Dec->UsedBits);
    seiToneMappingTmp.coded_data_bit_depth = (unsigned char)read_u_v(
        8, "SEI: coded_data_bit_depth", buf, &p_Dec->UsedBits);
    seiToneMappingTmp.sei_bit_depth =
        (unsigned char)read_u_v(8, "SEI: sei_bit_depth", buf, &p_Dec->UsedBits);
    seiToneMappingTmp.model_id =
        read_ue_v("SEI: model_id", buf, &p_Dec->UsedBits);
    ...
    if (seiToneMappingTmp.model_id == 0) {  // linear mapping with clipping
      seiToneMappingTmp.min_value =
          read_u_v(32, "SEI: min_value", buf, &p_Dec->UsedBits);
      ...
      seiToneMappingTmp.sigmoid_midpoint =
          read_u_v(32, "SEI: sigmoid_midpoint", buf, &p_Dec->UsedBits);
      seiToneMappingTmp.sigmoid_width =
          read_u_v(32, "SEI: sigmoid_width", buf, &p_Dec->UsedBits);
#ifdef PRINT_TONE_MAPPING
      ...
      for (i = 0; i < max_output_num; i++) {
        seiToneMappingTmp.start_of_coded_interval[i] =
            read_u_v((((seiToneMappingTmp.coded_data_bit_depth + 7) >> 3) << 3),
                     "SEI: start_of_coded_interval", buf, &p_Dec->UsedBits);
      ...
      seiToneMappingTmp.num_pivots =
          read_u_v(16, "SEI: num_pivots", buf, &p_Dec->UsedBits);
#ifdef PRINT_TONE_MAPPING
      printf("num_pivots = %d\n", seiToneMappingTmp.num_pivots);
#endif
      ...
      for (i = 1; i < seiToneMappingTmp.num_pivots + 1; i++) {
        seiToneMappingTmp.coded_pivot_value[i] =
            read_u_v((((seiToneMappingTmp.coded_data_bit_depth + 7) >> 3) << 3),
                     "SEI: coded_pivot_value", buf, &p_Dec->UsedBits);
        seiToneMappingTmp.sei_pivot_value[i] =
            read_u_v((((seiToneMappingTmp.sei_bit_depth + 7) >> 3) << 3),
                     "SEI: sei_pivot_value", buf, &p_Dec->UsedBits);
#ifdef PRINT_TONE_MAPPING
      ...
      switch (seiToneMappingTmp.model_id) {
        ...
        case 1:  // sigmoid mapping
          ...
          for (i = 0; i < max_coded_num; i++) {
            double tmp =
                1.0 +
                exp(-6 * (double)(i - seiToneMappingTmp.sigmoid_midpoint) /
                    seiToneMappingTmp.sigmoid_width);
            p_Vid->seiToneMapping->lut[i] =
...
void interpret_post_filter_hints_info(byte *payload, int size,
  ...
  filter_hint_size_y =
      read_ue_v("SEI: filter_hint_size_y", buf,
                &p_Dec->UsedBits);  // interpret post-filter hint SEI here
  filter_hint_size_x =
      read_ue_v("SEI: filter_hint_size_x", buf,
                &p_Dec->UsedBits);  // interpret post-filter hint SEI here
  filter_hint_type =
      read_u_v(2, "SEI: filter_hint_type", buf,
               &p_Dec->UsedBits);  // interpret post-filter hint SEI here
    ...
    for (cy = 0; cy < filter_hint_size_y; cy++)
      for (cx = 0; cx < filter_hint_size_x; cx++)
        filter_hint[color_component][cy][cx] =
            read_se_v("SEI: filter_hint", buf,
                      &p_Dec->UsedBits);  // interpret post-filter hint SEI here

  additional_extension_flag =
      read_u_1("SEI: additional_extension_flag", buf,
               &p_Dec->UsedBits);  // interpret post-filter hint SEI here
...
void interpret_green_metadata_info(byte *payload, int size,
                                   VideoParameters *p_Vid) {
  ...
  seiGreenMetadataInfo.green_metadata_type = (unsigned char)read_u_v(
      8, "SEI: green_metadata_type", buf, &p_Dec->UsedBits);
#ifdef PRINT_GREEN_METADATA_INFO
  printf("green_metadata_type                 = %d\n",
         seiGreenMetadataInfo.green_metadata_type);
#endif
  if (seiGreenMetadataInfo.green_metadata_type == 0) {
    seiGreenMetadataInfo.period_type = (unsigned char)read_u_v(
        8, "SEI: green_metadata_period_type", buf, &p_Dec->UsedBits);
#ifdef PRINT_GREEN_METADATA_INFO
    ...
    if (seiGreenMetadataInfo.period_type == 2) {
      seiGreenMetadataInfo.num_seconds = (unsigned short)read_u_v(
          16, "SEI: green_metadata_num_seconds", buf, &p_Dec->UsedBits);
#ifdef PRINT_GREEN_METADATA_INFO
      printf("green_metadata_num_seconds      = %d\n",
             seiGreenMetadataInfo.num_seconds);
#endif
    } else if (seiGreenMetadataInfo.period_type == 3) {
      seiGreenMetadataInfo.num_pictures = (unsigned short)read_u_v(
          16, "SEI: green_metadata_num_pictures", buf, &p_Dec->UsedBits);
#ifdef PRINT_GREEN_METADATA_INFO
    ...
    seiGreenMetadataInfo.percent_non_zero_macroblocks = (unsigned char)read_u_v(
        8, "SEI: percent_non_zero_macroblocks", buf, &p_Dec->UsedBits);
    seiGreenMetadataInfo.percent_intra_coded_macroblocks =
        (unsigned char)read_u_v(8, "SEI: percent_intra_coded_macroblocks", buf,
                                &p_Dec->UsedBits);
    seiGreenMetadataInfo.percent_six_tap_filtering = (unsigned char)read_u_v(
        8, "SEI: percent_six_tap_filtering", buf, &p_Dec->UsedBits);
    seiGreenMetadataInfo.percent_alpha_point_deblocking_instance =
        (unsigned char)read_u_v(8,
                                "SEI: percent_alpha_point_deblocking_instance",
                                buf, &p_Dec->UsedBits);
    ...
    printf("percent_alpha_point_deblocking_instance      = %f\n",
           (float)seiGreenMetadataInfo.percent_alpha_point_deblocking_instance /
               255);
#endif

  } else if (seiGreenMetadataInfo.green_metadata_type == 1) {
    seiGreenMetadataInfo.xsd_metric_type = (unsigned char)read_u_v(
        8, "SEI: xsd_metric_type", buf, &p_Dec->UsedBits);
    seiGreenMetadataInfo.xsd_metric_value = (unsigned short)read_u_v(
        16, "SEI: xsd_metric_value", buf, &p_Dec->UsedBits);
#ifdef PRINT_GREEN_METADATA_INFO
