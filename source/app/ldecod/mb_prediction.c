...
int mb_pred_intra4x4(Macroblock *currMB, ColorPlane curr_plane,
  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++) {
    for (k = block8x8 * 4; k < block8x8 * 4 + 4; k++) {
      ...
      // PREDICTION
      //===== INTRA PREDICTION =====
      if (currSlice->intra_pred_4x4(currMB, curr_plane, ioff, joff, i4, j4) ==
  ...
  if (currMB->cbp != 0) currSlice->is_reset_coeff = FALSE;
  return 1;
}
...
int mb_pred_p_inter8x8(Macroblock *currMB, ColorPlane curr_plane,
  ...
  Slice *currSlice = currMB->p_Slice;
  int smb =
      currSlice->slice_type == SP_SLICE && (currMB->is_intra_block == FALSE);
      ...
      } else  // co-located skip or inter mode
      {
        ...
        if (INVALIDINDEX != mapped_idx) {
          ...
          mv_info->ref_idx[LIST_0] =
              (char)mapped_idx;  // colocated->ref_idx[refList];
          mv_info->ref_idx[LIST_1] = 0;
        } else if (INVALIDINDEX == mapped_idx) {
          error(
              "temporal direct error: colocated block has ref that is "
              "unavailable",
              -1111);
        }
      }
