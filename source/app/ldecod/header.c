...
int FirstPartOfSliceHeader(Slice *currSlice) {
  ...
  p_Dec->UsedBits = partition->bitstream
                        ->frame_bitoffset;  // was hardcoded to 31 for previous
                                            // start-code. This is better.

  // Get first_mb_in_slice
  currSlice->start_mb_nr =
      read_ue_v("SH: first_mb_in_slice", currStream, &p_Dec->UsedBits);
  ...
  currSlice->pic_parameter_set_id =
      read_ue_v("SH: pic_parameter_set_id", currStream, &p_Dec->UsedBits);

  if (p_Vid->separate_colour_plane_flag)
    currSlice->colour_plane_id =
        read_u_v(2, "SH: colour_plane_id", currStream, &p_Dec->UsedBits);
  else
...
int RestOfSliceHeader(Slice *currSlice) {
  ...
  currSlice->frame_num =
      read_u_v(active_sps->log2_max_frame_num_minus4 + 4, "SH: frame_num",
  ...
  if (active_sps->frame_mbs_only_flag) {
    ...
    currSlice->field_pic_flag =
        read_u_1("SH: field_pic_flag", currStream, &p_Dec->UsedBits);
    if (currSlice->field_pic_flag) {
      // bottom_field_flag  u(1)
      currSlice->bottom_field_flag =
          (byte)read_u_1("SH: bottom_field_flag", currStream, &p_Dec->UsedBits);
      p_Vid->structure =
  ...
  if (currSlice->idr_flag) {
    currSlice->idr_pic_id =
        read_ue_v("SH: idr_pic_id", currStream, &p_Dec->UsedBits);
  }
#if (MVC_EXTENSION_ENABLE)
  else if (currSlice->svc_extension_flag == 0 &&
           currSlice->NaluHeaderMVCExt.non_idr_flag == 0) {
    currSlice->idr_pic_id =
        read_ue_v("SH: idr_pic_id", currStream, &p_Dec->UsedBits);
  }
#endif
  ...
  if (active_sps->pic_order_cnt_type == 0) {
    currSlice->pic_order_cnt_lsb =
        read_u_v(active_sps->log2_max_pic_order_cnt_lsb_minus4 + 4,
                 "SH: pic_order_cnt_lsb", currStream, &p_Dec->UsedBits);
    if (p_Vid->active_pps->bottom_field_pic_order_in_frame_present_flag == 1 &&
        !currSlice->field_pic_flag)
      currSlice->delta_pic_order_cnt_bottom = read_se_v(
          "SH: delta_pic_order_cnt_bottom", currStream, &p_Dec->UsedBits);
    else
  ...
  if (active_sps->pic_order_cnt_type == 1) {
    if (!active_sps->delta_pic_order_always_zero_flag) {
      currSlice->delta_pic_order_cnt[0] =
          read_se_v("SH: delta_pic_order_cnt[0]", currStream, &p_Dec->UsedBits);
      if (p_Vid->active_pps->bottom_field_pic_order_in_frame_present_flag ==
              1 &&
          !currSlice->field_pic_flag)
        currSlice->delta_pic_order_cnt[1] = read_se_v(
            "SH: delta_pic_order_cnt[1]", currStream, &p_Dec->UsedBits);
      else
  ...
  if (p_Vid->active_pps->redundant_pic_cnt_present_flag) {
    currSlice->redundant_pic_cnt =
        read_ue_v("SH: redundant_pic_cnt", currStream, &p_Dec->UsedBits);
  }

  if (currSlice->slice_type == B_SLICE) {
    currSlice->direct_spatial_mv_pred_flag = read_u_1(
        "SH: direct_spatial_mv_pred_flag", currStream, &p_Dec->UsedBits);
  }

  currSlice->num_ref_idx_active[LIST_0] =
      p_Vid->active_pps->num_ref_idx_l0_default_active_minus1 + 1;
  currSlice->num_ref_idx_active[LIST_1] =
      p_Vid->active_pps->num_ref_idx_l1_default_active_minus1 + 1;

  if (currSlice->slice_type == P_SLICE || currSlice->slice_type == SP_SLICE ||
      currSlice->slice_type == B_SLICE) {
    val =
        read_u_1("SH: num_ref_idx_override_flag", currStream, &p_Dec->UsedBits);
    if (val) {
      currSlice->num_ref_idx_active[LIST_0] =
          1 + read_ue_v("SH: num_ref_idx_l0_active_minus1", currStream,
                        &p_Dec->UsedBits);

      if (currSlice->slice_type == B_SLICE) {
        currSlice->num_ref_idx_active[LIST_1] =
            1 + read_ue_v("SH: num_ref_idx_l1_active_minus1", currStream,
                          &p_Dec->UsedBits);
      }
    }
  }
  ...
  if (p_Vid->active_pps->entropy_coding_mode_flag &&
    ...
    currSlice->model_number =
        read_ue_v("SH: cabac_init_idc", currStream, &p_Dec->UsedBits);
  } else {
    currSlice->model_number = 0;
  }

  currSlice->slice_qp_delta = val =
      read_se_v("SH: slice_qp_delta", currStream, &p_Dec->UsedBits);
  ...
  if (currSlice->slice_type == SP_SLICE || currSlice->slice_type == SI_SLICE) {
    if (currSlice->slice_type == SP_SLICE) {
      currSlice->sp_switch =
          read_u_1("SH: sp_for_switch_flag", currStream, &p_Dec->UsedBits);
    }
    currSlice->slice_qs_delta = val =
        read_se_v("SH: slice_qs_delta", currStream, &p_Dec->UsedBits);
  ...
  if (p_Vid->active_pps->deblocking_filter_control_present_flag) {
    currSlice->DFDisableIdc = (short)read_ue_v(
        "SH: disable_deblocking_filter_idc", currStream, &p_Dec->UsedBits);

    if (currSlice->DFDisableIdc != 1) {
      currSlice->DFAlphaC0Offset =
          (short)(2 * read_se_v("SH: slice_alpha_c0_offset_div2", currStream,
                                &p_Dec->UsedBits));
      currSlice->DFBetaOffset =
          (short)(2 * read_se_v("SH: slice_beta_offset_div2", currStream,
                                &p_Dec->UsedBits));
    } else {
...
#if DPF_PARAM_DISP
  printf("Slice:%d, DFParameters:(%d,%d,%d)\n\n", currSlice->current_slice_nr,
         currSlice->DFDisableIdc, currSlice->DFAlphaC0Offset,
         currSlice->DFBetaOffset);
#endif

  // The conformance point for intra profiles is without deblocking, but
  // decoders are still recommended to filter the output. We allow in the
  // decoder config to skip the loop filtering. This is achieved by modifying
  // the parameters here.
  if (is_HI_intra_only_profile(active_sps->profile_idc,
                               active_sps->constrained_set3_flag) &&
  ...
  if (p_Vid->active_pps->num_slice_groups_minus1 > 0 &&
    ...
    currSlice->slice_group_change_cycle = read_u_v(
        len, "SH: slice_group_change_cycle", currStream, &p_Dec->UsedBits);
  }
...
static void ref_pic_list_reordering(Slice *currSlice) {
  ...
  if (currSlice->slice_type != I_SLICE && currSlice->slice_type != SI_SLICE) {
    val = currSlice->ref_pic_list_reordering_flag[LIST_0] = read_u_1(
        "SH: ref_pic_list_reordering_flag_l0", currStream, &p_Dec->UsedBits);

    if (val) {
      i = 0;
      do {
        val = currSlice->modification_of_pic_nums_idc[LIST_0][i] =
            read_ue_v("SH: modification_of_pic_nums_idc_l0", currStream,
                      &p_Dec->UsedBits);
        if (val == 0 || val == 1) {
          currSlice->abs_diff_pic_num_minus1[LIST_0][i] = read_ue_v(
              "SH: abs_diff_pic_num_minus1_l0", currStream, &p_Dec->UsedBits);
        } else {
          if (val == 2) {
            currSlice->long_term_pic_idx[LIST_0][i] = read_ue_v(
                "SH: long_term_pic_idx_l0", currStream, &p_Dec->UsedBits);
          }
        }
        i++;
  ...
  if (currSlice->slice_type == B_SLICE) {
    val = currSlice->ref_pic_list_reordering_flag[LIST_1] = read_u_1(
        "SH: ref_pic_list_reordering_flag_l1", currStream, &p_Dec->UsedBits);

    if (val) {
      i = 0;
      do {
        val = currSlice->modification_of_pic_nums_idc[LIST_1][i] =
            read_ue_v("SH: modification_of_pic_nums_idc_l1", currStream,
                      &p_Dec->UsedBits);
        if (val == 0 || val == 1) {
          currSlice->abs_diff_pic_num_minus1[LIST_1][i] = read_ue_v(
              "SH: abs_diff_pic_num_minus1_l1", currStream, &p_Dec->UsedBits);
        } else {
          if (val == 2) {
            currSlice->long_term_pic_idx[LIST_1][i] = read_ue_v(
                "SH: long_term_pic_idx_l1", currStream, &p_Dec->UsedBits);
          }
        }
        i++;
...
static void ref_pic_list_mvc_modification(Slice *currSlice) {
  ...
  if ((currSlice->slice_type % 5) != I_SLICE &&
    ...
    val = currSlice->ref_pic_list_reordering_flag[LIST_0] = read_u_1(
        "SH: ref_pic_list_modification_flag_l0", currStream, &p_Dec->UsedBits);
        ...
        if (val == 0 || val == 1) {
          ...
          if (val == 2) {
            ...
            currSlice->abs_diff_view_idx_minus1[LIST_0][i] =
                read_ue_v("SH: abs_diff_view_idx_minus1_l0", currStream,
                          &p_Dec->UsedBits);
          }
        }
        i++;
        ...
        if (val == 0 || val == 1) {
          ...
          if (val == 2) {
            ...
            currSlice->abs_diff_view_idx_minus1[LIST_1][i] =
                read_ue_v("SH: abs_diff_view_idx_minus1_l1", currStream,
                          &p_Dec->UsedBits);
          }
        }
        i++;
...
static void pred_weight_table(Slice *currSlice) {
  ...
  currSlice->luma_log2_weight_denom = (unsigned short)read_ue_v(
      "SH: luma_log2_weight_denom", currStream, &p_Dec->UsedBits);
  ...
  if (0 != active_sps->chroma_format_idc) {
    currSlice->chroma_log2_weight_denom = (unsigned short)read_ue_v(
        "SH: chroma_log2_weight_denom", currStream, &p_Dec->UsedBits);
  ...
  for (i = 0; i < currSlice->num_ref_idx_active[LIST_0]; i++) {
    luma_weight_flag_l0 =
        read_u_1("SH: luma_weight_flag_l0", currStream, &p_Dec->UsedBits);
    ...
    if (luma_weight_flag_l0) {
      ...
      currSlice->wp_offset[LIST_0][i][0] = currSlice->wp_offset[LIST_0][i][0]
                                           << (p_Vid->bitdepth_luma - 8);
    } else {
      currSlice->wp_weight[LIST_0][i][0] = 1
    ...
    if (active_sps->chroma_format_idc != 0) {
      chroma_weight_flag_l0 =
          read_u_1("SH: chroma_weight_flag_l0", currStream, &p_Dec->UsedBits);
  ...
  if ((currSlice->slice_type == B_SLICE) &&
    ...
    for (i = 0; i < currSlice->num_ref_idx_active[LIST_1]; i++) {
      luma_weight_flag_l1 =
          read_u_1("SH: luma_weight_flag_l1", currStream, &p_Dec->UsedBits);
      ...
      if (luma_weight_flag_l1) {
        ...
        currSlice->wp_offset[LIST_1][i][0] = currSlice->wp_offset[LIST_1][i][0]
                                             << (p_Vid->bitdepth_luma - 8);
      } else {
        currSlice->wp_weight[LIST_1][i][0] =
            1 << currSlice->luma_log2_weight_denom;
        currSlice->wp_offset[LIST_1][i][0] = 0;
      }

      if (active_sps->chroma_format_idc != 0) {
        chroma_weight_flag_l1 =
            read_u_1("SH: chroma_weight_flag_l1", currStream, &p_Dec->UsedBits);
...
#if (MVC_EXTENSION_ENABLE)
  if (pSlice->idr_flag || (pSlice->svc_extension_flag == 0 &&
                           pSlice->NaluHeaderMVCExt.non_idr_flag == 0))
#else
  if (pSlice->idr_flag)
#endif
  {
    pSlice->no_output_of_prior_pics_flag = read_u_1(
        "SH: no_output_of_prior_pics_flag", currStream, &p_Dec->UsedBits);
    ...
    pSlice->adaptive_ref_pic_buffering_flag = read_u_1(
        "SH: adaptive_ref_pic_buffering_flag", currStream, &p_Dec->UsedBits);
    if (pSlice->adaptive_ref_pic_buffering_flag) {
      ...
      do {
        ...
        val = tmp_drpm->memory_management_control_operation =
            read_ue_v("SH: memory_management_control_operation", currStream,
                      &p_Dec->UsedBits);

        if ((val == 1) || (val == 3)) {
          tmp_drpm->difference_of_pic_nums_minus1 =
              read_ue_v("SH: difference_of_pic_nums_minus1", currStream,
                        &p_Dec->UsedBits);
        }
        if (val == 2) {
        ...
        if ((val == 3) || (val == 6)) {
          tmp_drpm->long_term_frame_idx = read_ue_v(
              "SH: long_term_frame_idx", currStream, &p_Dec->UsedBits);
        }
        if (val == 4) {
          tmp_drpm->max_long_term_frame_idx_plus1 = read_ue_v(
              "SH: max_long_term_pic_idx_plus1", currStream, &p_Dec->UsedBits);
        }
    ...
    case 1:  // POC MODE 1
      ...
      if (pSlice->idr_flag) {
        ...
        if (pSlice->frame_num <
          ...
          p_Vid->FrameNumOffset =
              p_Vid->PreviousFrameNumOffset + p_Vid->max_frame_num;
        } else {
      ...
      if (pSlice->AbsFrameNum) {
        ...
        p_Vid->FrameNumInPicOrderCntCycle =
            (pSlice->AbsFrameNum - 1) %
            active_sps->num_ref_frames_in_pic_order_cnt_cycle;
        p_Vid->ExpectedPicOrderCnt = p_Vid->PicOrderCntCycleCnt *
                                     p_Vid->ExpectedDeltaPerPicOrderCntCycle;
    ...
    case 2:                  // POC MODE 2
      if (pSlice->idr_flag)  // IDR picture
      {
        ...
        if (pSlice->field_pic_flag == 0)
          pSlice->toppoc = pSlice->bottompoc = pSlice->framepoc =
              pSlice->ThisPOC;
