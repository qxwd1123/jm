...
int InterpretSPS(VideoParameters *p_Vid, DataPartition *p,
  ...
  sps->constrained_set0_flag =
      read_u_1("SPS: constrained_set0_flag", s, &p_Dec->UsedBits);
  sps->constrained_set1_flag =
      read_u_1("SPS: constrained_set1_flag", s, &p_Dec->UsedBits);
  sps->constrained_set2_flag =
      read_u_1("SPS: constrained_set2_flag", s, &p_Dec->UsedBits);
  sps->constrained_set3_flag =
      read_u_1("SPS: constrained_set3_flag", s, &p_Dec->UsedBits);
#if (MVC_EXTENSION_ENABLE)
  sps->constrained_set4_flag =
      read_u_1("SPS: constrained_set4_flag", s, &p_Dec->UsedBits);
  sps->constrained_set5_flag =
      read_u_1("SPS: constrained_set5_flag", s, &p_Dec->UsedBits);
  ...
  if (reserved_zero != 0) {
    printf(
        "Warning, reserved_zero flag not equal to 0. Possibly new "
        "constrained_setX flag introduced.\n");
  }

  sps->level_idc = read_u_v(8, "SPS: level_idc", s, &p_Dec->UsedBits);

  sps->seq_parameter_set_id =
      read_ue_v("SPS: seq_parameter_set_id", s, &p_Dec->UsedBits);
  ...
  if ((sps->profile_idc == FREXT_HP) || (sps->profile_idc == FREXT_Hi10P) ||
    ...
    sps->chroma_format_idc =
        read_ue_v("SPS: chroma_format_idc", s, &p_Dec->UsedBits);

    if (sps->chroma_format_idc == YUV444) {
      sps->separate_colour_plane_flag =
          read_u_1("SPS: separate_colour_plane_flag", s, &p_Dec->UsedBits);
    }

    sps->bit_depth_luma_minus8 =
        read_ue_v("SPS: bit_depth_luma_minus8", s, &p_Dec->UsedBits);
    sps->bit_depth_chroma_minus8 =
        read_ue_v("SPS: bit_depth_chroma_minus8", s, &p_Dec->UsedBits);
    // checking;
    ...
    sps->lossless_qpprime_flag =
        read_u_1("SPS: lossless_qpprime_y_zero_flag", s, &p_Dec->UsedBits);

    sps->seq_scaling_matrix_present_flag =
        read_u_1("SPS: seq_scaling_matrix_present_flag", s, &p_Dec->UsedBits);

    if (sps->seq_scaling_matrix_present_flag) {
      n_ScalingList = (sps->chroma_format_idc != YUV444) ? 8 : 12;
      for (i = 0; i < n_ScalingList; i++) {
        sps->seq_scaling_list_present_flag[i] =
            read_u_1("SPS: seq_scaling_list_present_flag", s, &p_Dec->UsedBits);
  ...
  sps->log2_max_frame_num_minus4 =
      read_ue_v("SPS: log2_max_frame_num_minus4", s, &p_Dec->UsedBits);
  sps->pic_order_cnt_type =
      read_ue_v("SPS: pic_order_cnt_type", s, &p_Dec->UsedBits);

  if (sps->pic_order_cnt_type == 0)
    sps->log2_max_pic_order_cnt_lsb_minus4 = read_ue_v(
        "SPS: log2_max_pic_order_cnt_lsb_minus4", s, &p_Dec->UsedBits);
  else if (sps->pic_order_cnt_type == 1) {
    sps->delta_pic_order_always_zero_flag =
        read_u_1("SPS: delta_pic_order_always_zero_flag", s, &p_Dec->UsedBits);
    sps->offset_for_non_ref_pic =
        read_se_v("SPS: offset_for_non_ref_pic", s, &p_Dec->UsedBits);
    sps->offset_for_top_to_bottom_field =
        read_se_v("SPS: offset_for_top_to_bottom_field", s, &p_Dec->UsedBits);
    sps->num_ref_frames_in_pic_order_cnt_cycle = read_ue_v(
        "SPS: num_ref_frames_in_pic_order_cnt_cycle", s, &p_Dec->UsedBits);
    for (i = 0; i < sps->num_ref_frames_in_pic_order_cnt_cycle; i++)
      sps->offset_for_ref_frame[i] =
          read_se_v("SPS: offset_for_ref_frame[i]", s, &p_Dec->UsedBits);
  }
  sps->num_ref_frames = read_ue_v("SPS: num_ref_frames", s, &p_Dec->UsedBits);
  sps->gaps_in_frame_num_value_allowed_flag = read_u_1(
      "SPS: gaps_in_frame_num_value_allowed_flag", s, &p_Dec->UsedBits);
  sps->pic_width_in_mbs_minus1 =
      read_ue_v("SPS: pic_width_in_mbs_minus1", s, &p_Dec->UsedBits);
  sps->pic_height_in_map_units_minus1 =
      read_ue_v("SPS: pic_height_in_map_units_minus1", s, &p_Dec->UsedBits);
  sps->frame_mbs_only_flag =
      read_u_1("SPS: frame_mbs_only_flag", s, &p_Dec->UsedBits);
  if (!sps->frame_mbs_only_flag) {
    sps->mb_adaptive_frame_field_flag =
        read_u_1("SPS: mb_adaptive_frame_field_flag", s, &p_Dec->UsedBits);
  }
  // printf("interlace flags %d %d\n", sps->frame_mbs_only_flag,
  // sps->mb_adaptive_frame_field_flag);
  sps->direct_8x8_inference_flag =
      read_u_1("SPS: direct_8x8_inference_flag", s, &p_Dec->UsedBits);
  sps->frame_cropping_flag =
      read_u_1("SPS: frame_cropping_flag", s, &p_Dec->UsedBits);

  if (sps->frame_cropping_flag) {
    sps->frame_crop_left_offset =
        read_ue_v("SPS: frame_crop_left_offset", s, &p_Dec->UsedBits);
    sps->frame_crop_right_offset =
        read_ue_v("SPS: frame_crop_right_offset", s, &p_Dec->UsedBits);
    sps->frame_crop_top_offset =
        read_ue_v("SPS: frame_crop_top_offset", s, &p_Dec->UsedBits);
    sps->frame_crop_bottom_offset =
        read_ue_v("SPS: frame_crop_bottom_offset", s, &p_Dec->UsedBits);
  }
  sps->vui_parameters_present_flag = (Boolean)read_u_1(
      "SPS: vui_parameters_present_flag", s, &p_Dec->UsedBits);
...
static int InterpretSubsetSPS(VideoParameters *p_Vid, DataPartition *p,
  ...
  *curr_seq_set_id = sps->seq_parameter_set_id;
  ...
  if (subset_sps->Valid || subset_sps->num_views_minus1 >= 0) {
    if (memcmp(&subset_sps->sps, sps,
               sizeof(seq_parameter_set_rbsp_t) - sizeof(int)))
      assert(0);
  ...
  if (is_MVC_profile(subset_sps->sps.profile_idc)) {
    subset_sps->bit_equal_to_one =
        read_u_1("bit_equal_to_one", s, &p_Dec->UsedBits);
    ...
    subset_sps->mvc_vui_parameters_present_flag =
        read_u_1("mvc_vui_parameters_present_flag", s, &p_Dec->UsedBits);
  ...
  additional_extension2_flag =
      read_u_1("additional_extension2_flag", s, &p_Dec->UsedBits);
  ...
  if (subset_sps->sps.Valid) subset_sps->Valid = TRUE;
  ...
  if (sps->vui_parameters_present_flag) {
    ...
    if (sps->vui_seq_parameters.aspect_ratio_info_present_flag) {
      ...
      if (255 == sps->vui_seq_parameters.aspect_ratio_idc) {
        ...
        sps->vui_seq_parameters.sar_height = (unsigned short)read_u_v(
            16, "VUI: sar_height", s, &p_Dec->UsedBits);
      }
    }
    ...
    if (sps->vui_seq_parameters.chroma_location_info_present_flag) {
      sps->vui_seq_parameters.chroma_sample_loc_type_top_field = read_ue_v(
          "VUI: chroma_sample_loc_type_top_field", s, &p_Dec->UsedBits);
      sps->vui_seq_parameters.chroma_sample_loc_type_bottom_field = read_ue_v(
          "VUI: chroma_sample_loc_type_bottom_field", s, &p_Dec->UsedBits);
    }
    ...
    if (sps->vui_seq_parameters.timing_info_present_flag) {
      ...
      sps->vui_seq_parameters.time_scale =
          read_u_v(32, "VUI: time_scale", s, &p_Dec->UsedBits);
    ...
    sps->vui_seq_parameters.nal_hrd_parameters_present_flag =
        read_u_1("VUI: nal_hrd_parameters_present_flag", s, &p_Dec->UsedBits);
    ...
    if (sps->vui_seq_parameters.nal_hrd_parameters_present_flag ||
      ...
      sps->vui_seq_parameters.low_delay_hrd_flag =
          read_u_1("VUI: low_delay_hrd_flag", s, &p_Dec->UsedBits);
    }
...
int ReadHRDParameters(DataPartition *p, hrd_parameters_t *hrd) {
  ...
  for (SchedSelIdx = 0; SchedSelIdx <= hrd->cpb_cnt_minus1; SchedSelIdx++) {
    hrd->bit_rate_value_minus1[SchedSelIdx] =
        read_ue_v("VUI: bit_rate_value_minus1", s, &p_Dec->UsedBits);
    hrd->cpb_size_value_minus1[SchedSelIdx] =
        read_ue_v("VUI: cpb_size_value_minus1", s, &p_Dec->UsedBits);
    hrd->cbr_flag[SchedSelIdx] = read_u_1("VUI: cbr_flag", s, &p_Dec->UsedBits);
  }

  hrd->initial_cpb_removal_delay_length_minus1 = read_u_v(
      5, "VUI: initial_cpb_removal_delay_length_minus1", s, &p_Dec->UsedBits);
  ...
  hrd->time_offset_length =
      read_u_v(5, "VUI: time_offset_length", s, &p_Dec->UsedBits);
...
int InterpretPPS(VideoParameters *p_Vid, DataPartition *p,
  ...
  pps->pic_parameter_set_id =
      read_ue_v("PPS: pic_parameter_set_id", s, &p_Dec->UsedBits);
  pps->seq_parameter_set_id =
      read_ue_v("PPS: seq_parameter_set_id", s, &p_Dec->UsedBits);
  pps->entropy_coding_mode_flag =
      read_u_1("PPS: entropy_coding_mode_flag", s, &p_Dec->UsedBits);
  ...
  pps->bottom_field_pic_order_in_frame_present_flag = read_u_1(
      "PPS: bottom_field_pic_order_in_frame_present_flag", s, &p_Dec->UsedBits);

  pps->num_slice_groups_minus1 =
      read_ue_v("PPS: num_slice_groups_minus1", s, &p_Dec->UsedBits);

  // FMO stuff begins here
  if (pps->num_slice_groups_minus1 > 0) {
    pps->slice_group_map_type =
        read_ue_v("PPS: slice_group_map_type", s, &p_Dec->UsedBits);
    if (pps->slice_group_map_type == 0) {
      for (i = 0; i <= pps->num_slice_groups_minus1; i++)
        pps->run_length_minus1[i] =
            read_ue_v("PPS: run_length_minus1 [i]", s, &p_Dec->UsedBits);
    } else if (pps->slice_group_map_type == 2) {
      for (i = 0; i < pps->num_slice_groups_minus1; i++) {
        //! JVT-F078: avoid reference of SPS by using ue(v) instead of u(v)
        pps->top_left[i] = read_ue_v("PPS: top_left [i]", s, &p_Dec->UsedBits);
        pps->bottom_right[i] =
            read_ue_v("PPS: bottom_right [i]", s, &p_Dec->UsedBits);
      }
    } else if (pps->slice_group_map_type == 3 ||
               pps->slice_group_map_type == 4 ||
               pps->slice_group_map_type == 5) {
      pps->slice_group_change_direction_flag = read_u_1(
          "PPS: slice_group_change_direction_flag", s, &p_Dec->UsedBits);
      pps->slice_group_change_rate_minus1 =
          read_ue_v("PPS: slice_group_change_rate_minus1", s, &p_Dec->UsedBits);
      ...
      pps->pic_size_in_map_units_minus1 =
          read_ue_v("PPS: pic_size_in_map_units_minus1", s, &p_Dec->UsedBits);
  ...
  pps->num_ref_idx_l0_default_active_minus1 = read_ue_v(
      "PPS: num_ref_idx_l0_default_active_minus1", s, &p_Dec->UsedBits);
  pps->num_ref_idx_l1_default_active_minus1 = read_ue_v(
      "PPS: num_ref_idx_l1_default_active_minus1", s, &p_Dec->UsedBits);
  pps->weighted_pred_flag =
      read_u_1("PPS: weighted_pred_flag", s, &p_Dec->UsedBits);
  pps->weighted_bipred_idc =
      read_u_v(2, "PPS: weighted_bipred_idc", s, &p_Dec->UsedBits);
  pps->pic_init_qp_minus26 =
      read_se_v("PPS: pic_init_qp_minus26", s, &p_Dec->UsedBits);
  pps->pic_init_qs_minus26 =
      read_se_v("PPS: pic_init_qs_minus26", s, &p_Dec->UsedBits);

  pps->chroma_qp_index_offset =
      read_se_v("PPS: chroma_qp_index_offset", s, &p_Dec->UsedBits);

  pps->deblocking_filter_control_present_flag = read_u_1(
      "PPS: deblocking_filter_control_present_flag", s, &p_Dec->UsedBits);
  pps->constrained_intra_pred_flag =
      read_u_1("PPS: constrained_intra_pred_flag", s, &p_Dec->UsedBits);
  pps->redundant_pic_cnt_present_flag =
      read_u_1("PPS: redundant_pic_cnt_present_flag", s, &p_Dec->UsedBits);
                     ...
                     s->bitstream_length))  // more_data_in_rbsp()
  {
    ...
    pps->transform_8x8_mode_flag =
        read_u_1("PPS: transform_8x8_mode_flag", s, &p_Dec->UsedBits);
    pps->pic_scaling_matrix_present_flag =
        read_u_1("PPS: pic_scaling_matrix_present_flag", s, &p_Dec->UsedBits);
    ...
    if (pps->pic_scaling_matrix_present_flag) {
      ...
      for (i = 0; i < n_ScalingList; i++) {
        pps->pic_scaling_list_present_flag[i] =
            read_u_1("PPS: pic_scaling_list_present_flag", s, &p_Dec->UsedBits);
    ...
    pps->second_chroma_qp_index_offset =
        read_se_v("PPS: second_chroma_qp_index_offset", s, &p_Dec->UsedBits);
  } else {
...
void ProcessSubsetSPS(VideoParameters *p_Vid, NALU_t *nalu) {
  ...
  } else if (subset_sps->num_views_minus1 == 1 &&
             (subset_sps->view_id[0] != 0 || subset_sps->view_id[1] != 1)) {
...
void reset_format_info(seq_parameter_set_rbsp_t *sps, VideoParameters *p_Vid,
  ...
  if (sps->frame_cropping_flag) {
    ...
    crop_right =
        SubWidthC[sps->chroma_format_idc] * sps->frame_crop_right_offset;
    crop_top = SubHeightC[sps->chroma_format_idc] *
               (2 - sps->frame_mbs_only_flag) * sps->frame_crop_top_offset;
  ...
  if (sps->frame_cropping_flag) {
    ...
    crop_bottom =
        (2 - sps->frame_mbs_only_flag) * sps->frame_crop_bottom_offset;
  } else {
    crop_left = crop_right = crop_top = crop_bottom = 0;
  }
  ...
  if (p_Vid->first_sps == TRUE) {
    ...
    if (!p_Inp->bDisplayDecParams) {
      ...
      fprintf(stdout, "Image Format : %dx%d (%dx%d)\n", source->width[0],
              source->height[0], p_Vid->width, p_Vid->height);
      ...
      fprintf(stdout, "(%d:%d:%d)\n", source->bit_depth[0],
              source->bit_depth[1], source->bit_depth[2]);
      fprintf(stdout,
              "----------------------------------------------------------------"
              "----------\n");
    }
...
static void setup_layer_info(VideoParameters *p_Vid,
                             seq_parameter_set_rbsp_t *sps,
                             LayerParameters *p_Lps) {
...
static void set_coding_par(seq_parameter_set_rbsp_t *sps,
  ...
  cps->FrameHeightInMbs =
      (2 - sps->frame_mbs_only_flag) * cps->PicHeightInMapUnits;
  ...
  if (cps->bitdepth_luma > cps->bitdepth_chroma ||
      sps->chroma_format_idc == YUV400)
  ...
  if (sps->chroma_format_idc != YUV400) {
    ...
    cps->mb_size[1][0] = cps->mb_size[2][0] = cps->mb_cr_size_x =
        (sps->chroma_format_idc == YUV420 || sps->chroma_format_idc == YUV422)
            ? 8
            : 16;
    cps->mb_size[1][1] = cps->mb_size[2][1] = cps->mb_cr_size_y =
        (sps->chroma_format_idc == YUV444 || sps->chroma_format_idc == YUV422)
            ? 16
            : 8;
  ...
  cps->mb_size_shift[0][0] = cps->mb_size_shift[0][1] =
      CeilLog2_sf(cps->mb_size[0][0]);
  cps->mb_size_shift[1][0] = cps->mb_size_shift[2][0] =
      CeilLog2_sf(cps->mb_size[1][0]);
  cps->mb_size_shift[1][1] = cps->mb_size_shift[2][1] =
      CeilLog2_sf(cps->mb_size[1][1]);
  ...
  if (p_Vid->active_sps != sps) {
    ...
    if (/*p_Vid->last_pic_width_in_mbs_minus1 !=
        ...
        (p_Vid->last_profile_idc != p_Vid->active_sps->profile_idc &&
         ...
         !p_Vid->p_Dpb_layer[0]
              ->init_done /*&& is_BL_profile(p_Vid->last_profile_idc)*/)) {
      // init_frext(p_Vid);
      ...
      init_dpb(p_Vid, p_Vid->p_Dpb_layer[0], 1);
               ...
               (is_MVC_profile(p_Vid->last_profile_idc) ||
      ...
      // for now lets re_init both buffers. Later, we should only re_init
      // appropriate one Note that we seem to be doing this for every frame
    ...
    // p_Vid->p_Dpb_layer[0]->num_ref_frames =
    // p_Vid->active_sps->num_ref_frames; p_Vid->p_Dpb_layer[1]->num_ref_frames
    ...
    // for now lets init both buffers. Later, we should only re_init appropriate
    // one
    // init_dpb(p_Vid, p_Vid->p_Dpb_layer[0], 1);
...
void UseParameterSet(Slice *currSlice) {
  ...
  if (pps->Valid != TRUE)
    printf(
        "Trying to use an invalid (uninitialized) Picture Parameter Set with "
        "ID %d, expect the unexpected...\n",
        PicParsetId);
#if (MVC_EXTENSION_ENABLE)
  if (currSlice->svc_extension_flag == -1) {
    if (sps->Valid != TRUE)
      printf(
          "PicParset %d references an invalid (uninitialized) Sequence "
    ...
    if (p_Vid->SubsetSeqParSet[pps->seq_parameter_set_id].Valid != TRUE)
      printf(
          "PicParset %d references an invalid (uninitialized) Subset Sequence "
          "Parameter Set with ID %d, expect the unexpected...\n",
          PicParsetId, (int)pps->seq_parameter_set_id);
  }
#else
  if (sps->Valid != TRUE)
    printf(
        "PicParset %d references an invalid (uninitialized) Sequence Parameter "
        "Set with ID %d, expect the unexpected...\n",
        PicParsetId, (int)pps->seq_parameter_set_id);
#endif
...
void seq_parameter_set_mvc_extension(
  ...
  subset_sps->num_views_minus1 =
      read_ue_v("num_views_minus1", s, &p_Dec->UsedBits);
  ...
  for (i = 1; i < num_views; i++) {
    subset_sps->num_anchor_refs_l0[i] =
        read_ue_v("num_anchor_refs_l0", s, &p_Dec->UsedBits);
    if (subset_sps->num_anchor_refs_l0[i] > 0) {
      ...
      for (j = 0; j < subset_sps->num_anchor_refs_l0[i]; j++)
        subset_sps->anchor_ref_l0[i][j] =
            read_ue_v("anchor_ref_l0", s, &p_Dec->UsedBits);
    }

    subset_sps->num_anchor_refs_l1[i] =
        read_ue_v("num_anchor_refs_l1", s, &p_Dec->UsedBits);
    if (subset_sps->num_anchor_refs_l1[i] > 0) {
      ...
      for (j = 0; j < subset_sps->num_anchor_refs_l1[i]; j++)
        subset_sps->anchor_ref_l1[i][j] =
            read_ue_v("anchor_ref_l1", s, &p_Dec->UsedBits);
    }
  }
  for (i = 1; i < num_views; i++) {
    subset_sps->num_non_anchor_refs_l0[i] =
        read_ue_v("num_non_anchor_refs_l0", s, &p_Dec->UsedBits);
    if (subset_sps->num_non_anchor_refs_l0[i] > 0) {
      ...
      for (j = 0; j < subset_sps->num_non_anchor_refs_l0[i]; j++)
        subset_sps->non_anchor_ref_l0[i][j] =
            read_ue_v("non_anchor_ref_l0", s, &p_Dec->UsedBits);
    }
    subset_sps->num_non_anchor_refs_l1[i] =
        read_ue_v("num_non_anchor_refs_l1", s, &p_Dec->UsedBits);
    if (subset_sps->num_non_anchor_refs_l1[i] > 0) {
      ...
      for (j = 0; j < subset_sps->num_non_anchor_refs_l1[i]; j++)
        subset_sps->non_anchor_ref_l1[i][j] =
            read_ue_v("non_anchor_ref_l1", s, &p_Dec->UsedBits);
    }
  }
  subset_sps->num_level_values_signalled_minus1 =
      read_ue_v("num_level_values_signalled_minus1", s, &p_Dec->UsedBits);
  ...
  for (i = 0; i <= subset_sps->num_level_values_signalled_minus1; i++) {
    ...
    subset_sps->num_applicable_ops_minus1[i] =
        read_ue_v("num_applicable_ops_minus1", s, &p_Dec->UsedBits);
    if (subset_sps->num_applicable_ops_minus1[i] >= 0) {
      if ((subset_sps->applicable_op_temporal_id[i] = (int *)calloc(
               1 + subset_sps->num_applicable_ops_minus1[i], sizeof(int))) ==
          NULL)
        no_mem_exit(
            "init_subset_seq_parameter_set: "
            "subset_sps->applicable_op_temporal_id[i]");
      if ((subset_sps->applicable_op_num_target_views_minus1[i] = (int *)calloc(
               1 + subset_sps->num_applicable_ops_minus1[i], sizeof(int))) ==
          NULL)
        no_mem_exit(
            "init_subset_seq_parameter_set: "
            "subset_sps->applicable_op_num_target_views_minus1[i]");
      if ((subset_sps->applicable_op_target_view_id[i] = (int **)calloc(
               1 + subset_sps->num_applicable_ops_minus1[i], sizeof(int *))) ==
          NULL)
        no_mem_exit(
            "init_subset_seq_parameter_set: "
            "subset_sps->applicable_op_target_view_id[i]");
      if ((subset_sps->applicable_op_num_views_minus1[i] = (int *)calloc(
               1 + subset_sps->num_applicable_ops_minus1[i], sizeof(int))) ==
          NULL)
        no_mem_exit(
            "init_subset_seq_parameter_set: "
      ...
      for (j = 0; j <= subset_sps->num_applicable_ops_minus1[i]; j++) {
        ...
        subset_sps->applicable_op_temporal_id[i][j] =
            read_u_v(3, "applicable_op_temporal_id", s, &p_Dec->UsedBits);
        subset_sps->applicable_op_num_target_views_minus1[i][j] = read_ue_v(
            "applicable_op_num_target_views_minus1", s, &p_Dec->UsedBits);
        if (subset_sps->applicable_op_num_target_views_minus1[i][j] >= 0) {
          ...
          for (k = 0;
            ...
            subset_sps->applicable_op_target_view_id[i][j][k] =
                read_ue_v("applicable_op_target_view_id", s, &p_Dec->UsedBits);
        }
        subset_sps->applicable_op_num_views_minus1[i][j] =
            read_ue_v("applicable_op_num_views_minus1", s, &p_Dec->UsedBits);
      }
    }
  }
}
...
void hrd_parameters(MVCVUI_t *pMVCVUI, Bitstream *s) {
  ...
  pMVCVUI->cpb_cnt_minus1 =
      (char)read_ue_v("cpb_cnt_minus1", s, &p_Dec->UsedBits);
  assert(pMVCVUI->cpb_cnt_minus1 <= 31);
  pMVCVUI->bit_rate_scale =
      (char)read_u_v(4, "bit_rate_scale", s, &p_Dec->UsedBits);
  pMVCVUI->cpb_size_scale =
      (char)read_u_v(4, "cpb_size_scale", s, &p_Dec->UsedBits);
  for (i = 0; i <= pMVCVUI->cpb_cnt_minus1; i++) {
    pMVCVUI->bit_rate_value_minus1[i] =
        read_ue_v("bit_rate_value_minus1", s, &p_Dec->UsedBits);
    pMVCVUI->cpb_size_value_minus1[i] =
        read_ue_v("cpb_size_value_minus1", s, &p_Dec->UsedBits);
    pMVCVUI->cbr_flag[i] = (char)read_u_1("cbr_flag", s, &p_Dec->UsedBits);
  }
  pMVCVUI->initial_cpb_removal_delay_length_minus1 = (char)read_u_v(
      5, "initial_cpb_removal_delay_length_minus1", s, &p_Dec->UsedBits);
  pMVCVUI->cpb_removal_delay_length_minus1 =
      (char)read_u_v(5, "cpb_removal_delay_length_minus1", s, &p_Dec->UsedBits);
  pMVCVUI->dpb_output_delay_length_minus1 =
      (char)read_u_v(5, "dpb_output_delay_length_minus1", s, &p_Dec->UsedBits);
  pMVCVUI->time_offset_length =
      (char)read_u_v(5, "time_offset_length", s, &p_Dec->UsedBits);
}

void mvc_vui_parameters_extension(MVCVUI_t *pMVCVUI, Bitstream *s) {
  int i, j, iNumOps;

  pMVCVUI->num_ops_minus1 =
      read_ue_v("vui_mvc_num_ops_minus1", s, &p_Dec->UsedBits);
  ...
  if (iNumOps > 0) {
    ...
    for (i = 0; i < iNumOps; i++) {
      pMVCVUI->temporal_id[i] =
          (char)read_u_v(3, "vui_mvc_temporal_id", s, &p_Dec->UsedBits);
      pMVCVUI->num_target_output_views_minus1[i] = read_ue_v(
          "vui_mvc_num_target_output_views_minus1", s, &p_Dec->UsedBits);
      ...
      for (j = 0; j <= pMVCVUI->num_target_output_views_minus1[i]; j++)
        pMVCVUI->view_id[i][j] =
            read_ue_v("vui_mvc_view_id", s, &p_Dec->UsedBits);
      pMVCVUI->timing_info_present_flag[i] = (char)read_u_1(
          "vui_mvc_timing_info_present_flag", s, &p_Dec->UsedBits);
      if (pMVCVUI->timing_info_present_flag[i]) {
        pMVCVUI->num_units_in_tick[i] =
            read_u_v(32, "vui_mvc_num_units_in_tick", s, &p_Dec->UsedBits);
        pMVCVUI->time_scale[i] =
            read_u_v(32, "vui_mvc_time_scale", s, &p_Dec->UsedBits);
        pMVCVUI->fixed_frame_rate_flag[i] = (char)read_u_1(
            "vui_mvc_fixed_frame_rate_flag", s, &p_Dec->UsedBits);
      }
      pMVCVUI->nal_hrd_parameters_present_flag[i] = (char)read_u_1(
          "vui_mvc_nal_hrd_parameters_present_flag", s, &p_Dec->UsedBits);
      if (pMVCVUI->nal_hrd_parameters_present_flag[i])
        hrd_parameters(pMVCVUI, s);
      pMVCVUI->vcl_hrd_parameters_present_flag[i] = (char)read_u_1(
          "vcl_hrd_parameters_present_flag", s, &p_Dec->UsedBits);
      if (pMVCVUI->vcl_hrd_parameters_present_flag[i])
        hrd_parameters(pMVCVUI, s);
      if (pMVCVUI->nal_hrd_parameters_present_flag[i] ||
          pMVCVUI->vcl_hrd_parameters_present_flag[i])
        pMVCVUI->low_delay_hrd_flag[i] =
            (char)read_u_1("vui_mvc_low_delay_hrd_flag", s, &p_Dec->UsedBits);
      pMVCVUI->pic_struct_present_flag[i] = (char)read_u_1(
          "vui_mvc_pic_struct_present_flag", s, &p_Dec->UsedBits);
    }
  }
}
...
int GetBaseViewId(VideoParameters *p_Vid,
  ...
  *subset_sps = NULL;
  ...
  if (i < MAXSPS) *subset_sps = curr_subset_sps;
  return iBaseViewId;
}
...
void get_max_dec_frame_buf_size(seq_parameter_set_rbsp_t *sps) {
  ...
  switch (sps->level_idc) {
    case 0:
      // if there is no level defined, we expect experimental usage and return a
      // DPB size of 16
      size = 16 * pic_size_mb;
