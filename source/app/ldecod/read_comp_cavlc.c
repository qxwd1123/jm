...
extern void read_delta_quant(SyntaxElement *currSE, DataPartition *dP,
 ...
 ************************************************************************
 */
void read_coeff_4x4_CAVLC(Macroblock *currMB, int block_type, int i, int j,
  ...
  *number_coefficients = numcoeff;
  ...
  if (numcoeff) {
    ...
    for (k = numcoeff - 1 - numtrailingones; k >= 0; k--) {
      ...
      if (abslevel == 1) ++numones;

      // update VLC table
      if (abslevel > incVlc[vlcnum]) ++vlcnum;

      if (k == numcoeff - 1 - numtrailingones && abslevel > 3) vlcnum = 2;
    }
 ...
 ************************************************************************
 */
void read_coeff_4x4_CAVLC_444(Macroblock *currMB, int block_type, int i, int j,
                              int levarr[16], int runarr[16],
                              int *number_coefficients) {
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  switch (block_type) {
    ...
    case CB:
      ...
      dptype = ((currMB->is_intra_block == TRUE)) ? SE_LUM_AC_INTRA
                                                  : SE_LUM_AC_INTER;
...
************************************************************************
*/
static void read_comp_coeff_4x4_CAVLC(Macroblock *currMB, ColorPlane pl,
      ...
      if (cbp & (1 << b8))  // test if the block contains any coefficients
      {
        for (j = block_y4; j < block_y4 + 8; j += BLOCK_SIZE) {
          for (i = block_x4; i < block_x4 + 8; i += BLOCK_SIZE) {
            currSlice->read_coeff_4x4_CAVLC(currMB, cur_context, i >> 2, j >> 2,
...
************************************************************************
*/
static void read_comp_coeff_4x4_CAVLC_ls(Macroblock *currMB, ColorPlane pl,
                                         int (*InvLevelScale4x4)[4], int qp_per,
                                         int cbp, byte **nzcoeff) {
  int block_y, block_x, b8;
  int i, j, k;
      ...
      if (cbp & (1 << b8)) /* are there any coeff in current block at all */
      {
        for (j = block_y; j < block_y + 2; ++j) {
          for (i = block_x; i < block_x + 2; ++i) {
            currSlice->read_coeff_4x4_CAVLC(currMB, cur_context, i, j, levarr,
                                            runarr, &numcoeff);
...
************************************************************************
*/
static void read_comp_coeff_8x8_CAVLC(Macroblock *currMB, ColorPlane pl,
...
************************************************************************
*/
static void read_comp_coeff_8x8_CAVLC_ls(Macroblock *currMB, ColorPlane pl,
                                         int (*InvLevelScale8x8)[8], int qp_per,
                                         int cbp, byte **nzcoeff) {
  int block_y, block_x, b4, b8;
  int i, j, k;
...
static void read_CBP_and_coeffs_from_NAL_CAVLC_400(Macroblock *currMB) {
  ...
  } else  // read DC coeffs for new intra modes
  {
    ...
    if (!currMB->dpl_flag) {
      ...
      currSlice->read_coeff_4x4_CAVLC(currMB, LUMA_INTRA16x16DC, 0, 0, levarr,
                                      runarr, &numcoeff);
  ...
  if (cbp) {
    ...
    {
      currMB->read_comp_coeff_4x4_CAVLC(currMB, PLANE_Y, InvLevelScale4x4,
                                        qp_per, cbp,
                                        p_Vid->nz_coeff[mb_nr][PLANE_Y]);
    } else  // 8x8 transform
    {
      currMB->read_comp_coeff_8x8_CAVLC(currMB, PLANE_Y, InvLevelScale8x8,
                                        qp_per, cbp,
                                        p_Vid->nz_coeff[mb_nr][PLANE_Y]);
    }
  } else {
  ...
  if (cbp > 15) {
    for (ll = 0; ll < 3; ll += 2) {
      ...
      uv = ll >> 1;
      {
        ...
        //===================== CHROMA DC YUV422 ======================
        currSlice->read_coeff_4x4_CAVLC(currMB, CHROMA_DC, 0, 0, levarr, runarr,
                                        &numcoeff);
        coef_ctr = -1;
        for (k = 0; k < numcoeff; ++k) {
          if (levarr[k] != 0) {
      ...
      for (b8 = 0; b8 < p_Vid->num_blk8x8_uv; ++b8) {
        ...
        InvLevelScale4x4 =
            intra
                ...
                : currSlice
                      ->InvLevelScale4x4_Inter[PLANE_U + uv][qp_rem_uv[uv]];

        for (b4 = 0; b4 < 4; ++b4) {
          i = cofuv_blk_x[1][b8][b4];
          j = cofuv_blk_y[1][b8][b4];

          currSlice->read_coeff_4x4_CAVLC(currMB, CHROMA_AC, i + 2 * uv, j + 4,
                                          levarr, runarr, &numcoeff);
          coef_ctr = 0;
          ...
          for (k = 0; k < numcoeff; ++k) {
            if (levarr[k] != 0) {
              ...
              currSlice->cof[PLANE_U + uv][(j << 2) + j0][(i << 2) + i0] =
                  rshift_rnd_sf((levarr[k] * InvLevelScale4x4[j0][i0])
                                    << qp_per_uv[uv],
                                4);
              // currSlice->fcf[PLANE_U + uv][(j<<2) + j0][(i<<2) + i0] =
              // levarr[k];
            }
          }
        }
...
static void read_CBP_and_coeffs_from_NAL_CAVLC_444(Macroblock *currMB) {
  ...
  for (uv = PLANE_U; uv <= PLANE_V; ++uv) {
    ...
    {
      if (uv == PLANE_U)
        currSlice->read_coeff_4x4_CAVLC(currMB, CB_INTRA16x16DC, 0, 0, levarr,
                                        runarr, &numcoeff);
      else
        currSlice->read_coeff_4x4_CAVLC(currMB, CR_INTRA16x16DC, 0, 0, levarr,
                                        runarr, &numcoeff);
      ...
      for (k = 0; k < numcoeff; ++k) {
        if (levarr[k] != 0)  // leave if level == 0
        {
          ...
          i0 = pos_scan4x4[coef_ctr][0];
          j0 = pos_scan4x4[coef_ctr][1];
          currSlice->cof[uv][j0 << 2][i0 << 2] =
              levarr[k];  // add new intra DC coeff
          // currSlice->fcf[uv][j0<<2][i0<<2] = levarr[k];// add new intra DC
          // coeff
        }  // if leavarr[k]
      }    // k loop
    ...
    InvLevelScale4x4 =
        intra ? currSlice->InvLevelScale4x4_Intra[uv][qp_rem_uv[uv]]
              : currSlice->InvLevelScale4x4_Inter[uv][qp_rem_uv[uv]];
    InvLevelScale8x8 =
        intra ? currSlice->InvLevelScale8x8_Intra[uv][qp_rem_uv[uv]]
              : currSlice->InvLevelScale8x8_Inter[uv][qp_rem_uv[uv]];

    if (!currMB->luma_transform_size_8x8_flag)  // 4x4 transform
    {
      currMB->read_comp_coeff_4x4_CAVLC(currMB, (ColorPlane)(uv),
                                        InvLevelScale4x4, qp_per_uv[uv], cbp,
                                        p_Vid->nz_coeff[mb_nr][uv]);
    } else  // 8x8 transform
    {
      currMB->read_comp_coeff_8x8_CAVLC(currMB, (ColorPlane)(uv),
...
static void read_CBP_and_coeffs_from_NAL_CAVLC_420(Macroblock *currMB) {
  ...
  if (cbp > 15) {
    for (ll = 0; ll < 3; ll += 2) {
      ...
      currSlice->cofu[0] = currSlice->cofu[1] = currSlice->cofu[2] =
          currSlice->cofu[3] = 0;
      coef_ctr = -1;
