...
static void get_db_strength(VideoParameters *p_Vid, StorablePicture *p,
                            int MbQAddr);
...
extern void get_strength_ver_MBAff(byte *Strength, Macroblock *MbQ, int edge,
                                   int mvlimit, StorablePicture *p);
extern void get_strength_hor_MBAff(byte *Strength, Macroblock *MbQ, int edge,
                                   int mvlimit, StorablePicture *p);
...
static void DeblockMb(VideoParameters *p_Vid, StorablePicture *p, int MbQAddr) {
  ...
  if (MbQ->DFDisableIdc == 1) {
    ...
    if (p->mb_aff_frame_flag == 1) CheckAvailabilityOfNeighborsMBAFF(MbQ);
    ...
    for (edge = 0; edge < 4; ++edge) {
      ...
      if (edge || filterTopMbEdgeFlag) {
            ...
            MbQ->mixedModeEdgeFlag)  // currSlice->mixedModeEdgeFlag)
        {
          ...
          MbQ->DeblockCall = 2;
          get_strength_hor_MBAff(Strength, MbQ, MB_BLOCK_SIZE, mvlimit,
...
static void perform_db(VideoParameters *p_Vid, StorablePicture *p,
                       int MbQAddr) {
  Macroblock *MbQ = &(p_Vid->mb_data[MbQAddr]);  // current Mb
  ...
  if (MbQ->DFDisableIdc == 1) {
    ...
    for (edge = 0; edge < 4; ++edge) {
      ...
      if (edge || filterLeftMbEdgeFlag) {
        ...
        if (Strength[0] != 0 || Strength[1] != 0 || Strength[2] != 0 ||
            Strength[3] !=
                0)  // only if one of the 4 first Strength bytes is != 0
        {
          if (filterNon8x8LumaEdgesFlag[edge]) {
            p_Vid->EdgeLoopLumaVer(PLANE_Y, imgY, Strength, MbQ, edge << 2);
            if (currSlice->chroma444_not_separate) {
