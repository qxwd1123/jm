...
static inline byte getfbyte(ANNEXB_t *annex_b) {
  if (0 == annex_b->bytesinbuffer) {
    if (0 == getChunk(annex_b)) return 0;
  }
...
int get_annex_b_NALU(VideoParameters *p_Vid, NALU_t *nalu, ANNEXB_t *annex_b) {
  ...
  if (annex_b->nextstartcodebytes != 0) {
    ...
    while (!annex_b->is_eof) {
      pos++;
      if ((*(pBuf++) = getfbyte(annex_b)) != 0) break;
    }
  }
  ...
  if (*(pBuf - 1) != 1 || pos < 3) {
    printf(
        "get_annex_b_NALU: no Start Code at the beginning of the NALU, return "
        "-1\n");
    return -1;
  }
  ...
  if (!annex_b->IsFirstByteStreamNALU && LeadingZero8BitsCount > 0) {
    printf(
        "get_annex_b_NALU: The leading_zero_8bits syntax can only be present "
  ...
  while (!StartCodeFound) {
    if (annex_b->is_eof == TRUE) {
      ...
      // printf ("get_annex_b_NALU, eof case: pos %d nalu->len %d,
  ...
  if (info3 == 1)  // if the detected start code is 00 00 01,
                   // trailing_zero_8bits is sure not to be present
  {
    pBuf -= 5;
    while (*(pBuf--) == 0) pos--;
  ...
  // printf ("get_annex_b_NALU, regular case: pos %d nalu->len %d,
  // nalu->reference_idc %d, nal_unit_type %d \n", pos, nalu->len,
  // nalu->nal_reference_idc, nalu->nal_unit_type);
#if TRACE
  fprintf(p_Dec->p_trace,
