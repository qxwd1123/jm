...
static void gen_pic_list_from_frame_interview_list(
    ...
    StorablePicture **list, char *list_size) {
  int i;
...
void init_lists_p_slice_mvc(Slice *currSlice) {
  ...
  if (currSlice->svc_extension_flag == 0) {
    ...
    if (currSlice->structure == FRAME) {
      append_interview_list(p_Vid->p_Dpb_layer[1], 0, 0,
      ...
      append_interview_list(p_Vid->p_Dpb_layer[1], currSlice->structure, 0,
                            currSlice->fs_listinterview0,
                            &currSlice->listinterviewidx0, currPOC,
                            curr_view_id, anchor_pic_flag);
  ...
  if (currSlice->svc_extension_flag == 0) {
    ...
    if (currSlice->structure == FRAME) {
      append_interview_list(p_Vid->p_Dpb_layer[1], 0, 0,
                            ...
                            &currSlice->listinterviewidx0, currPOC,
                            curr_view_id, anchor_pic_flag);
      append_interview_list(p_Vid->p_Dpb_layer[1], 0, 1,
      ...
      gen_pic_list_from_frame_interview_list(
          ...
          currSlice->listinterviewidx0, currSlice->listX[0],
          &currSlice->listXsize[0]);
      append_interview_list(p_Vid->p_Dpb_layer[1], currSlice->structure, 1,
                            currSlice->fs_listinterview1,
                            &currSlice->listinterviewidx1, currPOC,
                            curr_view_id, anchor_pic_flag);
      gen_pic_list_from_frame_interview_list(
...
static void reorder_interview(VideoParameters *p_Vid, Slice *currSlice,
  ...
  if (picLX) {
    ...
    for (cIdx = *refIdxLX; cIdx <= num_ref_idx_lX_active_minus1 + 1; cIdx++) {
      if ((GetViewIdx(p_Vid, RefPicListX[cIdx]->view_id) != targetViewID) ||
...
void reorder_ref_pic_list_mvc(Slice *currSlice, int cur_list, int **anchor_ref,
  ...
  for (i = 0; modification_of_pic_nums_idc[i] != 3; i++) {
    ...
    } else if (modification_of_pic_nums_idc[i] ==
               2)  //(modification_of_pic_nums_idc[i] == 2)
    {
      reorder_long_term(currSlice, currSlice->listX[cur_list],
                        num_ref_idx_lX_active_minus1, long_term_pic_idx[i],
                        &refIdxLX, view_id);
    } else {
      if (modification_of_pic_nums_idc[i] ==
          4)  //(modification_of_pic_nums_idc[i] == 4)
      {
  ...
  if ((currSlice->slice_type != I_SLICE) &&
    ...
    if (p_Vid->no_reference_picture ==
      ...
      if (p_Vid->non_conforming_stream)
        ...
        error(
            "RefPicList0[ num_ref_idx_l0_active_minus1 ] in MVC layer is equal "
            "to 'no reference picture', invalid bitstream",
            500);
    }
    // that's a definition
