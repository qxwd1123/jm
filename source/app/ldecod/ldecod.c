...
static void alloc_video_params(VideoParameters **p_Vid) {
  ...
  for (i = 0; i < MAX_NUM_DPB_LAYERS; i++) {
    ...
    if (((*p_Vid)->p_EncodePar[i] =
             (CodingParameters *)calloc(1, sizeof(CodingParameters))) == NULL)
      no_mem_exit("alloc_video_params:p_Vid->p_EncodePar[i]");
    ((*p_Vid)->p_EncodePar[i])->layer_id = i;
    if (((*p_Vid)->p_LayerPar[i] =
             (LayerParameters *)calloc(1, sizeof(LayerParameters))) == NULL)
  ...
  if (p_Vid != NULL) {
    ...
    if (p_Vid->ppSliceList) {
      ...
      for (i = 0; i < p_Vid->iNumOfSlicesAllocated; i++)
        if (p_Vid->ppSliceList[i]) free_slice(p_Vid->ppSliceList[i]);
...
#if ENABLE_DEC_STATS
  if ((p_Vid->dec_stats =
           (DecStatParameters *)malloc(sizeof(DecStatParameters))) == NULL)
 ...
 */
static void Report(VideoParameters *p_Vid) {
  static const char yuv_formats[4][4] = {
    { "400" }, { "420" }, { "422" }, { "444" }
  };
  ...
  if ((p_log = fopen(string, "r")) == 0)  // check if file exist
  {
    ...
    } else  // Create header to new file
    {
      fprintf(p_log,
              ...
              "----------------------------------------------------\n");
      fprintf(p_log,
              "|  Decoder statistics. This file is made first time, later runs "
              "are appended               |\n");
      fprintf(p_log,
              " ---------------------------------------------------------------"
              "---------------------------------------------------- \n");
...
Slice *malloc_slice(InputParameters *p_Inp, VideoParameters *p_Vid) {
  ...
  memory_size +=
      get_mem2Dwp(&(currSlice->wp_params), 2, MAX_REFERENCE_PICTURES);
...
int init_global_buffers(VideoParameters *p_Vid, int layer_id) {
  ...
  if (cps->yuv_format != YUV400) {
    memory_size +=
        get_mem3Dpel(&cps->imgUV_ref, 2, cps->height_cr, cps->width_cr);
  } else
    cps->imgUV_ref = NULL;
  ...
  if ((cps->separate_colour_plane_flag != 0)) {
    for (i = 0; i < MAX_PLANE; ++i) {
      if (((cps->mb_data_JV[i]) = (Macroblock *)calloc(
               cps->FrameSizeInMbs, sizeof(Macroblock))) == NULL)
        no_mem_exit("init_global_buffers: cps->mb_data_JV");
    }
    cps->mb_data = NULL;
  } else {
    if (((cps->mb_data) = (Macroblock *)calloc(cps->FrameSizeInMbs,
                                               sizeof(Macroblock))) == NULL)
      no_mem_exit("init_global_buffers: cps->mb_data");
  }
  if ((cps->separate_colour_plane_flag != 0)) {
    for (i = 0; i < MAX_PLANE; ++i) {
      if (((cps->intra_block_JV[i]) =
               (char *)calloc(cps->FrameSizeInMbs, sizeof(char))) == NULL)
  ...
  // MB.
  if (((cps->PicPos) = (BlockPos *)calloc(cps->FrameSizeInMbs + 1,
  ...
  if ((cps->separate_colour_plane_flag != 0)) {
    for (i = 0; i < MAX_PLANE; ++i) {
      get_mem2D(&(cps->ipredmode_JV[i]), 4 * cps->FrameHeightInMbs,
                4 * cps->PicWidthInMbs);
    }
    cps->ipredmode = NULL;
  } else
    memory_size += get_mem2D(&(cps->ipredmode), 4 * cps->FrameHeightInMbs,
                             4 * cps->PicWidthInMbs);

  // CAVLC mem
  memory_size += get_mem4D(&(cps->nz_coeff), cps->FrameSizeInMbs, 3, BLOCK_SIZE,
                           BLOCK_SIZE);
  if ((cps->separate_colour_plane_flag != 0)) {
    for (i = 0; i < MAX_PLANE; ++i) {
      get_mem2Dint(&(cps->siblock_JV[i]), cps->FrameHeightInMbs,
                   cps->PicWidthInMbs);
    ...
    memory_size += get_mem2Dint(&(cps->siblock), cps->FrameHeightInMbs,
                                cps->PicWidthInMbs);
  }
  init_qp_process(cps);
...
void free_layer_buffers(VideoParameters *p_Vid, int layer_id) {
  ...
  if (!p_Vid->global_init_done[layer_id]) return;
...
void ClearDecPicList(VideoParameters *p_Vid) {
  ...
  if (pPic && (pPic != p_Vid->pDecOuputPic)) {
    ...
    while (pPicTail->pNext) pPicTail = pPicTail->pNext;
...
DecodedPicList *get_one_avail_dec_pic_from_list(DecodedPicList *pDecPicList,
                                                int b3D, int view_id) {
...
#if (!MVC_EXTENSION_ENABLE)
  if ((strcasecmp(p_Inp->outfile, "\"\"") != 0) &&
      (strlen(p_Inp->outfile) > 0)) {
    if ((pDecoder->p_Vid->p_out =
...
int FinitDecoder(DecodedPicList **ppDecPicList) {
  DecoderParams *pDecoder = p_Dec;
  if (!pDecoder) return DEC_GEN_NOERR;
  ClearDecPicList(pDecoder->p_Vid);
#if (MVC_EXTENSION_ENABLE)
...
int CloseDecoder() {
  ...
  DecoderParams *pDecoder = p_Dec;
  if (!pDecoder) return DEC_CLOSE_NOERR;
  ...
  switch (pDecoder->p_Inp->FileFormat) {
    default:
    case PAR_OF_ANNEXB: close_annex_b(pDecoder->p_Vid->annex_b); break;
    case PAR_OF_RTP: CloseRTPFile(&pDecoder->p_Vid->BitStreamFile); break;
  }
...
#else
  if (pDecoder->p_Vid->p_out >= 0) close(pDecoder->p_Vid->p_out);
#endif

  if (pDecoder->p_Vid->p_ref != -1) close(pDecoder->p_Vid->p_ref);
...
void OpenOutputFiles(VideoParameters *p_Vid, int view0_id, int view1_id) {
  ...
  if ((strcasecmp(p_Inp->outfile, "\"\"") != 0) &&
    ...
    if (strcmp("nul", chBuf)) {
      ...
      if ((p_Vid->p_out_mvc[0] = open(out_ViewFileName[0], OPENFLAGS_WRITE,
                                      OPEN_PERMISSIONS)) == -1) {
      ...
      if ((p_Vid->p_out_mvc[1] = open(out_ViewFileName[1], OPENFLAGS_WRITE,
                                      OPEN_PERMISSIONS)) == -1) {
        snprintf(errortext, ET_SIZE, "Error open file %s ",
                 out_ViewFileName[1]);
        fprintf(stderr, "%s\n", errortext);
