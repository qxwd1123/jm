...
void print_usage(char *argv[]) {
  ...
  printf(
      "Note: the input file needs to be a H.264/AVC RTP dump file (lencod: "
      "OutFileMode=1)");
  printf(
      "Usage: %s input_file output_file loss_percent <keep_leading_packets>\n",
      argv[0]);
  exit(-1);
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 5) {
    for (i = 0; i < atoi(argv[4]); i++) {
      if (4 != fread(&bufsize, 1, 4, fr)) return 0;
      if (4 != fread(&intime, 1, 4, fr)) {
        printf(
            "Panic, cannot read timestamp (is this a valid RTP dump file?)\n");
        return -1;
      }
      if (bufsize != fread(buf, 1, bufsize, fr)) {
        printf(
            "Cannot read packet with indicated length (is this a valid RTP "
            "dump file?), exit\n");
        return -3;
      }
  ...
  for (;;) {
    ...
    if (bufsize != fread(buf, 1, bufsize, fr)) {
      printf(
          "Cannot read packet with indicated length (is this a valid RTP dump "
          "file?)\n");
      return -3;
    }
