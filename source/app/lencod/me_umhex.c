 ...
 *   1. UMHEX_get_mem() and UMHEX_free_mem() are functions for allocation and
 *release of memories about motion estimation
...
static const MotionVector Diamond[4] = {
  { -4, 0 }, { 4, 0 }, { 0, -4 }, { 0, 4 }
};
static const MotionVector Hexagon[6] = { { -8, 0 }, { 8, 0 },  { -4, -8 },
                                         { 4, 8 },  { -4, 8 }, { 4, -8 } };
static const short Big_Hexagon_X[16] = { 0, -8, -16, -16, -16, -16, -16, -8,
                                         0, 8,  16,  16,  16,  16,  16,  8 };
static const short Big_Hexagon_Y[16] = { 8,   12,  8,  4,  0, -4, -8, -12,
                                         -16, -12, -8, -4, 0, 4,  8,  12 };

static const int Multi_Ref_Thd[8] = { 0, 300, 120, 120, 60, 30, 30, 15 };
static const int Big_Hexagon_Thd[8] = {
  0, 3000, 1500, 1500, 800, 400, 400, 200
};
...
void UMHEX_DefineThresholdMB(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  float scale_factor =
      (float)((1 - p_Inp->UMHexScale * 0.1) +
              p_Inp->UMHexScale * 0.1 * (p_Vid->width / MIN_IMG_WIDTH));
  ...
  Thresh4x4 =
      ((1 << gb_q_bits) - gb_qp_const) /
      imax(1, p_Vid->p_Quant->q_params_4x4[0][1][gb_qp_rem][0][0].ScaleComp);
  ...
  for (i = 1; i < 8; i++) {
    ...
    p_UMHex->Median_Pred_Thd_MB[i] =
        (distblk)(Median_Pred_Thd[i] * scale_factor * QP_factor * dbScalar);
    // ET_thd2: early termination after every circle of 16 points Big-Hex Search
    p_UMHex->Big_Hexagon_Thd_MB[i] =
        (distblk)(Big_Hexagon_Thd[i] * scale_factor * QP_factor * dbScalar);
    // threshold for multi ref case
    p_UMHex->Multi_Ref_Thd_MB[i] =
        (distblk)(Multi_Ref_Thd[i] * scale_factor * QP_factor * dbScalar);
    // threshold for usage of DSR technique. DSR ref to JVT-R088
    p_UMHex->Threshold_DSR_MB[i] =
        (distblk)(Threshold_DSR[i] * scale_factor * QP_factor * dbScalar);
  }
}
...
int UMHEX_get_mem(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  int memory_size = 0;
  int search_range = p_Inp->SearchMode[0] == UM_HEX ? p_Inp->search_range[0]
                                                    : p_Inp->search_range[1];

  if (NULL ==
      (p_UMHex->flag_intra = calloc((p_Vid->width >> 4) + 1, sizeof(byte))))
    no_mem_exit("UMHEX_get_mem: p_UMHex->flag_intra");  // fwf 20050330

  memory_size += get_mem2D(&p_UMHex->McostState, 2 * search_range + 1,
                           2 * search_range + 1);

  memory_size += get_mem4Ddistblk(&(p_UMHex->fastme_ref_cost),
                                  p_Vid->max_num_references, 9, 4, 4);
  memory_size += get_mem3Ddistblk(&(p_UMHex->fastme_l0_cost), 9,
                                  p_Vid->height >> 2, p_Vid->width >> 2);
  memory_size += get_mem3Ddistblk(&(p_UMHex->fastme_l1_cost), 9,
                                  p_Vid->height >> 2, p_Vid->width >> 2);
  memory_size +=
      get_mem2Ddistblk(&(p_UMHex->fastme_best_cost), 7, p_Vid->width >> 2);

  memory_size += get_mem2D(&p_UMHex->SearchState, 7, 7);
  if (p_Inp->BiPredMotionEstimation == 1)  // memory allocation for bipred mode
  {
    memory_size +=
        get_mem3Ddistblk(&(p_UMHex->fastme_l0_cost_bipred), 9,
                         p_Vid->height >> 2, p_Vid->width >> 2);  // for bipred
    memory_size +=
        get_mem3Ddistblk(&(p_UMHex->fastme_l1_cost_bipred), 9,
                         p_Vid->height >> 2, p_Vid->width >> 2);  // for bipred
  }
...
*value) int       lambda_factor // <--  lagrangian parameter for determining
*motion cost \par Two macro definitions defined in this program:
...
UMHEXIntegerPelBlockMotionSearch(
    ...
    MotionVector
        *pred_mv,  // < <--  motion vector predictor (x|y) in sub-pel units
    MEBlock *mv_block,
    distblk min_mcost,  // < <--  minimum motion cost (cost for center or huge
                        // value)
    int lambda_factor   // < <--  lagrangian parameter for determining motion
                        // cost
) {
  ...
  int pos;
  distblk mcost;
  distblk *SAD_prediction =
      p_UMHex->fastme_best_cost[blocktype - 1];  // multi ref SAD prediction
  int i, j, m;
  ...
  distblk ET_Thred =
      p_UMHex->Median_Pred_Thd_MB[blocktype];  // ET threshold in use
  int search_range = mv_block->searchRange.max_x >> 2;

  short pic_pix_x = mv_block->pos_x_padded;
  short pic_pix_y = mv_block->pos_y_padded;
  pred.mv_x =
      pic_pix_x + pred_mv->mv_x;  // predicted position x (in sub-pel units)
  pred.mv_y =
      pic_pix_y + pred_mv->mv_y;  // predicted position y (in sub-pel units)
  ...
  memset(p_UMHex->McostState[0], 0,
         (2 * p_Inp->search_range[p_Vid->view_id] + 1) *
             (2 * p_Inp->search_range[p_Vid->view_id] + 1));
  ...
  mcost += mv_block->computePredFPel(ref_picture, mv_block, min_mcost - mcost,
                                     &cand);
  ...
  /***********************************init process*************************/
  // for multi ref
  if (ref > 0 && currSlice->structure == FRAME && min_mcost > ET_Thred &&
      SAD_prediction[pic_pix_x2] < p_UMHex->Multi_Ref_Thd_MB[blocktype])
    goto terminate_step;
  ...
  } else  // hybrid search for main search loop
  {
    /****************************(MV and SAD
     * prediction)********************************/
    UMHEX_setup(currMB, ref, mv_block->list, block_y, block_x, blocktype,
                currSlice->all_mv);
    ET_Thred =
        p_UMHex->Big_Hexagon_Thd_MB[blocktype];  // ET_Thd2: early termination
                                                 // Threshold for strong motion
    ...
    if (p_UMHex->pred_SAD == 0) {
      ...
      betaFourth_1 = p_UMHex->Bsize[blocktype] /
                         ((float)p_UMHex->pred_SAD * p_UMHex->pred_SAD) -
                     p_UMHex->AlphaFourth_1[blocktype];
      betaFourth_2 = p_UMHex->Bsize[blocktype] /
                         ((float)p_UMHex->pred_SAD * p_UMHex->pred_SAD) -
                     p_UMHex->AlphaFourth_2[blocktype];
    }
    /*********************************************end of init
     * ***********************************************/
  }
  ...
  if (p_UMHex->pred_MV_ref_flag ==
      1)  // Notes: for interlace case, ref==1 should be added
  {
...
fourth_2_step:  // sub step 2: small Diamond search
  ...
  for (i = 0; i < search_range; i++) {
    ...
    if (best.mv_x == iMinNow.mv_x && best.mv_y == iMinNow.mv_y) break;
  }
...
terminate_step:
  ...
  for (i = 0; i < (blocksize_x >> 2); i++) {
    for (j = 0; j < (blocksize_y >> 2); j++) {
      if (mv_block->list == 0) {
        p_UMHex->fastme_ref_cost[ref][blocktype][block_y + j][block_x + i] =
            min_mcost;
        if (ref == 0)
          p_UMHex->fastme_l0_cost[blocktype][(currMB->block_y) + block_y + j]
                                 [(currMB->block_x) + block_x + i] = min_mcost;
      } else {
        p_UMHex->fastme_l1_cost[blocktype][(currMB->block_y) + block_y + j]
                               [(currMB->block_x) + block_x + i] = min_mcost;
      }
    }
  }
...
UMHEXSubPelBlockMotionSearch(
  ...
  Slice *currSlice = currMB->p_Slice;
  UMHexStruct *p_UMHex = p_Vid->p_UMHex;
  static const MotionVector DiamondQ[4] = {
    { -1, 0 }, { 0, 1 }, { 1, 0 }, { 0, -1 }
  };
  distblk mcost;
  ...
  memset(p_UMHex->SearchState[0], 0,
  ...
  if (!p_Vid->start_me_refinement_hp) {
    ...
    mcost += mv_block->computePredQPel(ref_picture, mv_block, min_mcost - mcost,
                                       &cand_pad);  //&cand);
  ...
  if (pred_frac_mv_x != 0 || pred_frac_mv_y != 0) {
    ...
    mcost = mv_cost(p_Vid, lambda_factor, &cand, pred_mv);
    p_UMHex->SearchState[cand.mv_y - mv->mv_y + dynamic_search_range]
                        [cand.mv_x - mv->mv_x + dynamic_search_range] = 1;
    cand_pad = pad_MVs(cand, mv_block);  // cand = pad_MVs (cand, mv_block);
  ...
  for (i = 0; i < dynamic_search_range; i++) {
    ...
    for (m = 0; m < 4; m++) {
      ...
      if (iabs(cand.mv_x - mv->mv_x) <= dynamic_search_range &&
          iabs(cand.mv_y - mv->mv_y) <= dynamic_search_range) {
        if (!p_UMHex
                 ->SearchState[cand.mv_y - mv->mv_y + dynamic_search_range]
                              [cand.mv_x - mv->mv_x + dynamic_search_range]) {
          p_UMHex->SearchState[cand.mv_y - mv->mv_y + dynamic_search_range]
                              [cand.mv_x - mv->mv_x + dynamic_search_range] = 1;
          mcost = mv_cost(p_Vid, lambda_factor, &cand, pred_mv);
          cand_pad =
              pad_MVs(cand, mv_block);  // cand = pad_MVs (cand, mv_block);

          mcost += mv_block->computePredQPel(
              ref_picture, mv_block, min_mcost - mcost, &cand_pad);  // &cand);
          if (mcost < min_mcost) {
            min_mcost = mcost;
            currmv = cand;
            abort_search = 0;
...
UMHEXSubPelBlockME(Macroblock *currMB,  // <-- Current Macroblock
                   MotionVector *pred_mv,  // <--  motion vector predictor (x|y)
                                           // in sub-pel units
                   MEBlock *mv_block,
                   distblk min_mcost,  // <--  minimum motion cost (cost for
                                       // center or huge value)
                   int *lambda) {
  if (mv_block->blocktype > 3) {
    min_mcost = UMHEXSubPelBlockMotionSearch(currMB, pred_mv, mv_block,
                                             min_mcost, lambda[Q_PEL]);
  } else {
    min_mcost =
        sub_pel_motion_estimation(currMB, pred_mv, mv_block, min_mcost, lambda);
  }
...
* 2. void UMHEX_skip_intrabk_SAD() set the SAD to zero if neigouring block
*coding type is intra \date 2003.4
************************************************************************
*/
void UMHEX_decide_intrabk_SAD(Macroblock *currMB) {
  ...
  if (currSlice->slice_type != I_SLICE && currSlice->slice_type != SI_SLICE) {
    if (currMB->pix_x == 0 && currMB->pix_y == 0) {
      ...
      p_UMHex->flag_intra_SAD =
          ((p_UMHex->flag_intra[(currMB->pix_x) >> 4]) ||
           (p_UMHex->flag_intra[((currMB->pix_x) >> 4) - 1]) ||
           (p_UMHex->flag_intra[((currMB->pix_x) >> 4) + 1]));
    }
  }
  return;
...
void UMHEX_skip_intrabk_SAD(Macroblock *currMB, int ref_max) {
  ...
  if (p_Vid->number > 0)
    p_UMHex->flag_intra[(currMB->pix_x) >> 4] =
        (currMB->best_mode == 9 || currMB->best_mode == 10) ? 1 : 0;

  if (currSlice->slice_type != I_SLICE && currSlice->slice_type != SI_SLICE &&
      (currMB->best_mode == 9 || currMB->best_mode == 10)) {
    for (k = 0; k < 9; k++) {
      for (j = 0; j < 4; j++) {
        for (i = 0; i < 4; i++) {
...
void UMHEX_setup(Macroblock *currMB, short ref, int list, int block_y,
                 int block_x, int blocktype, MotionVector *****all_mv) {
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  UMHexStruct *p_UMHex = p_Vid->p_UMHex;
  ...
  n_Bframe =
      (N_Bframe) ? (p_Vid->p_Stats->frame_ctr[B_SLICE] % (N_Bframe + 1)) : 0;

  /**************************** MV prediction **********************/
  // MV uplayer prediction
  if (blocktype > 1) {
    temp_blocktype = indication_blocktype[blocktype];
    p_UMHex->pred_MV_uplayer[0] =
        all_mv[list][ref][temp_blocktype][block_y][block_x].mv_x;
    p_UMHex->pred_MV_uplayer[1] =
        all_mv[list][ref][temp_blocktype][block_y][block_x].mv_y;
  }
  ...
  if (list == 0) {
    if (p_Vid->field_picture) {
      if (ref > 1) {
        p_UMHex->pred_MV_ref[0] =
            all_mv[0][ref - 2][blocktype][block_y][block_x].mv_x;
        p_UMHex->pred_MV_ref[0] = (int)(p_UMHex->pred_MV_ref[0] *
                                        ((ref >> 1) + 1) / (float)((ref >> 1)));
        p_UMHex->pred_MV_ref[1] =
            all_mv[0][ref - 2][blocktype][block_y][block_x].mv_y;
        p_UMHex->pred_MV_ref[1] = (int)(p_UMHex->pred_MV_ref[1] *
                                        ((ref >> 1) + 1) / (float)((ref >> 1)));
        p_UMHex->pred_MV_ref_flag = 1;
      }
      if (currSlice->slice_type == B_SLICE && (ref == 0 || ref == 1)) {
        p_UMHex->pred_MV_ref[0] =
            (int)(all_mv[1][0][blocktype][block_y][block_x].mv_x * (-n_Bframe) /
                  (N_Bframe - n_Bframe + 1.0f));
        p_UMHex->pred_MV_ref[1] =
            (int)(all_mv[1][0][blocktype][block_y][block_x].mv_y * (-n_Bframe) /
                  (N_Bframe - n_Bframe + 1.0f));
        p_UMHex->pred_MV_ref_flag = 1;
      }
    } else  // frame case
    {
      if (ref > 0) {
        p_UMHex->pred_MV_ref[0] =
            all_mv[0][ref - 1][blocktype][block_y][block_x].mv_x;
        p_UMHex->pred_MV_ref[0] =
            (int)(p_UMHex->pred_MV_ref[0] * (ref + 1) / (float)(ref));
        p_UMHex->pred_MV_ref[1] =
            all_mv[0][ref - 1][blocktype][block_y][block_x].mv_y;
        p_UMHex->pred_MV_ref[1] =
            (int)(p_UMHex->pred_MV_ref[1] * (ref + 1) / (float)(ref));
        p_UMHex->pred_MV_ref_flag = 1;
      }
      if (currSlice->slice_type == B_SLICE &&
          (ref == 0))  // B frame forward prediction, first ref
      {
        p_UMHex->pred_MV_ref[0] =
            (int)(all_mv[1][0][blocktype][block_y][block_x].mv_x * (-n_Bframe) /
                  (N_Bframe - n_Bframe + 1.0f));
  ...
  if (list == 0 && ref > 0)  // pred_SAD_ref
  {
    if (p_UMHex->flag_intra_SAD)  // add this for irregular motion
    {
      ...
      if (p_Vid->field_picture) {
        if (ref > 1) {
          p_UMHex->pred_SAD =
              p_UMHex->fastme_ref_cost[ref - 2][blocktype][block_y][block_x];
        } else {
          p_UMHex->pred_SAD =
              p_UMHex->fastme_ref_cost[0][blocktype][block_y][block_x];
        }
      } else {
        p_UMHex->pred_SAD =
            p_UMHex->fastme_ref_cost[ref - 1][blocktype][block_y][block_x];
      }
    }
  } else if (blocktype > 1)  // pred_SAD_uplayer
  {
    if (p_UMHex->flag_intra_SAD) {
      ...
      p_UMHex->pred_SAD =
          (list == 1) ? (p_UMHex->fastme_l1_cost[temp_blocktype]
                                                [(currMB->block_y) + block_y]
                                                [(currMB->block_x) + block_x])
                      : (p_UMHex->fastme_l0_cost[temp_blocktype]
                                                [(currMB->block_y) + block_y]
                                                [(currMB->block_x) + block_x]);
      p_UMHex->pred_SAD /= 2;
    }
  } else
    p_UMHex->pred_SAD = 0;  // pred_SAD_space
}

/*!
************************************************************************
* \brief
*    UMHEXBipredIntegerPelBlockMotionSearch: fast pixel block motion search for
*bipred mode this algrithm is called UMHexagonS(see JVT-D016),which includes
*    four steps with different kinds of search patterns
* \author
*   Main contributors: (see contributors.h for copyright, address and
*affiliation details)
...
UMHEXBipredIntegerPelBlockMotionSearch(
    ...
    MotionVector
        *pred_mv1,  // <--  motion vector predictor (x|y) in sub-pel units
    MotionVector
        *pred_mv2,      // <--  motion vector predictor (x|y) in sub-pel units
    MotionVector *mv1,  // <--> in: search center (x|y) / out: motion vector
                        // (x|y) - in sub-pel units
  ...
  StorablePicture *ref_picture1 =
      currSlice->listX[list + currMB->list_offset][ref];
  StorablePicture *ref_picture2 =
      currSlice
          ->listX[list == 0 ? 1 + currMB->list_offset : currMB->list_offset][0];

  MotionVector iMinNow, best, cand;

  MotionVector pred1 =
      pad_MVs(*pred_mv1, mv_block);  // predicted position x (in sub-pel units)
  MotionVector pred2 =
      pad_MVs(*pred_mv2, mv_block);  // predicted position x (in sub-pel units)
  MotionVector center1 =
      pad_MVs(*mv1, mv_block);  // predicted position x (in sub-pel units)
  MotionVector center2 =
      pad_MVs(*mv2, mv_block);  // predicted position x (in sub-pel units)
  ...
  //////allocate memory for search state//////////////////////////
  memset(p_UMHex->McostState[0], 0,
         (2 * search_range + 1) * (2 * search_range + 1));
  ...
  mcost += mv_block->computeBiPredFPel(ref_picture1, ref_picture2, mv_block,
                                       DISTBLK_MAX - mcost, &center1, &cand);
  ...
  if (min_mcost < ET_Thred) {
    ...
    if (list == 0) {
      if (p_Vid->field_picture) {
        p_UMHex->pred_MV_ref[0] =
            (int)(bipred_mv[1][0][blocktype][block_y][block_x].mv_x *
                  (-n_Bframe) / (N_Bframe - n_Bframe + 1.0f));
        p_UMHex->pred_MV_ref[1] =
            (int)(bipred_mv[1][0][blocktype][block_y][block_x].mv_y *
                  (-n_Bframe) / (N_Bframe - n_Bframe + 1.0f));
      } else  // frame case
      {
        p_UMHex->pred_MV_ref[0] =
    ...
    /******************************SAD
     * prediction**********************************/
    p_UMHex->pred_SAD = distblkmin(distblkmin(p_UMHex->SAD_a, p_UMHex->SAD_b),
                                   p_UMHex->SAD_c);  // pred_SAD_space
    ...
    if (p_UMHex->pred_SAD == 0) {
      ...
      betaFourth_1 =
          p_UMHex->Bsize[blocktype] / (p_UMHex->pred_SAD * p_UMHex->pred_SAD) -
          p_UMHex->AlphaFourth_1[blocktype];
      betaFourth_2 =
          p_UMHex->Bsize[blocktype] / (p_UMHex->pred_SAD * p_UMHex->pred_SAD) -
          p_UMHex->AlphaFourth_2[blocktype];
...
terminate_step:
  for (i = 0; i < (blocksize_x >> 2); i++) {
    for (j = 0; j < (blocksize_y >> 2); j++) {
      if (list == 0) {
        p_UMHex
            ->fastme_l0_cost_bipred[blocktype][(currMB->block_y) + block_y + j]
                                   [(currMB->block_x) + block_x + i] =
            min_mcost;
      } else {
        p_UMHex
            ->fastme_l1_cost_bipred[blocktype][(currMB->block_y) + block_y + j]
                                   [(currMB->block_x) + block_x + i] =
            min_mcost;
      }
    }
...
************************************************************************
*/
void UMHEXSetMotionVectorPredictor(Macroblock *currMB, MotionVector *pmv,
                                   struct pic_motion_params **mv_info,
                                   short ref_frame, int list, int mb_x,
  ...
  distblk ***fastme_l0_cost_flag =
      (p_UMHex->bipred_flag ? p_UMHex->fastme_l0_cost_bipred
                            : p_UMHex->fastme_l0_cost);
  distblk ***fastme_l1_cost_flag =
      (p_UMHex->bipred_flag ? p_UMHex->fastme_l1_cost_bipred
                            : p_UMHex->fastme_l1_cost);
      ...
      rFrameL =
          block_a.available
              ? (p_Vid->mb_data[block_a.mb_addr].mb_field
                     ? mv_info[block_a.pos_y][block_a.pos_x].ref_idx[list]
                     : mv_info[block_a.pos_y][block_a.pos_x].ref_idx[list] * 2)
              : -1;
      rFrameU =
          block_b.available
              ? (p_Vid->mb_data[block_b.mb_addr].mb_field
                     ? mv_info[block_b.pos_y][block_b.pos_x].ref_idx[list]
                     : mv_info[block_b.pos_y][block_b.pos_x].ref_idx[list] * 2)
              : -1;
      rFrameUR =
          block_c.available
              ? (p_Vid->mb_data[block_c.mb_addr].mb_field
                     ? mv_info[block_c.pos_y][block_c.pos_x].ref_idx[list]
                     : mv_info[block_c.pos_y][block_c.pos_x].ref_idx[list] * 2)
              : -1;
    } else {
      rFrameL =
          block_a.available
  ...
  if (blockshape_x == 8 && blockshape_y == 16) {
    if (mb_x == 0) {
      if (rFrameL == ref_frame) mvPredType = MVPRED_L;
    } else {
      if (rFrameUR == ref_frame) mvPredType = MVPRED_UR;
    }
  } else if (blockshape_x == 16 && blockshape_y == 8) {
    if (mb_y == 0) {
      if (rFrameU == ref_frame) mvPredType = MVPRED_U;
    } else {
      if (rFrameL == ref_frame) mvPredType = MVPRED_L;
    }
  }
  ...
  if ((p_Inp->UMHexDSR == 1 || p_Inp->BiPredMotionEstimation == 1)) {
    p_UMHex->SAD_a =
        block_a.available
            ? ((list == 1)
                   ? (fastme_l1_cost_flag[p_UMHex->UMHEX_blocktype]
                                         [block_a.pos_y][block_a.pos_x])
                   : (fastme_l0_cost_flag[p_UMHex->UMHEX_blocktype]
                                         [block_a.pos_y][block_a.pos_x]))
            : 0;
    p_UMHex->SAD_b =
        block_b.available
  ...
  for (hv = 0; hv < 2; hv++) {
    if (!p_Vid->mb_aff_frame_flag || hv == 0) {
      mv_a = block_a.available
                 ? mv_info[block_a.pos_y][block_a.pos_x].mv[list].mv_x
                 : 0;
      mv_b = block_b.available
                 ? mv_info[block_b.pos_y][block_b.pos_x].mv[list].mv_x
                 : 0;
      mv_c = block_c.available
                 ? mv_info[block_c.pos_y][block_c.pos_x].mv[list].mv_x
                 : 0;
    } else {
      if (p_Vid->mb_data[currMB->mbAddrX].mb_field) {
        mv_a =
    ...
    switch (mvPredType) {
      case MVPRED_MEDIAN:
        if (!(block_b.available || block_c.available)) {
          ...
          pred_vec = (short)(mv_a + mv_b + mv_c - imin(mv_a, imin(mv_b, mv_c)) -
                             imax(mv_a, imax(mv_b, mv_c)));
        }
        break;
      case MVPRED_L: pred_vec = (short)mv_a; break;
      case MVPRED_U: pred_vec = (short)mv_b; break;
      case MVPRED_UR: pred_vec = (short)mv_c; break;
      default: break;
    }
    ...
    if (p_Inp->UMHexDSR) {
      ...
      if (dsr_mv_avail < 2) {
        ...
        if (dsr_mv_sum == 0)
          dsr_small_search_range =
              (p_Inp->search_range[p_Vid->view_id] + 4) >> 3;
        else if (dsr_mv_sum > 3)
          dsr_small_search_range =
              (p_Inp->search_range[p_Vid->view_id] + 2) >> 2;
        else
          dsr_small_search_range =
              (3 * p_Inp->search_range[p_Vid->view_id] + 8) >> 4;
        dsr_temp_search_range[hv] =
            imin(p_Inp->search_range[p_Vid->view_id],
                 imax(dsr_small_search_range, dsr_mv_max << 1));
        if (distblkmax(p_UMHex->SAD_a,
                       distblkmax(p_UMHex->SAD_b, p_UMHex->SAD_c)) >
            p_UMHex->Threshold_DSR_MB[p_UMHex->UMHEX_blocktype])
          dsr_temp_search_range[hv] = p_Inp->search_range[p_Vid->view_id];
      }
    }
  }
  ...
  if (p_Inp->UMHexDSR) {
    ...
    if (p_Inp->full_search == 2) {
      ...
      int scale = ((imin(ref_frame, 1) + 1) *
                   imin(2, p_UMHex->BlockType_LUT[(blockshape_y >> 2) - 1]
                                                 [(blockshape_x >> 2) - 1]));
      mv_block->searchRange.min_x = -search_range / scale;
      mv_block->searchRange.max_x = search_range / scale;
      mv_block->searchRange.min_y = -search_range / scale;
