...
EPZS_sub_pel_motion_estimation(
  ...
  int max_pos2 =
      ((!p_Vid->start_me_refinement_hp || !p_Vid->start_me_refinement_qp)
           ? imax(1, mv_block->search_pos2)
  ...
  *********************************/
  ...
  for (best_pos = 0, pos = p_Vid->start_me_refinement_hp;
       pos < imin(5, max_pos2); ++pos) {
    cand = add_MVs(search_point_hp[pos], &padded_mv);

    //----- set motion vector cost -----
    mcost = mv_cost(p_Vid, lambda_factor, &cand, &pred_mv);
    if (mcost < second_mcost) {
      mcost += mv_block->computePredHPel(ref_picture, mv_block,
  ...
  if (best_pos == 0 && (pred->mv_x == mv->mv_x) && (pred->mv_y == mv->mv_y) &&
      min_mcost < sub_threshold) {
    return min_mcost;
  }

  if (mv_block->search_pos2 >= 9) {
    if (best_pos != 0 ||
        (iabs(pred->mv_x - mv->mv_x) + iabs(pred->mv_y - mv->mv_y))) {
  ...
  ************************************/
  ...
  for (pos = p_Vid->start_me_refinement_qp; pos < end_pos; ++pos) {
    ...
    if (mcost < second_mcost) {
      mcost += mv_block->computePredQPel(ref_picture, mv_block,
                                         second_mcost - mcost, &cand);
...
*/
distblk  //  ==> minimum motion cost after search
EPZS_sub_pel_bipred_motion_estimation(
    Macroblock *currMB,   // <--  current Macroblock
    MEBlock *mv_block,    // <--  motion vector information
    ...
    MotionVector *
        mv1,  // <--> in: search center  / out: motion vector - in sub-pel units
    MotionVector
        *mv2,  // <--> in: search center / out: motion vector - in sun-pel units
    distblk
        min_mcost,  // <--  minimum motion cost (cost for center or huge value)
    int *lambda     // <--  lagrangian parameter for determining motion cost
) {
  ...
  *********************************/
  ...
  for (best_pos = 0, pos = start_hp; pos < 5; ++pos) {
    ...
    if (mcost < second_mcost) {
      mcost +=
          mv_block->computeBiPredHPel(ref_picture1, ref_picture2, mv_block,
                                      second_mcost - mcost, &cand, &padded_mv2);
  ...
  if (best_pos == 0 && (pred1->mv_x == mv1->mv_x) &&
      (pred1->mv_y == mv1->mv_y) && min_mcost < sub_threshold) {
    return min_mcost;
  }

  if (mv_block->search_pos2 >= 9) {
    if (best_pos != 0 ||
        (iabs(pred1->mv_x - mv1->mv_x) + iabs(pred1->mv_y - mv1->mv_y))) {
      start_pos = next_start_pos[best_pos][second_pos];
      end_pos = next_end_pos[best_pos][second_pos];
      ...
      for (pos = start_pos; pos < end_pos; ++pos) {
        ...
        if (mcost < min_mcost) {
          mcost += mv_block->computeBiPredHPel(ref_picture1, ref_picture2,
                                               mv_block, min_mcost - mcost,
                                               &cand, &padded_mv2);
  ...
  ************************************/
  ...
  for (pos = p_Vid->start_me_refinement_qp; pos < 5; ++pos) {
    ...
    if (mcost < second_mcost) {
      mcost +=
          mv_block->computeBiPredQPel(ref_picture1, ref_picture2, mv_block,
                                      second_mcost - mcost, &cand, &padded_mv2);
  ...
  if ((min_mcost > sub_threshold)) {
    if (best_pos != 0 ||
      ...
      for (pos = start_pos; pos < end_pos; ++pos) {
        ...
        if (mcost < min_mcost) {
          mcost += mv_block->computeBiPredQPel(ref_picture1, ref_picture2,
                                               mv_block, min_mcost - mcost,
                                               &cand, &padded_mv2);
