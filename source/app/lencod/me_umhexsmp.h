...
struct umhex_smp_struct {
  ...
  distblk ***l0_cost;  // store SAD information needed for forward median and
                       // uplayer prediction
  distblk ***l1_cost;  // store SAD information needed for backward median and
                       // uplayer prediction
...
extern void smpUMHEX_setup(Macroblock *currMB, short, int, int, int, int,
                           MotionVector *****);
extern distblk smpUMHEXBipredIntegerPelBlockMotionSearch(
    Macroblock *, int, MotionVector *, MotionVector *, MotionVector *,
    MotionVector *, MEBlock *, int, distblk, int);
extern distblk smpUMHEXIntegerPelBlockMotionSearch(Macroblock *currMB,
                                                   MotionVector *pred_mv,
                                                   MEBlock *mv_block,
                                                   distblk min_mcost,
                                                   int lambda_factor);
extern distblk smpUMHEXSubPelBlockMotionSearch(Macroblock *currMB,
                                               MotionVector *pred_mv,
                                               MEBlock *mv_block,
                                               distblk min_mcost,
                                               int lambda_factor);
extern distblk smpUMHEXFullSubPelBlockMotionSearch(Macroblock *currMB,
                                                   MotionVector *pred_mv,
                                                   MEBlock *mv_block,
                                                   distblk min_mcost,
                                                   int lambda_factor);
extern distblk smpUMHEXSubPelBlockME(Macroblock *currMB, MotionVector *pred_mv,
