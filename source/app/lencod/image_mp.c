...
void frame_picture_mp_p_slice(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_Inp->WPIterMC) p_Vid->frameOffsetAvail = 1;

#if (DBG_IMAGE_MP)
  printf("rd_pass = %d: %d (%.0f, %.0f, %.0f)\n", rd_pass,
         p_Vid->frame_pic[0]->bits_per_picture,
         p_Vid->frame_pic[0]->distortion.value[0],
         p_Vid->frame_pic[0]->distortion.value[1],
         p_Vid->frame_pic[0]->distortion.value[2]);
#endif
  ...
  if (p_Inp->GenerateMultiplePPS) {
    ...
    if (wp_pass) {
      ...
      p_Vid->p_curr_frm_struct->qp = p_Vid->qp;
      frame_picture(p_Vid, p_Vid->frame_pic[rd_pass], &p_Vid->imgData, rd_pass);
      selection = picture_coding_decision(p_Vid, p_Vid->frame_pic[0],
                                          p_Vid->frame_pic[rd_pass], rd_qp);
#if (DBG_IMAGE_MP)
      ...
      printf("rd_pass = %d: %d (%.0f, %.0f, %.0f)\n", rd_pass,
             ...
             p_Vid->frame_pic[rd_pass]->distortion.value[0],
             p_Vid->frame_pic[rd_pass]->distortion.value[1],
             p_Vid->frame_pic[rd_pass]->distortion.value[2]);
#endif
      ...
      if (p_Inp->WPMethod == 0 || p_Inp->WPMCPrecision) {
        ...
        if (wp_pass) {
          ...
          frame_picture(p_Vid, p_Vid->frame_pic[rd_pass], &p_Vid->imgData,
                        rd_pass);
          selection = picture_coding_decision(p_Vid, p_Vid->frame_pic[0],
                                              p_Vid->frame_pic[rd_pass], rd_qp);
#if (DBG_IMAGE_MP)
  ...
  if (p_Vid->EvaluateDBOff) {
    ...
    p_Vid->active_pps =
        (best_method == EXP_WP ? p_Vid->PicParSet[1] : p_Vid->PicParSet[0]);
    if (frame_type != P_SLICE) {
      set_slice_type(p_Vid, p_Inp, frame_type);
      populate_frame_slice_type(p_Inp, p_Vid->p_curr_frm_struct, frame_type,
                                p_Vid->p_pred->max_num_slices);
    }
    p_Vid->TurnDBOff = 1;
    p_Vid->write_macroblock = FALSE;
...
void frame_picture_mp_b_slice(VideoParameters *p_Vid, InputParameters *p_Inp) {
    ...
    if (p_Inp->GenerateMultiplePPS) {
      ...
      if (p_Vid->TestWPBSlice(dummy_slice, 0) == 1) {
        ...
        wp_pass = 1;
                 ...
                 (p_Inp->WPMCPrecBSlice == 2 ||
                  (p_Inp->WPMCPrecBSlice == 1 && p_Vid->nal_reference_idc))) {
        p_Vid->active_pps = p_Vid->PicParSet[1];
        wp_pass = 1;
      }
    }
  ...
  if (p_Inp->RDPictureFrameQPBSlice) {
    // frame QP pass
    p_Vid->active_pps = best_method == EXP_WP   ? p_Vid->PicParSet[1]
                        : best_method == IMP_WP ? p_Vid->PicParSet[2]
                                                : p_Vid->PicParSet[0];
    // p_Vid->qp = (p_Vid->nal_reference_idc==0 ? rd_qp + 1 : rd_qp - 1);
  ...
  if (p_Inp->RDPictureDirectMode) {
    ...
    p_Vid->active_pps = apply_wp == IMP_WP   ? p_Vid->PicParSet[1]
                        : apply_wp == EXP_WP ? p_Vid->PicParSet[2]
                                             : p_Vid->PicParSet[0];
