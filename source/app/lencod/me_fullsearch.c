...
full_search_motion_estimation(
  ...
  int search_range =
      imin(mv_block->searchRange.max_x, mv_block->searchRange.max_y) >> 2;
  distblk mcost;
  int pos;
  ...
  StorablePicture *ref_picture =
      currSlice->listX[mv_block->list + currMB->list_offset][ref];
  ...
  int check_for_00 = (mv_block->blocktype == 1 && !p_Inp->rdopt &&
                      currSlice->slice_type != B_SLICE && ref == 0);
  center.mv_x = mv_block->pos_x_padded +
                mv->mv_x;  // center position x (in sub-pel units)
  center.mv_y = mv_block->pos_y_padded +
                mv->mv_y;  // center position y (in sub-pel units)
  pred.mv_x = mv_block->pos_x_padded +
              pred_mv->mv_x;  // predicted position x (in sub-pel units)
  pred.mv_y = mv_block->pos_y_padded +
              pred_mv->mv_y;  // predicted position y (in sub-pel units)
  ...
  for (pos = 0; pos < max_pos; pos++) {
    ...
    if (check_for_00 && cand.mv_x == mv_block->pos_x_padded &&
        cand.mv_y == mv_block->pos_y_padded) {
 ...
 */
distblk  //  ==> minimum motion cost after search
full_search_bipred_motion_estimation(
    Macroblock *currMB,      // <--  current Macroblock
    int list,                // <--  reference list
    MotionVector *pred_mv1,  // <--  motion vector predictor from first list
                             // (x|y) in sub-pel units
    MotionVector *pred_mv2,  // <--  motion vector predictor from second list
                             // (x|y) in sub-pel units
  ...
  int best_pos = 0;  // position with minimum motion cost
  int max_pos = (2 * (search_range >> 2) + 1) *
                (2 * (search_range >> 2) + 1);  // number of search positions
  ...
  center2.mv_x =
      ...
      mv2->mv_x;  // center position x of static mv (in sub-pel units)
  center2.mv_y =
      mv_block->pos_y_padded +
      mv2->mv_y;  // center position y of static mv (in sub-pel units)
...
sub_pel_motion_estimation(
  ...
  int check_position0 =
      (!p_Inp->rdopt && currSlice->slice_type != B_SLICE && ref == 0 &&
       mv_block->blocktype == 1 && mv->mv_x == 0 && mv->mv_y == 0);
  ...
  for (best_pos = 0, pos = p_Vid->start_me_refinement_hp; pos < max_pos2;
    ...
    mcost += mv_block->computePredHPel(ref_picture, mv_block, min_mcost - mcost,
                                       &cmv);
...
sub_pel_bipred_motion_estimation(
    ...
    MotionVector
        *pred_mv1,  // <--  motion vector predictor (x) in sub-pel units
    MotionVector
        *pred_mv2,      // <--  motion vector predictor (x) in sub-pel units
    MotionVector *mv1,  // <--> in: search center (x) / out: motion vector (x) -
                        // in sub-pel units
    MotionVector *mv2,  // <--> in: search center (x) / out: motion vector (x) -
                        // in sub-pel units
    distblk
        min_mcost,  // <--  minimum motion cost (cost for center or huge value)
    int *lambda     // <--  lagrangian parameter for determining motion cost
  ...
  for (best_pos = 0, pos = start_hp; pos < max_pos2; pos++) {
    cand.mv_x =
        mv1->mv_x + p_Vid->spiral_hpel_search[pos].mv_x;  // quarter-pel units
    cand.mv_y =
        mv1->mv_y + p_Vid->spiral_hpel_search[pos].mv_y;  // quarter-pel units
    ...
    cmv = pad_MVs(cand, mv_block);
    mcost += mv_block->computeBiPredHPel(ref_picture1, ref_picture2, mv_block,
                                         min_mcost - mcost, &cmv, &smv);
  ...
  ************************************/
  ...
  for (best_pos = 0, pos = p_Vid->start_me_refinement_qp;
       pos < mv_block->search_pos4; pos++) {
    cand.mv_x =
        mv1->mv_x + p_Vid->spiral_search[pos].mv_x;  // quarter-pel units
    cand.mv_y =
        mv1->mv_y + p_Vid->spiral_search[pos].mv_y;  // quarter-pel units
    ...
    cmv = pad_MVs(cand, mv_block);
    mcost += mv_block->computeBiPredQPel(ref_picture1, ref_picture2, mv_block,
                                         min_mcost - mcost, &cmv, &smv);
 ...
 */
distblk  //  ==> minimum motion cost after search
full_sub_pel_motion_estimation(
    Macroblock *currMB,  // <--  Current Macroblock
    MotionVector *pred,  // <--  motion vector predictor in sub-pel units
    MEBlock *mv_block,   // <--  motion estimation structure
    distblk
...
*/
distblk  //  ==> minimum motion cost after search
full_sub_pel_bipred_motion_estimation(
    Macroblock *currMB,  // <--  current Macroblock
    MEBlock *mv_block,   // <--  motion vector information
    int list,            // <--  reference picture list
    MotionVector
