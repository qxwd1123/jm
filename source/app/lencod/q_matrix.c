...
const int dequant_coef[6][4][4] = { { { 10, 13, 10, 13 },
                                      { 13, 16, 13, 16 },
                                      { 10, 13, 10, 13 },
                                      { 13, 16, 13, 16 } },
                                    { { 11, 14, 11, 14 },
...
static const short Quant_intra_default[16] = { 6,  13, 20, 28, 13, 20, 28, 32,
                                               20, 28, 32, 37, 28, 32, 37, 42 };

static const short Quant_inter_default[16] = { 10, 14, 20, 24, 14, 20, 24, 27,
  ...
  for (i = 0; i < item; i += cnt) {
    ...
    if (0 > (MapIdx = CheckParameterName(items[i + cnt], &type))) {
      snprintf(errortext, ET_SIZE,
               " Parsing error in quantization matrix config file: Parameter "
    ...
    if (strcmp("=", items[i + cnt])) {
      snprintf(errortext, ET_SIZE,
               " Parsing error in quantization matrix config file: '=' "
    ...
    for (j = 0; j < range; j++) {
      if (1 != sscanf(items[i + cnt + j], "%d", &IntContent)) {
        snprintf(errortext, ET_SIZE,
                 " Parsing error in quantization matrix file: Expected "
    ...
    if (p_Inp->ScalingListPresentFlag[i]) {
      ...
      if (matrix4x4_check[i]) {
        ...
        for (cnt = 0; cnt < 16; cnt++) {
          if (ScalingList[cnt] < 0 ||
              ScalingList[cnt] >
                  255)  // ScalingList[0]=0 to indicate use default matrix
          {
            fail = 1;
            break;
          }
...
static void allocate_QMatrix(QuantParameters *p_Quant, InputParameters *p_Inp) {
  ...
  if ((p_Quant->qp_per_matrix = (int *)malloc((MAX_QP + 1 + bitdepth_qp_scale) *
                                              sizeof(int))) == NULL)
    no_mem_exit("allocate_QMatrix: p_Quant->qp_per_matrix");
  if ((p_Quant->qp_rem_matrix = (int *)malloc((MAX_QP + 1 + bitdepth_qp_scale) *
                                              sizeof(int))) == NULL)
    no_mem_exit("allocate_QMatrix: p_Quant->qp_per_matrix");
...
static void set_default_quant4x4(LevelQuantParams **q_params_4x4,
                                 const int (*quant)[4],
                                 const int (*dequant)[4]) {
  int i, j;
  for (j = 0; j < 4; j++) {
...
void CalculateQuant4x4Param(VideoParameters *p_Vid) {
  ...
  if (!active_sps->seq_scaling_matrix_present_flag &&
      !active_pps->pic_scaling_matrix_present_flag)  // set to no q-matrix
    no_q_matrix = TRUE;
  else {
  ...
  if (no_q_matrix == TRUE) {
    for (k_mod = 0; k_mod <= max_qp; k_mod++) {
      k = k_mod % 6;
      set_default_quant4x4(p_Quant->q_params_4x4[0][0][k_mod], quant_coef[k],
                           dequant_coef[k]);
      set_default_quant4x4(p_Quant->q_params_4x4[0][1][k_mod], quant_coef[k],
                           dequant_coef[k]);
      set_default_quant4x4(p_Quant->q_params_4x4[1][0][k_mod], quant_coef[k],
                           dequant_coef[k]);
      set_default_quant4x4(p_Quant->q_params_4x4[1][1][k_mod], quant_coef[k],
                           dequant_coef[k]);
      set_default_quant4x4(p_Quant->q_params_4x4[2][0][k_mod], quant_coef[k],
                           dequant_coef[k]);
      set_default_quant4x4(p_Quant->q_params_4x4[2][1][k_mod], quant_coef[k],
                           dequant_coef[k]);
    }
  } else {
    for (k_mod = 0; k_mod <= max_qp; k_mod++) {
      ...
      for (j = 0; j < 4; j++) {
        for (i = 0; i < 4; i++) {
          ...
          if ((!present[0]) || p_QScale->UseDefaultScalingMatrix4x4Flag[0]) {
            p_Quant->q_params_4x4[0][1][k_mod][j][i].ScaleComp =
                (quant_coef[k][j][i] << 4) / Quant_intra_default[temp];
            p_Quant->q_params_4x4[0][1][k_mod][j][i].InvScaleComp =
                dequant_coef[k][j][i] * Quant_intra_default[temp];
          } else {
            p_Quant->q_params_4x4[0][1][k_mod][j][i].ScaleComp =
                (quant_coef[k][j][i] << 4) / p_QScale->ScalingList4x4[0][temp];
            p_Quant->q_params_4x4[0][1][k_mod][j][i].InvScaleComp =
                dequant_coef[k][j][i] * p_QScale->ScalingList4x4[0][temp];
          }

          if (!present[1]) {
            p_Quant->q_params_4x4[1][1][k_mod][j][i].ScaleComp =
                p_Quant->q_params_4x4[0][1][k_mod][j][i].ScaleComp;
            p_Quant->q_params_4x4[1][1][k_mod][j][i].InvScaleComp =
                p_Quant->q_params_4x4[0][1][k_mod][j][i].InvScaleComp;
          } else {
            p_Quant->q_params_4x4[1][1][k_mod][j][i].ScaleComp =
                (quant_coef[k][j][i] << 4) /
                (p_QScale->UseDefaultScalingMatrix4x4Flag[1]
                     ? Quant_intra_default[temp]
                     : p_QScale->ScalingList4x4[1][temp]);
            p_Quant->q_params_4x4[1][1][k_mod][j][i].InvScaleComp =
                dequant_coef[k][j][i] *
                (p_QScale->UseDefaultScalingMatrix4x4Flag[1]
                     ? Quant_intra_default[temp]
          ...
          if (!present[2]) {
            p_Quant->q_params_4x4[2][1][k_mod][j][i].ScaleComp =
                p_Quant->q_params_4x4[1][1][k_mod][j][i].ScaleComp;
            p_Quant->q_params_4x4[2][1][k_mod][j][i].InvScaleComp =
                p_Quant->q_params_4x4[1][1][k_mod][j][i].InvScaleComp;
          } else {
            p_Quant->q_params_4x4[2][1][k_mod][j][i].ScaleComp =
                (quant_coef[k][j][i] << 4) /
                (p_QScale->UseDefaultScalingMatrix4x4Flag[2]
                     ? Quant_intra_default[temp]
                     : p_QScale->ScalingList4x4[2][temp]);
            p_Quant->q_params_4x4[2][1][k_mod][j][i].InvScaleComp =
                dequant_coef[k][j][i] *
                (p_QScale->UseDefaultScalingMatrix4x4Flag[2]
                     ? Quant_intra_default[temp]
          ...
          if ((!present[3]) || p_QScale->UseDefaultScalingMatrix4x4Flag[3]) {
            p_Quant->q_params_4x4[0][0][k_mod][j][i].ScaleComp =
                (quant_coef[k][j][i] << 4) / Quant_inter_default[temp];
            p_Quant->q_params_4x4[0][0][k_mod][j][i].InvScaleComp =
                dequant_coef[k][j][i] * Quant_inter_default[temp];
          } else {
            p_Quant->q_params_4x4[0][0][k_mod][j][i].ScaleComp =
                (quant_coef[k][j][i] << 4) / p_QScale->ScalingList4x4[3][temp];
            p_Quant->q_params_4x4[0][0][k_mod][j][i].InvScaleComp =
                dequant_coef[k][j][i] * p_QScale->ScalingList4x4[3][temp];
          }

          if (!present[4]) {
            p_Quant->q_params_4x4[1][0][k_mod][j][i].ScaleComp =
                p_Quant->q_params_4x4[0][0][k_mod][j][i].ScaleComp;
            p_Quant->q_params_4x4[1][0][k_mod][j][i].InvScaleComp =
                p_Quant->q_params_4x4[0][0][k_mod][j][i].InvScaleComp;
          } else {
            p_Quant->q_params_4x4[1][0][k_mod][j][i].ScaleComp =
                (quant_coef[k][j][i] << 4) /
                (p_QScale->UseDefaultScalingMatrix4x4Flag[4]
                     ? Quant_inter_default[temp]
                     : p_QScale->ScalingList4x4[4][temp]);
            p_Quant->q_params_4x4[1][0][k_mod][j][i].InvScaleComp =
                dequant_coef[k][j][i] *
                (p_QScale->UseDefaultScalingMatrix4x4Flag[4]
                     ? Quant_inter_default[temp]
          ...
          if (!present[5]) {
            p_Quant->q_params_4x4[2][0][k_mod][j][i].ScaleComp =
                p_Quant->q_params_4x4[1][0][k_mod][j][i].ScaleComp;
            p_Quant->q_params_4x4[2][0][k_mod][j][i].InvScaleComp =
                p_Quant->q_params_4x4[1][0][k_mod][j][i].InvScaleComp;
          } else {
            p_Quant->q_params_4x4[2][0][k_mod][j][i].ScaleComp =
                (quant_coef[k][j][i] << 4) /
                (p_QScale->UseDefaultScalingMatrix4x4Flag[5]
                     ? Quant_inter_default[temp]
                     : p_QScale->ScalingList4x4[5][temp]);
            p_Quant->q_params_4x4[2][0][k_mod][j][i].InvScaleComp =
                dequant_coef[k][j][i] *
                (p_QScale->UseDefaultScalingMatrix4x4Flag[5]
                     ? Quant_inter_default[temp]
...
void CalculateQuant8x8Param(VideoParameters *p_Vid) {
  ...
  if (!active_sps->seq_scaling_matrix_present_flag &&
      !active_pps->pic_scaling_matrix_present_flag)  // set to default matrix
    no_q_matrix = TRUE;
  else {
  ...
  if (no_q_matrix == TRUE) {
    for (k = 0; k <= max_qp; k++) {
      ...
      for (j = 0; j < 8; j++) {
        for (i = 0; i < 8; i++) {
          // intra Y
          p_Quant->q_params_8x8[0][1][k][j][i].ScaleComp =
              quant_coef8[k_mod][j][i];
          p_Quant->q_params_8x8[0][1][k][j][i].InvScaleComp =
              dequant_coef8[k_mod][j][i] << 4;

          // inter Y
          p_Quant->q_params_8x8[0][0][k][j][i].ScaleComp =
              quant_coef8[k_mod][j][i];
          p_Quant->q_params_8x8[0][0][k][j][i].InvScaleComp =
              dequant_coef8[k_mod][j][i] << 4;

          if (n_ScalingList8x8 > 2)  // 4:4:4 case only
          {
            // intra U
            p_Quant->q_params_8x8[1][1][k][j][i].ScaleComp =
                quant_coef8[k_mod][j][i];
            p_Quant->q_params_8x8[1][1][k][j][i].InvScaleComp =
                dequant_coef8[k_mod][j][i] << 4;

            // intra V
            p_Quant->q_params_8x8[2][1][k][j][i].ScaleComp =
                quant_coef8[k_mod][j][i];
            p_Quant->q_params_8x8[2][1][k][j][i].InvScaleComp =
                dequant_coef8[k_mod][j][i] << 4;

            // inter U
            p_Quant->q_params_8x8[1][0][k][j][i].ScaleComp =
                quant_coef8[k_mod][j][i];
            p_Quant->q_params_8x8[1][0][k][j][i].InvScaleComp =
                dequant_coef8[k_mod][j][i] << 4;

            // inter V
            p_Quant->q_params_8x8[2][0][k][j][i].ScaleComp =
                quant_coef8[k_mod][j][i];
            p_Quant->q_params_8x8[2][0][k][j][i].InvScaleComp =
                dequant_coef8[k_mod][j][i] << 4;
          }
        }
      }
      ...
      for (j = 0; j < 8; j++) {
        for (i = 0; i < 8; i++) {
          ...
          if ((!present[0]) || p_QScale->UseDefaultScalingMatrix8x8Flag[0]) {
            p_Quant->q_params_8x8[0][1][k][j][i].ScaleComp =
                (quant_coef8[k_mod][j][i] << 4) / Quant8_intra_default[temp];
            p_Quant->q_params_8x8[0][1][k][j][i].InvScaleComp =
                dequant_coef8[k_mod][j][i] * Quant8_intra_default[temp];
          } else {
            p_Quant->q_params_8x8[0][1][k][j][i].ScaleComp =
                (quant_coef8[k_mod][j][i] << 4) /
                p_QScale->ScalingList8x8[0][temp];
            p_Quant->q_params_8x8[0][1][k][j][i].InvScaleComp =
                dequant_coef8[k_mod][j][i] * p_QScale->ScalingList8x8[0][temp];
          }

          if ((!present[1]) || p_QScale->UseDefaultScalingMatrix8x8Flag[1]) {
            p_Quant->q_params_8x8[0][0][k][j][i].ScaleComp =
                (quant_coef8[k_mod][j][i] << 4) / Quant8_inter_default[temp];
            p_Quant->q_params_8x8[0][0][k][j][i].InvScaleComp =
                dequant_coef8[k_mod][j][i] * Quant8_inter_default[temp];
          } else {
            p_Quant->q_params_8x8[0][0][k][j][i].ScaleComp =
                (quant_coef8[k_mod][j][i] << 4) /
                p_QScale->ScalingList8x8[1][temp];
            p_Quant->q_params_8x8[0][0][k][j][i].InvScaleComp =
                dequant_coef8[k_mod][j][i] * p_QScale->ScalingList8x8[1][temp];
          }
          ...
          if (n_ScalingList8x8 > 2) {
            ...
            if (!present[2]) {
              ...
              p_Quant->q_params_8x8[1][1][k][j][i].ScaleComp =
                  (quant_coef8[k_mod][j][i] << 4) /
                  (p_QScale->UseDefaultScalingMatrix8x8Flag[2]
                       ? Quant8_intra_default[temp]
                       : p_QScale->ScalingList8x8[2][temp]);
              p_Quant->q_params_8x8[1][1][k][j][i].InvScaleComp =
                  dequant_coef8[k_mod][j][i] *
                  (p_QScale->UseDefaultScalingMatrix8x8Flag[2]
                       ? Quant8_intra_default[temp]
                       : p_QScale->ScalingList8x8[2][temp]);
            }
            ...
            if (!present[3]) {
              ...
              p_Quant->q_params_8x8[1][0][k][j][i].ScaleComp =
                  (quant_coef8[k_mod][j][i] << 4) /
                  (p_QScale->UseDefaultScalingMatrix8x8Flag[3]
                       ? Quant8_inter_default[temp]
                       : p_QScale->ScalingList8x8[3][temp]);
              p_Quant->q_params_8x8[1][0][k][j][i].InvScaleComp =
                  dequant_coef8[k_mod][j][i] *
                  (p_QScale->UseDefaultScalingMatrix8x8Flag[3]
                       ? Quant8_inter_default[temp]
            ...
            if (!present[4]) {
              ...
              p_Quant->q_params_8x8[2][1][k][j][i].ScaleComp =
                  (quant_coef8[k_mod][j][i] << 4) /
                  (p_QScale->UseDefaultScalingMatrix8x8Flag[4]
                       ? Quant8_intra_default[temp]
                       : p_QScale->ScalingList8x8[4][temp]);
              p_Quant->q_params_8x8[2][1][k][j][i].InvScaleComp =
                  dequant_coef8[k_mod][j][i] *
                  (p_QScale->UseDefaultScalingMatrix8x8Flag[4]
                       ? Quant8_intra_default[temp]
            ...
            if (!present[5]) {
              ...
              p_Quant->q_params_8x8[2][0][k][j][i].ScaleComp =
                  (quant_coef8[k_mod][j][i] << 4) /
                  (p_QScale->UseDefaultScalingMatrix8x8Flag[5]
                       ? Quant8_inter_default[temp]
                       : p_QScale->ScalingList8x8[5][temp]);
              p_Quant->q_params_8x8[2][0][k][j][i].InvScaleComp =
                  dequant_coef8[k_mod][j][i] *
                  (p_QScale->UseDefaultScalingMatrix8x8Flag[5]
                       ? Quant8_inter_default[temp]
