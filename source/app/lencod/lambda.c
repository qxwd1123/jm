...
static void SetLambda(VideoParameters *p_Vid, int j, int qp,
                      double lambda_scale) {
  ...
  for (k = F_PEL; k <= Q_PEL; k++) {
    // p_Vid->lambda_me[j][qp][k] =  (p_Inp->MEErrorMetric[k] == ERROR_SSE) ?
    // p_Vid->lambda_md[j][qp] : sqrt(p_Vid->lambda_md[j][qp]);
    // p_Vid->lambda_me[j][qp][k] =  sqrt(p_Vid->lambda_md[j][qp]);
    p_Vid->lambda_me[j][qp][k] =
        (p_Inp->MEErrorMetric[k] == ERROR_SSE && p_Inp->MESoftenSSEMetric == 0)
            ? p_Vid->lambda_md[j][qp]
            : sqrt(p_Vid->lambda_md[j][qp]);
...
static void CalcMaxLamdaMD(VideoParameters *p_Vid, double *p_lambda_md) {
  ...
  max_lambda_md =
      floor(((double)DISTBLK_MAX) / iBits / (1 << LAMBDA_ACCURACY_BITS));
...
void get_implicit_lambda_p_slice(Slice *currSlice) {
  ...
  double lambda_md;
  double lambda_scale =
      p_Inp->DisableDistanceLambdaScale
          ? 1.0
          : 1.0 - dClip3(0.0, 0.5, 0.05 * (double)p_Inp->jumpd);
  // limit lambda for mode decison;
  int bLimitsLambdaMD = ((p_Inp->EnableIPCM > 0) && (IMGTYPE == 0));
  double dMaxLambdaMD = 0;

  if (bLimitsLambdaMD) CalcMaxLamdaMD(p_Vid, &dMaxLambdaMD);
  ...
  for (qp = -currSlice->bitdepth_luma_qp_scale; qp < 52; qp++) {
    ...
    lambda_md = ((p_Inp->MEErrorMetric[H_PEL] == ERROR_SATD &&
                  p_Inp->MEErrorMetric[Q_PEL] == ERROR_SATD)
                     ? 1.00
                     : 0.95) *
                lambda_md;
    ...
    if (p_Inp->CtxAdptLagrangeMult == 1) {
      ...
      p_Vid->lambda_mf_factor[P_SLICE][qp] =
          log(p_Vid->lambda_me[P_SLICE][lambda_qp][Q_PEL] + 1.0) / log(2.0);
    }
  }
}
  ...
  for (qp = -currSlice->bitdepth_luma_qp_scale; qp < 52; qp++) {
    ...
    if (p_cur_frm->layer != 0 && currSlice->nal_reference_idc) {
      if (p_Inp->HierarchicalCoding == 2)
        lambda_md *=
            (1.0 - dmin(0.4, 0.2 * (double)(p_cur_frm->p_atom->gop_levels -
                                            p_cur_frm->layer)));
      else
        lambda_md *= 0.80;
    }
    ...
    if (p_Inp->CtxAdptLagrangeMult == 1) {
      int lambda_qp =
          (qp >= 32 && !p_Inp->RCEnable) ? imax(0, qp - 4) : imax(0, qp - 6);
      p_Vid->lambda_mf_factor[B_SLICE][qp] =
          log(p_Vid->lambda_me[B_SLICE][lambda_qp][Q_PEL] + 1.0) / log(2.0);
    }
  }
}
...
void get_implicit_lambda_i_slice(Slice *currSlice) {
  ...
  for (qp = -currSlice->bitdepth_luma_qp_scale; qp < 52; qp++) {
    ...
    if (p_Inp->CtxAdptLagrangeMult == 1) {
      int lambda_qp =
          (qp >= 32 && !p_Inp->RCEnable) ? imax(0, qp - 4) : imax(0, qp - 6);
      p_Vid->lambda_mf_factor[I_SLICE][qp] =
          log(p_Vid->lambda_me[I_SLICE][lambda_qp][Q_PEL] + 1.0) / log(2.0);
    }
  }
}
...
void get_implicit_lambda_sp_slice(Slice *currSlice) {
  ...
  for (qp = -currSlice->bitdepth_luma_qp_scale; qp < 52; qp++) {
    ...
    if (p_Inp->UseRDOQuant && p_Vid->type == I_SLICE && p_Vid->qp == qp)
      ...
      lambda_md =
          0.68 * pow(2.0, qp_temp / 3.0) * dClip3(1.4, 3.0, (qp_temp / 12.0));
    } else
      lambda_md =
          0.85 * pow(2.0, qp_temp / 3.0) * dClip3(1.4, 3.0, (qp_temp / 12.0));
    ...
    if (p_Inp->CtxAdptLagrangeMult == 1) {
      int lambda_qp =
          (qp >= 32 && !p_Inp->RCEnable) ? imax(0, qp - 4) : imax(0, qp - 6);
      p_Vid->lambda_mf_factor[slice_index][qp] =
          log(p_Vid->lambda_me[slice_index][lambda_qp][Q_PEL] + 1.0) / log(2.0);
    }
  }
}
...
void get_explicit_lambda(Slice *currSlice) {
  ...
  for (qp = -currSlice->bitdepth_luma_qp_scale; qp < 52; qp++) {
    ...
    if (currSlice->view_id) {
      p_Vid->lambda_md[j][qp] =
          p_Vid->lambda_md[j][qp] * p_Inp->enh_layer_me_lambda_multiplier;
    }
#endif

    // clip lambda;
    if (bLimitsLambdaMD) ClipLambda(&dMaxLambdaMD, &p_Vid->lambda_md[j][qp]);

    SetLambda(p_Vid, j, qp,
              ((p_Inp->MEErrorMetric[H_PEL] == ERROR_SATD &&
                p_Inp->MEErrorMetric[Q_PEL] == ERROR_SATD)
                   ? 1.00
                   : 0.95));
    ...
    if (currSlice->view_id) {
      p_Vid->lambda_md[j][qp] =
          tmp_lambda_md * p_Inp->enh_layer_lambda_multiplier;
      // clip lambda;
      if (bLimitsLambdaMD) ClipLambda(&dMaxLambdaMD, &p_Vid->lambda_md[j][qp]);
      p_Vid->lambda_md[j][qp] *= ((p_Inp->MEErrorMetric[H_PEL] == ERROR_SATD &&
                                   p_Inp->MEErrorMetric[Q_PEL] == ERROR_SATD)
                                      ? 1.00
                                      : 0.95);
    }
