 ...
 *      Code modularized to support more distortion estimation algorithms in
 *June 2009 by
...
int allocate_errdo_mem(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  memory_size += get_mem3Dint(&p_Vid->p_decs->res_img, MAX_PLANE, MB_BLOCK_SIZE,
                              MB_BLOCK_SIZE);
  memory_size += get_mem3Dint(&p_Vid->p_decs->res_mb_best8x8, MAX_PLANE,
  ...
  p_Vid->p_decs->trans_dist_wo_res =
      NULL;  // it is used for P8x8, where residual may be set to 0
  p_Vid->p_decs->trans_dist_wo_res_bestY_b8x8 =
      NULL;  // it is used for P8x8, where residual may be set to 0
  p_Vid->p_decs->trans_err_bestY_mb = NULL;
  p_Vid->p_decs->trans_err_bestY_b8x8 = NULL;
  p_Vid->p_decs->trans_err_wo_res =
      NULL;  // it is used for P8x8, where residual may be set to 0
  p_Vid->p_decs->trans_err_wo_res_bestY_b8x8 =
      NULL;  // it is used for P8x8, where residual may be set to 0
  ...
  switch (p_Inp->de) {
    case LLN:
      ...
      memory_size +=
          get_mem3Dpel(&p_Vid->p_decs->dec_mb_pred, p_Inp->NoOfDecoders,
                       MB_BLOCK_SIZE, MB_BLOCK_SIZE);
      memory_size +=
          get_mem3Dpel(&p_Vid->p_decs->dec_mbY_best, p_Inp->NoOfDecoders,
                       MB_BLOCK_SIZE, MB_BLOCK_SIZE);
      memory_size +=
          get_mem4Dpel(&p_Vid->p_decs->dec_mbY_best8x8, 2, p_Inp->NoOfDecoders,
                       MB_BLOCK_SIZE, MB_BLOCK_SIZE);
      memory_size +=
          get_mem4Dpel(&p_Vid->p_decs->dec_mb_pred_best8x8, 2,
                       p_Inp->NoOfDecoders, MB_BLOCK_SIZE, MB_BLOCK_SIZE);
      break;
    default:;
  }
  return memory_size;
 ...
 *    (Currently only copy concealment is implemented. Can extend to other
 *concealment types when available.)
 *
 *************************************************************************************
 */
...
/******************************************************************************************
 *
 *  Finds reference picture with nearest POC to current picture to use for error
 *concealment
 *
 *******************************************************************************************
 */
StorablePicture *find_nearest_ref_picture(DecodedPictureBuffer *p_Dpb,
                                          int poc) {
  unsigned int i;
 ...
 *************************************************************************************
 */
void errdo_compute_residue(Macroblock *currMB, imgpel **imgY, int **res_img,
                           imgpel **mb_pred, int b8block, int block_size) {
  int i, j;
...
void errdo_store_best_b8x8(Macroblock *currMB, int transform8x8, int block) {
  ...
  switch (p_Inp->de) {
    case LLN:
      errdo_store_best_block_multihyp(
          p_Inp, p_Vid->p_decs->dec_mbY_best8x8[transform8x8],
          p_Vid->enc_picture->de_mem->p_dec_img[0], block, currMB->pix_x,
          currMB->pix_y, BLOCK_SIZE_8x8);
      errdo_store_best_block_multihyp(
          p_Inp, p_Vid->p_decs->dec_mb_pred_best8x8[transform8x8],
          p_Vid->p_decs->dec_mb_pred, block, 0, 0, BLOCK_SIZE_8x8);
      break;
    default:;
  }
}
 ...
 *************************************************************************************
 * \brief
 *    get the best 8x8 block of estimated distortion. Original code seems to
 *have problem. Why not get p_Vid->p_decs->dec_mb_pred_best8x8? But since there
 *is errdo_get_best_P8x8() in SetCoeffAndReconstruction8x8(),
 *errdo_get_best_b8x8 seems not necessary.
 *
 * \param
 *
 * \note
...
void errdo_get_best_b8x8(Macroblock *currMB, int transform8x8, int block) {
  ...
  switch (p_Inp->de) {
    case LLN:
      errdo_get_best_block_multihyp(
          currMB, p_Vid->enc_picture->de_mem->p_dec_img[0],
          p_Vid->p_decs->dec_mbY_best8x8[transform8x8], block, BLOCK_SIZE_8x8);
      break;
    default:;
  }
}
...
void errdo_store_best_MB(Macroblock *currMB) {
  ...
  switch (p_Inp->de) {
    case LLN:
      errdo_store_best_block_multihyp(p_Inp, p_Vid->p_decs->dec_mbY_best,
                                      p_Vid->enc_picture->de_mem->p_dec_img[0],
                                      0, currMB->pix_x, currMB->pix_y,
                                      MB_BLOCK_SIZE);
      break;
    default:;
  }
...
void errdo_get_best_MB(Macroblock *currMB) {
  ...
  switch (p_Inp->de) {
    case LLN:
      errdo_get_best_block_multihyp(
          currMB, p_Vid->enc_picture->de_mem->p_dec_img[0],
          p_Vid->p_decs->dec_mbY_best, 0, MB_BLOCK_SIZE);
      break;
    default:;
  }
}
  ...
  switch (p_Inp->de) {
    case LLN:
      if (p_Vid->p_decs->rec_type == 0) {
        errdo_get_best_block_multihyp(
            currMB, p_Vid->enc_picture->de_mem->p_dec_img[0],
            p_Vid->p_decs->dec_mb_pred_best8x8[transform8x8], 0, MB_BLOCK_SIZE);
      } else {
        errdo_get_best_block_multihyp(
            currMB, p_Vid->enc_picture->de_mem->p_dec_img[0],
            p_Vid->p_decs->dec_mbY_best8x8[transform8x8], 0, MB_BLOCK_SIZE);
      }
      break;
    default:;
  }
 ...
 *************************************************************************************
 */
void errdo_alloc_storable_picture(StorablePicture *p, VideoParameters *p_Vid,
                                  InputParameters *p_Inp, int size_x,
                                  int size_y, int size_x_cr, int size_y_cr) {
  Dist_Estm *s;
  int dec, ndec, nplane;
  ...
  switch (p_Inp->de) {
    case LLN:
      ...
      if (ndec == 0) {
        printf(
            "Number of decoders cannot be zero for LLN and fast LLN "
            "algorithms, resetting to 30");
        ndec = 30;
      }
      get_mem3D(&(s->mb_error_map), ndec, size_y / MB_BLOCK_SIZE,
                size_x / MB_BLOCK_SIZE);
      get_mem3Dpel(&(s->dec_imgY), ndec, size_y, size_x);

      // This seems somewhat inefficient. Why not allocate array as [ndec][x]
      // where x goes from 0 to 2?
      if ((s->p_dec_img[0] = (imgpel ***)calloc(ndec, sizeof(imgpel **))) ==
          NULL) {
      ...
      if (p_Vid->yuv_format != YUV400) {
        ...
        if ((s->p_dec_img[1] = (imgpel ***)calloc(ndec, sizeof(imgpel **))) ==
            NULL) {
          no_mem_exit("errdo.c: p_dec_img[1]");
        }
        if ((s->p_dec_img[2] = (imgpel ***)calloc(ndec, sizeof(imgpel **))) ==
            NULL) {
