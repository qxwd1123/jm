...
void img2buf(imgpel **imgX, unsigned char *buf, int size_x, int size_y,
  ...
  if ((sizeof(char) == sizeof(imgpel)) &&
    ...
    if (crop_left == 0 && crop_top == 0 && crop_right == 0 &&
      ...
      for (j = 0; j < theight; j++) memcpy(buf + j * twidth, imgX[j], twidth);
    } else {
      for (i = 0; i < theight; i++)
        // memcpy(buf + crop_left + (i * twidth),&(imgX[i +
        // crop_top][crop_left]), twidth); //? bug???
    ...
    if (testEndian()) {
      ...
      switch (symbol_size_in_bytes) {
        ...
        case 2: {
          for (i = crop_top; i < size_y - crop_bottom; i++)
            for (j = crop_left; j < size_x - crop_right; j++) {
              ...
              memcpy(buf + ((j - crop_left + ((i - crop_top) * (twidth))) * 2),
                     &(ui16), 2);
            }
          break;
        }
        case 4: {
          for (i = crop_top; i < size_y - crop_bottom; i++)
            for (j = crop_left; j < size_x - crop_right; j++) {
              ...
              memcpy(buf + ((j - crop_left + ((i - crop_top) * (twidth))) * 4),
                     &(ui32), 4);
            }
          break;
        }
        default: {
          error(
              "writing only to formats of 8, 16 or 32 bit allowed on big "
              "endian architecture",
              500);
          break;
        }
      ...
      if (sizeof(imgpel) < symbol_size_in_bytes) {
        // this should not happen. we should not have smaller imgpel than our
        // source material.
        size = sizeof(imgpel);
        // clear buffer
      ...
      for (i = crop_top; i < size_y - crop_bottom; i++) {
        ...
        for (j = crop_left; j < size_x - crop_right; j++) {
          memcpy(buf + ((j + i_pos) * symbol_size_in_bytes), &(imgX[i][j]),
                 size);
        }
      }
    }
...
void write_out_picture(StorablePicture *p, FrameFormat *output, int p_out) {
  ...
  Boolean rgb_output =
      (Boolean)(output->color_model != CM_YUV && output->yuv_format == YUV444);
  unsigned char *buf;

  if (p->non_existing) return;
  ...
  if (rgb_output) {
    ...
    if (p->frame_cropping_flag) {
      ...
      crop_top = SubHeightC[p->chroma_format_idc] *
                 (2 - p->frame_mbs_only_flag) * p->frame_crop_top_offset;
      crop_bottom = SubHeightC[p->chroma_format_idc] *
                    (2 - p->frame_mbs_only_flag) * p->frame_crop_bottom_offset;
    } else {
      crop_left = crop_right = crop_top = crop_bottom = 0;
  ...
  img2buf(p->imgY, buf, p->size_x, p->size_y, symbol_size_in_bytes, crop_left,
  ...
  if (p->chroma_format_idc != YUV400) {
    ...
    if (!rgb_output) {
      img2buf(p->imgUV[1], buf, p->size_x_cr, p->size_y_cr,
...
void clear_picture(VideoParameters *p_Vid, StorablePicture *p) {
  ...
  if (p_Vid->bitdepth_luma == 8) {
    // this does not seem to be right since it seems to work only for imgpel ==
    // byte should be fixed
    for (i = 0; i < p->size_y; i++)
      memset(p->imgY[i], p_Vid->dc_pred_value_comp[0],
  ...
  if (p_Vid->bitdepth_chroma == 8) {
    for (i = 0; i < p->size_y_cr; i++)
      memset(p->imgUV[0][i], p_Vid->dc_pred_value_comp[1],
             p->size_x_cr * sizeof(imgpel));
    for (i = 0; i < p->size_y_cr; i++)
      memset(p->imgUV[1][i], p_Vid->dc_pred_value_comp[2],
             p->size_x_cr * sizeof(imgpel));
  } else {
    int k, j;
    for (k = 0; k < 2; k++) {
 ...
 *generated and combined into the frame buffer. \param p_Vid point to
 *VideoParameters \param fs FrameStore that contains a single field \param
 *output Frame format to \param p_out Output file
 ************************************************************************
 */
void write_unpaired_field(VideoParameters *p_Vid, FrameStore *fs,
                          FrameFormat *output, int p_out) {
  StorablePicture *p;
  assert(fs->is_used < 3);
  if (fs->is_used & 1) {
    // we have a top field
    ...
    p = fs->top_field;
    fs->bottom_field = alloc_storable_picture(
        p_Vid, BOTTOM_FIELD, p->size_x, p->size_y, p->size_x_cr, p->size_y_cr);
  ...
  if (fs->is_used & 2) {
    ...
    p = fs->bottom_field;
    fs->top_field = alloc_storable_picture(
        p_Vid, TOP_FIELD, p->size_x, p->size_y, p->size_x_cr, p->size_y_cr);
    ...
    if (fs->top_field->frame_cropping_flag) {
      fs->top_field->frame_crop_top_offset =
          fs->bottom_field->frame_crop_top_offset;
      fs->top_field->frame_crop_bottom_offset =
          fs->bottom_field->frame_crop_bottom_offset;
      fs->top_field->frame_crop_left_offset =
          fs->bottom_field->frame_crop_left_offset;
      fs->top_field->frame_crop_right_offset =
          fs->bottom_field->frame_crop_right_offset;
    }
    dpb_combine_field_yuv(p_Vid, fs);
    write_picture(fs->frame, output, p_out);
  }
 ...
 ************************************************************************
 */
void flush_direct_output(VideoParameters *p_Vid, FrameFormat *output,
                         int p_out) {
...
void direct_output(VideoParameters *p_Vid, StorablePicture *p,
  ...
  switch (p->structure) {
    ...
    case BOTTOM_FIELD:
      ...
      break;
    default: printf("invalid picture type\n"); break;
  }
...
************************************************************************
*/
void direct_output_paff(VideoParameters *p_Vid, StorablePicture *p,
                        FrameFormat *output, int p_out) {
