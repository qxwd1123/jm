...
static void generateChroma00(VideoParameters *p_Vid, int size_x_minus1,
                             int size_y_minus1, imgpel **wImgDst,
                             imgpel **imgUV) {
  int i;  //, j;
  ...
  for (; jpad < 0; jpad++) {
    memcpy(wImgDst[jpad] - p_Vid->pad_size_uv_x,
           wImgDst[jpad - 1] - p_Vid->pad_size_uv_x,
           (2 * p_Vid->pad_size_uv_x + size_x_minus1 + 1) * sizeof(imgpel));
  }
  ...
  for (jpad = size_y_minus1 + 1;
       jpad < size_y_minus1 + 1 + p_Vid->pad_size_uv_y; jpad++) {
    memcpy(wImgDst[jpad] - p_Vid->pad_size_uv_x,
           wImgDst[jpad - 1] - p_Vid->pad_size_uv_x,
           (2 * p_Vid->pad_size_uv_x + size_x_minus1 + 1) * sizeof(imgpel));
  }
 ...
 ************************************************************************
 */
static void generateChroma01(VideoParameters *p_Vid, int size_x_minus1,
                             int size_y_minus1, int weight00, int weight01,
                             imgpel **wImgDst, imgpel **imgUV) {
  int i;  //, j;
  int jpad = -p_Vid->pad_size_uv_y;
  int cur_value;
  imgpel *wBufDst;
 ...
 ************************************************************************
 */
static void generateChroma10(VideoParameters *p_Vid, int size_x_minus1,
                             int size_y_minus1, int weight00, int weight10,
                             imgpel **wImgDst, imgpel **imgUV) {
  int i;  //, j;
  int jpad = -p_Vid->pad_size_uv_y;
  int cur_value;
  ...
  for (jpad = 0; jpad < size_y_minus1; jpad++) {
    ...
    cur_value =
        rshift_rnd_sf(weight00 * (*wBufSrc0) + weight10 * (*wBufSrc1), 6);
    for (i = -p_Vid->pad_size_uv_x; i < 0; i++) {
 ...
 ************************************************************************
 */
static void generateChromaXX(VideoParameters *p_Vid, int size_x_minus1,
                             int size_y_minus1, int weight00, int weight01,
                             int weight10, int weight11, imgpel **wImgDst,
                             imgpel **imgUV) {
  int i;  //, j;
  int jpad = -p_Vid->pad_size_uv_y;
  int cur_value;
  ...
  for (jpad = 0; jpad < size_y_minus1; jpad++) {
    ...
    cur_value =
        rshift_rnd_sf(weight0001 * (*wBufSrc0) + weight1011 * (*wBufSrc1), 6);
    for (i = -p_Vid->pad_size_uv_x; i < 0; i++) {
      *(wBufDst++) = (imgpel)cur_value;
    }
  ...
  for (uv = 0; uv < 2; uv++) {
    ...
    for (suby = 0, k = 0; suby < subimages_y; suby++, k += mul_y) {
      ...
      for (subx = 0, l = 0; subx < subimages_x; subx++, l += mul_x) {
        ...
        {
          generateChroma00(p_Vid, size_x_minus1, size_y_minus1,
                           curr_img_sub[suby][subx], curr_img);
        } else if (weight10 == 0 && weight11 == 0)  // horizontal
        {
          generateChroma01(p_Vid, size_x_minus1, size_y_minus1, weight00,
                           weight01, curr_img_sub[suby][subx], curr_img);
        } else if (weight01 == 0 && weight11 == 0)  // vertical
        {
          generateChroma10(p_Vid, size_x_minus1, size_y_minus1, weight00,
                           weight10, curr_img_sub[suby][subx], curr_img);
        } else  // diagonal
        {
          generateChromaXX(p_Vid, size_x_minus1, size_y_minus1, weight00,
                           weight01, weight10, weight11,
                           curr_img_sub[suby][subx], curr_img);
        }
      }
    }
