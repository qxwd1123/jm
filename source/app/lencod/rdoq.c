...
void init_rdoq_slice(Slice *currSlice) {
  // currSlice->norm_factor_4x4 = (double) ((int64) 1 << (2 * DQ_BITS + 19)); //
  // norm factor 4x4 is basically (1<<31) currSlice->norm_factor_8x8 = (double)
  // ((int64) 1 << (2 * Q_BITS_8 + 9)); // norm factor 8x8 is basically (1<<41)
...
****************************************************************************
*/
int init_trellis_data_DC_cr_CAVLC(Macroblock *currMB, int **tblock, int qp_per,
  ...
  for (coeff_ctr = 0; coeff_ctr < end_coeff_ctr; coeff_ctr++) {
    ...
    if (*m7 == 0) {
      ...
      if (dataLevel->noLevels == 1)
        ...
        dataLevel->pre_level =
            (iabs(*m7) * q_params->ScaleComp + q_params->OffsetComp) >> q_bits;
...
****************************************************************************
*/
int init_trellis_data_DC_cr_CABAC(Macroblock *currMB, int **tblock, int qp_per,
                                  int qp_rem, LevelQuantParams *q_params,
                                  const byte *p_scan,
...
void get_dQP_table(Slice *currSlice) {
  int qp_offset = (currSlice->slice_type == B_SLICE)
                      ? (currSlice->RDOQ_QP_Num / 3)
                      : (currSlice->RDOQ_QP_Num >> 1);
...
void trellis_mp(Macroblock *currMB) {
  ...
  if (p_Inp->symbol_mode == CABAC) {
    ...
    if (p_Inp->Transform8x8Mode) estRunLevel_CABAC(currMB, LUMA_8x8);
  ...
  for (deltaQPCnt = 0; deltaQPCnt < currSlice->RDOQ_QP_Num; deltaQPCnt++) {
    ...
    // It seems that pushing the masterQP as first helps things when fast me is
    // enabled. Could there be an issue with motion estimation?
...
#if RDOQ_BASE
    if (deltaQP != 0 &&
        !(p_Vid->qp - qp_anchor >= -2 && p_Vid->qp - qp_anchor <= 1) &&
        currMB->mb_left && currMB->mb_up && currSlice->slice_type == P_SLICE)
      continue;
    if (deltaQP != 0 &&
        !(p_Vid->qp - qp_anchor >= -1 && p_Vid->qp - qp_anchor <= 2) &&
        currMB->mb_left && currMB->mb_up && currSlice->slice_type == B_SLICE)
      continue;
#endif
    ...
    if (currSlice->rddata_trellis_curr.min_rdcost <
        currSlice->rddata_trellis_best.min_rdcost)
      copy_rddata_trellis(currMB, &currSlice->rddata_trellis_best,
                          currSlice->rddata);

    if (p_Inp->RDOQ_CP_MV) currSlice->rdoq_motion_copy = 1;

#if (!RDOQ_BASE)
    if (p_Inp->RDOQ_Fast == 1) {
      if ((p_Vid->qp - currSlice->rddata_trellis_best.qp > 1)) break;
      if ((currSlice->rddata_trellis_curr.cbp == 0) &&
          (currSlice->rddata_trellis_curr.mb_type != 0))
        break;
      if ((currSlice->rddata_trellis_best.mb_type == 0) &&
          (currSlice->rddata_trellis_best.cbp == 0))
        break;
    }
#endif
  }
...
void RDOQ_update_mode(Slice *currSlice, RD_PARAMS *enc_mb) {
  ...
  int *InterSearch =
      p_Inp->InterSearch[(currSlice->p_Vid->num_of_layers > 1)
                             ? currSlice->view_id
                             : 0][(currSlice->slice_type == B_SLICE)];
#else
  ...
  int i;
  for (i = 0; i < MAXMODE; i++) enc_mb->valid[i] = 0;
  ...
  if (mb_type == P8x8) {
    ...
    enc_mb->valid[5] =
        (short)(InterSearch[5] && !(p_Inp->Transform8x8Mode == 2));
    enc_mb->valid[6] =
        (short)(InterSearch[6] && !(p_Inp->Transform8x8Mode == 2));
    enc_mb->valid[7] =
        (short)(InterSearch[7] && !(p_Inp->Transform8x8Mode == 2));
  }
}
...
void copy_rddata_trellis(Macroblock *currMB, RD_DATA *dest, RD_DATA *src) {
  ...
  memcpy(&dest->rec_mb[0][0][0], &src->rec_mb[0][0][0],
         MB_PIXELS * sizeof(imgpel));

  if (p_Vid->yuv_format != YUV400) {
    memcpy(&dest->rec_mb[1][0][0], &src->rec_mb[1][0][0],
           2 * MB_PIXELS * sizeof(imgpel));
  }

  memcpy(&dest->cofAC[0][0][0][0], &src->cofAC[0][0][0][0],
  ...
  if (p_Vid->type != I_SLICE && p_Vid->type != SI_SLICE) {
    ...
    memcpy(&dest->all_mv[0][0][0][0][0], &src->all_mv[0][0][0][0][0],
           p_Vid->max_num_references * 288 *
               sizeof(MotionVector));  // 2 * ref * 9 * 4 * 4
  }

  memcpy(dest->intra_pred_modes, src->intra_pred_modes,
         MB_BLOCK_PARTITIONS * sizeof(char));
  memcpy(dest->intra_pred_modes8x8, src->intra_pred_modes8x8,
         MB_BLOCK_PARTITIONS * sizeof(char));
  for (j = currMB->block_y; j < currMB->block_y + BLOCK_MULTIPLE; j++)
    memcpy(&dest->ipredmode[j][currMB->block_x],
           &src->ipredmode[j][currMB->block_x], BLOCK_MULTIPLE * sizeof(char));

  memcpy(&dest->refar[LIST_0][0][0], &src->refar[LIST_0][0][0],
         2 * BLOCK_MULTIPLE * BLOCK_MULTIPLE * sizeof(char));
}

void updateMV_mp(Macroblock *currMB, distblk *m_cost, short ref, int list,
                 int h, int v, int blocktype, int block8x8) {
  InputParameters *p_Inp = currMB->p_Inp;
  Slice *currSlice = currMB->p_Slice;
