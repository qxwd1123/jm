...
void encode_one_macroblock_highloss(Macroblock *currMB) {
  ...
  int lambda_mf[3];
  short runs = (short)((p_Inp->RestrictRef == 1 &&
                        (pslice || (bslice && p_Vid->nal_reference_idc > 0)))
                           ? 2
                           : 1);
  ...
  for (rerun = 0; rerun < runs; rerun++) {
    if (runs == 2) p_Inp->rdopt = (rerun == 0) ? 1 : 3;
    ...
    if (!intra) {
        ...
        if (enc_mb.valid[mode]) {
          for (cost = 0, block = 0; block < (mode == 1 ? 1 : 2); block++) {
            ...
            if (bslice) {
              ...
              // references
              list_prediction_cost(currMB, BI_PRED, block, mode, &enc_mb,
                                   bmcost, best.ref);
            ...
            if (mode > 1 && block == 0)
              currSlice->set_ref_and_motion_vectors(currMB, motion, &best,
                                                    block);
      ...
      if (enc_mb.valid[P8x8]) {
        ...
        if (p_Inp->Transform8x8Mode) {
          ...
          for (block = 0; block < 4; block++) {
            currSlice->submacroblock_mode_decision(
                currMB, &enc_mb, p_RDO->tr8x8, p_RDO->cofAC8x8ts[block], block,
                &cost);
        ...
        if (p_Inp->Transform8x8Mode != 2) {
          ...
          for (block = 0; block < 4; block++) {
            currSlice->submacroblock_mode_decision(
                currMB, &enc_mb, p_RDO->tr4x4, p_RDO->coefAC8x8[block], block,
                &cost);
            ...
            if (p_Inp->SkipIntraInInterSlices) {
              if (!intra && mode >= I4MB) {
                if (currMB->best_mode <= 3 && currMB->best_cbp == 0 &&
                    (currMB->min_rdcost <
                     weighted_cost(enc_mb.lambda_mdfp, 5))) {
                  continue;
    ...
    if (rerun == 0) intra1 = is_intra(currMB);
  }  // for (rerun=0; rerun<runs; rerun++)
