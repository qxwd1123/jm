...
static void set_ref_and_motion_vectors_P_slice(Macroblock *currMB,
...
static distblk compute_sad4x4_cost(VideoParameters *p_Vid, imgpel **cur_img,
                                   imgpel **prd_img, int pic_opix_x,
                                   distblk min_cost);
static distblk compute_sse4x4_cost(VideoParameters *p_Vid, imgpel **cur_img,
                                   imgpel **prd_img, int pic_opix_x,
                                   distblk min_cost);
static distblk compute_satd4x4_cost(VideoParameters *p_Vid, imgpel **cur_img,
                                    imgpel **prd_img, int pic_opix_x,
                                    distblk min_cost);
static distblk compute_comp4x4_cost(VideoParameters *p_Vid, imgpel **cur_img,
                                    imgpel **prd_img, int pic_opix_x,
                                    distblk min_cost);

static distblk rdcost_for_4x4_intra_blocks(Macroblock *currMB, int *nonzero,
                                           int b8, int b4, int ipmode,
                                           int lambda, int mostProbableMode,
                                           distblk min_rdcost);
static distblk rdcost_for_4x4_intra_blocks_444(Macroblock *currMB, int *nonzero,
                                               int b8, int b4, int ipmode,
                                               int lambda, int mostProbableMode,
                                               distblk min_rdcost);

static inline void copy_motion_vectors_MB(Slice *currSlice, RD_DATA *rdopt) {
  memcpy(&currSlice->all_mv[LIST_0][0][0][0][0],
         &rdopt->all_mv[LIST_0][0][0][0][0],
         288 * currSlice->max_num_references * sizeof(MotionVector));
}
...
void init_rdopt(Slice *currSlice) {
  ...
  currSlice->set_lagrangian_multipliers = p_Inp->rdopt == 0
                                              ? SetLagrangianMultipliersOff
                                              : SetLagrangianMultipliersOn;

  switch (p_Inp->rdopt) {
    case 0: currSlice->encode_one_macroblock = encode_one_macroblock_low; break;
    case 1:
    default:
  ...
  if (currSlice->mb_aff_frame_flag ||
      (currSlice->UseRDOQuant && currSlice->RDOQ_QP_Num > 1))
    currSlice->set_stored_mb_parameters =
        set_stored_macroblock_parameters_mpass;
  else {
  ...
  currSlice->rdcost_for_4x4_intra_blocks = (p_Vid->yuv_format == YUV444)
                                               ? rdcost_for_4x4_intra_blocks_444
                                               : rdcost_for_4x4_intra_blocks;
  currSlice->rdcost_for_8x8_intra_blocks = (p_Vid->yuv_format == YUV444)
                                               ? rdcost_for_8x8_intra_blocks_444
                                               : rdcost_for_8x8_intra_blocks;
  ...
  if (p_Inp->rdopt == 0) {
    currSlice->mode_decision_for_I8x8_blocks =
        (p_Vid->yuv_format == YUV444) ? mode_decision_for_I8x8_blocks_JM_Low444
                                      : mode_decision_for_I8x8_blocks_JM_Low;
    currSlice->mode_decision_for_I4x4_blocks =
        (p_Vid->yuv_format == YUV444) ? mode_decision_for_I4x4_blocks_JM_Low444
                                      : mode_decision_for_I4x4_blocks_JM_Low;
  } else {
    currSlice->mode_decision_for_I8x8_blocks =
        (p_Vid->yuv_format == YUV444) ? mode_decision_for_I8x8_blocks_JM_High444
                                      : mode_decision_for_I8x8_blocks_JM_High;
    currSlice->mode_decision_for_I4x4_blocks =
        (p_Vid->yuv_format == YUV444) ? mode_decision_for_I4x4_blocks_JM_High444
                                      : mode_decision_for_I4x4_blocks_JM_High;
  }
 ...
 *
 * \note
 *    The new values of the p_Vid->pixel_map are taken from the temporary buffer
 *p_Vid->refresh_map
 *
 *************************************************************************************
 */
    ...
    for (my = 0; my < p_Vid->height >> 3; my++) {
      for (mx = 0; mx < p_Vid->width >> 3; mx++) {
        ...
        if (p_Vid->refresh_map[my][mx]) {
          ...
          for (y = my * 8; y < j; y++) {
            for (x = mx * 8; x < i; x++) {
              p_Vid->pixel_map[y][x] = (byte)imin(p_Vid->pixel_map[y][x] + 1,
 ...
 *    The information about intra-refreshed areas is kept in the
 *p_Vid->pixel_map.
 *
 *************************************************************************************
 */
int CheckReliabilityOfRef(Macroblock *currMB, int block, int list_idx, int ref,
                          int mode) {
      ...
      if (dy == 0 && dx == 0)  // full-pel
      {
        for (y = y_pos; y < y_pos + BLOCK_SIZE; y++)
          for (x = x_pos; x < x_pos + BLOCK_SIZE; x++)
            if (p_Vid->pixel_map[iClip3(0, maxold_y, y)]
                                [iClip3(0, maxold_x, x)] < ref_frame)
              return 0;
          ...
          for (y = y_pos; y < y_pos + BLOCK_SIZE; y++) {
            ...
            for (x = x_pos; x < x_pos + BLOCK_SIZE; x++) {
              for (xx = -2; xx < 4; xx++) {
                ...
                if (p_Vid->pixel_map[pres_y][pres_x] < ref_frame) return 0;
              }
            }
          }
        } else if (dx == 0) {
...
static distblk rdcost_for_4x4_intra_blocks(Macroblock *currMB, int *nonzero,
  ...
  *nonzero = currMB->residual_transform_quant_luma_4x4(currMB, PLANE_Y, block_x,
                                                       block_y, &dummy, 1);

  //===== get distortion (SSD) of 4x4 block =====
  distortion += compute_SSE4x4(&p_Vid->pCurImg[pic_opix_y],
                               &p_Vid->enc_picture->imgY[pic_pix_y], pic_pix_x,
                               pic_pix_x);
  ...
  //=====
  se.value1 = (mostProbableMode == ipmode) ? -1
              : ipmode < mostProbableMode  ? ipmode
                                           : ipmode - 1;
  ...
  rdcost =
      distortion + weighted_cost(lambda, rate);  //((distblk)lambda) * rate;
  ...
  *nonzero = currMB->residual_transform_quant_luma_4x4(currMB, PLANE_Y, block_x,
  ...
  if (p_Vid->P444_joined) {
    for (k = PLANE_U; k <= PLANE_V; k++) {
      ...
      currMB->c_nzCbCr[k] = currMB->residual_transform_quant_luma_4x4(
          currMB, k, block_x, block_y, &dummy, 1);
      distortion += compute_SSE4x4(&p_Vid->pCurImg[pic_opix_y],
                                   &p_Vid->enc_picture->p_curr_img[pic_pix_y],
                                   pic_pix_x, pic_pix_x);
    }
...
distblk rdcost_for_8x8blocks(
  ...
  if (direct) {
    int p_dir =
        currSlice->direct_pdir[currMB->block_y + j0][currMB->block_x + i0];
    if (p_dir < 0)  // mode not allowed
      return (DISTBLK_MAX);
    else {
      int list_mode[2] = { 0, 0 };
      *cnt_nonz = currSlice->luma_residual_coding_8x8(
          currMB, &cbp, cbp_blk, block, (short)p_dir, list_mode,
          currSlice
              ->direct_ref_idx[currMB->block_y + j0][currMB->block_x + i0]);
    }
  } else {
    if (pdir == 2 && active_pps->weighted_bipred_idc == 1) {
      int weight_sum = (active_pps->weighted_bipred_idc == 1)
                           ? currSlice->wbp_weight[0][l0_ref][l1_ref][0] +
                                 currSlice->wbp_weight[1][l0_ref][l1_ref][0]
                           : 0;
    ...
    *cnt_nonz = currSlice->luma_residual_coding_8x8(
        currMB, &cbp, cbp_blk, block, pdir, list_mode, part->ref);
  }
  ...
  if (p_Inp->rdopt == 3) {
    // Zhifeng 090611
    distortion = p_Vid->estimate_distortion(
        currMB, block, 8, mode, pdir, min_rdcost);  //===== get residue =====
  } else {
    distortion += compute_SSE8x8(&p_Vid->pCurImg[currMB->opix_y + pay],
                                 &p_Vid->enc_picture->imgY[currMB->pix_y + pay],
                                 currMB->pix_x + pax, currMB->pix_x + pax);
  }

  if (p_Vid->P444_joined) {
    distortion +=
        compute_SSE8x8(&p_Vid->pImgOrg[1][currMB->opix_y + pay],
                       &p_Vid->enc_picture->imgUV[0][currMB->pix_y + pay],
                       currMB->pix_x + pax, currMB->pix_x + pax);
    distortion +=
        compute_SSE8x8(&p_Vid->pImgOrg[2][currMB->opix_y + pay],
                       &p_Vid->enc_picture->imgUV[1][currMB->pix_y + pay],
                       currMB->pix_x + pax, currMB->pix_x + pax);
  }

  // Early termination
  if (distortion >= min_rdcost) return (distortion);
  ...
  if (!direct) {
    ...
    if ((currSlice->num_ref_idx_active[LIST_1] > 1 &&
    ...
    if (pdir == 0 || pdir == 2) {
      rate += writeMotionVector8x8(currMB, i0, j0, i0 + 2, j0 + 2, l0_ref,
                                   LIST_0, mode, currMB->b8x8[block].bipred);
    }
    if (pdir == 1 || pdir == 2) {
      rate += writeMotionVector8x8(currMB, i0, j0, i0 + 2, j0 + 2, l1_ref,
                                   LIST_1, mode, currMB->b8x8[block].bipred);
    }
  }
  ...
  if (currSlice->symbol_mode != CAVLC) {
    ...
    writeCBP_BIT_CABAC(currMB, block, ((*cnt_nonz > 0) ? 1 : 0), dataTr->cbp8x8,
                       eep_dp, currSlice->tex_ctx);
  ...
  if (*cnt_nonz) {
    rate += writeCoeff8x8(currMB, PLANE_Y, block, mode,
                          currMB->luma_transform_size_8x8_flag);
  }

  if (p_Vid->P444_joined) {
    rate += writeCoeff8x8(currMB, PLANE_U, block, mode,
                          currMB->luma_transform_size_8x8_flag);
    rate += writeCoeff8x8(currMB, PLANE_V, block, mode,
                          currMB->luma_transform_size_8x8_flag);
  }
...
void set_modes_and_refs_for_blocks_p_slice(Macroblock *currMB, short mode) {
  ...
  if (mode == 0 || mode == I4MB || mode == I16MB || mode == I8MB ||
    ...
    if (mode == 1) {
      short curref = b8x8info->best[mode][0].pdir == 0
                         ? b8x8info->best[mode][0].ref[LIST_0]
                         : -1;
      for (j = currMB->block_y; j < currMB->block_y + 4; j++) {
        for (i = currMB->block_x; i < currMB->block_x + 4; i++) {
          motion[j][i].ref_pic[LIST_0] =
              currSlice->listX[LIST_0 + currMB->list_offset][curref];
      ...
      curref = b8x8info->best[mode][2].pdir == 0
                   ? b8x8info->best[mode][2].ref[LIST_0]
                   : -1;
      ...
      curref = (b8x8info->best[mode][1].pdir == 0)
                   ? b8x8info->best[mode][1].ref[LIST_0]
                   : -1;
      for (j = currMB->block_y; j < currMB->block_y + 4; j++) {
      ...
      for (j = 0; j < 4; j++) {
        ...
        motion[block_y][block_x].ref_pic[LIST_0] =
            currSlice->listX[LIST_0 + currMB->list_offset][curref];
...
void set_modes_and_refs_for_blocks_b_slice(Macroblock *currMB, short mode) {
  ...
  switch (mode) {
    case 0:
      ...
      currMB->b8x8[0].pdir =
          currSlice->direct_pdir[currMB->block_y][currMB->block_x];
      currMB->b8x8[1].pdir =
          currSlice->direct_pdir[currMB->block_y][currMB->block_x + 2];
      currMB->b8x8[2].pdir =
          currSlice->direct_pdir[currMB->block_y + 2][currMB->block_x];
      currMB->b8x8[3].pdir =
          currSlice->direct_pdir[currMB->block_y + 2][currMB->block_x + 2];
      break;
    case 1:
    case 2:
    case 3:
...
#define IS_FW                                                                  \
  ((b8x8info->best[mode][k].pdir == 0 || b8x8info->best[mode][k].pdir == 2) && \
   (mode != P8x8 || b8x8info->best[mode][k].mode != 0))
#define IS_BW                                                                  \
  ((b8x8info->best[mode][k].pdir == 1 || b8x8info->best[mode][k].pdir == 2) && \
   (mode != P8x8 || b8x8info->best[mode][k].mode != 0))
    ...
    for (block8x8 = 0; block8x8 < 4; block8x8++) {
      for (clist = LIST_0; clist <= LIST_1; clist++) {
        ...
        if (b8x8info->best[mode][block8x8].pdir == 2) {
          ...
          curref =
              (clist == b8x8info->best[mode][block8x8].pdir) ? bestref : -1;
        }
    ...
    for (j = 0; j < 4; j++) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        if (b8x8info->best[mode][k].mode == 0) {
          motion[block_y][block_x].ref_idx[LIST_0] =
              currSlice->direct_ref_idx[block_y][block_x][LIST_0];
          motion[block_y][block_x].ref_idx[LIST_1] =
              currSlice->direct_ref_idx[block_y][block_x][LIST_1];
        } else {
          motion[block_y][block_x].ref_idx[LIST_0] =
              (IS_FW ? b8x8info->best[mode][k].ref[LIST_0] : -1);
          motion[block_y][block_x].ref_idx[LIST_1] =
              (IS_BW ? b8x8info->best[mode][k].ref[LIST_1] : -1);
        }
      }
    }
  }
  ...
  if (currMB->luma_transform_size_8x8_flag && (currMB->valid_8x8)) {
    ...
    for (j = currMB->block_y; j < currMB->block_y + BLOCK_MULTIPLE; j++) {
      for (i = currMB->block_x; i < currMB->block_x + BLOCK_MULTIPLE; i++) {
        ...
        motion[j][i].ref_pic[LIST_0] =
            (cur_ref >= 0
                 ? currSlice->listX[LIST_0 + currMB->list_offset][cur_ref]
                 : NULL);
      }
    }
    ...
    for (block = 0; block < 4; block++) {
      memcpy(currSlice->cofAC[block][0][0], p_RDO->cofAC8x8ts[block][0][0][0],
             4 * 2 * 65 * sizeof(int));
    }
    ...
    if (p_Vid->P444_joined) {
      for (uv = 0; uv < 2; uv++) {
        for (block = 0; block < 4; block++) {
          memcpy(currSlice->cofAC[4 + block + uv * 4][0][0],
                 p_RDO->cofAC8x8ts[block][uv + 1][0][0],
                 4 * 2 * 65 * sizeof(int));
        }
      }
    }
    ...
    if (p_RDO->tr8x8->cnt_nonz_8x8 <= _LUMA_8x8_COEFF_COST_ &&
        ((currMB->qp_scaled[0]) != 0 ||
         p_Vid->active_sps->lossless_qpprime_flag == 0) &&
        (currSlice->slice_type !=
         SP_SLICE))  // modif ES added last condition (we probably never go
                     // there so is the next modification useful ? check)
    {
      currMB->cbp = 0;
      currMB->cbp_blk = 0;

      copy_image_data_16x16(&p_Vid->enc_picture->imgY[currMB->pix_y],
                            p_RDO->tr8x8->mpr8x8, currMB->pix_x, 0);
      ...
      if (currSlice->slice_type == SP_SLICE && !p_Vid->sp2_frame_indicator) {
        for (j = 0; j < MB_BLOCK_SIZE; j++) {
          memcpy(&p_Vid->lrec[currMB->pix_y + j][currMB->pix_x],
                 p_RDO->tr8x8->lrec[j], MB_BLOCK_SIZE * sizeof(int));
        }
      }
      ...
      if (p_Vid->P444_joined) {
        ...
        copy_image_data_16x16(&p_Vid->enc_picture->imgUV[0][currMB->pix_y],
                              p_RDO->tr8x8->mpr8x8CbCr[0], currMB->pix_x, 0);
        copy_image_data_16x16(&p_Vid->enc_picture->imgUV[1][currMB->pix_y],
                              p_RDO->tr8x8->mpr8x8CbCr[1], currMB->pix_x, 0);

        for (uv = 0; uv < 2; uv++) {
          for (block = 0; block < 4; block++) {
            memset(currSlice->cofAC[4 + block + uv * 4][0][0], 0,
                   4 * 2 * 65 * sizeof(int));
          }
        }
      }
      ...
      copy_image_data_16x16(&p_Vid->enc_picture->imgY[currMB->pix_y],
                            p_RDO->tr8x8->rec_mbY8x8, currMB->pix_x, 0);
      ...
      if (p_Vid->P444_joined) {
        ...
        copy_image_data_16x16(&p_Vid->enc_picture->imgUV[0][currMB->pix_y],
                              p_RDO->tr8x8->rec_mb8x8_cr[0], currMB->pix_x, 0);
        copy_image_data_16x16(&p_Vid->enc_picture->imgUV[1][currMB->pix_y],
                              p_RDO->tr8x8->rec_mb8x8_cr[1], currMB->pix_x, 0);
      }
    }
    if (p_Inp->rdopt == 3) {
    ...
    for (block = 0; block < 4; block++) {
      memcpy(currSlice->cofAC[block][0][0], p_RDO->coefAC8x8[block][0][0][0],
             4 * 2 * 65 * sizeof(int));
    }

    if (currSlice->P444_joined) {
      for (block = 0; block < 4; block++) {
        memcpy(currSlice->cofAC[block + 4][0][0],
               p_RDO->coefAC8x8[block][1][0][0], 4 * 2 * 65 * sizeof(int));
        memcpy(currSlice->cofAC[block + 8][0][0],
               p_RDO->coefAC8x8[block][2][0][0], 4 * 2 * 65 * sizeof(int));
      }
    }

    if (((p_Inp->disthres && p_RDO->tr4x4->cnt_nonz_8x8 <= 0) ||
         (p_RDO->tr4x4->cnt_nonz_8x8 <= 5)) &&
      ...
      copy_image_data_16x16(&p_Vid->enc_picture->imgY[currMB->pix_y],
                            p_RDO->tr4x4->mpr8x8, currMB->pix_x, 0);
      ...
      if (currSlice->slice_type == SP_SLICE && !p_Vid->sp2_frame_indicator) {
        for (j = 0; j < MB_BLOCK_SIZE; j++) {
          memcpy(&p_Vid->lrec[currMB->pix_y + j][currMB->pix_x],
                 p_RDO->tr4x4->lrec[j],
                 MB_BLOCK_SIZE * sizeof(int));  // restore coeff. SP frame
        }
      }
      ...
      if (currSlice->P444_joined) {
        ...
        copy_image_data_16x16(&p_Vid->enc_picture->imgUV[0][currMB->pix_y],
                              p_RDO->tr4x4->mpr8x8CbCr[0], currMB->pix_x, 0);
        copy_image_data_16x16(&p_Vid->enc_picture->imgUV[1][currMB->pix_y],
                              p_RDO->tr4x4->mpr8x8CbCr[1], currMB->pix_x, 0);
      ...
      copy_image_data_16x16(&p_Vid->enc_picture->imgY[currMB->pix_y],
                            p_RDO->tr4x4->rec_mbY8x8, currMB->pix_x, 0);
      ...
      if (currSlice->slice_type == SP_SLICE && !p_Vid->sp2_frame_indicator) {
        for (j = 0; j < MB_BLOCK_SIZE; j++) {
          memcpy(&p_Vid->lrec[currMB->pix_y + j][currMB->pix_x],
                 p_RDO->tr4x4->lrec[j], MB_BLOCK_SIZE * sizeof(int));
        }
      }
      if (currSlice->P444_joined) {
        currSlice->cmp_cbp[1] = currSlice->cmp_cbp[2] = p_RDO->tr4x4->cbp8x8;
        copy_image_data_16x16(&p_Vid->enc_picture->imgUV[0][currMB->pix_y],
                              p_RDO->tr4x4->rec_mb8x8_cr[0], currMB->pix_x, 0);
        copy_image_data_16x16(&p_Vid->enc_picture->imgUV[1][currMB->pix_y],
                              p_RDO->tr4x4->rec_mb8x8_cr[1], currMB->pix_x, 0);
      }
    }
    if (p_Inp->rdopt == 3) {
  ...
  if (currMB->luma_transform_size_8x8_flag && (currMB->valid_8x8)) {
    ...
    for (j = 0; j < 4; j++) {
      for (i = 0; i < 4; i++) {
        k = 2 * (j >> 1) + (i >> 1);
        motion[currMB->block_y + j][currMB->block_x + i].ref_idx[LIST_0] =
            ((currMB->b8x8[k].pdir & 0x01) == 0)
                ? p_RDO->tr8x8->part[k].ref[LIST_0]
                : -1;
        motion[currMB->block_y + j][currMB->block_x + i].ref_idx[LIST_1] =
            (currMB->b8x8[k].pdir > 0) ? p_RDO->tr8x8->part[k].ref[LIST_1] : -1;
      }
    }
    ...
    for (j = currMB->block_y; j < currMB->block_y + BLOCK_MULTIPLE; j++) {
      for (i = currMB->block_x; i < currMB->block_x + BLOCK_MULTIPLE; i++) {
        ...
        motion[j][i].ref_pic[LIST_1] =
            (cur_ref >= 0
                 ? currSlice->listX[LIST_1 + currMB->list_offset][cur_ref]
                 : NULL);
      }
    }
    ...
    if (p_RDO->tr8x8->cnt_nonz_8x8 <= _LUMA_8x8_COEFF_COST_ &&
        ((currMB->qp_scaled[0]) != 0 ||
         p_Vid->active_sps->lossless_qpprime_flag ==
             0))  // modif ES added last condition (we probably never go there
                  // so is the next modification useful ? check)
    {
      currMB->cbp = 0;
      currMB->cbp_blk = 0;
...
void prepare_ipcm_mode(Macroblock *currMB) {
  ...
  if ((p_Vid->yuv_format != YUV400) &&
    ...
    copy_image_data(&p_Vid->enc_picture->imgUV[0][currMB->pix_c_y],
                    &p_Vid->pImgOrg[1][currMB->opix_c_y], currMB->pix_c_x,
                    currMB->pix_c_x, p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y);
    copy_image_data(&p_Vid->enc_picture->imgUV[1][currMB->pix_c_y],
                    &p_Vid->pImgOrg[2][currMB->opix_c_y], currMB->pix_c_x,
                    currMB->pix_c_x, p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y);
  }
...
int RDCost_for_macroblocks(
    Macroblock *currMB,  // <-- Current Macroblock to code
    int lambda,          // <-- lagrange multiplier
    short mode)  // <-- mode (0-COPY/DIRECT, 1-16x16, 2-16x8, 3-8x16, 4-8x8(+),
                 // 5-Intra4x4, 6-Intra16x16)
{
  Slice *currSlice = currMB->p_Slice;
  RDOPTStructure *p_RDO = currSlice->p_RDO;
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  // Test MV limits for Skip Mode. This could be necessary for MBAFF case Frame
  // MBs.
  if ((currSlice->mb_aff_frame_flag) && (!currMB->mb_field) &&
      (currSlice->slice_type == P_SLICE) && (mode == 0)) {
  ...
  if (mode == P8x8) {
    if ((currMB->luma_transform_size_8x8_flag && !currMB->valid_8x8) ||
        (!currMB->luma_transform_size_8x8_flag && !currMB->valid_4x4))
      return 0;
  }
  ...
  // if (p_Inp->rdopt == 3)
  //{
  //   // We need the reconstructed prediction residue for the simulated
  //   decoders. compute_residue_block (currMB,
  //   &p_Vid->enc_picture->p_curr_img[currMB->pix_y],
  //   p_Vid->p_decs->res_img[0], mode == I16MB ? currSlice->mpr_16x16[0][
  //   (short) currMB->i16mode] : currSlice->mb_pred[0], 0, 16);
  // }

  // Rate control
  if (p_Inp->RCEnable) {
    if (mode == I16MB)
      memcpy(p_RDO->pred[0], curr_mpr_16x16[(short)currMB->i16mode][0],
             MB_PIXELS * sizeof(imgpel));
    else
  ...
  if (p_Inp->rdopt == 3) {
    distortion =
        p_Vid->estimate_distortion(currMB, 0, 16, mode, 0, currMB->min_rdcost);
    // if (errdo_distortion (currMB, mode, &distortion) == 0)
    //   return 0;
  } else {
    distortion = currSlice->getDistortion(currMB);
  }
  if (distortion > currMB->min_rdcost) return 0;
  // printf("passed distortion %.2f %.2f\n", (double)distortion,
  // currMB->min_rdcost);

  if (currMB->qp_scaled[0] == 0 &&
      p_Vid->active_sps->lossless_qpprime_flag == 1 && distortion != 0)
    return 0;
  ...
  if (use_of_cc) {
    if (currMB->mb_type != 0 ||
      ...
      // cod counter and macroblock mode are written ==> do not consider code
      // counter
  ...
  //
  if ((currSlice->slice_type != I_SLICE) && (p_Inp->BiasSkipRDO == 1) &&
      (mode == 1) && (currMB->best_mode == 0) &&
      (currMB->min_dcost > 4 * distortion) &&
      (currMB->min_dcost >
       ((64 * (256 + 2 * p_Vid->mb_cr_size_y * p_Vid->mb_cr_size_x))
  ...
  if ((rdcost >= currMB->min_rdcost) ||
    ...
    // This might not always be good since mode 0 is more biased towards rate
    // than quality.
    if ((currSlice->slice_type != P_SLICE || mode != 0 ||
         rdcost != currMB->min_rdcost) ||
        is_FREXT_profile(p_Inp->ProfileIDC))
#endif
      return 0;
  }
  ...
  if ((currSlice->mb_aff_frame_flag) &&
      (mode ? 0
            : ((currSlice->slice_type == B_SLICE)
                   ? !currMB->cbp
                   : 1)))  // AFF and current is skip
  {
    if (currMB->mbAddrX & 0x01)  // bottom
    {
      if (prevMB->mb_type
              ? 0
              : ((currSlice->slice_type == B_SLICE) ? !prevMB->cbp
                                                    : 1))  // top is skip
      {
        if (!(field_flag_inference(currMB) ==
              currMB->mb_field))  // skip only allowed when correct inference
          return 0;
      }
    }
  }
...
#ifdef BEST_NZ_COEFF
  for (j = 0; j < 4; j++)
    memcpy(&p_Vid->gaaiMBAFF_NZCoeff[j][0],
           &p_Vid->nz_coeff[currMB->mbAddrX][j][0],
           (4 + p_Vid->num_blk8x8_uv) * sizeof(int));
#endif
...
void store_macroblock_parameters(Macroblock *currMB, int mode) {
  ...
  memcpy(currSlice->p_RDO->best8x8, currMB->b8x8,
         BLOCK_MULTIPLE * sizeof(Info8x8));

  memcpy(currSlice->b4_intra_pred_modes, currMB->intra_pred_modes,
         MB_BLOCK_PARTITIONS * sizeof(char));
  memcpy(currSlice->b8_intra_pred_modes8x8, currMB->intra_pred_modes8x8,
         MB_BLOCK_PARTITIONS * sizeof(char));

  for (j = 0; j < BLOCK_MULTIPLE; j++) {
    memcpy(&currSlice->b4_ipredmode[j * BLOCK_MULTIPLE],
           &p_Vid->ipredmode[currMB->block_y + j][currMB->block_x],
           BLOCK_MULTIPLE * sizeof(char));
    memcpy(&currSlice->b8_ipredmode8x8[j][0],
           &p_Vid->ipredmode8x8[currMB->block_y + j][currMB->block_x],
           BLOCK_MULTIPLE * sizeof(char));
  }
  //--- reconstructed blocks ----
  copy_image_data_16x16(p_RDO->rec_mb[0],
                        &p_Vid->enc_picture->imgY[currMB->pix_y], 0,
                        currMB->pix_x);

  if ((currSlice->slice_type == SP_SLICE) && p_Vid->sp2_frame_indicator == 0) {
    for (j = 0; j < MB_BLOCK_SIZE; j++) {
      memcpy(p_RDO->lrec_rec[j], &p_Vid->lrec[currMB->pix_y + j][currMB->pix_x],
             MB_BLOCK_SIZE * sizeof(int));  // store coefficients SP frame
    }
  }

  // modes 0, and 1 of a B frame
  if (p_Vid->AdaptiveRounding && (currSlice->slice_type == B_SLICE) &&
      mode <= 1) {
  ...
  if (p_Vid->yuv_format != YUV400) {
    ...
    for (k = 0; k < 2; k++) {
      copy_image_data(
          p_RDO->rec_mb[k + 1], &p_Vid->enc_picture->imgUV[k][currMB->pix_c_y],
          0, currMB->pix_c_x, p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y);
    }

    if ((currSlice->slice_type == SP_SLICE) &&
        p_Vid->sp2_frame_indicator == 0) {
      // store uv coefficients SP frame
      for (k = 0; k < 2; k++) {
        for (j = 0; j < p_Vid->mb_cr_size_y; j++) {
          memcpy(p_RDO->lrec_rec_uv[k][j],
                 &p_Vid->lrec_uv[k][currMB->pix_c_y + j][currMB->pix_c_x],
...
static void set_stored_macroblock_parameters_mpass(Macroblock *currMB) {
  ...
  // Luma
  copy_image_data_16x16(&imgY[currMB->pix_y], p_RDO->rec_mb[0], currMB->pix_x,
                        0);
  ...
  if ((currSlice->slice_type == SP_SLICE) && p_Vid->sp2_frame_indicator == 0) {
    for (j = 0; j < MB_BLOCK_SIZE; j++)
      memcpy(&p_Vid->lrec[currMB->pix_y + j][currMB->pix_x], p_RDO->lrec_rec[j],
             MB_BLOCK_SIZE * sizeof(int));  // restore coeff SP frame
  }
  ...
  if (p_Vid->yuv_format != YUV400) {
    for (k = 0; k < 2; k++) {
      copy_image_data(&imgUV[k][currMB->pix_c_y], p_RDO->rec_mb[k + 1],
                      currMB->pix_c_x, 0, p_Vid->mb_cr_size_x,
                      p_Vid->mb_cr_size_y);
    }

    // Copy coding info into rddata structure for MBAFF or RDOQ
    copy_image_data(currSlice->rddata->rec_mb[1], p_RDO->rec_mb[1], 0, 0,
                    p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y);
    copy_image_data(currSlice->rddata->rec_mb[2], p_RDO->rec_mb[2], 0, 0,
                    p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y);

    if ((currSlice->slice_type == SP_SLICE) && !p_Vid->sp2_frame_indicator) {
      for (k = 0; k < 2; k++) {
        for (j = 0; j < p_Vid->mb_cr_size_y; j++) {
          memcpy(&p_Vid->lrec_uv[k][currMB->pix_c_y + j][currMB->pix_c_x],
                 p_RDO->lrec_rec_uv[k][j], p_Vid->mb_cr_size_x * sizeof(int));
        }
      }
    }
  }
  ...
  memcpy(currMB->b8x8, currSlice->p_RDO->best8x8,
         BLOCK_MULTIPLE * sizeof(Info8x8));
  ...
  memcpy(currSlice->rddata->cofAC[0][0][0], currSlice->cofAC[0][0][0],
         (4 + p_Vid->num_blk8x8_uv) * 4 * 2 * 65 * sizeof(int));
  memcpy(currSlice->rddata->cofDC[0][0], currSlice->cofDC[0][0],
         3 * 2 * 18 * sizeof(int));

  memcpy(currSlice->rddata->b8x8, currSlice->p_RDO->best8x8,
         BLOCK_MULTIPLE * sizeof(Info8x8));

  // if P8x8 mode and transform size 4x4 choosen, restore motion vector data for
  // this transform size
  if (mode == P8x8 && !currMB->temp_transform_size_8x8_flag &&
      p_Inp->Transform8x8Mode && (currMB->valid_8x8 == TRUE)) {
    RestoreMV8x8(currSlice, 1);
  }

  //==== transform size flag ====
  if (p_Vid->P444_joined) {
    if (((currMB->cbp == 0) && currSlice->cmp_cbp[1] == 0 &&
         currSlice->cmp_cbp[2] == 0) &&
        (currMB->mb_type != I4MB && currMB->mb_type != I8MB))
      currMB->luma_transform_size_8x8_flag = FALSE;
    else
      currMB->luma_transform_size_8x8_flag =
          currMB->temp_transform_size_8x8_flag;
  } else {
    if (((currMB->cbp & 15) == 0) &&
        (currMB->mb_type != I4MB && currMB->mb_type != I8MB))
      currMB->luma_transform_size_8x8_flag = FALSE;
  ...
  currSlice->rddata->luma_transform_size_8x8_flag =
      currMB->luma_transform_size_8x8_flag;
  ...
  for (j = 0; j < 4; j++) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if ((currMB->b8x8[k].pdir == 1) || is_intra(currMB)) {
        ...
        if (currMB->b8x8[k].bipred && (currMB->b8x8[k].pdir == 2) &&
            is_bipred_enabled(p_Vid, currMB->mb_type)) {
          motion[block_y][block_x].ref_idx[LIST_0] = 0;
          motion[block_y][block_x].ref_pic[LIST_0] =
              currSlice->listX[LIST_0 + currMB->list_offset][0];
          motion[block_y][block_x].mv[LIST_0] =
              currSlice->bipred_mv[currMB->b8x8[k].bipred - 1][LIST_0][0]
                                  [(short)currMB->b8x8[k].mode][j][i];
          ...
          if (cur_ref >= 0) {
            motion[block_y][block_x].ref_pic[LIST_0] =
                currSlice->listX[LIST_0 + currMB->list_offset][(short)cur_ref];
            motion[block_y][block_x].mv[LIST_0] =
                currSlice->all_mv[LIST_0][(short)cur_ref]
                                 [(short)currMB->b8x8[k].mode][j][i];
          }
          // MBAFF or RDOQ
      ...
      for (i = 0; i < 4; i++) {
        ...
        if (is_intra(currMB) || (currMB->b8x8[k].pdir == 0)) {
          ...
          if (currMB->b8x8[k].bipred && (currMB->b8x8[k].pdir == 2) &&
            ...
            motion[block_y][block_x].ref_pic[LIST_1] =
                currSlice->listX[LIST_1 + currMB->list_offset][0];
            motion[block_y][block_x].mv[LIST_1] =
                currSlice->bipred_mv[currMB->b8x8[k].bipred - 1][LIST_1][0]
                                    [(short)currMB->b8x8[k].mode][j][i];
            ...
            motion[block_y][block_x].ref_pic[LIST_1] =
                currSlice->listX[LIST_1 + currMB->list_offset]
                                [(short)p_RDO->l1_refframe[j][i]];
            motion[block_y][block_x].mv[LIST_1] =
                currSlice->all_mv[LIST_1][(short)p_RDO->l1_refframe[j][i]]
                                 [(short)currMB->b8x8[k].mode][j][i];
  ...
  if (currMB->mb_type == I8MB) {
    memcpy(currMB->intra_pred_modes8x8, currSlice->b8_intra_pred_modes8x8,
           MB_BLOCK_PARTITIONS * sizeof(char));
    memcpy(currMB->intra_pred_modes, currSlice->b8_intra_pred_modes8x8,
           MB_BLOCK_PARTITIONS * sizeof(char));
    for (j = 0; j < BLOCK_MULTIPLE; j++) {
      memcpy(&p_Vid->ipredmode[currMB->block_y + j][currMB->block_x],
             currSlice->b8_ipredmode8x8[j], BLOCK_MULTIPLE * sizeof(char));
      memcpy(&p_Vid->ipredmode8x8[currMB->block_y + j][currMB->block_x],
             currSlice->b8_ipredmode8x8[j], BLOCK_MULTIPLE * sizeof(char));
    }
  } else if (mode != I4MB && mode != I8MB) {
    memset(currMB->intra_pred_modes, DC_PRED,
           MB_BLOCK_PARTITIONS * sizeof(char));
    for (j = currMB->block_y; j < currMB->block_y + BLOCK_MULTIPLE; j++)
      memset(&p_Vid->ipredmode[j][currMB->block_x], DC_PRED,
             BLOCK_MULTIPLE * sizeof(char));
  }
  // Residue Color Transform
  else if (mode == I4MB) {
    memcpy(currMB->intra_pred_modes, currSlice->b4_intra_pred_modes,
           MB_BLOCK_PARTITIONS * sizeof(char));
    for (j = 0; j < BLOCK_MULTIPLE; j++)
      memcpy(&p_Vid->ipredmode[currMB->block_y + j][currMB->block_x],
             &currSlice->b4_ipredmode[BLOCK_MULTIPLE * j],
             BLOCK_MULTIPLE * sizeof(char));
  }
  ...
  memcpy(currSlice->rddata->intra_pred_modes, currMB->intra_pred_modes,
         MB_BLOCK_PARTITIONS * sizeof(char));
  memcpy(currSlice->rddata->intra_pred_modes8x8, currMB->intra_pred_modes8x8,
         MB_BLOCK_PARTITIONS * sizeof(char));
  for (j = currMB->block_y; j < currMB->block_y + BLOCK_MULTIPLE; j++)
    memcpy(&currSlice->rddata->ipredmode[j][currMB->block_x],
           &ipredmodes[j][currMB->block_x], BLOCK_MULTIPLE * sizeof(char));
...
void set_ref_and_motion_vectors_P_slice(Macroblock *currMB,
                                        PicMotionParams **motion, Info8x8 *part,
                                        int block) {
  int mode = part->mode;
  int j = 0, i;
  ...
  if (part->pdir < 0) {
    ...
    for (j = j0; j < j1; j++) {
      ...
      for (i = i0; i < i1; i++) {
        ...
        mv->ref_pic[LIST_0] =
            currSlice->listX[LIST_0 + currMB->list_offset][fwref];
        ;
...
static void set_ref_and_motion_vectors_B_slice(Macroblock *currMB,
  ...
  if (part->pdir < 0) {
      ...
      if (part->bipred && (part->pdir == 2) && is_bipred_enabled(p_Vid, mode)) {
        for (j = j0; j < j1; j++) {
          ...
          for (i = i0; i < i1; i++) {
            ...
            motion[block_y][block_x].mv[LIST_0] =
                currSlice->bipred_mv[part->bipred - 1][LIST_0][0][mode][j][i];
            motion[block_y][block_x].ref_idx[LIST_0] = 0;
            motion[block_y][block_x].ref_pic[LIST_0] =
                currSlice->listX[LIST_0 + currMB->list_offset][0];
          }
          ...
          for (j = j0; j < j1; j++) {
            ...
            for (i = i0; i < i1; i++) {
              ...
              motion[block_y][block_x].ref_pic[LIST_0] =
                  currSlice->listX[LIST_0 + currMB->list_offset][fwref];
              motion[block_y][block_x].mv[LIST_0] =
                  currSlice->all_mv[LIST_0][fwref][mode][j][i];
    ...
    if ((part->pdir == 1 || part->pdir == 2)) {
      if (part->bipred && (part->pdir == 2) && is_bipred_enabled(p_Vid, mode)) {
        for (j = j0; j < j1; j++) {
          ...
          for (i = i0; i < i1; i++) {
            ...
            motion[block_y][block_x].mv[LIST_1] =
                currSlice->bipred_mv[part->bipred - 1][LIST_1][0][mode][j][i];
            motion[block_y][block_x].ref_idx[LIST_1] = 0;
            motion[block_y][block_x].ref_pic[LIST_1] =
                currSlice->listX[LIST_1 + currMB->list_offset][0];
          }
          ...
          for (j = j0; j < j1; j++) {
            ...
            for (i = i0; i < i1; i++) {
              ...
              if (bwref != currSlice->direct_ref_idx[block_y][block_x][LIST_1])
                printf("error\n");
              motion[block_y][block_x].ref_pic[LIST_1] =
                  currSlice->listX[LIST_1 + currMB->list_offset][bwref];
              motion[block_y][block_x].mv[LIST_1] =
                  currSlice->all_mv[LIST_1][bwref][mode][j][i];
              motion[block_y][block_x].ref_idx[LIST_1] = (char)
                  bwref;  // currSlice->direct_ref_idx[block_y][block_x][LIST_1];
              // motion[block_y][block_x].ref_pic [LIST_1] =
              // currSlice->listX[LIST_1+currMB->list_offset][(short)motion[block_y][block_x].ref_idx
              // [LIST_1]];
            }
          }
        } else {
      ...
      for (block_y = currMB->block_y + j0; block_y < currMB->block_y + j1;
           block_y++) {
        for (block_x = currMB->block_x + i0; block_x < currMB->block_x + i1;
             block_x++) {
          motion[block_y][block_x].ref_pic[LIST_1] = NULL;
          motion[block_y][block_x].mv[LIST_1] = zero_mv;
          motion[block_y][block_x].ref_idx[LIST_1] = -1;
...
void StoreMVBlock8x8(Slice *currSlice, int dir, int block8x8, int mode,
                     Info8x8 *B8x8Info) {
  RDOPTStructure *p_RDO = currSlice->p_RDO;
  ...
  if (currSlice->slice_type != B_SLICE) {
    if (B8x8Info->pdir >=
        0)  //(mode8!=IBLOCK)&&(mode8!=I16MB))  // && ref != -1)
    {
      int l0_ref = B8x8Info->ref[LIST_0];
      memcpy(&lc_l0_mv8x8[j0][i0], &all_mv[LIST_0][l0_ref][mode][j0][i0],
             2 * sizeof(MotionVector));
      memcpy(&lc_l0_mv8x8[j1][i0], &all_mv[LIST_0][l0_ref][mode][j1][i0],
             2 * sizeof(MotionVector));
    }
  } else {
    ...
    } else if (pdir8 == 1)  // list1
    {
      ...
      memcpy(&lc_l1_mv8x8[j0][i0], &all_mv[LIST_1][l1_ref][mode][j0][i0],
             2 * sizeof(MotionVector));
      memcpy(&lc_l1_mv8x8[j1][i0], &all_mv[LIST_1][l1_ref][mode][j1][i0],
             2 * sizeof(MotionVector));
...
void StoreMV8x8(Slice *currSlice, int dir) {
  ...
  for (block8x8 = 0; block8x8 < 4; block8x8++)
    StoreMVBlock8x8(currSlice, dir, block8x8, p_RDO->tr8x8->part[block8x8].mode,
...
*************************************************************************************
*/
void RestoreMVBlock8x8(Slice *currSlice, int dir, int block8x8,
                       RD_8x8DATA *tr) {
  RDOPTStructure *p_RDO = currSlice->p_RDO;
  MotionVector *****all_mv = currSlice->all_mv;
  MotionVector **lc_l0_mv8x8 = p_RDO->all_mv8x8[dir][LIST_0];
  ...
  if (currSlice->slice_type != B_SLICE) {
    ...
    {
      memcpy(&all_mv[LIST_0][l0_ref][4][j0][i0], &lc_l0_mv8x8[j0][i0],
             2 * sizeof(MotionVector));
      memcpy(&all_mv[LIST_0][l0_ref][4][j1][i0], &lc_l0_mv8x8[j1][i0],
             2 * sizeof(MotionVector));
    }
  } else {
    if (pdir8 == 0)  // list0
    {
      memcpy(&all_mv[LIST_0][l0_ref][mode][j0][i0], &lc_l0_mv8x8[j0][i0],
             2 * sizeof(MotionVector));
      memcpy(&all_mv[LIST_0][l0_ref][mode][j1][i0], &lc_l0_mv8x8[j1][i0],
             2 * sizeof(MotionVector));
    } else if (pdir8 == 1)  // list1
    {
      memcpy(&all_mv[LIST_1][l1_ref][mode][j0][i0], &lc_l1_mv8x8[j0][i0],
             2 * sizeof(MotionVector));
      memcpy(&all_mv[LIST_1][l1_ref][mode][j1][i0], &lc_l1_mv8x8[j1][i0],
             2 * sizeof(MotionVector));
    } else if (pdir8 == 2)  // bipred
    {
      if (bipred_me) {
 ...
 *************************************************************************************
 */
void StoreNewMotionVectorsBlock8x8(Slice *currSlice, int dir, int block8x8,
                                   Info8x8 *B8x8Info) {
  RDOPTStructure *p_RDO = currSlice->p_RDO;
  ...
  if (currSlice->slice_type != B_SLICE) {
    int l0_ref = B8x8Info->ref[LIST_0];
    memcpy(&lc_l0_mv8x8[0][i0], &all_mv_l0[l0_ref][mode][j0][i0],
           2 * sizeof(MotionVector));
    memcpy(&lc_l0_mv8x8[1][i0], &all_mv_l0[l0_ref][mode][j1][i0],
           2 * sizeof(MotionVector));
    memset(&lc_l1_mv8x8[0][i0], 0, 2 * sizeof(MotionVector));
    memset(&lc_l1_mv8x8[1][i0], 0, 2 * sizeof(MotionVector));
    return;
    ...
    if ((pdir8 == 1 || pdir8 == 2)) {
      int l1_ref = B8x8Info->ref[LIST_1];
      memcpy(&lc_l1_mv8x8[0][i0], &all_mv_l1[l1_ref][mode][j0][i0],
             2 * sizeof(MotionVector));
      memcpy(&lc_l1_mv8x8[1][i0], &all_mv_l1[l1_ref][mode][j1][i0],
             2 * sizeof(MotionVector));
    } else {
      memset(&lc_l1_mv8x8[0][i0], 0, 2 * sizeof(MotionVector));
      memset(&lc_l1_mv8x8[1][i0], 0, 2 * sizeof(MotionVector));
 ...
 *************************************************************************************
 */
void store_8x8_motion_vectors_p_slice(Slice *currSlice, int dir, int block8x8,
                                      Info8x8 *B8x8Info) {
  RDOPTStructure *p_RDO = currSlice->p_RDO;
  ...
  if (B8x8Info->pdir < 0) {
    ...
    MotionVector **all_mv_l0 =
        currSlice->all_mv[LIST_0][(short)B8x8Info->ref[LIST_0]]
                         [(short)B8x8Info->mode];
 ...
 *************************************************************************************
 */
void store_8x8_motion_vectors_b_slice(Slice *currSlice, int dir, int block8x8,
                                      Info8x8 *B8x8Info) {
  RDOPTStructure *p_RDO = currSlice->p_RDO;
  int i0 = (block8x8 & 0x01) << 1;
  int j0 = (block8x8 >> 1) << 1;
...
void set_mbaff_parameters(Macroblock *currMB) {
  ...
  //===== reconstruction values =====
  copy_image_data_16x16(rdopt->rec_mb[0],
                        &p_Vid->enc_picture->imgY[currMB->pix_y], 0,
                        currMB->pix_x);

  if (p_Vid->yuv_format != YUV400) {
    copy_image_data(rdopt->rec_mb[1],
                    &p_Vid->enc_picture->imgUV[0][currMB->pix_c_y], 0,
                    currMB->pix_c_x, p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y);
    copy_image_data(rdopt->rec_mb[2],
                    &p_Vid->enc_picture->imgUV[1][currMB->pix_c_y], 0,
                    currMB->pix_c_x, p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y);
  }
  ...
  memcpy(rdopt->cofAC[0][0][0], currSlice->cofAC[0][0][0],
         (4 + p_Vid->num_blk8x8_uv) * 4 * 2 * 65 * sizeof(int));
  ...
  if (currSlice->slice_type == B_SLICE) {
      ...
      for (j = 0; j < BLOCK_MULTIPLE; j++) {
        for (i = 0; i < BLOCK_MULTIPLE; i++) {
          ...
          if (currMB->b8x8[k].bipred == 0) {
            rdopt->refar[LIST_0][j][i] =
                motion[currMB->block_y + j][currMB->block_x + i]
                    .ref_idx[LIST_0];
            rdopt->refar[LIST_1][j][i] =
                motion[currMB->block_y + j][currMB->block_x + i]
                    .ref_idx[LIST_1];
          } else {
    ...
    for (j = 0; j < BLOCK_MULTIPLE; j++) {
      for (i = 0; i < BLOCK_MULTIPLE; i++) {
        rdopt->refar[LIST_0][j][i] =
            motion[currMB->block_y + j][currMB->block_x + i].ref_idx[LIST_0];
        // rdopt->refar[LIST_1][j][i] = motion[currMB->block_y +
        // j][currMB->block_x + i].ref_idx[LIST_1];
      }
    }
  }

  memcpy(rdopt->intra_pred_modes, currMB->intra_pred_modes,
         MB_BLOCK_PARTITIONS * sizeof(char));
  memcpy(rdopt->intra_pred_modes8x8, currMB->intra_pred_modes8x8,
         MB_BLOCK_PARTITIONS * sizeof(char));
  for (j = currMB->block_y; j < currMB->block_y + 4; j++) {
    memcpy(&rdopt->ipredmode[j][currMB->block_x],
           &ipredmodes[j][currMB->block_x], BLOCK_MULTIPLE * sizeof(char));
  }
}

void assign_enc_picture_params(Macroblock *currMB, int mode, Info8x8 *best,
                               int block) {
  int i, j;
  int block_y, block_x;
  int list;
    ...
    if ((best->pdir != 2) && (best->pdir != list)) {
      ...
      switch (best->bipred) {
        ...
        case 1:
          curr_mv =
              currSlice
                  ->bipred_mv[0][list][0][mode];  // best->ref[LIST_0] has to be
                                                  // zero in this case
        ...
        case 2:
          curr_mv =
              currSlice
                  ->bipred_mv[1][list][0][mode];  // best->ref[LIST_0] has to be
                                                  // zero in this case
          listX = currSlice->listX[list + list_offset][0];
          break;
        default: break;
 ...
 *************************************************************************************
 */
void set_block8x8_info(Block8x8Info *b8x8info, int mode, int block,
                       Info8x8 *best) {
 ...
 *************************************************************************************
 */
void set_subblock8x8_info(Block8x8Info *b8x8info, int mode, int block,
                          RD_8x8DATA *tr) {
...
void update_refresh_map(Macroblock *currMB, int intra, int intra1) {
  ...
  if (p_Inp->RestrictRef == 1) {
    ...
    if (p_Inp->rdopt < 2) {
      p_Vid->refresh_map[2 * currMB->mb_y][2 * currMB->mb_x] =
          (byte)(intra ? 1 : 0);
      p_Vid->refresh_map[2 * currMB->mb_y][2 * currMB->mb_x + 1] =
          (byte)(intra ? 1 : 0);
      p_Vid->refresh_map[2 * currMB->mb_y + 1][2 * currMB->mb_x] =
          (byte)(intra ? 1 : 0);
      p_Vid->refresh_map[2 * currMB->mb_y + 1][2 * currMB->mb_x + 1] =
          (byte)(intra ? 1 : 0);
    } else if (p_Inp->rdopt == 3) {
      p_Vid->refresh_map[2 * currMB->mb_y][2 * currMB->mb_x] =
          (byte)(intra1 == 0 &&
                         (currMB->mb_type == I16MB || currMB->mb_type == I4MB)
                     ? 1
                     : 0);
      p_Vid->refresh_map[2 * currMB->mb_y][2 * currMB->mb_x + 1] =
          (byte)(intra1 == 0 &&
    ...
    p_Vid->refresh_map[2 * currMB->mb_y][2 * currMB->mb_x] =
        (byte)(currMB->mb_type == I16MB || currMB->mb_type == I4MB ? 1 : 0);
    p_Vid->refresh_map[2 * currMB->mb_y][2 * currMB->mb_x + 1] =
        (byte)(currMB->mb_type == I16MB || currMB->mb_type == I4MB ? 1 : 0);
    p_Vid->refresh_map[2 * currMB->mb_y + 1][2 * currMB->mb_x] =
        (byte)(currMB->mb_type == I16MB || currMB->mb_type == I4MB ? 1 : 0);
...
int valid_intra_mode(Slice *currSlice, int ipmode) {
  ...
  if (p_Inp->IntraDisableInterOnly == 0 ||
      (currSlice->slice_type != I_SLICE && currSlice->slice_type != SI_SLICE)) {
    if (p_Inp->Intra4x4ParDisable &&
        (ipmode == VERT_PRED || ipmode == HOR_PRED))
      return 0;

    if (p_Inp->Intra4x4DiagDisable &&
        (ipmode == DIAG_DOWN_LEFT_PRED || ipmode == DIAG_DOWN_RIGHT_PRED))
      return 0;

    if (p_Inp->Intra4x4DirDisable && ipmode >= VERT_RIGHT_PRED) return 0;
  }
  return 1;
}

distblk compute_sad4x4_cost(VideoParameters *p_Vid, imgpel **cur_img,
                            imgpel **prd_img, int pic_opix_x,
                            distblk min_cost) {
...
distblk compute_sse4x4_cost(VideoParameters *p_Vid, imgpel **cur_img,
                            imgpel **prd_img, int pic_opix_x,
                            distblk min_cost) {
  int j, i;
  imgpel *cur_line, *prd_line;
...
distblk compute_satd4x4_cost(VideoParameters *p_Vid, imgpel **cur_img,
                             imgpel **prd_img, int pic_opix_x,
                             distblk min_cost) {
  int j, i;
  imgpel *cur_line, *prd_line;
...
void update_qp_cbp(Macroblock *currMB) {
  ...
  if ((currMB->cbp != 0 || currMB->best_mode == I16MB) &&
...
void copy_rdopt_data(Macroblock *currMB) {
  ...
  memcpy(currSlice->cofAC[0][0][0], rdopt->cofAC[0][0][0],
         (4 + p_Vid->num_blk8x8_uv) * 4 * 2 * 65 * sizeof(int));
  ...
  for (j = 0; j < BLOCK_MULTIPLE; j++) {
    block_y = currMB->block_y + j;
    for (i = 0; i < BLOCK_MULTIPLE; i++) {
      motion[block_y][currMB->block_x + i].ref_idx[LIST_0] =
          rdopt->refar[LIST_0][j][i];
      motion[block_y][currMB->block_x + i].ref_pic[LIST_0] =
          rdopt->refar[LIST_0][j][i] < 0
              ? NULL
              : currSlice->listX[LIST_0 + list_offset]
                                [(short)rdopt->refar[LIST_0][j][i]];
    }
  }
  ...
  if (currSlice->slice_type == B_SLICE) {
    for (j = 0; j < BLOCK_MULTIPLE; j++) {
      block_y = currMB->block_y + j;
      for (i = 0; i < BLOCK_MULTIPLE; i++) {
        motion[block_y][currMB->block_x + i].ref_idx[LIST_1] =
            rdopt->refar[LIST_1][j][i];
        motion[block_y][currMB->block_x + i].ref_pic[LIST_1] =
            rdopt->refar[LIST_1][j][i] < 0
                ? NULL
                : currSlice->listX[LIST_1 + list_offset]
                                  [(short)rdopt->refar[LIST_1][j][i]];
      }
    }
  }
  ...
  //===== reconstruction values =====
  copy_image_data_16x16(&p_Vid->enc_picture->imgY[currMB->pix_y],
                        rdopt->rec_mb[0], currMB->pix_x, 0);

  if (p_Vid->yuv_format != YUV400) {
    copy_image_data(&p_Vid->enc_picture->imgUV[0][currMB->pix_c_y],
                    rdopt->rec_mb[1], currMB->pix_c_x, 0, p_Vid->mb_cr_size_x,
                    p_Vid->mb_cr_size_y);
    copy_image_data(&p_Vid->enc_picture->imgUV[1][currMB->pix_c_y],
                    rdopt->rec_mb[2], currMB->pix_c_x, 0, p_Vid->mb_cr_size_x,
                    p_Vid->mb_cr_size_y);
  }
  ...
  //==== intra prediction modes ====
  if (mode == P8x8) {
    memcpy(currMB->intra_pred_modes, rdopt->intra_pred_modes,
           MB_BLOCK_PARTITIONS * sizeof(char));
    for (j = currMB->block_y; j < currMB->block_y + BLOCK_MULTIPLE; j++)
      memcpy(&p_Vid->ipredmode[j][currMB->block_x],
             &rdopt->ipredmode[j][currMB->block_x],
             BLOCK_MULTIPLE * sizeof(char));
  } else if (mode != I4MB && mode != I8MB) {
    memset(currMB->intra_pred_modes, DC_PRED,
           MB_BLOCK_PARTITIONS * sizeof(char));
    ...
    memcpy(currMB->intra_pred_modes, rdopt->intra_pred_modes,
           MB_BLOCK_PARTITIONS * sizeof(char));
    memcpy(currMB->intra_pred_modes8x8, rdopt->intra_pred_modes8x8,
           MB_BLOCK_PARTITIONS * sizeof(char));
    for (j = currMB->block_y; j < currMB->block_y + BLOCK_MULTIPLE; j++) {
      memcpy(&p_Vid->ipredmode[j][currMB->block_x],
             &rdopt->ipredmode[j][currMB->block_x],
    ...
    if (!is_intra(currMB)) {
      ...
      for (j = 0; j < 4; j++) {
        for (i = 0; i < 4; i++) {
          ...
          if (b8pdir != 1) {
            motion[j + currMB->block_y][i + currMB->block_x].mv[LIST_0] =
                rdopt->all_mv[LIST_0][(short)rdopt->refar[LIST_0][j][i]][b8mode]
                             [j][i];
          } else {
            motion[j + currMB->block_y][i + currMB->block_x].mv[LIST_0] =
                zero_mv;
          ...
          if (currSlice->slice_type == B_SLICE) {
            if (b8pdir != 0) {
              motion[j + currMB->block_y][i + currMB->block_x].mv[LIST_1] =
                  rdopt->all_mv[LIST_1][(short)rdopt->refar[LIST_1][j][i]]
                               [b8mode][j][i];
            } else {
              motion[j + currMB->block_y][i + currMB->block_x].mv[LIST_1] =
                  zero_mv;
