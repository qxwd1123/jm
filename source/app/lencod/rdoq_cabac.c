...
static const int entropyBits[128] = {
  895,    943,    994,    1048,   1105,   1165,   1228,   1294,   1364,
  1439,   1517,   1599,   1686,   1778,   1875,   1978,   2086,   2200,
  2321,   2448,   2583,   2725,   2876,   3034,   3202,   3380,   3568,
  3767,   3977,   4199,   4435,   4684,   4948,   5228,   5525,   5840,
  6173,   6527,   6903,   7303,   7727,   8178,   8658,   9169,   9714,
  10294,  10914,  11575,  12282,  13038,  13849,  14717,  15650,  16653,
  17734,  18899,  20159,  21523,  23005,  24617,  26378,  28306,  30426,
  32768,  32768,  35232,  37696,  40159,  42623,  45087,  47551,  50015,
  52479,  54942,  57406,  59870,  62334,  64798,  67262,  69725,  72189,
  74653,  77117,  79581,  82044,  84508,  86972,  89436,  91900,  94363,
  96827,  99291,  101755, 104219, 106683, 109146, 111610, 114074, 116538,
  119002, 121465, 123929, 126393, 128857, 131321, 133785, 136248, 138712,
  141176, 143640, 146104, 148568, 151031, 153495, 155959, 158423, 160887,
  163351, 165814, 168278, 170742, 173207, 175669, 178134, 180598, 183061,
  185525, 187989
};
...
static void est_CBP_block_bit(Macroblock *currMB, int type) {
  ...
  for (ctx = 0; ctx <= 3; ctx++) {
    cabacEstBits->blockCbpBits[ctx][0] = biari_no_bits(
        0, currSlice->tex_ctx->bcbp_contexts[type2ctx_bcbp[type]] + ctx);

    cabacEstBits->blockCbpBits[ctx][1] = biari_no_bits(
        1, currSlice->tex_ctx->bcbp_contexts[type2ctx_bcbp[type]] + ctx);
  }
}
...
static void est_significance_map(Macroblock *currMB, int type) {
  ...
  {
    cabacEstBits->significantBits[pos2ctxmap[k]][0] =
        biari_no_bits(0, map_ctx + pos2ctxmap[k]);

    cabacEstBits->significantBits[pos2ctxmap[k]][1] =
        biari_no_bits(1, map_ctx + pos2ctxmap[k]);

    cabacEstBits->lastBits[pos2ctxlast[k]][0] =
        biari_no_bits(0, last_ctx + pos2ctxlast[k]);

    cabacEstBits->lastBits[pos2ctxlast[k]][1] =
        biari_no_bits(1, last_ctx + pos2ctxlast[k]);
  }
...
static void est_significant_coefficients(Macroblock *currMB, int type) {
  ...
  for (ctx = 0; ctx <= 4; ctx++) {
    cabacEstBits->greaterOneBits[0][ctx][0] = biari_no_bits(
        0, currSlice->tex_ctx->one_contexts[type2ctx_one[type]] + ctx);

    cabacEstBits->greaterOneBits[0][ctx][1] = biari_no_bits(
        1, currSlice->tex_ctx->one_contexts[type2ctx_one[type]] + ctx);
  }

  for (ctx = 0; ctx <= maxCtx; ctx++) {
    cabacEstBits->greaterOneBits[1][ctx][0] = biari_no_bits(
        0, currSlice->tex_ctx->abs_contexts[type2ctx_abs[type]] + ctx);

    cabacEstBits->greaterOneState[ctx] = biari_state(
        0, currSlice->tex_ctx->abs_contexts[type2ctx_abs[type]] + ctx);

    cabacEstBits->greaterOneBits[1][ctx][1] = biari_no_bits(
        1, currSlice->tex_ctx->abs_contexts[type2ctx_abs[type]] + ctx);
  }
}
...
void precalculate_unary_exp_golomb_level(VideoParameters *p_Vid) {
  ...
  for (state = 0; state <= 63; state++) {
    ...
    for (symbol = 0; symbol < MAX_PREC_COEFF; symbol++) {
      p_Vid->precalcUnaryLevelTab[ctx_state0][symbol] =
          est_unary_exp_golomb_level_bits(symbol, estBits0, estBits1);

      // symbol 0 is LPS
      p_Vid->precalcUnaryLevelTab[ctx_state1][symbol] =
          est_unary_exp_golomb_level_bits(symbol, estBits1, estBits0);
    }
  }
}
...
void estRunLevel_CABAC(Macroblock *currMB,
                       int context)  // writes CABAC run/level
{
...
int est_write_and_store_CBP_block_bit(Macroblock *currMB, int type) {
  ...
  if (p_Vid->enc_picture->chroma_format_idc != YUV444 ||
      (currMB->p_Inp->separate_colour_plane_flag != 0)) {
    if (type != LUMA_8x8) {
      if (block_b.available) {
        if (p_Vid->mb_data[block_b.mb_addr].mb_type == IPCM)
          upper_bit = 1;
        else
          upper_bit = get_bit(p_Vid->mb_data[block_b.mb_addr].cbp_bits[0],
                              bit + bit_pos_b);
      }
      ...
      if (block_a.available) {
        if (p_Vid->mb_data[block_a.mb_addr].mb_type == IPCM)
          ...
          left_bit = get_bit(p_Vid->mb_data[block_a.mb_addr].cbp_bits[0],
                             bit + bit_pos_a);
      }

      ctx = 2 * upper_bit + left_bit;
      //===== encode symbol =====
      estBits = cabacEstBits->blockCbpBits[ctx][0] -
                cabacEstBits->blockCbpBits[ctx][1];
    }
  } else {
    if (block_b.available) {
      if (p_Vid->mb_data[block_b.mb_addr].mb_type == IPCM)
        ...
        if (type == LUMA_8x8)
          upper_bit = get_bit(p_Vid->mb_data[block_b.mb_addr].cbp_bits_8x8[0],
                              bit + bit_pos_b);
        else if (type == CB_8x8)
          upper_bit = get_bit(p_Vid->mb_data[block_b.mb_addr].cbp_bits_8x8[1],
                              bit + bit_pos_b);
        else if (type == CR_8x8)
          upper_bit = get_bit(p_Vid->mb_data[block_b.mb_addr].cbp_bits_8x8[2],
                              bit + bit_pos_b);
        else if ((type == CB_4x4) || (type == CB_4x8) || (type == CB_8x4) ||
                 (type == CB_16AC) || (type == CB_16DC))
          upper_bit = get_bit(p_Vid->mb_data[block_b.mb_addr].cbp_bits[1],
                              bit + bit_pos_b);
        else if ((type == CR_4x4) || (type == CR_4x8) || (type == CR_8x4) ||
                 (type == CR_16AC) || (type == CR_16DC))
          upper_bit = get_bit(p_Vid->mb_data[block_b.mb_addr].cbp_bits[2],
                              bit + bit_pos_b);
        else
          upper_bit = get_bit(p_Vid->mb_data[block_b.mb_addr].cbp_bits[0],
                              bit + bit_pos_b);
    ...
    if (block_a.available) {
      if (p_Vid->mb_data[block_a.mb_addr].mb_type == IPCM)
        ...
        if (type == LUMA_8x8)
          left_bit = get_bit(p_Vid->mb_data[block_a.mb_addr].cbp_bits_8x8[0],
                             bit + bit_pos_a);
        else if (type == CB_8x8)
          left_bit = get_bit(p_Vid->mb_data[block_a.mb_addr].cbp_bits_8x8[1],
                             bit + bit_pos_a);
        else if (type == CR_8x8)
          left_bit = get_bit(p_Vid->mb_data[block_a.mb_addr].cbp_bits_8x8[2],
                             bit + bit_pos_a);
        else if ((type == CB_4x4) || (type == CB_4x8) || (type == CB_8x4) ||
                 (type == CB_16AC) || (type == CB_16DC))
          left_bit = get_bit(p_Vid->mb_data[block_a.mb_addr].cbp_bits[1],
                             bit + bit_pos_a);
        else if ((type == CR_4x4) || (type == CR_4x8) || (type == CR_8x4) ||
                 (type == CR_16AC) || (type == CR_16DC))
          left_bit = get_bit(p_Vid->mb_data[block_a.mb_addr].cbp_bits[2],
                             bit + bit_pos_a);
        else
          left_bit = get_bit(p_Vid->mb_data[block_a.mb_addr].cbp_bits[0],
                             bit + bit_pos_a);
    ...
    //===== encode symbol =====
    estBits =
        cabacEstBits->blockCbpBits[ctx][0] - cabacEstBits->blockCbpBits[ctx][1];
  }
...
****************************************************************************
*/
void est_writeRunLevel_CABAC(Macroblock *currMB, levelDataStruct levelData[],
                             int levelTabMin[], int type, double lambda,
                             int kInit, int kStop, int noCoeff, int estCBP) {
  VideoParameters *p_Vid = currMB->p_Vid;
  Slice *currSlice = currMB->p_Slice;
  estBitsCabacStruct *cabacEstBits = &currSlice->estBitsCabac[type];
  int k, i;
      ...
      for (i = 1; i < dataLevel->noLevels; i++) {
        ...
        if (greater_one) {
          ...
          if ((dataLevel->level[i] - 2) < MAX_PREC_COEFF) {
            estBits +=
                p_Vid->precalcUnaryLevelTab[cabacEstBits->greaterOneState[ctx]]
                                           [dataLevel->level[i] - 2];
          } else {
            estBits += est_unary_exp_golomb_level_encode(
                currMB, dataLevel->level[i] - 2, ctx, type);
          }
...
int init_trellis_data_4x4_CABAC(Macroblock *currMB, int **tblock,
                                ...
                                const byte *p_scan, levelDataStruct *dataLevel,
                                int *kStart, int *kStop, int type) {
  VideoParameters *p_Vid = currMB->p_Vid;
...
****************************************************************************
*/
int init_trellis_data_8x8_CABAC(Macroblock *currMB, int **tblock, int block_x,
                                int qp_per, int qp_rem,
                                LevelQuantParams **q_params, const byte *p_scan,
                                levelDataStruct *dataLevel, int *kStart,
                                int *kStop) {
  Slice *currSlice = currMB->p_Slice;
  int noCoeff = 0;
  int *m7;
...
****************************************************************************
*/
int init_trellis_data_DC_CABAC(Macroblock *currMB, int **tblock, int qp_per,
                               int qp_rem, LevelQuantParams *q_params,
                               const byte *p_scan, levelDataStruct *dataLevel,
  ...
  int *m7;
  double err,
      estErr = (double)estErr4x4[qp_rem][0][0] /
               currSlice->norm_factor_4x4;  // note that we could also use int64
