...
Boolean mb_is_available(int mbAddr, Macroblock *currMB) {
  ...
  if ((mbAddr < 0) || (mbAddr > ((int)p_Vid->PicSizeInMbs - 1))) return FALSE;

  // the following line checks both: slice number and if the mb has been decoded
  if (!currMB->DeblockCall) {
    if (p_Vid->mb_data[mbAddr].slice_nr != currMB->slice_nr) return FALSE;
  }
...
void CheckAvailabilityOfNeighbors(Macroblock *currMB) {
  ...
  if (p_Vid->mb_aff_frame_flag) {
    ...
    currMB->mbAvailA = (byte)(mb_is_available(currMB->mbAddrA, currMB) &&
                              ((PicPos[cur_mb_pair].x) != 0));
    currMB->mbAvailB = (byte)(mb_is_available(currMB->mbAddrB, currMB));
    currMB->mbAvailC = (byte)(mb_is_available(currMB->mbAddrC, currMB) &&
                              ((PicPos[cur_mb_pair + 1].x) != 0));
    currMB->mbAvailD = (byte)(mb_is_available(currMB->mbAddrD, currMB) &&
                              ((PicPos[cur_mb_pair].x) != 0));
  } else {
 ...
 ************************************************************************
 */
void get_mb_block_pos_normal(BlockPos *PicPos, int mb_addr, short *x,
                             short *y) {
