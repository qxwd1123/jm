...
static void EdgeLoopChromaHor(imgpel **Img, byte Strength[MB_BLOCK_SIZE],
                              Macroblock *MbQ, int edge, int width, int uv);
...
static void GetStrengthVer(byte Strength[MB_BLOCK_SIZE], Macroblock *MbQ,
  ...
  if ((currSlice->slice_type == SP_SLICE) ||
    ...
    if (!(MbQ->mb_type == I4MB || MbQ->mb_type == I8MB ||
          MbQ->mb_type == I16MB || MbQ->mb_type == IPCM)) {
      PixelPos pixMB;
      Macroblock *MbP;
      int xQ = edge - 1;
      ...
      if (!(MbP->mb_type == I4MB || MbP->mb_type == I8MB ||
            MbP->mb_type == I16MB || MbP->mb_type == IPCM)) {
        PixelPos pixP = pixMB;
        ...
        for (idx = 0; idx < MB_BLOCK_SIZE; idx += BLOCK_SIZE) {
          ...
          if (((MbQ->cbp_blk & i64_power2(blkQ)) != 0) ||
              ((MbP->cbp_blk & i64_power2(blkP)) != 0))
            StrValue = 2;
...
static void EdgeLoopLumaHor(ColorPlane pl, imgpel **Img, byte Strength[16],
  ...
  PixelPos pixMB1;
  getNonAffNeighbour(MbQ, 0, (edge < MB_BLOCK_SIZE ? edge - 1 : 0),
                     p_Vid->mb_size[IS_LUMA], &pixMB1);
