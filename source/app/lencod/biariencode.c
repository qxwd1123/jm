...
void reset_pic_bin_count(VideoParameters *p_Vid) { p_Vid->pic_bin_count = 0; }

int get_pic_bin_count(VideoParameters *p_Vid) { return p_Vid->pic_bin_count; }
...
EncodingEnvironmentPtr arienco_create_encoding_environment(void) {
  ...
  if ((eep = (EncodingEnvironmentPtr)calloc(1, sizeof(EncodingEnvironment))) ==
      NULL)
...
static inline void put_one_byte_final(EncodingEnvironmentPtr eep,
                                      unsigned int b) {
...
static forceinline void put_buffer(EncodingEnvironmentPtr eep) {
  while (eep->Epbuf >= 0) {
    eep->Ecodestrm[(*eep->Ecodestrm_len)++] =
        (byte)((eep->Ebuffer >> ((eep->Epbuf--) << 3)) & 0xFF);
  }
  while (eep->C > 7) {
    eep->C -= 8;
    ++(eep->E);
...
static inline void put_last_chunk_plus_outstanding(EncodingEnvironmentPtr eep,
                                                   unsigned int l) {
  while (eep->Echunks_outstanding > 0) {
...
static inline void put_last_chunk_plus_outstanding_final(
    EncodingEnvironmentPtr eep, unsigned int l) {
  while (eep->Echunks_outstanding > 0) {
    put_one_word(eep, 0xFFFF);
    --(eep->Echunks_outstanding);
  }
...
void arienco_done_encoding(Macroblock *currMB, EncodingEnvironmentPtr eep) {
  ...
  int remaining_bits =
      BITS_TO_LOAD -
      eep->Ebits_to_go;  // output (2 + remaining) bits for terminating the
                         // codeword + one stop bit
  unsigned char mask;
  ...
  } else if (remaining_bits <= 13)  // two terminating bytes
  {
    ...
    put_last_chunk_plus_outstanding_final(
        eep, ((low >> (MASK_BITS)) & 0xFF));  // mask out the 8 MSBs for output
    ...
    if (remaining_bits > 6) {
      ...
      put_one_byte_final(
          eep, 128);  // second byte contains terminating stop bit '1' only
    }
  } else  // three terminating bytes
  {
    put_last_chunk_plus_outstanding(
        eep,
        ((low >> B_BITS) & B_LOAD_MASK));  // mask out the 16 MSBs for output
    put_buffer(eep);
    ...
    if (remaining_bits > 14) {
      ...
      put_one_byte_final(
          eep, 128);  // third byte contains terminating stop bit '1' only
    }
  }
  eep->Ebits_to_go = 8;
}
 ...
 ************************************************************************
 */
void biari_encode_symbol(EncodingEnvironmentPtr eep, int symbol,
  ...
  } else  // LPS
  {
    ...
    if (!bi_ct->state) bi_ct->MPS ^= 0x01;  // switch MPS if necessary
