...
extern void set_access_method(int *access_method, MotionVector *blk, int min_x,
                              int min_y, int max_x, int max_y);

extern void PrepareMEParams(Slice *currSlice, MEBlock *mv_block,
                            int ChromaMEEnable, int list, int ref);
extern void PrepareBiPredMEParams(Slice *currSlice, MEBlock *mv_block,
                                  int ChromaMEEnable, int list, int list_offset,
                                  int ref);

extern void init_motion_search_module(VideoParameters *p_Vid,
                                      InputParameters *p_Inp);
extern void clear_motion_search_module(VideoParameters *p_Vid,
                                       InputParameters *p_Inp);
...
extern distblk BlockMotionSearch(Macroblock *currMB, MEBlock *mv_block, int,
                                 int, int *);
extern void init_mv_block(Macroblock *currMB, MEBlock *mv_block,
                          short blocktype, int list, char ref_idx, short mb_x,
                          short mb_y);
extern void get_original_block(VideoParameters *p_Vid, MEBlock *mv_block);
extern void free_mv_block(MEBlock *mv_block);
extern void update_mv_block(Macroblock *currMB, MEBlock *mv_block, int h,
                            int v);
extern void get_search_range(MEBlock *mv_block, InputParameters *p_Inp,
...
#else
#if (USE_RND_COST)
  return (i64_rshift_rnd_sf(((int64)lambda) * ((int64)(bits)),
...
static inline distblk mv_cost(const VideoParameters *p_Vid, int lambda,
                              const MotionVector *mv, const MotionVector *pmv) {
#if JCOST_CALC_SCALEUP
  int *mvbits = p_Vid->mvbits;
  return (((distblk)lambda) * ((distblk)(mvbits[mv->mv_x - pmv->mv_x] +
                                         mvbits[mv->mv_y - pmv->mv_y])));
#else
#if (USE_RND_COST)
  return (rshift_rnd_sf((lambda * (p_Vid->mvbits[mv->mv_x - pmv->mv_x] +
                                   p_Vid->mvbits[mv->mv_y - pmv->mv_y])),
                        LAMBDA_ACCURACY_BITS));
#else
  return ((lambda * (p_Vid->mvbits[mv->mv_x - pmv->mv_x] +
                     p_Vid->mvbits[mv->mv_y - pmv->mv_y])) >>
          LAMBDA_ACCURACY_BITS);
#endif
#endif
...
static inline distblk ref_cost(const Slice *currSlice, int lambda, short ref,
                               int list_offset) {
...
#else
#if (USE_RND_COST)
    return (rshift_rnd_sf((lambda) * (p_Vid->refbits[(ref)]),
                          LAMBDA_ACCURACY_BITS));
#else
