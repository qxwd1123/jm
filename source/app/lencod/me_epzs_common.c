...
static const short BLOCK_PARENT[8] = {
  1, 1, 1, 1, 2, 4, 4, 5
...
// Other definitions
static const char EPZS_PATTERN[6][20] = { "Diamond",           "Square",
                                          "Extended Diamond",  "Large Diamond",
                                          "SBP Large Diamond", "PMVFAST" };
static const char EPZS_DUAL_PATTERN[7][20] = {
  "Disabled",      "Diamond",           "Square", "Extended Diamond",
  "Large Diamond", "SBP Large Diamond", "PMVFAST"
};
static const char EPZS_FIXED_PREDICTORS[4][20] = { "Disabled", "All P",
                                                   "All P + B", "Aggressive" };
...
static const short pattern_data[7][12][4] = { { // Small Diamond pattern
                                                { 0, 4, 3, 3 },
                                                { 4, 0, 0, 3 },
                                                { 0, -4, 1, 3 },
                                                { -4, 0, 2, 3 } },
...
static EPZSStructure *allocEPZSpattern(int searchpoints) {
  ...
  if (NULL == s) no_mem_exit("alloc_EPZSpattern: s");
...
************************************************************************
*/
static void assignEPZSpattern(EPZSStructure *pattern, int type, int stopSearch,
                              int nextLast, EPZSStructure *nextpattern) {
  int i;
...
void EPZS_setup_engine(Macroblock *currMB, InputParameters *p_Inp) {
  currMB->IntPelME = (p_Inp->EPZSSubPelGrid) ? EPZS_integer_motion_estimation
                                             : EPZS_motion_estimation;
  currMB->BiPredME = (p_Inp->EPZSSubPelGrid)
                         ? EPZS_integer_bipred_motion_estimation
                         : EPZS_bipred_motion_estimation;
...
int EPZSInit(VideoParameters *p_Vid) {
  ...
  if (!p_Vid->sbdiamond) {
    ...
    assignEPZSpattern(p_Vid->sbdiamond, SBDIAMOND, FALSE, TRUE,
                      p_Vid->sdiamond);
  }
  if (!p_Vid->pmvfast) {
...
************************************************************************
*/
static EPZSColocParams *allocEPZScolocated(int size_x, int size_y,
                                           int mb_adaptive_frame_field_flag) {
  EPZSColocParams *s;
  s = calloc(1, sizeof(EPZSColocParams));
  if (NULL == s) no_mem_exit("alloc_EPZScolocated: s");
  ...
  if (mb_adaptive_frame_field_flag) {
    get_mem3Dmv(&(s->top), 2, (size_y >> (BLOCK_SHIFT + 1)),
                (size_x >> BLOCK_SHIFT));
    get_mem3Dmv(&(s->bot), 2, (size_y >> (BLOCK_SHIFT + 1)),
                (size_x >> BLOCK_SHIFT));
  }
...
static void EPZSWindowPredictorInit(short search_range,
                                    EPZSStructure *predictor, short mode) {
  short pos;
  ...
  } else  // if (mode == 0)
  {
    for (pos = (short)(RoundLog2(search_range) - 2); pos > -1; pos--) {
      ...
      // fieldsearchpos = ((3 * searchpos + 1) << search_range_qpel) >> (pos +
      // 1);
...
int EPZSStructInit(Slice *currSlice) {
  ...
  double chroma_weight =
      p_Inp->ChromaMEEnable ? pel_error_me_cr * p_Inp->ChromaMEWeight *
                                  (double)(p_Vid->width_cr * p_Vid->height_cr) /
                                  (double)(p_Vid->width * p_Vid->height)
                            : 0;
  int searchlevels = RoundLog2(p_Inp->search_range[p_Vid->view_id]) - 1;
  int searcharray =
      p_Inp->BiPredMotionEstimation
          ? (2 * imax(p_Inp->search_range[p_Vid->view_id],
  ...
  for (i = 0; i < 8; ++i) {
    ...
    if ((currSlice->view_id) && (p_Inp->EnableEnhLayerEPZSScalers)) {
      p_EPZS->medthres[i] = p_Inp->EPZSMedThresScale[currSlice->view_id] *
                            (MED_THRES_BASE[i] * pel_error_me +
                             (int)(MED_THRES_BASE[i] * chroma_weight + 0.5));
      p_EPZS->maxthres[i] = p_Inp->EPZSMaxThresScale[currSlice->view_id] *
                            (MAX_THRES_BASE[i] * pel_error_me +
                             (int)(MAX_THRES_BASE[i] * chroma_weight + 0.5));
      p_EPZS->minthres[i] = p_Inp->EPZSMinThresScale[currSlice->view_id] *
                            (MIN_THRES_BASE[i] * pel_error_me +
                             (int)(MIN_THRES_BASE[i] * chroma_weight + 0.5));
      p_EPZS->subthres[i] = p_Inp->EPZSSubPelThresScale[currSlice->view_id] *
                            (MED_THRES_BASE[i] * pel_error_me +
                             (int)(MED_THRES_BASE[i] * chroma_weight + 0.5));
    } else {
      p_EPZS->medthres[i] = p_Inp->EPZSMedThresScale[0] *
                            (MED_THRES_BASE[i] * pel_error_me +
                             (int)(MED_THRES_BASE[i] * chroma_weight + 0.5));
      p_EPZS->maxthres[i] = p_Inp->EPZSMaxThresScale[0] *
                            (MAX_THRES_BASE[i] * pel_error_me +
                             (int)(MAX_THRES_BASE[i] * chroma_weight + 0.5));
      p_EPZS->minthres[i] = p_Inp->EPZSMinThresScale[0] *
                            (MIN_THRES_BASE[i] * pel_error_me +
                             (int)(MIN_THRES_BASE[i] * chroma_weight + 0.5));
      p_EPZS->subthres[i] = p_Inp->EPZSSubPelThresScale[0] *
                            (MED_THRES_BASE[i] * pel_error_me +
                             (int)(MED_THRES_BASE[i] * chroma_weight + 0.5));
    }
#else
    p_EPZS->medthres[i] = p_Inp->EPZSMedThresScale *
                          (MED_THRES_BASE[i] * pel_error_me +
                           (int)(MED_THRES_BASE[i] * chroma_weight + 0.5));
    p_EPZS->maxthres[i] = p_Inp->EPZSMaxThresScale *
                          (MAX_THRES_BASE[i] * pel_error_me +
                           (int)(MAX_THRES_BASE[i] * chroma_weight + 0.5));
    p_EPZS->minthres[i] = p_Inp->EPZSMinThresScale *
                          (MIN_THRES_BASE[i] * pel_error_me +
                           (int)(MIN_THRES_BASE[i] * chroma_weight + 0.5));
    p_EPZS->subthres[i] = p_Inp->EPZSSubPelThresScale *
                          (MED_THRES_BASE[i] * pel_error_me +
                           (int)(MED_THRES_BASE[i] * chroma_weight + 0.5));
#endif
  ...
  EPZSWindowPredictorInit((short)p_Inp->search_range[p_Vid->view_id],
                          p_EPZS->window_predictor, 0);
  EPZSWindowPredictorInit((short)p_Inp->search_range[p_Vid->view_id],
                          p_EPZS->window_predictor_ext, 1);

  //! Also assign search predictor memory
  // hierarchical + maxwindow + spatial + blocktype + temporal + memspatial
#if (MVC_EXTENSION_ENABLE)
  p_EPZS->predictor =
      allocEPZSpattern(10 + searchlevels * 24 + 5 + 5 +
                       9 * (p_Inp->EPZSTemporal[0] | p_Inp->EPZSTemporal[1]) +
                       3 * (p_Inp->EPZSSpatialMem));
#else
  p_EPZS->predictor =
      allocEPZSpattern(10 + searchlevels * 24 + 5 + 5 +
                       9 * (p_Inp->EPZSTemporal) + 3 * (p_Inp->EPZSSpatialMem));
#endif

  //! Finally assign memory for all other elements
  //! (distortion, EPZSMap, and temporal predictors)

  // memory_size += get_offset_mem2Dshort(&EPZSMap, searcharray, searcharray,
  // (searcharray>>1), (searcharray>>1));
  memory_size += get_mem3Ddistblk(&(p_EPZS->distortion), max_list_number, 7,
                                  (p_Vid->width + MB_BLOCK_SIZE) / BLOCK_SIZE);
  if (p_Inp->BiPredMotionEstimation)
    memory_size +=
        get_mem3Ddistblk(&(p_EPZS->bi_distortion), max_list_number, 7,
                         (p_Vid->width + MB_BLOCK_SIZE) / BLOCK_SIZE);
  memory_size +=
      get_mem3Ddistblk(&(p_EPZS->distortion_hpel), max_list_number, 7,
                       (p_Vid->width + MB_BLOCK_SIZE) / BLOCK_SIZE);
  memory_size +=
      get_mem2Dshort((short ***)&(p_EPZS->EPZSMap), searcharray, searcharray);

  if (p_Inp->EPZSSpatialMem) {
#if EPZSREF
    memory_size +=
        get_mem5Dmv(&(p_EPZS->p_motion), 6, p_Vid->max_num_references, 7, 4,
                    p_Vid->width / BLOCK_SIZE);
#else
    memory_size +=
        get_mem4Dmv(&(p_EPZS->p_motion), 6, 7, 4, p_Vid->width / BLOCK_SIZE);
#endif
  }
  ...
  {
    p_EPZS->p_colocated =
        allocEPZScolocated(p_Vid->width, p_Vid->height,
                           p_Vid->active_sps->mb_adaptive_frame_field_flag);
  }

  switch (p_Inp->EPZSPattern) {
    case 5: p_EPZS->searchPattern = p_Vid->pmvfast; break;
    case 4: p_EPZS->searchPattern = p_Vid->sbdiamond; break;
    case 3: p_EPZS->searchPattern = p_Vid->ldiamond; break;
    case 2: p_EPZS->searchPattern = p_Vid->ediamond; break;
    case 1: p_EPZS->searchPattern = p_Vid->square; break;
    case 0:
    default: p_EPZS->searchPattern = p_Vid->sdiamond; break;
  }

  switch (p_Inp->EPZSDual) {
    case 6: p_EPZS->searchPatternD = p_Vid->pmvfast; break;
    case 5: p_EPZS->searchPatternD = p_Vid->sbdiamond; break;
    case 4: p_EPZS->searchPatternD = p_Vid->ldiamond; break;
    case 3: p_EPZS->searchPatternD = p_Vid->ediamond; break;
    case 2: p_EPZS->searchPatternD = p_Vid->square; break;
    case 1:
    default: p_EPZS->searchPatternD = p_Vid->sdiamond; break;
  }
...
void EPZSStructDelete(Slice *currSlice) {
  ...
  // free_offset_mem2Dshort(EPZSMap, searcharray, (searcharray>>1),
  // (searcharray>>1));
  free_mem2Dshort((short **)p_EPZS->EPZSMap);
  free_mem3Ddistblk(p_EPZS->distortion);
  free_mem3Ddistblk(p_EPZS->distortion_hpel);

  if (p_Inp->BiPredMotionEstimation) free_mem3Ddistblk(p_EPZS->bi_distortion);
...
void EPZSSliceInit(Slice *currSlice) {
    ...
    for (k = 0; k < currSlice->listXsize[j]; ++k) {
      for (i = 0; i < currSlice->listXsize[j]; ++i) {
        ...
        if (iTRp != 0) {
          ...
          p_EPZS->mv_scale[j][i][k] =
              iClip3(-2048, 2047, rshift_rnd_sf((iTRb * prescale), 6));
        } else
...
#endif
  {
    ...
    for (j = 0; j < 2 + (currSlice->mb_aff_frame_flag << 2); j += 2) {
      for (i = 0; i < currSlice->listXsize[j]; ++i) {
        if (j == 0)
          ...
          iTRb =
              iClip3(-128, 127, p_Pic->bottom_poc - listX[LIST_0 + j][i]->poc);
        iTRp = iClip3(-128, 127,
                      listX[list + j][0]->poc - listX[LIST_0 + j][i]->poc);
        ...
        } else  // This could not happen but lets use it in case that reference
                // is removed.
          prescale = 256;

        epzs_scale[0][j][i] =
            rshift_rnd_sf((p_EPZS->mv_scale[j][0][i] * prescale), 8);
        epzs_scale[0][j + 1][i] = prescale - 256;

        if (currSlice->listXsize[list + j] > 1) {
          iTRp = iClip3(-128, 127,
                        listX[list + j][1]->poc - listX[LIST_0 + j][i]->poc);
          if (iTRp != 0) {
            prescale = (16384 + iabs(iTRp / 2)) / iTRp;
            prescale = iClip3(-2048, 2047, rshift_rnd_sf((iTRb * prescale), 6));
            // prescale = (iTRb * prescale + 32) >> 6;
          } else  // This could not happen but lets use it for case that
                  // reference is removed.
            prescale = 256;

          epzs_scale[1][j][i] =
              rshift_rnd_sf((p_EPZS->mv_scale[j][1][i] * prescale), 8);
        ...
        for (j = 0; j < fs->size_y >> 2; ++j) {
          ...
          for (i = 0; i < fs->size_x >> 2; ++i) {
            if (fs->mv_info[j][i].field_frame) {
              ...
              if (iabs(p_Pic->poc - fs_bottom->poc) >
                  iabs(p_Pic->poc - fs_top->poc)) {
                tempmv_scale[LIST_0] = 256;
                tempmv_scale[LIST_1] = 0;

                if (fs->mv_info[jdiv][i].ref_pic[LIST_0] == NULL &&
                    currSlice->listXsize[LIST_0] > 1) {
                  fsx = fs_top1;
                  loffset = 1;
                } else {
                  fsx = fs_top;
                ...
                if (fs->mv_info[jdiv][i].ref_pic[LIST_0] != NULL) {
                  for (iref = 0;
                       ...
                       ++iref) {
                    if (currSlice->listX[LIST_0][iref] ==
                        fs->mv_info[jdiv][i].ref_pic[LIST_0]) {
                  ...
                  compute_scaled(&MotionVector0[j][i], &MotionVector1[j][i],
                                 tempmv_scale, &fsx->mv_info[jj][i].mv[LIST_0],
                                 invmv_precision);
                } else {
                ...
                if (fs->mv_info[jdiv + 4][i].ref_pic[LIST_0] != NULL) {
                  for (iref = 0;
                       ...
                       ++iref) {
                    if (currSlice->listX[LIST_0][iref] ==
                        fs->mv_info[jdiv + 4][i].ref_pic[LIST_0]) {
                      tempmv_scale[LIST_0] = epzs_scale[loffset][LIST_0][iref];
                      tempmv_scale[LIST_1] = epzs_scale[loffset][LIST_1][iref];
                      break;
                    }
              ...
              if (fs->mv_info[j][i].ref_pic[LIST_0] == NULL &&
                  currSlice->listXsize[LIST_0] > 1) {
                fsx = fs1;
                loffset = 1;
              } else {
              ...
              if (fsx->mv_info[j][i].ref_pic[LIST_0] != NULL) {
                for (iref = 0;
                     ...
                     ++iref) {
                  if (currSlice->listX[LIST_0][iref] ==
                      fsx->mv_info[j][i].ref_pic[LIST_0]) {
                    tempmv_scale[LIST_0] = epzs_scale[loffset][LIST_0][iref];
                    tempmv_scale[LIST_1] = epzs_scale[loffset][LIST_1][iref];
                    break;
                  }
                ...
                compute_scaled(&MotionVector0[j][i], &MotionVector1[j][i],
                               tempmv_scale, &fsx->mv_info[j][i].mv[LIST_0],
                               invmv_precision);
              } else {
                MotionVector0[j][i] = zero_mv;
                MotionVector1[j][i] = zero_mv;
        ...
        for (j = 0; j < fs->size_y >> 3; ++j) {
          for (i = 0; i < fs->size_x >> 2; ++i) {
            if (!currSlice->mb_aff_frame_flag) {
              ...
              tempmv_scale[LIST_0] = 256;
              tempmv_scale[LIST_1] = 0;
              if (fs_bottom->mv_info[j][i].ref_pic[LIST_0] == NULL &&
                  currSlice->listXsize[LIST_0] > 1) {
                fsx = fs_bottom1;
                loffset = 1;
              } else {
              ...
              if (fsx->mv_info[j][i].ref_pic[LIST_0] != NULL) {
                for (iref = 0;
                     iref < imin(2 * currSlice->num_ref_idx_active[LIST_0],
                                 currSlice->listXsize[LIST_0 + 4]);
                     ++iref) {
                  if (currSlice->listX[LIST_0 + 4][iref] ==
                      fsx->mv_info[j][i].ref_pic[LIST_0]) {
                    tempmv_scale[LIST_0] =
                        epzs_scale[loffset][LIST_0 + 4][iref];
                    tempmv_scale[LIST_1] =
                        epzs_scale[loffset][LIST_1 + 4][iref];
                    break;
                  }
                }

                compute_scaled(&p->bot[LIST_0][j][i], &p->bot[LIST_1][j][i],
                               tempmv_scale, &fsx->mv_info[j][i].mv[LIST_0],
                               invmv_precision);
              } else {
              ...
              if (!fs->mv_info[2 * j][i].field_frame) {
                p->bot[LIST_0][j][i].mv_y =
                    (p->bot[LIST_0][j][i].mv_y + 1) >> 1;
                p->bot[LIST_1][j][i].mv_y =
                    (p->bot[LIST_1][j][i].mv_y + 1) >> 1;
              }

              tempmv_scale[LIST_0] = 256;
              tempmv_scale[LIST_1] = 0;
              if (fs_top->mv_info[j][i].ref_pic[LIST_0] == NULL &&
                  currSlice->listXsize[LIST_0] > 1) {
                fsx = fs_top1;
                loffset = 1;
              } else {
              ...
              if (fsx->mv_info[j][i].ref_pic[LIST_0] != NULL) {
                for (iref = 0;
                     iref < imin(2 * currSlice->num_ref_idx_active[LIST_0],
                                 currSlice->listXsize[LIST_0 + 2]);
                     ++iref) {
                  if (currSlice->listX[LIST_0 + 2][iref] ==
                      fsx->mv_info[j][i].ref_pic[LIST_0]) {
                    tempmv_scale[LIST_0] =
                        epzs_scale[loffset][LIST_0 + 2][iref];
                    tempmv_scale[LIST_1] =
                        epzs_scale[loffset][LIST_1 + 2][iref];
                    break;
                  }
                }

                compute_scaled(&p->top[LIST_0][j][i], &p->top[LIST_1][j][i],
                               tempmv_scale, &fsx->mv_info[j][i].mv[LIST_0],
                               invmv_precision);
              } else {
              ...
              if (!fs->mv_info[2 * j][i].field_frame) {
                p->top[LIST_0][j][i].mv_y =
                    (p->top[LIST_0][j][i].mv_y + 1) >> 1;
                p->top[LIST_1][j][i].mv_y =
            ...
            if (fs->mv_info[j][i].field_frame) {
              ...
              if (fsx->mv_info[jdiv][i].ref_pic[LIST_0] != NULL) {
                for (iref = 0;
                     ...
                     ++iref) {
                  if (currSlice->listX[LIST_0][iref] ==
                      fsx->mv_info[jdiv][i].ref_pic[LIST_0]) {
                    tempmv_scale[LIST_0] = epzs_scale[loffset][LIST_0][iref];
                    tempmv_scale[LIST_1] = epzs_scale[loffset][LIST_1][iref];
                    break;
                  }
                }
                if (iabs(p_Pic->poc - fsx->bottom_field->poc) >
                    iabs(p_Pic->poc - fsx->top_field->poc)) {
                  compute_scaled(&MotionVector0[j][i], &MotionVector1[j][i],
                                 tempmv_scale,
                                 &fsx->top_field->mv_info[jj][i].mv[LIST_0],
                                 invmv_precision);
                } else {
                  compute_scaled(&MotionVector0[j][i], &MotionVector1[j][i],
                                 tempmv_scale,
                                 &fsx->bottom_field->mv_info[jj][i].mv[LIST_0],
                                 invmv_precision);
                }
              } else {
                MotionVector0[j][i] = zero_mv;
      ...
      for (j = 0; j < fs->size_y >> 2; ++j) {
        for (i = 0; i < fs->size_x >> 2; ++i) {
          ...
          if (fsx->mv_info[j][i].ref_pic[LIST_0] != NULL &&
              (!p_Vid->view_id ||
               ((fsx->ref_pic_na[0] < 0 ||
                 fsx->mv_info[j][i].ref_idx[LIST_0] != fsx->ref_pic_na[0])))) {
            for (iref = 0; iref < imin(currSlice->num_ref_idx_active[LIST_0],
                                       currSlice->listXsize[LIST_0]);
                 ++iref) {
              if (currSlice->listX[LIST_0][iref] ==
    ...
    if (!currSlice->active_sps->frame_mbs_only_flag) {
      for (j = 0; j < fs->size_y >> 2; ++j) {
        for (i = 0; i < fs->size_x >> 2; ++i) {
          if ((!currSlice->mb_aff_frame_flag && !currSlice->structure &&
               fs->mv_info[j][i].field_frame) ||
            ...
            MotionVector0[j][i].mv_y =
                (short)rshift_rnd_sf(MotionVector0[j][i].mv_y, 1);
            MotionVector1[j][i].mv_y =
                (short)rshift_rnd_sf(MotionVector1[j][i].mv_y, 1);
          }
        }
      }
    }
...
static void is_block_available(Macroblock *currMB, StorablePicture *ref_picture,
                               MEBlock *mv_block, int block_available[4]) {
  if ((mv_block->block_y << 2) > 0) {
    if ((mv_block->block_x << 2) < 8)  // first column of 8x8 blocks
    {
      if ((mv_block->block_y << 2) == 8) {
        block_available[0] = (mv_block->blocksize_x != MB_BLOCK_SIZE) ||
                             (currMB->mb_x < (ref_picture->size_x >> 4) - 1);
      } else {
        block_available[0] =
            ((mv_block->block_x << 2) + mv_block->blocksize_x != 8) ||
            (currMB->mb_x < (ref_picture->size_x >> 4) - 1);
      }
    } else {
      block_available[0] =
          ((mv_block->block_x << 2) + mv_block->blocksize_x != MB_BLOCK_SIZE) ||
          (currMB->mb_x < (ref_picture->size_x >> 4) - 1);
    }
  } else {
    block_available[0] =
  ...
  block_available[1] =
      ((mv_block->block_y << 2) + mv_block->blocksize_y != MB_BLOCK_SIZE) ||
      ((currMB->mb_y < (ref_picture->size_y >> 4) - 1));
...
************************************************************************
*/
void EPZSBlockTypePredictorsMB(Slice *currSlice, MEBlock *mv_block,
  ...
  if (ref > 0) {
    cur_mv = &point[*prednum].motion;
    scale_mv(cur_mv, p_EPZS->mv_scale[list][ref][ref - 1],
             &all_mv[ref - 1][blocktype][block_y][block_x], 8);
    //*prednum += ((cur_mv->mv_x | cur_mv->mv_y) != 0);
    *prednum += (*((int *)cur_mv) != 0);

    if (ref > 1) {
      cur_mv = &point[*prednum].motion;
      scale_mv(cur_mv, p_EPZS->mv_scale[list][ref][0],
               &all_mv[0][blocktype][block_y][block_x], 8);
      //*prednum += ((cur_mv->mv_x | cur_mv->mv_y) != 0);
      *prednum += (*((int *)cur_mv) != 0);
    }
  }
...
***********************************************************************
*/
short EPZS_spatial_predictors(EPZSParameters *p_EPZS, MEBlock *mv_block,
                              int list, int list_offset, short ref,
  ...
  if (!p_Vid->mb_aff_frame_flag) {
    ...
    if (block[0].available) {
      scale_mv(&point->motion, mot_scale[refA],
               &mv_info[block[0].pos_y][block[0].pos_x].mv[list], 8);
      /*
    ...
    if (block[1].available) {
      scale_mv(&point->motion, mot_scale[refB],
               &mv_info[block[1].pos_y][block[1].pos_x].mv[list], 8);
      /*
      if (*((int*) &point->motion) == 0)
      {
      point->motion.mv_x = 0;
    ...
    if (block[2].available) {
      scale_mv(&point->motion, mot_scale[refC],
               &mv_info[block[2].pos_y][block[2].pos_x].mv[list], 8);
      /*
      if (*((int*) &point->motion) == 0)
      {
      point->motion.mv_x = -12;
    ...
    if (block[3].available) {
      scale_mv(&point->motion, mot_scale[refD],
               &mv_info[block[3].pos_y][block[3].pos_x].mv[list], 8);
      /*
      if (*((int*) &point->motion) == 0)
      {
      point->motion.mv_x = 0;
    ...
    if (list_offset) {
      refA = block[0].available
                 ? p_Vid->mb_data[block[0].mb_addr].mb_field
                       ? (short)mv_info[block[0].pos_y][block[0].pos_x]
                             .ref_idx[list]
                       : (short)mv_info[block[0].pos_y][block[0].pos_x]
                                 .ref_idx[list] *
                             2
      ...
      refB = block[1].available
                 ? p_Vid->mb_data[block[1].mb_addr].mb_field
                       ? (short)mv_info[block[1].pos_y][block[1].pos_x]
                             .ref_idx[list]
                       : (short)mv_info[block[1].pos_y][block[1].pos_x]
                                 .ref_idx[list] *
                             2
      ...
      refC = block[2].available
                 ? p_Vid->mb_data[block[2].mb_addr].mb_field
                       ? (short)mv_info[block[2].pos_y][block[2].pos_x]
                             .ref_idx[list]
                       : (short)mv_info[block[2].pos_y][block[2].pos_x]
                                 .ref_idx[list] *
                             2
      ...
      refD = block[3].available
                 ? p_Vid->mb_data[block[3].mb_addr].mb_field
                       ? (short)mv_info[block[3].pos_y][block[3].pos_x]
                             .ref_idx[list]
                       : (short)mv_info[block[3].pos_y][block[3].pos_x]
                                 .ref_idx[list] *
                             2
...
void EPZS_temporal_predictors(
    ...
    int *prednum, distblk stopCriterion, distblk min_mcost) {
  int list_offset = currMB->list_offset;
  int blockshape_x =
      (mv_block->blocksize_x >> 2);  // horizontal block size in 4-pel units
  int blockshape_y =
      (mv_block->blocksize_y >> 2);  // vertical block size in 4-pel units
  ...
  MotionVector **col_mv = (list_offset == 0)   ? p_Coloc->frame[list]
                          : (list_offset == 2) ? p_Coloc->top[list]
                                               : p_Coloc->bot[list];
  MotionVector *cur_mv = &point[*prednum].motion;
  ...
  *prednum += add_predictor(cur_mv, col_mv[o_block_y][o_block_x], mvScale, 8);
  if (min_mcost > stopCriterion && ref < 2) {
    ...
    if (block_available[2]) {
      *prednum += add_predictor(&point[*prednum].motion,
                                col_mv[o_block_y][o_block_x - 1], mvScale, 8);
      // Up_Left
      if (block_available[3]) {
        *prednum +=
            add_predictor(&point[*prednum].motion,
                          col_mv[o_block_y - 1][o_block_x - 1], mvScale, 8);
      }

      // Down_Left
      if (block_available[1]) {
        *prednum += add_predictor(
            &point[*prednum].motion,
            col_mv[o_block_y + blockshape_y][o_block_x - 1], mvScale, 8);
      }
    }
    ...
    if (block_available[3]) {
      *prednum += add_predictor(&point[*prednum].motion,
                                col_mv[o_block_y - 1][o_block_x], mvScale, 8);
    }

    // Up - Right
    if (block_available[0]) {
      *prednum += add_predictor(&point[*prednum].motion,
                                col_mv[o_block_y][o_block_x + blockshape_x],
                                mvScale, 8);
      if (block_available[3]) {
        *prednum += add_predictor(
            &point[*prednum].motion,
            col_mv[o_block_y - 1][o_block_x + blockshape_x], mvScale, 8);
      }

      if (block_available[1]) {
        *prednum += add_predictor(
            &point[*prednum].motion,
            col_mv[o_block_y + blockshape_y][o_block_x + blockshape_x], mvScale,
            8);
      }
    }
    ...
    if (block_available[1]) {
      *prednum += add_predictor(&point[*prednum].motion,
                                col_mv[o_block_y + blockshape_y][o_block_x],
                                mvScale, 8);
    }
  }
}
...
************************************************************************
*/
void EPZSBlockTypePredictors(Slice *currSlice, MEBlock *mv_block, SPoint *point,
                             int *prednum) {
  int blocktype = mv_block->blocktype;
  int block_x = mv_block->block_x;
  int block_y = mv_block->block_y;
...
************************************************************************
*/
void EPZSWindowPredictors(MotionVector *mv, EPZSStructure *predictor,
                          int *prednum, EPZSStructure *windowPred) {
  int pos;
...
***********************************************************************
*/
void EPZS_spatial_memory_predictors(
    EPZSParameters *p_EPZS,  //!< EPZS Parameters
    ...
    int img_width)           //!< image width
{
  ...
  *prednum += ((point[*prednum].motion.mv_x != 0) ||
  ...
  // Up-Right predictor
  point[*prednum].motion.mv_x =
      (pic_x + bs_x < img_width)
          ? (by > 0)
                ? (short)rshift_rnd_sf(
                      (mot_scale * prd_mv[by - bs_y][pic_x + bs_x].mv_x), 8)
                : (short)rshift_rnd_sf(
                      (mot_scale * prd_mv[4 - bs_y][pic_x + bs_x].mv_x), 8)
          : 0;
  point[*prednum].motion.mv_y =
      (pic_x + bs_x < img_width)
          ? (by > 0)
                ? (short)rshift_rnd_sf(
                      (mot_scale * prd_mv[by - bs_y][pic_x + bs_x].mv_y), 8)
                : (short)rshift_rnd_sf(
                      (mot_scale * prd_mv[4 - bs_y][pic_x + bs_x].mv_y), 8)
          : 0;
  *prednum += ((point[*prednum].motion.mv_x != 0) ||
               (point[*prednum].motion.mv_y != 0));
#endif
...
***********************************************************************
*/
void EPZS_hierarchical_predictors(
    EPZSParameters *p_EPZS,        //!< EPZS Parameters
    MEBlock *mv_block,             //!< Motion estimation structure
    int *prednum,                  //!< predictor position
    StorablePicture *ref_picture,  //! <-- Current reference picture
...
*************************************************************************************
*/
distblk EPZSDetermineStopCriterion(EPZSParameters *p_EPZS, distblk *prevSad,
                                   MEBlock *mv_block, distblk lambda_dist) {
  int blocktype = mv_block->blocktype;
  ...
  stopCriterion =
      distblkmin(stopCriterion, p_EPZS->maxthres[blocktype] + lambda_dist);
  stopCriterion = (8 * distblkmax(p_EPZS->medthres[blocktype] + lambda_dist,
                                  stopCriterion) +
                   1 * p_EPZS->medthres[blocktype]) >>
                  3;
  ...
  if (stats_file == 1) {
    ...
    if ((p_Inp->num_of_views == 2) && (p_Inp->EnableEnhLayerEPZSScalers)) {
      fprintf(stat, " BL EPZS Threshold Multipliers     : (%d %d %d)\n",
              p_Inp->EPZSMedThresScale[0], p_Inp->EPZSMinThresScale[0],
              p_Inp->EPZSMaxThresScale[0]);
      fprintf(stat, " EL EPZS Threshold Multipliers     : (%d %d %d)\n",
              p_Inp->EPZSMedThresScale[1], p_Inp->EPZSMinThresScale[1],
              p_Inp->EPZSMaxThresScale[1]);
    } else {
...
#else
    fprintf(stat, " EPZS Threshold Multipliers   : (%d %d %d)\n",
            p_Inp->EPZSMedThresScale, p_Inp->EPZSMinThresScale,
            p_Inp->EPZSMaxThresScale);
#endif
