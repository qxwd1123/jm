...
struct inp_par_enc {
  ...
  int search_range[2];  /*!< search range - integer pel search and 16x16 blocks.
                           The search window is  generally around the predicted
                           vector. Max vector is 2xmcrange.  */
  ...
  int P_List0_refs_org[2];  //!< number of reference picture in list 0 in P
                            //!< pictures
  int B_List0_refs_org[2];  //!< number of reference picture in list 0 in B
                            //!< pictures
  int B_List1_refs_org[2];  //!< number of reference picture in list 1 in B
                            //!< pictures
  ...
  int is_interleaved;
  int src_resize;  //!< Control if input sequence will be resized (currently
  ...
  int MVCInterViewReorder;  //!< Reorder References according to interview
                            //!< pictures
  int MVCFlipViews;  //!< Reverse the order of the views in the bitstream (view
                     //!< 1 has VOIdx 0 and view 1 has VOIdx 0)
  int MVCInterViewForceB;      //!< Force B slices for enhancement layer
  int View1QPOffset;           //!< QP offset during rate control for View 1
  int enable_inter_view_flag;  //!< Enables inter_view_flag (allows pictures
                               //!< that are to be used for inter-view only
                               //!< prediction)
  double enh_layer_lambda_multiplier;  //!< Weight lambda for enhancement layer
  double enh_layer_me_lambda_multiplier;  //!< Weight ME lambda for enhancement
                                          //!< layer

#endif
  VideoDataFile input_file1;         //!< Input video file1
  char outfile[FILE_NAME_SIZE];      //!< H.264 compressed output bitstream
  char ReconFile[FILE_NAME_SIZE];    //!< Reconstructed Pictures (view 0 for MVC
                                     //!< profile)
  ...
  int BiPredMERefinements;  //!< Max number of Iterations for Bi-predictive
                            //!< motion estimation
  int BiPredMESearchRange[2];  //!< Search range of Bi-predictive motion
                               //!< estimation
  ...
  int sp_output_indicator;  //!< Flag indicating whether coefficients are output
                            ...
                            //!< (0: not used, 1: used)
  char sp_output_filename[FILE_NAME_SIZE];  //!< Filename where SP coefficients
                                            //!< are output
  char
      sp2_input_filename1[FILE_NAME_SIZE];  //!< Filename of coefficients of the
                                            //!< first bitstream when encoding
                                            //!< SP frames to switch bitstreams
  char sp2_input_filename2
      [FILE_NAME_SIZE];  //!< Filenames of coefficients of the second bitstream
                         //!< when encoding SP frames to switch bitstreams

  // Weighted Prediction
  int WeightedPrediction;    //!< Weighted prediction for P frames (0: not used,
                             //!< 1: explicit)
  ...
  int PSliceSkipDecisionMethod;  //!< Use of a NaturalSkip method for deciding
                                 //!< skip modes in P slices
  int BRefPictures;  //!< B coded reference pictures replace P pictures (0: not
                     //!< used, 1: used)
  int HierarchicalCoding;
  int HierarchyLevelQPEnable;
  char ExplicitHierarchyFormat[INPUT_TEXT_SIZE];  //!< Explicit GOP format
                                                  //!< (HierarchicalCoding==3).
  // explicit sequence information parameters
  int ExplicitSeqCoding;
  char ExplicitSeqFile[FILE_NAME_SIZE];
  int LowDelay;  //!< Apply HierarchicalCoding without delay (i.e., encode in
                 //!< the captured/display order)

  int ReferenceReorder;      //!< Reordering based on Poc distances
  int EnableReorderBslice;   //!< Perform reordering for B slice List0. Only
                             //!< makes sense if MSE reordering is used.
  int UseDistortionReorder;  //!< for method ReferenceReorder = 1, use
                             //!< distortion based reordering
  ...
  // FMO
  char SliceGroupConfigFileName[FILE_NAME_SIZE];  //!< Filename for config info
                                                  //!< fot type 0, 2, 6
  ...
  unsigned int *top_left;  //!< top_left and bottom_right store values
                           //!< indicating foregrounds
  unsigned int *bottom_right;
  byte *slice_group_id;  //!< slice_group_id is for slice group type being 6
  int *
      run_length_minus1;  //!< run_length_minus1 is for slice group type being 0
  ...
  char QOffsetMatrixFile[FILE_NAME_SIZE];  //!< Quantization Offset matrix cfg
                                           //!< file
  int OffsetMatrixPresentFlag;  //!< Enable Explicit Quantization Offset
                                //!< Matrices
  ...
  int rc_cpb_size;
  int SEIVUI32Pulldown;  //!< Enable 3:2 pulldown through VUI and SEI metadata
                         //!< signalling. Three methods are supported.
  int SEIFPAType;        //!< Value of frame_packing_arrangement_type in SEI FPA
                         //!< message
