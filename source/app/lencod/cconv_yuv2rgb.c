...
int create_RGB_memory(VideoParameters *p_Vid) {
  ...
  for (j = 0; j < 3; j++) {
    memory_size +=
        get_mem2Dpel(&p_Vid->imgRGB_src.data[j], p_Vid->height, p_Vid->width);
  }
  for (j = 0; j < 3; j++) {
    memory_size +=
        get_mem2Dpel(&p_Vid->imgRGB_ref.data[j], p_Vid->height, p_Vid->width);
  }
...
*************************************************************************************
*/
void YUVtoRGB(VideoParameters *p_Vid, ImageStructure *YUV,
              ImageStructure *RGB) {
  int i, j, j_cr, i_cr;
  int sy, su, sv;
  int wbuv, wguv, wruv;
