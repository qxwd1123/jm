 ...
 *
 * \brief
 *    Headerfile for Improved Motion Compensatation Precision Scheme using
 *Weighted Prediction \author Main contributors (see contributors.h for
 *copyright, address and affiliation details)
...
typedef struct wpx_object {
  int num_wp_ref_list[2];  // num of elements in each of the above matrices
                           // [LIST]
  WeightedPredRefX *wp_ref_list[2];  // structure with reordering and WP
                                     // information for ref frames [LIST]
...
// Note that at some point, InputParameters p_Inp contents should be copied into
// VideoParameters *p_Vid. This would eliminate need of having to use both
// structures
