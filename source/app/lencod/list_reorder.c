...
static void poc_ref_pic_reorder_dummy(Slice *currSlice,
...
void init_ref_pic_list_reordering(Slice *currSlice, int refReorderMethod,
  ...
  else if ((refReorderMethod == 1) && (currSlice->layer_id > 0) &&
           (currSlice->p_Inp->MVCInterViewReorder > 0)) {
    currSlice->poc_ref_pic_reorder_frame = poc_ref_pic_reorder_frame_enh;
    currSlice->reorder_lists = reorder_lists;
  } else if ((refReorderMethod == 0) && (currSlice->layer_id > 0) &&
             (currSlice->p_Inp->MVCInterViewReorder > 0)) {
    currSlice->poc_ref_pic_reorder_frame = poc_ref_pic_reorder_frame_enh;
    currSlice->reorder_lists = reorder_lists;
  }
#endif
  else if ((refReorderMethod == 0) || (currSlice->slice_type == I_SLICE) ||
...
void poc_ref_pic_reorder_frame_default(Slice *currSlice,
  ...
  int *modification_of_pic_nums_idc =
      currSlice->modification_of_pic_nums_idc[list_no];
  ...
  for (i = 0; i < ((int)num_refs - 1); i++) {
    for (j = i + 1; j < (int)num_refs; j++) {
      if (poc_diff[i] > poc_diff[j] ||
          (poc_diff[i] == poc_diff[j] && list_sign[j] > list_sign[i])) {
  ...
  if (p_Inp->EnableOpenGOP) {
    ...
    for (i = 0; i < currSlice->listXsize[list_no]; i++) {
      if (currSlice->listX[list_no][i]->poc < p_Vid->last_valid_reference &&
...
************************************************************************
*/
void poc_ref_pic_reorder_frame_enh(Slice *currSlice,
                                   unsigned num_ref_idx_lX_active,
                                   int list_no) {
  StorablePicture **list = currSlice->listX[list_no];
  VideoParameters *p_Vid = currSlice->p_Vid;
  ...
  // This code is idiotic since this can be found immediately given that
  // reference is in p_Dpb->fs_ilref
  ...
  for (i = 0; i < num_refs; i++) {
    ...
    if (p_Dpb->fs_ref[i]->is_used == 3 &&
      ...
      // printf("Reference %d %d %d %d %d\n", i, p_Dpb->fs_ref[i]->frame->poc,
      // p_Enc_Pic->poc, poc_diff[i], list_sign[i]);
    }
  }
  ...
  if (currSlice->layer_id > 0) {
    ...
    // We need to have a flag in the code that points to how many such
    // references exist and appropriately scan fl_ilref.
    j = 0;
    k = 0;
    for (i = 0; i < (int)num_ref_idx_lX_active + found; i++) {
      if ((found > 0) && (i == inter_layer_ref_idx[k])) {
        list[i] = inter_layer_ref[k++];
        // printf("values %d %d", (int) list[i],  (int)
        // p_Dpb->fs_ilref[0]->frame);
        found--;
      } else {
        list[i] = re_order[j++];
      }
...
************************************************************************
*/
void poc_ref_pic_reorder_field_enh(Slice *currSlice,
                                   unsigned num_ref_idx_lX_active,
                                   int list_no) {
  StorablePicture **list = currSlice->listX[list_no];
  VideoParameters *p_Vid = currSlice->p_Vid;
  ...
  for (i = 0; i < num_refs; i++) {
    ...
    num_flds = (currSlice->structure == BOTTOM_FIELD &&
                (p_Enc_Pic->poc == (pField[0]->poc + 1)))
                   ? 1
                   : 2;
    ...
    for (fld = 0; fld < num_flds; fld++) {
      if ((pFrameStore->is_used & field_used[fld]) &&
          pField[fld]->used_for_reference && !(pField[fld]->is_long_term)) {
...
************************************************************************
*/
void poc_ref_pic_reorder_field(Slice *currSlice, unsigned num_ref_idx_lX_active,
                               int list_no) {
  StorablePicture **list = currSlice->listX[list_no];
  int *modification_of_pic_nums_idc =
      currSlice->modification_of_pic_nums_idc[list_no];
  ...
  for (i = 0; i < p_Dpb->ref_frames_in_buffer; i++) {
    ...
    for (fld = 0; fld < num_flds; fld++) {
      ...
      if ((pFrameStore->is_used & field_used[fld]) &&
          pField[fld]->used_for_reference && !(pField[fld]->is_long_term) &&
          (p_Vid->p_Inp->num_of_views == 1 ||
           pField[fld]->view_id == p_Vid->view_id))
#else
      if ((pFrameStore->is_used & field_used[fld]) &&
          pField[fld]->used_for_reference && !(pField[fld]->is_long_term))
#endif
 ...
 ************************************************************************
 * \brief
 *    Decide reference picture reordering based on mse distortion given zero
 *mvs; Frame only
 ************************************************************************
 */
void mse_ref_pic_reorder_frame(Slice *currSlice, unsigned num_ref_idx_lX_active,
                               int list_no) {
  StorablePicture **list = currSlice->listX[list_no];
  VideoParameters *p_Vid = currSlice->p_Vid;
  ...
  // printf("num %d (%d %d)\n", num_refs, p_Vid->ThisPOC,
  // p_Vid->last_valid_reference);
  if (num_refs > 1)  // pointless to reorder if only one reference
  {
    // access all references in dpb and extract their distortion information
    // versus the current reference
    for (i = 0; i < num_refs; i++) {
      ...
      // open gop test
      if (p_Inp->EnableOpenGOP) {
        if (p_Dpb->fs_ref[i]->poc < p_Vid->last_valid_reference &&
            p_Vid->ThisPOC >= p_Vid->last_valid_reference) {
...
************************************************************************
*/
void tlyr_ref_pic_reorder_frame_default(Slice *currSlice,
                                        unsigned num_ref_idx_lX_active,
                                        int list_no) {
  StorablePicture **list = currSlice->listX[list_no];
  int *modification_of_pic_nums_idc =
...
void reorder_lists(Slice *currSlice) {
  ...
  if (p_Inp->ReferenceReorder &&
      ((currSlice->slice_type == P_SLICE ||
       ...
       (p_Inp->UseDistortionReorder == 1 && p_Inp->EnableReorderBslice == 1 &&
  ...
  if (currSlice->slice_type == B_SLICE &&
    ...
    iSendAddtionalRPLR =
        (p_Inp->BIdenticalList == 1 &&
         ((currSlice->ThisPOC / 2) % (p_Inp->NumberBFrames + 1)) == 0) ||
  ...
  if (p_Inp->useCRA) {
    if (currSlice->slice_type == B_SLICE &&
        currSlice->p_Dpb->ref_frames_in_buffer >= 2) {
      if (((currSlice->ThisPOC / 2) - (p_Inp->NumberBFrames + 1)) %
              p_Inp->intra_period ==
          0) {
        int i, diff_num;
        alloc_ref_pic_list_reordering_buffer(currSlice);
        ...
        diff_num =
            currSlice->listX[LIST_0][0]->frame_num - currSlice->frame_num;
        if (diff_num <= 0) {
          ...
          if (currSlice->abs_diff_pic_num_minus1[LIST_1][0] < 0) {
            currSlice->abs_diff_pic_num_minus1[LIST_1][0] =
                currSlice->max_frame_num - 1;
          }
        } else {
...
void wp_mcprec_reorder_lists(Slice *currSlice) {
  ...
  if (currSlice->slice_type ==
      B_SLICE)  // type should be part of currSlice not p_Vid
  {
    for (i = 0; i < currSlice->num_ref_idx_active[LIST_1] + 1; i++) {
      currSlice->modification_of_pic_nums_idc[LIST_1][i] = 3;
      currSlice->abs_diff_pic_num_minus1[LIST_1][i] = 0;
...
void set_default_ref_pic_lists(Slice *currSlice) {
  ...
  for (list = LIST_0;
       list <= (currSlice->slice_type == P_SLICE ? LIST_0 : LIST_1); list++) {
    for (ref = 0; ref < currSlice->listXsize[list]; ref++) {
      currSlice->default_pic_num[list][ref] =
          currSlice->listX[list][ref]->pic_num;
#if MVC_EXTENSION_ENABLE
      currSlice->default_view_id[list][ref] =
          currSlice->listX[list][ref]->view_id;
#endif
    }
  }
}
...
void reorder_against_default_ref_pic_lists(Slice *currSlice, int cur_list) {
  ...
  for (i = 0; i < currSlice->listXsize[cur_list]; i++) {
    ...
    // cur_list,i,currSlice->default_pic_num[cur_list][i],
    // currSlice->listX[cur_list][i]->pic_num);
    // if(currSlice->default_pic_num[cur_list][i] !=
    // currSlice->listX[cur_list][i]->pic_num)
    if (
#if MVC_EXTENSION_ENABLE
        currSlice->default_view_id[cur_list][i] !=
            currSlice->listX[cur_list][i]->view_id ||
#endif
        currSlice->default_pic_num[cur_list][i] !=
            currSlice->listX[cur_list][i]->pic_num) {
  ...
  if (!currSlice->ref_pic_list_reordering_flag[cur_list]) return;
