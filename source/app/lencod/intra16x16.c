 ...
 ************************************************************************
 */
static inline void get_i16x16_dc(imgpel **cur_pred, imgpel *PredPel,
 ...
 ************************************************************************
 */
static inline void get_i16x16_plane(imgpel **cur_pred, imgpel *PredPel,
 ...
 ************************************************************************
 */
void set_intrapred_16x16(Macroblock *currMB, ColorPlane pl, int *left_available,
  ...
  imgpel **img_enc = (currMB->p_Slice->P444_joined)
                         ? p_Vid->enc_picture->p_img[pl]
  ...
  if (*up_available) {
    ...
    for (i = 1; i < 17; ++i) PredPel[i] = (imgpel)p_Vid->dc_pred_value;
  }
  ...
  if (*left_available) {
    ...
    for (i = 17; i < 33; ++i) PredPel[i] = (imgpel)p_Vid->dc_pred_value;
  }
 ...
 ************************************************************************
 */
void set_intrapred_16x16_mbaff(Macroblock *currMB, ColorPlane pl,
                               int *left_available, int *up_available,
                               int *all_available) {
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
 ...
 ************************************************************************
 */
void get_intrapred_16x16(Macroblock *currMB, ColorPlane pl, int i16x16_mode,
                         int left_available, int up_available) {
  imgpel *PredPel = currMB->intra16x16_pred[pl];  // array of predictor pels
  ...
  switch (i16x16_mode) {
    ...
    case HOR_PRED_16:
      ...
      break;
    case DC_PRED_16:
      get_i16x16_dc(curr_mpr_16x16[DC_PRED_16], PredPel, left_available,
                    up_available);
      break;
    case PLANE_16:
      get_i16x16_plane(curr_mpr_16x16[PLANE_16], PredPel,
...
distblk distI16x16_satd(Macroblock *currMB, imgpel **img_org, imgpel **pred_img,
                        distblk min_cost) {
  ...
  for (jj = 0; jj < 4; jj++) {
    for (ii = 0; ii < 4; ii++) {
      ...
      if (i32Cost > imin_cost) return (min_cost);
...
distblk distI16x16_sad(Macroblock *currMB, imgpel **img_org, imgpel **pred_img,
                       distblk min_cost) {
  imgpel *cur_img, *prd_img;
  int i32Cost = 0;
  int i, j;
...
distblk distI16x16_sse(Macroblock *currMB, imgpel **img_org, imgpel **pred_img,
                       distblk min_cost) {
  imgpel *cur_img, *prd_img;
  int i, j, i32Cost = 0;
  int imin_cost = dist_down(min_cost);
...
distblk find_sad_16x16_JM(Macroblock *currMB) {
  ...
  if (currSlice->P444_joined) {
    currSlice->set_intrapred_16x16(currMB, PLANE_U, &left_avail, &up_avail,
                                   &left_up_avail);
    currSlice->set_intrapred_16x16(currMB, PLANE_V, &left_avail, &up_avail,
                                   &left_up_avail);
  }
  ...
  for (k = VERT_PRED_16; k <= PLANE_16; k++) {
    ...
    if (!((k == VERT_PRED_16 && !up_avail) ||
          (k == HOR_PRED_16 && !left_avail) ||
          (k == PLANE_16 && (!left_avail || !up_avail || !left_up_avail)))) {
      get_intrapred_16x16(currMB, PLANE_Y, k, left_avail, up_avail);
      current_intra_sad_2 = currSlice->distI16x16(
          currMB, p_Vid->pCurImg, curr_mpr_16x16[k], best_intra_sad2);
      if (currSlice->P444_joined) {
        get_intrapred_16x16(currMB, PLANE_U, k, left_avail, up_avail);
        current_intra_sad_2 +=
            currSlice->distI16x16(currMB, p_Vid->pImgOrg[1],
                                  currSlice->mpr_16x16[1][k], best_intra_sad2);
        get_intrapred_16x16(currMB, PLANE_V, k, left_avail, up_avail);
        current_intra_sad_2 +=
            currSlice->distI16x16(currMB, p_Vid->pImgOrg[2],
                                  currSlice->mpr_16x16[2][k], best_intra_sad2);
      }
