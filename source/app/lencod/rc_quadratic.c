 ...
 *************************************************************************************
 */
void rc_alloc_quadratic(VideoParameters *p_Vid, InputParameters *p_Inp,
                        RCQuadratic **p_quad) {
 ...
 *************************************************************************************
 */
void rc_copy_quadratic(VideoParameters *p_Vid, InputParameters *p_Inp,
                       RCQuadratic *dst, RCQuadratic *src) {
  int rcBufSize = p_Vid->FrameSizeInMbs / p_Inp->basicunit;
  ...
  /* copy MADs */
  memcpy((void *)dst->BUPFMAD, (void *)src->BUPFMAD,
         (rcBufSize) * sizeof(double));
  memcpy((void *)dst->BUCFMAD, (void *)src->BUCFMAD,
         (rcBufSize) * sizeof(double));
  memcpy((void *)dst->FCBUPFMAD, (void *)src->FCBUPFMAD,
         (rcBufSize) * sizeof(double));
  memcpy((void *)dst->FCBUCFMAD, (void *)src->FCBUCFMAD,
         (rcBufSize) * sizeof(double));
}
 ...
 *************************************************************************************
 */
void rc_init_seq(VideoParameters *p_Vid, InputParameters *p_Inp,
                 RCQuadratic *p_quad, RCGeneric *p_gen) {
  double L1, L2, L3, bpp;
  int qp, i;

  switch (p_Inp->RCUpdateMode) {
    case RC_MODE_0: p_Vid->updateQP = updateQPRC0; break;
    case RC_MODE_1: p_Vid->updateQP = updateQPRC1; break;
    case RC_MODE_2: p_Vid->updateQP = updateQPRC2; break;
    case RC_MODE_3: p_Vid->updateQP = updateQPRC3; break;
    default: p_Vid->updateQP = updateQPRC0; break;
  }
 ...
 *************************************************************************************
 */
void rc_init_GOP(VideoParameters *p_Vid, InputParameters *p_Inp,
                 RCQuadratic *p_quad, RCGeneric *p_gen, int np, int nb) {
  // int OverBits;
  int GOPDquant;
  int64 AllocatedBits;
  ...
  switch (p_Inp->RCUpdateMode) {
    case RC_MODE_3: {
      ...
      int i_period = p_Inp->intra_period == 0
                         ? p_Inp->idr_period
                         : (p_Inp->idr_period == 0
                                ? p_Inp->intra_period
                                : imin(p_Inp->intra_period, p_Inp->idr_period));
      ...
      if (p_Inp->NumberBFrames) {
        if (p_Inp->HierarchicalCoding == 1)  // two layers: even/odd
        {
          ...
          num_frames[1] = (p_Inp->NumberBFrames - num_frames[0]) >= 0
                              ? (p_Inp->NumberBFrames - num_frames[0])
                              : 0;
        } else if (p_Inp->HierarchicalCoding ==
                   2)  // binary hierarchical structure
        {
          ...
          fprintf(
              stderr,
              "\n RCUpdateMode=3 and HierarchicalCoding == 3 are currently not "
              "supported");  // This error message should be moved elsewhere and
                             // have proper memory deallocation
          exit(1);
      ...
      numer = (float)(((!i_period ? 1 : i_period) * gop) *
                      ((double)p_Inp->bit_rate / p_Inp->source.frame_rate));
      denom = 0.0F;
      ...
      p_gen->RCISliceBits =
          i_period ? (int)(p_Inp->RCISliceBitRatio * p_gen->RCPSliceBits + 0.5)
                   : 0;

      for (level = 0; level < levels; level++) {
        p_gen->RCBSliceBits[level] = (int)floor(
            p_Inp->RCBSliceBitRatio[level] * p_gen->RCPSliceBits + 0.5);
      }

      p_gen->NISlice = i_period ? (p_Inp->no_frames / i_period) : 0;
      p_gen->NPSlice = (p_Inp->no_frames / (1 + p_Inp->NumberBFrames)) -
                       p_gen->NISlice;  // approximate but good enough for
                                        // sufficient number of frames
    } break;
    default: break;
  }

  /* check if the last GOP over uses its budget. If yes, the initial QP of the I
  frame in the coming  GOP will be increased.*/

  // OverBits=-(int)(p_gen->RemainingBits);

  /*initialize the lower bound and the upper bound for the target bits of each
   * frame, HRD consideration*/
  p_quad->LowerBound =
      (int)(p_gen->RemainingBits + p_quad->bit_rate / p_quad->frame_rate);
  p_quad->UpperBound1 =
      (int)(p_gen->RemainingBits + (p_quad->bit_rate * 2.048));

  /*compute the total number of bits for the current GOP*/
  AllocatedBits =
      (int64)floor((1 + np + nb) * p_quad->bit_rate / p_quad->frame_rate + 0.5);
  ...
  /*field coding*/
  // p_gen->NoGranularFieldRC = ( p_Inp->PicInterlace || !p_Inp->MbInterlace ||
  // p_Inp->basicunit != p_Vid->FrameSizeInMbs );
  ...
  if (p_gen->NumberofGOP == 1) {
    ...
    p_quad->PAverageQp =
        (int)(1.0 * p_quad->TotalQpforPPicture / p_quad->NumberofPPicture +
              0.5);

    GOPDquant = (int)((1.0 * (np + nb + 1) / 15.0) + 0.5);
    if (GOPDquant > 2) GOPDquant = 2;

    p_quad->PAverageQp -= GOPDquant;

    if (p_quad->PAverageQp > (p_quad->QPLastPFrame - 2)) p_quad->PAverageQp--;

    // QP is constrained by QP of previous GOP
    p_quad->PAverageQp = iClip3(p_quad->QPLastGOP - 2, p_quad->QPLastGOP + 2,
                                p_quad->PAverageQp);
    // Also clipped within range.
    p_quad->PAverageQp =
        iClip3(p_Vid->RCMinQP + p_quad->bitdepth_qp_scale,
               p_Vid->RCMaxQP + p_quad->bitdepth_qp_scale, p_quad->PAverageQp);
 ...
 *************************************************************************************
 */
void rc_init_pict(VideoParameters *p_Vid, InputParameters *p_Inp,
                  RCQuadratic *p_quad, RCGeneric *p_gen, int fieldpic,
                  int topfield, int targetcomputation, float mult) {
  int tmp_T;
  ...
  /* Normally, the bandwidth for the VBR case is estimated by
     a congestion control algorithm. A bandwidth curve can be predefined if we
     only want to test the proposed algorithm */
  ...
  if ((fieldpic || topfield) && targetcomputation) {
    if (p_Vid->type == P_SLICE ||
        (p_Inp->RCUpdateMode == RC_MODE_1 && (p_Vid->number != 0))) {
      ...
      if (p_quad->PrevBitRate != p_quad->bit_rate)
        p_gen->RemainingBits +=
            (int)floor((p_quad->bit_rate - p_quad->PrevBitRate) *
                           (p_quad->Np + p_quad->Nb) / p_quad->frame_rate +
                       0.5);
      ...
      if (p_Vid->BasicUnit == p_Vid->FrameSizeInMbs) {
        if (p_quad->NumberofPPicture == 1) {
          ...
          p_quad->DeltaP =
              (p_gen->CurrentBufferFullness - p_quad->GOPTargetBufferLevel) /
              (p_quad->TotalPFrame - 1);
      ...
      /* basic unit layer rate control */
        ...
        if (p_quad->NumberofCodedPFrame > 0) {
          ...
          if (((p_Inp->PicInterlace == ADAPTIVE_CODING) ||
               (p_Inp->MbInterlace)) &&
              (p_gen->FieldControl == 1))
            memcpy((void *)p_quad->FCBUPFMAD, (void *)p_quad->FCBUCFMAD,
                   p_quad->TotalNumberofBasicUnit * sizeof(double));
          else
            memcpy((void *)p_quad->BUPFMAD, (void *)p_quad->BUCFMAD,
                   p_quad->TotalNumberofBasicUnit * sizeof(double));
        }
      ...
      if (p_quad->NumberofCodedPFrame == 1) p_quad->AveWp = p_quad->Wp;

      if ((p_quad->NumberofCodedPFrame < 8) &&
          (p_quad->NumberofCodedPFrame > 1))
        p_quad->AveWp =
            (p_quad->AveWp + p_quad->Wp * (p_quad->NumberofCodedPFrame - 1)) /
      ...
      if (p_Inp->NumberBFrames > 0) {
        ...
        p_quad->TargetBufferLevel +=
            (p_quad->AveWp * (p_Inp->NumberBFrames + 1) * p_quad->bit_rate /
                 (p_quad->frame_rate *
                  (p_quad->AveWp + p_quad->AveWb * p_Inp->NumberBFrames)) -
             p_quad->bit_rate / p_quad->frame_rate);
      }
      ...
      if (p_gen->NumberofCodedBFrame == 1) {
        ...
        if (p_gen->NumberofCodedBFrame < 8)
          p_quad->AveWb =
              (p_quad->AveWb + p_quad->Wb * (p_gen->NumberofCodedBFrame - 1)) /
    ...
    if (p_Vid->type == P_SLICE ||
        ((p_Vid->number != 0) && (p_Inp->RCUpdateMode == RC_MODE_1 ||
                                  p_Inp->RCUpdateMode == RC_MODE_3))) {
      /* frame layer rate control */
      if (p_Vid->BasicUnit == p_Vid->FrameSizeInMbs ||
          (p_Inp->RCUpdateMode == RC_MODE_3)) {
        if (p_quad->NumberofCodedPFrame > 0) {
          if (p_Inp->RCUpdateMode == RC_MODE_3) {
            int level_idx =
                (p_Vid->type == B_SLICE && p_Inp->HierarchicalCoding)
                    ? (p_Vid->p_curr_frm_struct->layer - 1)
                    : 0;
            int bitrate = (p_Vid->type == B_SLICE)
                              ? p_gen->RCBSliceBits[level_idx]
                              : (p_Vid->type == P_SLICE ? p_gen->RCPSliceBits
                                                        : p_gen->RCISliceBits);
            int level, denom = p_gen->NISlice * p_gen->RCISliceBits +
                               p_gen->NPSlice * p_gen->RCPSliceBits;
            ...
            p_quad->Target = (int)floor(
                (float)(1.0 * bitrate * p_gen->RemainingBits) / (float)denom +
                0.5F);
            // target given original taget rate and buffer considerations
            tmp_T = imax(
                0, (int)floor((double)bitrate -
                              p_quad->GAMMAP * (p_gen->CurrentBufferFullness -
                                                p_quad->TargetBufferLevel) +
                              0.5));
            // translate Target rate from B or I "domain" to P domain since the
            // P RC model is going to be used to select the QP for hierarchical
            // coding adjust the target QP to account for different temporal
            // levels
            switch (p_Vid->type) {
              case B_SLICE:
                p_quad->Target = (int)floor(
                    (float)p_quad->Target / p_Inp->RCBoverPRatio + 0.5F);
                break;
              case I_SLICE:
                p_quad->Target = (int)floor(
                    (float)p_quad->Target / (p_Inp->RCIoverPRatio * 4.0) +
                    0.5F);  // 4x accounts for the fact that header bits reduce
                            // the percentage of texture
                break;
              case P_SLICE:
              default: break;
            }
            ...
            p_quad->Target = (int)floor(
                p_quad->Wp * p_gen->RemainingBits /
                    (p_quad->Np * p_quad->Wp + p_quad->Nb * p_quad->Wb) +
                0.5);
            tmp_T = imax(
                0, (int)floor(p_quad->bit_rate / p_quad->frame_rate -
                              ...
                              0.5));
            p_quad->Target = (int)floor(
                p_quad->BETAP * (p_quad->Target - tmp_T) + tmp_T + 0.5);
          }
        }
      }
      /* basic unit layer rate control */
      ...
      if (p_Inp->RCUpdateMode != RC_MODE_3 || p_Vid->type == P_SLICE)
        p_quad->Target =
            iClip3(p_quad->LowerBound, p_quad->UpperBound2, p_quad->Target);
      if ((topfield) ||
          (fieldpic &&
           ((p_Inp->PicInterlace == ADAPTIVE_CODING) || (p_Inp->MbInterlace))))
  ...
  if ((p_Vid->type == P_SLICE ||
       (p_Inp->RCUpdateMode == RC_MODE_1 && (p_Vid->number != 0))) &&
      p_Vid->BasicUnit < p_Vid->FrameSizeInMbs && p_gen->FieldControl == 1) {
 ...
 *************************************************************************************
 */
void rc_update_pict(VideoParameters *p_Vid, InputParameters *p_Inp,
                    RCQuadratic *p_quad, RCGeneric *p_gen, int nbits) {
  int delta_bits =
      (nbits - (int)floor(p_quad->bit_rate / p_quad->frame_rate + 0.5F));
  // remaining # of bits in GOP
  p_gen->RemainingBits -= nbits;
  p_gen->CurrentBufferFullness += delta_bits;

  // update the lower bound and the upper bound for the target bits of each
  // frame, HRD consideration
  ...
  if (p_Vid->type == P_SLICE ||
      (p_Inp->RCUpdateMode == RC_MODE_1 && p_Vid->curr_frm_idx)) {
...
void rc_update_picture(VideoParameters *p_Vid, InputParameters *p_Inp,
                       int bits) {
  rc_update_pict(p_Vid, p_Inp, p_Vid->p_rc_quad, p_Vid->p_rc_gen, bits);
}

int updateComplexity(VideoParameters *p_Vid, RCQuadratic *p_quad,
                     RCGeneric *p_gen, Boolean is_updated, int nbits) {
  double Avem_Qc;
  ...
  /* basic unit layer rate control */
    ...
    if (is_updated) {
      if (p_gen->NoGranularFieldRC == 0 || p_gen->FieldControl == 0) {
        Avem_Qc = (double)p_quad->TotalFrameQP /
                  (double)p_quad->TotalNumberofBasicUnit;
 ...
 *************************************************************************************
 */
void rc_update_pict_frame(VideoParameters *p_Vid, InputParameters *p_Inp,
                          RCQuadratic *p_quad, RCGeneric *p_gen, int nbits) {
  ...
  switch (p_Inp->RCUpdateMode) {
    ...
    case RC_MODE_2:
    default:
      complexity = updateComplexity(p_Vid, p_quad, p_gen,
                                    (Boolean)(p_Vid->type == P_SLICE), nbits);
      if (p_Vid->type == P_SLICE) {
        if (p_gen->NoGranularFieldRC == 0 || p_gen->FieldControl == 0)
    ...
    case RC_MODE_3:
      complexity = updateComplexity(p_Vid, p_quad, p_gen,
                                    (Boolean)(p_Vid->type == P_SLICE), nbits);
      if (p_Vid->type == I_SLICE && (p_Vid->number != 0)) p_gen->NISlice--;
      ...
      if (p_Vid->type == P_SLICE) {
        ...
        updateBparams(p_quad, p_gen, complexity);
        p_gen->hierNb[p_Inp->HierarchicalCoding
                          ? (p_Vid->p_curr_frm_struct->layer - 1)
                          : 0]--;
      }
      break;
 ...
 *************************************************************************************
 */
void updateRCModel(VideoParameters *p_Vid, InputParameters *p_Inp,
                   RCQuadratic *p_quad, RCGeneric *p_gen) {
  int n_windowSize;
  int i;
  ...
  if (p_Vid->type == P_SLICE ||
    ...
    /*basic unit layer rate control*/
      ...
      p_quad->CurrentFrameMAD =
          (double)((p_gen->TotalMADBasicUnit >> 8) / p_Vid->BasicUnit);
      p_gen->TotalMADBasicUnit = 0;

      /* compute the average number of header bits*/
      p_quad->CodedBasicUnit =
          p_quad->TotalNumberofBasicUnit - p_quad->NumberofBasicUnit;
      if (p_quad->CodedBasicUnit > 0) {
        p_quad->PAveHeaderBits1 =
            (int)((double)(p_quad->PAveHeaderBits1 *
                               (p_quad->CodedBasicUnit - 1) +
                           p_gen->NumberofBasicUnitHeaderBits) /
                      p_quad->CodedBasicUnit +
                  0.5);
        if (p_quad->PAveHeaderBits3 == 0)
          p_quad->PAveHeaderBits2 = p_quad->PAveHeaderBits1;
        else {
          p_quad->PAveHeaderBits2 =
              (int)((double)(p_quad->PAveHeaderBits1 * p_quad->CodedBasicUnit +
                             p_quad->PAveHeaderBits3 *
                                 p_quad->NumberofBasicUnit) /
                        p_quad->TotalNumberofBasicUnit +
                    0.5);
        }
      }

      if ((p_quad->NumberofBasicUnit >= p_quad->TotalNumberofBasicUnit) ||
          (p_quad->NumberofBasicUnit < 0)) {
        fprintf(stderr,
                "Write into invalid memory in updateRCModel at frame %d, "
                "p_quad->NumberofBasicUnit %d\n",
      ...
      if (((p_Inp->PicInterlace == ADAPTIVE_CODING) || (p_Inp->MbInterlace)) &&
          (p_gen->FieldControl == 1))
        p_quad->FCBUCFMAD[p_quad->TotalNumberofBasicUnit - 1 -
                          p_quad->NumberofBasicUnit] = p_quad->CurrentFrameMAD;
      else
        p_quad->BUCFMAD[p_quad->TotalNumberofBasicUnit - 1 -
                        p_quad->NumberofBasicUnit] = p_quad->CurrentFrameMAD;

      if (p_quad->NumberofBasicUnit != 0)
        m_Nc = p_quad->NumberofCodedPFrame * p_quad->TotalNumberofBasicUnit +
               p_quad->CodedBasicUnit;
      else
        m_Nc =
            (p_quad->NumberofCodedPFrame - 1) * p_quad->TotalNumberofBasicUnit +
            p_quad->CodedBasicUnit;
    }

    if (m_Nc > 1) MADModelFlag = TRUE;
    ...
    p_quad->Pm_rgQp[0] =
        QP2Qstep(p_quad->m_Qc);  //*1.0/p_quad->CurrentFrameMAD;
    /*frame layer rate control*/
    if (p_Vid->BasicUnit == p_Vid->FrameSizeInMbs)
      p_quad->Pm_rgRp[0] =
          p_gen->NumberofTextureBits * 1.0 / p_quad->CurrentFrameMAD;
    /*basic unit layer rate control*/
    else
      p_quad->Pm_rgRp[0] =
          p_gen->NumberofBasicUnitTextureBits * 1.0 / p_quad->CurrentFrameMAD;
    ...
    n_windowSize =
        (p_quad->CurrentFrameMAD > p_quad->PreviousFrameMAD)
            ? (int)(p_quad->PreviousFrameMAD / p_quad->CurrentFrameMAD *
                    (RC_MODEL_HISTORY - 1))
            : (int)(p_quad->CurrentFrameMAD / p_quad->PreviousFrameMAD *
                    (RC_MODEL_HISTORY - 1));
    ...
    for (i = 0; i < (int)n_windowSize; i++) {
      error[i] = p_quad->m_X1 / p_quad->m_rgQp[i] +
                 p_quad->m_X2 / (p_quad->m_rgQp[i] * p_quad->m_rgQp[i]) -
                 p_quad->m_rgRp[i];
      std += error[i] * error[i];
    }
    threshold = (n_windowSize == 2) ? 0 : sqrt(std / n_windowSize);
    for (i = 0; i < (int)n_windowSize; i++) {
      if (fabs(error[i]) > threshold) m_rgRejected[i] = TRUE;
    }
 ...
 *************************************************************************************
 */
void RCModelEstimator(VideoParameters *p_Vid, InputParameters *p_Inp,
                      RCQuadratic *p_quad, int n_windowSize,
  ...
  for (i = 0; i < n_windowSize;
       i++) {  // find the number of samples which are not rejected
    if (m_rgRejected[i]) n_realSize--;
  }
  ...
  for (i = 0; i < n_windowSize; i++) {
    if (!m_rgRejected[i]) oneSampleQ = p_quad->m_rgQp[i];
  }
  for (i = 0; i < n_windowSize;
       i++) {  // if all non-rejected Q are the same, take 1st order model
 ...
 *************************************************************************************
 */
void updateMADModel(VideoParameters *p_Vid, InputParameters *p_Inp,
                    RCQuadratic *p_quad, RCGeneric *p_gen) {
  int n_windowSize;
  int i;
  double std = 0.0, threshold;
  ...
  if (p_quad->NumberofCodedPFrame > 0) {
    ...
    if (p_Vid->BasicUnit == p_Vid->FrameSizeInMbs)
      ...
      if (((p_Inp->PicInterlace == ADAPTIVE_CODING) || (p_Inp->MbInterlace)) &&
          (p_gen->FieldControl == 1))
        p_quad->ReferenceMAD[0] =
            p_quad->FCBUPFMAD[p_quad->TotalNumberofBasicUnit - 1 -
                              p_quad->NumberofBasicUnit];
      else
        p_quad->ReferenceMAD[0] =
            p_quad->BUPFMAD[p_quad->TotalNumberofBasicUnit - 1 -
                            p_quad->NumberofBasicUnit];
    }
    ...
    n_windowSize =
        (p_quad->CurrentFrameMAD > p_quad->PreviousFrameMAD)
            ? (int)((float)(RC_MODEL_HISTORY - 1) * p_quad->PreviousFrameMAD /
                    p_quad->CurrentFrameMAD)
            : (int)((float)(RC_MODEL_HISTORY - 1) * p_quad->CurrentFrameMAD /
                    p_quad->PreviousFrameMAD);
    ...
    // remove outlier
    for (i = 0; i < n_windowSize; i++) {
      error[i] = p_quad->MADPictureC1 * p_quad->ReferenceMAD[i] +
                 p_quad->MADPictureC2 - p_quad->PictureMAD[i];
      std += (error[i] * error[i]);
    }

    threshold = (n_windowSize == 2) ? 0 : sqrt(std / n_windowSize);
    for (i = 0; i < n_windowSize; i++) {
      if (fabs(error[i]) > threshold) PictureRejected[i] = TRUE;
    }
    // always include the last data point
 ...
 *************************************************************************************
 */
void MADModelEstimator(VideoParameters *p_Vid, InputParameters *p_Inp,
                       RCQuadratic *p_quad, int n_windowSize,
  ...
  for (i = 0; i < n_windowSize;
       i++) {  // find the number of samples which are not rejected
    if (PictureRejected[i]) n_realSize--;
  }
  ...
  for (i = 0; i < n_windowSize; i++) {
    if (!PictureRejected[i]) oneSampleQ = p_quad->PictureMAD[i];
  }

  for (i = 0; i < n_windowSize;
       i++) {  // if all non-rejected MAD are the same, take 1st order model
    if ((p_quad->PictureMAD[i] != oneSampleQ) && !PictureRejected[i])
      estimateX2 = TRUE;
    if (!PictureRejected[i])
      p_quad->MADPictureC1 +=
          p_quad->PictureMAD[i] / (p_quad->ReferenceMAD[i] * n_realSize);
  }
 ...
 *************************************************************************************
 */
int updateQPRC0(VideoParameters *p_Vid, InputParameters *p_Inp,
                RCQuadratic *p_quad, RCGeneric *p_gen, int topfield) {
  int m_Bits;
  int BFrameNumber;
  int StepSize;
  int SumofBasicUnit;
  ...
  /* frame layer rate control */
  if (p_Vid->BasicUnit == p_Vid->FrameSizeInMbs) {
    /* fixed quantization parameter is used to coded I frame, the first P frame
    and the first B frame the quantization parameter is adjusted according the
    available channel bandwidth and the type of video */
    /*top field*/
    if ((topfield) || (p_gen->FieldControl == 0)) {
      if (p_Vid->type == I_SLICE) {
        ...
        if (p_Inp->NumberBFrames == 1) {
          ...
          p_quad->m_Qc =
              imax(p_quad->m_Qc, imax(p_quad->PrevLastQP, p_quad->CurrLastQP));
          p_quad->m_Qc = imax(p_quad->m_Qc, p_quad->CurrLastQP + 1);
          p_quad->m_Qc = iClip3(p_Vid->RCMinQP + p_quad->bitdepth_qp_scale,
                                p_Vid->RCMaxQP + p_quad->bitdepth_qp_scale,
                                p_quad->m_Qc);  // Clipping
        } else {
          BFrameNumber = (p_quad->NumberofBFrames + 1) % p_Inp->NumberBFrames;
          if (BFrameNumber == 0) BFrameNumber = p_Inp->NumberBFrames;
          ...
          if ((p_quad->CurrLastQP - p_quad->PrevLastQP) <=
              (-2 * p_Inp->NumberBFrames - 3))
            StepSize = -3;
          else if ((p_quad->CurrLastQP - p_quad->PrevLastQP) ==
                   (-2 * p_Inp->NumberBFrames - 2))
            StepSize = -2;
          else if ((p_quad->CurrLastQP - p_quad->PrevLastQP) ==
                   (-2 * p_Inp->NumberBFrames - 1))
            StepSize = -1;
          else if ((p_quad->CurrLastQP - p_quad->PrevLastQP) ==
                   (-2 * p_Inp->NumberBFrames))
            StepSize = 0;
          else if ((p_quad->CurrLastQP - p_quad->PrevLastQP) ==
                   (-2 * p_Inp->NumberBFrames + 1))
            StepSize = 1;
          ...
          p_quad->m_Qc += iClip3(
              -2 * (BFrameNumber - 1), 2 * (BFrameNumber - 1),
              (BFrameNumber - 1) * (p_quad->CurrLastQP - p_quad->PrevLastQP) /
        ...
        p_quad->CurrentFrameMAD =
            p_quad->MADPictureC1 * p_quad->PreviousPictureMAD +
            p_quad->MADPictureC2;
        ...
        if (p_quad->Target < 0) {
          ...
          m_Bits = imax(m_Bits, (int)(p_quad->bit_rate /
                                      (MINVALUE * p_quad->frame_rate)));
          ...
          p_quad->m_Qc = iClip3(p_Vid->RCMinQP + p_quad->bitdepth_qp_scale,
                                p_Vid->RCMaxQP + p_quad->bitdepth_qp_scale,
                                p_quad->m_Qc);  // clipping
          p_quad->m_Qc = iClip3(m_Qp - MaxQpChange, m_Qp + MaxQpChange,
                                p_quad->m_Qc);  // control variation
        }
  ...
  /*basic unit layer rate control*/
    ...
    if (p_Vid->type == I_SLICE) {
      ...
      if ((p_gen->NumberofGOP == 1) && (p_quad->NumberofPPicture == 0)) {
        if ((p_gen->FieldControl == 0) ||
            ((p_gen->FieldControl == 1) && (p_gen->NoGranularFieldRC == 0)))
        ...
        /*the average QP of the previous frame is used to coded the first basic
         * unit of the current frame or field*/
        if (p_quad->NumberofBasicUnit == SumofBasicUnit)
          ...
          p_quad->Target -= (p_gen->NumberofBasicUnitHeaderBits +
                             p_gen->NumberofBasicUnitTextureBits);
          p_gen->NumberofBasicUnitHeaderBits = 0;
          p_gen->NumberofBasicUnitTextureBits = 0;
          if (p_quad->Target < 0)
            return updateNegativeTarget(p_Vid, p_Inp, p_quad, p_gen, topfield,
                                        m_Qp);
          else {
            /*predict the MAD of current picture*/
 ...
 *************************************************************************************
 */
int updateQPRC1(VideoParameters *p_Vid, InputParameters *p_Inp,
                RCQuadratic *p_quad, RCGeneric *p_gen, int topfield) {
  int m_Bits;
  int SumofBasicUnit;
  int MaxQpChange, m_Qp, m_Hp;
  ...
  if (p_Vid->BasicUnit == p_Vid->FrameSizeInMbs) {
    /* fixed quantization parameter is used to coded I frame, the first P frame
    and the first B frame the quantization parameter is adjusted according the
    available channel bandwidth and the type of vide */
    /*top field*/
    if ((topfield) || (p_gen->FieldControl == 0)) {
      if (p_Vid->number == 0) {
        p_quad->m_Qc = p_quad->MyInitialQp;
    ...
    /*bottom field*/
      ...
      if (p_gen->NoGranularFieldRC == 0) updateBottomField(p_Inp, p_quad);
      return p_quad->m_Qc;
    }
  }
  /*basic unit layer rate control*/
 ...
 *************************************************************************************
 */
int updateQPRC2(VideoParameters *p_Vid, InputParameters *p_Inp,
                RCQuadratic *p_quad, RCGeneric *p_gen, int topfield) {
  int m_Bits;
  int SumofBasicUnit;
  int MaxQpChange, m_Qp, m_Hp;
    ...
    if ((topfield) || (p_gen->FieldControl == 0)) {
      if (p_Vid->number == 0) {
        ...
        if (p_Vid->p_curr_frm_struct->idr_flag)
          ...
          p_quad->m_Qc =
              p_quad->CurrLastQP;  // Set QP to average qp of last P frame

        return p_quad->m_Qc;
      } else if (p_Vid->type == B_SLICE) {
        int prevQP = imax(p_quad->PrevLastQP, p_quad->CurrLastQP);
        // for more than one consecutive B frames the below call will overwrite
        // the old anchor frame QP with the current value it should be called
        // once in the B-frame sequence....this should be modified for the BU <
        // frame as well
        if ((p_Inp->PicInterlace == ADAPTIVE_CODING) || (p_Inp->MbInterlace))
          updateQPInterlace(p_quad, p_gen);
  ...
  /*basic unit layer rate control*/
    ...
    if (p_Vid->number == 0) {
      ...
      p_quad->m_Qc =
          p_quad->PrevLastQP;  // Set QP to average qp of last P frame
      p_quad->PrevLastQP = p_quad->CurrLastQP;
 ...
 *************************************************************************************
 */
int updateQPRC3(VideoParameters *p_Vid, InputParameters *p_Inp,
                RCQuadratic *p_quad, RCGeneric *p_gen, int topfield) {
  int m_Bits;
  int SumofBasicUnit;
  int MaxQpChange, m_Qp, m_Hp;
    ...
    if ((topfield) || (p_gen->FieldControl == 0)) {
      if (p_Vid->number == 0) {
        ...
        if (((p_Vid->type == B_SLICE &&
              (p_Vid->p_curr_frm_struct->layer &&
               p_Vid->p_curr_frm_struct->atom_idx == 1)) ||
             p_Vid->type == I_SLICE) &&
            ((p_Inp->PicInterlace == ADAPTIVE_CODING) || (p_Inp->MbInterlace)))
          updateQPInterlace(p_quad, p_gen);
        /*adaptive field/frame coding*/
        ...
        if (p_Vid->BasicUnit < p_Vid->FrameSizeInMbs &&
            p_Vid->type != P_SLICE) {
          // when RC_MODE_3 is set and basic unit is smaller than a frame, note
          // that: the linear MAD model and the quadratic QP model operate on
          // small units and not on a whole frame; we therefore have to account
          // for this
          p_quad->PreviousPictureMAD = p_quad->PreviousWholeFrameMAD;
        }
        if (p_Vid->type == I_SLICE)
          m_Hp = 0;  // it is usually a very small portion of the total I_SLICE
                     // bit budget
        ...
        if (p_Vid->type == B_SLICE) {
          ...
          if (p_Inp->HierarchicalCoding && p_Vid->p_curr_frm_struct->layer)
            p_quad->m_Qc -= (p_Vid->p_curr_frm_struct->p_atom->gop_levels -
                             p_Vid->p_curr_frm_struct->layer);
          // check bounds
          p_quad->m_Qc = iClip3(prevqp - (p_Inp->HierarchicalCoding ? 0 : 5),
                                prevqp + 5, p_quad->m_Qc);  // control variation
          p_quad->m_Qc = iClip3(p_Vid->RCMinQP + p_quad->bitdepth_qp_scale,
                                p_Vid->RCMaxQP + p_quad->bitdepth_qp_scale,
                                p_quad->m_Qc);  // clipping
        }
 ...
 *************************************************************************************
 */
int updateFirstP(VideoParameters *p_Vid, InputParameters *p_Inp,
                 RCQuadratic *p_quad, RCGeneric *p_gen, int topfield) {
  ...
  if ((!topfield) && (p_quad->NumberofBasicUnit == 0)) {
    ...
    if ((p_Vid->active_sps->frame_mbs_only_flag) ||
        (p_Inp->PicInterlace == FIELD_CODING)) {
      p_quad->TotalQpforPPicture += p_quad->m_Qc;
      p_quad->PrevLastQP = p_quad->CurrLastQP;
      p_quad->CurrLastQP = p_quad->m_Qc;
 ...
 *************************************************************************************
 */
int updateNegativeTarget(VideoParameters *p_Vid, InputParameters *p_Inp,
                         RCQuadratic *p_quad, RCGeneric *p_gen, int topfield,
                         int m_Qp) {
  int PAverageQP;
  ...
  p_quad->m_Qc = imin(p_quad->m_Qc,
                      p_Vid->RCMaxQP + p_quad->bitdepth_qp_scale);  // clipping
  ...
  if (p_quad->NumberofBasicUnit == 0) {
    if ((!topfield) || (p_gen->FieldControl == 0)) {
      ...
      if ((p_Vid->active_sps->frame_mbs_only_flag) ||
          (p_Inp->PicInterlace == FIELD_CODING)) {
        PAverageQP = (int)((double)p_quad->TotalFrameQP /
                               (double)p_quad->TotalNumberofBasicUnit +
                           0.5);
      ...
      /*adaptive field/frame coding*/
      else if ((p_Inp->PicInterlace == ADAPTIVE_CODING) ||
               (p_Inp->MbInterlace)) {
        if (p_gen->FieldControl == 0) {
          PAverageQP = (int)((double)p_quad->TotalFrameQP /
                                 (double)p_quad->TotalNumberofBasicUnit +
 ...
 *************************************************************************************
 */
int updateFirstBU(VideoParameters *p_Vid, InputParameters *p_Inp,
                  RCQuadratic *p_quad, RCGeneric *p_gen, int topfield) {
  /*adaptive field/frame coding*/
  if (((p_Inp->PicInterlace == ADAPTIVE_CODING) || (p_Inp->MbInterlace)) &&
      (p_gen->FieldControl == 0)) {
  ...
  if (p_quad->Target <= 0) {
    ...
    if (topfield || (p_gen->FieldControl == 0)) p_quad->GOPOverdue = TRUE;
  } else {
    p_quad->m_Qc = p_quad->PAveFrameQP;
  }
  p_quad->TotalFrameQP += p_quad->m_Qc;
 ...
 *************************************************************************************
 */
void updateLastBU(VideoParameters *p_Vid, InputParameters *p_Inp,
                  RCQuadratic *p_quad, RCGeneric *p_gen, int topfield) {
  int PAverageQP;
 ...
 *************************************************************************************
 */
void predictCurrPicMAD(InputParameters *p_Inp, RCQuadratic *p_quad,
                       RCGeneric *p_gen) {
  int i;
  if (((p_Inp->PicInterlace == ADAPTIVE_CODING) || (p_Inp->MbInterlace)) &&
      (p_gen->FieldControl == 1)) {
    p_quad->CurrentFrameMAD =
        p_quad->MADPictureC1 *
            p_quad->FCBUPFMAD[p_quad->TotalNumberofBasicUnit -
                              p_quad->NumberofBasicUnit] +
        p_quad->MADPictureC2;
    p_quad->TotalBUMAD = 0;
    for (i = p_quad->TotalNumberofBasicUnit - 1;
         i >= (p_quad->TotalNumberofBasicUnit - p_quad->NumberofBasicUnit);
         i--) {
      p_quad->CurrentBUMAD =
          p_quad->MADPictureC1 * p_quad->FCBUPFMAD[i] + p_quad->MADPictureC2;
      p_quad->TotalBUMAD += p_quad->CurrentBUMAD * p_quad->CurrentBUMAD;
    }
  } else {
    p_quad->CurrentFrameMAD =
        p_quad->MADPictureC1 * p_quad->BUPFMAD[p_quad->TotalNumberofBasicUnit -
                                               p_quad->NumberofBasicUnit] +
        p_quad->MADPictureC2;
    p_quad->TotalBUMAD = 0;
    for (i = p_quad->TotalNumberofBasicUnit - 1;
         ...
         i--) {
      p_quad->CurrentBUMAD =
          p_quad->MADPictureC1 * p_quad->BUPFMAD[i] + p_quad->MADPictureC2;
      p_quad->TotalBUMAD += p_quad->CurrentBUMAD * p_quad->CurrentBUMAD;
    }
  }
}
 ...
 *************************************************************************************
 */
void updateModelQPBU(VideoParameters *p_Vid, InputParameters *p_Inp,
                     RCQuadratic *p_quad, int m_Qp) {
  double dtmp, m_Qstep;
  int m_Bits;
  /*compute the total number of bits for the current basic unit*/
  m_Bits = (int)(p_quad->Target * p_quad->CurrentFrameMAD *
                 p_quad->CurrentFrameMAD / p_quad->TotalBUMAD);
  /*compute the number of texture bits*/
  m_Bits -= p_quad->PAveHeaderBits2;

  m_Bits =
      imax(m_Bits, (int)(p_quad->bit_rate / (MINVALUE * p_quad->frame_rate *
                                             p_quad->TotalNumberofBasicUnit)));

  dtmp = p_quad->CurrentFrameMAD * p_quad->CurrentFrameMAD * p_quad->m_X1 *
             p_quad->m_X1 +
         4 * p_quad->m_X2 * p_quad->CurrentFrameMAD * m_Bits;
  if ((p_quad->m_X2 == 0.0) || (dtmp < 0) ||
      ((sqrt(dtmp) - p_quad->m_X1 * p_quad->CurrentFrameMAD) <=
       0.0))  // fall back 1st order mode
    m_Qstep = (float)(p_quad->m_X1 * p_quad->CurrentFrameMAD / (double)m_Bits);
  else  // 2nd order mode
    m_Qstep = (float)((2 * p_quad->m_X2 * p_quad->CurrentFrameMAD) /
                      (sqrt(dtmp) - p_quad->m_X1 * p_quad->CurrentFrameMAD));

  p_quad->m_Qc = Qstep2QP(m_Qstep, p_quad->bitdepth_qp_scale);
  p_quad->m_Qc =
      imin(m_Qp + p_quad->DDquant, p_quad->m_Qc);  // control variation
  ...
  p_quad->m_Qc =
      iClip3(m_Qp - p_quad->DDquant, p_Vid->RCMaxQP + p_quad->bitdepth_qp_scale,
             p_quad->m_Qc);  // clipping
  if (p_Inp->basicunit >= p_quad->MBPerRow)
...
void updateModelQPFrame(RCQuadratic *p_quad, int m_Bits) {
  ...
  dtmp = p_quad->CurrentFrameMAD * p_quad->m_X1 * p_quad->CurrentFrameMAD *
             p_quad->m_X1 +
         4 * p_quad->m_X2 * p_quad->CurrentFrameMAD * m_Bits;
  if ((p_quad->m_X2 == 0.0) || (dtmp < 0) ||
      ((sqrt(dtmp) - p_quad->m_X1 * p_quad->CurrentFrameMAD) <=
...
int rc_handle_mb(Macroblock *currMB, int prev_mb) {
  ...
  if (prev_mb > -1) {
    if (p_Inp->MbInterlace == ADAPTIVE_CODING && !p_Vid->bot_MB &&
        currMB->mb_field)
      mb_qp = prevMB->qp;
  }
  ...
  if (p_Inp->basicunit != p_Vid->FrameSizeInMbs) {
    ...
    if (((p_Vid->type == I_SLICE || p_Vid->type == B_SLICE) &&
         p_Inp->RCUpdateMode != RC_MODE_1) ||
        !(p_Vid->number)) {
      return mb_qp;
      ...
      if (!p_Vid->write_macroblock) {
        if (!((p_Inp->MbInterlace) && p_Vid->bot_MB)) {
          if (p_Inp->RCUpdateMode <= MAX_RC_MODE &&
            ...
            // frame coding
            if (p_Vid->active_sps->frame_mbs_only_flag) {
              p_Vid->BasicUnitQP = p_Vid->updateQP(p_Vid, p_Inp, p_quad, p_gen,
                                                   p_gen->TopFieldFlag) -
                                   p_quad->bitdepth_qp_scale;
            }
            // picture adaptive field/frame coding
            else if (p_Inp->MbInterlace ||
                     ((p_Inp->PicInterlace != FRAME_CODING) &&
                      (p_gen->NoGranularFieldRC == 0))) {
              p_Vid->BasicUnitQP = p_Vid->updateQP(p_Vid, p_Inp, p_quad, p_gen,
                                                   p_gen->TopFieldFlag) -
                                   p_quad->bitdepth_qp_scale;
          ...
          if (p_Vid->current_mb_nr == 0) p_Vid->BasicUnitQP = mb_qp;
...
void rc_init_top_field(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  p_Vid->rc_init_pict_ptr(p_Vid, p_Inp, p_quad, p_gen, 0, 1,
                          (p_Inp->PicInterlace == FIELD_CODING), 1.0F);
  p_Vid->p_curr_frm_struct->qp = p_Vid->qp =
      p_Vid->updateQP(p_Vid, p_Inp, p_quad, p_gen, 1) -
      p_quad->bitdepth_qp_scale;
}
 ...
 *************************************************************************************
 */
void rc_init_bottom_field(VideoParameters *p_Vid, InputParameters *p_Inp,
                          int TopFieldBits) {
  RCGeneric *p_gen = p_Vid->p_rc_gen;
  RCQuadratic *p_quad = p_Vid->p_rc_quad;
  ...
  p_Vid->p_curr_frm_struct->qp = p_Vid->qp =
      p_Vid->updateQP(p_Vid, p_Inp, p_quad, p_gen, 0) -
      p_quad->bitdepth_qp_scale;
}
 ...
 *************************************************************************************
 */
void rc_init_frame_rdpic(VideoParameters *p_Vid, InputParameters *p_Inp,
                         float rateRatio) {
  RCGeneric *p_gen = p_Vid->p_rc_gen;
  ...
  switch (p_Inp->RCUpdateMode) {
    ...
    case RC_MODE_3:
      ...
      if (p_Inp->RDPictureDecision || p_Inp->MbInterlace == ADAPTIVE_CODING ||
          p_Inp->PicInterlace == ADAPTIVE_CODING) {
