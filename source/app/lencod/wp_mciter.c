...
void EstimateWPPSliceAlg2(Slice *currSlice, int select_offset) {
  ...
  if (p_Inp->ChromaWeightSupport == 1) {
    for (k = 0; k < 2; k++) {
      dc_org_UV[k] = ComputeImgSum(p_Vid->pImgOrg[k + 1], p_Vid->height_cr,
                                   p_Vid->width_cr);
    }
  }
      ...
      if (wpxDetermineWP(currSlice, clist, n)) {
        ...
        if (p_Inp->ChromaWeightSupport == 1) {
          for (k = 0; k < 2; k++) {
            ...
            dc_ref_UV[n][k] =
                ComputeImgSum(tmpPtr, p_Vid->height_cr, p_Vid->width_cr);
          }
        }
        ...
        if (select_offset == 0) {
          if (dc_ref[n] != 0.0)
            weight[clist][n][0] =
                (short)(default_weight[0] * dc_org / dc_ref[n] + 0.5);
          else
            weight[clist][n][0] =
          ...
          if (p_Inp->ChromaWeightSupport == 1) {
            if (dc_ref_UV[n][0] != 0)
              weight[clist][n][1] =
                  (short)(default_weight[1] * dc_org_UV[0] / dc_ref_UV[n][0] +
                          0.5);
            else
              weight[clist][n][1] =
                  default_weight[1];  // only used when reference picture is
                                      // black
            weight[clist][n][1] = sClip3(-128, 127, weight[clist][n][1]);

            if (dc_ref_UV[n][1] != 0)
              weight[clist][n][2] =
                  (short)(default_weight[2] * dc_org_UV[1] / dc_ref_UV[n][1] +
                          0.5);
            else
              weight[clist][n][2] =
                  default_weight[2];  // only used when reference picture is
                                      // black
          ...
          offset[clist][n][0] =
              (offset[clist][n][0] + ((p_Vid->bitdepth_luma - 8) >> 1)) >>
              (p_Vid->bitdepth_luma - 8);
          offset[clist][n][0] = sClip3(-128, 127, offset[clist][n][0]);
          offset[clist][n][0] = offset[clist][n][0]
                                << (p_Vid->bitdepth_luma - 8);
          weight[clist][n][0] = default_weight[0];

          if (p_Inp->ChromaWeightSupport == 1) {
            offset[clist][n][1] =
                (short)((dc_org_UV[0] - dc_ref_UV[n][0]) / (p_Vid->size_cr) +
                        0.5);
            offset[clist][n][1] =
                (offset[clist][n][1] + ((p_Vid->bitdepth_chroma - 8) >> 1)) >>
                (p_Vid->bitdepth_chroma - 8);
            offset[clist][n][1] = sClip3(-128, 127, offset[clist][n][1]);
            offset[clist][n][1] = offset[clist][n][1]
                                  << (p_Vid->bitdepth_chroma - 8);

            weight[clist][n][1] = default_weight[1];

            offset[clist][n][2] =
                (short)((dc_org_UV[1] - dc_ref_UV[n][1]) / (p_Vid->size_cr) +
                        0.5);
            offset[clist][n][2] =
                (offset[clist][n][2] + ((p_Vid->bitdepth_chroma - 8) >> 1)) >>
                (p_Vid->bitdepth_chroma - 8);
            offset[clist][n][2] = sClip3(-128, 127, offset[clist][n][2]);
            offset[clist][n][2] = offset[clist][n][2]
                                  << (p_Vid->bitdepth_chroma - 8);
  ...
  if (p_Vid->active_pps->weighted_bipred_idc == 2)  //! implicit mode
  {
    for (i = 0; i < currSlice->listXsize[LIST_0]; i++) {
      for (j = 0; j < currSlice->listXsize[LIST_1]; j++) {
        td = (short)iClip3(-128, 127,
                           (currSlice->listX[LIST_1][j]->poc -
                            currSlice->listX[LIST_0][i]->poc));
        tb = (short)iClip3(
            -128, 127,
            (p_Vid->enc_picture->poc - currSlice->listX[LIST_0][i]->poc));
        for (comp = 0; comp < 3; comp++) {
          ...
          if (td == 0) {
            ...
            if (im_weight[1][i][j][comp] < -64 ||
                im_weight[1][i][j][comp] > 128)
              im_weight[1][i][j][comp] = default_weight[comp];
        ...
        printf("%d imp weight[%d][%d] = %d  , %d (%d %d %d) (%d %d) (%d %d)\n",
               p_Vid->enc_picture->poc, i, j, im_weight[0][i][j][0],
               im_weight[1][i][j][0], p_Vid->enc_picture->poc,
               currSlice->listX[LIST_0][i]->poc,
               currSlice->listX[LIST_1][j]->poc, DistScaleFactor, tx, td, tb);
#endif
      }
    }
      ...
      for (n = 0; n < currSlice->listXsize[clist]; n++) {
        if (wpxDetermineWP(currSlice, clist, n)) {
          ...
          if (p_Inp->ChromaWeightSupport == 1) {
            for (k = 0; k < 2; k++) {
              ...
              if (dc_ref_UV[clist][n][k] != 0.0)
                wf_weight = (short)(default_weight[k + 1] * dc_org_UV[k] /
                                        dc_ref_UV[clist][n][k] +
                                    0.5);
              else
                wf_weight = default_weight[k + 1];  // only used when reference
                                                    // picture is black
          ...
          for (i = 0; i < 3; i++) {
            ...
            currSlice->wp_offset[clist][n][i] = offset[clist][n][i];
#if DEBUG_WP
            printf("%d %d\n", currSlice->wp_weight[clist][index][comp],
                   currSlice->wp_offset[clist][index][comp]);
#endif
          }
        }
      }
    ...
    if (p_Vid->active_pps->weighted_bipred_idc != 1) {
      for (clist = 0; clist < 2 + list_offset; clist++) {
        for (index = 0; index < currSlice->listXsize[clist]; index++) {
          memcpy(currSlice->wp_weight[clist][index], default_weight,
                 3 * sizeof(short));
    ...
    for (i = 0; i < currSlice->listXsize[LIST_0]; i++) {
      for (j = 0; j < currSlice->listXsize[LIST_1]; j++) {
        ...
        printf("bpw weight[%d][%d] = %d  , %d (%d %d %d) (%d %d) (%d %d)\n", i,
               j, currSlice->wbp_weight[0][i][j][0],
               currSlice->wbp_weight[1][i][j][0], p_Vid->enc_picture->poc,
               currSlice->listX[LIST_0][i]->poc,
               currSlice->listX[LIST_1][j]->poc, DistScaleFactor, tx, tx, tx);
#endif
      }
    }
  }
  ...
  if (select_method == 1)  //! implicit mode
  {
    ...
    */
    ...
    if (select_method == 0)  //! explicit mode
    {
      for (clist = 0; clist < 2 + list_offset; clist++) {
        for (index = 0; index < currSlice->listXsize[clist]; index++) {
          memcpy(p_Vid->currentSlice->wp_weight[clist][index],
                 weight[clist][index], 3 * sizeof(short));
          memcpy(p_Vid->currentSlice->wp_offset[clist][index],
                 offset[clist][index], 3 * sizeof(short));
        }
      }
    } else {
      for (clist = 0; clist < 2 + list_offset; clist++) {
...
#if DEBUG_WP
        printf ("bpw weight[%d][%d] = %d  , %d (%d %d %d) (%d %d) (%d %d)\n", i,
j, p_Vid->currentSlice->wbp_weight[0][i][j][0],
p_Vid->currentSlice->wbp_weight[1][i][j][0],
        p_Vid->enc_picture->poc,currSlice->listX[LIST_0][i]->poc,
currSlice->listX[LIST_1][j]->poc, DistScaleFactor ,tx,tx,tx); #endif
      }
    }
    */
  }
  ...
  if (select_method == 0)  //! implicit mode
  {
    ...
    active_refs[0] =
        (p_Inp->B_List0_refs[view_id] == 0
             ? currSlice->listXsize[0]
             : imin(p_Inp->B_List0_refs[view_id], currSlice->listXsize[0]));
    active_refs[1] =
        (p_Inp->B_List1_refs[view_id] == 0
             ? currSlice->listXsize[1]
             : imin(p_Inp->B_List1_refs[view_id], currSlice->listXsize[1]));
    ...
    {
      {
        ...
        if (is_intra(currMB))  // intra macroblocks are not used for calculation
                               // of the filter coeffs.
          continue;
          ...
          if (ref_frame != -1) {
            for (yi = 0; yi < 4; yi++) {    // y
              for (xj = 0; xj < 4; xj++) {  // x
                ...
                // y_pos =
                // imax(0,imin(out4Y_height,4*(y+yi)+4*IMG_PAD_SIZE+mvy));
                y_pos = imax(-4 * IMG_PAD_SIZE_Y,
                             imin(out4Y_height, 4 * (y + yi) + mvy));
                // x_pos = imax(0,imin(out4Y_width,
                // 4*(x+xj)+4*IMG_PAD_SIZE+mvx));
                x_pos = imax(-4 * IMG_PAD_SIZE_X,
                             imin(out4Y_width, 4 * (x + xj) + mvx));

                temp = currSlice->listX[LIST_0][ref_frame]->p_curr_img_sub[(
                    y_pos & 0x03)][(x_pos & 0x03)][y_pos >> 2][x_pos >> 2];
    ...
    for (list = 0; list < numlists; list++) {
      for (frame = 0; frame < currSlice->listXsize[list]; frame++) {
        ...
        if (p_Vid->frameOffsetCount[list][frame] > 0) {
          offset =
              (int)(fabs(dtemp) / (double)p_Vid->frameOffsetCount[list][frame] +
                    0.5);
          ...
          // printf("list %d frame %d offset %d frameOffsetCount %d,
          // frameOffsetTotal %d\n", list, frame,
          // p_Vid->frameOffset[list][frame],
          // p_Vid->frameOffsetCount[list][frame],
          // p_Vid->frameOffsetTotal[list][frame]);
        }
        // else
        //{
