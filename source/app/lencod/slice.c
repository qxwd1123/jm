...
int allocate_block_mem(Slice *currSlice) {
  ...
  alloc_size +=
      get_mem2Dint(&currSlice->tblk16x16, MB_BLOCK_SIZE, MB_BLOCK_SIZE);
  alloc_size += get_mem4Dint(&currSlice->i16blk4x4, BLOCK_SIZE, BLOCK_SIZE,
...
void free_block_mem(Slice *currSlice) {
  if (currSlice->i16blk4x4) free_mem4Dint(currSlice->i16blk4x4);
  if (currSlice->tblk16x16) free_mem2Dint(currSlice->tblk16x16);
  if (currSlice->tblk4x4) free_mem2Dint(currSlice->tblk4x4);
}
...
static void CAVLC_init(Slice *currSlice) {
  memset(&currSlice->p_Vid->nz_coeff[0][0][0], 0,
         currSlice->PicSizeInMbs * 4 * (4 + currSlice->num_blk8x8_uv) *
             sizeof(int));
}
...
static int alloc_rddata(Slice *currSlice, RD_DATA *rd_data) {
  ...
  alloc_size +=
      get_mem3Dpel(&(rd_data->rec_mb), 3, MB_BLOCK_SIZE, MB_BLOCK_SIZE);

  alloc_size += get_mem_ACcoeff(currSlice->p_Vid, &(rd_data->cofAC));
  alloc_size += get_mem_DCcoeff(&(rd_data->cofDC));

  if ((currSlice->slice_type != I_SLICE) && currSlice->slice_type != SI_SLICE) {
    alloc_size += get_mem5Dmv(&(rd_data->all_mv), 2,
                              currSlice->max_num_references, 9, 4, 4);
  }

  // Why is this stored as height_blk * width_blk?
  alloc_size += get_mem2D((byte ***)&(rd_data->ipredmode),
                          currSlice->height_blk, currSlice->width_blk);
...
static void free_rddata(Slice *currSlice, RD_DATA *rd_data) {
  if (rd_data->refar) free_mem3D((byte ***)rd_data->refar);
  if (rd_data->ipredmode) free_mem2D((byte **)rd_data->ipredmode);

  if ((currSlice->slice_type != I_SLICE) && currSlice->slice_type != SI_SLICE) {
    if (rd_data->all_mv) free_mem5Dmv(rd_data->all_mv);
  }

  if (rd_data->cofDC) free_mem_DCcoeff(rd_data->cofDC);
  if (rd_data->cofAC) free_mem_ACcoeff(rd_data->cofAC);

  if (rd_data->rec_mb) free_mem3Dpel(rd_data->rec_mb);
}
...
static int start_slice(Slice *currSlice, StatParameters *cur_stats) {
  ...
  RTPUpdateTimestamp(p_Vid, currSlice->frame_no);  // this has no side effects,
                                                   // just leave it for all NALs
  ...
  for (i = 0; i < NumberOfPartitions; i++) {
    ...
    if (currSlice->symbol_mode == CABAC) {
      ...
      if (currStream->bits_to_go != 8) header_len += currStream->bits_to_go;
  ...
  if (p_Vid->p_Inp->num_of_views > 1 && p_Vid->view_id == 1 &&
      currSlice->nal_reference_idc && currSlice->slice_nr == 0) {
    for (i = 0; i < currSlice->listXsize[0]; i++) {
      if (currSlice->listX[0][i]->view_id == 0) break;
    }
    if (i < currSlice->listXsize[0]) p_Vid->enc_picture->ref_pic_na[0] = i;
    for (i = 0; i < currSlice->listXsize[1]; i++) {
      if (currSlice->listX[1][i]->view_id == 0) break;
    }
    if (i < currSlice->listXsize[1]) p_Vid->enc_picture->ref_pic_na[1] = i;
  }
#endif
  return header_len;
}
...
void create_slice_nalus(Slice *currSlice, int is_bottom) {
  ...
  // const int buffer_size = 500 + p_Vid->FrameSizeInMbs * (128 + 256 *
  // p_Vid->bitdepth_luma + 512 * p_Vid->bitdepth_chroma);
  ...
  for (part = 0; part < currSlice->max_part_nr; part++) {
    if (currSlice->partArr[part].bitstream->write_flag) {
      ...
      nalu->startcodeprefix_len = 1 + (currSlice->start_mb_nr == 0 && part == 0
                                           ? ZEROBYTES_SHORTSTARTCODE + 1
      ...
      if (currSlice->idr_flag) {
        ...
        if (currSlice->partition_mode == 0) {
          ...
          if (p_Vid->num_of_layers == 2)  // if(p_Inp->num_of_views==2)
          {
            nalu->nal_unit_type = p_Vid->dpb_layer_id /*p_Vid->view_id*/ == 0
                                      ? NALU_TYPE_SLICE
                                      : NALU_TYPE_SLC_EXT;
          } else
 ...
 ************************************************************************
 */
static int terminate_slice(Macroblock *currMB, int lastslice,
  ...
  for (part = 0; part < currSlice->max_part_nr; part++) {
    ...
    if (currStream->write_flag) {
      if (currSlice->symbol_mode == CAVLC) {
        ...
        currNalu->len = RBSPtoEBSP(currNalu->buf, currStream->streamBuffer,
                                   currStream->byte_pos);
      } else  // CABAC
      {
  ...
  cur_stats->bit_use_stuffing_bits[currSlice->slice_type] +=
      currMB->bits.mb_stuffing - tmp_stuffingbits;
...
static void init_bipred_enabled(VideoParameters *p_Vid) {
  ...
  if (p_Vid->currentSlice->slice_type != B_SLICE ||
      !p_Inp->BiPredMotionEstimation)
    return;
...
************************************************************************
*/
int encode_one_slice(VideoParameters *p_Vid, int SliceGroupId,
                     int TotalCodedMBs) {
  InputParameters *p_Inp = p_Vid->p_Inp;
  ...
  // printf ("\n\nEncode_one_slice: PictureID %d SliceGroupId %d  SliceID %d
  // FirstMB %d \n", p_Vid->frame_no, SliceGroupId, p_Vid->current_slice_nr,
  ...
  // Done here since we may wish to disable adaptive rounding on occasional
  // intervals (even at a frame or gop level).
  ...
  p_Vid->checkref = (short)(p_Inp->rdopt && p_Inp->RestrictRef &&
  ...
  // Rate control
  if (p_Inp->RCEnable) rc_store_slice_header_bits(p_Vid, p_Inp, len);
  ...
  while (end_of_slice == FALSE)  // loop over macroblocks
  {
    ...
    if (p_Vid->AdaptiveRounding && p_Inp->AdaptRndPeriod &&
        (p_Vid->current_mb_nr % p_Inp->AdaptRndPeriod == 0)) {
      CalculateOffset4x4Param(p_Vid);
      if (p_Inp->Transform8x8Mode) CalculateOffset8x8Param(p_Vid);
    }
    ...
    if (currSlice->UseRDOQuant)  // This needs revisit
      ...
      currSlice->rddata =
          &currSlice->rddata_top_frame_mb;  // store data in top frame MB
    ...
    //       printf ("encode_one_slice: mb %d,  slice %d,   bitbuf bytepos %d
    //       EOS %d\n", p_Vid->current_mb_nr, p_Vid->current_slice_nr,
    //       currSlice->partArr[0].bitstream->byte_pos, end_of_slice);

    if (recode_macroblock ==
        FALSE)  // The final processing of the macroblock has been done
    {
      p_Vid->SumFrameQP += currMB->qp;
      CurrentMbAddr = FmoGetNextMBNr(p_Vid, CurrentMbAddr);
      if (CurrentMbAddr == -1)  // end of slice
      {
        // printf ("FMO End of Slice Group detected, current MBs %d, force end
        // of slice\n", NumberOfCodedMBs+1);
        end_of_slice = TRUE;
      }
      NumberOfCodedMBs++;  // only here we are sure that the coded MB is
                           // actually included in the slice
      ...
      if (p_Vid->current_mb_nr ==
          -1)  // The first MB of the slice group  is too big,
               // which means it's impossible to encode picture using current
               // slice bits restriction
      {
        snprintf(errortext, ET_SIZE,
                 "Error encoding first MB with specified parameter, bits of "
                 "current MB may be too big");
        error(errortext, 300);
      }
    }
  }
  ...
  terminate_slice(
      currMB, (NumberOfCodedMBs + TotalCodedMBs >= (int)p_Vid->PicSizeInMbs),
      cur_stats);
...
************************************************************************
*/
int encode_one_slice_MBAFF(VideoParameters *p_Vid, int SliceGroupId,
                           int TotalCodedMBs) {
  InputParameters *p_Inp = p_Vid->p_Inp;
  Boolean end_of_slice = FALSE;
  ...
  while (end_of_slice == FALSE)  // loop over macroblocks
  {
    ...
    //! This following ugly code breaks slices, at least for a slice mode that
    //! accumulates a certain number of bits into one slice. The suggested
    //! algorithm is as follows:
    //!
    //! SaveState (Bitstream, stats,  etc. etc.);
    //! BitsForThisMBPairInFrameMode = CodeMB (Upper, FRAME_MODE) + CodeMB
    //! (Lower, FRAME_MODE); DistortionForThisMBPairInFrameMode =
    //! CalculateDistortion(Upper) + CalculateDistortion (Lower);
    //! RestoreState();
    //! BitsForThisMBPairInFieldMode = CodeMB (Upper, FIELD_MODE) + CodeMB
    //! (Lower, FIELD_MODE); DistortionForThisMBPairInFrameMode =
    //! CalculateDistortion(Upper) + CalculateDistortion (Lower); FrameFieldMode
    //! = Decision (...) RestoreState() if (FrameFieldMode == FRAME) {
    ...
    //!   1. CABAC/CA-VLC state:  It seems that the CABAC/CA_VLC states are
    //!   changed during the
    //!      dummy encoding processes (for the R-D based selection), but that
    //!      they are never reset, once the selection is made.  I believe that
    //!      this breaks the MB-adaptive frame/field coding.  The necessary code
    //!      for the state saves is readily available in macroblock.c,
    //!      start_macroblock() and end_macroblock() (this code needs to be
    //!      double checked that it works with CA-VLC as well
    //!   2. would it be an option to allocate Bitstreams with zero data in them
    //!   (or copy the
    //!      already generated bitstream) for the "test coding"?

    p_Vid->write_macroblock = FALSE;
    if (p_Inp->MbInterlace == ADAPTIVE_CODING ||
        p_Inp->MbInterlace == FRAME_MB_PAIR_CODING) {
      ...
      if (p_Inp->RCEnable && p_Inp->RCUpdateMode <= MAX_RC_MODE) {
        if (p_Inp->MbInterlace == ADAPTIVE_CODING &&
            ...
            (p_Vid->NumberofCodedMacroBlocks % p_Vid->BasicUnit) == 0)
          rc_copy_quadratic(p_Vid, p_Inp, p_Vid->p_rc_quad_init,
                            p_Vid->p_rc_quad);  // save initial RC status
        if (p_Inp->MbInterlace == ADAPTIVE_CODING)
          rc_copy_generic(p_Vid, p_Vid->p_rc_gen_init,
                          p_Vid->p_rc_gen);  // save initial RC status
      }
      ...
      currSlice->rddata =
          &currSlice->rddata_bot_frame_mb;  // store data in top frame MB
      p_Vid->masterQP = p_Vid->qp;
      currSlice->encode_one_macroblock(currMB);  // code the MB as frame
      end_encode_one_macroblock(currMB);
      ...
      if (p_Inp->RCEnable && p_Inp->RCUpdateMode <= MAX_RC_MODE) {
        if (p_Inp->MbInterlace == ADAPTIVE_CODING &&
            p_Vid->NumberofCodedMacroBlocks > 0 &&
            (p_Vid->NumberofCodedMacroBlocks % p_Vid->BasicUnit) == 0)
          rc_copy_quadratic(p_Vid, p_Inp, p_Vid->p_rc_quad_best,
                            p_Vid->p_rc_quad);  // restore initial RC status

        if (p_Inp->MbInterlace == ADAPTIVE_CODING)
          rc_copy_generic(p_Vid, p_Vid->p_rc_gen_best,
                          p_Vid->p_rc_gen);  // save frame RC stats
      }
    ...
    if ((p_Inp->MbInterlace == ADAPTIVE_CODING) ||
        (p_Inp->MbInterlace == FIELD_CODING)) {
      // Rate control
      p_Vid->bot_MB = FALSE;
      ...
      if (p_Inp->RCEnable && p_Inp->RCUpdateMode <= MAX_RC_MODE) {
        if (p_Inp->MbInterlace == ADAPTIVE_CODING &&
            ...
            (p_Vid->NumberofCodedMacroBlocks % p_Vid->BasicUnit) == 0)
          rc_copy_quadratic(
              p_Vid, p_Inp, p_Vid->p_rc_quad,
              p_Vid->p_rc_quad_init);  // restore initial RC status

        if (p_Inp->MbInterlace == ADAPTIVE_CODING)
          rc_copy_generic(p_Vid, p_Vid->p_rc_gen,
                          p_Vid->p_rc_gen_init);  // reset RC stats
      }

      start_macroblock(currSlice, &currMB, CurrentMbAddr, TRUE);

      currSlice->rddata =
          &currSlice->rddata_top_field_mb;  // store data in top frame MB
      //        TopFieldIsSkipped = 0;        // set the top field MB skipped
      //        flag to 0
      p_Vid->masterQP = p_Vid->qp;
      ...
      currSlice->rddata =
          &currSlice->rddata_bot_field_mb;  // store data in top frame MB
      p_Vid->masterQP = p_Vid->qp;
      currSlice->encode_one_macroblock(currMB);  // code the MB as field
      end_encode_one_macroblock(currMB);
    ...
    if (((p_Inp->MbInterlace == ADAPTIVE_CODING) &&
      ...
      if (p_Inp->RCEnable && p_Inp->RCUpdateMode <= MAX_RC_MODE) {
        if (p_Inp->MbInterlace == ADAPTIVE_CODING &&
          ...
          rc_copy_quadratic(
              p_Vid, p_Inp, p_Vid->p_rc_quad,
              p_Vid->p_rc_quad_best);  // restore initial RC status

        if (p_Inp->MbInterlace == ADAPTIVE_CODING)
          rc_copy_generic(p_Vid, p_Vid->p_rc_gen,
                          p_Vid->p_rc_gen_best);  // restore frame RC stats
      }
    ...
    currSlice->rddata = p_Vid->field_mode ? &currSlice->rddata_top_field_mb
                                          : &currSlice->rddata_top_frame_mb;
    copy_rdopt_data(
        currMB);  // copy the MB data for Top MB from the temp buffers
    write_macroblock(currMB, 1);  // write the Top MB data to the bitstream
    end_macroblock(currMB, &end_of_slice,
                   &recode_macroblock);  // done coding the Top MB
    currMB->prev_recode_mb = recode_macroblock;
        ...
        FALSE)  // The final processing of the macroblock has been done
    {
      ...
      currSlice->rddata = p_Vid->field_mode ? &currSlice->rddata_bot_field_mb
                                            : &currSlice->rddata_bot_frame_mb;
      copy_rdopt_data(
          currMB);  // copy the MB data for Bottom MB from the temp buffers
    ...
    if (p_Vid->MBPairIsField)  // if MB Pair was coded as field the buffer size
                               // variables back to frame mode
    {
      p_Inp->num_ref_frames >>= 1;
      currSlice->num_ref_idx_active[LIST_0] -= 1;
      currSlice->num_ref_idx_active[LIST_0] >>= 1;
    ...
    if (!end_of_slice) {
      ...
      if (CurrentMbAddr == FmoGetLastCodedMBOfSliceGroup(
                               p_Vid, FmoMB2SliceGroup(p_Vid, CurrentMbAddr)))
        end_of_slice =
            TRUE;  // just in case it doesn't get set in end_macroblock
    }
  }
...
static void setup_cabac(Slice *currSlice, char *listXsize) {
  ...
  if (currSlice->slice_type == I_SLICE) {
    ...
    for (i = 0; i < 6; i++) {
      switch (listXsize[i]) {
        case 0: currSlice->writeRefFrame[i] = NULL; break;
        case 1: currSlice->writeRefFrame[i] = writeRefPic_Dummy; break;
        default: currSlice->writeRefFrame[i] = writeRefPic_B_CABAC;
      }
    }
  } else {
    ...
    for (i = 0; i < 6; i++) {
      switch (listXsize[i]) {
        case 0: currSlice->writeRefFrame[i] = NULL; break;
        case 1: currSlice->writeRefFrame[i] = writeRefPic_Dummy; break;
        default: currSlice->writeRefFrame[i] = writeRefPic_P_CABAC;
      }
    }
  }
  ...
  if (active_sps->chroma_format_idc == YUV444)
    currSlice->write_and_store_CBP_block_bit =
        write_and_store_CBP_block_bit_444;
  else
...
static void setup_cavlc(Slice *currSlice, char *listXsize) {
  ...
  for (i = 0; i < 6; i++) {
    switch (listXsize[i]) {
      case 0: currSlice->writeRefFrame[i] = NULL; break;
      case 1: currSlice->writeRefFrame[i] = writeRefPic_Dummy; break;
      case 2: currSlice->writeRefFrame[i] = writeRefPic_2Ref_CAVLC; break;
      default: currSlice->writeRefFrame[i] = writeRefPic_NRef_CAVLC; break;
    }
  }
...
static void setup_slice(Slice *currSlice) {
  ...
  if (currSlice->slice_type == P_SLICE || currSlice->slice_type == SP_SLICE) {
    currSlice->weighted_prediction =
        (byte)currSlice->p_Vid->active_pps->weighted_pred_flag;
    currSlice->set_modes_and_refs_for_blocks =
        set_modes_and_refs_for_blocks_p_slice;
    currSlice->set_coeff_and_recon_8x8 = set_coeff_and_recon_8x8_p_slice;
    currSlice->init_lists = init_lists_p_slice;
    currSlice->submacroblock_mode_decision =
        submacroblock_mode_decision_p_slice;
    currSlice->set_motion_vectors_mb = SetMotionVectorsMBPSlice;
  } else if (currSlice->slice_type == B_SLICE) {
    currSlice->weighted_prediction =
        (byte)currSlice->p_Vid->active_pps->weighted_bipred_idc;
    currSlice->set_modes_and_refs_for_blocks =
        set_modes_and_refs_for_blocks_b_slice;
    currSlice->set_coeff_and_recon_8x8 = set_coeff_and_recon_8x8_b_slice;
    currSlice->init_lists = init_lists_b_slice;
    currSlice->submacroblock_mode_decision =
        submacroblock_mode_decision_b_slice;
    currSlice->set_motion_vectors_mb = SetMotionVectorsMBBSlice;
  } else {
    currSlice->set_modes_and_refs_for_blocks =
        set_modes_and_refs_for_blocks_i_slice;
  ...
  if (currSlice->mb_aff_frame_flag) {
    ...
    currSlice->rdo_low_intra_chroma_decision =
        rdo_low_intra_chroma_decision_mbaff;
...
static void ClipWPParams(Slice *currSlice) {
  // Keeping this disabled since it seems that all WP functions already do
  // clipping internally Otherwise, this should be modified to support different
  // bitdepths and to also properly clip wbp_weights.
  /*
  VideoParameters *p_Vid = currSlice->p_Vid;
  int list, ref, comp;
  int bitdepth[3] = { (p_Vid->bitdepth_luma - 8), (p_Vid->bitdepth_chroma - 8),
  (p_Vid->bitdepth_chroma - 8)};

  for(list = 0; list <= (currSlice->slice_type == B_SLICE ? LIST_1:LIST_0);
  list++)
  {
    for(ref = 0; ref < currSlice->listXsize[list]; ref++)
    {
      ...
      {
        //currSlice->wp_weight[list][ref][comp] = sClip3(-128 , 127,
  currSlice->wp_weight[list][ref][comp]);
        //currSlice->wp_offset[list][ref][comp] = sClip3(-128, 127,
  currSlice->wp_offset[list][ref][comp]);
      }
    }
  }
  */
...
void setup_open_gop_lists(VideoParameters *p_Vid, Slice *currSlice) {
  ...
  if (currSlice->slice_type != I_SLICE && currSlice->slice_type != SI_SLICE) {
    for (i = 0; i < currSlice->listXsize[0]; i++) {
      if (currSlice->listX[LIST_0][i]->poc < p_Vid->last_valid_reference &&
          p_Vid->ThisPOC >= p_Vid->last_valid_reference) {
    ...
    for (i = 0; i < currSlice->listXsize[LIST_1]; i++) {
      if (currSlice->listX[LIST_1][i]->poc < p_Vid->last_valid_reference &&
          p_Vid->ThisPOC >= p_Vid->last_valid_reference) {
        // remove invalid reference from list
    ...
    // Below code is not the best but avoids problem with the creation of an
    // empty list. For MVC we could be doing more intelligent tricks with the
    // inter-layer reference.
    if (currSlice->slice_type == B_SLICE) {
      if (currSlice->listXsize[LIST_1] == 0 &&
          currSlice->listXsize[LIST_0] != 0) {
        ...
        currSlice->listXsize[LIST_1]++;
      } else if (currSlice->listXsize[LIST_0] == 0 &&
                 currSlice->listXsize[LIST_1] != 0) {
...
void set_slice(VideoParameters *p_Vid, Slice *currSlice) {
  InputParameters *p_Inp = p_Vid->p_Inp;
  // Using this trick we can basically reference back to the image and input
  ...
  currSlice->si_frame_indicator =
      currSlice->slice_type == SI_SLICE ? TRUE : FALSE;
  ...
  currSlice->num_ref_idx_active[LIST_0] =
      (char)(p_Vid->active_pps->num_ref_idx_l0_default_active_minus1 + 1);
  currSlice->num_ref_idx_active[LIST_1] =
      (char)(p_Vid->active_pps->num_ref_idx_l1_default_active_minus1 + 1);
...
void init_slice(VideoParameters *p_Vid, Slice **currSlice, int start_mb_addr) {
  ...
  if (p_Inp->num_of_views == 2) {
    int enable_inter_view =
        (p_Vid->nal_reference_idc || p_Inp->enable_inter_view_flag) ? 1 : 0;
    p_Vid->priority_id = 0;
    p_Vid->temporal_id = 0;
    ...
    if (p_Vid->structure != BOTTOM_FIELD)
      p_Vid->anchor_pic_flag[0] = (((p_Vid->currentPicture->idr_flag &&
                                     p_Vid->nal_reference_idc != 0) ||
                                    p_Vid->prev_view_is_anchor)
                                       ? 1
                                       : 0);
    else                              // bottom will follow top for now
      p_Vid->anchor_pic_flag[1] = 0;  // p_Vid->anchor_pic_flag[0];
    p_Vid->non_idr_flag[p_Vid->structure != BOTTOM_FIELD ? 0 : 1] =
        !p_Vid->anchor_pic_flag[p_Vid->structure != BOTTOM_FIELD ? 0 : 1];
  }
#endif
  ...
  *currSlice = currPic->slices[currPic->no_slices - 1];
  ...
  if (p_Inp->redundant_pic_flag) {
    if (!p_Vid->redundant_coding) {
      (*currSlice)->num_ref_idx_active[LIST_0] =
          (char)imin(p_Vid->number, p_Inp->NumRefPrimary);
    } else {
  ...
  // code now also considers fields. Issue whether we should account this within
  // the appropriate input p_Inp directly
  if (((*currSlice)->slice_type == P_SLICE ||
       (*currSlice)->slice_type == SP_SLICE) &&
      p_Inp->P_List0_refs[layer_id]) {
    (*currSlice)->num_ref_idx_active[LIST_0] = (char)imin(
        (*currSlice)->num_ref_idx_active[LIST_0],
        p_Inp->P_List0_refs[layer_id] * ((p_Vid->structure != 0) + 1));
#if CRA
    if (p_Inp->useCRA) {
      if ((((*currSlice)->ThisPOC / 2) - (p_Vid->p_Inp->NumberBFrames + 1)) %
              p_Vid->p_Inp->intra_period ==
          0) {
  ...
  if ((*currSlice)->slice_type == B_SLICE) {
    if (p_Inp->B_List0_refs[layer_id]) {
      ...
      if (p_Inp->BLevel0MoreRef) {
        if (((*currSlice)->ThisPOC / 2) % (p_Vid->p_Inp->NumberBFrames + 1) ==
          ...
          (*currSlice)->num_ref_idx_active[LIST_0] = (char)imin(
              (*currSlice)->num_ref_idx_active[LIST_0],
              p_Inp->B_List0_refs[layer_id] * ((p_Vid->structure != 0) + 1));
        }
      } else
#endif
        (*currSlice)->num_ref_idx_active[LIST_0] = (char)imin(
    ...
    if (p_Inp->B_List1_refs[layer_id]) {
      (*currSlice)->num_ref_idx_active[LIST_1] = (char)imin(
          (*currSlice)->num_ref_idx_active[LIST_1],
          p_Inp->B_List1_refs[layer_id] * ((p_Vid->structure != 0) + 1));
#if B0_MORE_REF
      if (p_Inp->BLevel0MoreRef) {
        if (((*currSlice)->ThisPOC / 2) % (p_Vid->p_Inp->NumberBFrames + 1) ==
            0) {
    ...
    get_mem3D((byte ****)(void *)&(*currSlice)->direct_ref_idx,
              (*currSlice)->height_blk, (*currSlice)->width_blk, 2);
    get_mem2D((byte ***)(void *)&(*currSlice)->direct_pdir,
              (*currSlice)->height_blk, (*currSlice)->width_blk);
  }
  ...
  if (p_Vid->is_hme == 0) {
    ...
    if (p_Inp->HM50RefStructure) {
      if (p_Vid->structure == FRAME &&
          p_Dpb->ref_frames_in_buffer == active_sps->num_ref_frames)
        hm50_ref_management_frame_pic(p_Dpb, p_Vid->frame_num);
    }
#endif

#if LD_REF_SETTING
    if (p_Inp->HMEDisableMMCO == 0 && p_Inp->LDRefSetting &&
        p_Inp->UnconstrainedLDRef == 0) {
      if (p_Vid->structure == FRAME &&
          p_Dpb->ref_frames_in_buffer == active_sps->num_ref_frames)
    ...
    init_ref_pic_list_reordering(*currSlice, p_Inp->ReferenceReorder,
                                 p_Inp->UseDistortionReorder);
    alloc_ref_pic_list_reordering_buffer(*currSlice);

#if (MVC_EXTENSION_ENABLE)
    p_Vid->MVCInterViewReorder = 0;

    if (p_Vid->num_of_layers == 2 && p_Inp->MVCInterViewReorder == 1 &&
        layer_id == 1) {
      ...
      if ((*currSlice)->listXsize[0] != 0) {
        StorablePicture *Inter_ViewL0 =
            (*currSlice)->listX[0][(*currSlice)->listXsize[0] - 1];
        // StorablePicture *Inter_ViewL0 =
        // (*currSlice)->p_Dpb->fs_ilref[0]->frame; printf("value %d %d %d \n",
        // (*currSlice)->listXsize[0], (*currSlice)->p_Dpb->fs_ilref[0]->frame,
        // (*currSlice)->p_Dpb->fs_ilref[0]->frame);
      ...
      if (p_Inp->redundant_pic_flag && p_Vid->redundant_coding) {
        ...
        (*currSlice)->abs_diff_pic_num_minus1[LIST_0][0] =
            p_Vid->redundant_ref_idx - 1;
    ...
    if (p_Inp->EnableOpenGOP) setup_open_gop_lists(p_Vid, *currSlice);
  ...
  if (((*currSlice)->slice_type != I_SLICE) &&
    ...
    alloc_size += get_mem5Dmv(&((*currSlice)->all_mv), 2,
                              (*currSlice)->max_num_references, 9, 4, 4);

    if (p_Inp->BiPredMotionEstimation &&
        ((*currSlice)->slice_type == B_SLICE)) {
      alloc_size += get_mem6Dmv(&((*currSlice)->bipred_mv), 2, 2,
                                (*currSlice)->max_num_references, 9, 4, 4);
    }

    if (p_Inp->UseRDOQuant && p_Inp->RDOQ_QP_Num > 1) {
      if (p_Inp->Transform8x8Mode && p_Inp->RDOQ_CP_MV) {
        get_mem4Dmv(&(*currSlice)->tmp_mv8, 2, (*currSlice)->max_num_references,
                    4, 4);
        get_mem3Ddistblk(&(*currSlice)->motion_cost8, 2,
                         (*currSlice)->max_num_references, 4);
        get_mem4Dmv(&(*currSlice)->tmp_mv4, 2, (*currSlice)->max_num_references,
                    4, 4);
        get_mem3Ddistblk(&(*currSlice)->motion_cost4, 2,
                         (*currSlice)->max_num_references, 4);
      }
    }
  }

  if (p_Vid->mb_aff_frame_flag) init_mbaff_lists(*currSlice);
  ...
  if (((*currSlice)->slice_type != I_SLICE &&
    ...
    if ((*currSlice)->slice_type == P_SLICE ||
        (*currSlice)->slice_type == SP_SLICE) {
      int wp_type = (p_Inp->GenerateMultiplePPS && p_Inp->RDPictureDecision) &&
  ...
  if ((*currSlice)->slice_type != I_SLICE &&
    ...
    if (p_Inp->SearchMode[layer_id] == EPZS) {
      if (((*currSlice)->p_EPZS =
               (EPZSParameters *)calloc(1, sizeof(EPZSParameters))) == NULL)
  ...
  // assign luma common reference picture pointers to be used for ME/sub-pel
  // interpolation

  for (i = 0; i < active_ref_lists; i++) {
    for (j = 0; j < (*currSlice)->listXsize[i]; j++) {
      if ((*currSlice)->listX[i][j]) {
        (*currSlice)->listX[i][j]->p_curr_img =
            (*currSlice)->listX[i][j]->p_img[(short)p_Vid->colour_plane_id];
        (*currSlice)->listX[i][j]->p_curr_img_sub =
            (*currSlice)->listX[i][j]->p_img_sub[(short)p_Vid->colour_plane_id];
      }
    }
  }

  if (p_Inp->UseRDOQuant) {
    if (((*currSlice)->estBitsCabac = (estBitsCabacStruct *)calloc(
             NUM_BLOCK_TYPES, sizeof(estBitsCabacStruct))) == NULL)
  ...
  get_mem3Dpel(&((*currSlice)->mb_pred), MAX_PLANE, MB_BLOCK_SIZE,
               MB_BLOCK_SIZE);
  get_mem3Dint(&((*currSlice)->mb_rres), MAX_PLANE, MB_BLOCK_SIZE,
               MB_BLOCK_SIZE);
  get_mem3Dint(&((*currSlice)->mb_ores), MAX_PLANE, MB_BLOCK_SIZE,
               MB_BLOCK_SIZE);
  get_mem4Dpel(&((*currSlice)->mpr_4x4), MAX_PLANE, 9, MB_BLOCK_SIZE,
               MB_BLOCK_SIZE);
  get_mem4Dpel(&((*currSlice)->mpr_8x8), MAX_PLANE, 9, MB_BLOCK_SIZE,
               MB_BLOCK_SIZE);
  get_mem4Dpel(&((*currSlice)->mpr_16x16), MAX_PLANE, 5, MB_BLOCK_SIZE,
               MB_BLOCK_SIZE);
 ...
 ************************************************************************
 */
void init_slice_lite(VideoParameters *p_Vid, Slice **currSlice,
                     int start_mb_addr) {
  InputParameters *p_Inp = p_Vid->p_Inp;
  int i, j;
#if (MVC_EXTENSION_ENABLE)
  ...
  if (p_Vid->num_of_layers == 2 && p_Inp->MVCInterViewReorder == 1 &&
    ...
    if ((*currSlice)->listXsize[0] != 0) {
      ...
      for (i = (*currSlice)->listXsize[0] - 1; i > 0; i--)
        (*currSlice)->listX[0][i] = (*currSlice)->listX[0][i - 1];
      (*currSlice)->listX[0][0] = Inter_ViewL0;
      (*currSlice)->listXsize[0] = (char)imin(
...
static Slice *malloc_slice(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  switch (p_Inp->slice_mode) {
    case 2:
      // buffer_size = imax(2 * p_Inp->slice_argument, 500 + (128 + 256 *
      // p_Vid->bitdepth_luma + 512 * p_Vid->bitdepth_chroma));
      buffer_size = imax(2 * p_Inp->slice_argument, 764);
      break;
    case 1:
      buffer_size =
          500 + p_Inp->slice_argument * ((128 + 256 * p_Vid->bitdepth_luma +
                                          cr_size * p_Vid->bitdepth_chroma) >>
                                         3);
      break;
    default:
      buffer_size =
          500 + p_Vid->FrameSizeInMbs * ((128 + 256 * p_Vid->bitdepth_luma +
                                          cr_size * p_Vid->bitdepth_chroma) >>
                                         3);
      break;
  }
  ...
  // KS: this is approx. max. allowed code picture size
  if ((currSlice = (Slice *)calloc(1, sizeof(Slice))) == NULL)
    no_mem_exit("malloc_slice: currSlice structure");

  currSlice->p_Vid = p_Vid;
  currSlice->p_Inp = p_Inp;

  if (((currSlice->p_RDO) =
           (RDOPTStructure *)calloc(1, sizeof(RDOPTStructure))) == NULL)
  ...
  if (p_Vid->currentPicture->idr_flag) currSlice->max_part_nr = 1;
  ...
  if ((currSlice->partArr = (DataPartition *)calloc(
           currSlice->max_part_nr, sizeof(DataPartition))) == NULL)
    no_mem_exit("malloc_slice: partArr");
  for (i = 0; i < currSlice->max_part_nr; i++)  // loop over all data partitions
  {
    dataPart = &(currSlice->partArr[i]);
    if ((dataPart->bitstream = (Bitstream *)calloc(1, sizeof(Bitstream))) ==
        NULL)
      no_mem_exit("malloc_slice: Bitstream");
    if ((dataPart->bitstream->streamBuffer =
             (byte *)calloc(buffer_size, sizeof(byte))) == NULL)
  ...
  if (p_Inp->WeightedPrediction || p_Inp->WeightedBiprediction ||
      p_Inp->GenerateMultiplePPS) {
    // Currently only use up to 32 references. Need to use different indicator
    // such as maximum num of references in list
    ...
    get_mem4Dshort(&currSlice->wbp_weight, 6, MAX_REFERENCE_PICTURES,
 ...
 ************************************************************************
 * \brief
 *    Allocates a light-weight slice structure along with its dependent data
 *structures \return Pointer to a Slice
 ************************************************************************
 */
static Slice *malloc_slice_lite(VideoParameters *p_Vid,
...
void free_slice(Slice *currSlice) {
  if (currSlice != NULL) {
    ...
    for (i = 0; i < currSlice->max_part_nr;
         i++)  // loop over all data partitions
    {
    ...
    if (currSlice->cofAC) free_mem_ACcoeff(currSlice->cofAC);
    if (currSlice->cofDC) free_mem_DCcoeff(currSlice->cofDC);

    if (currSlice->mb_rres) free_mem3Dint(currSlice->mb_rres);
    if (currSlice->mb_ores) free_mem3Dint(currSlice->mb_ores);
    if (currSlice->mb_pred) free_mem3Dpel(currSlice->mb_pred);
    if (currSlice->mpr_16x16) free_mem4Dpel(currSlice->mpr_16x16);
    if (currSlice->mpr_8x8) free_mem4Dpel(currSlice->mpr_8x8);
    if (currSlice->mpr_4x4) free_mem4Dpel(currSlice->mpr_4x4);
    ...
    if (currSlice->symbol_mode == CABAC) {
      if (currSlice->mot_ctx) delete_contexts_MotionInfo(currSlice->mot_ctx);
      if (currSlice->tex_ctx) delete_contexts_TextureInfo(currSlice->tex_ctx);
    }
    ...
    if ((currSlice->slice_type == P_SLICE) ||
        (currSlice->slice_type == SP_SLICE) ||
        (currSlice->slice_type == B_SLICE)) {
      if (currSlice->all_mv) free_mem5Dmv(currSlice->all_mv);
      ...
      if (currSlice->UseRDOQuant && currSlice->RDOQ_QP_Num > 1) {
        if (p_Inp->Transform8x8Mode && p_Inp->RDOQ_CP_MV) {
          if (currSlice->tmp_mv8) free_mem4Dmv(currSlice->tmp_mv8);
          if (currSlice->motion_cost8)
            free_mem3Ddistblk(currSlice->motion_cost8);
          if (currSlice->tmp_mv4) free_mem4Dmv(currSlice->tmp_mv4);
    ...
    if (currSlice->slice_type == B_SLICE) {
      ...
      if (currSlice->direct_pdir) free_mem2D((byte **)currSlice->direct_pdir);
    }
    ...
    if (currSlice->slice_type != I_SLICE && currSlice->slice_type != SI_SLICE) {
      if (p_Inp->SearchMode[p_Vid->view_id] == EPZS) {
        if (currSlice->p_EPZS) EPZSStructDelete(currSlice);
      }
    }
    ...
    switch (p_Vid->yuv_format) {
      case YUV420:
        for (qp = -currSlice->bitdepth_luma_qp_scale; qp < 52; qp++) {
          for (k = 0; k < 3; k++) {
            if ((p_Inp->MEErrorMetric[k] == ERROR_SAD) &&
                (p_Inp->ChromaMEEnable)) {
              p_Vid->lambda_mf[j][qp][k] =
                  (3 * p_Vid->lambda_mf[j][qp][k] + 1) >> 1;
...
void SetLagrangianMultipliersOn(Slice *currSlice) {
  ...
  if (p_Inp->UseExplicitLambdaParams ==
      1)  // consideration of explicit lambda weights.
    get_explicit_lambda(currSlice);
  else if (p_Inp->UseExplicitLambdaParams ==
           2)  // consideration of fixed lambda values.
    get_fixed_lambda(currSlice);
  else {
    switch (currSlice->slice_type) {
      case I_SLICE: get_implicit_lambda_i_slice(currSlice); break;
      default:
      case P_SLICE: get_implicit_lambda_p_slice(currSlice); break;
      case B_SLICE: get_implicit_lambda_b_slice(currSlice); break;
      case SI_SLICE:
      case SP_SLICE: get_implicit_lambda_sp_slice(currSlice); break;
    }
  }
  ...
  for (j = 0; j < 6; j++) {
    for (qp = -currSlice->bitdepth_luma_qp_scale; qp < 52; qp++) {
      ...
      switch (p_Inp->UseExplicitLambdaParams) {
        case 1:  // explicit lambda weights
          p_Vid->lambda_md[j][qp] =
              sqrt(p_Inp->LambdaWeight[j] * pow(2, qp_temp / 3.0));
          break;
      ...
      for (k = F_PEL; k <= Q_PEL; k++) {
        p_Vid->lambda_me[j][qp][k] =
            (p_Inp->MEErrorMetric[k] == ERROR_SSE &&
             p_Inp->MESoftenSSEMetric == 0)
                ? (p_Vid->lambda_md[j][qp] * p_Vid->lambda_md[j][qp])
                : p_Vid->lambda_md[j][qp];
      ...
      if (p_Inp->CtxAdptLagrangeMult == 1) {
        int lambda_qp =
            (qp >= 32 && !p_Inp->RCEnable) ? imax(0, qp - 4) : imax(0, qp - 6);
        p_Vid->lambda_mf_factor[j][qp] =
            log(p_Vid->lambda_me[j][lambda_qp][Q_PEL] + 1.0) / log(2.0);
      }
    }
  }
  UpdateMELambda(currSlice);
