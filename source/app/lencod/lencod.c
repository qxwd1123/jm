 ...
 ***********************************************************************
 *  \mainpage
 *     This is the H.264/AVC encoder reference software. For detailed
...
static void alloc_video_params(VideoParameters **p_Vid) {
  ...
  if ((((*p_Vid)->p_Dist) =
           (DistortionParams *)calloc(1, sizeof(DistortionParams))) == NULL)
    no_mem_exit("alloc_video_params: p_Dist");
  if ((((*p_Vid)->p_Stats) =
           (StatParameters *)calloc(1, sizeof(StatParameters))) == NULL)
    no_mem_exit("alloc_video_params: p_Stats");
  if (((*p_Vid)->p_Dpb_layer[0] = (DecodedPictureBuffer *)calloc(
           MAX_NUM_DPB_LAYERS, sizeof(DecodedPictureBuffer))) == NULL)
  ...
  if ((((*p_Vid)->p_Quant) =
           (QuantParameters *)calloc(1, sizeof(QuantParameters))) == NULL)
    no_mem_exit("alloc_video_params: p_Quant");
  if ((((*p_Vid)->p_QScale) =
           (ScaleParameters *)calloc(1, sizeof(ScaleParameters))) == NULL)
    no_mem_exit("alloc_video_params: p_QScale");
  if ((((*p_Vid)->p_SEI) = (SEIParameters *)calloc(1, sizeof(SEIParameters))) ==
      NULL)
 ...
 ***********************************************************************
 */
static void free_encoder(EncoderParams *p_Enc) { free_pointer(p_Enc); }
...
void setup_dpb_layer(DecodedPictureBuffer *p_Dpb, VideoParameters *p_Vid,
  ...
  switch (p_Inp->OnTheFlyFractMCP) {
    case OTF_L1:
      ...
      p_Dpb->pf_get_block_chroma[OTF_ME] = (p_Vid->P444_joined)
                                               ? (get_block_luma_otf_L1)
                                               : (me_get_block_chroma_otf_L1);
      p_Dpb->pf_get_block_chroma[OTF_MC] = (p_Vid->P444_joined)
                                               ? (get_block_luma_otf_L1)
                                               : (mc_get_block_chroma_otf_L1);
      p_Dpb->pf_OneComponentChromaPrediction4x4_regenerate =
          OneComponentChromaPrediction4x4_regenerate;
      p_Dpb->pf_OneComponentChromaPrediction4x4_retrieve =
          OneComponentChromaPrediction4x4_regenerate;
      break;
    case OTF_L2:
      p_Dpb->pf_computeSAD = computeSAD_otf;
      ...
      p_Dpb->pf_get_block_chroma[OTF_ME] = p_Dpb->pf_get_block_chroma[OTF_MC] =
          (p_Vid->P444_joined) ? (get_block_luma_otf_L2)
                               : (get_block_chroma_otf_L2);
      p_Dpb->pf_OneComponentChromaPrediction4x4_regenerate =
          OneComponentChromaPrediction4x4_regenerate;
      p_Dpb->pf_OneComponentChromaPrediction4x4_retrieve =
          OneComponentChromaPrediction4x4_regenerate;
...
static void set_storage_format(VideoParameters *p_Vid, FrameFormat *p_src,
                               FrameFormat *p_dst) {
  ...
  *p_dst = *p_src;
  ...
  p_dst->width[1] = p_dst->width[2] =
      p_dst->width[0] * mb_width_cr[p_dst->yuv_format] / 16;
  p_dst->height[1] = p_dst->height[2] =
      p_dst->height[0] * mb_height_cr[p_dst->yuv_format] / 16;
...
static void init_encoder(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_Inp->Log2MaxFNumMinus4 == -1) {
    p_Vid->log2_max_frame_num_minus4 = iClip3(
        0, 12, (int)(CeilLog2(p_Inp->no_frames) - 4));  // hack for now...
  } else
    p_Vid->log2_max_frame_num_minus4 = p_Inp->Log2MaxFNumMinus4;

  if (p_Vid->log2_max_frame_num_minus4 == 0 && p_Inp->num_ref_frames == 16) {
    snprintf(errortext, ET_SIZE,
             " NumberReferenceFrames=%d and Log2MaxFNumMinus4=%d may lead to "
             "an invalid value of frame_num.",
             p_Inp->num_ref_frames, p_Inp->Log2MaxFNumMinus4);
    error(errortext, 500);
  }

  // set proper p_Vid->log2_max_pic_order_cnt_lsb_minus4.
  if (p_Inp->Log2MaxPOCLsbMinus4 == -1)
    p_Vid->log2_max_pic_order_cnt_lsb_minus4 = iClip3(
        0, 12,
        (int)(CeilLog2(imax(p_Inp->no_frames, (p_Inp->NumberBFrames + 1) << 1)
                       << 1) -
              4));  // hack for now
  else
    p_Vid->log2_max_pic_order_cnt_lsb_minus4 = p_Inp->Log2MaxPOCLsbMinus4;

  if (((1 << (p_Vid->log2_max_pic_order_cnt_lsb_minus4 + 3)) <
       p_Inp->jumpd * 4) &&
      p_Inp->Log2MaxPOCLsbMinus4 != -1)
    error(
        "log2_max_pic_order_cnt_lsb_minus4 might not be sufficient for "
        "encoding. Increase value.",
        400);

  if (strlen(p_Inp->ReconFile) > 0 &&
      (p_Vid->p_dec =
           open(p_Inp->ReconFile, OPENFLAGS_WRITE, OPEN_PERMISSIONS)) == -1) {
  ...
  if (p_Vid->num_of_layers == 2) {
    if (strlen(p_Inp->ReconFile2) > 0 &&
        (p_Vid->p_dec2 = open(p_Inp->ReconFile2, OPENFLAGS_WRITE,
                              OPEN_PERMISSIONS)) == -1) {
  ...
  if (p_Vid->auto_crop_bottom || p_Vid->auto_crop_right) {
    fprintf(stderr,
            "Warning: Automatic cropping activated: Coded frame Size: %dx%d\n",
  ...
  if (p_Inp->num_slice_groups_minus1 > 0) {
    if ((p_Inp->slice_group_map_type >= 3) &&
        (p_Inp->slice_group_map_type <= 5))
      p_Inp->num_slice_groups_minus1 = 1;
  }

  if (p_Inp->RCEnable) {
    if (p_Inp->basicunit == 0)
      p_Inp->basicunit = (p_Inp->output.height[0] + p_Vid->auto_crop_bottom) *
                         (p_Inp->output.width[0] + p_Vid->auto_crop_right) /
                         256;

    if (((p_Inp->output.height[0] + p_Vid->auto_crop_bottom) *
         (p_Inp->output.width[0] + p_Vid->auto_crop_right) / 256) %
            p_Inp->basicunit !=
        0) {
      snprintf(errortext, ET_SIZE,
               "Frame size in macroblocks must be a multiple of BasicUnit.");
      error(errortext, 500);
    }
  }
  ...
  set_storage_format(p_Vid, &(p_Inp->source),
                     &(p_Vid->p_Dpb_layer[0]->storage_format));
  if (p_Vid->num_of_layers == 2) {
    {
      p_Vid->output_format[1] = p_Inp->output;
      set_storage_format(p_Vid, &(p_Inp->source),
                         &(p_Vid->p_Dpb_layer[1]->storage_format));
    }
  }
  // end;
  ...
  if (p_Vid->num_of_layers == 2)
    p_Vid->Bit_Buffer =
        (long *)malloc(((p_Inp->no_frames << 1) + 1) * sizeof(long));
  else
  ...
  if (p_Vid->num_of_layers == 2) {
    ...
    memset(p_Vid->p_Dist->metric_v[0], 0,
           TOTAL_DIST_TYPES * sizeof(DistMetric));
    memset(p_Vid->p_Dist->metric_v[1], 0,
           TOTAL_DIST_TYPES * sizeof(DistMetric));
  }
#endif
  ...
  if (p_Inp->WPMCPrecision) wpxInitWPXPasses(p_Vid, p_Inp);

  init_motion_search_module(p_Vid, p_Inp);
  information_init(p_Vid, p_Inp, p_Vid->p_Stats);

  if (p_Inp->DistortionYUVtoRGB) init_YUVtoRGB(p_Vid, p_Inp);

  // Rate control
  if (p_Inp->RCEnable) rc_init_sequence(p_Vid, p_Inp);
  ...
  if (p_Inp->UseRDOQuant) precalculate_unary_exp_golomb_level(p_Vid);

  if (p_Inp->ExplicitSeqCoding) OpenExplicitSeqFile(p_Vid, p_Inp);
...
void setup_coding_layer(VideoParameters *p_Vid) {
  ...
  if (p_Inp->ChromaMCBuffer)
    p_Vid->OneComponentChromaPrediction4x4 =
        p_Dpb->pf_OneComponentChromaPrediction4x4_retrieve;
  else
    p_Vid->OneComponentChromaPrediction4x4 =
        p_Dpb->pf_OneComponentChromaPrediction4x4_regenerate;
...
************************************************************************
*/
static void prepare_frame_params(VideoParameters *p_Vid, InputParameters *p_Inp,
  ...
  if (p_Inp->ExplicitSeqCoding) {
    ExpFrameInfo *info =
        &p_Vid->expSeq->info[curr_frame_to_code % p_Vid->expSeq->no_frames];
    ReadExplicitSeqFile(p_Vid->expSeq, p_Vid->expSFile, curr_frame_to_code);
    // override and overwrite whatever was in p_cur_frm
    populate_frame_explicit(info, p_Inp, p_cur_frm,
                            p_seq_struct->max_num_slices);
  }

  // populate coding parameters for the current frame (frame_no, slice_type,
  // nal_ref_idc, poc, reset frame_num)
  ...
  if (p_cur_frm->type == SI_SLICE) {
    ...
    set_slice_type(
        p_Vid, p_Inp,
        p_cur_frm->type);  // dominant slice type; perhaps it is better to use
                           // the type of the first slice?
  }
  p_Vid->nal_reference_idc = p_cur_frm->nal_ref_idc;
  switch (p_Vid->pic_order_cnt_type) {
    case 0: get_poc_type_zero(p_Vid, p_Inp, p_cur_frm); break;
    case 1: get_poc_type_one(p_Vid, p_Inp, p_cur_frm); break;
    default:
    case 2: get_poc_type_zero(p_Vid, p_Inp, p_cur_frm); break;
  }
  ...
  p_Vid->layer = ((p_Vid->curr_frm_idx - p_Vid->last_idr_code_order) %
...
static void encode_sequence(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  for (curr_frame_to_code = 0; curr_frame_to_code < frames_to_code;
       curr_frame_to_code++) {
#if (MVC_EXTENSION_ENABLE)
    if (p_Inp->num_of_views == 2) {
      if ((curr_frame_to_code & 1) == 0)  // call only for view_id 0
      {
        // determine whether to populate additional frames in the prediction
        // structure
        if ((curr_frame_to_code >> 1) >= p_Vid->p_pred->pop_start_frame) {
          int start = p_seq_struct->pop_start_frame, end;

          populate_frm_struct(p_Vid, p_Inp, p_seq_struct,
      ...
      p_Vid->p_curr_frm_struct =
          p_frm + (p_Vid->curr_frm_idx %
      ...
      if (p_Vid->view_id == 1) {
        ...
        p_Vid->p_curr_frm_struct->qp = p_Vid->qp =
            iClip3(-p_Vid->bitdepth_luma_qp_scale, MAX_QP,
                   p_Vid->AverageFrameQP + p_Inp->View1QPOffset);
      } else {
...
#endif
    {
      ...
      if (curr_frame_to_code >= p_Vid->p_pred->pop_start_frame) {
        populate_frm_struct(p_Vid, p_Inp, p_seq_struct,
                            p_Inp->FrmStructBufferLength, frames_to_code);
      }
      p_Vid->curr_frm_idx = curr_frame_to_code;
      p_Vid->p_curr_frm_struct =
          p_frm + (p_Vid->curr_frm_idx %
    ...
    if (p_Inp->EnableOpenGOP && p_Vid->p_curr_frm_struct->random_access) {
      if (p_Inp->PicInterlace) {
        if (p_Vid->p_curr_frm_struct->p_top_fld_pic->p_Slice[0].type ==
                I_SLICE &&
            p_Vid->p_curr_frm_struct
                ->random_access)  // Currently encoder always codes top field as
                                  // I
        {
...
void free_encoder_memory(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (-1 != p_Vid->p_dec) close(p_Vid->p_dec);

#if (MVC_EXTENSION_ENABLE)
  if (p_Vid->num_of_layers == 2) CloseFiles(&p_Inp->input_file2);
  if (-1 != p_Vid->p_dec2) close(p_Vid->p_dec2);
#endif

  if (p_Enc->p_trace) fclose(p_Enc->p_trace);
  ...
  if (p_Inp->ExplicitSeqCoding) CloseExplicitSeqFile(p_Vid);
...
static void init_img(VideoParameters *p_Vid) {
  ...
  p_Vid->P444_joined =
      (p_Vid->yuv_format == YUV444 && (p_Inp->separate_colour_plane_flag == 0));
  ...
  p_Vid->max_imgpel_value_comp_sq[0] =
      p_Vid->max_pel_value_comp[0] * p_Vid->max_pel_value_comp[0];
  ...
  if (p_Vid->active_sps->profile_idc == BASELINE ||
      p_Vid->active_sps->profile_idc == MAIN ||
      p_Vid->active_sps->profile_idc == EXTENDED)
    p_Vid->min_IPCM_value = 1;  // See Annex A for restriction in pcm sample
                                // values for pre FRExt profiles
  else
  ...
  if (p_Vid->yuv_format != YUV400) {
    ...
    p_Vid->max_pel_value_comp[2] = p_Vid->max_pel_value_comp[1];
    p_Vid->max_imgpel_value_comp_sq[1] =
        p_Vid->max_pel_value_comp[1] * p_Vid->max_pel_value_comp[1];
    p_Vid->max_imgpel_value_comp_sq[2] =
        p_Vid->max_pel_value_comp[2] * p_Vid->max_pel_value_comp[2];
    p_Vid->num_blk8x8_uv = (1 << p_Vid->yuv_format) & (~(0x1));
    p_Vid->num_cdc_coeff = p_Vid->num_blk8x8_uv << 1;

    p_Vid->mb_size[1][0] = p_Vid->mb_size[2][0] = p_Vid->mb_cr_size_x =
        (p_Vid->yuv_format == YUV420 || p_Vid->yuv_format == YUV422) ? 8 : 16;
    p_Vid->mb_size[1][1] = p_Vid->mb_size[2][1] = p_Vid->mb_cr_size_y =
        (p_Vid->yuv_format == YUV444 || p_Vid->yuv_format == YUV422) ? 16 : 8;
  ...
  p_Vid->max_bitCount =
      128 + 256 * p_Vid->bitdepth_luma +
      2 * p_Vid->mb_cr_size_y * p_Vid->mb_cr_size_x * p_Vid->bitdepth_chroma;
  // p_Vid->max_bitCount =  (128 + 256 * p_Vid->bitdepth_luma + 2
  // *p_Vid->mb_cr_size_y * p_Vid->mb_cr_size_x * p_Vid->bitdepth_chroma)*2;
  ...
  p_Vid->max_num_references = p_Vid->active_sps->frame_mbs_only_flag
                                  ? p_Vid->active_sps->num_ref_frames
                                  : 2 * p_Vid->active_sps->num_ref_frames;
  ...
  p_Vid->framerate =
      (float)p_Inp->output
          .frame_rate;  // The basic frame rate (of the original sequence)

  if (p_Inp->AdaptiveRounding) {
    if (p_Vid->yuv_format != 0) {
      get_mem4Dint(&(p_Vid->ARCofAdj4x4), 3, MAXMODE, MB_BLOCK_SIZE,
                   MB_BLOCK_SIZE);  // all modes
      get_mem4Dint(&(p_Vid->ARCofAdj8x8), p_Vid->P444_joined ? 3 : 1, MAXMODE,
                   MB_BLOCK_SIZE, MB_BLOCK_SIZE);  // modes 0, 1, 2, 3, P8x8
    } else {
      get_mem4Dint(&(p_Vid->ARCofAdj4x4), 1, MAXMODE, MB_BLOCK_SIZE,
                   MB_BLOCK_SIZE);  // all modes
      get_mem4Dint(&(p_Vid->ARCofAdj8x8), 1, MAXMODE, MB_BLOCK_SIZE,
                   MB_BLOCK_SIZE);  // modes 0, 1, 2, 3, P8x8
    }
  }
  ...
  p_Vid->PicHeightInMapUnits =
      (p_Vid->active_sps->frame_mbs_only_flag ? p_Vid->FrameHeightInMbs
                                              : p_Vid->FrameHeightInMbs >> 1);

  if ((p_Vid->b8x8info = (Block8x8Info *)calloc(1, sizeof(Block8x8Info))) ==
      NULL)
    no_mem_exit("init_img: p_Vid->block8x8info");

  if ((p_Inp->separate_colour_plane_flag != 0)) {
    for (i = 0; i < MAX_PLANE; i++) {
      // if ((p_Vid->mb_data_JV[i] = (Macroblock *)
      // calloc(p_Vid->FrameSizeInMbs,sizeof(Macroblock))) == NULL)
      if ((p_Vid->mb_data_JV[i] = alloc_mbs(p_Vid, p_Vid->FrameSizeInMbs,
                                            p_Vid->num_of_layers)) == NULL)
    ...
    if ((p_Vid->mb_data = alloc_mbs(p_Vid, p_Vid->FrameSizeInMbs,
                                    p_Vid->num_of_layers)) == NULL)
  ...
  if (p_Inp->UseConstrainedIntraPred) {
    if ((p_Vid->intra_block =
             (short *)calloc(p_Vid->FrameSizeInMbs, sizeof(short))) == NULL)
      no_mem_exit("init_img: p_Vid->intra_block");
  }

  if (p_Inp->CtxAdptLagrangeMult == 1) {
    if ((p_Vid->mb16x16_cost_frame =
             (double *)calloc(p_Vid->FrameSizeInMbs, sizeof(double))) == NULL) {
      no_mem_exit("init p_Vid->mb16x16_cost_frame");
    }
  }
  get_mem2D((byte ***)&(p_Vid->ipredmode), p_Vid->height_blk,
            p_Vid->width_blk);  // need two extra rows at right and bottom
  get_mem2D(
      (byte ***)&(p_Vid->ipredmode8x8), p_Vid->height_blk,
      p_Vid->width_blk);  // help storage for ipredmode 8x8, inserted by YV
  memset(&(p_Vid->ipredmode[0][0]), -1,
         p_Vid->height_blk * p_Vid->width_blk * sizeof(char));
  memset(&(p_Vid->ipredmode8x8[0][0]), -1,
         p_Vid->height_blk * p_Vid->width_blk * sizeof(char));

  // CAVLC mem
  if (p_Vid->num_of_layers > 1) {
    get_mem3Dint(&(p_Vid->nz_coeff_buf[0]), p_Vid->FrameSizeInMbs, 4,
                 4 + p_Vid->num_blk8x8_uv);
    if (p_Vid->p_Dpb_layer[1]->storage_format.yuv_format != YUV400 &&
        p_Vid->p_Dpb_layer[1]->storage_format.yuv_format !=
            p_Vid->p_Dpb_layer[0]->storage_format.yuv_format) {
      int num_blk8x8_uv =
          (1 << p_Vid->p_Dpb_layer[1]->storage_format.yuv_format) & (~(0x1));
      get_mem3Dint(&(p_Vid->nz_coeff_buf[1]), p_Vid->FrameSizeInMbs, 4,
                   4 + num_blk8x8_uv);
    } else
  ...
  get_mem2Dolm(&(p_Vid->lambda_buf[0]), 10, 52 + p_Vid->bitdepth_luma_qp_scale,
               p_Vid->bitdepth_luma_qp_scale);
  p_Vid->lambda = p_Vid->lambda_buf[0];
  get_mem2Dodouble(&(p_Vid->lambda_md_buf[0]), 10,
                   52 + p_Vid->bitdepth_luma_qp_scale,
                   p_Vid->bitdepth_luma_qp_scale);
  p_Vid->lambda_md = p_Vid->lambda_md_buf[0];
  get_mem3Dodouble(&(p_Vid->lambda_me_buf[0]), 10,
                   52 + p_Vid->bitdepth_luma_qp_scale, 3,
                   p_Vid->bitdepth_luma_qp_scale);
  p_Vid->lambda_me = p_Vid->lambda_me_buf[0];
  get_mem3Doint(&(p_Vid->lambda_mf_buf[0]), 10,
                52 + p_Vid->bitdepth_luma_qp_scale, 3,
                p_Vid->bitdepth_luma_qp_scale);
  p_Vid->lambda_mf = p_Vid->lambda_mf_buf[0];
  if (p_Inp->UseRDOQuant) {
    get_mem2Dodouble(&(p_Vid->lambda_rdoq_buf[0]), 10,
                     52 + p_Vid->bitdepth_luma_qp_scale,
                     p_Vid->bitdepth_luma_qp_scale);
    p_Vid->lambda_rdoq = p_Vid->lambda_rdoq_buf[0];
  }
  if (p_Inp->CtxAdptLagrangeMult == 1) {
    get_mem2Dodouble(&(p_Vid->lambda_mf_factor_buf[0]), 10,
                     52 + p_Vid->bitdepth_luma_qp_scale,
                     p_Vid->bitdepth_luma_qp_scale);
  ...
  if (((p_Inp->RDPictureDecision) && p_Inp->GenerateMultiplePPS) ||
      (p_Inp->WeightedPrediction || p_Inp->WeightedBiprediction)) {
    int num_slices;

    if (p_Inp->slice_mode == 1) {
      num_slices = p_Vid->FrameSizeInMbs / p_Inp->slice_argument;
      if ((unsigned int)(num_slices * p_Inp->slice_argument) <
          p_Vid->FrameSizeInMbs)
        num_slices++;
    } else
      num_slices = 1;

    // All weight structures assigned using "MAX_REFERENCE_PICTURES". This plays
    // it safe since one may consider reordering or use this for MVC/FCFR coding
    // where references are expanded by 1.
    p_Vid->num_slices_wp = num_slices;
    get_mem4Dshort(&(p_Vid->wp_weights), 3, 2, MAX_REFERENCE_PICTURES,
                   num_slices);
    get_mem4Dshort(&(p_Vid->wp_offsets), 3, 2, MAX_REFERENCE_PICTURES,
                   num_slices);
    get_mem5Dshort(&(p_Vid->wbp_weight), 3, 2, MAX_REFERENCE_PICTURES,
                   MAX_REFERENCE_PICTURES, num_slices);
  } else {
  ...
  RandomIntraInit(p_Vid, p_Vid->PicWidthInMbs, p_Vid->FrameHeightInMbs,
                  p_Inp->RandomIntraMBRefresh);
  ...
  p_Vid->ChromaArrayType =
      p_Inp->separate_colour_plane_flag ? 0 : p_Inp->output.yuv_format;
  ...
  p_Vid->max_pic_order_cnt_lsb =
      1 << (p_Vid->log2_max_pic_order_cnt_lsb_minus4 + 4);
...
static void free_img(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_Vid->wp_weights) free_mem4Dshort(p_Vid->wp_weights);
  if (p_Vid->wp_offsets) free_mem4Dshort(p_Vid->wp_offsets);
  if (p_Vid->wbp_weight) free_mem5Dshort(p_Vid->wbp_weight);
...
int init_orig_buffers(VideoParameters *p_Vid, ImageData *imgData) {
  ...
  imgData->format.auto_crop_bottom_cr =
      (p_Vid->auto_crop_bottom * mb_height_cr[p_Vid->yuv_format]) /
      MB_BLOCK_SIZE;
  imgData->format.auto_crop_right_cr =
      (p_Vid->auto_crop_right * mb_width_cr[p_Vid->yuv_format]) / MB_BLOCK_SIZE;
  ...
  imgData->top_stride[1] = imgData->top_stride[2] = imgData->bot_stride[1] =
      imgData->bot_stride[2] = imgData->frm_stride[1] << 1;

  imgData->frm_data_buf[0][0] = imgData->frm_data_buf[0][1] =
      imgData->frm_data_buf[0][2] = NULL;
  imgData->frm_data_buf[1][0] = imgData->frm_data_buf[1][1] =
      imgData->frm_data_buf[1][2] = NULL;
  if ((p_Inp->separate_colour_plane_flag != 0)) {
    for (nplane = 0; nplane < MAX_PLANE; nplane++) {
      memory_size += get_mem2Dpel(&(imgData->frm_data[nplane]), p_Vid->height,
                                  p_Vid->width);
    }
  } else {
    // imgData->format = p_Inp->input_file1.format;
    memory_size +=
        get_mem2Dpel(&(imgData->frm_data[0]), p_Vid->height, p_Vid->width);

    if (p_Vid->yuv_format != YUV400) {
      int i, j, k;
      memory_size += get_mem2Dpel(&(imgData->frm_data[1]), p_Vid->height_cr,
                                  p_Vid->width_cr);
      memory_size += get_mem2Dpel(&(imgData->frm_data[2]), p_Vid->height_cr,
                                  p_Vid->width_cr);

      if (sizeof(imgpel) == sizeof(unsigned char)) {
        for (k = 1; k < 3; k++)
          memset(&(imgData->frm_data[k][0][0]), 128,
                 p_Vid->height_cr * p_Vid->width_cr * sizeof(imgpel));
      } else {
        for (k = 1; k < 3; k++)
  ...
  if (!p_Vid->active_sps->frame_mbs_only_flag) {
    ...
    memory_size +=
        init_top_bot_planes(imgData->frm_data[0], p_Vid->height,
                            &(imgData->top_data[0]), &(imgData->bot_data[0]));

    if (p_Vid->yuv_format != YUV400) {
      memory_size += 4 * (sizeof(imgpel **));

      memory_size +=
          init_top_bot_planes(imgData->frm_data[1], p_Vid->height_cr,
                              &(imgData->top_data[1]), &(imgData->bot_data[1]));
      memory_size +=
          init_top_bot_planes(imgData->frm_data[2], p_Vid->height_cr,
                              &(imgData->top_data[2]), &(imgData->bot_data[2]));
    }
  }
  return memory_size;
}
...
static int init_global_buffers(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if ((p_Vid->enc_frame_picture =
           (StorablePicture **)malloc(6 * sizeof(StorablePicture *))) == NULL)
    no_mem_exit("init_global_buffers: *p_Vid->enc_frame_picture");

  for (j = 0; j < 6; j++) p_Vid->enc_frame_picture[j] = NULL;

  if ((p_Vid->enc_field_picture =
           (StorablePicture **)malloc(2 * sizeof(StorablePicture *))) == NULL)
    no_mem_exit("init_global_buffers: *p_Vid->enc_field_picture");

  for (j = 0; j < 2; j++) p_Vid->enc_field_picture[j] = NULL;

  if ((p_Vid->frame_pic =
           (Picture **)malloc(p_Vid->frm_iter * sizeof(Picture *))) == NULL)
    no_mem_exit("init_global_buffers: *p_Vid->frame_pic");

  for (j = 0; j < p_Vid->frm_iter; j++) p_Vid->frame_pic[j] = malloc_picture();

  if (p_Inp->si_frame_indicator || p_Inp->sp_periodicity) {
    p_Vid->number_sp2_frames = 0;
    p_Vid->frame_pic_si =
        malloc_picture();  // picture buffer for the encoded SI picture
  ...
  if (p_Inp->PicInterlace != FRAME_CODING) {
    ...
    for (j = 0; j < 2; j++) p_Vid->field_pic1[j] = malloc_picture();

    if (p_Vid->num_of_layers == 2) {
      if ((p_Vid->field_pic2 = (Picture **)malloc(2 * sizeof(Picture *))) ==
          NULL)
        no_mem_exit("init_global_buffers: *p_Vid->field_pic2");

      for (j = 0; j < 2; j++) p_Vid->field_pic2[j] = malloc_picture();
    }
  }
#else
  // Allocate memory for field picture coding
  if (p_Inp->PicInterlace != FRAME_CODING) {
    ...
    for (j = 0; j < 2; j++) p_Vid->field_pic[j] = malloc_picture();
  }
#endif
  ...
  if (p_Inp->RestrictRef) {
    ...
    memory_size +=
        get_mem2D(&p_Vid->refresh_map, p_Vid->height >> 3, p_Vid->width >> 3);
  }
  ...
  if (!p_Vid->active_sps->frame_mbs_only_flag) {
    for (j = 0; j < p_Vid->num_of_layers; j++) {
      ...
      memory_size +=
          get_mem2Dpel(&(p_Vid->imgY_com_buf[j]), cps->height, cps->width);
      if (cps->yuv_format != YUV400) {
        memory_size += get_mem2Dpel(&(p_Vid->imgUV_com_buf[j][0]),
                                    cps->height_cr, cps->width_cr);
        memory_size += get_mem2Dpel(&(p_Vid->imgUV_com_buf[j][1]),
                                    cps->height_cr, cps->width_cr);
      }
    }
    // set;
  ...
  if (!p_Inp->IntraProfile) {
    if (p_Inp->SearchMode[0] == UM_HEX || p_Inp->SearchMode[1] == UM_HEX) {
      if ((p_Vid->p_UMHex = (UMHexStruct *)calloc(1, sizeof(UMHexStruct))) ==
          NULL)
        no_mem_exit("init_mv_block: p_Vid->p_UMHex");
      memory_size += UMHEX_get_mem(p_Vid, p_Inp);
    }
    if (p_Inp->SearchMode[0] == UM_HEX_SIMPLE ||
        p_Inp->SearchMode[1] == UM_HEX_SIMPLE) {
      if ((p_Vid->p_UMHexSMP =
               (UMHexSMPStruct *)calloc(1, sizeof(UMHexSMPStruct))) == NULL)
  ...
  if (p_Inp->RCEnable) rc_allocate_memory(p_Vid, p_Inp);

  if (p_Inp->redundant_pic_flag) {
    for (j = 0; j < p_Vid->num_of_layers; j++) {
      CodingParameters *cps = p_Vid->p_EncodePar[j];
      memory_size +=
          get_mem2Dpel(&p_Vid->imgY_tmp_buf[j], cps->height, cps->width);
      memory_size += get_mem2Dpel(&p_Vid->imgUV_tmp_buf[j][0], cps->height_cr,
                                  cps->width_cr);
      memory_size += get_mem2Dpel(&p_Vid->imgUV_tmp_buf[j][1], cps->height_cr,
                                  cps->width_cr);
    }
  ...
  if (!(p_Inp->OnTheFlyFractMCP) ||
      (p_Inp->OnTheFlyFractMCP == OTF_L1))  // JLT : on-the-fly compatibility
  {
    memory_size +=
        get_mem2Dint_pad(&p_Vid->imgY_sub_tmp, p_Vid->height, p_Vid->width,
                         IMG_PAD_SIZE_Y, IMG_PAD_SIZE_X);
  }
...
static void free_global_buffers(VideoParameters *p_Vid,
  ...
  if (p_Vid->frame_pic) {
    for (j = 0; j < p_Vid->frm_iter; j++) {
      if (p_Vid->frame_pic[j]) free_picture(p_Vid->frame_pic[j]);
    }
  ...
  if (p_Vid->field_pic1) {
    for (j = 0; j < 2; j++) {
      if (p_Vid->field_pic1[j]) free_picture(p_Vid->field_pic1[j]);
    }
    free_pointer(p_Vid->field_pic1);

    if (p_Vid->num_of_layers == 2) {
      for (j = 0; j < 2; j++) {
        if (p_Vid->field_pic2[j]) free_picture(p_Vid->field_pic2[j]);
      }
  ...
  if (p_Vid->field_pic) {
    for (j = 0; j < 2; j++) {
      if (p_Vid->field_pic[j]) free_picture(p_Vid->field_pic[j]);
    }
  ...
  free_mem3Dodouble(p_Vid->lambda_me_buf[0], 10,
                    52 + p_Vid->bitdepth_luma_qp_scale,
                    p_Vid->bitdepth_luma_qp_scale);
  p_Vid->lambda_me = p_Vid->lambda_me_buf[0] = p_Vid->lambda_me_buf[1] = NULL;
  free_mem3Doint(p_Vid->lambda_mf_buf[0], 10,
                 52 + p_Vid->bitdepth_luma_qp_scale,
                 p_Vid->bitdepth_luma_qp_scale);
  ...
  if (p_Inp->UseRDOQuant) {
    ...
    p_Vid->lambda_rdoq = p_Vid->lambda_rdoq_buf[0] = p_Vid->lambda_rdoq_buf[1] =
        NULL;
  }
  if (p_Inp->CtxAdptLagrangeMult == 1) {
    free_mem2Dodouble(p_Vid->lambda_mf_factor_buf[0],
                      p_Vid->bitdepth_luma_qp_scale);
    p_Vid->lambda_mf_factor = p_Vid->lambda_mf_factor_buf[0] =
        p_Vid->lambda_mf_factor_buf[1] = NULL;
  }
  ...
  if (p_Inp->RCEnable) rc_free_memory(p_Vid, p_Inp);
 ...
 ************************************************************************
 */
void free_mem_ACcoeff(int ****cofAC) { free_mem4Dint(cofAC); }
 ...
 ************************************************************************
 */
void free_mem_ACcoeff_new(int *****cofAC) { free_mem5Dint(cofAC); }
 ...
 ************************************************************************
 */
void free_mem_DCcoeff(int ***cofDC) { free_mem3Dint(cofDC); }
 ...
 */
static void set_level_indices(VideoParameters *p_Vid) {
  p_Vid->LevelIndex = get_level_index(p_Vid->active_sps->profile_idc,
                                      p_Vid->active_sps->level_idc,
...
void init_redundant_frame(VideoParameters *p_Vid, InputParameters *p_Inp) {
  if (p_Inp->redundant_pic_flag) {
    ...
    if ((1 << p_Inp->NumRedundantHierarchy) > p_Inp->PrimaryGOPLength) {
      error("PrimaryGOPLength must be greater than 2^NumRedundantHeirarchy",
            100);
    }
...
void set_redundant_frame(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_Inp->NumRedundantHierarchy > 1) {
    if (p_Vid->frameNuminGOP == (GOPlength >> 2) ||
        p_Vid->frameNuminGOP == ((GOPlength * 3) >> 2)) {
      p_Vid->redundant_coding = 0;
      p_Vid->key_frame = 1;
  ...
  if (p_Inp->NumRedundantHierarchy > 2) {
    if (p_Vid->frameNuminGOP == GOPlength >> 3 ||
        p_Vid->frameNuminGOP == ((GOPlength * 3) >> 3) ||
        p_Vid->frameNuminGOP == ((GOPlength * 5) >> 3) ||
        p_Vid->frameNuminGOP == ((GOPlength * 7) & 0x03)) {
      p_Vid->redundant_coding = 0;
      p_Vid->key_frame = 1;
  ...
  if (p_Inp->NumRedundantHierarchy > 3) {
    if (p_Vid->frameNuminGOP == (GOPlength >> 4) ||
        p_Vid->frameNuminGOP == ((GOPlength * 3) >> 4) ||
        p_Vid->frameNuminGOP == ((GOPlength * 5) >> 4) ||
        p_Vid->frameNuminGOP == ((GOPlength * 7) >> 4) ||
        p_Vid->frameNuminGOP == ((GOPlength * 9) >> 4) ||
        p_Vid->frameNuminGOP == ((GOPlength * 11) >> 4) ||
 ...
 ************************************************************************
 */
void encode_one_redundant_frame(VideoParameters *p_Vid,
                                InputParameters *p_Inp) {
  p_Vid->key_frame = 0;
...
static void gen_enc_par(VideoParameters *p_Vid, int layer_idx) {
  ...
  cps->P444_joined =
      (cps->yuv_format == YUV444 && (p_Inp->separate_colour_plane_flag == 0));
  ...
  cps->max_imgpel_value_comp_sq[0] =
      cps->max_pel_value_comp[0] * cps->max_pel_value_comp[0];
  ...
  if (cps->yuv_format != YUV400) {
    ...
    cps->max_imgpel_value_comp_sq[1] =
        cps->max_pel_value_comp[1] * cps->max_pel_value_comp[1];
    cps->max_imgpel_value_comp_sq[2] =
        cps->max_pel_value_comp[2] * cps->max_pel_value_comp[2];
    cps->num_blk8x8_uv = (1 << cps->yuv_format) & (~(0x1));
    cps->num_cdc_coeff = cps->num_blk8x8_uv << 1;
    cps->mb_size[1][0] = cps->mb_size[2][0] = cps->mb_cr_size_x =
        (cps->yuv_format == YUV420 || cps->yuv_format == YUV422) ? 8 : 16;
    cps->mb_size[1][1] = cps->mb_size[2][1] = cps->mb_cr_size_y =
        (cps->yuv_format == YUV444 || cps->yuv_format == YUV422) ? 16 : 8;
  ...
  cps->max_bitCount =
      128 + 256 * cps->bitdepth_luma +
      2 * cps->mb_cr_size_y * cps->mb_cr_size_x * cps->bitdepth_chroma;
  ...
  cps->PicHeightInMapUnits =
      (p_Vid->sps[layer_idx]->frame_mbs_only_flag ? cps->FrameHeightInMbs
...
static void generate_encode_parameters(VideoParameters *p_Vid) {
  int i;
  p_Vid->p_EncodePar[0] = (CodingParameters *)calloc(p_Vid->num_of_layers,
                                                     sizeof(CodingParameters));
  // init;
  for (i = 0; i < p_Vid->num_of_layers; i++) {
