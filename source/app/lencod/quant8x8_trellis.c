...
************************************************************************
*/
static void rdoq_8x8_CABAC(Macroblock *currMB, int **tblock, int block_x,
  ...
  noCoeff = init_trellis_data_8x8_CABAC(currMB, tblock, block_x, qp_per, qp_rem,
                                        q_params_8x8, p_scan, &levelData[0],
                                        &kStart, &kStop);
  est_writeRunLevel_CABAC(currMB, levelData, levelTrellis, LUMA_8x8, lambda_md,
                          kStart, kStop, noCoeff, 0);
}
...
************************************************************************
*/
static void rdoq_8x8_CAVLC(Macroblock *currMB, int **tblock, int block_y,
  ...
  init_trellis_data_8x8_CAVLC(currMB, tblock, block_x, qp_per, qp_rem,
                              q_params_8x8, p_scan, levelData);

  for (k = 0; k < 4; k++)
    est_RunLevel_CAVLC(currMB, levelData[k], levelTrellis[k], LUMA, b8, k, 16,
                       lambda_md);
}
 ...
 ************************************************************************
 */
int quant_8x8_trellis(Macroblock *currMB, int **tblock,
  ...
  rdoq_8x8_CABAC(currMB, tblock, block_x, qp_per, qp_rem, q_params_8x8, p_scan,
                 levelTrellis);
  ...
  for (coeff_ctr = 0; coeff_ctr < 64; coeff_ctr++) {
    ...
    if (*m7 != 0) {
      ...
      if (level != 0) {
        ...
        *m7 = rshift_rnd_sf(
            ((level * q_params_8x8[j][i].InvScaleComp) << qp_per), 6);
        *ACL++ = level;
        *ACR++ = run;
 ...
 ************************************************************************
 */
int quant_8x8cavlc_trellis(Macroblock *currMB, int **tblock,
  ...
  rdoq_8x8_CAVLC(currMB, tblock, block_y, block_x, qp_per, qp_rem, q_params_8x8,
                 p_scan, levelTrellis);
