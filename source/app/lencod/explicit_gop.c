...
void init_gop_structure(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  p_Vid->gop_structure =
      calloc(imax(10, max_gopsize), sizeof(GOP_DATA));  // +1 for reordering
...
void clear_gop_structure(VideoParameters *p_Vid) {
  if (p_Vid->gop_structure) free(p_Vid->gop_structure);
}
...
void interpret_gop_structure(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (nLength > 0) {
        ...
        switch (p_Inp->ExplicitHierarchyFormat[i]) {
          ...
          default:
            snprintf(errortext, ET_SIZE,
                     "Slice Type invalid in ExplicitHierarchyFormat param. "
        ...
        if (order_read == 0) {
          if (isdigit((int)(*(p_Inp->ExplicitHierarchyFormat + i)))) {
            ...
            if (display_no < 0 || display_no >= p_Inp->NumberBFrames) {
              snprintf(errortext, ET_SIZE,
                       "Invalid Frame Order value. Frame position needs to be "
                       "in [0,%d] range.",
            ...
            for (k = 0; k < coded_frame; k++) {
              if (p_Vid->gop_structure[k].display_no == display_no) {
                snprintf(errortext, ET_SIZE,
                         "Frame Order value %d in frame %d already used for "
                         "enhancement frame %d.",
                         display_no, coded_frame, k);
                error(errortext, 400);
              }
            }
          } else {
            snprintf(errortext, ET_SIZE,
                     "Slice Type needs to be followed by Display Order. Please "
                     "check configuration file.");
            error(errortext, 400);
          }
        } else if (order_read == 1) {
          if (stored_read == 0 &&
              !(isdigit((int)(*(p_Inp->ExplicitHierarchyFormat + i))))) {
            switch (p_Inp->ExplicitHierarchyFormat[i]) {
              case 'E':
              case 'e':
                p_Vid->gop_structure[coded_frame].reference_idc =
              ...
              default:
                snprintf(errortext, ET_SIZE,
                         "Reference_IDC invalid in ExplicitHierarchyFormat "
                         "param. Please check configuration file.");
                error(errortext, 400);
                break;
            }
            stored_read = 1;
            ...
            if (isdigit((int)(*(p_Inp->ExplicitHierarchyFormat + i)))) {
              ...
              p_Vid->gop_structure[coded_frame].slice_qp_off =
                  iClip3(-p_Vid->bitdepth_luma_qp_scale, 51, dqp);
              qp_read = 1;
            } else {
              snprintf(errortext, ET_SIZE,
                       "Reference_IDC needs to be followed by QP. Please check "
                       "configuration file.");
              error(errortext, 400);
            }
          } else if (stored_read == 1 && qp_read == 1 &&
                     temporal_layer_read == 0) {
            if (!(isdigit((int)(*(p_Inp->ExplicitHierarchyFormat + i))))) {
              if (p_Inp->ExplicitHierarchyFormat[i] == 't' ||
                  p_Inp->ExplicitHierarchyFormat[i] == 'T') {
...

          {
            ...
            if (coded_frame >= p_Inp->NumberBFrames) {
              snprintf(errortext, ET_SIZE,
                       "Total number of frames in Enhancement GOP need to be "
                       "fewer or equal to NumberBFrames parameter.");
              error(errortext, 400);
            }
          }
        }
    ...
    snprintf(
        errortext, ET_SIZE,
        "ExplicitHierarchyFormat is empty. Please check configuration file.");
    error(errortext, 400);
  }
