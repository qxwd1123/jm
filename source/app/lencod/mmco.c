...
void mmco_long_term(VideoParameters *p_Vid, int current_pic_num) {
  ...
  if (!p_Vid->currentPicture->idr_flag) {
    if (p_Vid->dec_ref_pic_marking_buffer != NULL) return;

    if (NULL == (tmp_drpm = (DecRefPicMarking_t *)calloc(
                     1, sizeof(DecRefPicMarking_t))))
    ...
    if (NULL == (tmp_drpm2 = (DecRefPicMarking_t *)calloc(
                     1, sizeof(DecRefPicMarking_t))))
...
#if HM50_LIKE_MMCO
void hm50_ref_management_frame_pic(DecodedPictureBuffer *p_Dpb,
                                   int current_pic_num) {
  ...
  if (p_Vid->p_Inp->NumberBFrames != 7 || p_Vid->p_Inp->num_ref_frames != 4 ||
  ...
  if (p_Vid->p_Inp->useCRA) {
    if ((p_Vid->currentSlice->slice_type == B_SLICE ||
         p_Vid->currentSlice->slice_type == P_SLICE) &&
        p_Vid->currentSlice->p_Dpb->ref_frames_in_buffer >= 2) {
      if (((p_Vid->currentSlice->ThisPOC / 2) -
           (p_Vid->p_Inp->NumberBFrames + 1)) %
  ...
  if (p_Vid->currentPicture->idr_flag) return;

  if ((p_Dpb->ref_frames_in_buffer + p_Dpb->ltref_frames_in_buffer) == 0)
    return;

  switch (p_Vid->currentSlice->ThisPOC / 2 % 8) {
    case 0: target_poc = p_Vid->currentSlice->ThisPOC - 32; break;
    case 4: target_poc = p_Vid->currentSlice->ThisPOC - 16; break;
    case 2: target_poc = p_Vid->currentSlice->ThisPOC - 8; break;
    case 6: target_poc = p_Vid->currentSlice->ThisPOC - 8; break;
    default: target_poc = -1; break;
  }

  if (target_poc < 0) return;

  for (i = 0; i < p_Dpb->used_size; i++) {
    if (p_Dpb->fs[i]->is_reference && (!(p_Dpb->fs[i]->is_long_term)) &&
        p_Dpb->fs[i]->poc == target_poc) {
...
#if CRA
void cra_ref_management_frame_pic(DecodedPictureBuffer *p_Dpb,
                                  int current_pic_num) {
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  unsigned i;
  int pic_num = 0;
  ...
  int last_intra_poc = p_Vid->currentSlice->ThisPOC / 2 /
                       p_Vid->p_Inp->intra_period * p_Vid->p_Inp->intra_period *
                       2;
  ...
  for (i = 0; i < p_Dpb->used_size; i++) {
    if (p_Dpb->fs[i]->is_reference && (!(p_Dpb->fs[i]->is_long_term)) &&
      ...
      if (tmp_drpm2->difference_of_pic_nums_minus1 < 0) {
        tmp_drpm2->difference_of_pic_nums_minus1 +=
            p_Vid->currentSlice->max_frame_num;
      }
      if (tmp_drpm2->difference_of_pic_nums_minus1 >=
          (int)p_Vid->currentSlice->max_frame_num) {
        tmp_drpm2->difference_of_pic_nums_minus1 -=
            p_Vid->currentSlice->max_frame_num;
      }
...
#if LD_REF_SETTING
void low_delay_ref_management_frame_pic(DecodedPictureBuffer *p_Dpb,
                                        int current_pic_num) {
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  unsigned i;
  int pic_num = 0;
  ...
  for (i = 0; i < p_Dpb->used_size; i++) {
    if ((p_Vid->currentSlice->ThisPOC / 2 - p_Dpb->fs[i]->poc / 2) > 4) {
      if (p_Dpb->fs[i]->is_reference && (!(p_Dpb->fs[i]->is_long_term)) &&
          p_Dpb->fs[i]->poc < min_poc_not_4x &&
          (p_Dpb->fs[i]->poc / 2) % (p_Vid->p_Inp->NumberBFrames + 1) != 0) {
      ...
      if (p_Dpb->fs[i]->is_reference && (!(p_Dpb->fs[i]->is_long_term)) &&
          p_Dpb->fs[i]->poc < min_poc_not_4x &&
          (p_Dpb->fs[i]->poc / 2) % ((p_Vid->p_Inp->NumberBFrames + 1) / 2) !=
              0) {
        min_poc_not_4x = p_Dpb->fs[i]->frame->poc;
        pic_num = p_Dpb->fs[i]->frame->pic_num;
      }
...
void poc_based_ref_management_frame_pic(DecodedPictureBuffer *p_Dpb,
                                        int current_pic_num) {
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  unsigned i;
  int pic_num = 0;
...
void poc_based_ref_management_field_pic(DecodedPictureBuffer *p_Dpb,
                                        int current_pic_num) {
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  unsigned int i;
  ...
  if (NULL ==
      (tmp_drpm = (DecRefPicMarking_t *)calloc(1, sizeof(DecRefPicMarking_t))))
    no_mem_exit("poc_based_ref_management_field_pic: tmp_drpm");
  tmp_drpm->Next = NULL;
  tmp_drpm->memory_management_control_operation = 0;
  ...
  if (NULL ==
      (tmp_drpm2 = (DecRefPicMarking_t *)calloc(1, sizeof(DecRefPicMarking_t))))
    no_mem_exit("poc_based_ref_management_field_pic: tmp_drpm2");
  tmp_drpm2->Next = tmp_drpm;
  tmp_drpm2->memory_management_control_operation = 1;
  tmp_drpm2->difference_of_pic_nums_minus1 = current_pic_num - pic_num1 - 1;

  if (NULL ==
      (tmp_drpm3 = (DecRefPicMarking_t *)calloc(1, sizeof(DecRefPicMarking_t))))
    no_mem_exit("poc_based_ref_management_field_pic: tmp_drpm3");
...
void tlyr_based_ref_management_frame_pic(VideoParameters *p_Vid,
                                         int current_pic_num) {
  unsigned i, first = 1;
  ...
  for (i = 0; i < p_Dpb->used_size; i++) {
    if (p_Dpb->fs[i]->is_reference && (!(p_Dpb->fs[i]->is_long_term)) &&
        p_Dpb->fs[i]->frame->temporal_layer >
            p_Vid->enc_picture->temporal_layer) {
      if (NULL == (tmp_drpm = (DecRefPicMarking_t *)calloc(
                       1, sizeof(DecRefPicMarking_t))))
        no_mem_exit("poc_based_ref_management: tmp_drpm2");
      tmp_drpm->memory_management_control_operation = 1;
      tmp_drpm->difference_of_pic_nums_minus1 =
          current_pic_num - p_Dpb->fs[i]->frame->pic_num - 1;
