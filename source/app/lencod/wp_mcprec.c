...
void wpxInitWPXObject(VideoParameters *p_Vid) {
  ...
  // wp_ref_l0
  if ((p_Vid->pWPX->wp_ref_list[LIST_0] = (WeightedPredRefX *)calloc(
           MAX_REFERENCE_PICTURES, sizeof(WeightedPredRefX))) == NULL)
    no_mem_exit("wpxInitWPXObject: p_Vid->pWPX->wp_ref_list[0]");
  // wp_ref_l1
  if ((p_Vid->pWPX->wp_ref_list[LIST_1] = (WeightedPredRefX *)calloc(
           MAX_REFERENCE_PICTURES, sizeof(WeightedPredRefX))) == NULL)
...
void wpxFreeWPXObject(VideoParameters *p_Vid) {
  // wp_ref_l0
  if (p_Vid->pWPX->wp_ref_list[LIST_0]) free(p_Vid->pWPX->wp_ref_list[LIST_0]);
  p_Vid->pWPX->wp_ref_list[LIST_0] = NULL;
  // wp_ref_l1
  if (p_Vid->pWPX->wp_ref_list[LIST_1]) free(p_Vid->pWPX->wp_ref_list[LIST_1]);
...
void wpxModifyRefPicList(Slice *currSlice) {
  ...
  if (currSlice->slice_type == B_SLICE) {
    ...
    for (i = 0; i < (unsigned int)(currSlice->num_ref_idx_active[LIST_1]);
         i++) {
      default_order_list1[i] = list[i]->pic_num;
    }
  }

  // obtain the ref_pic_list using POC-based reordering if currSlice->slice_type
  // != B_SLICE
  if (currSlice->slice_type == P_SLICE) {
    ...
    for (i = 0; i < p_Dpb->ref_frames_in_buffer; i++) {
      ...
      if (p_Dpb->fs_ref[i]->is_used == 3 &&
        ...
        abs_poc_dist =
            iabs(p_Dpb->fs_ref[i]->frame->poc - p_Vid->enc_picture->poc);
        poc_diff[i] = abs_poc_dist;
        list_sign[i] =
            (p_Vid->enc_picture->poc < p_Dpb->fs_ref[i]->frame->poc) ? +1 : -1;
      }
    }
  ...
  // loop over two lists
  for (pred_list = 0;
       pred_list < (1 + (currSlice->slice_type == B_SLICE ? 1 : 0));
       pred_list++) {
    if (pred_list == LIST_0) {
      list_order =
          (currSlice->slice_type == P_SLICE) ? re_order : default_order_list0;
    } else {
    ...
    switch (p_Vid->pWPX->curr_wp_rd_pass->algorithm) {
      ...
      case WP_MCPREC_MINUS1:
        ...
        for (j = cloned_refs; j < p_Dpb->ref_frames_in_buffer; j++) {
          p_Vid->pWPX->wp_ref_list[pred_list][j].PicNum =
              list_order[j - (cloned_refs - 1)];
    ...
    p_Vid->pWPX->num_wp_ref_list[pred_list] =
        imin(p_Vid->pWPX->num_wp_ref_list[pred_list],
             (pred_list == LIST_0) ? currSlice->num_ref_idx_active[LIST_0]
...
void wpxAdaptRefNum(Slice *currSlice) {
  ...
  if (p_Vid->pWPX->curr_wp_rd_pass->algorithm == WP_REGULAR) {
    switch (currSlice->slice_type) {
      ...
      case I_SLICE: break;
      case P_SLICE:
        if (currSlice->num_ref_idx_active[LIST_0] ==
            (p_Inp->P_List0_refs[view_id] *
             ((currSlice->structure != 0) + 1))) {
          currSlice->listXsize[LIST_0] = currSlice->num_ref_idx_active[LIST_0] =
              (char)imax(((currSlice->structure != 0) + 1),
      ...
      case B_SLICE:
        ...
        if (currSlice->num_ref_idx_active[LIST_1] ==
            (p_Inp->B_List1_refs[view_id] *
             ((currSlice->structure != 0) + 1))) {
          currSlice->listXsize[LIST_1] = currSlice->num_ref_idx_active[LIST_1] =
              (char)imax(((currSlice->structure != 0) + 1),
                         currSlice->num_ref_idx_active[LIST_1] -
                             ((currSlice->structure != 0) + 1));
        }
