...
void end_encode_one_macroblock(Macroblock *currMB) {
  ...
  if ((currSlice->mb_aff_frame_flag) && (currMB->mbAddrX & 0x01) &&
      (currMB->mb_type
           ? 0
           : (((currSlice->slice_type == B_SLICE)) ? !currMB->cbp
                                                   : 1))  // bottom is skip
      && ((currMB->PrevMB)->mb_type
              ? 0
              : (((currSlice->slice_type == B_SLICE)) ? !(currMB->PrevMB)->cbp
                                                      : 1)) &&
      !(field_flag_inference(currMB) == currMB->mb_field))  // top is skip
  {
    currSlice->rddata->min_rdcost =
        1e30;  // don't allow coding of a MB pair as skip if wrong inference
  } else
...
void init_enc_mb_params(Macroblock *currMB, RD_PARAMS *enc_mb, int intra) {
  ...
  int *InterSearch =
      p_Inp->InterSearch[(p_Vid->num_of_layers > 1) ? currSlice->view_id : 0]
  ...
  enc_mb->curr_mb_field =
      (short)((currSlice->mb_aff_frame_flag) && (currMB->mb_field));

  // Set valid modes
  enc_mb->valid[I8MB] =
      (short)((!p_Inp->DisableIntraInInter[p_Vid->view_id] || intra)
                  ? p_Inp->Transform8x8Mode
                  : 0);
  enc_mb->valid[I4MB] =
      (short)((!p_Inp->DisableIntraInInter[p_Vid->view_id] || intra)
                  ? ((p_Inp->Transform8x8Mode == 2) ? 0 : 1)
                  : 0);
  enc_mb->valid[I4MB] =
      (short)((!p_Inp->DisableIntra4x4) ? enc_mb->valid[I4MB] : 0);
  enc_mb->valid[I16MB] =
      (short)((!p_Inp->DisableIntraInInter[p_Vid->view_id] || intra) ? 1 : 0);
  enc_mb->valid[I16MB] =
      (short)((!p_Inp->DisableIntra16x16) ? enc_mb->valid[I16MB] : 0);
  enc_mb->valid[IPCM] =
      (short)((!p_Inp->DisableIntraInInter[p_Vid->view_id] || intra)
                  ? (p_Inp->EnableIPCM > 0 ? 1 : 0)
                  : 0);
  enc_mb->valid[SI4MB] = 0;
  // enc_mb->valid[SI4MB] = (short) (currSlice->slice_type == SI_SLICE);
  // enc_mb->valid[SI4MB] = (short) ( p_Inp->DisableIntra4x4 ? 0 :
  // enc_mb->valid[SI4MB]);
  ...
  enc_mb->valid[5] =
      (short)(!intra && InterSearch[5] && !(p_Inp->Transform8x8Mode == 2));
  enc_mb->valid[6] =
      (short)(!intra && InterSearch[6] && !(p_Inp->Transform8x8Mode == 2));
  enc_mb->valid[7] =
      (short)(!intra && InterSearch[7] && !(p_Inp->Transform8x8Mode == 2));
  enc_mb->valid[P8x8] = (short)(enc_mb->valid[4] || enc_mb->valid[5] ||
                                enc_mb->valid[6] || enc_mb->valid[7]);

  if (currSlice->UseRDOQuant && p_Inp->RDOQ_CP_Mode &&
      (p_Vid->qp != p_Vid->masterQP))
  ...
  memcpy(enc_mb->lambda_me,
         p_Vid->lambda_me[currSlice->slice_type][p_Vid->masterQP],
         3 * sizeof(double));
  memcpy(enc_mb->lambda_mf,
         p_Vid->lambda_mf[currSlice->slice_type][p_Vid->masterQP],
         3 * sizeof(int));
      ...
      for (l = currMB->list_offset; l <= currMB->list_offset + LIST_1; l++) {
        for (k = 0; k < currSlice->listXsize[l]; k++) {
          ...
          if ((currMB->mbAddrX & 0x01) == 0 &&
              currSlice->listX[l][k]->structure == BOTTOM_FIELD)
            currSlice->listX[l][k]->chroma_vector_adjustment = -2;
          if ((currMB->mbAddrX & 0x01) == 1 &&
              currSlice->listX[l][k]->structure == TOP_FIELD)
            currSlice->listX[l][k]->chroma_vector_adjustment = 2;
        }
      }
    } else {
...
void update_mcost(Slice *currSlice, int ref_lambda, short ref, int cur_list,
                  distblk mcost, distblk *bmcost, char *best_ref) {
  if (mcost < *bmcost) {
...
*************************************************************************************
*/
void list_prediction_cost(Macroblock *currMB, int list, int block, int mode,
                          RD_PARAMS *enc_mb, distblk bmcost[5],
                          char best_ref[2]) {
  Slice *currSlice = currMB->p_Slice;
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  ...
  short ref;
  int cur_list =
      list < BI_PRED ? currMB->list_offset + list : currMB->list_offset;
  int ref_lambda =
      (p_Inp->rdopt) ? enc_mb->lambda_mf[Q_PEL] : enc_mb->lambda_mf[Q_PEL] >> 2;
  ...
  if (list < BI_PRED) {
    ...
    int sp_indicator =
        (p_Inp->sp2_frame_indicator || p_Inp->sp_output_indicator);
    ...
    if (!p_Vid->checkref || list) {
      if (p_Inp->sp2_frame_indicator == 0 && p_Inp->sp_output_indicator == 0) {
        for (ref = 0; ref < currSlice->listXsize[cur_list]; ref++) {
          update_mcost(currSlice, ref_lambda, ref, cur_list,
                       motion_cost[ref][block], &bmcost[list], &best_ref[list]);
        }
      } else {
        for (ref = 0; ref < currSlice->listXsize[cur_list]; ref++) {
          // limit the number of reference frames to 1 when switching SP frames
          // are used
          if ((((currSlice->slice_type != P_SLICE &&
                 currSlice->slice_type != SP_SLICE)) ||
               (ref == 0))) {
            update_mcost(currSlice, ref_lambda, ref, cur_list,
                         motion_cost[ref][block], &bmcost[list],
                         &best_ref[list]);
          }
        }
      }
      ...
      for (ref = 0; ref < currSlice->listXsize[cur_list]; ref++) {
        if (ref == 0 ||
            (p_Inp->RestrictRef &&
             CheckReliabilityOfRef(currMB, block, list, ref, mode))) {
          // limit the number of reference frames to 1 when switching SP frames
          // are used
          if (!(sp_indicator) || (((currSlice->slice_type != P_SLICE &&
                                    currSlice->slice_type != SP_SLICE)) ||
                                  (ref == 0))) {
            update_mcost(currSlice, ref_lambda, ref, cur_list,
                         motion_cost[ref][block], &bmcost[list],
    ...
    if (p_Vid->active_pps->weighted_bipred_idc == 1) {
      int weight_sum =
          currSlice
              ->wbp_weight[0][(int)best_ref[LIST_0]][(int)best_ref[LIST_1]][0] +
          currSlice
              ->wbp_weight[1][(int)best_ref[LIST_0]][(int)best_ref[LIST_1]][0];

      if (weight_sum < -128 || weight_sum > 127) {
        bmcost[list] = DISTBLK_MAX;
      } else {
        bmcost[list] =
            ref_cost(currSlice, ref_lambda, (short)best_ref[LIST_0], cur_list) +
            ref_cost(currSlice, ref_lambda, (short)best_ref[LIST_1],
                     cur_list + LIST_1);
        bmcost[list] += BIDPartitionCost(currMB, mode, block, best_ref,
                                         enc_mb->lambda_mf[Q_PEL]);
      }
    } else {
      bmcost[list] =
          ref_cost(currSlice, ref_lambda, (short)best_ref[LIST_0], cur_list) +
    ...
    if (p_Vid->active_pps->weighted_bipred_idc == 1) {
      int weight_sum =
          currSlice->wbp_weight[0][0][0][0] + currSlice->wbp_weight[1][0][0][0];

      if (weight_sum < -128 || weight_sum > 127) {
        bmcost[list] = DISTBLK_MAX;
      } else {
        bmcost[list] = ref_cost(currSlice, ref_lambda, 0, cur_list) +
                       ref_cost(currSlice, ref_lambda, 0, cur_list + LIST_1);
        bmcost[list] += BPredPartitionCost(
            currMB, mode, block, 0, 0, enc_mb->lambda_mf[Q_PEL], !(list & 1));
      }
    } else {
      bmcost[list] = ref_cost(currSlice, ref_lambda, 0, cur_list) +
                     ref_cost(currSlice, ref_lambda, 0, cur_list + LIST_1);
      bmcost[list] += BPredPartitionCost(currMB, mode, block, 0, 0,
                                         enc_mb->lambda_mf[Q_PEL], !(list & 1));
    }
  }
}
...
*************************************************************************************
*/
void determine_prediction_list(distblk bmcost[5], Info8x8 *best,
                               distblk *cost) {
  int bestlist;
...
*************************************************************************************
*/
void compute_mode_RD_cost(Macroblock *currMB, RD_PARAMS *enc_mb, short mode,
                          short *inter_skip) {
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  ...
  if ((p_Inp->FastCrIntraDecision) ||
    ...
    do {
      // This seems to have a problem since we are not properly copying the
      // right b8x8info (transform based)
      terminate_16x16 = bslice_16x16_termination_control(
          p_Inp, p_Vid->b8x8info, &ctr16x16, mode, bslice);
      do {
        RD_8x8DATA *t8x8_data =
            currMB->luma_transform_size_8x8_flag ? p_RDO->tr8x8 : p_RDO->tr4x4;

        if (mode ==
            P8x8)  //&& !(p_Inp->RDOQ_QP_Num > 1 && currSlice->dQPCnt > 0))
        {
        ...
        if (CheckPredictionParams(currMB, p_Vid->b8x8info, mode) == TRUE) {
          if (RDCost_for_macroblocks(currMB, enc_mb->lambda_mdfp, mode)) {
            ...
            if (p_Inp->RCEnable) {
              if (mode == P8x8) {
                rc_store_diff(currSlice->diffy, &p_Vid->pCurImg[currMB->opix_y],
                              currMB->pix_x, t8x8_data->mpr8x8);
              } else
                rc_store_diff(currSlice->diffy, &p_Vid->pCurImg[currMB->opix_y],
                              currMB->pix_x, p_RDO->pred);
            }
    ...
    // Encode with no coefficients. Currently only for direct. This could be
    // extended to all other modes as in example.
    // if (bslice && mode < P8x8 && (*inter_skip == 0) && enc_mb->valid[mode] &&
    ...
    if ((bslice && mode == 0 && (*inter_skip == 0) && enc_mb->valid[mode] &&
      ...
      if (mode == P8x8) {
        ...
        p_Vid->b8x8info->best[P8x8][2] = t8x8_data->part[2];
        p_Vid->b8x8info->best[P8x8][3] = t8x8_data->part[3];
        if (p_Inp->Transform8x8Mode == 1 && currMB->valid_8x8 &&
            currMB->valid_4x4) {
      ...
      if (CheckPredictionParams(currMB, p_Vid->b8x8info, mode) == TRUE) {
        if (RDCost_for_macroblocks(currMB, enc_mb->lambda_mdfp, mode)) {
          ...
          if (p_Vid->AdaptiveRounding) reset_adaptive_rounding_direct(p_Vid);
...
void get_initial_mb16x16_cost(Macroblock *currMB) {
  ...
  if (currMB->mb_left && currMB->mb_up) {
    p_Vid->mb16x16_cost =
        (p_Vid->mb16x16_cost_frame[currMB->mbAddrX - 1] +
         p_Vid->mb16x16_cost_frame[currMB->mbAddrX - (p_Vid->width >> 4)] + 1) /
        2.0;
  } else if (currMB->mb_left) {
    p_Vid->mb16x16_cost = p_Vid->mb16x16_cost_frame[currMB->mbAddrX - 1];
  } else if (currMB->mb_up) {
    p_Vid->mb16x16_cost =
        p_Vid->mb16x16_cost_frame[currMB->mbAddrX - (p_Vid->width >> 4)];
  } else {
    p_Vid->mb16x16_cost = CALM_MF_FACTOR_THRESHOLD;
  }

  p_RDO->lambda_mf_factor =
      p_Vid->mb16x16_cost < CALM_MF_FACTOR_THRESHOLD
          ? 1.0
          : sqrt(p_Vid->mb16x16_cost /
                 (CALM_MF_FACTOR_THRESHOLD *
                  p_Vid->lambda_mf_factor[currSlice->slice_type][p_Vid->qp]));
}
...
void adjust_mb16x16_cost(Macroblock *currMB, distblk cost) {
  ...
  p_RDO->lambda_mf_factor =
      (p_Vid->mb16x16_cost <
       CALM_MF_FACTOR_THRESHOLD * (1 << LAMBDA_ACCURACY_BITS))
          ? 1.0
          : sqrt(p_Vid->mb16x16_cost /
                 (CALM_MF_FACTOR_THRESHOLD * (1 << LAMBDA_ACCURACY_BITS) *
                  p_Vid->lambda_mf_factor[currSlice->slice_type][p_Vid->qp]));
...
void update_lambda_costs(Macroblock *currMB, RD_PARAMS *enc_mb,
                         int lambda_mf[3]) {
  InputParameters *p_Inp = currMB->p_Inp;
  ...
  for (MEPos = 0; MEPos < 3; MEPos++) {
    lambda_mf[MEPos] =
        p_Inp->CtxAdptLagrangeMult == 0
            ? enc_mb->lambda_mf[MEPos]
            : (int)(enc_mb->lambda_mf[MEPos] * sqrt(p_RDO->lambda_mf_factor));
  }
}
...
int transform_termination_control(Macroblock *currMB, int mode) {
  ...
  if (p_Inp->Transform8x8Mode == 1) {
    //=========== try the 8x8 transform with mb_types 16x16,16x8, 8x16, 8x8, and
    // DIRECT 16x16 ===========
    if (currMB->luma_transform_size_8x8_flag == FALSE &&
        ((mode >= 1 && mode <= 3) ||
         ((currSlice->slice_type == B_SLICE) && mode == 0 &&
          p_Vid->active_sps->direct_8x8_inference_flag) ||
         (mode == P8x8)))
    // if (currMB->luma_transform_size_8x8_flag == FALSE &&
    //((mode >= 1 && mode <= 3) || ((currSlice->slice_type == B_SLICE) && mode
    //== 0 && active_sps->direct_8x8_inference_flag)))
    {
 ...
 *************************************************************************************
 * \brief
 *    Update prediction direction for mode P16x16 to check all prediction
 *directions
 *************************************************************************************
 */
int bslice_16x16_termination_control(InputParameters *p_Inp,
                                     Block8x8Info *b8x8info, int *ctr16x16,
                                     int mode, int bslice) {
  int lastcheck = 1;
  ...
  if (mode == 1 && bslice) {
    ...
    switch (*ctr16x16) {
      ...
      case 4:
        ...
        bipred_me = 2;
        break;
      default: error("invalid 'ctr16x16' value", -1); break;
    }
    for (i = 0; i < 4; i++) {
...
**************************************************************************************
*/
void get_bipred_cost(Macroblock *currMB, int mode, int block, int i, int j,
                     Info8x8 *best, RD_PARAMS *enc_mb, distblk bmcost[5]) {
  Slice *currSlice = currMB->p_Slice;
  ...
  if (best->ref[0] == 0 && best->ref[1] == 0) {
    ...
    if ((single_mv_l0->mv_x != bi0_mv_l0->mv_x) ||
        (single_mv_l0->mv_y != bi0_mv_l0->mv_y) ||
        (single_mv_l1->mv_x != bi0_mv_l1->mv_x) ||
        (single_mv_l1->mv_y != bi0_mv_l1->mv_y))
      list_prediction_cost(currMB, BI_PRED_L0, block, mode, enc_mb, bmcost, 0);
    else
      bmcost[BI_PRED_L0] = MaxVal;

    if ((single_mv_l0->mv_x != bi1_mv_l0->mv_x) ||
        (single_mv_l0->mv_y != bi1_mv_l0->mv_y) ||
        (single_mv_l1->mv_x != bi1_mv_l1->mv_x) ||
        (single_mv_l1->mv_y != bi1_mv_l1->mv_y)) {
      if ((bi0_mv_l0->mv_x != bi1_mv_l0->mv_x) ||
          (bi0_mv_l0->mv_y != bi1_mv_l0->mv_y) ||
          (bi0_mv_l1->mv_x != bi1_mv_l1->mv_x) ||
          (bi0_mv_l1->mv_y != bi1_mv_l1->mv_y))
        list_prediction_cost(currMB, BI_PRED_L1, block, mode, enc_mb, bmcost,
                             0);
      else
