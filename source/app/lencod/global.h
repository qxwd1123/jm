...
//! bit counter for a macroblock. Note that it seems safe to change all to
//! unsigned short for 16x16 MBs. May be an issue if MB > 32x32
struct bit_counter {
  int mb_total;
...
typedef struct rd_8x8_data {
  ...
  int **lrec;  // transform and quantized coefficients will be stored here for
               // SP frames
  imgpel **mpr8x8;
  imgpel ***mpr8x8CbCr;
  imgpel **rec_mbY8x8;
...
typedef struct {
  ...
  // This information is used for hierarchical coding with temporal scalability
  // feature If ReferenceReorder = 2, then
  //    reference reordering is applied based on POC and then frames of higher
  //    temporal layer are prevented from being referenced
  // If PocMemoryManagement = 2, then
  //    after encoding a reference frame with temporal layer T, a list of MMCO
  //    commands to delete higher temporal layer frames is created
...
typedef struct me_block {
  ...
  char list;           //!< current list (list 0 or list 1). This is needed for
                       //!< bipredictive ME
  ...
  // functions
  distblk (*computePredFPel)(struct storable_picture *, struct me_block *,
                             distblk, MotionVector *);
  distblk (*computePredHPel)(struct storable_picture *, struct me_block *,
                             distblk, MotionVector *);
  distblk (*computePredQPel)(struct storable_picture *, struct me_block *,
                             distblk, MotionVector *);
  distblk (*computeBiPredFPel)(struct storable_picture *,
                               struct storable_picture *, struct me_block *,
                               distblk, MotionVector *, MotionVector *);
  distblk (*computeBiPredHPel)(struct storable_picture *,
                               struct storable_picture *, struct me_block *,
                               distblk, MotionVector *, MotionVector *);
  distblk (*computeBiPredQPel)(struct storable_picture *,
                               struct storable_picture *, struct me_block *,
                               distblk, MotionVector *, MotionVector *);
} MEBlock;
...
typedef struct macroblock_enc {
  ...
  short mvd[2][BLOCK_MULTIPLE][BLOCK_MULTIPLE]
           [2];       //!< indices correspond to [list][block_y][block_x][x,y]
  char c_ipred_mode;  //!< chroma intra prediction mode
  char i16mode;

  Info8x8 b8x8[4];
  // imgpel              intra4x4_pred[16][17]; //!< 4x4 Intra prediction
  // samples
  ...
  char intra_pred_modes8x8[MB_BLOCK_PARTITIONS];  //!< four 8x8 blocks in a
                                                  //!< macroblock

  int64 cbp_blk;  //!< 1 bit set for every 4x4 block with coefs (not implemented
                  //!< for INTRA)
  int64 cbp_bits[3];
  ...
  // Functions
  void (*GetMVPredictor)(struct macroblock_enc *currMB, PixelPos *block,
  ...
  distblk (*IntPelME)(struct macroblock_enc *currMB, MotionVector *,
                      struct me_block *mv_block, distblk, int);
  distblk (*SubPelBiPredME)(struct macroblock_enc *currMB, struct me_block *,
                            int list, MotionVector *pred_mv1,
                            MotionVector *pred_mv2, MotionVector *mv1,
                            MotionVector *mv2, distblk min_mcost,
                            int *lambda_factor);
  distblk (*SubPelME)(struct macroblock_enc *currMB, MotionVector *pred_mv,
                      struct me_block *mv_block, distblk min_mcost,
                      int *lambda_factor);
  distblk (*BiPredME)(struct macroblock_enc *currMB, int, MotionVector *,
                      MotionVector *, MotionVector *, MotionVector *,
                      struct me_block *, int, distblk, int);
  int (*residual_transform_quant_luma_4x4)(struct macroblock_enc *currMB,
                                           ColorPlane pl, int block_x,
                                           int block_y, int *coeff_cost,
                                           int intra);
  int (*residual_transform_quant_luma_16x16)(struct macroblock_enc *currMB,
                                             ColorPlane pl);
  int (*residual_transform_quant_luma_8x8)(struct macroblock_enc *currMB,
                                           ColorPlane pl, int b8,
                                           int *coeff_cost, int intra);
  int (*residual_transform_quant_chroma_4x4[2])(struct macroblock_enc *currMB,
                                                int uv, int i11);
...
typedef struct slice {
  ...
  char direct_spatial_mv_pred_flag;  //!< Direct Mode type to be used (0:
                                     //!< Temporal, 1: Spatial)
  ...
  byte
      weighted_bipred_idc;  //!< Use of weighted biprediction (note that
                            //!< weighted_pred_flag is probably not needed here)
  ...
  short max_num_references;  //!< maximum number of reference pictures that may
                             //!< occur
  ...
  short ***
      *wbp_weight;  //!< weight in [list][fwd_index][bwd_idx][component] order

  int *****cofAC_new;  //!< AC coefficients
                       //!< [comp][8x8block][4x4block][level/run][scan_pos]
  ...
  imgpel ****mpr_16x16;  //!< prediction samples for 16x16 intra prediction
                         //!< modes (and chroma)
  imgpel ***mb_pred;     //!< current best prediction mode
  int ***mb_rres;        //!< the diff pixel values between the original
                   //!< macroblock/block and its prediction (reconstructed)
  int ***mb_ores;  //!< the diff pixel values between the original
                   //!< macroblock/block and its prediction (original)
  ...
  // Some Cabac related parameters (could be put in a different structure so we
  // can dynamically allocate them when needed)
  int coeff[64];
  int coeff_ctr;
  int pos;
  ...
  int (*mode_decision_for_I4x4_blocks)(Macroblock *currMB, int b8, int b4,
                                       int lambda, distblk *min_cost);
  int (*mode_decision_for_I8x8_blocks)(Macroblock *currMB, int b8, int lambda,
                                       distblk *min_cost);
  distblk (*rdcost_for_4x4_intra_blocks)(Macroblock *currMB, int *c_nz, int b8,
                                         int b4, int ipmode, int lambda,
                                         int mostProbableMode,
                                         distblk min_rdcost);
  distblk (*rdcost_for_8x8_intra_blocks)(Macroblock *currMB, int *c_nz, int b8,
                                         int ipmode, int lambda,
                                         distblk min_rdcost,
                                         int mostProbableMode);
  void (*submacroblock_mode_decision)(Macroblock *currMB, RD_PARAMS *,
                                      RD_8x8DATA *, int ****, int, distblk *);
  void (*intra_chroma_RD_decision)(Macroblock *currMB, RD_PARAMS *);

  void (*set_intrapred_4x4)(Macroblock *currMB, ColorPlane pl, int img_x,
                            int img_y, int *left_available, int *up_available,
                            int *all_available);
  void (*set_intrapred_8x8)(Macroblock *currMB, ColorPlane pl, int img_x,
                            int img_y, int *left_available, int *up_available,
                            int *all_available);
  void (*set_intrapred_16x16)(Macroblock *currMB, ColorPlane pl,
                              int *left_available, int *up_available,
                              int *all_available);
  void (*intra_chroma_prediction)(Macroblock *currMB, int *, int *, int *);
  void (*rdo_low_intra_chroma_decision)(Macroblock *currMB, int mb_available_up,
  ...
  void (*set_ref_and_motion_vectors)(Macroblock *currMB,
                                     struct pic_motion_params **motion,
                                     Info8x8 *part, int block);

  void (*luma_residual_coding)(Macroblock *currMB);
  int (*luma_residual_coding_8x8)(Macroblock *currMB, int *, int64 *, int,
                                  short, int[2], char *);
  void (*chroma_residual_coding)(Macroblock *currMB);

  distblk (*compute_cost8x8)(struct video_par *p_Vid, imgpel **cur_img,
                             imgpel **prd_img, int pic_opix_x,
                             distblk min_cost);
  distblk (*compute_cost4x4)(struct video_par *p_Vid, imgpel **cur_img,
                             imgpel **prd_img, int pic_opix_x,
                             distblk min_cost);
  distblk (*find_sad_16x16)(Macroblock *currMB);
  distblk (*distI16x16)(Macroblock *currMB, imgpel **img_org, imgpel **pred_img,
                        distblk min_cost);
  ...
  void (*writeMB_typeInfo)(Macroblock *currMB, SyntaxElement *se,
                           DataPartition *dP);
  void (*writeMB_Skip)(Macroblock *currMB, SyntaxElement *se,
                       DataPartition *dP);
  void (*writeIntraPredMode)(SyntaxElement *se, DataPartition *dP);
  void (*writeB8_typeInfo)(SyntaxElement *se, DataPartition *dP);
  void (*writeRefFrame[6])(Macroblock *currMB, SyntaxElement *se,
                           DataPartition *dP);
  ...
  void (*writeCIPredMode)(Macroblock *currMB, SyntaxElement *se,
                          DataPartition *dP);
  void (*writeFieldModeInfo)(Macroblock *currMB, SyntaxElement *se,
                             DataPartition *dP);
  void (*writeMB_transform_size)(Macroblock *currMB, SyntaxElement *se,
                                 DataPartition *dP);
  int (*writeCoeff16x16)(Macroblock *currMB, ColorPlane);
  int (*writeCoeff4x4_CAVLC)(Macroblock *currMB, int block_type, int b8, int b4,
                             int param);
  void (*write_and_store_CBP_block_bit)(Macroblock *currMB,
                                        EncodingEnvironmentPtr eep_dp, int type,
                                        int cbp_bit,
  ...
  void (*poc_ref_pic_reorder_frame)(struct slice *currSlice,
                                    unsigned num_ref_idx_lX_active,
                                    int list_no);

  // Quantization
  int (*quant_4x4)(Macroblock *currMB, int **tblock,
                   struct quant_methods *q_method);
  int (*quant_ac4x4cr)(Macroblock *currMB, int **tblock,
                       struct quant_methods *q_method);
  int (*quant_dc4x4)(Macroblock *currMB, int **tblock, int qp, int *DCLevel,
                     int *DCRun, LevelQuantParams *q_params_4x4,
                     const byte (*pos_scan)[2]);
  int (*quant_ac4x4)(Macroblock *currMB, int **tblock,
                     struct quant_methods *q_method);
  int (*quant_8x8)(Macroblock *currMB, int **tblock,
                   struct quant_methods *q_method);
  int (*quant_8x8cavlc)(Macroblock *currMB, int **tblock,
                        struct quant_methods *q_method, int ***cofAC);

  int (*quant_dc_cr)(Macroblock *currMB, int **tblock, int qp, int *DCLevel,
                     int *DCRun, LevelQuantParams *q_params_4x4, int **fadjust,
                     const byte (*pos_scan)[2]);
  void (*rdoq_4x4)(Macroblock *currMB, int **tblock,
                   struct quant_methods *q_method, int levelTrellis[16]);

  void (*rdoq_dc)(Macroblock *currMB, int **tblock, int qp_per, int qp_rem,
                  LevelQuantParams *q_params_4x4, const byte (*pos_scan)[2],
                  int levelTrellis[16], int type);

  void (*rdoq_ac4x4)(Macroblock *currMB, int **tblock,
                     struct quant_methods *q_method, int levelTrellis[16]);

  void (*rdoq_dc_cr)(Macroblock *currMB, int **tblock, int qp_per, int qp_rem,
                     LevelQuantParams *q_params_4x4, const byte (*pos_scan)[2],
                     int levelTrellis[16], int type);

  void (*set_modes_and_reframe)(Macroblock *currMB, int b8, short *p_dir,
                                int list_mode[2], char *list_ref_idx);
  void (*store_8x8_motion_vectors)(struct slice *currSlice, int dir,
                                   int block8x8, Info8x8 *B8x8Info);
...
typedef struct distortion_params {
  ...
  DistMetric metric_v[2]
                     [TOTAL_DIST_TYPES];  //!< Distortion metrics for each view
#endif
} DistortionParams;
...
typedef struct coding_par {
  ...
  int max_pel_value_comp[MAX_PLANE];  //!< max value that one picture element
                                      ...
                                      //!< pic_unit_bitdepth)
  int max_imgpel_value_comp_sq[MAX_PLANE];  //!< max value that one picture
                                            //!< element (pixel) can take
                                            //!< (depends on pic_unit_bitdepth)
  ...
  // uint32 dc_pred_value;                 //!< DC prediction value for current
  // component short max_imgpel_value;              //!< max value that one
  // picture element (pixel) can take (depends on pic_unit_bitdepth)
  char md5String[2][33];
} CodingParameters;
...
typedef struct video_par {
  ...
  int max_num_references;  //!< maximum number of reference pictures that may
                           //!< occur
  ...
  int frm_struct_buffer;  //!< length of the frame struct buffer (this may also
                          //!< be a define)
  int last_idr_code_order;
  int last_idr_disp_order;
  int last_mmco_5_code_order;  //!< it is a good idea to re-initialize POCs
                               //!< after such a frame: while seemingly not
                               //!< required it is critical for good implicit WP
                               //!< performance (I *THINK*) since POCs are set
                               //!< internally to zero after an MMCO=5 command.
  int last_mmco_5_disp_order;
  int curr_frm_idx;  //!< frame we wish to code in coding order (points also to
                     //!< p_frm)
  ...
  imgpel **pCurImg;  //!< Reference image. Luma for other profiles, can be any
                     //!< component for 4:4:4
  ...
  short start_me_refinement_hp;  //!< if set then recheck the center position
                                 //!< when doing half-pel motion refinement
  short start_me_refinement_qp;  //!< if set then recheck the center position
                                 //!< when doing quarter-pel motion refinement
  ...
  byte **pixel_map;  //!< Shows the latest reference frame that is reliable for
                     //!< each pixel
  ...
  Picture *currentPicture;  //!< The coded picture currently in the works
                            //!< (typically p_frame_pic, p_Vid->field_pic[0], or
                            //!< p_Vid->field_pic[1])
  ...
  int frm_iter;  //!< frame variations to create (useful for multiple coding
                 //!< passes)

  unsigned int field_picture;
  signed int toppoc;     //!< poc for this frame or field
  signed int bottompoc;  //!< for completeness - poc of bottom field of a frame
                         //!< (always = poc+1)
  signed int framepoc;   //!< min (toppoc, bottompoc)
  signed int ThisPOC;    //!< current picture POC
  ...
  short max_imgpel_value;  //!< max value that one picture element (pixel) can
                           //!< take (depends on pic_unit_bitdepth)
  ...
  int ChromaArrayType;
  Macroblock *mb_data_JV[MAX_PLANE];  //!< mb_data to be used during 4:4:4
                                      //!< independent mode encoding
  char colour_plane_id;  //!< colour_plane_id of the current coded slice (valid
                         //!< only when separate_colour_plane_flag is 1)
  ...
  int view_id;
#if (MVC_EXTENSION_ENABLE)
  int sec_view_force_fld;  //!< if view 0 is coded as FLD then force also view
                           //!< to be coded as FLD as well
  ...
  uint16 CurrentRTPSequenceNumber;  //!< The RTP sequence number of the current
                                    //!< packet
  ...
  struct storable_picture
      *enc_frame_picture_JV[MAX_PLANE];  //!< enc_frame to be used during 4:4:4
                                         //!< independent mode encoding
  ...
  int EvaluateDBOff;
  int TurnDBOff;
  // Note that these function pointers definitely affect now parallelization
  // since they are only allocated once. We need to add such info at maybe
  // within picture information or at a lower level RC
  int (*updateQP)(struct video_par *p_Vid, InputParameters *p_Inp,
                  RCQuadratic *p_quad, RCGeneric *p_gen, int topfield);
  void (*rc_update_pict_frame_ptr)(struct video_par *p_Vid,
                                   InputParameters *p_Inp, RCQuadratic *p_quad,
                                   RCGeneric *p_gen, int nbits);
  void (*rc_update_picture_ptr)(struct video_par *p_Vid, InputParameters *p_Inp,
                                int bits);
  void (*rc_init_pict_ptr)(struct video_par *p_Vid, InputParameters *p_Inp,
  ...
  int (*WriteNALU)(
      struct video_par *p_Vid, NALU_t *n,
      FILE **f_out);  //! Hides the write function in Annex B or RTP
  void (*error_conceal_picture)(struct video_par *p_Vid,
                                struct storable_picture *enc_pic, int decoder);
  distblk (*estimate_distortion)(Macroblock *currMB, int block, int block_size,
                                 short mode, short pdir, distblk min_rdcost);
  // function pointer for different ways of obtaining chroma interpolation
  void (*OneComponentChromaPrediction4x4)(Macroblock *currMB, imgpel *, int,
                                          int, MotionVector **,
                                          struct storable_picture *listX, int);
  ...
  void (*EdgeLoopLumaHor)(ColorPlane pl, imgpel **Img, byte Strength[16],
                          Macroblock *MbQ, int edge, int width);
  ...
  // ME distortion Function pointers. We need to move this to the MB or slice
  // level
  distblk (*computeUniPred[6])(struct storable_picture *ref1, struct me_block *,
                               distblk, MotionVector *);
  distblk (*computeBiPred1[3])(struct storable_picture *ref1,
                               struct storable_picture *ref2, struct me_block *,
                               distblk, MotionVector *, MotionVector *);
  distblk (*computeBiPred2[3])(struct storable_picture *ref1,
                               struct storable_picture *ref2, struct me_block *,
                               distblk, MotionVector *, MotionVector *);
  ...
  // deprecative varialbes, they will be removed in future, so donot add
  // varaibles here;
  ColorFormat yuv_format;
  int P444_joined;
  short bitdepth_luma;
...
extern void copyblock_sp(Macroblock *currMB, ColorPlane pl, int pos_mb1,
                         int pos_mb2);
extern int residual_transform_quant_chroma_4x4(Macroblock *currMB, int uv,
                                               int cr_cbp);
extern int residual_transform_quant_chroma_4x4_sp(Macroblock *currMB, int uv,
                                                  int i11);
extern int residual_transform_quant_chroma_4x4_sp2(Macroblock *currMB, int uv,
                                                   int i11);

extern distblk GetDirectCostMB(Macroblock *currMB);
extern distblk GetDirectCost8x8(Macroblock *currMB, int, distblk *);

extern int picture_coding_decision(VideoParameters *p_Vid, Picture *picture1,
                                   Picture *picture2, int qp);
...
//============= restriction of reference frames based on the latest
// intra-refreshes==========
...
extern void set_slice_type(VideoParameters *p_Vid, InputParameters *p_Inp,
                           int slice_type);
extern void free_encoder_memory(VideoParameters *p_Vid, InputParameters *p_Inp);
extern void output_SP_coefficients(VideoParameters *p_Vid,
                                   InputParameters *p_Inp);
extern void read_SP_coefficients(VideoParameters *p_Vid,
                                 InputParameters *p_Inp);
extern void init_redundant_frame(VideoParameters *p_Vid,
                                 InputParameters *p_Inp);
extern void set_redundant_frame(VideoParameters *p_Vid, InputParameters *p_Inp);
extern void encode_one_redundant_frame(VideoParameters *p_Vid,
                                       InputParameters *p_Inp);
...
static inline int is_intra(Macroblock *curr_MB) {
  return ((curr_MB)->mb_type == SI4MB || (curr_MB)->mb_type == I4MB ||
          (curr_MB)->mb_type == I16MB || (curr_MB)->mb_type == I8MB ||
          (curr_MB)->mb_type == IPCM);
}
