...
#if defined _DEBUG
#define TRACE \
  0  //!< 0:Trace off 1:Trace on 2:detailed CABAC context information
#else
#define TRACE \
  0  //!< 0:Trace off 1:Trace on 2:detailed CABAC context information
#endif
...
#define ENABLE_FIELD_CTX \
  1  //!< Enables field context types for CABAC. If disabled, results in speedup
     //!< for progressive content.
#define ENABLE_HIGH444_CTX \
  1  //!< Enables High 444 context types for CABAC. If disabled, results in
     //!< speedup of non High444 profile encodings.
#define DEBUG_BITDEPTH \
  0  //!< Ensures that > 8 bit content have no values that would result in out
     //!< of range results
#define ALLOW_GRAYSCALE 1  //!< Allow encoding in grayscale
#define ZEROSNR 1          //!< PSNR computation method
#define USE_RND_COST \
  0  //!< Perform ME RD decision using a rounding estimate of the motion cost
...
#define INTRA_RDCOSTCALC_NNZ \
  1  // 1: to recover block's nzn after rdcost calculation;
...
//#define BEST_NZ_COEFF 1   // yuwen 2005.11.03 => for high complexity mode
// decision (CAVLC, #TotalCoeff)

// defines for creating similar coding structures like HM
#define OUTPUT_REF_LIST \
  0  //!< for debug purpose, output the reference picture list. do not support
     //!< field

#define B0_MORE_REF \
  1  ///!< used for random access setting, treat POC(HM like poc, =JM
     /// poc/2)%(NumberOfBFrames+1)==0 as B0, i.e. use more references at
     /// temporal level 0
#define KEEP_B_SAME_LIST \
  1  ///!< keep B pictures using two identical reference picture lists
#define CRA \
  1  ///!< used for random access setting, HM CRA like random access point
     ///!< using open GOP is not enough, for example, 32 is an Intra picture,
     /// when encoding 40, everything is OK.
     ///!< but when encoding 36, there is one reference picture (32) in list0;
     /// actually, there should be two (32, 40) as HM.
     ///!< setting number of reference frame to 1 at picture 40, using RPLR to
     /// let list1 use 32 as reference,
     ///!< and marking all pictures unused for reference except 32 to solve this
     /// problem
#define HM50_LIKE_MMCO \
  1  ///!< use the HM-5.0 like MMCO, keep the identical referencing structure as
     /// HM-5.0
#define LD_REF_SETTING \
  1  ///!< used for low delay setting, 1+X referencing structure, as proposed in
     /// JCTVC-F701

// AVC Profile IDC definitions
enum {
  NO_PROFILE = 0,        //!< disable profile checking for experimental coding
                         //!< (enables FRExt, but disables MV)
...
#define _LUMA_MB_COEFF_COST_ \
  5  //!< threshold for luma coeffs of inter Macroblocks
#define _LUMA_8x8_COEFF_COST_ \
  5  //!< threshold for luma coeffs of 8x8 Inter Partition

//#define IMG_PAD_SIZE           20 //!< Number of pixels padded around the
// reference frame (>=4) #define IMG_PAD_SIZE_TIMES4    80 //!< Number of pixels
// padded around the reference frame in subpel units(>=16)
#define IMG_PAD_SIZE_X \
  32  //!< Number of pixels padded around the reference frame (>=4)
#define IMG_PAD_SIZE_Y \
  20  //!< Number of pixels padded around the reference frame (>=4)
...
#define MAX_SYMBOLS_PER_MB \
  1200  //!< Maximum number of different syntax elements for one MB
        // CAVLC needs more symbols per MB

#define MAX_PART_NR                                   \
  3 /*!< Maximum number of different data partitions. \
...
enum  // JLT : on-the-fly levels/modes
{
  ...
  OTF_L1 = 1,  // Store full pel & interpolated 1/2 pel positions; 1/4 pel
               // positions interpolate on-the-fly
  OTF_L2 = 2   // Store only full pell positions; 1/2 & 1/4 pel positions
               // interpolate on-the-fly
} OTFMode;

enum { OTF_ME = 0, OTF_MC = 1 } OTFFunction;
