...
void encode_one_macroblock_low(Macroblock *currMB) {
  ...
  short intra = (short)(islice || (currSlice->slice_type == SI_SLICE) ||
  ...
  if (p_Inp->num_of_views == 2)
    intra |= (currSlice->num_ref_idx_active[LIST_0] == 0 &&
              currSlice->num_ref_idx_active[LIST_1] ==
                  0);  // force intra if no ref available
#endif
  ...
  if (!intra) {
    ...
    currMB->best_mode = 10;  // let us set best_mode to an intra mode to avoid
                             // possible bug with RDOQ
      ...
      if (enc_mb.valid[mode]) {
        for (cost = 0, block = 0; block < (mode == 1 ? 1 : 2); block++) {
          ...
          if (bslice) {
            ...
            // currently Bi predictive ME is only supported for modes 1, 2, 3
            // and ref 0
        ...
        if (p_Inp->Transform8x8Mode)  // for inter rd-off, set 8x8 to do 8x8
                                      // transform
        {
          currSlice->set_modes_and_refs_for_blocks(currMB, (short)mode);
          currMB->luma_transform_size_8x8_flag =
              (byte)transform_decision(currMB, -1, &cost);
        }
    ...
    if (enc_mb.valid[P8x8]) {
      ...
      memset(currSlice->cofAC[0][0][0], 0,
      ...
      if (p_Inp->Transform8x8Mode) {
        ...
        //===========================================================
        //=====  LOOP OVER 8x8 SUB-PARTITIONS  (Motion Estimation & Mode
        // Decision) =====
        for (cost_direct = 0, block = 0; block < 4; block++) {
          submacroblock_mode_decision_low(
              currMB, &enc_mb, p_RDO->tr8x8, p_RDO->cofAC8x8ts[block],
        ...
        currMB->luma_transform_size_8x8_flag =
            FALSE;  // switch to 4x4 transform size
      }             // if (p_Inp->Transform8x8Mode)
      ...
      if (p_Inp->Transform8x8Mode != 2) {
        ...
        for (cost_direct = 0, block = 0; block < 4; block++) {
          submacroblock_mode_decision_low(
              currMB, &enc_mb, p_RDO->tr4x4, p_RDO->coefAC8x8[block],
      ...
      if (((p_Inp->Transform8x8Mode < 2) &&
           (p_RDO->tr4x4->mb_p8x8_cost < min_cost)) ||
          ((p_Inp->Transform8x8Mode > 0) &&
           (p_RDO->tr8x8->mb_p8x8_cost < min_cost))) {
      ...
      }  // if ((p_RDO->tr4x4->mb_p8x8_cost < min_cost ||
         // p_RDO->tr8x8->mb_p8x8_cost < min_cost))
      p_Vid->giRDOpt_B8OnlyFlag = FALSE;
    }

    // Find a motion vector for the Skip mode
    if (pslice) FindSkipModeMotionVector(currMB);
  } else  // if (!intra)
  {
  ...
  if (enc_mb.valid[0] && bslice)  // check DIRECT MODE
  {
    if (have_direct) {
      switch (p_Inp->Transform8x8Mode) {
        case 1:  // Mixture of 8x8 & 4x4 transform
          cost = ((cost8x8_direct < cost_direct) ||
    ...
    if (cost <= min_cost) {
      ...
      currMB->luma_transform_size_8x8_flag =
          tmp_8x8_flag;                                 // restore if not best
  ...
  {
    currMB->luma_transform_size_8x8_flag =
        TRUE;  // at this point cost will ALWAYS be less than min_cost
    ...
    if (rd_cost <= currMB->min_rdcost)  // HYU_NOTE. bug fix. 08/15/07
    {
      ...
      copy_image_data_16x16(temp_img[0],
                            &p_Vid->enc_picture->imgY[currMB->pix_y], 0,
                            currMB->pix_x);

      if (p_Vid->P444_joined) {
        copy_image_data_16x16(temp_img[1],
                              &p_Vid->enc_picture->imgUV[0][currMB->pix_y], 0,
                              currMB->pix_x);
        copy_image_data_16x16(temp_img[2],
                              &p_Vid->enc_picture->imgUV[1][currMB->pix_y], 0,
                              currMB->pix_x);
      }
  ...
  {
    rd_cost = find_best_mode_I16x16_MB(currMB, enc_mb.lambda_mdfp,
                                       currMB->min_rdcost);
    ...
    if (rd_cost < currMB->min_rdcost) {
      ...
      if (p_Inp->RCEnable)
        rc_store_diff(currSlice->diffy, &p_Vid->pCurImg[currMB->opix_y],
                      currMB->pix_x,
                      currSlice->mpr_16x16[0][(short)currMB->i16mode]);

      currMB->best_mode = I16MB;
      currMB->min_rdcost = rd_cost;
      currMB->cbp =
          currMB->residual_transform_quant_luma_16x16(currMB, PLANE_Y);
  ...
  //---------------------------------------------------------------------------
  {
    ...
    if (currMB->best_mode == P8x8) {
      if (currMB->luma_transform_size_8x8_flag && (p_RDO->tr8x8->cbp8x8 == 0) &&
    ...
    if (((currMB->cbp & 15) == 0) && currMB->mb_type != I4MB &&
        currMB->mb_type != I8MB)
      currMB->luma_transform_size_8x8_flag = FALSE;
    ...
    //===== check for SKIP mode =====
    if (p_Vid->P444_joined) {
      if ((pslice) && currMB->best_mode == 1 && currMB->cbp == 0 &&
          currSlice->cmp_cbp[1] == 0 && currSlice->cmp_cbp[2] == 0 &&
          motion[currMB->block_y][currMB->block_x].ref_idx[LIST_0] == 0 &&
          motion[currMB->block_y][currMB->block_x].mv[LIST_0].mv_x ==
              allmvs->mv_x &&
          motion[currMB->block_y][currMB->block_x].mv[LIST_0].mv_y ==
              allmvs->mv_y) {
        currMB->mb_type = currMB->b8x8[0].mode = currMB->b8x8[1].mode =
            currMB->b8x8[2].mode = currMB->b8x8[3].mode = 0;
        currMB->luma_transform_size_8x8_flag = FALSE;
      }
    ...
    if (currSlice->mb_aff_frame_flag ||
        (p_Inp->UseRDOQuant && currSlice->RDOQ_QP_Num > 1))
  ...
  if (p_Inp->RestrictRef) update_refresh_map(currMB, intra, intra1);

  /*update adaptive rounding offset p_Inp*/
  if (p_Vid->AdaptiveRounding) {
    update_offset_params(currMB, currMB->best_mode,
                         currMB->luma_transform_size_8x8_flag);
  }
