...
void generate_pred_error_8x8(imgpel **cur_img, imgpel **prd_img,
 ...
 *************************************************************************************
 */
void LowPassForIntra8x8Pred(imgpel *PredPel, int block_up_left, int block_up,
                            int block_left) {
  int i;
  imgpel LoopArray[25];
  ...
  if (block_up) {
    ...
    LoopArray[16] =
        (imgpel)((PredPel[15] + PredPel[16] + (PredPel[16] << 1) + 2) >> 2);
  }

  if (block_up_left) {
    if (block_up && block_left) {
      LoopArray[0] =
          (imgpel)(((PredPel[0] << 1) + PredPel[1] + PredPel[17] + 2) >> 2);
    } else {
      if (block_up)
        LoopArray[0] =
            (imgpel)((PredPel[0] + (PredPel[0] << 1) + PredPel[1] + 2) >> 2);
      else if (block_left)
        LoopArray[0] =
            (imgpel)((PredPel[0] + (PredPel[0] << 1) + PredPel[17] + 2) >> 2);
    }
  }
  ...
  if (block_left) {
    if (block_up_left)
      LoopArray[17] =
          (imgpel)((PredPel[0] + (PredPel[17] << 1) + PredPel[18] + 2) >> 2);
    else
      LoopArray[17] =
          (imgpel)((PredPel[17] + (PredPel[17] << 1) + PredPel[18] + 2) >> 2);
    ...
    LoopArray[24] =
        (imgpel)((PredPel[23] + (PredPel[24] << 1) + PredPel[24] + 2) >> 2);
  }
...
static inline void get_i8x8_horizontal(imgpel **cur_pred, imgpel *PredPel) {
  ...
  for (i = 0; i < BLOCK_SIZE_8x8; i++) {
    cur_pred[i][0] = cur_pred[i][1] = cur_pred[i][2] = cur_pred[i][3] =
        cur_pred[i][4] = cur_pred[i][5] = cur_pred[i][6] = cur_pred[i][7] =
            (imgpel)(&P_Q)[i];
  }
}
 ...
 ************************************************************************
 */
static inline void get_i8x8_dc(imgpel **cur_pred, imgpel *PredPel,
                               int left_available, int up_available) {
  int i, j, s0 = 0;
  if (up_available && left_available) {
    // no edge
    s0 = rshift_rnd_sf((P_A + P_B + P_C + P_D + P_E + P_F + P_G + P_H + P_Q +
                        P_R + P_S + P_T + P_U + P_V + P_W + P_X),
                       4);
 ...
 ************************************************************************
 */
void set_intrapred_8x8(Macroblock *currMB, ColorPlane pl, int img_x, int img_y,
                       int *left_available, int *up_available,
                       int *all_available) {
  ...
  *all_available =
  ...
  LowPassForIntra8x8Pred(PredPel, block_available_up_left, block_available_up,
                         block_available_left);
}
 ...
 ************************************************************************
 */
void set_intrapred_8x8_mbaff(Macroblock *currMB, ColorPlane pl, int img_x,
 ...
 ************************************************************************
 */
void get_intrapred_8x8(Macroblock *currMB, ColorPlane pl, int i8x8_mode,
                       int left_available, int up_available) {
  imgpel *PredPel = currMB->intra8x8_pred[pl];  // array of predictor pels
  ...
  switch (i8x8_mode) {
    case VERT_PRED: get_i8x8_vertical(curr_mpr_8x8[VERT_PRED], PredPel); break;
    case HOR_PRED: get_i8x8_horizontal(curr_mpr_8x8[HOR_PRED], PredPel); break;
    case DC_PRED:
    ...
    case VERT_LEFT_PRED:
      ...
      break;
    case HOR_UP_PRED: get_i8x8_horup(curr_mpr_8x8[HOR_UP_PRED], PredPel); break;
