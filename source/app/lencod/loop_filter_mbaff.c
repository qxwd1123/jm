...
static void get_strength_ver_MBAff(byte Strength[MB_BLOCK_SIZE],
                                   Macroblock *MbQ, int edge, int mvlimit);
static void get_strength_hor_MBAff(byte Strength[MB_BLOCK_SIZE],
                                   Macroblock *MbQ, int edge, int mvlimit);
static void edge_loop_luma_ver_MBAff(ColorPlane pl, imgpel **Img,
                                     byte Strength[MB_BLOCK_SIZE],
                                     Macroblock *MbQ, int edge);
static void edge_loop_luma_hor_MBAff(ColorPlane pl, imgpel **Img,
                                     byte Strength[MB_BLOCK_SIZE],
                                     Macroblock *MbQ, int edge, int width);
static void edge_loop_chroma_ver_MBAff(imgpel **Img,
                                       byte Strength[MB_BLOCK_SIZE],
                                       Macroblock *MbQ, int edge, int uv);
static void edge_loop_chroma_hor_MBAff(imgpel **Img,
                                       byte Strength[MB_BLOCK_SIZE],
                                       Macroblock *MbQ, int edge, int width,
                                       int uv);
  ...
  for (idx = 0; idx < 16; ++idx) {
    ...
    if (p_Vid->type == SP_SLICE || p_Vid->type == SI_SLICE) {
      ...
      if (!(MbP->mb_type == I4MB || MbP->mb_type == I16MB ||
            MbP->mb_type == I8MB || MbP->mb_type == IPCM) &&
          !(MbQ->mb_type == I4MB || MbQ->mb_type == I16MB ||
            MbQ->mb_type == I8MB || MbQ->mb_type == IPCM)) {
        if (((MbQ->cbp_blk & ((int64)1 << blkQ)) != 0) ||
            ((MbP->cbp_blk & ((int64)1 << blkP)) != 0))
          Strength[idx] = 2;
        else {
          ...
          if (p_Vid->mixedModeEdgeFlag) {
            ...
            blk_x2 = (short)(pixP.pos_x >> 2);
            {
              ...
              StorablePicturePtr ref_p0 = mv_info_p->ref_idx[LIST_0] == -1
                                              ? NULL
                                              : mv_info_p->ref_pic[LIST_0];
              StorablePicturePtr ref_q0 = mv_info_q->ref_idx[LIST_0] == -1
                                              ? NULL
                                              : mv_info_q->ref_pic[LIST_0];
              StorablePicturePtr ref_p1 = mv_info_p->ref_idx[LIST_1] == -1
                                              ? NULL
                                              : mv_info_p->ref_pic[LIST_1];
              StorablePicturePtr ref_q1 = mv_info_q->ref_idx[LIST_1] == -1
                                              ? NULL
                                              : mv_info_q->ref_pic[LIST_1];
...
static void get_strength_hor_MBAff(byte Strength[MB_BLOCK_SIZE],
  ...
  for (idx = 0; idx < 16; ++idx) {
    ...
    if (p_Vid->type == SP_SLICE || p_Vid->type == SI_SLICE) {
      Strength[idx] =
          (edge == 0 &&
           (((!p_Vid->mb_aff_frame_flag && (p_Vid->structure == FRAME)) ||
 ...
 */
static void edge_loop_luma_ver_MBAff(ColorPlane pl, imgpel **Img,
                                     byte Strength[16], Macroblock *MbQ,
                                     int edge) {
...
*****************************************************************************************
*/
static void edge_loop_chroma_ver_MBAff(imgpel **Img, byte Strength[16],
                                       Macroblock *MbQ, int edge, int uv) {
  int pel, Strng;
  int C0, tc0, dif;
  imgpel L1, L0, R0, R1;
...
*****************************************************************************************
*/
static void edge_loop_chroma_hor_MBAff(imgpel **Img, byte Strength[16],
                                       Macroblock *MbQ, int edge, int width,
                                       int uv) {
  int pel, Strng;
  int incP, incQ;
