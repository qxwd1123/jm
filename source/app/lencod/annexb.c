...
int WriteAnnexbNALU(VideoParameters *p_Vid, NALU_t *n, FILE **f_annexb) {
  ...
  first_byte = (unsigned char)((n->forbidden_bit << 7) |
                               (n->nal_reference_idc << 5) | n->nal_unit_type);
  ...
  // n->nal_reference_idc, n->nal_unit_type);
  ...
  if (n->nal_unit_type == NALU_TYPE_PREFIX ||
    ...
    first_byte = (unsigned char)((n->svc_extension_flag << 7) |
                                 (n->non_idr_flag << 6) | n->priority_id);
    if (1 != fwrite(&first_byte, 1, 1, *f_annexb)) {
      printf("Fatal: cannot write %d bytes to bitstream file, exit (-1)\n", 1);
      exit(-1);
    }
    ...
    first_byte =
        (unsigned char)(((view_id & 3) << 6) | (n->temporal_id << 3) |
                        (n->anchor_pic_flag << 2) | (n->inter_view_flag << 1) |
                        n->reserved_one_bit);
    if (1 != fwrite(&first_byte, 1, 1, *f_annexb)) {
      printf("Fatal: cannot write %d bytes to bitstream file, exit (-1)\n", 1);
      exit(-1);
  ...
  fflush(*f_annexb);
#if TRACE
  // fprintf (p_Enc->p_trace, "\nAnnex B NALU w/ %s startcode, len %d,
  // forbidden_bit %d, nal_reference_idc %d, nal_unit_type %d\n\n\n",
  //   n->startcodeprefix_len == 4?"long":"short", n->len + 1, n->forbidden_bit,
  //   n->nal_reference_idc, n->nal_unit_type);
  fprintf(p_Enc->p_trace, "\nAnnex B NALU w/ %s startcode, len %d,",
          n->startcodeprefix_len == 4 ? "long" : "short", n->len + 1);
  fprintf(p_Enc->p_trace, "\n                forbidden_bit       %d,",
          n->forbidden_bit);
  fprintf(p_Enc->p_trace, "\n                nal_reference_idc   %d,",
          n->nal_reference_idc);
  ...
  if (n->nal_unit_type == NALU_TYPE_PREFIX ||
      n->nal_unit_type == NALU_TYPE_SLC_EXT) {
    fprintf(p_Enc->p_trace, "\n                svc_extension_flag  %d ",
            n->svc_extension_flag);
    fprintf(p_Enc->p_trace, "\n                non_idr_flag        %d ",
            n->non_idr_flag);
    fprintf(p_Enc->p_trace, "\n                priority_id         %d ",
            n->priority_id);
    ...
    fprintf(p_Enc->p_trace, "\n                view_id             %d ",
            p_Vid->p_Inp->MVCFlipViews ? !(n->view_id) : n->view_id);
    fprintf(p_Enc->p_trace, "\n                temporal_id         %d ",
            n->temporal_id);
    fprintf(p_Enc->p_trace, "\n                anchor_pic_flag     %d ",
            n->anchor_pic_flag);
