...
static const unsigned int MaxFs[] = {
  ...
  8192, 8192, 8704, 22080, 36864, 36864, 139264, 139264, 139264, UINT_MAX
};
// Level                                1.b   1  1.1  1.2  1.3   2  2.1  2.2
// 3  3.1  3.2    4  4.1  4.2   5  5.1  5.2   6  6.1  6.2   unconstrained
static const unsigned int MinCR[] = { 2, 2, 2, 2, 2, 2, 2, 2, 2, 4, 4,
                                      4, 2, 2, 2, 2, 2, 2, 2, 2, 0 };
// Level                                  1   1b  1.1  1.2  1.3 2   2.1   2.2 3
// 3.1    3.2      4    4.1    4.2       5     5.1     5.2 6     6.1     6.2
// unconstrained
static const unsigned int MaxBR[] = { 64,      128,    192,    384,    768,
                                      2000,    4000,   4000,   10000,  14000,
                                      20000,   20000,  50000,  50000,  135000,
                                      240000,  240000, 240000, 480000, 800000,
                                      UINT_MAX };
// Level                                  1   1b  1.1   1.2   1.3 2   2.1   2.2
// 3    3.1    3.2      4    4.1    4.2       5     5.1     5.2
// 6     6.1     6.2  unconstrained
static const unsigned int MaxCPB[] = { 175,     350,    500,    1000,   2000,
                                       2000,    4000,   4000,   10000,  14000,
                                       20000,   25000,  62500,  62500,  135000,
                                       240000,  240000, 240000, 480000, 800000,
                                       UINT_MAX };
// Max macroblock processing rate
// Level                                   1    1b   1.1   1.2    1.3 2    2.1
// 2.2      3     3.1     3.2       4     4.1     4.2       5     5.1      5.2
// 6      6.1      6.2  unconstrained
static const unsigned int MaxMBPS[] = {
  1485,   1485,   3000,    6000,    11880,   11880,    19800,
  20250,  40500,  108000,  216000,  245760,  245760,   522240,
  589824, 983040, 2073600, 4177920, 8355840, 16711680, UINT_MAX
};
...
static const int LEVELVMVLIMIT[21][6] = {
  ...
  { -8192, 8191, -16384, 16383, -32768,
    32767 },  // level 6   unconstrained (signed 16 bit)
  { -8192, 8191, -16384, 16383, -32768,
    32767 },  // level 6.1 unconstrained (signed 16 bit)
  { -8192, 8191, -16384, 16383, -32768,
    32767 },  // level 6.2 unconstrained (signed 16 bit)
  { -8192, 8191, -16384, 16383, -32768,
    32767 }  // unconstrained (signed 16 bit)
};
...
Boolean is_valid_level(unsigned int profileIdc, unsigned int levelIdc) {
  // level 1b is indicated by value 9 in FRExt and (value 11 +
  // constraint_set3_flag) in base spec profiles
  switch (levelIdc) {
    case 9: return (is_FREXT_profile(profileIdc));
    case 10:
    case 11:
    case 12:
    case 13:
 ...
 */
int get_level_index(unsigned int profile_idc, unsigned int level_idc,
                    unsigned int constrained_set3_flag) {
  switch (level_idc) {
    case 9:
    ...
    case 11:
      if (!is_FREXT_profile(profile_idc) && (constrained_set3_flag == 0))
        ...
        return LEVEL_1b;
      break;
    case 12: return LEVEL_1_2; break;
    case 13: return LEVEL_1_3; break;
    case 20: return LEVEL_2; break;
    case 21: return LEVEL_2_1; break;
    case 22: return LEVEL_2_2; break;
    case 30: return LEVEL_3; break;
    case 31: return LEVEL_3_1; break;
    case 32: return LEVEL_3_2; break;
    case 40: return LEVEL_4; break;
    case 41: return LEVEL_4_1; break;
    case 42: return LEVEL_4_2; break;
    case 50: return LEVEL_5; break;
    case 51: return LEVEL_5_1; break;
    case 52: return LEVEL_5_2; break;
    case 60: return LEVEL_6; break;
    case 61: return LEVEL_6_1; break;
    case 62: return LEVEL_6_2; break;
    case 0:
      return LEVEL_UNCONSTRAINED;  // unconstrained experimental coding
      break;
    default:
      fprintf(stderr,
              "Warning: unknown LevelIDC, setting to (mostly) unconstrained\n");
      return 17;
      break;
  }
}
 ...
 ***********************************************************************
 */
unsigned int getMaxFs(unsigned int profile_idc, unsigned int level_idc,
                      unsigned int constrained_set3_flag) {
 ...
 ***********************************************************************
 */
unsigned int getMaxMBPS(unsigned int profile_idc, unsigned int level_idc,
                        unsigned int constrained_set3_flag) {
  if (!is_valid_level(profile_idc, level_idc)) {
    error("getMaxMBPS: Unknown LevelIdc", 500);
  }

  return MaxMBPS[get_level_index(profile_idc, level_idc,
 ...
 ***********************************************************************
 */
unsigned int getMinCR(unsigned int profile_idc, unsigned int level_idc,
                      unsigned int constrained_set3_flag) {
  if (!is_valid_level(profile_idc, level_idc)) {
 ...
 ***********************************************************************
 */
unsigned int getMaxBR(unsigned int profile_idc, unsigned int level_idc,
                      unsigned int constrained_set3_flag) {
  if (!is_valid_level(profile_idc, level_idc)) {
 ...
 ***********************************************************************
 */
unsigned int getMaxCPB(unsigned int profile_idc, unsigned int level_idc,
                       unsigned int constrained_set3_flag) {
  if (!is_valid_level(profile_idc, level_idc)) {
...
void profile_check(InputParameters *p_Inp) {
  if ((p_Inp->ProfileIDC != BASELINE) && (p_Inp->ProfileIDC != MAIN) &&
      (p_Inp->ProfileIDC != EXTENDED) && (p_Inp->ProfileIDC != FREXT_HP) &&
    ...
    snprintf(errortext, ET_SIZE,
             ...
             "predictive or High 4:4:4 Intra),\n  44 (CAVLC 4:4:4 Intra)\n 118 "
  ...
  if (p_Inp->ProfileIDC == NO_PROFILE) {
    printf(
        "WARNING: ProfileIDC equal to 0 disables profile checking. \nThis "
        "allows experimental coding, but will create non-conforming "
        "streams!\n");
    return;
  }
  ...
  if ((p_Inp->partition_mode) && (p_Inp->symbol_mode == CABAC)) {
    snprintf(errortext, ET_SIZE,
             "Data partitioning and CABAC is not supported in any profile.");
    error(errortext, 500);
  }
  ...
  if (p_Inp->redundant_pic_flag) {
    if (p_Inp->ProfileIDC != BASELINE) {
      snprintf(errortext, ET_SIZE,
               "Redundant pictures are only allowed in Baseline profile "
               "(ProfileIDC = 66).");
      error(errortext, 500);
    }
  }
  ...
  if ((p_Inp->partition_mode) && (p_Inp->ProfileIDC != EXTENDED)) {
    snprintf(errortext, ET_SIZE,
             "Data partitioning is only allowed in Extended profile "
             "(ProfileIDC = 88).");
    error(errortext, 500);
  }
  ...
  if (p_Inp->ChromaIntraDisable && p_Inp->FastCrIntraDecision) {
    fprintf(stderr,
            "\n Warning: ChromaIntraDisable and FastCrIntraDecision cannot be "
  ...
  if ((p_Inp->sp_periodicity) && (p_Inp->ProfileIDC != EXTENDED)) {
    snprintf(
        errortext, ET_SIZE,
        "SP pictures are only allowed in Extended profile (ProfileIDC = 88).");
    error(errortext, 500);
  }

  // baseline
  if (p_Inp->ProfileIDC == BASELINE) {
    if ((p_Inp->NumberBFrames || p_Inp->BRefPictures == 2) &&
        p_Inp->PReplaceBSlice == 0) {
      snprintf(
          errortext, ET_SIZE,
          "B slices are not allowed in Baseline profile (ProfileIDC = 66).");
      error(errortext, 500);
    }
    if (p_Inp->WeightedPrediction) {
      snprintf(errortext, ET_SIZE,
               "Weighted prediction is not allowed in Baseline profile "
               "(ProfileIDC = 66).");
      error(errortext, 500);
    }
    ...
    if (p_Inp->symbol_mode == CABAC) {
      snprintf(errortext, ET_SIZE,
               "CABAC is not allowed in Baseline profile (ProfileIDC = 66).");
      error(errortext, 500);
    }
    if ((p_Inp->PicInterlace) || (p_Inp->MbInterlace)) {
      snprintf(errortext, ET_SIZE,
               "Interlace tools are not allowed in Baseline profile "
               "(ProfileIDC = 66).");
      error(errortext, 500);
    }
    if (p_Inp->GenerateMultiplePPS != 0) {
      snprintf(errortext, ET_SIZE,
               "GenerateMultiplePPS is not supported for Baseline profile "
               "because it requires enabling Weighted prediction.\n");
      error(errortext, 400);
    }
  }
  ...
  if (p_Inp->ProfileIDC == MAIN) {
    if (p_Inp->num_slice_groups_minus1) {
      snprintf(errortext, ET_SIZE,
               "num_slice_groups_minus1>0 (FMO) is not allowed in Main profile "
               "(ProfileIDC = 77).");
      error(errortext, 500);
    }
  }
  ...
  if (p_Inp->ProfileIDC == EXTENDED) {
    if (!p_Inp->directInferenceFlag) {
      snprintf(errortext, ET_SIZE,
               "direct_8x8_inference flag must be equal to 1 in Extended "
               "profile (ProfileIDC = 88).");
      error(errortext, 500);
    }

    if (p_Inp->symbol_mode == CABAC) {
      snprintf(errortext, ET_SIZE,
               "CABAC is not allowed in Extended profile (ProfileIDC = 88).");
      error(errortext, 500);
    }
  }
  ...
  if (p_Inp->separate_colour_plane_flag) {
    if (p_Inp->yuv_format != 3) {
      fprintf(stderr,
              "\nWarning: SeparateColourPlane has only effect in 4:4:4 chroma "
    ...
    if (p_Inp->ChromaMEEnable) {
      snprintf(errortext, ET_SIZE,
               "\nChromaMEEnable is not allowed when SeparateColourPlane is "
               "enabled.");
      error(errortext, 500);
    }
  }
  ...
  if (p_Inp->ProfileIDC == FREXT_CAVLC444) {
    if (p_Inp->symbol_mode != CAVLC) {
      snprintf(errortext, ET_SIZE,
               "\nCABAC is not allowed in CAVLC 4:4:4 Intra profile "
               "(ProfileIDC = 44).");
      error(errortext, 500);
    }
    if (!p_Inp->IntraProfile) {
      fprintf(stderr,
              "\nWarning: ProfileIDC equal to 44 implies an Intra only "
              "profile, setting IntraProfile = 1.");
  ...
  if (p_Inp->IntraProfile == 0 && p_Inp->num_ref_frames == 0) {
    snprintf(errortext, ET_SIZE,
             "\nProfiles other than IntraProfile require NumberReferenceFrames "
             "> 0.");
    error(errortext, 500);
  }
  ...
  if (p_Inp->yuv_format == YUV422 &&
      (p_Inp->ProfileIDC != FREXT_Hi422 && p_Inp->ProfileIDC != FREXT_Hi444 &&
       p_Inp->ProfileIDC != FREXT_CAVLC444)) {
    snprintf(errortext, ET_SIZE,
             "\nFRExt Profile(YUV Format) Error!\nYUV422 can be used only with "
             "ProfileIDC %d or %d or %d\n",
             FREXT_Hi422, FREXT_Hi444, FREXT_CAVLC444);
    error(errortext, 500);
  }
  if (p_Inp->yuv_format == YUV444 && (p_Inp->ProfileIDC != FREXT_Hi444 &&
                                      p_Inp->ProfileIDC != FREXT_CAVLC444)) {
    snprintf(errortext, ET_SIZE,
             "\nFRExt Profile(YUV Format) Error!\nYUV444 can be used only with "
             "ProfileIDC %d or %d.\n",
  ...
  if (p_Inp->rdopt == 2 && (is_FREXT_profile(p_Inp->ProfileIDC))) {
    snprintf(errortext, ET_SIZE,
             "Fast Mode Decision methods not supported in FREX Profiles");
    error(errortext, 500);
  }
  ...
  if (is_MVC_profile(p_Inp->ProfileIDC) && p_Inp->num_of_views != 2) {
    snprintf(errortext, ET_SIZE,
             "NumberOfViews must be two if ProfileIDC is set to 118 (Multiview "
             "High Profile). Otherwise (for a single view) please select a "
             "non-multiview profile such as 100.");
    error(errortext, 500);
  }

  //  if (p_Inp->PicInterlace == 2 && p_Inp->MVCInterViewReorder != 0)
  //  {
  //    snprintf(errortext, ET_SIZE, "MVCInterViewReorder not supported with
  //    Adaptive Frame Field Coding"); error (errortext, 500);
  //  }

  if (p_Inp->MVCInterViewReorder) {
    if (!is_MVC_profile(p_Inp->ProfileIDC)) {
      snprintf(errortext, ET_SIZE,
               "ProfileIDC must be 118, 128, 134, or 135 to use "
               "MVCInterViewReorder=1.");
      error(errortext, 500);
    }
  }
  if (is_MVC_profile(p_Inp->ProfileIDC)) {
    if (p_Inp->ReferenceReorder > 1) {
      snprintf(errortext, ET_SIZE,
               "ReferenceReorder > 1 is not supported with the Multiview (118) "
               "or Stereo High (128) profiles and is therefore disabled. \n");
      p_Inp->ReferenceReorder = 0;
    }
    if ((p_Inp->PocMemoryManagement) && (p_Inp->PicInterlace > 0)) {
      snprintf(
          errortext, ET_SIZE,
          "PocMemoryManagement>0 is not supported with the Multiview (118) or "
          "Stereo High (128) profiles and is therefore disabled. \n");
  ...
  if (p_Inp->of_mode == PAR_OF_RTP && (is_MVC_profile(p_Inp->ProfileIDC))) {
    snprintf(errortext, ET_SIZE,
             "RTP output mode is not compatible with MVC profiles.");
    error(errortext, 500);
  }
...
void level_check(VideoParameters *p_Vid, InputParameters *p_Inp) {
  unsigned int PicSizeInMbs =
      ...
      8;
  unsigned int MBProcessingRate =
      (unsigned int)(PicSizeInMbs * p_Inp->output.frame_rate + 0.5);
  int cpbBrFactor = (p_Inp->ProfileIDC >= FREXT_HP) ? 1500 : 1200;

  if (p_Inp->LevelIDC == 0) {
    printf(
        "WARNING: LevelIDC equal to 0 disables level checking. \nThis allows "
        "experimental coding, but will create non-conforming streams!\n");
    return;
  }

  if (!is_valid_level((p_Inp)->ProfileIDC, (p_Inp)->LevelIDC)) {
    snprintf(errortext, ET_SIZE,
             "\nSpecified LevelIDC value is undefined. Please check your "
             "settings.\n");
    error(errortext, 500);
  }
  ...
  if ((p_Inp->LevelIDC >= 30) && (p_Inp->directInferenceFlag == 0)) {
    fprintf(stderr,
            "\nWarning: LevelIDC 3.0 and above require direct_8x8_inference to "
            "be set to 1. Please check your settings.\n");
    p_Inp->directInferenceFlag = 1;
  }
  if (((p_Inp->LevelIDC < 21) || (p_Inp->LevelIDC > 41)) &&
      (p_Inp->PicInterlace > 0 || p_Inp->MbInterlace > 0)) {
    snprintf(errortext, ET_SIZE,
             "\nInterlace modes only supported for LevelIDC in the range of 21 "
             "and 41. Please check your settings.\n");
    error(errortext, 500);
  }

  if (PicSizeInMbs > getMaxFs(p_Inp->ProfileIDC, p_Inp->LevelIDC, 0)) {
    snprintf(errortext, ET_SIZE,
             "\nPicSizeInMbs exceeds maximum allowed size at specified "
             "LevelIdc %.1f\n",
  ...
  if (p_Inp->IntraProfile && (PicSizeInMbs > 1620) && p_Inp->slice_mode != 1) {
    error("\nIntraProfile with PicSizeInMbs > 1620 requires SliceMode equal 1.",
          500);
  }

  if (p_Inp->IntraProfile && (PicSizeInMbs > 1620) &&
      ((unsigned int)p_Inp->slice_argument >
       (getMaxFs(p_Inp->ProfileIDC, p_Inp->LevelIDC, 0) >> 2))) {
    // when PicSizeInMbs is greater than 1620, the number of macroblocks in any
    // coded slice shall not exceed MaxFS / 4
    snprintf(errortext, ET_SIZE,
             "\nIntraProfile requires SliceArgument smaller or equal to 1/4 "
             "MaxFs at specified LevelIdc %d.",
             p_Inp->LevelIDC);
    error(errortext, 500);
  }
  ...
  if (MBProcessingRate > getMaxMBPS(p_Inp->ProfileIDC, p_Inp->LevelIDC, 0)) {
    snprintf(errortext, ET_SIZE,
             "\nMB Processing Rate (%d) exceeds maximum allowed processing "
             "rate (%d) at specified LevelIdc %.1f\n",
             MBProcessingRate,
             getMaxMBPS(p_Inp->ProfileIDC, p_Inp->LevelIDC, 0),
             (float)p_Inp->LevelIDC / 10.0);
    error(errortext, 500);
  }

  if (p_Inp->bit_rate >
      (int)(cpbBrFactor * getMaxBR(p_Inp->ProfileIDC, p_Inp->LevelIDC, 0))) {
    snprintf(errortext, ET_SIZE,
             "\nBit Rate (%d) exceeds maximum allowed bit rate (%d) at "
             "specified LevelIdc %.1f for NAL HRD\n",
             p_Inp->bit_rate,
             cpbBrFactor * getMaxBR(p_Inp->ProfileIDC, p_Inp->LevelIDC, 0),
             (float)p_Inp->LevelIDC / 10.0);
    error(errortext, 500);
  }
}
...
void update_mv_limits(VideoParameters *p_Vid, byte is_field) {
  ...
  memcpy(p_Vid->MaxHmvR, LEVELHMVLIMIT[p_Vid->LevelIndex < 17 ? 0 : 1],
         6 * sizeof(int));
  if (is_field) {
    int i;
    for (i = 0; i < 6; i++)
 ...
 ***********************************************************************
 */
void clip_mv_range(VideoParameters *p_Vid, int search_range, MotionVector *mv,
                   int res) {
  res <<= 1;

  mv->mv_x = (short)iClip3(p_Vid->MaxHmvR[0 + res] + search_range,
                           p_Vid->MaxHmvR[1 + res] - search_range, mv->mv_x);
  mv->mv_y = (short)iClip3(p_Vid->MaxVmvR[0 + res] + search_range,
                           p_Vid->MaxVmvR[1 + res] - search_range, mv->mv_y);
}
...
void test_clip_mvs(VideoParameters *p_Vid, MotionVector *mv, Boolean write_mb) {
  if ((mv->mv_x < p_Vid->MaxHmvR[4]) || (mv->mv_x > p_Vid->MaxHmvR[5]) ||
      (mv->mv_y < p_Vid->MaxVmvR[4]) || (mv->mv_y > p_Vid->MaxVmvR[5])) {
    if (write_mb == TRUE)
      printf(
          "Warning MVs (%d %d) were out of range x(%d %d) y(%d %d). Clipping "
          "mvs before writing\n",
          mv->mv_x, mv->mv_y, p_Vid->MaxHmvR[4], p_Vid->MaxHmvR[5],
          p_Vid->MaxVmvR[4], p_Vid->MaxVmvR[5]);
...
int out_of_bounds_mvs(VideoParameters *p_Vid, const MotionVector *mv) {
  return ((mv->mv_x < p_Vid->MaxHmvR[4]) || (mv->mv_x > p_Vid->MaxHmvR[5]) ||
          (mv->mv_y < p_Vid->MaxVmvR[4]) || (mv->mv_y > p_Vid->MaxVmvR[5]));
}

int InvalidWeightsForBiPrediction(Slice *currSlice, Block8x8Info *b8x8info,
                                  int mode) {
  ...
  for (cur_blk = 0; cur_blk < 4; cur_blk++) {
    if (b8x8info->best[mode][cur_blk].pdir == 2) {
      ...
      for (cur_comp = 0;
           cur_comp < (active_sps->chroma_format_idc == YUV400 ? 1 : 3);
           cur_comp++) {
      ...
      if (invalid_mode == 1) break;
    }
  }
  return invalid_mode;
}

int InvalidMotionVectors(VideoParameters *p_Vid, Slice *currSlice,
                         Block8x8Info *b8x8info, int mode) {
  int cur_blk;
  int l0ref, l1ref;
  int invalid_mode = 0;
  int i, j;

  if (mode > P8x8) return invalid_mode;

  // Brute force method. Note that this ignores currently subpartitions in 8x8
  // modes
  for (cur_blk = 0; cur_blk < 4; cur_blk++) {
    i = (cur_blk & 0x01) << 1;
    j = (cur_blk >> 1) << 1;
    switch (b8x8info->best[mode][cur_blk].pdir) {
      case 0:
        ...
        if (out_of_bounds_mvs(p_Vid,
                              &currSlice->all_mv[LIST_0][l0ref][mode][j][i])) {
          invalid_mode = 1;
          return invalid_mode;
        }
        break;
      case 1:
        ...
        if (out_of_bounds_mvs(p_Vid,
                              &currSlice->all_mv[LIST_1][l1ref][mode][j][i])) {
          invalid_mode = 1;
          return invalid_mode;
        }
        break;
...
Boolean CheckPredictionParams(Macroblock *currMB, Block8x8Info *b8x8info,
                              int mode) {
  ...
  if (mode == P8x8 && currMB->luma_transform_size_8x8_flag == TRUE) {
    ...
    for (i = 0; i < 4; i++) {
      if (b8x8info->best[P8x8][i].mode != 4 &&
          b8x8info->best[P8x8][i].mode != 0) {
        return FALSE;
      }
    }
  }
    ...
    if (mode == 0) {
      ...
      for (j = currMB->block_y; j < currMB->block_y + 4; j++) {
        for (i = currMB->block_x; i < currMB->block_x + 4; i++) {
          if (currSlice->direct_pdir[j][i] <
              0)  // direct_pdir should be moved and become part of p_Vid
                  // parameters
            return FALSE;
        }
      }
