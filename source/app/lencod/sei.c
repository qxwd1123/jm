...
void InitSEIMessages(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (p_SEI->sei_message[i].data == NULL)
      no_mem_exit("InitSEIMessages: sei_message[i].data");
...
void CloseSEIMessages(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_Inp->NumFramesInELSubSeq != 0) CloseSubseqLayerInfo();
  ...
  for (i = 0; i < MAX_LAYER_NUMBER; i++) {
    if (p_SEI->sei_message[i].data) free(p_SEI->sei_message[i].data);
...
Boolean HaveAggregationSEI(VideoParameters *p_Vid) {
  ...
  if (p_SEI->sei_message[AGGREGATION_SEI].available && p_Vid->type != B_SLICE)
    return TRUE;
  if (p_SEI->seiHasSubseqInfo) return TRUE;
  if (p_SEI->seiHasSubseqLayerInfo && p_Vid->number == 0) return TRUE;
  if (p_SEI->seiHasSubseqChar) return TRUE;
  if (p_SEI->seiHasSceneInformation) return TRUE;
  if (p_SEI->seiHasPanScanRectInfo) return TRUE;
  if (p_SEI->seiHasUser_data_unregistered_info) return TRUE;
  if (p_SEI->seiHasUser_data_registered_itu_t_t35_info) return TRUE;
  if (p_SEI->seiHasRecoveryPoint_info) return TRUE;
  if (p_SEI->seiHasTone_mapping) return TRUE;
  if (p_SEI->seiHasPostFilterHints_info) return TRUE;
  if (p_SEI->seiHasBufferingPeriod_info) return TRUE;
  if (p_SEI->seiHasPicTiming_info) return TRUE;
  if (p_SEI->seiHasDRPMRepetition_info) return TRUE;

  return FALSE;
  //  return p_Inp->SparePictureOption && ( seiHasSpare_picture ||
 ...
 ************************************************************************
 */
static void write_sei_message(SEIParameters *p_SEI, int id, byte *payload,
...
void AppendTmpbits2Buf(Bitstream *dest, Bitstream *source) {
  ...
  for (i = 0; i < source->byte_pos; i++) {
    ...
    for (j = 0; j < 8; j++) {
      dest->byte_buf <<= 1;
      if (source->streamBuffer[i] & mask) dest->byte_buf |= 1;
      dest->bits_to_go--;
      mask >>= 1;
  ...
  // copy the last byte, there are still (8-source->bits_to_go) bits in the
  // source buffer
 ...
 */
static void InitSparePicture(SEIParameters *p_SEI) {
  if (p_SEI->seiSparePicturePayload.data != NULL) CloseSparePicture(p_SEI);

  p_SEI->seiSparePicturePayload.data = malloc(sizeof(Bitstream));
  if (p_SEI->seiSparePicturePayload.data == NULL)
    no_mem_exit("InitSparePicture: seiSparePicturePayload.data");
  p_SEI->seiSparePicturePayload.data->streamBuffer = malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiSparePicturePayload.data->streamBuffer == NULL)
    no_mem_exit("InitSparePicture: seiSparePicturePayload.data->streamBuffer");
...
void CalculateSparePicture() {
  ...
  if ( tmpBitstream == NULL ) no_mem_exit("CalculateSparePicture:
tmpBitstream"); tmpBitstream->streamBuffer = malloc(MAXRTPPAYLOADLEN); if (
tmpBitstream->streamBuffer == NULL ) no_mem_exit("CalculateSparePicture:
tmpBitstream->streamBuffer"); memset( tmpBitstream->streamBuffer, 0,
MAXRTPPAYLOADLEN);
  ...
  for (m=0; m<possible_spare_pic_num; m++)
  {
    ...
    if (m==0)
      CandidateSpareFrameNum = seiSparePicturePayload.target_frame_num - 1; //
TargetFrameNum - 1; else CandidateSpareFrameNum = SpareFrameNum - 1; if (
CandidateSpareFrameNum < 0 ) CandidateSpareFrameNum = MAX_FN - 1; SpareFrameNum
= fb->picbuf_short[m+1]->frame_num_256; delta_spare_frame_num =
CandidateSpareFrameNum - SpareFrameNum; assert( delta_spare_frame_num == 0 );
...
//    if (m==0)
    ...
    ComposeSparePictureMessage(p_SEI, delta_spare_frame_num, ref_area_indicator,
tmpBitstream); seiSparePicturePayload.num_spare_pics++; }  // END for (m=0; m<2;
m++)
 ...
 ************************************************************************
 */
void ComposeSparePictureMessage(SEIParameters *p_SEI, int delta_spare_frame_num,
                                int ref_area_indicator,
                                Bitstream *tmpBitstream) {
 ...
 ************************************************************************
 */
Boolean CompressSpareMBMap(VideoParameters *p_Vid, unsigned char **map_sp,
                           Bitstream *bitstream) {
  int j, k;
  int noc, bit0, bitc;
  SyntaxElement sym;
  ...
  for (j = 0; j < p_Vid->height >> 4; j++)
    for (k = 0; k < p_Vid->width >> 4; k++) {
      ...
      if (map_sp[y][x] == bitc)
        ...
        sym.value1 = noc;
        size_compressed += writeSyntaxElement2Buf_UVLC(
            &sym,
            bitstream);  // the return value indicate the num of bits written
        noc = 0;
      }
      // go to the next mb:
...
static void FinalizeSpareMBMap(VideoParameters *p_Vid) {
  ...
  if (dest->streamBuffer == NULL)
    no_mem_exit("FinalizeSpareMBMap: dest->streamBuffer");
  dest->bits_to_go = 8;
  dest->byte_pos = 0;
  dest->byte_buf = 0;
  ...
  //    delta_frame_num
  delta_frame_num =
      CurrFrameNum - p_SEI->seiSparePicturePayload.target_frame_num;
...
void InitSubseqInfo(SEIParameters *p_SEI, int currLayer) {
  ...
  if (p_SEI->seiSubseqInfo[currLayer].data == NULL)
    no_mem_exit("InitSubseqInfo: p_SEI->seiSubseqInfo[currLayer].data");
  p_SEI->seiSubseqInfo[currLayer].data->streamBuffer = malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiSubseqInfo[currLayer].data->streamBuffer == NULL)
    no_mem_exit(
        "InitSubseqInfo: p_SEI->seiSubseqInfo[currLayer].data->streamBuffer");
  ...
  memset(p_SEI->seiSubseqInfo[currLayer].data->streamBuffer, 0,
         MAXRTPPAYLOADLEN);
}
 ...
 ************************************************************************
 */
void UpdateSubseqInfo(VideoParameters *p_Vid, InputParameters *p_Inp,
                      int currLayer) {
  SEIParameters *p_SEI = p_Vid->p_SEI;
  if (p_Vid->type != B_SLICE) {
    p_SEI->seiSubseqInfo[currLayer].stored_frame_cnt++;
    p_SEI->seiSubseqInfo[currLayer].stored_frame_cnt =
        p_SEI->seiSubseqInfo[currLayer].stored_frame_cnt % MAX_FN;
  }
  ...
  if (currLayer == 1) {
    if ((((p_Vid->curr_frm_idx - p_Vid->last_idr_code_order) %
              (p_Inp->NumFramesInELSubSeq + 1) ==
          0) &&
         (p_Inp->NumberBFrames != 0) &&
         ((p_Vid->curr_frm_idx - p_Vid->last_idr_code_order) >
          0)) ||  // there are B frames
        (((p_Vid->curr_frm_idx - p_Vid->last_idr_code_order) %
              (p_Inp->NumFramesInELSubSeq + 1) ==
          p_Inp->NumFramesInELSubSeq) &&
         (p_Inp->NumberBFrames == 0))  // there are no B frames
    )
      p_SEI->seiSubseqInfo[currLayer].last_picture_flag = 1;
    else
      p_SEI->seiSubseqInfo[currLayer].last_picture_flag = 0;
...
static void FinalizeSubseqInfo(SEIParameters *p_SEI, int currLayer) {
  ...
  //  p_SEI->seiSubseqInfo[currLayer].last_picture_flag,
  //  p_SEI->seiSubseqInfo[currLayer].stored_frame_cnt );
}
...
static void FinalizeSubseqLayerInfo(SEIParameters *p_SEI) {
  ...
  for (i = 0; i < p_SEI->seiSubseqLayerInfo.layer_number; i++) {
    *((uint16 *)&(p_SEI->seiSubseqLayerInfo.data[pos])) =
        p_SEI->seiSubseqLayerInfo.bit_rate[i];
    pos += 2;
    *((uint16 *)&(p_SEI->seiSubseqLayerInfo.data[pos])) =
        p_SEI->seiSubseqLayerInfo.frame_rate[i];
    pos += 2;
...
void InitSubseqChar(VideoParameters *p_Vid) {
  ...
  if (p_SEI->seiSubseqChar.data == NULL)
    no_mem_exit("InitSubseqChar: p_SEI->seiSubseqChar.data");
  p_SEI->seiSubseqChar.data->streamBuffer = malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiSubseqChar.data->streamBuffer == NULL)
    no_mem_exit("InitSubseqChar: p_SEI->seiSubseqChar.data->streamBuffer");
...
void InitSceneInformation(SEIParameters *p_SEI) {
  ...
  if (p_SEI->seiSceneInformation.data == NULL)
    no_mem_exit("InitSceneInformation: seiSceneInformation.data");
  p_SEI->seiSceneInformation.data->streamBuffer = malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiSceneInformation.data->streamBuffer == NULL)
    no_mem_exit("InitSceneInformation: seiSceneInformation.data->streamBuffer");
...
// HasSceneInformation: To include a scene information SEI into the next
// slice/DP,
//      set HasSceneInformation to be TRUE when calling this function.
//      Otherwise, set HasSceneInformation to be FALSE.
void UpdateSceneInformation(SEIParameters *p_SEI, Boolean HasSceneInformation,
                            int sceneID, int sceneTransType,
                            int secondSceneID) {
...
static void InitPanScanRectInfo(SEIParameters *p_SEI) {
  ...
  if (p_SEI->seiPanScanRectInfo.data == NULL)
    no_mem_exit("InitPanScanRectInfo: p_SEI->seiPanScanRectInfo.data");
  p_SEI->seiPanScanRectInfo.data->streamBuffer = malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiPanScanRectInfo.data->streamBuffer == NULL)
    no_mem_exit(
        "InitPanScanRectInfo: p_SEI->seiPanScanRectInfo.data->streamBuffer");
  ...
  memset(p_SEI->seiPanScanRectInfo.pan_scan_rect_left_offset, 0,
         3 * sizeof(int));
  memset(p_SEI->seiPanScanRectInfo.pan_scan_rect_right_offset, 0,
         3 * sizeof(int));
  memset(p_SEI->seiPanScanRectInfo.pan_scan_rect_top_offset, 0,
         3 * sizeof(int));
  memset(p_SEI->seiPanScanRectInfo.pan_scan_rect_bottom_offset, 0,
         3 * sizeof(int));
...
void FinalizePanScanRectInfo(SEIParameters *p_SEI) {
  ...
  write_ue_v("SEI: pan_scan_rect_id",
             p_SEI->seiPanScanRectInfo.pan_scan_rect_id, bitstream);
  write_u_1("SEI: pan_scan_rect_cancel_flag",
            p_SEI->seiPanScanRectInfo.pan_scan_rect_cancel_flag, bitstream);
  if (!(p_SEI->seiPanScanRectInfo.pan_scan_rect_cancel_flag)) {
    write_ue_v("SEI: pan_scan_cnt_minus1",
               p_SEI->seiPanScanRectInfo.pan_scan_cnt_minus1, bitstream);
    for (i = 0; i <= p_SEI->seiPanScanRectInfo.pan_scan_cnt_minus1; i++) {
      write_se_v("SEI: pan_scan_rect_left_offset[X]",
                 p_SEI->seiPanScanRectInfo.pan_scan_rect_left_offset[i],
                 bitstream);
      write_se_v("SEI: pan_scan_rect_right_offset[X]",
                 p_SEI->seiPanScanRectInfo.pan_scan_rect_right_offset[i],
                 bitstream);
      write_se_v("SEI: pan_scan_rect_top_offset[X]",
                 p_SEI->seiPanScanRectInfo.pan_scan_rect_top_offset[i],
                 bitstream);
      write_se_v("SEI: pan_scan_rect_bottom_offset[X]",
                 p_SEI->seiPanScanRectInfo.pan_scan_rect_bottom_offset[i],
                 bitstream);
    }
    write_ue_v("SEI: pac_scan_rect_repetition_period",
               p_SEI->seiPanScanRectInfo.pac_scan_rect_repetition_period,
               bitstream);
  }
...
static void InitUser_data_unregistered(SEIParameters *p_SEI) {
  ...
  if (p_SEI->seiUser_data_unregistered.data == NULL)
    no_mem_exit(
        "InitUser_data_unregistered: p_SEI->seiUser_data_unregistered.data");
  p_SEI->seiUser_data_unregistered.data->streamBuffer =
      malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiUser_data_unregistered.data->streamBuffer == NULL)
    no_mem_exit(
        "InitUser_data_unregistered: "
        "p_SEI->seiUser_data_unregistered.data->streamBuffer");
  p_SEI->seiUser_data_unregistered.byte = malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiUser_data_unregistered.byte == NULL)
    no_mem_exit(
        "InitUser_data_unregistered: p_SEI->seiUser_data_unregistered.byte");
  ClearUser_data_unregistered(p_SEI);
}

static void ClearUser_data_unregistered(SEIParameters *p_SEI) {
  memset(p_SEI->seiUser_data_unregistered.data->streamBuffer, 0,
         MAXRTPPAYLOADLEN);
...
void InitUser_data_registered_itu_t_t35(SEIParameters *p_SEI) {
  ...
  if (p_SEI->seiUser_data_registered_itu_t_t35.data == NULL)
    no_mem_exit(
        "InitUser_data_unregistered: "
        "p_SEI->seiUser_data_registered_itu_t_t35.data");
  p_SEI->seiUser_data_registered_itu_t_t35.data->streamBuffer =
      malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiUser_data_registered_itu_t_t35.data->streamBuffer == NULL)
    no_mem_exit(
        "InitUser_data_unregistered: "
        "p_SEI->seiUser_data_registered_itu_t_t35.data->streamBuffer");
  ...
  if (p_SEI->seiUser_data_registered_itu_t_t35.data == NULL)
    no_mem_exit(
        "InitUser_data_unregistered: "
        "p_SEI->seiUser_data_registered_itu_t_t35.byte");
  ClearUser_data_registered_itu_t_t35(p_SEI);
}

static void ClearUser_data_registered_itu_t_t35(SEIParameters *p_SEI) {
  memset(p_SEI->seiUser_data_registered_itu_t_t35.data->streamBuffer, 0,
         MAXRTPPAYLOADLEN);
  ...
  p_SEI->seiUser_data_registered_itu_t_t35
...
void UpdateUser_data_registered_itu_t_t35(SEIParameters *p_SEI) {
  ...
  if (country_code < 0xFF) {
    ...
    p_SEI->seiUser_data_registered_itu_t_t35
        .itu_t_t35_country_code_extension_byte = country_code - 0xFF;
  }
...
// #define PRINT_USER_DATA_REGISTERED_ITU_T_T35_INFO
  ...
  if (p_SEI->seiUser_data_registered_itu_t_t35.itu_t_t35_country_code == 0xFF) {
    sym.bitpattern = p_SEI->seiUser_data_registered_itu_t_t35
...
static void InitRandomAccess(SEIParameters *p_SEI) {
  ...
  if (p_SEI->seiRecoveryPoint.data == NULL)
    no_mem_exit("InitRandomAccess: seiRandomAccess.data");
  p_SEI->seiRecoveryPoint.data->streamBuffer = malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiRecoveryPoint.data->streamBuffer == NULL)
    no_mem_exit("InitRandomAccess: seiRandomAccess.data->streamBuffer");
...
static void FinalizeRandomAccess(SEIParameters *p_SEI) {
  ...
  write_ue_v("SEI: recovery_frame_cnt",
             p_SEI->seiRecoveryPoint.recovery_frame_cnt, bitstream);
  write_u_1("SEI: exact_match_flag", p_SEI->seiRecoveryPoint.exact_match_flag,
            bitstream);
  write_u_1("SEI: broken_link_flag", p_SEI->seiRecoveryPoint.broken_link_flag,
            bitstream);
  write_u_v(2, "SEI: changing_slice_group_idc",
            p_SEI->seiRecoveryPoint.changing_slice_group_idc, bitstream);
 ...
 **++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 */
static int ParseToneMappingConfigFile(SEIParameters *p_SEI,
                                      InputParameters *p_Inp,
                                      ToneMappingSEI *pSeiToneMapping) {
  int i;
  int ret;
  FILE *fp;
  char buf[1024];
  ...
  if ((fp = fopen(p_Inp->ToneMappingFile, "r")) == NULL) {
    fprintf(
        stderr,
        "Tone mapping config file %s is not found, disable tone mapping SEI\n",
        p_Inp->ToneMappingFile);
    p_SEI->seiHasTone_mapping = FALSE;
  ...
  while (fscanf(fp, "%s", buf) != EOF) {
    ...
    // (model_id == 2)
      ...
      if (ret != 0) {
        error(
            "ParseToneMappingConfigFile: error parsing tone mapping config "
            "file",
            500);
      }
    ...
    if (ret != 1) {
      error(
          "ParseToneMappingConfigFile: error parsing tone mapping config file",
          500);
    }
  }
...
static void InitToneMapping(SEIParameters *p_SEI, InputParameters *p_Inp) {
  ...
  if (p_SEI->seiToneMapping.data == NULL)
    no_mem_exit("InitToneMapping: seiToneMapping.data");
  p_SEI->seiToneMapping.data->streamBuffer = malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiToneMapping.data->streamBuffer == NULL)
    no_mem_exit("InitToneMapping: seiToneMapping.data->streamBuffer");
...
static void FinalizeToneMapping(VideoParameters *p_Vid) {
  ...
  write_u_1("SEI: tone_map_cancel_flag",
            p_SEI->seiToneMapping.tone_map_cancel_flag, bitstream);
  ...
  if (!p_SEI->seiToneMapping.tone_map_cancel_flag) {
    write_ue_v("SEI: tone_map_repetition_period",
               p_SEI->seiToneMapping.tone_map_repetition_period, bitstream);
    write_u_v(8, "SEI: coded_data_bit_depth",
              p_SEI->seiToneMapping.coded_data_bit_depth, bitstream);
    write_u_v(8, "SEI: sei_bit_depth", p_SEI->seiToneMapping.sei_bit_depth,
              bitstream);
...
#endif
    if (p_SEI->seiToneMapping.model_id == 0) {  // linear mapping
      write_u_v(32, "SEI: min_value", p_SEI->seiToneMapping.min_value,
                bitstream);
      write_u_v(32, "SEI: min_value", p_SEI->seiToneMapping.max_value,
                bitstream);
#ifdef PRINT_TONE_MAPPING
      printf("min_value = %d, max_value = %d\n",
             p_SEI->seiToneMapping.min_value, p_SEI->seiToneMapping.max_value);
#endif
    } else if (p_SEI->seiToneMapping.model_id == 1) {  // sigmoidal mapping
      write_u_v(32, "SEI: sigmoid_midpoint",
                p_SEI->seiToneMapping.sigmoid_midpoint, bitstream);
      write_u_v(32, "SEI: sigmoid_width", p_SEI->seiToneMapping.sigmoid_width,
                bitstream);
#ifdef PRINT_TONE_MAPPING
      printf("sigmoid_midpoint = %d, sigmoid_width = %d\n",
             p_SEI->seiToneMapping.sigmoid_midpoint,
             p_SEI->seiToneMapping.sigmoid_width);
#endif
    } else if (p_SEI->seiToneMapping.model_id ==
               2) {  // user defined table mapping
      int bit_depth_val = 1 << p_SEI->seiToneMapping.sei_bit_depth;
      for (i = 0; i < bit_depth_val; i++) {
        write_u_v(
            (((p_SEI->seiToneMapping.coded_data_bit_depth + 7) >> 3) << 3),
            "SEI: start_of_coded_interval",
            p_SEI->seiToneMapping.start_of_coded_interval[i], bitstream);
#ifdef PRINT_TONE_MAPPING
        // printf("start_of_coded_interval[%d] = %d\n", i,
        // p_SEI->seiToneMapping.start_of_coded_interval[i]);
#endif
      }
    } else if (p_SEI->seiToneMapping.model_id ==
               3) {  // piece-wise linear mapping
      write_u_v(16, "SEI: num_pivots", p_SEI->seiToneMapping.num_pivots,
                bitstream);
#ifdef PRINT_TONE_MAPPING
      ...
      for (i = 0; i < p_SEI->seiToneMapping.num_pivots; i++) {
        write_u_v(
            (((p_SEI->seiToneMapping.coded_data_bit_depth + 7) >> 3) << 3),
            "SEI: coded_pivot_value",
            p_SEI->seiToneMapping.coded_pivot_value[i], bitstream);
        write_u_v((((p_SEI->seiToneMapping.sei_bit_depth + 7) >> 3) << 3),
                  "SEI: sei_pivot_value",
                  p_SEI->seiToneMapping.sei_pivot_value[i], bitstream);
#ifdef PRINT_TONE_MAPPING
        printf("coded_pivot_value[%d] = %d, sei_pivot_value[%d] = %d\n", i,
               p_SEI->seiToneMapping.coded_pivot_value[i], i,
               p_SEI->seiToneMapping.sei_pivot_value[i]);
#endif
      }
    }
...
static void InitPostFilterHints(SEIParameters *p_SEI) {
  ...
  if (p_SEI->seiPostFilterHints.data == NULL)
    no_mem_exit("InitPostFilterHints: p_SEI->seiPostFilterHints.data");
  p_SEI->seiPostFilterHints.data->streamBuffer = malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiPostFilterHints.data->streamBuffer == NULL)
    no_mem_exit(
        "InitPostFilterHints: p_SEI->seiPostFilterHints.data->streamBuffer");
...
void UpdatePostFilterHints(SEIParameters *p_SEI) {
  ...
  p_SEI->seiPostFilterHints.filter_hint_type =
      0;  // define filter_hint_type here
  p_SEI->seiPostFilterHints.filter_hint_size_y =
      p_SEI->seiPostFilterHints.filter_hint_size_x =
          5;  // define filter_hint_size here
  get_mem3Dint(&p_SEI->seiPostFilterHints.filter_hint, 3,
               p_SEI->seiPostFilterHints.filter_hint_size_y,
               p_SEI->seiPostFilterHints.filter_hint_size_x);
  ...
  for (color_component = 0; color_component < 3; color_component++)
    for (cy = 0; cy < p_SEI->seiPostFilterHints.filter_hint_size_y; cy++)
      for (cx = 0; cx < p_SEI->seiPostFilterHints.filter_hint_size_x; cx++)
        p_SEI->seiPostFilterHints.filter_hint[color_component][cy][cx] =
            1;  // define filter_hint here
...
static void FinalizePostFilterHints(SEIParameters *p_SEI) {
  ...
  write_ue_v("SEI: post_filter_hint_size_y",
             p_SEI->seiPostFilterHints.filter_hint_size_y, bitstream);
  write_ue_v("SEI: post_filter_hint_size_x",
             p_SEI->seiPostFilterHints.filter_hint_size_x, bitstream);
  write_u_v(2, "SEI: post_filter_hint_type",
            p_SEI->seiPostFilterHints.filter_hint_type, bitstream);

  for (color_component = 0; color_component < 3; color_component++)
    for (cy = 0; cy < p_SEI->seiPostFilterHints.filter_hint_size_y; cy++)
      for (cx = 0; cx < p_SEI->seiPostFilterHints.filter_hint_size_x; cx++)
        write_se_v(
            "SEI: post_filter_hints",
            p_SEI->seiPostFilterHints.filter_hint[color_component][cy][cx],
            bitstream);

  write_u_1("SEI: post_filter_additional_extension_flag",
            p_SEI->seiPostFilterHints.additional_extension_flag, bitstream);
...
void ClearBufferingPeriod(SEIParameters *p_SEI,
  ...
  p_SEI->seiBufferingPeriod.seq_parameter_set_id =
      active_sps->seq_parameter_set_id;
  if (active_sps->vui_seq_parameters.nal_hrd_parameters_present_flag) {
    for (SchedSelIdx = 0;
         SchedSelIdx <=
         active_sps->vui_seq_parameters.nal_hrd_parameters.cpb_cnt_minus1;
         SchedSelIdx++) {
      p_SEI->seiBufferingPeriod.nal_initial_cpb_removal_delay[SchedSelIdx] = 0;
      p_SEI->seiBufferingPeriod
          .nal_initial_cpb_removal_delay_offset[SchedSelIdx] = 0;
    }
  }
  if (active_sps->vui_seq_parameters.vcl_hrd_parameters_present_flag) {
    for (SchedSelIdx = 0;
         SchedSelIdx <=
         active_sps->vui_seq_parameters.vcl_hrd_parameters.cpb_cnt_minus1;
         SchedSelIdx++) {
      p_SEI->seiBufferingPeriod.vcl_initial_cpb_removal_delay[SchedSelIdx] = 0;
      p_SEI->seiBufferingPeriod
          .vcl_initial_cpb_removal_delay_offset[SchedSelIdx] = 0;
    }
  }
...
static void FinalizeBufferingPeriod(SEIParameters *p_SEI,
                                    seq_parameter_set_rbsp_t *active_sps) {
  unsigned int SchedSelIdx;
  Bitstream *bitstream = p_SEI->seiBufferingPeriod.data;

  write_ue_v("SEI: seq_parameter_set_id",
             p_SEI->seiBufferingPeriod.seq_parameter_set_id, bitstream);
  if (active_sps->vui_seq_parameters.nal_hrd_parameters_present_flag) {
    for (SchedSelIdx = 0;
         SchedSelIdx <=
         active_sps->vui_seq_parameters.nal_hrd_parameters.cpb_cnt_minus1;
         SchedSelIdx++) {
      write_u_v(
          active_sps->vui_seq_parameters.nal_hrd_parameters
                  .initial_cpb_removal_delay_length_minus1 +
              1,
          "SEI: initial_cpb_removal_delay",
          p_SEI->seiBufferingPeriod.nal_initial_cpb_removal_delay[SchedSelIdx],
          bitstream);
      write_u_v(active_sps->vui_seq_parameters.nal_hrd_parameters
                        .initial_cpb_removal_delay_length_minus1 +
                    1,
                "SEI: initial_cpb_removal_delay_offset",
                p_SEI->seiBufferingPeriod
                    .nal_initial_cpb_removal_delay_offset[SchedSelIdx],
                bitstream);
    }
  }
  if (active_sps->vui_seq_parameters.vcl_hrd_parameters_present_flag) {
    for (SchedSelIdx = 0;
         ...
         SchedSelIdx++) {
      write_u_v(
          active_sps->vui_seq_parameters.vcl_hrd_parameters
                  .initial_cpb_removal_delay_length_minus1 +
              1,
          "SEI: initial_cpb_removal_delay",
          p_SEI->seiBufferingPeriod.vcl_initial_cpb_removal_delay[SchedSelIdx],
          bitstream);
      write_u_v(active_sps->vui_seq_parameters.vcl_hrd_parameters
                        .initial_cpb_removal_delay_length_minus1 +
                    1,
                "SEI: initial_cpb_removal_delay_offset",
                p_SEI->seiBufferingPeriod
                    .vcl_initial_cpb_removal_delay_offset[SchedSelIdx],
                bitstream);
    }
  }
...
void ClearPicTiming(SEIParameters *p_SEI) {
  ...
  memset(p_SEI->seiPicTiming.clock_timestamp_flag, 0,
         MAX_PIC_STRUCT_VALUE * sizeof(Boolean));  // 0 == FALSE
 ...
 ************************************************************************
 */
static int get_pic_struct(VideoParameters *p_Vid, int frame_no,
                          int field_pic_flag, int top) {
  ...
  if (field_pic_flag) {
    ...
    if (!p_Inp->SEIVUI32Pulldown) {
      ...
      switch (p_Inp->SEIVUI32Pulldown) {
        default:
        case 1: return pic_struct_a[order]; break;
        case 2: return pic_struct_b[order]; break;
        case 3: return pic_struct_c[order]; break;
        case 4: return pic_struct_d[order]; break;
        case 5: return pic_struct_e[order]; break;
      }
    }
  }
}
...
void UpdatePicTiming(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_Inp->SEIVUI32Pulldown) {
    ...
    // pic_struct
    p_Vid->pic_struct = get_pic_struct(p_Vid, p_Vid->frm_no_in_file, fld_flag,
                                       (p_Vid->structure == TOP_FIELD) ? 1 : 0);
    ...
    if (active_sps->vui_seq_parameters.pic_struct_present_flag) {
      ...
      switch (p_SEI->seiPicTiming.pic_struct) {
        ...
        case 7:
          // frame doubling
          assert((p_Vid->fld_flag == FALSE) &&
                 active_sps->vui_seq_parameters.fixed_frame_rate_flag == 1);
          NumClockTS = 2;
          break;
        case 8:
          // frame tripling
...
static void FinalizePicTiming(VideoParameters *p_Vid) {
  ...
  if (CpbDpbDelaysPresentFlag) {
    write_u_v(hrd->cpb_removal_delay_length_minus1 + 1,
              "SEI: cpb_removal_delay", p_SEI->seiPicTiming.cpb_removal_delay,
              bitstream);
    write_u_v(hrd->dpb_output_delay_length_minus1 + 1, "SEI: dpb_output_delay",
              p_SEI->seiPicTiming.dpb_output_delay, bitstream);
  }
  if (active_sps->vui_seq_parameters.pic_struct_present_flag) {
    int NumClockTS = 0, i;
    ...
    for (i = 0; i < NumClockTS; i++) {
      write_u_1("SEI: clock_timestamp_flag",
                p_SEI->seiPicTiming.clock_timestamp_flag[i], bitstream);
      if (p_SEI->seiPicTiming.clock_timestamp_flag[i]) {
        write_u_v(2, "SEI: ct_type", p_SEI->seiPicTiming.ct_type, bitstream);
        write_u_1("SEI: nuit_field_based_flag",
                  p_SEI->seiPicTiming.nuit_field_based_flag, bitstream);
        write_u_v(5, "SEI: counting_type", p_SEI->seiPicTiming.counting_type,
                  bitstream);
        write_u_1("SEI: full_timestamp_flag",
                  p_SEI->seiPicTiming.full_timestamp_flag, bitstream);
        write_u_1("SEI: discontinuity_flag",
                  p_SEI->seiPicTiming.discontinuity_flag, bitstream);
        write_u_1("SEI: cnt_dropped_flag", p_SEI->seiPicTiming.cnt_dropped_flag,
                  bitstream);
        write_u_v(8, "SEI: n_frames", p_SEI->seiPicTiming.n_frames, bitstream);

        if (p_SEI->seiPicTiming.full_timestamp_flag) {
          write_u_v(6, "SEI: seconds_value", p_SEI->seiPicTiming.seconds_value,
                    bitstream);
          write_u_v(6, "SEI: minutes_value", p_SEI->seiPicTiming.minutes_value,
                    bitstream);
          write_u_v(5, "SEI: hours_value", p_SEI->seiPicTiming.hours_value,
                    bitstream);
        } else {
          write_u_1("SEI: seconds_flag", p_SEI->seiPicTiming.seconds_flag,
                    bitstream);
          if (p_SEI->seiPicTiming.seconds_flag) {
            write_u_v(6, "SEI: seconds_value",
                      p_SEI->seiPicTiming.seconds_value, bitstream);
            write_u_1("SEI: minutes_flag", p_SEI->seiPicTiming.minutes_flag,
                      bitstream);
            if (p_SEI->seiPicTiming.minutes_flag) {
              write_u_v(6, "SEI: minutes_value",
                        p_SEI->seiPicTiming.minutes_value, bitstream);
              write_u_1("SEI: hours_flag", p_SEI->seiPicTiming.hours_flag,
                        bitstream);
              if (p_SEI->seiPicTiming.hours_flag) {
                write_u_v(5, "SEI: hours_value",
                          p_SEI->seiPicTiming.hours_value, bitstream);
              }
            }
          }
        }
        if (hrd == NULL) {
          write_u_v(24, "SEI: time_offset", p_SEI->seiPicTiming.time_offset,
                    bitstream);
        } else {
          write_u_v(hrd->time_offset_length, "SEI: time_offset",
                    p_SEI->seiPicTiming.time_offset, bitstream);
        }
      }
    }
  }
...
static void InitFramePackingArrangement(VideoParameters *p_Vid) {
  ...
  p_SEI->seiFramePackingArrangement.data->streamBuffer =
      malloc(MAXRTPPAYLOADLEN);
  if (p_SEI->seiFramePackingArrangement.data->streamBuffer == NULL)
    no_mem_exit(
        "InitFramePackingArrangement: "
        "seiFramePackingArrangement.data->streamBuffer");

  ClearFramePackingArrangement(p_SEI);
}

void ClearFramePackingArrangement(SEIParameters *p_SEI) {
  memset(p_SEI->seiFramePackingArrangement.data->streamBuffer, 0,
         MAXRTPPAYLOADLEN);
  ...
  p_SEI->seiFramePackingArrangement.frame_packing_arrangement_cancel_flag =
      FALSE;
  ...
  p_SEI->seiFramePackingArrangement
      .frame_packing_arrangement_repetition_period = 0;
  p_SEI->seiFramePackingArrangement.frame_packing_arrangement_extension_flag =
      FALSE;

  p_SEI->seiHasFramePackingArrangement_info = FALSE;
}

void UpdateFramePackingArrangement(VideoParameters *p_Vid,
                                   InputParameters *p_Inp) {
  SEIParameters *p_SEI = p_Vid->p_SEI;
  ...
  if (p_Inp->SEIFPAType != -1) {
    ...
    p_SEI->seiFramePackingArrangement.frame_packing_arrangement_type =
        p_Inp->SEIFPAType;
  }
}
...
static void FinalizeFramePackingArrangement(SEIParameters *p_SEI,
  ...
  write_ue_v(
      "SEI: frame_packing_arrangement_id",
      (int)p_SEI->seiFramePackingArrangement.frame_packing_arrangement_id,
      bitstream);
  write_u_1("SEI: frame_packing_arrangement_cancel_flag",
            (int)p_SEI->seiFramePackingArrangement
                .frame_packing_arrangement_cancel_flag,
            bitstream);
  if (p_SEI->seiFramePackingArrangement.frame_packing_arrangement_cancel_flag ==
      FALSE) {
    write_u_v(
        7, "SEI: frame_packing_arrangement_type",
        (int)p_SEI->seiFramePackingArrangement.frame_packing_arrangement_type,
        bitstream);
    write_u_1("SEI: quincunx_sampling_flag",
              (int)p_SEI->seiFramePackingArrangement.quincunx_sampling_flag,
              bitstream);
    write_u_v(
        6, "SEI: content_interpretation_type",
        (int)p_SEI->seiFramePackingArrangement.content_interpretation_type,
        bitstream);
    write_u_1("SEI: spatial_flipping_flag",
              (int)p_SEI->seiFramePackingArrangement.spatial_flipping_flag,
              bitstream);
    write_u_1("SEI: frame0_flipped_flag",
              (int)p_SEI->seiFramePackingArrangement.frame0_flipped_flag,
              bitstream);
    write_u_1("SEI: field_views_flag",
              (int)p_SEI->seiFramePackingArrangement.field_views_flag,
              bitstream);
    write_u_1(
        "SEI: current_frame_is_frame0_flag",
        (int)p_SEI->seiFramePackingArrangement.current_frame_is_frame0_flag,
        bitstream);
    write_u_1("SEI: frame0_self_contained_flag",
              (int)p_SEI->seiFramePackingArrangement.frame0_self_contained_flag,
              bitstream);
    write_u_1("SEI: frame1_self_contained_flag",
              (int)p_SEI->seiFramePackingArrangement.frame1_self_contained_flag,
              bitstream);
    if (p_SEI->seiFramePackingArrangement.quincunx_sampling_flag == FALSE &&
        p_SEI->seiFramePackingArrangement.frame_packing_arrangement_type != 5) {
      write_u_v(4, "SEI: frame0_grid_position_x",
                (int)p_SEI->seiFramePackingArrangement.frame0_grid_position_x,
                bitstream);
      write_u_v(4, "SEI: frame0_grid_position_y",
                (int)p_SEI->seiFramePackingArrangement.frame0_grid_position_y,
                bitstream);
      write_u_v(4, "SEI: frame1_grid_position_x",
                (int)p_SEI->seiFramePackingArrangement.frame1_grid_position_x,
                bitstream);
      write_u_v(4, "SEI: frame1_grid_position_y",
                (int)p_SEI->seiFramePackingArrangement.frame1_grid_position_y,
                bitstream);
    }

    write_u_v(8, "SEI: frame_packing_arrangement_reserved_byte",
              (int)p_SEI->seiFramePackingArrangement
                  .frame_packing_arrangement_reserved_byte,
              bitstream);
    write_ue_v("SEI: frame_packing_arrangement_repetition_period",
               (int)p_SEI->seiFramePackingArrangement
                   .frame_packing_arrangement_repetition_period,
               bitstream);
  }
  write_u_1("SEI: frame_packing_arrangement_extension_flag",
            (int)p_SEI->seiFramePackingArrangement
...
static void InitDRPMRepetition(SEIParameters *p_SEI) {
  ...
  if (p_SEI->seiDRPMRepetition.data->streamBuffer == NULL)
    no_mem_exit(
        "InitDRPMRepetition: p_SEI->seiDRPMRepetition.data->streamBuffer");
...
static void FinalizeDRPMRepetition(VideoParameters *p_Vid) {
  ...
  // SEI message bits
  write_u_1("SEI: original_idr_flag",
            p_SEI->seiDRPMRepetition.original_idr_flag, bitstream);
  write_ue_v("SEI: original_frame_num",
             p_SEI->seiDRPMRepetition.original_frame_num, bitstream);
  if (!active_sps->frame_mbs_only_flag) {
    write_u_1("SEI: original_field_pic_flag",
              p_SEI->seiDRPMRepetition.original_field_pic_flag, bitstream);
    if (p_SEI->seiDRPMRepetition.original_field_pic_flag) {
      write_u_1("SEI: original_bottom_field_flag",
                p_SEI->seiDRPMRepetition.original_bottom_field_flag, bitstream);
    }
  }
...
void PrepareAggregationSEIMessage(VideoParameters *p_Vid) {
  ...
  if (p_SEI->seiHasSparePicture && p_Vid->type != B_SLICE) {
    ...
    assert(p_SEI->seiSparePicturePayload.data->byte_pos ==
           p_SEI->seiSparePicturePayload.payloadSize);
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiSparePicturePayload.data->streamBuffer,
                      p_SEI->seiSparePicturePayload.payloadSize, SEI_SPARE_PIC);
    has_aggregation_sei_message = TRUE;
  }
  // write the sub sequence information sei paylaod to the aggregation sei
  // message
  if (p_SEI->seiHasSubseqInfo) {
    FinalizeSubseqInfo(p_SEI, p_Vid->layer);
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiSubseqInfo[p_Vid->layer].data->streamBuffer,
                      p_SEI->seiSubseqInfo[p_Vid->layer].payloadSize,
                      SEI_SUB_SEQ_INFO);
    ClearSubseqInfoPayload(p_SEI, p_Vid->layer);
    has_aggregation_sei_message = TRUE;
  }
  // write the sub sequence layer information sei paylaod to the aggregation sei
  // message
  if (p_SEI->seiHasSubseqLayerInfo && p_Vid->number == 0) {
    FinalizeSubseqLayerInfo(p_SEI);
    write_sei_message(p_SEI, AGGREGATION_SEI, p_SEI->seiSubseqLayerInfo.data,
                      p_SEI->seiSubseqLayerInfo.payloadSize,
                      SEI_SUB_SEQ_LAYER_CHARACTERISTICS);
    p_SEI->seiHasSubseqLayerInfo = FALSE;
    has_aggregation_sei_message = TRUE;
  }
  // write the sub sequence characteristics payload to the aggregation sei
  // message
  if (p_SEI->seiHasSubseqChar) {
    FinalizeSubseqChar(p_SEI);
    write_sei_message(
        p_SEI, AGGREGATION_SEI, p_SEI->seiSubseqChar.data->streamBuffer,
        p_SEI->seiSubseqChar.payloadSize, SEI_SUB_SEQ_CHARACTERISTICS);
    ClearSubseqCharPayload(p_SEI);
    has_aggregation_sei_message = TRUE;
  }
  // write the pan scan rectangle info sei playload to the aggregation sei
  // message
  if (p_SEI->seiHasPanScanRectInfo) {
    FinalizePanScanRectInfo(p_SEI);
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiPanScanRectInfo.data->streamBuffer,
                      p_SEI->seiPanScanRectInfo.payloadSize, SEI_PAN_SCAN_RECT);
    ClearPanScanRectInfoPayload(p_SEI);
    has_aggregation_sei_message = TRUE;
  }
  // write the arbitrary (unregistered) info sei playload to the aggregation sei
  // message
  if (p_SEI->seiHasUser_data_unregistered_info) {
    FinalizeUser_data_unregistered(p_SEI);
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiUser_data_unregistered.data->streamBuffer,
                      p_SEI->seiUser_data_unregistered.payloadSize,
  ...
  if (p_SEI->seiHasUser_data_registered_itu_t_t35_info) {
    ...
    write_sei_message(
        p_SEI, AGGREGATION_SEI,
        p_SEI->seiUser_data_registered_itu_t_t35.data->streamBuffer,
        p_SEI->seiUser_data_registered_itu_t_t35.payloadSize,
  ...
  if (p_SEI->seiHasSceneInformation) {
    ...
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiSceneInformation.data->streamBuffer,
                      p_SEI->seiSceneInformation.payloadSize, SEI_SCENE_INFO);
    has_aggregation_sei_message = TRUE;
  }
  ...
  if (p_SEI->seiHasTone_mapping) {
    ...
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiToneMapping.data->streamBuffer,
                      p_SEI->seiToneMapping.payloadSize, SEI_TONE_MAPPING);
  ...
  if (p_SEI->seiHasPostFilterHints_info) {
    ...
    write_sei_message(
        p_SEI, AGGREGATION_SEI, p_SEI->seiPostFilterHints.data->streamBuffer,
        p_SEI->seiPostFilterHints.payloadSize, SEI_POST_FILTER_HINTS);
    has_aggregation_sei_message = TRUE;
  }
  ...
  if (p_SEI->seiHasBufferingPeriod_info) {
    ...
    write_sei_message(
        p_SEI, AGGREGATION_SEI, p_SEI->seiBufferingPeriod.data->streamBuffer,
        p_SEI->seiBufferingPeriod.payloadSize, SEI_BUFFERING_PERIOD);
    has_aggregation_sei_message = TRUE;
  }
  ...
  if (p_SEI->seiHasPicTiming_info) {
    ...
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiPicTiming.data->streamBuffer,
                      p_SEI->seiPicTiming.payloadSize, SEI_PIC_TIMING);
    has_aggregation_sei_message = TRUE;
  }
  ...
  if (p_SEI->seiHasRecoveryPoint_info) {
    ...
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiRecoveryPoint.data->streamBuffer,
                      p_SEI->seiRecoveryPoint.payloadSize, SEI_RECOVERY_POINT);
    ClearRandomAccess(p_SEI);
    has_aggregation_sei_message = TRUE;
  }
  ...
  if (p_SEI->seiHasDRPMRepetition_info) {
    ...
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiDRPMRepetition.data->streamBuffer,
                      p_SEI->seiDRPMRepetition.payloadSize,
  ...
  if (p_SEI->seiHasFramePackingArrangement_info) {
    ...
    write_sei_message(p_SEI, AGGREGATION_SEI,
                      p_SEI->seiFramePackingArrangement.data->streamBuffer,
                      p_SEI->seiFramePackingArrangement.payloadSize,
