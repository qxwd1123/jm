...
// The following are local helpers, but may come handy in the future, hence
// public
extern void GenerateSequenceParameterSet(seq_parameter_set_rbsp_t *sps,
                                         VideoParameters *p_Vid, int SPS_id);
extern void GeneratePictureParameterSet(
    pic_parameter_set_rbsp_t *pps, seq_parameter_set_rbsp_t *sps,
    VideoParameters *p_Vid, InputParameters *p_Inp, int PPS_id,
    int WeightedPrediction, int WeightedBiprediction, int cb_qp_index_offset,
    int cr_qp_index_offset);

extern int Scaling_List(short *scalingListinput, short *scalingList,
                        int sizeOfScalingList, short *UseDefaultScalingMatrix,
                        Bitstream *bitstream);
#if (MVC_EXTENSION_ENABLE)
extern int GenerateSeq_parameter_set_rbsp(VideoParameters *p_Vid,
                                          seq_parameter_set_rbsp_t *sps,
                                          byte *buf, short Is_Subset);
#else
extern int GenerateSeq_parameter_set_rbsp(VideoParameters *p_Vid,
                                          seq_parameter_set_rbsp_t *sps,
                                          byte *buf);
#endif
extern int GeneratePic_parameter_set_rbsp(VideoParameters *p_Vid,
...
extern int WriteHRDParameters(seq_parameter_set_rbsp_t *sps,
                              Bitstream *bitstream);
extern void GenerateVUIParameters(seq_parameter_set_rbsp_t *sps,
