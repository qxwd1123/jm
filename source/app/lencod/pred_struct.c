...
static inline void pop_pred_struct_frm(PredStructFrm *p_dst, int slice_type,
                                       int nal_ref_idc, int disp_offset,
                                       int layer, int slice_qp_off,
                                       int random_access, int temporal_layer);
static void init_gop_struct(InputParameters *p_Inp, SeqStructure *p_seq_struct,
                            int is_idr, int *memory_size);
static void free_gop_struct(SeqStructure *p_seq_struct);
static void init_pred_struct(VideoParameters *p_Vid, InputParameters *p_Inp,
                             SeqStructure *p_seq_struct, int *memory_size);
...
static FrameUnitStruct *alloc_frame_buffer(InputParameters *p_Inp,
                                           int num_frames, int num_slices,
                                           int *memory_size);
...
static int establish_random_access(InputParameters *p_Inp,
                                   SeqStructure *p_seq_struct, int curr_frame,
                                   int avail_frames, int sim);
static int establish_intra(InputParameters *p_Inp, SeqStructure *p_seq_struct,
                           int curr_frame, int avail_frames, int sim);
static int establish_sp(InputParameters *p_Inp, SeqStructure *p_seq_struct,
                        int curr_frame, int avail_frames, int sim);
static int get_fixed_frame(InputParameters *p_Inp, SeqStructure *p_seq_struct,
                           int curr_frame, int avail_frames);
static int get_prd_index(InputParameters *p_Inp, SeqStructure *p_seq_struct,
                         int num_frames);
static int get_idr_index(InputParameters *p_Inp, SeqStructure *p_seq_struct,
                         int num_frames);
static int get_intra_index(InputParameters *p_Inp, SeqStructure *p_seq_struct,
                           int num_frames);
static void populate_frame(InputParameters *p_Inp, SeqStructure *p_seq_struct,
                           FrameUnitStruct *p_frm_struct,
                           PredStructAtom *p_cur_prd, int curr_frame,
                           int pred_frame, int idx, int num_slices,
                           int no_rnd_acc);
static void update_frame_indices(SeqStructure *p_seq_struct,
                                 FrameUnitStruct *p_frm_struct, int curr_frame,
                                 int pred_frame);
static int populate_reg_pred_struct_atom(VideoParameters *p_Vid,
                                         InputParameters *p_Inp,
                                         SeqStructure *p_seq_struct,
                                         int curr_frame, int proc_frames,
                                         int *terminate_pop);
static int populate_rnd_acc_pred_struct_atom(VideoParameters *p_Vid,
                                             InputParameters *p_Inp,
                                             SeqStructure *p_seq_struct,
                                             int curr_frame, int proc_frames,
                                             int *terminate_pop, int is_idr,
                                             int no_rnd_acc);
#if (MVC_EXTENSION_ENABLE)
static void copy_frame_mvc(InputParameters *p_Inp, FrameUnitStruct *p_src,
                           FrameUnitStruct *p_dst, int num_slices, int view_id);
#endif
...
void get_poc_type_zero(VideoParameters *p_Vid, InputParameters *p_Inp,
                       FrameUnitStruct *p_frm_struct) {
  FrameUnitStruct *p_cur =
      p_frm_struct;  // + (curr_frm_idx % p_Vid->frm_struct_buffer);
  int last_disp_order;
...
void get_poc_type_one(VideoParameters *p_Vid, InputParameters *p_Inp,
                      FrameUnitStruct *p_frm_struct) {
  ...
  // we need to get type 0 values first and then estimate delta_pic_order_cnt
  // with respect to those
  get_poc_type_zero(p_Vid, p_Inp, p_frm_struct);

  // next we use the decoder POC Type 1 code to determine the
  // delta_pic_order_cnt
  ...
  if (p_cur->idr_flag) {
    ...
    if (p_Vid->frame_num < p_EncodePar->prevFrameNum) {
      p_Vid->FrameNumOffset =
          p_EncodePar->prevFrameNumOffset + p_Vid->max_frame_num;
    } else {
  ...
  if (AbsFrameNum) {
    PicOrderCntCycleCnt =
        (AbsFrameNum - 1) / p_Vid->num_ref_frames_in_pic_order_cnt_cycle;
  ...
  p_Vid->prevFrameNumOffset = p_EncodePar->prevFrameNumOffset =
      p_Vid->FrameNumOffset;
}
 ...
 ***********************************************************************
 * \brief
 *    Initializes POC type 0 and 1: POC type 1 simulates the decoder's behavior
 *and sets values \n given the POCs calculated for POC type 0; needs to be
 *tested \param p_Vid pointer to the VideoParameters structure
 ***********************************************************************
 */
...
static PicStructure *alloc_pic_struct(int num_slices, int *memory_size) {
  ...
  if (p_pic == NULL) {
    fprintf(stderr,
            "\nNot enough memory or error during memory allocation: "
            "alloc_pic_struct()");
    exit(1);
  }
  *memory_size += sizeof(PicStructure);
  ...
  p_pic->p_Slice =
      (SliceStructure *)malloc(num_slices * sizeof(SliceStructure));
 ...
 * \param num_slices
 *    max number of slices used to code each frame unit in the cyclical frame
 *buffer \param memory_size pointer to the memory_size variable to count the
 *number of bytes allocated \return pointer to the allocated FrameUnitStruct
 ***********************************************************************
 */
...
static FrameUnitStruct *alloc_frame_buffer(InputParameters *p_Inp,
  ...
  // FrameUnitStruct
  FrameUnitStruct *p_frm_struct =
      (FrameUnitStruct *)malloc(num_frames * sizeof(FrameUnitStruct));
 ...
 *    pointer to the InputParameters structure
 * \param memory_size
 *    pointer to the memory_size variable that stores the number of bytes
 *allocated so far \return pointer to SeqStructure
 ***********************************************************************
 */

SeqStructure *init_seq_structure(VideoParameters *p_Vid, InputParameters *p_Inp,
                                 int *memory_size) {
#if (MVC_EXTENSION_ENABLE)
  int start, end;
#endif
  ...
  p_Inp->FrmStructBufferLength =
      imax(p_Inp->FrmStructBufferLength,
           imax(p_Inp->idr_period, p_Inp->intra_period) + p_Inp->NumberBFrames +
               p_Inp->intra_delay + 2);
  p_Vid->frm_struct_buffer = p_Inp->FrmStructBufferLength;
  p_Vid->frm_struct_buffer = imin(p_Vid->frm_struct_buffer, p_Inp->no_frames);
  p_Inp->FrmStructBufferLength =
      imin(p_Inp->FrmStructBufferLength, p_Inp->no_frames);
  ...
  *memory_size += sizeof(SeqStructure);
  ...
  p_seq_struct->max_num_slices =
      (p_Inp->slice_mode == 1)
          ? imax(1, p_Vid->FrameSizeInMbs / p_Inp->slice_argument)
          : 1;
  p_seq_struct->num_frames = p_Vid->frm_struct_buffer;
  p_seq_struct->p_frm =
      alloc_frame_buffer(p_Inp, p_seq_struct->num_frames,
                         p_seq_struct->max_num_slices, memory_size);
...
#if (MVC_EXTENSION_ENABLE)
  p_seq_struct->num_frames_mvc =
      p_seq_struct->num_frames *
      p_Inp->num_of_views;  // two views hence twice the buffer size
  p_seq_struct->p_frm_mvc =
      alloc_frame_buffer(p_Inp, p_seq_struct->num_frames_mvc,
                         p_seq_struct->max_num_slices, memory_size);
#endif
  ...
  populate_frm_struct(p_Vid, p_Inp, p_seq_struct, frames_to_pop,
                      p_Inp->no_frames);
 ...
 ***********************************************************************
 * \brief
 *    Establish whether the frame with coding order "curr_frame" will be coded
 *as an IDR picture \param p_Inp pointer to the InputParameters structure \param
 *p_seq_struct pointer to the sequence structure \param curr_frame coding order
 *of the current frame \param avail_frames frames available for population
 * \param sim
 ...
 *    if PreferDispOrder == 1 then it returns 1 + disp_offset (for the random
 *access frame)
 ***********************************************************************
 */
...
static int establish_random_access(InputParameters *p_Inp,
  ...
  if (!curr_frame)  // first frame in coding order
  {
    if (!(p_Inp->PreferDispOrder))  // coding order
    {
      ...
      // test each random access prediction structure, starting from the longest
      // one if random access is established then break from this loop and
    ...
    if (!(p_Inp->PreferDispOrder))  // coding order
    {
      switch (p_Inp->adaptive_idr_period) {
        ...
        case 1:
          ...
          break;
        case 2:
          delta = curr_frame - imax(p_seq_struct->last_random_access_frame,
                                    p_seq_struct->last_intra_frame);
          if (delta >= p_Inp->idr_period && delta >= p_Inp->MinIDRDistance) {
            is_random_access = 1;
          }
          break;
    ...
    } else  // display order (insertion)
    {
      ...
      for (idx = (p_seq_struct->num_gops - 1); idx >= 0; idx--) {
        ...
        // recall that frame "0" is the random access frame in those prediction
        // structures
        ...
        switch (p_Inp->adaptive_idr_period) {
          default:
          case 0:
            if (!curr_frame || (curr_frame + offset - p_Inp->intra_delay) %
                                       p_Inp->idr_period ==
                                   0) {
              is_random_access = 1 + idx;
            }
          ...
          case 1:
            ...
            if (!curr_frame || (delta >= p_Inp->idr_period &&
                                delta >= p_Inp->MinIDRDistance)) {
              is_random_access = 1 + idx;
            }
            break;
          case 2:
            delta = (curr_frame + offset) -
                    imax(p_seq_struct->last_rand_access_disp,
                         p_seq_struct->last_intra_disp);
            if (!curr_frame || (delta >= p_Inp->idr_period &&
                                delta >= p_Inp->MinIDRDistance)) {
              is_random_access = 1 + idx;
            }
  ...
  // additional case can be added to insert IDR based on *pre-analysis* and
  // other "pre-scient" tools
 ...
 * \brief
 *    Establish whether the frame with coding order "curr_frame" will be coded
 *as an INTRA picture \param p_Inp pointer to the InputParameters structure
 * \param p_seq_struct
 ...
 *    if PreferDispOrder == 1 then it returns 1 + disp_offset (for the intra
 *frame)
 ***********************************************************************
 */
...
static int establish_intra(InputParameters *p_Inp, SeqStructure *p_seq_struct,
  ...
  if (!curr_frame)  // first frame in coding order
  {
    ...
    if (!(p_Inp->PreferDispOrder))  // coding order
    {
      switch (p_Inp->adaptive_intra_period) {
        ...
        case 0:
          ...
          break;
        case 1:
          if ((curr_frame - imax(p_seq_struct->last_idr_frame,
                                 p_seq_struct->last_intra_frame)) %
                  p_Inp->intra_period ==
              0) {
            is_intra = 1;
    ...
    } else  // display order (insertion)
    {
      ...
      // test each random access prediction structure, staring from the longest
      // one if random access is established then break from this loop and
      // prepare output parameter
      for (idx = (p_seq_struct->num_intra_gops - 1); idx >= 0; idx--) {
        ...
        switch (p_Inp->adaptive_intra_period) {
          default:
          case 0:
            // intra_period operates as if EnableIDRGOP=0 which means that the
            // criteria for its insertion, when inserting based on display order
            // may be satisfied before (in terms of coding order) an IDR_period
            // with EnableIDRGOP=1 (for 0 there is no issue) this code ensures
            // that IDRPeriod always takes precedence over IntraPeriod
            if ((curr_frame + offset) % p_Inp->intra_period == 0 &&
                rand_acc_test_range(curr_frame, offset, p_Inp->idr_period)) {
              is_intra = 1 + idx;
            }
            break;
          case 2:
            // see comment above
            if (((curr_frame + offset) - imax(p_seq_struct->last_idr_disp,
                                              p_seq_struct->last_intra_disp)) %
                        p_Inp->intra_period ==
                    0 &&
                rand_acc_test_range(
                    curr_frame - imax(p_seq_struct->last_idr_disp,
                                      p_seq_struct->last_intra_disp),
                    offset, p_Inp->idr_period)) {
              is_intra = 1 + idx;
            }
            break;
        }
 ...
 * \brief
 *    Establish whether the frame with coding order "curr_frame" will be coded
 *as an SP picture \param p_Inp pointer to the InputParameters structure \param
 *p_seq_struct pointer to the sequence structure \param curr_frame coding order
 *of the current frame \param avail_frames frames available for population
 * \return
...
static int establish_sp(InputParameters *p_Inp, SeqStructure *p_seq_struct,
  ...
  if (p_Inp->sp_periodicity) {
    if (!(p_Inp->PreferDispOrder))  // coding order
    {
      if ((curr_frame - p_seq_struct->last_sp_frame) % p_Inp->sp_periodicity ==
          0) {
        is_sp = 1;
      }
    } else  // display order (insertion)
    {
      ...
      for (idx = (p_seq_struct->num_gops - 1); idx >= 0; idx--) {
        ...
        // now test if random access conditions are satisfied
        if ((curr_frame + offset - p_seq_struct->last_sp_disp) %
                p_Inp->sp_periodicity ==
            0) {
          is_sp = 1 + idx;
          break;
        }
 ...
 ***********************************************************************
 * \brief
 *    Find closest fixed frame (IDR, intra, or slice set through some
 *pre-analysis tool) \param p_Inp pointer to the InputParameters structure
 * \param p_seq_struct
 *    pointer to the sequence structure
 * \param curr_frame
 *    coding order of the current frame
...
static int get_fixed_frame(InputParameters *p_Inp, SeqStructure *p_seq_struct,
  ...
  while (idx < avail_frames) {
    is_rnd_acc = establish_random_access(p_Inp, p_seq_struct, curr_frame + idx,
                                         avail_frames - idx, 1);
    is_intra = establish_intra(p_Inp, p_seq_struct, curr_frame + idx,
                               avail_frames - idx, 1);
    is_sp = establish_sp(p_Inp, p_seq_struct, curr_frame + idx,
                         avail_frames - idx, 1);
  ...
  // signal that there is no fixed frame in the processed frames buffer
  // (functions have to interpret the negative output here)
  return -1;
}
 ...
 * \brief
 *    Find closest fixed frame (IDR, intra, or slice set through some
 *pre-analysis tool) for regular prediction structures and then try place the
 *longest possible structure... \param p_Inp pointer to the InputParameters
 *structure \param p_seq_struct pointer to the sequence structure \param
 *curr_frame coding order of the current frame \param avail_frames frames
 *available for population \return returns the coding order of the first "fixed"
 *frame, if none then -1
 ***********************************************************************
 */

static int get_fixed_frame_for_prd(InputParameters *p_Inp,
                                   SeqStructure *p_seq_struct, int curr_frame,
                                   int avail_frames) {
  int idx = 0;
  int is_rnd_acc = 0;
  int is_intra = 0;
  int is_sp = 0;
  PredStructAtom *p_longest_prd =
      p_seq_struct->p_prd + p_seq_struct->num_prds - 1;
  ...
  while (idx < avail_frames) {
    ...
    if (is_rnd_acc) {
      if (p_Inp->PreferDispOrder && !(p_Inp->EnableIDRGOP)) {
        ...
        // consider the case where even though there were enough available
        // frames to a large structure, these slots instead were allocated to a
        // very long intra structure, which reduces compression efficiency
...
static int get_prd_index(InputParameters *p_Inp, SeqStructure *p_seq_struct,
  ...
  for (curr_prd = (p_seq_struct->num_prds - 1); curr_prd >= 0; curr_prd--) {
    ...
    if (p_Inp->PreferPowerOfTwo && curr_prd != (p_seq_struct->num_prds - 1) &&
        !check_power_of_two(p_cur_prd->length)) {
      continue;
    }
...
static int get_idr_index(InputParameters *p_Inp, SeqStructure *p_seq_struct,
  ...
  for (curr_gop = (p_seq_struct->num_gops - 1); curr_gop >= 0; curr_gop--) {
    ...
    if (p_Inp->PreferPowerOfTwo && curr_gop != (p_seq_struct->num_gops - 1) &&
        !check_power_of_two(p_cur_gop->length)) {
      continue;
    }
...
static int get_intra_index(InputParameters *p_Inp, SeqStructure *p_seq_struct,
  ...
  // allocate atoms
  for (curr_gop = (p_seq_struct->num_intra_gops - 1); curr_gop >= 0;
       curr_gop--) {
    p_cur_gop = p_seq_struct->p_intra_gop + curr_gop;
    if (p_Inp->PreferPowerOfTwo &&
        curr_gop != (p_seq_struct->num_intra_gops - 1) &&
        !check_power_of_two(p_cur_gop->length)) {
      continue;
    }
    if (p_cur_gop->length <= num_frames) {
...
static void init_gop_struct(InputParameters *p_Inp, SeqStructure *p_seq_struct,
  ...
  if (is_idr) {
    ...
    if (p_Inp->idr_period == 1 || p_Inp->intra_period == 1) {
      ...
      p_seq_struct->num_gops =
          !p_Inp->EnableIDRGOP ? p_seq_struct->num_prds : 2;
    } else {
      p_seq_struct->num_gops =
          !p_Inp->EnableIDRGOP ? p_seq_struct->num_prds : 1;
    }
    ...
    p_seq_struct->p_gop = (PredStructAtom *)malloc(p_seq_struct->num_gops *
                                                   sizeof(PredStructAtom));
  ...
  } else  // intra GOP
  {
    ...
    p_seq_struct->p_intra_gop = (PredStructAtom *)malloc(
        p_seq_struct->num_intra_gops * sizeof(PredStructAtom));
  ...
  *memory_size += length * sizeof(PredStructFrm);
  ...
  if (p_Inp->intra_delay && p_Inp->intra_period != 1 &&
      p_Inp->idr_period != 1) {
    int proc_frames = 0;
    ...
    *memory_size += length * sizeof(PredStructFrm);
    ...
    pop_pred_struct_frm(p_dst, I_SLICE, NALU_PRIORITY_HIGHEST,
                        p_Inp->intra_delay, 0, 0, 1, 0);

    idr_idx++;

    while (proc_frames < p_Inp->intra_delay) {
      // given the available slots select the index to the pred structure to use
      // for current segment
      curr_prd =
          get_prd_index(p_Inp, p_seq_struct, p_Inp->intra_delay - proc_frames);
      p_cur_prd = p_seq_struct->p_prd + curr_prd;
      // set gop_levels
      ...
      for (idx = 0; idx < p_cur_prd->length; idx++, idr_idx++, proc_frames++) {
        ...
        pop_pred_struct_frm(p_dst, p_src->slice_type, p_src->nal_ref_idc,
                            start_frame - p_src->disp_offset, p_src->layer,
    ...
    if (!p_Inp->EnableIDRGOP || !is_idr)  // an additional atom: the longest one
    {
      ...
      p_cur_gop->gop_levels = 1;
      p_cur_gop->p_frm =
          (PredStructFrm *)malloc(length * sizeof(PredStructFrm));
      *memory_size += length * sizeof(PredStructFrm);
      idr_idx = 0;
      ...
      while (proc_frames < p_Inp->intra_delay) {
        // given the available slots select the index to the pred structure to
        // use for current segment
        curr_prd = get_prd_index(p_Inp, p_seq_struct,
                                 p_Inp->intra_delay - proc_frames);
        p_cur_prd = p_seq_struct->p_prd + curr_prd;
        // set gop_levels
        p_cur_gop->gop_levels = p_cur_prd->gop_levels;
        // populate gop structure from selected structure
        for (idx = 0; idx < p_cur_prd->length;
             idx++, idr_idx++, proc_frames++) {
          p_dst = p_cur_gop->p_frm + idr_idx;
          p_src = p_cur_prd->p_frm + idx;
      ...
      p_cur_gop->gop_levels =
          imax(p_cur_gop->gop_levels, p_cur_prd_tail->gop_levels);
      // and now place the prd struct *before* them in display order (set idx to
      // 1 to avoid copying the I/P frame, we only seek the hierarchy if it
      // exists)
      for (idx = 1; idx < p_cur_prd_tail->length; idx++, idr_idx++) {
        ...
        pop_pred_struct_frm(p_dst, p_src->slice_type, p_src->nal_ref_idc,
                            p_src->disp_offset, p_src->layer,
                            p_src->slice_qp_off, 0, p_src->temporal_layer);
      }
    }
  } else if (!(p_Inp->intra_delay) &&
             ((!p_Inp->EnableIDRGOP || !is_idr) && p_Inp->intra_period != 1 &&
              p_Inp->idr_period != 1)) {
    int gop_idx;
    int idr_idx = 0;
    ...
    for (gop_idx = 1; gop_idx < p_seq_struct->num_prds; gop_idx++) {
      ...
      *memory_size += length * sizeof(PredStructFrm);
      ...
      p_dst = p_cur_gop->p_frm + idr_idx;
      pop_pred_struct_frm(p_dst, I_SLICE, NALU_PRIORITY_HIGHEST,
                          p_cur_prd->p_frm[0].disp_offset, 0, 0, 1, 0);
      idr_idx++;
...
static void init_pred_struct(VideoParameters *p_Vid, InputParameters *p_Inp,
  ...
  p_seq_struct->num_prds = (p_Inp->idr_period == 1 || p_Inp->intra_period == 1)
                               ? 1
                               : (p_Inp->NumberBFrames + 1);

  p_seq_struct->p_prd =
      (PredStructAtom *)malloc(p_seq_struct->num_prds * sizeof(PredStructAtom));
  *memory_size += p_seq_struct->num_prds * sizeof(PredStructAtom);
  ...
  for (curr_prd = (p_seq_struct->num_prds - 1); curr_prd >= 0; curr_prd--) {
    ...
    switch (p_Inp->HierarchicalCoding) {
      default:
      case 0: pred_mode = 0; break;
      case 1: pred_mode = 1; break;
      case 2:
      ...
      case 3:
        ...
        if (p_Inp->LowDelay) {
          ...
          pred_mode =
              (length == (p_Inp->NumberBFrames + 1))
                  ? 3
                  : (check_power_of_two(length) ? 2 : (length > 4 ? 1 : 0));
        }
        break;
    }
    ...
    switch (pred_mode) {
      ...
      case 0:  // disposable B-pics of equal priority
        ...
        p_dst->layer = 0;
        p_dst->slice_qp_off = (p_Inp->BRefPictures == 2)
                                  ? (p_Inp->qp[P_SLICE] - p_Inp->qp[B_SLICE])
                                  : 0;  // ensure all types of frames have
                                        // identical QP (modifiers)
        ...
        for (idx = 1; idx < length; idx++) {
          ...
          p_dst->nal_ref_idc =
              (p_Inp->BRefPictures == 1 && p_dst->slice_type == B_SLICE)
                  ? NALU_PRIORITY_LOW
                  : NALU_PRIORITY_DISPOSABLE;
          p_dst->disp_offset = idx - 1;
          p_dst->layer = 2;
          p_dst->slice_qp_off =
              (p_Inp->PReplaceBSlice)
                  ? (p_Inp->qp[B_SLICE] - p_Inp->qp[P_SLICE])
                  : (p_dst->nal_ref_idc ? p_Inp->qpBRSOffset
                                        : 0);  // 2; 2 when depends on QPPSlice
                                               // (highest layer QP)
          p_dst->random_access = 0;
          p_dst->temporal_layer = 0;
        }
      ...
      case 1:  // even/odd hierarchy
        ...
        for (idx = 1; idx < length2; idx++) {
          ...
          p_dst->layer = 1;
          p_dst->slice_qp_off =
              p_dst->slice_type == B_SLICE
                  ? (p_Inp->HierarchyLevelQPEnable ? -1 : p_Inp->qpBRSOffset)
                  : (p_Inp->HierarchyLevelQPEnable ? 1 : 0);  // 1
          p_dst->random_access = 0;
          p_dst->temporal_layer = 0;
        }
      ...
      case 2:  // full binary decomposition
        ...
        if (length > 3) {
          ...
          for (layer = 1, order = 1, sum = 0, frm = 1; sum < (length - 1);
               frm = (frm << 1), sum += frm, layer++) {
            // layer frames (in *coding* order)
            for (idx = 0; idx < frm; idx++, order++) {
              p_dst = p_cur_prd->p_frm + order;

              p_dst->slice_type = (p_Inp->PReplaceBSlice) ? P_SLICE : B_SLICE;
              p_dst->nal_ref_idc = (frm == (length >> 1))
                                       ? NALU_PRIORITY_DISPOSABLE
                                       : NALU_PRIORITY_LOW;
              p_dst->disp_offset =
                  ((length - 1) >> layer) + idx * (length >> (layer - 1));
              p_dst->layer = layer;
              p_dst->slice_qp_off =
                  p_dst->slice_type == B_SLICE
                      ? (p_Inp->HierarchyLevelQPEnable
                             ? (-GOPlevels + layer)
                             : (p_dst->nal_ref_idc != NALU_PRIORITY_DISPOSABLE
                                    ? p_Inp->qpBRSOffset
                                    : 0))
                      : (p_Inp->HierarchyLevelQPEnable ? layer : 0);  // layer
              p_dst->random_access = 0;
              p_dst->temporal_layer = 0;
            }
          }
      ...
      case 3:  // explicit structure
      {
        ...
        if (p_Inp->LowDelay) {
          for (idx = 0; idx < (length - 1); idx++) {
            ...
            p_dst->layer =
                ((p_Vid->gop_structure[idx].hierarchy_layer == 1) ? 0 : 1) +
                1;  // invert in new notation
          ...
          for (idx = 1; idx < length; idx++) {
            ...
            p_dst->layer =
                ((p_Vid->gop_structure[idx - 1].hierarchy_layer == 1) ? 0 : 1) +
                1;  // invert in new notation
...
void populate_frm_struct(VideoParameters *p_Vid, InputParameters *p_Inp,
                         SeqStructure *p_seq_struct, int num_to_populate,
  ...
  p_seq_struct->curr_num_to_populate = imin(
      num_to_populate, init_frames_to_code - p_seq_struct->pop_start_frame);
  ...
  while (proc_frames < p_seq_struct->curr_num_to_populate) {
    ...
    // we have to check here for inserted frames (e.g. due to pre-analysis) and
    // assign them to the structure (future work)

    // establish if it is an IDR frame
    is_idr = establish_random_access(
        p_Inp, p_seq_struct, curr_frame,
        p_seq_struct->curr_num_to_populate - proc_frames, 0);
    if (is_idr && p_seq_struct->pop_flag != POP_INTRA &&
        p_seq_struct->pop_flag != POP_SP) {
      // random access structure
      int gop_frame = populate_rnd_acc_pred_struct_atom(
          p_Vid, p_Inp, p_seq_struct, curr_frame, proc_frames, &terminate_pop,
          is_idr, 0);
      // update counters
    ...
    {
      is_intra =
          establish_intra(p_Inp, p_seq_struct, curr_frame,
                          p_seq_struct->curr_num_to_populate - proc_frames, 0);
      ...
      {
        is_sp =
            establish_sp(p_Inp, p_seq_struct, curr_frame,
                         p_seq_struct->curr_num_to_populate - proc_frames, 0);
        if (is_sp) {
          // intra structure (not random access)
          int gop_frame = populate_rnd_acc_pred_struct_atom(
              p_Vid, p_Inp, p_seq_struct, curr_frame, proc_frames,
              &terminate_pop, is_sp, 2);
          // update counters
          proc_frames += gop_frame;
          curr_frame += gop_frame;
          if (terminate_pop) {
    ...
    if (!is_idr && !is_intra && !is_sp) {
      int pred_frame = populate_reg_pred_struct_atom(
          p_Vid, p_Inp, p_seq_struct, curr_frame, proc_frames, &terminate_pop);
      // update counters
...
void populate_reg_pic(InputParameters *p_Inp, PicStructure *p_pic,
                      FrameUnitStruct *p_frm_struct, int num_slices,
                      int is_bot_fld) {
  int slice;
  int slice_type = (is_bot_fld && p_frm_struct->type == I_SLICE)
                       ? (p_Inp->IntraBottom
                              ? I_SLICE
                              : (p_Inp->BRefPictures == 2 ? B_SLICE : P_SLICE))
                       : p_frm_struct->type;
  int deblock = p_Inp->DFSendParameters
                    ? (!(p_Inp->DFDisableIdc[p_frm_struct->nal_ref_idc]
                                            [p_frm_struct->type]))
                    : 1;
  int weighted_pred =
      (slice_type == P_SLICE)
          ? p_Inp->WeightedPrediction
          : ((slice_type == B_SLICE) ? p_Inp->WeightedBiprediction : 0);
  SliceStructure *p_Slice;

  // if bottom field picture idr_flag has to be 0 (even when an IDR top field
  // pic precedes it)
 ...
 * \param curr_frame
 *    global (sequence-level) coding order of the first frame (in coding order)
 *in the PredStructAtom \param pred_frame coding order with respect to
 *"curr_frame" (pred_frame + curr_frame + idx yields the absolute coding order
 *of the current frame) \param idx local (PredStructAtom-level) coding order
 *with respect to the start of the current PredStructAtom \param num_slices
 *    number of slices for the current picture
 * \param no_rnd_acc
...
static void populate_frame(InputParameters *p_Inp, SeqStructure *p_seq_struct,
  ...
  p_frm_struct->field_pic_flag =
      0;  // need to add better support for field coded pictures
  ...
  else if (p_Inp->DisposableP && p_frm_struct->type != I_SLICE &&
           !p_frm_struct
                ->layer)  // this way it will also apply to B_SLICEs in layer 0
  {
    ...
    p_frm_struct->idr_flag =
        (p_src->nal_ref_idc == NALU_PRIORITY_HIGHEST) ? 1 : 0;
  } else if (no_rnd_acc == 1 && !idx)  // no random access frame and frame first
                                       // in the struct (hence I_SLICE)
  {
    p_frm_struct->idr_flag = 0;
    p_frm_struct->nal_ref_idc = NALU_PRIORITY_HIGH;
    p_frm_struct->random_access = p_Inp->EnableOpenGOP ? 1 : 0;
  } else if (no_rnd_acc == 2 && !idx)  // no random access frame and frame first
                                       // in the struct (hence SP_SLICE)
  {
  ...
  // final QP (w/o RC) set here to ensure "mod_qp" and "type" have been
  // *finalized*
  p_frm_struct->qp = p_frm_struct->mod_qp + p_Inp->qp[p_frm_struct->type];
#if (DEBUG_PRED_STRUCT)
  fprintf(stderr, "\n\n Frame Population Stats for Frame %5d in coding order",
          curr_frame + pred_frame + idx);
  switch (p_frm_struct->type) {
    default:
    case I_SLICE: fprintf(stderr, "\n Slice Type:            I_SLICE "); break;
    case P_SLICE: fprintf(stderr, "\n Slice Type:            P_SLICE "); break;
    case B_SLICE: fprintf(stderr, "\n Slice Type:            B_SLICE "); break;
    case SP_SLICE:
      fprintf(stderr, "\n Slice Type:            SP_SLICE ");
      break;
    case SI_SLICE:
  ...
  switch (p_Inp->PicInterlace) {
    ...
    case FRAME_CODING:
      // frame
      populate_reg_pic(p_Inp, p_frm_struct->p_frame_pic, p_frm_struct,
                       num_slices, 0);
      break;
    case FIELD_CODING:
      // top field
      populate_reg_pic(p_Inp, p_frm_struct->p_top_fld_pic, p_frm_struct,
                       num_slices, 0);
      // bottom field
      populate_reg_pic(p_Inp, p_frm_struct->p_bot_fld_pic, p_frm_struct,
                       num_slices, 1);
      break;
    case ADAPTIVE_CODING:
      // frame
 ...
 *    global (sequence-level) coding order of the first frame (in coding order)
 *in the PredStructAtom \param pred_frame coding order with respect to
 *"curr_frame" (pred_frame + curr_frame yields the absolute coding order of the
 *current frame)
 ***********************************************************************
 */
 ...
 * \param curr_frame
 *    global (sequence-level) coding order of the first frame (in coding order)
 *in the PredStructAtom \param proc_frames frames that have been populated so
 *far out of the "num_to_populate" frames \param terminate_pop pointer to the
 *terminate_pop variable that signals termination of the frame population loop
 * \return
...
static int populate_reg_pred_struct_atom(VideoParameters *p_Vid,
  ...
  // first determine how many frames are available to be used to place our
  // prediction structures this finds the next place for a fixed
  // (IDR/intra/SP/SI) frame *after* current_frame (hence counts from
  // current_frame + 1 on)
  if (!(p_Inp->intra_delay)) {
    fixed_idx = get_fixed_frame_for_prd(
        p_Inp, p_seq_struct, curr_frame,
        p_seq_struct->curr_num_to_populate - proc_frames);
  } else {
    // IntraDelay currently does not support the improved prediction structure
    // allocation (get_fixed_frame_for_prd)
    fixed_idx =
        get_fixed_frame(p_Inp, p_seq_struct, curr_frame,
                        p_seq_struct->curr_num_to_populate - proc_frames);
  }
  if (fixed_idx == -1) {
  ...
  // loop through these frames and apply the appropriate prediction structure
  // (p_prd)
  while (pred_frame < avail_frames) {
    pred_idx = get_prd_index(p_Inp, p_seq_struct, avail_frames - pred_frame);
    // check here whether the prediction structure does not fit even if there is
    // no fixed frame detected; if we proceed we will allocate an inefficient
    // pred structure; better to terminate the frame population here
    ...
    // "pred_frame" is general index for the current frame starting from
    // "curr_frame" (it is zero for "curr_frame")
    for (idx = 0; idx < p_cur_prd->length; idx++)  //, pred_frame++ )
    {
      // set pointer
      p_frm_struct = p_seq_struct->p_frm + ((curr_frame + pred_frame + idx) %
                                            p_Vid->frm_struct_buffer);

      // assign values
      populate_frame(p_Inp, p_seq_struct, p_frm_struct, p_cur_prd, curr_frame,
                     pred_frame, idx, p_seq_struct->max_num_slices, 0);

      // update IDR and intra frame counters
      update_frame_indices(p_seq_struct, p_frm_struct, curr_frame,
                           pred_frame + idx);
    }
    pred_frame += idx;
  }
  return pred_frame;  // number of frames that have been populated by this
                      // function run
}
 ...
 *terminate_pop variable that signals termination of the frame population loop
 * \param is_idr
 *    first frame of the atom will be IDR if > 0. is_idr - 1 is equal to the
 *index of the Atom used to populate the frames \param no_rnd_acc if 1, then the
 *first frame in the (given) Atom is not a random access frame: Intra \n if 2,
 *then it is an SP frame \return number of frames that have been populated by
 *this function run
 ***********************************************************************
 */
...
static int populate_rnd_acc_pred_struct_atom(VideoParameters *p_Vid,
  ...
  // current_frame + 1 on)
  fixed_idx =
      get_fixed_frame(p_Inp, p_seq_struct, curr_frame + 1,
                      p_seq_struct->curr_num_to_populate - proc_frames - 1);
  if (fixed_idx == -1) {
    avail_frames = p_seq_struct->curr_num_to_populate - proc_frames;
  } else {
  ...
  if (!curr_frame && !(p_Inp->EnableIDRGOP) && !(p_Inp->intra_delay)) {
    ...
    gop_idx = 1;  // force middle IDR pred struct (2 corresponds to the longest
                  // that is though reserved for within the sequence)
  } else {
    if (!(p_Inp->PreferDispOrder))  // coding order
    {
  ...
  {
    // set pointer
    p_frm_struct = p_seq_struct->p_frm +
                   ((curr_frame + gop_frame + idx) % p_Vid->frm_struct_buffer);

    // assign values
    populate_frame(p_Inp, p_seq_struct, p_frm_struct, p_cur_gop, curr_frame,
                   gop_frame, idx, p_seq_struct->max_num_slices, no_rnd_acc);
...
void populate_frame_explicit(ExpFrameInfo *info, InputParameters *p_Inp,
                             FrameUnitStruct *p_frm_struct, int num_slices) {
  // Frame Unit Structure
...
void populate_frame_slice_type(InputParameters *p_Inp,
                               FrameUnitStruct *p_frm_struct, int slice_type,
                               int num_slices) {
  p_frm_struct->type = slice_type;

  // frame (RDPictureDecision only called for frame pictures so no need to check
  // here)
  populate_reg_pic(p_Inp, p_frm_struct->p_frame_pic, p_frm_struct, num_slices,
                   0);
}
...
void populate_frm_struct_mvc(VideoParameters *p_Vid, InputParameters *p_Inp,
                             SeqStructure *p_seq_struct, int start, int end) {
  int view_id;
  int idx;
  ...
  if (p_Inp->num_of_views == 2) {
    for (idx = start; idx < end; idx++) {
      ...
      copy_frame_mvc(p_Inp, p_curr_frm, p_curr_frm_mvc,
                     p_seq_struct->max_num_slices, 0);
      // view 1
      p_curr_frm_mvc =
          p_frm_mvc + (((idx << 1) + 1) % p_seq_struct->num_frames_mvc);
      copy_frame_mvc(p_Inp, p_curr_frm, p_curr_frm_mvc,
                     p_seq_struct->max_num_slices, 1);
    }
  } else {
    ...
    for (idx = start; idx < end; idx++) {
      ...
      for (view_id = 0; view_id < p_Inp->num_of_views; view_id++) {
        // view "view_id"
        p_curr_frm_mvc = p_frm_mvc + (((idx * p_Inp->num_of_views) + view_id) %
                                      p_seq_struct->num_frames_mvc);
        copy_frame_mvc(p_Inp, p_curr_frm, p_curr_frm_mvc,
                       p_seq_struct->max_num_slices, view_id);
      }
    }
  }
}
...
static void copy_frame_mvc(InputParameters *p_Inp, FrameUnitStruct *p_src,
                           FrameUnitStruct *p_dst, int num_slices,
                           int view_id) {
  // Frame Unit Structure
  p_dst->type =
      view_id ? p_Inp->MVCInterViewForceB != 0
                    ? B_SLICE
                    : (p_src->type == I_SLICE
                           ? (p_Inp->BRefPictures == 2 ? B_SLICE : P_SLICE)
                           : p_src->type)
  ...
  p_dst->nal_ref_idc =
      view_id ? (p_src->idr_flag ? NALU_PRIORITY_HIGH : p_src->nal_ref_idc)
              : p_src->nal_ref_idc;
  p_dst->qp = p_src->qp;
