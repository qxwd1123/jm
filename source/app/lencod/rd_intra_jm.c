 ...
 *************************************************************************************
 */
int mode_decision_for_I4x4_blocks_JM_High(Macroblock *currMB, int b8, int b4,
...
*************************************************************************************
*/
int mode_decision_for_I8x8_blocks_JM_High(Macroblock *currMB, int b8,
                                          int lambda, distblk *min_cost) {
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  Slice *currSlice = currMB->p_Slice;
  ...
  *min_cost = DISTBLK_MAX;
  ...
  for (ipmode = 0; ipmode < NO_INTRA_PMODE; ipmode++) {
    if ((ipmode == DC_PRED) ||
      ...
      // get prediction and prediction error
      generate_pred_error_8x8(&p_Vid->pCurImg[pic_opix_y],
                              currSlice->mpr_8x8[0][ipmode], &mb_pred[block_y],
  ...
  copy_image_data_8x8(&p_Vid->enc_picture->imgY[pic_pix_y], p_RDO->rec8x8[0],
                      pic_pix_x, 0);
  copy_image_data_8x8(&mb_pred[block_y], currSlice->mpr_8x8[0][best_ipmode],
                      block_x, 0);
 ...
 *************************************************************************************
 */
int Mode_Decision_for_IntraSubMBlocks(Macroblock *currMB, int b8, int lambda,
                                      distblk *cost, int non_zero[3]) {
  Slice *currSlice = currMB->p_Slice;
  int b4;
  distblk cost4x4;
  currMB->cr_cbp[0] = 0;
  ...
  *cost = weighted_cost(lambda, 6);  // 6 * lambda;
  for (b4 = 0; b4 < 4; b4++) {
    non_zero[0] |= currSlice->mode_decision_for_I4x4_blocks(currMB, b8, b4,
                                                            lambda, &cost4x4);
...
int mode_decision_for_I4x4_MB(Macroblock *currMB, int lambda, distblk *cost) {
  ...
  for (*cost = 0, b8 = 0; b8 < 4; b8++) {
    if (Mode_Decision_for_IntraSubMBlocks(currMB, b8, lambda, &cost8x8,
                                          non_zero)) {
      cbp |= (1 << b8);
    }
    *cost += cost8x8;
...
int mode_decision_for_I16x16_MB_RDO(Macroblock *currMB, int lambda) {
  ...
  currSlice->set_intrapred_16x16(currMB, PLANE_Y, &left_avail, &up_avail,
                                 &left_up_avail);
  ...
  for (k = 0; k < 4; k++) {
    if (p_Inp->IntraDisableInterOnly == 0 ||
      ...
      if (p_Inp->Intra16x16ParDisable &&
          (k == VERT_PRED_16 || k == HOR_PRED_16))
        continue;

      if (p_Inp->Intra16x16PlaneDisable && k == PLANE_16) continue;
    }

    if ((k == 0 && !up_avail) || (k == 1 && !left_avail) ||
        (k == 3 && (!left_avail || !up_avail || !left_up_avail)))
      continue;
    ...
    distortionY =
        compute_SSE16x16_thres(&p_Vid->pCurImg[currMB->opix_y],
                               &p_Vid->enc_picture->p_curr_img[currMB->pix_y],
                               currMB->pix_x, currMB->pix_x, min_rdcost);
    ...
    if (distortionY < min_rdcost - weighted_cost(lambda, 4)) {
      ...
      if (distortionY + weighted_cost(lambda, rate) < min_rdcost) {
        ...
        if (rdcost < min_rdcost) {
          ...
          for (b8 = 0; b8 < 4; b8++) {
            for (b4 = 0; b4 < 4; b4++) {
              memcpy(bestCofAC[b8][b4][0], currSlice->cofAC[b8][b4][0],
                     sizeof(int) * 65);
              memcpy(bestCofAC[b8][b4][1], currSlice->cofAC[b8][b4][1],
                     sizeof(int) * 65);
            }
          }
          ...
          for (i = 0; i < MB_BLOCK_SIZE; i++)
            memcpy(bestRec[i],
                   &p_Vid->enc_picture
                        ->p_curr_img[currMB->pix_y + i][currMB->pix_x],
  ...
  for (b8 = 0; b8 < 4; b8++) {
    for (b4 = 0; b4 < 4; b4++) {
      memcpy(currSlice->cofAC[b8][b4][0], bestCofAC[b8][b4][0],
             sizeof(int) * 65);
      memcpy(currSlice->cofAC[b8][b4][1], bestCofAC[b8][b4][1],
             sizeof(int) * 65);
    }
  }
  ...
  for (i = 0; i < MB_BLOCK_SIZE; i++) {
    memcpy(&p_Vid->enc_picture->p_curr_img[currMB->pix_y + i][currMB->pix_x],
           bestRec[i], MB_BLOCK_SIZE * sizeof(imgpel));
  }
