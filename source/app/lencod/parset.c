...
static const byte ZZ_SCAN8[64] = { 0,  1,  8,  16, 9,  2,  3,  10, 17, 24, 32,
...
void generate_parameter_sets(VideoParameters *p_Vid) {
  ...
  if (p_Inp->GenerateMultiplePPS) {
    ...
    if (is_FREXT_profile(sps->profile_idc)) {
      GeneratePictureParameterSet(p_Vid->PicParSet[0], sps, p_Vid, p_Inp, 0, 0,
                                  0, p_Inp->cb_qp_index_offset,
                                  p_Inp->cr_qp_index_offset);
      GeneratePictureParameterSet(p_Vid->PicParSet[1], sps, p_Vid, p_Inp, 1, 1,
                                  1, p_Inp->cb_qp_index_offset,
                                  p_Inp->cr_qp_index_offset);
      GeneratePictureParameterSet(p_Vid->PicParSet[2], sps, p_Vid, p_Inp, 2, 1,
                                  2, p_Inp->cb_qp_index_offset,
                                  p_Inp->cr_qp_index_offset);
    } else {
      GeneratePictureParameterSet(p_Vid->PicParSet[0], sps, p_Vid, p_Inp, 0, 0,
                                  0, p_Inp->chroma_qp_index_offset, 0);
      GeneratePictureParameterSet(p_Vid->PicParSet[1], sps, p_Vid, p_Inp, 1, 1,
                                  1, p_Inp->chroma_qp_index_offset, 0);
      GeneratePictureParameterSet(p_Vid->PicParSet[2], sps, p_Vid, p_Inp, 2, 1,
                                  2, p_Inp->chroma_qp_index_offset, 0);
    }
  } else {
    p_Vid->PicParSet[0] = AllocPPS();
    if (is_FREXT_profile(sps->profile_idc))
      GeneratePictureParameterSet(
          p_Vid->PicParSet[0], sps, p_Vid, p_Inp, 0, p_Inp->WeightedPrediction,
          p_Inp->WeightedBiprediction, p_Inp->cb_qp_index_offset,
          p_Inp->cr_qp_index_offset);
    else
      GeneratePictureParameterSet(
          p_Vid->PicParSet[0], sps, p_Vid, p_Inp, 0, p_Inp->WeightedPrediction,
          p_Inp->WeightedBiprediction, p_Inp->chroma_qp_index_offset, 0);
  }
...
NALU_t *GenerateSeq_parameter_set_NALU(VideoParameters *p_Vid) {
  ...
  RBSPlen = GenerateSeq_parameter_set_rbsp(
      p_Vid, p_Vid->sps[0] /*p_Vid->active_sps*/, rbsp, 0);
#else
...
NALU_t *GenerateSubsetSeq_parameter_set_NALU(VideoParameters *p_Vid) {
  ...
  RBSPlen = GenerateSeq_parameter_set_rbsp(
      p_Vid, p_Vid->sps[1] /*p_Vid->active_sps*/, rbsp, 1);
...
*************************************************************************************
* \brief
*    NALU_t *GeneratePic_parameter_set_NALU (VideoParameters *p_Vid, int
*PPS_id);
*
* \note
*    Uses the global variables through GenerateSequenceParameterSet()
...
NALU_t *GeneratePic_parameter_set_NALU(VideoParameters *p_Vid, int PPS_id) {
  ...
  RBSPlen =
      GeneratePic_parameter_set_rbsp(p_Vid, p_Vid->PicParSet[PPS_id], rbsp);
...
void GenerateSequenceParameterSet(
  ...
  if ((sps->level_idc == 9) &&
      !is_FREXT_profile(sps->profile_idc))  // Level 1.b
  {
  ...
  sps->lossless_qpprime_flag =
      p_Inp->LosslessCoding &
  ...
  sps->log2_max_pic_order_cnt_lsb_minus4 =
      p_Vid->log2_max_pic_order_cnt_lsb_minus4;

  sps->pic_order_cnt_type = p_Inp->pic_order_cnt_type;
  sps->num_ref_frames_in_pic_order_cnt_cycle =
      p_Vid->num_ref_frames_in_pic_order_cnt_cycle;
  sps->delta_pic_order_always_zero_flag =
      p_Vid->delta_pic_order_always_zero_flag;
  ...
  sps->frame_mbs_only_flag =
      (Boolean) !(p_Inp->PicInterlace || p_Inp->MbInterlace);

  // Picture size, finally a simple one :-)
  sps->pic_width_in_mbs_minus1 =
      ((p_Inp->output.width[0] + p_Vid->auto_crop_right) >> 4) - 1;
  sps->pic_height_in_map_units_minus1 =
      (((p_Inp->output.height[0] + p_Vid->auto_crop_bottom) >> 4) /
       (2 - sps->frame_mbs_only_flag)) -
      1;

  // a couple of flags, simple
  sps->mb_adaptive_frame_field_flag =
      (Boolean)(FRAME_CODING != p_Inp->MbInterlace);
  ...
  sps->vui_parameters_present_flag =
      (Boolean)((p_Inp->output.color_model != CM_YUV &&
                 p_Inp->output.yuv_format == YUV444) ||
                p_Inp->EnableVUISupport);
  sps->chroma_format_idc = p_Inp->output.yuv_format;
  sps->separate_colour_plane_flag = (sps->chroma_format_idc == YUV444)
                                        ? p_Inp->separate_colour_plane_flag
                                        : 0;

  if (sps->vui_parameters_present_flag) GenerateVUIParameters(sps, p_Inp);

  // Fidelity Range Extensions stuff
  if (frext_profile) {
    sps->seq_scaling_matrix_present_flag =
        (Boolean)(p_Inp->ScalingMatrixPresentFlag & 1);
    n_ScalingList = (sps->chroma_format_idc != YUV444) ? 8 : 12;
    for (i = 0; i < n_ScalingList; i++) {
      if (i < 6)
        sps->seq_scaling_list_present_flag[i] =
            (p_Inp->ScalingListPresentFlag[i] & 1);
      else {
    ...
    for (i = 0; i < 12; i++) sps->seq_scaling_list_present_flag[i] = 0;
  }
  ...
  if (p_Vid->auto_crop_right || p_Vid->auto_crop_bottom) {
    ...
    sps->frame_crop_right_offset =
        (p_Vid->auto_crop_right / SubWidthC[sps->chroma_format_idc]);
    sps->frame_crop_bottom_offset =
        (p_Vid->auto_crop_bottom /
 ...
 ************************************************************************
 */
void GeneratePictureParameterSet(
...

{
  ...
  if (frext_profile) {
    ...
    pps->pic_scaling_matrix_present_flag =
        (Boolean)((p_Inp->ScalingMatrixPresentFlag & 2) >> 1);
    n_ScalingList = (sps->chroma_format_idc != YUV444) ? 8 : 12;
    for (i = 0; i < n_ScalingList; i++) {
      if (i < 6)
        pps->pic_scaling_list_present_flag[i] =
            (p_Inp->ScalingListPresentFlag[i] & 2) >> 1;
      else {
    ...
    for (i = 0; i < 12; i++) pps->pic_scaling_list_present_flag[i] = 0;
  ...
  pps->bottom_field_pic_order_in_frame_present_flag =
      p_Vid->bottom_field_pic_order_in_frame_present_flag;
  ...
  if (pps->num_slice_groups_minus1 > 0) {
    if ((pps->slice_group_id =
             calloc((sps->pic_height_in_map_units_minus1 + 1) *
    ...
    switch (p_Inp->slice_group_map_type) {
      case 0:
        ...
        break;
      case 1: pps->slice_group_map_type = 1; break;
      case 2:
        // i loops from 0 to num_slice_groups_minus1-1, because no info for
        // background needed
        pps->slice_group_map_type = 2;
        FrameSizeInMapUnits = (sps->pic_height_in_map_units_minus1) *
                              (sps->pic_width_in_mbs_minus1 + 1);
        for (i = 0; i < pps->num_slice_groups_minus1; i++) {
          // make sure the macroblock map does not lie outside the picture
          pps->top_left[i] = imin(p_Inp->top_left[i], FrameSizeInMapUnits);
          pps->bottom_right[i] =
              imin(p_Inp->bottom_right[i], FrameSizeInMapUnits);
        }
        break;
      case 3:
      case 4:
      case 5:
        ...
        pps->slice_group_change_direction_flag =
            (Boolean)p_Inp->slice_group_change_direction_flag;
        pps->slice_group_change_rate_minus1 =
            p_Inp->slice_group_change_rate_minus1;
        break;
      case 6:
        pps->slice_group_map_type = 6;
        pps->pic_size_in_map_units_minus1 =
            (((p_Inp->output.height[0] + p_Vid->auto_crop_bottom) /
              MB_BLOCK_SIZE) /
             (2 - sps->frame_mbs_only_flag)) *
                ((p_Inp->output.width[0] + p_Vid->auto_crop_right) /
                 MB_BLOCK_SIZE) -
            1;
  ...
  pps->num_ref_idx_l0_default_active_minus1 =
      sps->frame_mbs_only_flag ? (sps->num_ref_frames - 1)
                               : (2 * sps->num_ref_frames - 1);  // set defaults
  ...
  pps->pic_init_qp_minus26 =
      0;  // hard coded to zero, QP lives in the slice header
  ...
  if (frext_profile) {
    ...
    pps->cb_qp_index_offset = pps->cr_qp_index_offset =
        pps->chroma_qp_index_offset;

  pps->deblocking_filter_control_present_flag =
      (Boolean)((p_Inp->DFSendParameters != 0) ||
 ...
 *************************************************************************************
 */
int Scaling_List(short *scalingListinput, short *scalingList,
                 int sizeOfScalingList, short *UseDefaultScalingMatrix,
                 Bitstream *bitstream) {
  int j, scanj;
  int len = 0;
  ...
  for (j = 0; j < sizeOfScalingList; j++) {
    ...
    if (nextScale != 0) {
      delta_scale = scalingListinput[scanj] -
                    lastScale;  // Calculate delta from the scalingList data
                                // from the input file
      if (delta_scale > 127)
      ...
      *UseDefaultScalingMatrix |=
          (scanj == 0 && nextScale == 0);  // Check first matrix value for zero
    }

    scalingList[scanj] =
        (short)((nextScale == 0)
                    ? lastScale
                    : nextScale);  // Update the actual scalingList matrix with
                                   // the correct values
 ...
 *************************************************************************************
 * \brief
 *    int GenerateSeq_parameter_set_rbsp (VideoParameters *p_Vid,
 *seq_parameter_set_rbsp_t *sps, char *rbsp);
 *
 * \param p_Vid
 ...
 * \param rbsp
 *    buffer to be filled with the rbsp, size should be at least
 *MAXIMUMPARSETRBSPSIZE \return size of the RBSP in bytes
 *
 * \note
...
#if (MVC_EXTENSION_ENABLE)
int GenerateSeq_parameter_set_rbsp(VideoParameters *p_Vid,
                                   seq_parameter_set_rbsp_t *sps, byte *rbsp,
                                   short Is_Subset)
#else
int GenerateSeq_parameter_set_rbsp(VideoParameters *p_Vid,
                                   seq_parameter_set_rbsp_t *sps, byte *rbsp)
#endif
{
  ...
  if (p_Vid->p_Inp->num_of_views == 2) {
    if (Is_Subset == 1) {
      assert(sps->profile_idc == MULTIVIEW_HIGH ||
  ...
  len += write_u_1("SPS: constrained_set0_flag", sps->constrained_set0_flag,
                   bitstream);
  len += write_u_1("SPS: constrained_set1_flag", sps->constrained_set1_flag,
                   bitstream);
  len += write_u_1("SPS: constrained_set2_flag", sps->constrained_set2_flag,
                   bitstream);
  len += write_u_1("SPS: constrained_set3_flag", sps->constrained_set3_flag,
                   bitstream);
  len += write_u_v(4, "SPS: reserved_zero_4bits", 0, bitstream);

  len += write_u_v(8, "SPS: level_idc", sps->level_idc, bitstream);

  len += write_ue_v("SPS: seq_parameter_set_id", sps->seq_parameter_set_id,
                    bitstream);

  // Fidelity Range Extensions stuff
  if (is_FREXT_profile(sps->profile_idc)) {
    len +=
        write_ue_v("SPS: chroma_format_idc", sps->chroma_format_idc, bitstream);
    if (sps->chroma_format_idc == YUV444)  // if(p_Vid->yuv_format == YUV444)
      len += write_u_1("SPS: separate_colour_plane_flag",
                       sps->separate_colour_plane_flag, bitstream);
    len += write_ue_v("SPS: bit_depth_luma_minus8", sps->bit_depth_luma_minus8,
                      bitstream);
    len += write_ue_v("SPS: bit_depth_chroma_minus8",
                      sps->bit_depth_chroma_minus8, bitstream);

    len += write_u_1("SPS: lossless_qpprime_y_zero_flag",
                     sps->lossless_qpprime_flag, bitstream);
    // other chroma info to be added in the future

    len += write_u_1("SPS: seq_scaling_matrix_present_flag",
                     sps->seq_scaling_matrix_present_flag, bitstream);

    if (sps->seq_scaling_matrix_present_flag) {
      ScaleParameters *p_QScale = p_Vid->p_QScale;
      n_ScalingList = (sps->chroma_format_idc != YUV444) ? 8 : 12;
      for (i = 0; i < n_ScalingList; i++) {
        len += write_u_1("SPS: seq_scaling_list_present_flag",
                         sps->seq_scaling_list_present_flag[i], bitstream);
        if (sps->seq_scaling_list_present_flag[i]) {
          if (i < 6)
            len += Scaling_List(
                p_QScale->ScalingList4x4input[i], p_QScale->ScalingList4x4[i],
                16, &p_QScale->UseDefaultScalingMatrix4x4Flag[i], bitstream);
          else
            len += Scaling_List(
                p_QScale->ScalingList8x8input[i - 6],
                p_QScale->ScalingList8x8[i - 6], 64,
                &p_QScale->UseDefaultScalingMatrix8x8Flag[i - 6], bitstream);
        }
      }
    }
  }

  len += write_ue_v("SPS: log2_max_frame_num_minus4",
                    sps->log2_max_frame_num_minus4, bitstream);
  len +=
      write_ue_v("SPS: pic_order_cnt_type", sps->pic_order_cnt_type, bitstream);

  if (sps->pic_order_cnt_type == 0)
    len += write_ue_v("SPS: log2_max_pic_order_cnt_lsb_minus4",
                      sps->log2_max_pic_order_cnt_lsb_minus4, bitstream);
  else if (sps->pic_order_cnt_type == 1) {
    len += write_u_1("SPS: delta_pic_order_always_zero_flag",
                     sps->delta_pic_order_always_zero_flag, bitstream);
    len += write_se_v("SPS: offset_for_non_ref_pic",
                      sps->offset_for_non_ref_pic, bitstream);
    len += write_se_v("SPS: offset_for_top_to_bottom_field",
                      sps->offset_for_top_to_bottom_field, bitstream);
    len += write_ue_v("SPS: num_ref_frames_in_pic_order_cnt_cycle",
                      sps->num_ref_frames_in_pic_order_cnt_cycle, bitstream);
    for (i = 0; i < sps->num_ref_frames_in_pic_order_cnt_cycle; i++)
      len += write_se_v("SPS: offset_for_ref_frame",
                        sps->offset_for_ref_frame[i], bitstream);
  }
  len += write_ue_v("SPS: num_ref_frames", sps->num_ref_frames, bitstream);
  len += write_u_1("SPS: gaps_in_frame_num_value_allowed_flag",
                   sps->gaps_in_frame_num_value_allowed_flag, bitstream);
  len += write_ue_v("SPS: pic_width_in_mbs_minus1",
                    sps->pic_width_in_mbs_minus1, bitstream);
  len += write_ue_v("SPS: pic_height_in_map_units_minus1",
                    sps->pic_height_in_map_units_minus1, bitstream);
  len += write_u_1("SPS: frame_mbs_only_flag", sps->frame_mbs_only_flag,
                   bitstream);
  if (!sps->frame_mbs_only_flag) {
    len += write_u_1("SPS: mb_adaptive_frame_field_flag",
                     sps->mb_adaptive_frame_field_flag, bitstream);
  }
  len += write_u_1("SPS: direct_8x8_inference_flag",
                   sps->direct_8x8_inference_flag, bitstream);

  len += write_u_1("SPS: frame_cropping_flag", sps->frame_cropping_flag,
                   bitstream);
  if (sps->frame_cropping_flag) {
    len += write_ue_v("SPS: frame_crop_left_offset",
                      sps->frame_crop_left_offset, bitstream);
    len += write_ue_v("SPS: frame_crop_right_offset",
                      sps->frame_crop_right_offset, bitstream);
    len += write_ue_v("SPS: frame_crop_top_offset", sps->frame_crop_top_offset,
                      bitstream);
    len += write_ue_v("SPS: frame_crop_bottom_offset",
                      sps->frame_crop_bottom_offset, bitstream);
  }

  len += write_u_1("SPS: vui_parameters_present_flag",
                   sps->vui_parameters_present_flag, bitstream);

  if (sps->vui_parameters_present_flag)
    len += GenerateVUI_parameters_rbsp(
        sps, bitstream);  // currently a dummy, asserting
  ...
  if (Is_Subset == 1) {
    ...
    // fixed to 2 views only
    {
      ...
      len += write_u_v(3, "SPS: applicable_op_temporal_id[ 0 ][ 0 ]", 0,
                       bitstream);
      len += write_ue_v("SPS: applicable_op_num_target_views_minus1[ 0 ][ 0 ]",
                        0, bitstream);
      len += write_ue_v("SPS: applicable_op_target_view_id[ 0 ][ 0 ][ 0 ]", 0,
                        bitstream);
      len += write_ue_v("SPS: applicable_op_num_views_minus1[ 0 ][ 0 ]", 0,
                        bitstream);
    }

    len += write_u_1("SPS: mvc_vui_parameters_present_flag",
                     sps->vui_parameters_present_flag, bitstream);
    {
      if (sps->vui_parameters_present_flag) {
        len += write_ue_v("SPS: vui_mvc_num_ops_minus1", 0, bitstream);
        len += write_u_v(3, "SPS: vui_mvc_temporal_id[ 0 ]", 0, bitstream);
        len += write_ue_v("SPS: vui_mvc_num_target_output_views_minus1[ 0 ]", 0,
                          bitstream);
        len += write_ue_v("SPS: vui_mvc_view_id[ 0 ][ 0 ]", 0, bitstream);
        len += write_u_1("SPS: vui_mvc_timing_info_present_flag[ 0 ]",
                         sps->vui_parameters_present_flag, bitstream);
        len += write_u_v(32, "SPS: vui_mvc_num_units_in_tick[ 0 ]",
                         sps->vui_seq_parameters.num_units_in_tick, bitstream);
        len += write_u_v(32, "SPS: vui_mvc_time_scale[ 0 ]",
                         sps->vui_seq_parameters.time_scale, bitstream);
        len +=
            write_u_1("SPS: vui_mvc_fixed_frame_rate_flag[ 0 ]",
                      sps->vui_seq_parameters.fixed_frame_rate_flag, bitstream);
        len += write_u_1(
            "SPS: vui_mvc_nal_hrd_parameters_present_flag[ 0 ]",
            sps->vui_seq_parameters.nal_hrd_parameters_present_flag, bitstream);
        if (sps->vui_seq_parameters.nal_hrd_parameters_present_flag) {
          len += WriteHRDParameters(sps, bitstream);
        }
        len += write_u_1(
            "SPS: vui_mvc_vcl_hrd_parameters_present_flag[ 0 ]",
            sps->vui_seq_parameters.vcl_hrd_parameters_present_flag, bitstream);
        ...
        if (sps->vui_seq_parameters.nal_hrd_parameters_present_flag ||
          ...
          len +=
              write_u_1("SPS: vui_mvc_low_delay_hrd_flag[ 0 ]",
                        sps->vui_seq_parameters.low_delay_hrd_flag, bitstream);
        }
        len += write_u_1("SPS: vui_mvc_pic_struct_present_flag[ 0 ]",
 ...
 ***********************************************************************************************
 * \brief
 *    int GeneratePic_parameter_set_rbsp (VideoParameters *p_Vid,
 *pic_parameter_set_rbsp_t *pps, char *rbsp);
 *
 * \param p_Vid
...
int GeneratePic_parameter_set_rbsp(VideoParameters *p_Vid,
                                   pic_parameter_set_rbsp_t *pps, byte *rbsp) {
  InputParameters *p_Inp = p_Vid->p_Inp;
  Bitstream *bitstream;
  int len = 0, LenInBytes;
  unsigned i;
  ...
  if ((bitstream = calloc(1, sizeof(Bitstream))) == NULL)
    no_mem_exit("PicParameterSet:bitstream");
  ...
  len += write_ue_v("PPS: pic_parameter_set_id", pps->pic_parameter_set_id,
                    bitstream);
  len += write_ue_v("PPS: seq_parameter_set_id", pps->seq_parameter_set_id,
                    bitstream);
  len += write_u_1("PPS: entropy_coding_mode_flag",
                   pps->entropy_coding_mode_flag, bitstream);
  len +=
      write_u_1("PPS: bottom_field_pic_order_in_frame_present_flag",
                pps->bottom_field_pic_order_in_frame_present_flag, bitstream);
  len += write_ue_v("PPS: num_slice_groups_minus1",
                    pps->num_slice_groups_minus1, bitstream);

  // FMO stuff
  if (pps->num_slice_groups_minus1 > 0) {
    len += write_ue_v("PPS: slice_group_map_type", pps->slice_group_map_type,
                      bitstream);
    if (pps->slice_group_map_type == 0)
      for (i = 0; i <= pps->num_slice_groups_minus1; i++)
        len += write_ue_v("PPS: run_length_minus1[i]",
                          pps->run_length_minus1[i], bitstream);
    else if (pps->slice_group_map_type == 2) {
      for (i = 0; i < pps->num_slice_groups_minus1; i++) {
        len += write_ue_v("PPS: top_left[i]", pps->top_left[i], bitstream);
        len +=
            write_ue_v("PPS: bottom_right[i]", pps->bottom_right[i], bitstream);
      }
    } else if (pps->slice_group_map_type == 3 ||
               pps->slice_group_map_type == 4 ||
               pps->slice_group_map_type == 5) {
      len += write_u_1("PPS: slice_group_change_direction_flag",
                       pps->slice_group_change_direction_flag, bitstream);
      len += write_ue_v("PPS: slice_group_change_rate_minus1",
                        pps->slice_group_change_rate_minus1, bitstream);
      ...
      len += write_ue_v("PPS: pic_size_in_map_units_minus1",
                        pps->pic_size_in_map_units_minus1, bitstream);
      for (i = 0; i <= pps->pic_size_in_map_units_minus1; i++)
        len += write_u_v(NumberBitsPerSliceGroupId, "PPS: >slice_group_id[i]",
                         pps->slice_group_id[i], bitstream);
    }
  }
  // End of FMO stuff

  len += write_ue_v("PPS: num_ref_idx_l0_default_active_minus1",
                    pps->num_ref_idx_l0_default_active_minus1, bitstream);
  len += write_ue_v("PPS: num_ref_idx_l1_default_active_minus1",
                    pps->num_ref_idx_l1_default_active_minus1, bitstream);
  len +=
      write_u_1("PPS: weighted_pred_flag", pps->weighted_pred_flag, bitstream);
  len += write_u_v(2, "PPS: weighted_bipred_idc", pps->weighted_bipred_idc,
                   bitstream);
  len += write_se_v("PPS: pic_init_qp_minus26", pps->pic_init_qp_minus26,
                    bitstream);
  len += write_se_v("PPS: pic_init_qs_minus26", pps->pic_init_qs_minus26,
                    bitstream);

  profile_idc = identify_profile(p_Inp);
  if (is_FREXT_profile(profile_idc))
    len += write_se_v("PPS: chroma_qp_index_offset", pps->cb_qp_index_offset,
                      bitstream);
  else
    len += write_se_v("PPS: chroma_qp_index_offset",
                      pps->chroma_qp_index_offset, bitstream);

  len += write_u_1("PPS: deblocking_filter_control_present_flag",
                   pps->deblocking_filter_control_present_flag, bitstream);
  len += write_u_1("PPS: constrained_intra_pred_flag",
                   pps->constrained_intra_pred_flag, bitstream);
  len += write_u_1("PPS: redundant_pic_cnt_present_flag",
                   pps->redundant_pic_cnt_present_flag, bitstream);

  // Fidelity Range Extensions stuff
  if (is_FREXT_profile(profile_idc)) {
    len += write_u_1("PPS: transform_8x8_mode_flag",
                     pps->transform_8x8_mode_flag, bitstream);
    len += write_u_1("PPS: pic_scaling_matrix_present_flag",
                     pps->pic_scaling_matrix_present_flag, bitstream);

    if (pps->pic_scaling_matrix_present_flag) {
      ScaleParameters *p_QScale = p_Vid->p_QScale;
      unsigned n_ScalingList =
          6 + ((p_Vid->active_sps->chroma_format_idc != 3) ? 2 : 6) *
                  pps->transform_8x8_mode_flag;

      for (i = 0; i < n_ScalingList; i++)  // SS-70226
      {
        len += write_u_1("PPS: pic_scaling_list_present_flag",
                         pps->pic_scaling_list_present_flag[i], bitstream);
    ...
    len += write_se_v("PPS: second_chroma_qp_index_offset",
                      pps->cr_qp_index_offset, bitstream);
  }
 ...
 *************************************************************************************
 */
int identify_profile(InputParameters *p_Inp) { return p_Inp->ProfileIDC; }
 ...
 *************************************************************************************
 */
int identify_level(InputParameters *p_Inp) { return p_Inp->LevelIDC; }
...
static int GenerateVUI_parameters_rbsp(seq_parameter_set_rbsp_t *sps,
  ...
  len +=
      write_u_1("VUI: aspect_ratio_info_present_flag",
                vui_seq_parameters->aspect_ratio_info_present_flag, bitstream);
  if (vui_seq_parameters->aspect_ratio_info_present_flag) {
    len += write_u_v(8, "VUI: aspect_ratio_idc",
                     vui_seq_parameters->aspect_ratio_idc, bitstream);
    if (vui_seq_parameters->aspect_ratio_idc == 255) {
      len += write_u_v(16, "VUI: sar_width", vui_seq_parameters->sar_width,
                       bitstream);
      len += write_u_v(16, "VUI: sar_height", vui_seq_parameters->sar_height,
                       bitstream);
    }
  }

  len += write_u_1("VUI: overscan_info_present_flag",
                   vui_seq_parameters->overscan_info_present_flag, bitstream);
  if (vui_seq_parameters->overscan_info_present_flag) {
    len += write_u_1("VUI: overscan_appropriate_flag",
                     vui_seq_parameters->overscan_appropriate_flag, bitstream);
  }

  len +=
      write_u_1("VUI: video_signal_type_present_flag",
                vui_seq_parameters->video_signal_type_present_flag, bitstream);
  if (vui_seq_parameters->video_signal_type_present_flag) {
    len += write_u_v(3, "VUI: video_format", vui_seq_parameters->video_format,
                     bitstream);
    len += write_u_1("VUI: video_full_range_flag",
                     vui_seq_parameters->video_full_range_flag, bitstream);
    len += write_u_1("VUI: colour_description_present_flag",
                     vui_seq_parameters->colour_description_present_flag,
                     bitstream);
    if (vui_seq_parameters->colour_description_present_flag) {
      len += write_u_v(8, "VUI: colour_primaries",
                       vui_seq_parameters->colour_primaries, bitstream);
      len += write_u_v(8, "VUI: transfer_characteristics",
                       vui_seq_parameters->transfer_characteristics, bitstream);
      len += write_u_v(8, "VUI: matrix_coefficients",
                       vui_seq_parameters->matrix_coefficients, bitstream);
    }
  }

  len += write_u_1("VUI: chroma_loc_info_present_flag",
                   vui_seq_parameters->chroma_location_info_present_flag,
                   bitstream);
  if (vui_seq_parameters->chroma_location_info_present_flag) {
    len += write_ue_v("VUI: chroma_sample_loc_type_top_field",
                      vui_seq_parameters->chroma_sample_loc_type_top_field,
                      bitstream);
    len += write_ue_v("VUI: chroma_sample_loc_type_bottom_field",
                      vui_seq_parameters->chroma_sample_loc_type_bottom_field,
                      bitstream);
  }

  len += write_u_1("VUI: timing_info_present_flag",
                   vui_seq_parameters->timing_info_present_flag, bitstream);
  // timing parameters
  if (vui_seq_parameters->timing_info_present_flag) {
    len += write_u_v(32, "VUI: num_units_in_tick",
                     vui_seq_parameters->num_units_in_tick, bitstream);
    len += write_u_v(32, "VUI: time_scale", vui_seq_parameters->time_scale,
                     bitstream);
    len += write_u_1("VUI: fixed_frame_rate_flag",
                     vui_seq_parameters->fixed_frame_rate_flag, bitstream);
  }
  // end of timing parameters
  // nal_hrd_parameters_present_flag
  len +=
      write_u_1("VUI: nal_hrd_parameters_present_flag",
                vui_seq_parameters->nal_hrd_parameters_present_flag, bitstream);
  if (vui_seq_parameters->nal_hrd_parameters_present_flag) {
    len += WriteHRDParameters(sps, bitstream);
  }
  // vcl_hrd_parameters_present_flag
  len +=
      write_u_1("VUI: vcl_hrd_parameters_present_flag",
                vui_seq_parameters->vcl_hrd_parameters_present_flag, bitstream);
  if (vui_seq_parameters->vcl_hrd_parameters_present_flag) {
    len += WriteHRDParameters(sps, bitstream);
  }
  if (vui_seq_parameters->nal_hrd_parameters_present_flag ||
      vui_seq_parameters->vcl_hrd_parameters_present_flag) {
    len += write_u_1("VUI: low_delay_hrd_flag",
                     vui_seq_parameters->low_delay_hrd_flag, bitstream);
  }
  len += write_u_1("VUI: pic_struct_present_flag",
                   vui_seq_parameters->pic_struct_present_flag, bitstream);

  len += write_u_1("VUI: bitstream_restriction_flag",
                   vui_seq_parameters->bitstream_restriction_flag, bitstream);
  if (vui_seq_parameters->bitstream_restriction_flag) {
    len += write_u_1(
        "VUI: motion_vectors_over_pic_boundaries_flag",
        vui_seq_parameters->motion_vectors_over_pic_boundaries_flag, bitstream);
    len += write_ue_v("VUI: max_bytes_per_pic_denom",
                      vui_seq_parameters->max_bytes_per_pic_denom, bitstream);
    len += write_ue_v("VUI: max_bits_per_mb_denom",
                      vui_seq_parameters->max_bits_per_mb_denom, bitstream);
    len += write_ue_v("VUI: log2_max_mv_length_horizontal",
                      vui_seq_parameters->log2_max_mv_length_horizontal,
                      bitstream);
    len +=
        write_ue_v("VUI: log2_max_mv_length_vertical",
                   vui_seq_parameters->log2_max_mv_length_vertical, bitstream);
    len += write_ue_v("VUI: num_reorder_frames",
                      vui_seq_parameters->num_reorder_frames, bitstream);
    len += write_ue_v("VUI: max_dec_frame_buffering",
                      vui_seq_parameters->max_dec_frame_buffering, bitstream);
  }
 ...
 *************************************************************************************
 * \brief
 *    int GenerateSEImessage_rbsp (int, bufferingperiod_information_struct*,
 *char*)
 *
 *
 * \return
...
#ifdef _WIN32
    len += write_u_v(32, "SEI: uuid_iso_iec_11578", (int)start_time.HighPart,
                     bitstream);
    len += write_u_v(32, "SEI: uuid_iso_iec_11578", (int)start_time.QuadPart,
                     bitstream);
#else
    len += write_u_v(32, "SEI: uuid_iso_iec_11578", (int)start_time.tv_sec,
                     bitstream);
    len += write_u_v(32, "SEI: uuid_iso_iec_11578", (int)start_time.tv_usec,
                     bitstream);
#endif
    len += write_u_v(32, "SEI: uuid_iso_iec_11578",
                     (int)(uuid_message[0] << 24) + (uuid_message[1] << 16) +
                         (uuid_message[2] << 8) + (uuid_message[3] << 0),
                     bitstream);
    len += write_u_v(32, "SEI: uuid_iso_iec_11578",
                     (int)(uuid_message[4] << 24) + (uuid_message[5] << 16) +
                         (uuid_message[6] << 8) + (uuid_message[7] << 0),
                     bitstream);
    for (i = 0; i < strlen(sei_message); i++)
      len += write_u_v(8, "SEI: user_data_payload_byte", sei_message[i],
                       bitstream);
 ...
 *************************************************************************************
 * \brief
 *    int WriteHRDParameters((seq_parameter_set_rbsp_t *sps, Bitstream
 **bitstream)
 *
 *
 * \return
...
int WriteHRDParameters(seq_parameter_set_rbsp_t *sps, Bitstream *bitstream) {
  ...
  for (SchedSelIdx = 0; SchedSelIdx <= (hrd->cpb_cnt_minus1); SchedSelIdx++) {
    len += write_ue_v("VUI: bit_rate_value_minus1",
                      hrd->bit_rate_value_minus1[SchedSelIdx], bitstream);
    len += write_ue_v("VUI: cpb_size_value_minus1",
                      hrd->cpb_size_value_minus1[SchedSelIdx], bitstream);
    len += write_u_1("VUI: cbr_flag", hrd->cbr_flag[SchedSelIdx], bitstream);
  }

  len += write_u_v(5, "VUI: initial_cpb_removal_delay_length_minus1",
                   hrd->initial_cpb_removal_delay_length_minus1, bitstream);
  len += write_u_v(5, "VUI: cpb_removal_delay_length_minus1",
                   hrd->cpb_removal_delay_length_minus1, bitstream);
  len += write_u_v(5, "VUI: dpb_output_delay_length_minus1",
                   hrd->dpb_output_delay_length_minus1, bitstream);
  len += write_u_v(5, "VUI: time_offset_length", hrd->time_offset_length,
                   bitstream);
...
static int getMaxDpbSize(seq_parameter_set_rbsp_t *active_sps) {
  ...
  switch (active_sps->level_idc) {
    ...
    default: error("undefined level", 500); break;
  }
 ...
 *************************************************************************************
 * \brief
 *    void GenerateVUIParameters(seq_parameter_set_rbsp_t *sps, InputParameters
 **p_Inp)
 *
 *
 * \return
...
void GenerateVUIParameters(seq_parameter_set_rbsp_t *sps,
  ...
  vui->aspect_ratio_info_present_flag =
      (Boolean)iVui->aspect_ratio_info_present_flag;
  ...
  vui->chroma_location_info_present_flag =
      (Boolean)iVui->chroma_location_info_present_flag;
  vui->chroma_sample_loc_type_top_field =
      (unsigned int)iVui->chroma_sample_loc_type_top_field;
  vui->chroma_sample_loc_type_bottom_field =
      (unsigned int)iVui->chroma_sample_loc_type_bottom_field;
  ...
  vui->nal_hrd_parameters_present_flag =
      (Boolean)iVui->nal_hrd_parameters_present_flag;
  ...
  for (SchedSelIdx = 0; SchedSelIdx <= nal_hrd->cpb_cnt_minus1; SchedSelIdx++) {
    nal_hrd->bit_rate_value_minus1[SchedSelIdx] =
        (unsigned int)iVui->nal_bit_rate_value_minus1;
    nal_hrd->cpb_size_value_minus1[SchedSelIdx] =
        (unsigned int)iVui->nal_cpb_size_value_minus1;
    nal_hrd->cbr_flag[SchedSelIdx] = (unsigned int)iVui->nal_vbr_cbr_flag;
  }
  nal_hrd->initial_cpb_removal_delay_length_minus1 =
      (unsigned int)iVui->nal_initial_cpb_removal_delay_length_minus1;
  nal_hrd->cpb_removal_delay_length_minus1 =
      (unsigned int)iVui->nal_cpb_removal_delay_length_minus1;
  nal_hrd->dpb_output_delay_length_minus1 =
      (unsigned int)iVui->nal_dpb_output_delay_length_minus1;
  nal_hrd->time_offset_length = (unsigned int)iVui->nal_time_offset_length;

  // VCL HRD parameters
  vui->vcl_hrd_parameters_present_flag =
      (Boolean)iVui->vcl_hrd_parameters_present_flag;
  ...
  for (SchedSelIdx = 0; SchedSelIdx <= vcl_hrd->cpb_cnt_minus1; SchedSelIdx++) {
    vcl_hrd->bit_rate_value_minus1[SchedSelIdx] =
        (unsigned int)iVui->vcl_bit_rate_value_minus1;
    vcl_hrd->cpb_size_value_minus1[SchedSelIdx] =
        (unsigned int)iVui->vcl_cpb_size_value_minus1;
    vcl_hrd->cbr_flag[SchedSelIdx] = (unsigned int)iVui->vcl_vbr_cbr_flag;
  }
  vcl_hrd->initial_cpb_removal_delay_length_minus1 =
      (unsigned int)iVui->vcl_initial_cpb_removal_delay_length_minus1;
  vcl_hrd->cpb_removal_delay_length_minus1 =
      (unsigned int)iVui->vcl_cpb_removal_delay_length_minus1;
  vcl_hrd->dpb_output_delay_length_minus1 =
      (unsigned int)iVui->vcl_dpb_output_delay_length_minus1;
  ...
  vui->motion_vectors_over_pic_boundaries_flag =
      (Boolean)iVui->motion_vectors_over_pic_boundaries_flag;
  ...
  vui->log2_max_mv_length_horizontal =
      (unsigned int)iVui->log2_max_mv_length_horizontal;
  vui->log2_max_mv_length_vertical =
      (unsigned int)iVui->log2_max_mv_length_vertical;
  vui->max_dec_frame_buffering =
      (unsigned int)imin(iVui->max_dec_frame_buffering, getMaxDpbSize(sps));
  vui->num_reorder_frames = (unsigned int)imin(
      iVui->num_reorder_frames, (int)vui->max_dec_frame_buffering);

  // special case to signal the RGB format
  if (p_Inp->output.color_model != CM_YUV &&
      p_Inp->output.yuv_format == YUV444) {
