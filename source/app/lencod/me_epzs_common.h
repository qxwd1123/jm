...
extern void EPZSBlockTypePredictorsMB(Slice *currSlice, MEBlock *mv_block,
                                      SPoint *point, int *prednum);
extern short EPZS_spatial_predictors(EPZSParameters *p_EPZS, MEBlock *mv_block,
                                     int list, int list_offset, short ref,
                                     struct pic_motion_params **mv_info);
extern void EPZS_temporal_predictors(Macroblock *currMB,
                                     StorablePicture *ref_picture,
                                     EPZSParameters *p_EPZS, MEBlock *mv_block,
                                     int *prednum, distblk stopCriterion,
                                     distblk min_mcost);
extern distblk EPZSDetermineStopCriterion(EPZSParameters *p_EPZS,
                                          distblk *prevSad, MEBlock *mv_block,
                                          distblk lambda_dist);
extern void EPZSBlockTypePredictors(Slice *currSlice, MEBlock *mv_block,
                                    SPoint *point, int *prednum);
extern void EPZSWindowPredictors(MotionVector *mv, EPZSStructure *predictor,
                                 int *prednum, EPZSStructure *windowPred);
extern void EPZS_spatial_memory_predictors(EPZSParameters *p_EPZS,
                                           MEBlock *mv_block, int list,
                                           int *prednum, int img_width);
extern void EPZS_hierarchical_predictors(EPZSParameters *p_EPZS,
                                         MEBlock *mv_block, int *prednum,
                                         StorablePicture *ref_picture,
                                         Slice *currSlice);
...
extern void EPZSOutputStats(InputParameters *p_Inp, FILE *stat,
                            short stats_file);
...
***********************************************************************
*/
static inline int add_predictor(MotionVector *cur_mv, MotionVector prd_mv,
                                int mvScale, int shift_mv) {
  *cur_mv = prd_mv;
...
static inline void scale_mv(MotionVector *out_mv, int scale,
                            const MotionVector *mv, int shift_mv) {
  out_mv->mv_x = (short)rshift_rnd_sf((scale * mv->mv_x), shift_mv);
  out_mv->mv_y = (short)rshift_rnd_sf((scale * mv->mv_y), shift_mv);
}

static inline void scale_mv_xy(MotionVector *out_mv, int *scale,
                               const MotionVector *mv, int shift_mv) {
  out_mv->mv_x = (short)rshift_rnd_sf((scale[0] * mv->mv_x), shift_mv);
  out_mv->mv_y = (short)rshift_rnd_sf((scale[1] * mv->mv_y), shift_mv);
}

static inline void compute_scaled(MotionVector *MotionVector0,
                                  MotionVector *MotionVector1,
                                  int tempmv_scale[2], const MotionVector *mv,
                                  int invmv_precision) {
  MotionVector0->mv_x = (short)iClip3(
      -32768, 32767,
      rshift_rnd_sf((tempmv_scale[LIST_0] * mv->mv_x), invmv_precision));
  MotionVector0->mv_y = (short)iClip3(
      -32768, 32767,
      rshift_rnd_sf((tempmv_scale[LIST_0] * mv->mv_y), invmv_precision));
  MotionVector1->mv_x = (short)iClip3(
      -32768, 32767,
      rshift_rnd_sf((tempmv_scale[LIST_1] * mv->mv_x), invmv_precision));
  MotionVector1->mv_y = (short)iClip3(
      -32768, 32767,
      rshift_rnd_sf((tempmv_scale[LIST_1] * mv->mv_y), invmv_precision));
}
