...
static void fillHMEIntImageMargin(imgpel **dstImg, int size_x, int size_y,
  ...
  wBufSrc = dstImg[0];
  // left IMG_PAD_SIZE
  for (i = 0; i < offset_x; i++) *(wBufDst++) = *wBufSrc;
  // center 0-(s->size_x)
  memcpy(wBufDst, wBufSrc, size_x * sizeof(imgpel));
  wBufDst += size_x;
  // right IMG_PAD_SIZE
  for (i = 0; i < offset_x; i++) *(wBufDst++) = wBufSrc[size_x_minus1];
  ...
  for (j = 0; j < size_y; j++) {
    ...
    wBufSrc = dstImg[j];
    // left IMG_PAD_SIZE
    for (i = 0; i < offset_x; i++) *(wBufDst++) = wBufSrc[0];
    wBufDst += size_x;
    // right IMG_PAD_SIZE
    for (i = 0; i < offset_x; i++) *(wBufDst++) = wBufSrc[size_x_minus1];
  }
...
void GetHMEIntImagesLuma(VideoParameters *p_Vid, int size_x, int size_y,
                         imgpel ***cImgInt) {
  ...
  for (iLevel = 0; iLevel < iPyramidLevels; iLevel++) {
    ...
    fillHMEIntImageMargin(cImgInt[iLevel], iWidth, iHeight, IMG_PAD_SIZE_X,
                          IMG_PAD_SIZE_Y);
  }
}
