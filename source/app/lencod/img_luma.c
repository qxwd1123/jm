 ...
 ************************************************************************
 */
static void getSubImageInteger(StorablePicture *s, imgpel **dstImg,
                               imgpel **srcImg) {
  int i, j;
  ...
  wBufSrc = srcImg[0];
  // left IMG_PAD_SIZE
  for (i = 0; i < IMG_PAD_SIZE_X; i++) *(wBufDst++) = wBufSrc[0];
  // center 0-(s->size_x)
  memcpy(wBufDst, wBufSrc, s->size_x * sizeof(imgpel));
  wBufDst += s->size_x;
  // right IMG_PAD_SIZE
  for (i = 0; i < IMG_PAD_SIZE_X; i++) *(wBufDst++) = wBufSrc[size_x_minus1];

  // Now copy remaining pad lines
  for (j = -IMG_PAD_SIZE_Y + 1; j < 1; ++j) {
    memcpy(dstImg[j] - IMG_PAD_SIZE_X, dstImg[j - 1] - IMG_PAD_SIZE_X,
           s->size_x_padded * sizeof(imgpel));
  }
...
static void getSubImageInteger_s(StorablePicture *s, imgpel **dstImg,
                                 imgpel **srcImg) {
  int i, j;
  int size_x_minus1 = s->size_x - 1;
 ...
 ************************************************************************
 */
static void getHorSubImageSixTap(VideoParameters *p_Vid, StorablePicture *s,
                                 imgpel **dstImg, imgpel **srcImg) {
  int is, jpad, ipad;
  ...
  for (jpad = -IMG_PAD_SIZE_Y; jpad < ypadded_size - IMG_PAD_SIZE_Y; jpad++) {
    ...
    // left padded area
    is = (tap0 * (*srcImgA++ + *srcImgD++) + tap1 * (*srcImgB + *srcImgE++) +
    ...
    *wBufDst++ = (imgpel)iClip1(p_Vid->max_imgpel_value, rshift_rnd_sf(is, 5));
    ...
    for (ipad = 2; ipad < xpadded_size - 4; ipad++) {
      is =
          (tap0 * (*srcImgA++ + *srcImgD++) + tap1 * (*srcImgB++ + *srcImgE++) +
           tap2 * (*srcImgC++ + *srcImgF++));

      *iBufDst++ = is;
      *wBufDst++ =
          (imgpel)iClip1(p_Vid->max_imgpel_value, rshift_rnd_sf(is, 5));
    }
 ...
 ************************************************************************
 */
static void getVerSubImageSixTap(VideoParameters *p_Vid, StorablePicture *s,
                                 imgpel **dstImg, imgpel **srcImg) {
  int is, jpad, ipad;
  int ypadded_size = s->size_y_padded;
  int xpadded_size = s->size_x_padded;
  ...
  // center
  for (jpad = 2 - IMG_PAD_SIZE_Y; jpad < ypadded_size - 3 - IMG_PAD_SIZE_Y;
       jpad++) {
    wxLineDst = dstImg[jpad] - IMG_PAD_SIZE_X;
    srcImgA = srcImg[jpad] - IMG_PAD_SIZE_X;
  ...
  // bottom
  for (jpad = ypadded_size - 3 - IMG_PAD_SIZE_Y;
 ...
 ************************************************************************
 */
static void getVerSubImageSixTapTmp(VideoParameters *p_Vid, StorablePicture *s,
                                    imgpel **dstImg) {
  int is, jpad, ipad;
  int ypadded_size = s->size_y_padded;
  int xpadded_size = s->size_x_padded;
  ...
  for (jpad = -IMG_PAD_SIZE_Y; jpad < 2 - IMG_PAD_SIZE_Y; jpad++) {
    ...
    for (ipad = 0; ipad < xpadded_size; ipad++) {
      ...
      wxLineDst[ipad] =
          (imgpel)iClip1(p_Vid->max_imgpel_value, rshift_rnd_sf(is, 10));
    }
  }
 ...
 ************************************************************************
 */
static void getSubImageBiLinear(StorablePicture *s, imgpel **dstImg,
                                imgpel **srcImgL, imgpel **srcImgR) {
  int jpad, ipad;
  int ypadded_size = s->size_y_padded;
  int xpadded_size = s->size_x_padded;
 ...
 ************************************************************************
 */
static void getHorSubImageBiLinear(StorablePicture *s, imgpel **dstImg,
                                   imgpel **srcImgL, imgpel **srcImgR) {
  int jpad, ipad;
  int ypadded_size = s->size_y_padded;
 ...
 ************************************************************************
 */
static void getVerSubImageBiLinear(StorablePicture *s, imgpel **dstImg,
                                   imgpel **srcImgT, imgpel **srcImgB) {
  int jpad, ipad;
 ...
 ************************************************************************
 */
static void getDiagSubImageBiLinear(StorablePicture *s, imgpel **dstImg,
                                    imgpel **srcImgT, imgpel **srcImgB) {
  int jpad, ipad;
