...
int ComputeMBMAD(int diffy[16][16]) {
  ...
  for (k = 0; k < 16; k++)
    for (l = 0; l < 16; l++) sum += iabs(diffy[k][l]);
...
void rc_copy_generic(VideoParameters *p_Vid, RCGeneric *dst, RCGeneric *src) {
  ...
  /* copy MADs */
  memcpy((void *)dst->MADofMB, (void *)src->MADofMB,
  ...
  switch (p_Inp->RCUpdateMode) {
    ...
    case RC_MODE_3:
      if (!(p_Vid->curr_frm_idx)) {
        /* number of P frames */
        np = (int)(p_Inp->no_frames + p_Inp->NumberBFrames) /
                 (1 + p_Inp->NumberBFrames) -
             1 + 1;  // approximate but good enough (hack...)
        /* number of B frames */
    ...
    case RC_MODE_2:
      if (p_Inp->idr_period == 0) {
        ...
        if ((p_Vid->curr_frm_idx / p_Inp->idr_period) >=
            (p_Inp->no_frames / p_Inp->idr_period)) {
...
void rc_init_frame(VideoParameters *p_Vid, InputParameters *p_Inp) {
  switch (p_Inp->RCUpdateMode) {
    ...
    case RC_MODE_3:
      ...
      if ((p_Inp->MbInterlace) && (p_Inp->basicunit < p_Vid->FrameSizeInMbs) &&
          (p_Vid->type == P_SLICE ||
           (p_Inp->RCUpdateMode == RC_MODE_1 && p_Vid->number)))
      ...
      if (p_Inp->RDPictureDecision) {
        rc_copy_quadratic(
            p_Vid, p_Inp, p_Vid->p_rc_quad_init,
            p_Vid->p_rc_quad);  // store rate allocation quadratic...
        rc_copy_generic(p_Vid, p_Vid->p_rc_gen_init,
                        p_Vid->p_rc_gen);  // ...and generic model
      }
      p_Vid->rc_init_pict_ptr(p_Vid, p_Inp, p_Vid->p_rc_quad, p_Vid->p_rc_gen,
                              1, 0, 1, 1.0F);
      ...
      p_Vid->p_curr_frm_struct->qp = p_Vid->qp =
          p_Vid->updateQP(p_Vid, p_Inp, p_Vid->p_rc_quad, p_Vid->p_rc_gen, 0) -
          p_Vid->p_rc_quad->bitdepth_qp_scale;
      break;
    default: break;
  }
}
...
void rc_store_slice_header_bits(VideoParameters *p_Vid, InputParameters *p_Inp,
                                int len) {
  switch (p_Inp->RCUpdateMode) {
    case RC_MODE_0:
    case RC_MODE_1:
...
*************************************************************************************
*/
void rc_store_diff(int diffy[16][16], imgpel **p_curImg, int cpix_x,
                   imgpel **prediction) {
  int i, j;
  int *iDst;
  imgpel *Src1, *Src2;
...
void rc_store_diff_16b(int diffy[16][16], imgpel **p_curImg, int cpix_x,
                       imgpel **prediction) {
  int i, j;
  int *iDst;
  uint16 *Src1, *Src2;
