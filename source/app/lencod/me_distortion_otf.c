...
************************************************************************
*/
distblk computeSAD_otf(StorablePicture *ref1, MEBlock *mv_block,
                       distblk min_mcost, MotionVector *cand) {
  int mcost = 0;
  ...
  imgpel data[MB_PIXELS];  // local allocation could be optimized by a global
                           // instanciation
  ...
  p_Dpb->pf_get_block_luma(p_Vid, ref_line, tmp_line, cand->mv_x, cand->mv_y,
                           blocksize_x, blocksize_y, ref1, 0);
  ...
  for (y = 0; y < blocksize_y; y++) {
    ...
    if (mcost > imin_cost) return (dist_scale_f((distblk)mcost));
  }
  if (mv_block->ChromaMEEnable) {
    ...
    for (k = 0; k < 2; k++) {
      ...
      ref_line = data;
      p_Dpb->pf_get_block_chroma[OTF_ME](p_Vid, ref_line, tmp_line, cand->mv_x,
                                         cand->mv_y, blocksize_x_cr,
                                         blocksize_y_cr, ref1, k + 1);
      mcr_cost = 0;
...
************************************************************************
*/
distblk computeSADWP_otf(StorablePicture *ref1, MEBlock *mv_block,
                         distblk min_mcost, MotionVector *cand) {
  int mcost = 0;
  int imin_cost = dist_down(min_mcost);
  int y, x;
  ...
  for (y = 0; y < blocksize_y; y++) {
    for (x = 0; x < blocksize_x; x += 4) {
      weighted_pel = iClip1(
          max_imgpel_value,
          ((weight * *ref_line++ + wp_luma_round) >> luma_log_weight_denom) +
              offset);
  ...
  if (mv_block->ChromaMEEnable) {
    ...
    for (k = 0; k < 2; k++) {
      ...
      for (y = 0; y < blocksize_y_cr; y++) {
        for (x = 0; x < blocksize_x_cr; x += 2) {
          weighted_pel = iClip1(max_imgpel_value_uv,
                                ((weight * *ref_line++ + wp_chroma_round) >>
...
************************************************************************
*/
distblk computeBiPredSAD1_otf(StorablePicture *ref1, StorablePicture *ref2,
                              MEBlock *mv_block, distblk min_mcost,
                              MotionVector *cand1, MotionVector *cand2) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
  int bi_diff;
  int y, x;
  ...
  imgpel data2[MB_PIXELS],
      data1[MB_PIXELS];  // local allocation could be optimized by a global
                         // instanciation
  int tmp_line[(MB_BLOCK_SIZE + 5) * (MB_BLOCK_SIZE + 5)];
  ...
  p_Dpb->pf_get_block_luma(p_Vid, ref2_line, tmp_line, cand2->mv_x, cand2->mv_y,
                           blocksize_x, blocksize_y, ref2, 0);
  p_Dpb->pf_get_block_luma(p_Vid, ref1_line, tmp_line, cand1->mv_x, cand1->mv_y,
                           blocksize_x, blocksize_y, ref1, 0);
  ...
  if (mv_block->ChromaMEEnable) {
    ...
    for (k = 1; k < 3; k++) {
      ...
      ref2_line = data2;
      ref1_line = data1;
      p_Dpb->pf_get_block_chroma[OTF_ME](
          p_Vid, ref2_line, tmp_line, cand2->mv_x, cand2->mv_y, blocksize_x_cr,
          blocksize_y_cr, ref2, k);
      p_Dpb->pf_get_block_chroma[OTF_ME](
          p_Vid, ref1_line, tmp_line, cand1->mv_x, cand1->mv_y, blocksize_x_cr,
          blocksize_y_cr, ref1, k);
...
************************************************************************
*/
distblk computeBiPredSAD2_otf(StorablePicture *ref1, StorablePicture *ref2,
                              MEBlock *mv_block, distblk min_mcost,
                              MotionVector *cand1, MotionVector *cand2) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
  ...
  for (y = 0; y < blocksize_y; y++) {
    for (x = 0; x < blocksize_x; x += 4) {
      ...
      weighted_pel = iClip1(max_imgpel_value,
                            ((pixel1 + pixel2 + lround) >> denom) + offsetBi);
...
************************************************************************
*/
distblk computeSATD_otf(StorablePicture *ref1, MEBlock *mv_block,
                        distblk min_mcost, MotionVector *cand) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
  int y, x, y4;
  ...
  if (!mv_block->test8x8) {  // 4x4 TRANSFORM
    src_size_x = blocksize_x - BLOCK_SIZE;
    src_size_mul = blocksize_x * BLOCK_SIZE;
    for (y = cand->mv_y; y < cand->mv_y + (blocksize_y << 2);
         y += (BLOCK_SIZE_SP)) {
      for (x = 0; x < blocksize_x; x += BLOCK_SIZE) {
        d = diff;
        ref_line = data;
        p_Dpb->pf_get_block_luma(p_Vid, ref_line, tmp_line,
                                 cand->mv_x + (x << 2), y, BLOCK_SIZE,
                                 BLOCK_SIZE, ref1, 0);
        src_line = src_tmp + x;
    ...
    for (y = cand->mv_y; y < cand->mv_y + (blocksize_y << 2);
      ...
      for (x = 0; x < blocksize_x; x += BLOCK_SIZE_8x8) {
        ...
        ref_line = data;
        p_Dpb->pf_get_block_luma(p_Vid, ref_line, tmp_line,
                                 cand->mv_x + (x << 2), y, BLOCK_SIZE_8x8,
                                 BLOCK_SIZE_8x8, ref1, 0);
        src_line = src_tmp + x;
...
************************************************************************
* \brief
*  JLT :  SAD computation of weighted samples _with_ Hadamard Transform (
*on-the-fly )
************************************************************************
*/
distblk computeSATDWP_otf(StorablePicture *ref1, MEBlock *mv_block,
                          distblk min_mcost, MotionVector *cand) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
  int y, x, y4;
...
************************************************************************
*/
distblk computeBiPredSATD1_otf(StorablePicture *ref1, StorablePicture *ref2,
                               MEBlock *mv_block, distblk min_mcost,
                               MotionVector *cand1, MotionVector *cand2) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
  ...
  if (!mv_block->test8x8) {  // 4x4 TRANSFORM
    ...
    for (y = 0; y < (blocksize_y << 2); y += (BLOCK_SIZE_SP)) {
      for (x = 0; x < blocksize_x; x += BLOCK_SIZE) {
        ...
        ref1_line = data1;
        src_line = src_tmp + x;
        p_Dpb->pf_get_block_luma(p_Vid, ref2_line, tmp_line,
                                 cand2->mv_x + (x << 2), cand2->mv_y + y,
                                 BLOCK_SIZE, BLOCK_SIZE, ref2, 0);
        p_Dpb->pf_get_block_luma(p_Vid, ref1_line, tmp_line,
                                 cand1->mv_x + (x << 2), cand1->mv_y + y,
                                 BLOCK_SIZE, BLOCK_SIZE, ref1, 0);
    ...
    for (y = 0; y < (blocksize_y << 2); y += BLOCK_SIZE_8x8_SP) {
      ...
      for (x = 0; x < blocksize_x; x += BLOCK_SIZE_8x8) {
        ...
        src_line = src_tmp + x;
        p_Dpb->pf_get_block_luma(p_Vid, ref2_line, tmp_line,
                                 cand2->mv_x + (x << 2), y_pos2, BLOCK_SIZE_8x8,
                                 BLOCK_SIZE_8x8, ref2, 0);
        p_Dpb->pf_get_block_luma(p_Vid, ref1_line, tmp_line,
                                 cand1->mv_x + (x << 2), y_pos1, BLOCK_SIZE_8x8,
                                 BLOCK_SIZE_8x8, ref1, 0);
...
************************************************************************
*/
distblk computeBiPredSATD2_otf(StorablePicture *ref1, StorablePicture *ref2,
                               MEBlock *mv_block, distblk min_mcost,
                               MotionVector *cand1, MotionVector *cand2) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
...
************************************************************************
*/
distblk computeSSE_otf(StorablePicture *ref1, MEBlock *mv_block,
                       distblk min_mcost, MotionVector *cand) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
  int y, x;
...
************************************************************************
*/
distblk computeSSEWP_otf(StorablePicture *ref1, MEBlock *mv_block,
                         distblk min_mcost, MotionVector *cand) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
  int y, x;
...
************************************************************************
*/
distblk computeBiPredSSE1_otf(StorablePicture *ref1, StorablePicture *ref2,
                              MEBlock *mv_block, distblk min_mcost,
                              MotionVector *cand1, MotionVector *cand2) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
...
************************************************************************
*/
distblk computeBiPredSSE2_otf(StorablePicture *ref1, StorablePicture *ref2,
                              MEBlock *mv_block, distblk min_mcost,
                              MotionVector *cand1, MotionVector *cand2) {
  int imin_cost = dist_down(min_mcost);
  int mcost = 0;
