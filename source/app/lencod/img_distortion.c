...
void find_distortion(VideoParameters *p_Vid, ImageData *imgData) {
  ...
  if (p_Vid->structure != FRAME) {
    // Luma.
    diff_cmp[0] += compute_SSE(p_Vid->pCurImg, p_Vid->imgY_com, 0, 0,
                               p_Inp->output.height[0], p_Inp->output.width[0]);

    // Chroma.
    if (p_Vid->yuv_format != YUV400) {
      diff_cmp[1] +=
          compute_SSE(p_Vid->pImgOrg[1], p_Vid->imgUV_com[0], 0, 0,
                      p_Inp->output.height[1], p_Inp->output.width[1]);
      diff_cmp[2] +=
          compute_SSE(p_Vid->pImgOrg[2], p_Vid->imgUV_com[1], 0, 0,
                      p_Inp->output.height[1], p_Inp->output.width[1]);
    }
  } else {
    ...
    // Luma.
    diff_cmp[0] +=
        compute_SSE(p_Vid->pImgOrg[0], p_Vid->enc_picture->imgY, 0, 0,
                    p_Inp->output.height[0], p_Inp->output.width[0]);
    ...
    if (p_Vid->yuv_format != YUV400) {
      ...
      diff_cmp[1] +=
          compute_SSE(p_Vid->pImgOrg[1], p_Vid->enc_picture->imgUV[0], 0, 0,
                      p_Inp->output.height[1], p_Inp->output.width[1]);
      diff_cmp[2] +=
          compute_SSE(p_Vid->pImgOrg[2], p_Vid->enc_picture->imgUV[1], 0, 0,
                      p_Inp->output.height[1], p_Inp->output.width[1]);
    }
  }
...
void select_img(VideoParameters *p_Vid, ImageStructure *imgSRC,
                ImageStructure *imgREF, ImageData *imgData) {
  InputParameters *p_Inp = p_Vid->p_Inp;
...
void compute_distortion(VideoParameters *p_Vid, ImageData *imgData) {
  ...
  if (p_Inp->Verbose != 0) {
    ...
    find_snr(p_Vid, &p_Vid->imgREF, &p_Vid->imgSRC, &p_Dist->metric[SSE],
             &p_Dist->metric[PSNR]);
    if (p_Inp->Distortion[SSIM] == 1)
      find_ssim(p_Vid, p_Inp, &p_Vid->imgREF, &p_Vid->imgSRC,
                &p_Dist->metric[SSIM]);
    if (p_Inp->Distortion[MS_SSIM] == 1)
      find_ms_ssim(p_Vid, p_Inp, &p_Vid->imgREF, &p_Vid->imgSRC,
                   &p_Dist->metric[MS_SSIM]);
    // RGB Distortion
    if (p_Inp->DistortionYUVtoRGB == 1) {
      YUVtoRGB(p_Vid, &p_Vid->imgREF, &p_Vid->imgRGB_ref);
      YUVtoRGB(p_Vid, &p_Vid->imgSRC, &p_Vid->imgRGB_src);
      find_snr(p_Vid, &p_Vid->imgRGB_ref, &p_Vid->imgRGB_src,
               &p_Dist->metric[SSE_RGB], &p_Dist->metric[PSNR_RGB]);
      if (p_Inp->Distortion[SSIM] == 1)
        find_ssim(p_Vid, p_Inp, &p_Vid->imgRGB_ref, &p_Vid->imgRGB_src,
                  &p_Dist->metric[SSIM_RGB]);
      if (p_Inp->Distortion[MS_SSIM] == 1)
        find_ms_ssim(p_Vid, p_Inp, &p_Vid->imgRGB_ref, &p_Vid->imgRGB_src,
                     &p_Dist->metric[MS_SSIM_RGB]);
    }
  }
}
