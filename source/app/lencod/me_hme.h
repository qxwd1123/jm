...
typedef struct hme_info {
  ...
  distblk *****p_hme_mdist;    //[level][list][ref][py][px], SAD only, does not
                               // include motion cost

  int64 **
      *hme_distortion;  //! [level][list][ref] overall distortion information
  int64 ***poc;
  int64 hme_tot_time;
  int64 hme_time;
  ...
  // function;
  distblk (*pf_computeSAD8x8_hme)(StorablePicture *ref1, MEBlock *mv_block,
                                  distblk min_mcost, MotionVector *cand);
  distblk (*pf_computeSAD16x16_hme)(StorablePicture *ref1, MEBlock *mv_block,
                                    distblk min_mcost, MotionVector *cand);
  distblk (*pf_computeSADWP8x8_hme)(StorablePicture *ref1, MEBlock *mv_block,
                                    distblk min_mcost, MotionVector *cand);
  distblk (*pf_computeSADWP16x16_hme)(StorablePicture *ref1, MEBlock *mv_block,
                                      distblk min_mcost, MotionVector *cand);
  distblk (*pf_computeSAD_hme)(StorablePicture *ref1, MEBlock *, distblk,
                               MotionVector *);
  distblk (*pf_computeBiPredSAD_hme)(StorablePicture *ref1,
                                     StorablePicture *ref2, MEBlock *mv_block,
                                     distblk min_mcost, MotionVector *cand1,
                                     MotionVector *cand2);
  distblk (*pf_computeSADWP_hme)(StorablePicture *ref1, MEBlock *, distblk,
                                 MotionVector *);
...
extern void HMELevelMotionSearch(Slice *currSlice, MEBlock *mv_block,
                                 int *lambda_factor);
extern void hme_get_neighbors(PixelPos *pBlkPos, int bx, int by, int iMaxBlkX);
extern void hme_get_neighbors2(PixelPos *pBlkPos, int bx, int by, int iMaxBlkX,
                               int iMaxBlkY);
extern void reduce_ref_pic_with_hme_info(Slice *currSlice, HMEInfo_t *pHMEInfo,
                                         int *lambda_factor);

// HME;
extern void AllocHMEMemory(imgpel ****pHmeImage, VideoParameters *p_Vid,
                           int size_y, int size_x, int offset_y, int offset_x,
                           int iStartLevel);
extern void FreeHMEMemory(imgpel ****pHmeImage, VideoParameters *p_Vid,
                          int iStartLevel, int iPadY, int iPadX);

extern void AllocateHMEMVInfo(MotionVector ******p_hme_mv,
                              VideoParameters *p_Vid);
extern void AllocateHMEMECost(distblk ******p_hme_mcost,
                              VideoParameters *p_Vid);
...
extern void AllocateHMEMEDist(distblk ******p_hme_mdist,
                              VideoParameters *p_Vid);
