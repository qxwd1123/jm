...

{
  ...
  p->packet[0] = (byte)(((p->v & 0x03) << 6) | ((p->p & 0x01) << 5) |
                        ((p->x & 0x01) << 4) | ((p->cc & 0x0F) << 0));

  p->packet[1] = (byte)(((p->m & 0x01) << 7) | ((p->pt & 0x7F) << 0));
...

{
  ...
  if (1 != fwrite(&p->packlen, 4, 1, f)) return -1;
  if (1 != fwrite(&intime, 4, 1, f)) return -1;
  if (1 != fwrite(p->packet, p->packlen, 1, f)) return -1;
  return 0;
}
...
int WriteRTPNALU(VideoParameters *p_Vid, NALU_t *n, FILE **f_rtp) {
  ...
  first_byte = (byte)(n->forbidden_bit << 7 | n->nal_reference_idc << 5 |
                      n->nal_unit_type);
  ...
  p->x = 0;
  p->cc = 0;
  p->m = (n->startcodeprefix_len == 4) &
         1;  // a long startcode of Annex B sets marker bit of RTP
...
void RTPUpdateTimestamp(VideoParameters *p_Vid, int tr) {
  ...
  {
    p_Vid->CurrentRTPTimestamp =
        0;  //! This is a violation of the security req. of
  ...
  /*! The following code assumes a wrap around of TR at 256, and
      ...
      The support for B frames is a bit tricky, because it is not easy to
     distinguish between a natural wrap-around of the tr, and the intentional
     going back of the tr because of a B frame.  It is solved here by a
     heuristic means: It is assumed that B frames are never "older" than 10 tr
     ticks.  Everything higher than 10 is considered a wrap around.
  */
...
void CloseRTPFile(FILE *f_rtp) { fclose(f_rtp); }
