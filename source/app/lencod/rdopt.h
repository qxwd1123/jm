...
typedef struct {
  double rdcost;
  double dcost;
  double rate;  // why is rate a double? Could we use int64 or even simply int
                // given that this is for a MB? Could a MB require int64 bits
                // (answer is no)?
  int cbp;
  distblk cost;
  short c_imode;
...
extern void store_8x8_motion_vectors_p_slice(Slice *currSlice, int dir,
                                             int block8x8, Info8x8 *B8x8Info);
extern void store_8x8_motion_vectors_b_slice(Slice *currSlice, int dir,
                                             int block8x8, Info8x8 *B8x8Info);

extern void set_modes_and_refs_for_blocks_p_slice(Macroblock *currMB,
                                                  short mode);
extern void set_modes_and_refs_for_blocks_b_slice(Macroblock *currMB,
                                                  short mode);
extern void set_modes_and_refs_for_blocks_i_slice(Macroblock *currMB,
                                                  short mode);
 ...
 *************************************************************************************
 */
static inline void copy_4x4block(imgpel **oblock, imgpel **iblock,
                                 int o_xoffset, int i_xoffset) {
  int y;
  for (y = 0; y < BLOCK_SIZE; y++) {
    memcpy(&oblock[y][o_xoffset], &iblock[y][i_xoffset],
