...
static void HMEPicMotionSearch(Slice *currSlice, MEBlock *mv_block,
                               int *lambda_factor);
static distblk HMEBlockMotionSearch(MotionVector **p_pic_mv,
                                    distblk **p_pic_mcost,
                                    distblk **p_pic_mdist, MEBlock *mv_block,
                                    int *lambda_factor);
static distblk HME_EPZSIntPelBlockMotionSearch_Enh(MotionVector *pred_mv,
                                                   MEBlock *mv_block,
                                                   MotionVector **pic_mv,
                                                   distblk **pic_mcost,
                                                   int lambda_factor);
...
void invoke_HME(VideoParameters *p_Vid, int pic_idx) {
  ...
  GenerateImagePyramid(p_Vid, pHMEInfo->iImageWidth, pHMEInfo->iImageHeight,
                       pHMEInfo->p_orig_img_pointer, 0, 0);

  // prepare HME. This code is copied from Yuwen's original implementation
  // and could use some substantial improvements.
  // The HME should not be using the existing slice code and be mixing up the
  // processes like this but instead be clean and independent. Calls to
  // init_slice should not be allowed.
...
void AllocHMEMemory(imgpel ****pHmeImage, VideoParameters *p_Vid, int size_y,
                    int size_x, int offset_y, int offset_x, int iStartLevel) {
  int i, iSx, iSy;
  int iPyramidLevels = p_Vid->pHMEInfo->iPyramidLevels;

  *pHmeImage = (imgpel ***)malloc(sizeof(imgpel **) * iPyramidLevels);
  for (i = 0; i < iStartLevel; i++) (*pHmeImage)[i] = NULL;
...
void FreeHMEMemory(imgpel ****pHmeImage, VideoParameters *p_Vid,
                   int iStartLevel, int iPadY, int iPadX) {
...
void AllocateHMEMVInfo(MotionVector ******p_hme_mv, VideoParameters *p_Vid) {
  ...
  *p_hme_mv =
      (MotionVector *****)malloc(iPyramidLevels * sizeof(MotionVector ****));
  for (i = 0; i < iPyramidLevels; i++) {
    iSx = (p_Vid->width >> (i + 3));
    iSy = (p_Vid->height >> (i + 3));
    get_mem4Dmv(&(*p_hme_mv)[i], iListNum, iMaxRefNum, iSy,
                iSx);  //[level][list][ref][py][px];
  }
}
...
void AllocateHMEMECost(distblk ******p_hme_mcost, VideoParameters *p_Vid) {
  ...
  *p_hme_mcost = (distblk *****)malloc(iPyramidLevels * sizeof(distblk ****));
  for (i = 0; i < iPyramidLevels; i++) {
    iSx = (p_Vid->width >> (i + 3));
    iSy = (p_Vid->height >> (i + 3));
    get_mem4Ddistblk(&(*p_hme_mcost)[i], iListNum, iMaxRefNum, iSy,
                     iSx);  //[level][list][ref][py][px];
  }
}
...
void AllocateHMEDistInfo(distblk ******p_hme_mdist, VideoParameters *p_Vid) {
  ...
  *p_hme_mdist = (distblk *****)malloc(iPyramidLevels * sizeof(distblk ****));
  for (i = 0; i < iPyramidLevels; i++) {
    iSx = (p_Vid->width >> (i + 3));
    iSy = (p_Vid->height >> (i + 3));
    get_mem4Ddistblk(&(*p_hme_mdist)[i], iListNum, iMaxRefNum, iSy,
                     iSx);  //[level][list][ref][py][px];
  }
}
...
void CalcSearchRange(VideoParameters *p_Vid, InputParameters *p_Inp,
                     int iMaxRefNum) {
  HMEInfo_t *pHMEInfo = p_Vid->pHMEInfo;
...
int InitHMEInfo(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_Inp->PyramidLevels > 0)
    ...
    int i = imax(
        1,
        (int)(log(imin(p_Vid->width / 88, p_Vid->height / 72)) / log(2.0) + 1));
  ...
  AllocHMEMemory(&(pHMEInfo->p_orig_img_pointer_layers[0]), p_Vid,
                 p_Vid->height, p_Vid->width, 0, 0, 1);
  ...
  get_mem3Dint64(&pHMEInfo->hme_distortion, pHMEInfo->iPyramidLevels, 3,
                 pHMEInfo->iMaxRefNum);
  get_mem3Dint64(&pHMEInfo->poc, pHMEInfo->iPyramidLevels, 3,
                 pHMEInfo->iMaxRefNum);

  pHMEInfo->p_HMESW =
      (SearchWindow *)malloc(pHMEInfo->iMaxRefNum * sizeof(SearchWindow));
  if (!pHMEInfo->p_HMESW) return -1;
  pHMEInfo->p_HMESWMin =
      (SearchWindow *)malloc(pHMEInfo->iMaxRefNum * sizeof(SearchWindow));
  if (!pHMEInfo->p_HMESWMin) return -1;
...

  {
    ...
    pHMEInfo->pTmpEncPic =
        alloc_storable_picture(p_Vid, (PictureStructure)FRAME, 16, 16, 8, 8);
    p_Vid->nal_reference_idc = nal_reference_idc;
  }

  get_mem3Dint(&pHMEInfo->hme_blk_good_store, 2, pic_size_y_in_blk,
               pic_size_x_in_blk);
...
void FreeHMEInfo(VideoParameters *p_Vid) {
  ...
  if (pHMEInfo) {
    if (p_Vid->p_Inp->num_of_views == 2) {
      if (pHMEInfo->p_orig_img_pointer_layers[0] !=
          pHMEInfo->p_orig_img_pointer_layers[1]) {
        FreeHMEMemory(&(pHMEInfo->p_orig_img_pointer_layers[1]), p_Vid, 1, 0,
                      0);
      } else
        pHMEInfo->p_orig_img_pointer_layers[1] = NULL;
    }
    ...
    if (pHMEInfo->hme_distortion) free_mem3Dint64(pHMEInfo->hme_distortion);
    if (pHMEInfo->hme_distortion) free_mem3Dint64(pHMEInfo->poc);
    ...
    if (pHMEInfo->hme_blk_seg_store) free_mem3Dint(pHMEInfo->hme_blk_seg_store);
    if (pHMEInfo->pWPReorderType) {
      if (*pHMEInfo->pWPReorderType)
        free((void *)(*pHMEInfo->pWPReorderType));
      else
        error(
            "FreeHMEInfo: trying to free unused memory "
            "*pHMEInfo->pWPReorderType",
            100);

      free(pHMEInfo->pWPReorderType);
    }
    if (pHMEInfo->ref_idx_map) free_mem3Dint(pHMEInfo->ref_idx_map);
    if (pHMEInfo->dup_ref_flag) free_mem3Dint(pHMEInfo->dup_ref_flag);
    if (pHMEInfo->wp_ref_list) free_mem3Dint(pHMEInfo->wp_ref_list);
    if (pHMEInfo->most_used_ref) free_mem2Dint(pHMEInfo->most_used_ref);
    if (pHMEInfo->perform_reorder_pass) free(pHMEInfo->perform_reorder_pass);
...
void HMEInitFrame(VideoParameters *p_Vid, HMEInfo_t *pHMEInfo) {
  pHMEInfo->hme_time = 0;
  pHMEInfo->iImageHeight =
      p_Vid->field_picture ? (p_Vid->height >> 1) : p_Vid->height;
  pHMEInfo->iImageWidth = p_Vid->width;
  pHMEInfo->p_orig_img_pointer =
      pHMEInfo->p_orig_img_pointer_layers[p_Vid->dpb_layer_id];
...
void hme_init_mv_block(VideoParameters *p_Vid, MEBlock *mv_block,
                       short blocktype) {
  ...
  get_mem2Dpel(&mv_block->orig_pic, 1,
               mv_block->blocksize_x * mv_block->blocksize_y);
  ...
  mv_block->computePredFPel =
      pHMEInfo->pf_computeSAD_hme;  // computeSAD_hme : computeSAD_hme_16b;
                                    // //p_Vid->computeUniPred[F_PEL];
...
static void SetMVFromUpperLevel(Slice *currSlice, int pyr_level) {
  VideoParameters *p_Vid = currSlice->p_Vid;
  HMEInfo_t *pHMEInfo = p_Vid->pHMEInfo;
  MotionVector *****p_hme_mv =
      pHMEInfo->p_hme_mv;  //[level][list][ref][py][px];
  int i, j;
      ...
      for (ref = 0; ref < currSlice->listXsize[list]; ref++) {
        ...
        for (j = 0; j < (blkheight >> 1); j++) {
          ...
          for (i = 0; i < (blkwidth >> 1); i++) {
            ...
            p_mv0[jj][ii] = p_mv0[jj][ii + 1] = p_mv0[jj + 1][ii] =
        ...
        if (blkheight & 1) {
          memcpy(p_mv0[blkheight - 1], p_mv0[blkheight - 2],
                 blkwidth * sizeof(MotionVector));
        }
      }
  } else {
    for (list = 0; list < numlists; list++)
      memset(*p_hme_mv[pyr_level][list][0], 0,
             currSlice->listXsize[list] * blkwidth * blkheight *
                 sizeof(MotionVector));
  }
}
...
void HMEGetPMV(VideoParameters *p_Vid, MotionVector **p_pic_mv, int level,
               int bx, int by, MotionVector *pred) {
  ...
  if (!by) {
    ...
    pred->mv_x = mv[0].mv_x + mv[1].mv_x + mv[2].mv_x -
                 smin(mv[0].mv_x, smin(mv[1].mv_x, mv[2].mv_x)) -
                 smax(mv[0].mv_x, smax(mv[1].mv_x, mv[2].mv_x));
    pred->mv_y = mv[0].mv_y + mv[1].mv_y + mv[2].mv_y -
                 smin(mv[0].mv_y, smin(mv[1].mv_y, mv[2].mv_y)) -
                 smax(mv[0].mv_y, smax(mv[1].mv_y, mv[2].mv_y));
  }
}
...
void hme_get_neighbors2(PixelPos *pBlkPos, int bx, int by, int iMaxBlkX,
                        int iMaxBlkY) {
  if (!bx) {
    pBlkPos[0].available = 0;
...
void HMELevelMotionSearch(Slice *currSlice, MEBlock *mv_block,
                          int *lambda_factor) {
  VideoParameters *p_Vid = currSlice->p_Vid;
  HMEInfo_t *pHMEInfo = p_Vid->pHMEInfo;
  distblk **p_pic_mcost;
    ...
    for (ref = 0; ref < currSlice->listXsize[list + 0]; ref++) {
      ...
      for (by = 0; by < (pic_size_y >> 3); by++) {
        for (bx = 0; bx < (pic_size_x >> 3); bx++) {
          ...
          mv_block->pos_x_padded =
              (short)(mv_block->pos_x << 2);  // + IMG_PAD_SIZE_X_TIMES4;
          mv_block->pos_y_padded =
              (short)(mv_block->pos_y << 2);  // + IMG_PAD_SIZE_Y_TIMES4;
          ...
          HMESetSearchRange(currPicSW + ref, pyr_level,
                            &(mv_block->searchRange), currPicSWMin + ref);

          pHMEInfo->hme_distortion[pyr_level][list][ref] +=
              HMEBlockMotionSearch(p_pic_mv, p_pic_mcost, p_pic_mdist, mv_block,
                                   lambda_factor);
        }
      }
    }
  ...
  //     ref, pHMEInfo->hme_distortion[pyr_level][list][ref]);
}
...
(2): it can make cache more efficient since it focus on each reference over all
blocks;
*******************************************************************/
static void HMEPicMotionSearch(Slice *currSlice, MEBlock *mv_block,
                               int *lambda_factor) {
  ...
  if (pHMEInfo->perform_reorder_pass)
    memset(pHMEInfo->perform_reorder_pass, 0,
           sizeof(int) * pHMEInfo->iPyramidLevels);

  assert(currSlice->slice_type != I_SLICE &&
         currSlice->slice_type != SI_SLICE &&
         currSlice->listXsize[0] <= p_Vid->pHMEInfo->iMaxRefNum);
  assert((currSlice->slice_type != B_SLICE) ||
         (currSlice->slice_type == B_SLICE &&
          currSlice->listXsize[1] <= p_Vid->pHMEInfo->iMaxRefNum));
  for (pyr_level = pHMEInfo->iPyramidLevels - 1; pyr_level >= iLowestLevel;
       pyr_level--) {
    pic_size_x = pHMEInfo->iImageWidth >> pyr_level;
    pic_size_y = pHMEInfo->iImageHeight >> pyr_level;
    ...
    mv_block->hme_ref_size_x_max =
        pic_size_x + IMG_PAD_SIZE_X - mv_block->blocksize_x;
    mv_block->hme_ref_size_y_max =
        pic_size_y + IMG_PAD_SIZE_Y - mv_block->blocksize_y;
...
static distblk HMEBlockMotionSearch(MotionVector **p_pic_mv,
  ...
  // do integer search;
  min_mcost = HME_EPZSIntPelBlockMotionSearch_Enh(
      &pred, mv_block, p_pic_mv, p_pic_mcost, lambda_factor[F_PEL] / 2);

  // set the cost and mv;
  p_pic_mv[by][bx] = *mv;
  p_pic_mcost[by][bx] = min_mcost;
  p_pic_mdist[by][bx] =
      min_mcost - mv_cost(p_Vid, lambda_factor[F_PEL], mv, &pred);
...
distblk hme_EPZSDetermineStopCriterion(EPZSParameters *p_EPZS,
                                       distblk **pic_mcost, MEBlock *mv_block,
                                       distblk lambda_dist) {
  ...
  sadC = block[2].available
             ? pic_mcost[by - 1][bx + 1]
             : (block[3].available ? pic_mcost[by - 1][bx - 1] : DISTBLK_MAX);
  ...
  stopCriterion = (9 * distblkmax(p_EPZS->medthres[blocktype] + lambda_dist,
                                  stopCriterion) +
                   2 * p_EPZS->medthres[blocktype]) >>
                  3;

  return stopCriterion + lambda_dist;
}

short hme_EPZSSpatialPredictors(EPZSParameters *p_EPZS, MEBlock *mv_block,
                                MotionVector **pic_mv) {
  PixelPos *block = mv_block->block;
...
void hme_EPZSWindowPredictors(MotionVector *mv, EPZSStructure *predictor,
...
HME_EPZSIntPelBlockMotionSearch_Enh(
    ...
    MotionVector **pic_mv, distblk **pic_mcost,
  ...
  if (neighbor_dist == 0) neighbor_dist = (distblk)(stopCriterion * 0.5);
  ...
  *EPZSPoint = p_EPZS->BlkCount;
  ...
  min_mcost += mv_block->computePredFPel(ref_picture, mv_block,
                                         DISTBLK_MAX - min_mcost, &cand);
  ...
  if (&tmv != &tmp) {
    if ((iabs(tmv.mv_x - mv->mv_x) - searchRange->max_x <= 0) &&
        (iabs(tmv.mv_y - mv->mv_y) - searchRange->max_y <= 0)) {
      ...
      if (mcost < min_mcost) {
        ...
        if ((mcost < min_mcost)) {  // small enough, then early terminate;
          tmp = tmv;
          min_mcost = mcost;
        }
      }
  ...
  //! If p_EPZS->medthres satisfied, then terminate, otherwise generate
  //! Predictors Condition could be strengthened by consideration distortion of
  //! adjacent partitions.
  if (min_mcost > stopCriterion) {
    ...
    // stopCriterion = EPZSDetermineStopCriterion (p_EPZS, prevSad, mv_block,
    // lambda_dist);
    if (mv_block->block[0].available || mv_block->block[1].available ||
        mv_block->block[2].available || mv_block->block[3].available)
      stopCriterion = hme_EPZSDetermineStopCriterion(p_EPZS, pic_mcost,
                                                     mv_block, lambda_dist);

    //! Add Spatial Predictors in predictor list.
    //! Scheme adds zero, left, top-left, top, top-right. Note that top-left
    //! adds very little in terms of performance and could be removed with
    //! little penalty if any.
    prednum = hme_EPZSSpatialPredictors(p_EPZS, mv_block /*->block*/, pic_mv);
    /*
    if (p_Inp->EPZSSpatialMem)
      EPZSSpatialMemPredictorsEnhanced (p_EPZS, mv_block, cur_list, &prednum,
    ref_picture->size_x >> 2);
    */

    // Temporal predictors
    {
      //  hme_EPZSTemporalPredictors(p_Vid->pHMEInfo, p_EPZS, mv_block,
      //  &prednum, stopCriterion, min_mcost);
    }

    //! Window Size Based Predictors
    //! Basically replaces a Hierarchical ME concept and helps escaping local
    //! minima, or determining large motion variations. Following predictors can
    //! be adjusted further (i.e. removed, conditioned etc) based on distortion,
    //! correlation of adjacent MVs, complexity etc. These predictors and their
    //! conditioning could also be moved after all other predictors have been
    //! tested. Adaptation could also be based on type of material and coding
    //! mode (i.e. field/frame coding,MBAFF etc considering the higher
    //! dependency with opposite parity field
    // conditionEPZS = ((min_mcost > stopCriterion)
    //  && (p_Inp->EPZSFixed > 1 || (p_Inp->EPZSFixed && currSlice->slice_type
    //  == P_SLICE)));
    // conditionEPZS = ((ref == 0) && (min_mcost > stopCriterion)
    //&& (p_Inp->EPZSFixed > 1 || (p_Inp->EPZSFixed && currSlice->slice_type ==
    // P_SLICE))); conditionEPZS = ((min_mcost > stopCriterion) && ((ref < 2 &&
    // blocktype < 4)

    conditionEPZS =
        ((min_mcost > 3 * stopCriterion) &&
         ((ref < 2 && blocktype < 4) || (ref < 1 && blocktype == 4)) &&
         (p_Inp->EPZSFixed > 1 ||
          (p_Inp->EPZSFixed && currSlice->slice_type == P_SLICE)) &&
         (mv_block->hme_level == p_Vid->pHMEInfo->iPyramidLevels - 1));

    if (conditionEPZS)
      hme_EPZSWindowPredictors(
          &tmp /*&(pic_mv[mv_block->pos_y2][mv_block->pos_x2]) mv*/,
          p_EPZS->predictor, &prednum,
          (/*(invalid_refs > 2) && */ (
               ref < 1 + (currSlice->structure !=
                          FRAME /*|| currMB->list_offset*/)) &&
           (min_mcost > 2 * neighbor_dist))  // FAST_MD
    ...
    //! Since already mvs for other blocktypes/references have been computed, we
    can reuse
    //! them in order to easier determine the optimal point. Use of predictors
    could depend
    //! on cost,
    //conditionEPZS = (ref == 0 || (ref > 0 && min_mcost > stopCriterion) ||
    currSlice->structure != FRAME || currMB->list_offset); conditionEPZS = (ref
    == 0 || (ref > 0 && min_mcost > 2 * stopCriterion));
    ...
    */
    ...
    for (pos = 0; pos < prednum; ++pos) {
      ...
      // if (((iabs (tmv.mv_x - mv->mv_x) > searchRange->max_x || iabs (tmv.mv_y
      // - mv->mv_y) > searchRange->max_y)) && (tmv.mv_x || tmv.mv_y))
      if ((iabs(tmv.mv_x - mv->mv_x) - searchRange->max_x <= 0) &&
          (iabs(tmv.mv_y - mv->mv_y) - searchRange->max_y <= 0)) {
        ...
        if (*EPZSPoint != p_EPZS->BlkCount) {
          *EPZSPoint = p_EPZS->BlkCount;
          ...
          if (mcost < second_mcost) {
            ...
            // else if (mcost < second_mcost && (tmp.mv_x != tmv.mv_x ||
            // tmp.mv_y != tmv.mv_y))
    ...
    if (min_mcost > stopCriterion) {
      ...
      if (p_Inp->EPZSPattern != 0) {
        if ((min_mcost <
             stopCriterion + ((3 * p_EPZS->medthres[blocktype]) >> 1))) {
          if ((tmp.mv_x == 0 && tmp.mv_y == 0) ||
              (iabs(tmp.mv_x - mv->mv_x) < (mv_range) &&
      ...
      for (;;) {
        ...
        do {
          ...
          if (nextLast ||
              ((tmp.mv_x == center.mv_x) && (tmp.mv_y == center.mv_y))) {
        ...
        if ((tmp.mv_x == 0 && tmp.mv_y == 0) ||
            (tmp.mv_x == mv->mv_x && tmp.mv_y == mv->mv_y)) {
          if (iabs(tmp.mv_x - mv->mv_x) < (mv_range) &&
              iabs(tmp.mv_y - mv->mv_y) < (mv_range))
            searchPatternF = p_Vid->sdiamond;
          else
            searchPatternF = p_Vid->square;
        ...
        //! Second best. Note that following code is identical as for best
        //! predictor.
        center = tmp2;
        checkMedian = FALSE;
      }
...
void hme_EPZSTemporalPredictors(
    HMEInfo_t *pHMEInfo,
