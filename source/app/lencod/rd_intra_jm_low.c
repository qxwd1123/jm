 ...
 *************************************************************************************
 */
int mode_decision_for_I4x4_blocks_JM_Low(Macroblock *currMB, int b8, int b4,
  ...
  *min_cost = DISTBLK_MAX;
  ...
  for (ipmode = 0; ipmode < NO_INTRA_PMODE; ipmode++) {
    ...
    if (valid_intra_mode(currSlice, ipmode) != 0 && available_mode) {
      ...
      if (cost < *min_cost) {
        cost += currSlice->compute_cost4x4(p_Vid, cur_img, mpr4x4[ipmode],
                                           pic_pix_x, *min_cost - cost);
  ...
  generate_pred_error_4x4(cur_img, mpr4x4[best_ipmode],
                          &currSlice->mb_pred[0][block_y],
                          &currSlice->mb_ores[0][block_y], pic_pix_x, block_x);
  ...
  if (currMB->mb_type == I4MB)
    nonzero = currMB->cr_cbp[0] = residual_transform_quant_luma_4x4(
 ...
 *************************************************************************************
 */
int mode_decision_for_I8x8_blocks_JM_Low(Macroblock *currMB, int b8, int lambda,
                                         distblk *min_cost) {
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  Slice *currSlice = currMB->p_Slice;
  ...
  *min_cost = DISTBLK_MAX;
  ...
  for (ipmode = 0; ipmode < NO_INTRA_PMODE; ipmode++) {
    if (ipmode != mostProbableMode) {
      if ((ipmode == DC_PRED) ||
          ...
          ((ipmode == HOR_PRED || ipmode == HOR_UP_PRED) && left_available) ||
          (all_available)) {
        get_intrapred_8x8(currMB, PLANE_Y, ipmode, left_available,
                          up_available);
        cost = fixedcost;

        if (cost < *min_cost) {
          cost += currSlice->compute_cost8x8(
              p_Vid, &p_Vid->pImgOrg[0][pic_opix_y],
              currSlice->mpr_8x8[0][ipmode], pic_pix_x, *min_cost - cost);
          if (cost < *min_cost) {
            best_ipmode = ipmode;
            *min_cost = cost;
          }
  ...
  generate_pred_error_8x8(&p_Vid->pCurImg[pic_opix_y],
                          ...
                          &mb_ores[block_y], pic_pix_x, block_x);
  currMB->ipmode_DPCM = (short)best_ipmode;
