...
void generate_pred_error_4x4(imgpel **cur_img, imgpel **prd_img,
                             imgpel **cur_prd, int **m7, int pic_opix_x,
                             int block_x) {
  int j, i, *m7_line;
...
static inline void get_i4x4_horizontal(imgpel **cur_pred, imgpel *PredPel) {
  ...
  for (i = 0; i < BLOCK_SIZE; i++) {
    cur_pred[i][0] = cur_pred[i][1] = cur_pred[i][2] = cur_pred[i][3] =
        (imgpel)(&P_I)[i];
  }
}
 ...
 ************************************************************************
 */
static inline void get_i4x4_dc(imgpel **cur_pred, imgpel *PredPel,
                               int left_available, int up_available) {
  int i, j, s0 = 0;
  if (up_available && left_available) {
    // no edge
    s0 = (P_A + P_B + P_C + P_D + P_I + P_J + P_K + P_L + 4) >>
         (BLOCK_SHIFT + 1);
  ...
  for (j = 0; j < BLOCK_SIZE; j++) {
    for (i = 0; i < BLOCK_SIZE; i++) cur_pred[j][i] = (imgpel)s0;
  }
}
...
static inline void get_i4x4_downleft(imgpel **cur_pred, imgpel *PredPel) {
  ...
  cur_pred[0][1] = cur_pred[1][0] =
      (imgpel)((P_B + P_D + ((P_C) << 1) + 2) >> 2);
  cur_pred[0][2] = cur_pred[1][1] = cur_pred[2][0] =
      (imgpel)((P_C + P_E + ((P_D) << 1) + 2) >> 2);
  cur_pred[0][3] = cur_pred[1][2] = cur_pred[2][1] = cur_pred[3][0] =
      (imgpel)((P_D + P_F + ((P_E) << 1) + 2) >> 2);
  cur_pred[1][3] = cur_pred[2][2] = cur_pred[3][1] =
      (imgpel)((P_E + P_G + ((P_F) << 1) + 2) >> 2);
  cur_pred[2][3] = cur_pred[3][2] =
      (imgpel)((P_F + P_H + ((P_G) << 1) + 2) >> 2);
...
static inline void get_i4x4_downright(imgpel **cur_pred, imgpel *PredPel) {
  ...
  cur_pred[2][0] = cur_pred[3][1] = (imgpel)((P_K + 2 * P_J + P_I + 2) >> 2);
  cur_pred[1][0] = cur_pred[2][1] = cur_pred[3][2] =
      (imgpel)((P_J + 2 * P_I + P_X + 2) >> 2);
  cur_pred[0][0] = cur_pred[1][1] = cur_pred[2][2] = cur_pred[3][3] =
      (imgpel)((P_I + 2 * P_X + P_A + 2) >> 2);
  cur_pred[0][1] = cur_pred[1][2] = cur_pred[2][3] =
      (imgpel)((P_X + 2 * P_A + P_B + 2) >> 2);
  cur_pred[0][2] = cur_pred[1][3] = (imgpel)((P_A + 2 * P_B + P_C + 2) >> 2);
...
static inline void get_i4x4_vertleft(imgpel **cur_pred, imgpel *PredPel) {
  ...
  cur_pred[0][1] = cur_pred[2][0] = (imgpel)((P_B + P_C + 1) >> 1);
  cur_pred[0][2] = cur_pred[2][1] = (imgpel)((P_C + P_D + 1) >> 1);
  cur_pred[0][3] = cur_pred[2][2] = (imgpel)((P_D + P_E + 1) >> 1);
  cur_pred[2][3] = (imgpel)((P_E + P_F + 1) >> 1);
  cur_pred[1][0] = (imgpel)((P_A + ((P_B) << 1) + P_C + 2) >> 2);
  cur_pred[1][1] = cur_pred[3][0] =
      (imgpel)((P_B + ((P_C) << 1) + P_D + 2) >> 2);
  cur_pred[1][2] = cur_pred[3][1] =
      (imgpel)((P_C + ((P_D) << 1) + P_E + 2) >> 2);
  cur_pred[1][3] = cur_pred[3][2] =
      (imgpel)((P_D + ((P_E) << 1) + P_F + 2) >> 2);
...
static inline void get_i4x4_vertright(imgpel **cur_pred, imgpel *PredPel) {
  cur_pred[0][0] = cur_pred[2][1] = (imgpel)((P_X + P_A + 1) >> 1);
  cur_pred[0][1] = cur_pred[2][2] = (imgpel)((P_A + P_B + 1) >> 1);
  cur_pred[0][2] = cur_pred[2][3] = (imgpel)((P_B + P_C + 1) >> 1);
  cur_pred[0][3] = (imgpel)((P_C + P_D + 1) >> 1);
  cur_pred[1][0] = cur_pred[3][1] = (imgpel)((P_I + 2 * P_X + P_A + 2) >> 2);
  cur_pred[1][1] = cur_pred[3][2] = (imgpel)((P_X + 2 * P_A + P_B + 2) >> 2);
  cur_pred[1][2] = cur_pred[3][3] = (imgpel)((P_A + 2 * P_B + P_C + 2) >> 2);
...
static inline void get_i4x4_hordown(imgpel **cur_pred, imgpel *PredPel) {
  cur_pred[0][0] = cur_pred[1][2] = (imgpel)((P_X + P_I + 1) >> 1);
  cur_pred[0][1] = cur_pred[1][3] = (imgpel)((P_I + 2 * P_X + P_A + 2) >> 2);
  cur_pred[0][2] = (imgpel)((P_X + 2 * P_A + P_B + 2) >> 2);
  cur_pred[0][3] = (imgpel)((P_A + 2 * P_B + P_C + 2) >> 2);
  cur_pred[1][0] = cur_pred[2][2] = (imgpel)((P_I + P_J + 1) >> 1);
  cur_pred[1][1] = cur_pred[2][3] = (imgpel)((P_X + 2 * P_I + P_J + 2) >> 2);
  cur_pred[2][0] = cur_pred[3][2] = (imgpel)((P_J + P_K + 1) >> 1);
  cur_pred[2][1] = cur_pred[3][3] = (imgpel)((P_I + 2 * P_J + P_K + 2) >> 2);
...
static inline void get_i4x4_horup(imgpel **cur_pred, imgpel *PredPel) {
  ...
  cur_pred[0][2] = cur_pred[1][0] = (imgpel)((P_J + P_K + 1) >> 1);
  cur_pred[0][3] = cur_pred[1][1] = (imgpel)((P_J + 2 * P_K + P_L + 2) >> 2);
  cur_pred[1][2] = cur_pred[2][0] = (imgpel)((P_K + P_L + 1) >> 1);
  cur_pred[1][3] = cur_pred[2][1] = (imgpel)((P_K + 2 * P_L + P_L + 2) >> 2);
  cur_pred[3][0] = cur_pred[2][2] = cur_pred[2][3] = cur_pred[3][1] =
      cur_pred[3][2] = cur_pred[3][3] = (imgpel)P_L;
}
 ...
 ************************************************************************
 */
void set_intrapred_4x4_mbaff(Macroblock *currMB, ColorPlane pl, int img_x,
  ...
  if (p_Inp->UseConstrainedIntraPred) {
    ...
    block_available_up_right =
        pix_c.available ? p_Vid->intra_block[pix_c.mb_addr] : 0;
  ...
  *all_available =
      block_available_up && block_available_left && block_available_up_left;

  i = (img_x & 15);

  // form predictor pels
  if (block_available_up) {
    memcpy(&PredPel[1], &img_enc[pix_b.pos_y][pix_b.pos_x],
  ...
  if (block_available_up_right) {
    memcpy(&PredPel[5], &img_enc[pix_c.pos_y][pix_c.pos_x],
           BLOCK_SIZE * sizeof(imgpel));
  } else {
    P_E = P_F = P_G = P_H = P_D;
  }
 ...
 ************************************************************************
 */
void set_intrapred_4x4(Macroblock *currMB, ColorPlane pl, int img_x, int img_y,
                       int *left_available, int *up_available,
                       int *all_available) {
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
 ...
 ************************************************************************
 */
void get_intrapred_4x4(Macroblock *currMB, ColorPlane pl, int i4x4_mode,
                       int img_x, int img_y, int left_available,
                       int up_available) {
  imgpel *PredPel = currMB->intra4x4_pred[pl];  // array of predictor pels
  imgpel ***curr_mpr_4x4 = currMB->p_Slice->mpr_4x4[pl];

  // Note that currently prediction values are always placed starting from (0,0)
  // and not according to block position.

  switch (i4x4_mode) {
    case VERT_PRED: get_i4x4_vertical(curr_mpr_4x4[VERT_PRED], PredPel); break;
    case HOR_PRED: get_i4x4_horizontal(curr_mpr_4x4[HOR_PRED], PredPel); break;
    case DC_PRED:
    ...
    case VERT_LEFT_PRED:
      ...
      break;
    case HOR_UP_PRED: get_i4x4_horup(curr_mpr_4x4[HOR_UP_PRED], PredPel); break;
