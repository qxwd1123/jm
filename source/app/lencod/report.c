...
void report_log_mode(VideoParameters *p_Vid, InputParameters *p_Inp,
...
void report_frame_statistic(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  {
    if ((p_stat_frm = fopen("stat_frame.dat", "a")) ==
    ...
    } else  // Create header for new log file
    {
      ...
      fprintf(
          p_stat_frm,
          "|            Encoder statistics. This file is generated during "
    ...
    fclose(p_stat_frm);
    if ((p_stat_frm = fopen("stat_frame.dat", "a")) ==
        NULL)  // File exists, just open for appending
    {
      snprintf(errortext, ET_SIZE, "Error open file %s  \n",
               "stat_frame.dat.dat");
      error(errortext, 500);
  ...
  if (p_Vid->frame_statistic_start) {
    fprintf(p_stat_frm,
            "|     ver     | Date  | Time  |    Sequence                  |Frm "
            "| QP |P/MbInt|   Bits   |  SNRY  |  SNRU  |  SNRV  |  I4  |  I8  "
            "| I16  | IC0  | IC1  | IC2  | IC3  | PI4  | PI8  | PI16 |  P0  |  "
            "P1  |  P2  |  P3  | P1*4*| P1*8*| P2*4*| P2*8*| P3*4*| P3*8*|  P8 "
            " | P8:4 | P4*4*| P4*8*| P8:5 | P8:6 | P8:7 | BI4  | BI8  | BI16 | "
            " B0  |  B1  |  B2  |  B3  | B0*4*| B0*8*| B1*4*| B1*8*| B2*4*| "
            "B2*8*| B3*4*| B3*8*|  B8  | B8:0 |B80*4*|B80*8*| B8:4 | B4*4*| "
            "B4*8*| B8:5 | B8:6 | B8:7 |\n");
    fprintf(
        p_stat_frm,
        " ---------------------------------------------------------------------"
  ...
  for (i = 0; i < 30; i++)
    name[i] = p_Inp->input_file1
                  .fname[i + imax(0, (int)(strlen(p_Inp->input_file1.fname) -
                                           30))];  // write last part of path,
                                                   // max 30 chars
  ...
  // report snr's
  fprintf(p_stat_frm, " %2.4f| %2.4f| %2.4f|", p_Dist->metric[PSNR].value[0],
          p_Dist->metric[PSNR].value[1], p_Dist->metric[PSNR].value[2]);

  // report modes
  // I-Modes
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[I_SLICE][I4MB]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[I_SLICE][I8MB]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[I_SLICE][I16MB]);
  ...
  // P-Modes
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[P_SLICE][I4MB]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[P_SLICE][I8MB]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[P_SLICE][I16MB]);
  ...
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[P_SLICE][1][0]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[P_SLICE][1][1]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[P_SLICE][2][0]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[P_SLICE][2][1]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[P_SLICE][3][0]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[P_SLICE][3][1]);

  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[P_SLICE][P8x8]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|", cur_stats->mode_use[P_SLICE][4]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[P_SLICE][4][0]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[P_SLICE][4][1]);
  ...
  // B-Modes
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[B_SLICE][I4MB]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[B_SLICE][I8MB]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[B_SLICE][I16MB]);
  ...
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][0][0]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][0][1]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][1][0]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][1][1]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][2][0]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][2][1]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][3][0]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][3][1]);

  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use[B_SLICE][P8x8]);
  fprintf(p_stat_frm, " %d|",
          (cur_stats->b8_mode_0_use[B_SLICE][0] +
           cur_stats->b8_mode_0_use[B_SLICE][1]));
  ...
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][4][0]);
  fprintf(p_stat_frm, " %5" FORMAT_OFF_T "|",
          cur_stats->mode_use_transform[B_SLICE][4][1]);
...
double report_slice_pred_stats(FILE *p_stat, StatParameters *p_Stats,
                               int slice_type, double bit_use,
                               char *slice_name) {
  fprintf(p_stat,
          "\n ---------------------|----------------|-----------------|");
  fprintf(p_stat, "\n   %8s           |   Mode used    | MotionInfo bits |",
          slice_name);
  fprintf(p_stat,
          "\n ---------------------|----------------|-----------------|");
  fprintf(p_stat,
          "\n Mode  0  (copy)      |  %5" FORMAT_OFF_T
          "         |    %8.2f     |",
          p_Stats->mode_use[slice_type][0],
          (double)p_Stats->bit_use_mode[slice_type][0] / bit_use);
  fprintf(p_stat,
          "\n Mode  1  (16x16)     |  %5" FORMAT_OFF_T
          "         |    %8.2f     |",
          p_Stats->mode_use[slice_type][1],
          (double)p_Stats->bit_use_mode[slice_type][1] / bit_use);
  fprintf(p_stat,
          "\n Mode  2  (16x8)      |  %5" FORMAT_OFF_T
          "         |    %8.2f     |",
          p_Stats->mode_use[slice_type][2],
          (double)p_Stats->bit_use_mode[slice_type][2] / bit_use);
  fprintf(p_stat,
          "\n Mode  3  (8x16)      |  %5" FORMAT_OFF_T
          "         |    %8.2f     |",
          p_Stats->mode_use[slice_type][3],
          (double)p_Stats->bit_use_mode[slice_type][3] / bit_use);
  fprintf(p_stat,
          "\n Mode  4  (8x8)       |  %5" FORMAT_OFF_T
          "         |    %8.2f     |",
          p_Stats->mode_use[slice_type][P8x8],
          (double)p_Stats->bit_use_mode[slice_type][P8x8] / bit_use);
  fprintf(p_stat,
          "\n Mode  5  intra 4x4   |  %5" FORMAT_OFF_T
          "         |-----------------|",
          p_Stats->mode_use[slice_type][I4MB]);
  fprintf(p_stat, "\n Mode  6  intra 8x8   |  %5" FORMAT_OFF_T "         |",
          p_Stats->mode_use[slice_type][I8MB]);
  fprintf(p_stat, "\n Mode  7+ intra 16x16 |  %5" FORMAT_OFF_T "         |",
          p_Stats->mode_use[slice_type][I16MB]);
  fprintf(p_stat, "\n Mode     intra IPCM  |  %5" FORMAT_OFF_T "         |",
          p_Stats->mode_use[slice_type][IPCM]);

  return (double)(p_Stats->bit_use_mode[slice_type][0] +
                  p_Stats->bit_use_mode[slice_type][1] +
                  p_Stats->bit_use_mode[slice_type][2] +
                  p_Stats->bit_use_mode[slice_type][3] +
                  p_Stats->bit_use_mode[slice_type][P8x8]) /
         bit_use;
}
...
void report_stats(VideoParameters *p_Vid, InputParameters *p_Inp,
                  StatParameters *p_Stats, int64 bit_use[NUM_SLICE_TYPES][2]) {
  DistortionParams *p_Dist = p_Vid->p_Dist;
  FILE *p_stat;  //!< status file for the last encoding session
  double mean_motion_info_bit_use[NUM_SLICE_TYPES] = { 0.0 };
  int i;

  if (strlen(p_Inp->StatsFile) == 0) strcpy(p_Inp->StatsFile, "stats.dat");
  ...
  for (i = 0; i < 3; i++) {
    fprintf(p_stat, " ME Level %1d Metric            : %s\n", i,
            DistortionType[p_Inp->MEErrorMetric[i]]);
  }
  fprintf(p_stat, " Mode Decision Metric         : %s\n",
          DistortionType[p_Inp->ModeDecisionMetric]);

  switch (p_Inp->ChromaMEEnable) {
    case 1: fprintf(p_stat, " ME for components            : YCbCr\n"); break;
    default: fprintf(p_stat, " ME for components            : Y\n"); break;
  }
  ...
  fprintf(p_stat, " References for P slices      : %d\n",
          p_Inp->P_List0_refs_org[0] ? p_Inp->P_List0_refs_org[0]
                                     : p_Inp->num_ref_frames_org);

  if (p_Stats->frame_ctr[B_SLICE] != 0) {
    fprintf(p_stat, " List0 refs for B slices      : %d\n",
            p_Inp->B_List0_refs_org[0] ? p_Inp->B_List0_refs_org[0]
                                       : p_Inp->num_ref_frames_org);
    fprintf(p_stat, " List1 refs for B slices      : %d\n",
            p_Inp->B_List1_refs_org[0] ? p_Inp->B_List1_refs_org[0]
                                       : p_Inp->num_ref_frames_org);
  }
  ...
  if (p_Vid->num_of_layers > 1) {
    fprintf(p_stat, " View 1 refs for P slices     : %d\n",
            p_Inp->P_List0_refs_org[1] ? p_Inp->P_List0_refs_org[1]
                                       : p_Inp->num_ref_frames_org);
    fprintf(p_stat, " View 1 L0 refs for B slices  : %d\n",
            p_Inp->B_List0_refs_org[1] ? p_Inp->B_List0_refs_org[1]
                                       : p_Inp->num_ref_frames_org);
    fprintf(p_stat, " View 1 L1 refs for B slices  : %d\n",
            p_Inp->B_List1_refs_org[1] ? p_Inp->B_List1_refs_org[1]
                                       : p_Inp->num_ref_frames_org);
  }
#endif
  ...
  if (p_Inp->MbInterlace) fprintf(p_stat, " MB Field Coding : On \n");
  ...
  if (p_Inp->full_search == 2)
    ...
    fprintf(p_stat,
            " Search range restrictions    : smaller blocks and older "
            "reference frames\n");
  ...
  fprintf(p_stat, "\n SNR U/V (dB)         |");
  fprintf(p_stat, " %5.2f/%5.2f    |", p_Dist->metric[PSNR].avslice[I_SLICE][1],
          p_Dist->metric[PSNR].avslice[I_SLICE][2]);
  fprintf(p_stat, " %5.2f/%5.2f   |", p_Dist->metric[PSNR].average[1],
  ...
  fprintf(
      ...
      p_Dist->metric[PSNR].avslice[I_SLICE][0],
      p_Dist->metric[PSNR].avslice[P_SLICE][0],
      p_Dist->metric[PSNR].avslice[B_SLICE][0]);
  fprintf(
      p_stat,
      "\n SNR U(dB)            |      %5.3f    |     %5.3f    |     %5.3f    |",
      p_Dist->metric[PSNR].avslice[I_SLICE][1],
      p_Dist->metric[PSNR].avslice[P_SLICE][1],
      p_Dist->metric[PSNR].avslice[B_SLICE][1]);
  fprintf(
      p_stat,
      "\n SNR V(dB)            |      %5.3f    |     %5.3f    |     %5.3f    |",
      p_Dist->metric[PSNR].avslice[I_SLICE][2],
      p_Dist->metric[PSNR].avslice[P_SLICE][2],
      p_Dist->metric[PSNR].avslice[B_SLICE][2]);
  fprintf(p_stat,
          "\n "
          "---------------------|----------------|---------------|-------------"
          "--|");
  ...
  fprintf(
      p_stat,
      "\n        QP            |      %5.3f    |     %5.3f    |     %5.3f    |",
      (float)p_Stats->quant[I_SLICE] /
          dmax(1.0, (float)p_Stats->num_macroblocks[I_SLICE]),
      (float)p_Stats->quant[P_SLICE] /
          dmax(1.0, (float)p_Stats->num_macroblocks[P_SLICE]),
      (float)p_Stats->quant[B_SLICE] /
          dmax(1.0, (float)p_Stats->num_macroblocks[B_SLICE]));
  fprintf(p_stat,
          "\n "
          "---------------------|----------------|---------------|-------------"
          "--|");
  ...
  fprintf(p_stat, "\n   Intra              |   Mode used    |");
  fprintf(p_stat, "\n ---------------------|----------------|");
  fprintf(p_stat, "\n Mode 0  intra 4x4    |  %5" FORMAT_OFF_T "         |",
          p_Stats->mode_use[I_SLICE][I4MB]);
  fprintf(p_stat, "\n Mode 1  intra 8x8    |  %5" FORMAT_OFF_T "         |",
          p_Stats->mode_use[I_SLICE][I8MB]);
  fprintf(p_stat, "\n Mode 2+ intra 16x16  |  %5" FORMAT_OFF_T "         |",
          p_Stats->mode_use[I_SLICE][I16MB]);
  fprintf(p_stat, "\n Mode    intra IPCM   |  %5" FORMAT_OFF_T "         |",
          p_Stats->mode_use[I_SLICE][IPCM]);

  // P slices
  if (p_Stats->frame_ctr[P_SLICE] != 0) {
    mean_motion_info_bit_use[P_SLICE] = report_slice_pred_stats(
        p_stat, p_Stats, P_SLICE, (double)bit_use[P_SLICE][0], "P Slice ");
  }
  // B slices
  if (p_Stats->frame_ctr[B_SLICE] != 0) {
    mean_motion_info_bit_use[B_SLICE] = report_slice_pred_stats(
        p_stat, p_Stats, B_SLICE, (double)bit_use[B_SLICE][0], "B Slice ");
  }
  // SP slices
  if (p_Stats->frame_ctr[SP_SLICE] != 0) {
    mean_motion_info_bit_use[SP_SLICE] = report_slice_pred_stats(
        p_stat, p_Stats, SP_SLICE, (double)bit_use[SP_SLICE][0], "SP Slice");
  }
  ...
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_header[I_SLICE] / bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_header[P_SLICE] / bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_header[B_SLICE] / bit_use[B_SLICE][0]);

  fprintf(p_stat, "\n Mode                 |");
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_mb_type[I_SLICE] / bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_mb_type[P_SLICE] / bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_mb_type[B_SLICE] / bit_use[B_SLICE][0]);
  ...
  fprintf(p_stat, "\n CBP Y/C              |");
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->tmp_bit_use_cbp[I_SLICE] / bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->tmp_bit_use_cbp[P_SLICE] / bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->tmp_bit_use_cbp[B_SLICE] / bit_use[B_SLICE][0]);

  // Print SP_SLICE
  fprintf(p_stat, "\n Coeffs. Y            |");
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[0][I_SLICE] / bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[0][P_SLICE] / bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[0][B_SLICE] / bit_use[B_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[0][SP_SLICE] / bit_use[SP_SLICE][0]);

  fprintf(p_stat, "\n Coeffs. C            |");
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeffC[I_SLICE] / bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeffC[P_SLICE] / bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeffC[B_SLICE] / bit_use[B_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeffC[SP_SLICE] / bit_use[SP_SLICE][0]);

  fprintf(p_stat, "\n Coeffs. CB           |");
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[1][I_SLICE] / bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[1][P_SLICE] / bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[1][B_SLICE] / bit_use[B_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[1][SP_SLICE] / bit_use[SP_SLICE][0]);

  fprintf(p_stat, "\n Coeffs. CR           |");
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[2][I_SLICE] / bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[2][P_SLICE] / bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[2][B_SLICE] / bit_use[B_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_coeff[2][SP_SLICE] / bit_use[SP_SLICE][0]);

  fprintf(p_stat, "\n Delta quant          |");
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_delta_quant[I_SLICE] / bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_delta_quant[P_SLICE] / bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_delta_quant[B_SLICE] / bit_use[B_SLICE][0]);

  fprintf(p_stat, "\n Stuffing Bits        |");
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_stuffing_bits[I_SLICE] / bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_stuffing_bits[P_SLICE] / bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)p_Stats->bit_use_stuffing_bits[B_SLICE] / bit_use[B_SLICE][0]);
  ...
  fprintf(p_stat, "\n average bits/frame   |");
  fprintf(p_stat, " %10.2f     |",
          (float)bit_use[I_SLICE][1] / (float)bit_use[I_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)bit_use[P_SLICE][1] / (float)bit_use[P_SLICE][0]);
  fprintf(p_stat, " %10.2f     |",
          (float)bit_use[B_SLICE][1] / (float)bit_use[B_SLICE][0]);
  fprintf(p_stat,
          "\n "
          "---------------------|----------------|----------------|------------"
          "----|");
...
void report_log(VideoParameters *p_Vid, InputParameters *p_Inp,
                StatParameters *p_Stats) {
  DistortionParams *p_Dist = p_Vid->p_Dist;
  ...
  if ((p_log = fopen("log.dat", "r")) == 0)  // check if file exists
  {
    ...
    } else  // Create header for new log file
    {
      fprintf(p_log,
              ...
              "--------------------------------------------------- \n");
      fprintf(p_log,
              "|                          Encoder statistics. This file is "
              "generated during first encoding session, new sessions will be "
              "appended                                                        "
              "                                                                "
              "                                                         |\n");
      fprintf(p_log,
              ...
              "--------------------------------------------------- \n");
      fprintf(p_log,
              "|     ver     | Date  | Time  |               Sequence          "
              "       | #Img |P/MbInt| QPI| QPP| QPB| Format  |Iperiod| #B | "
              "FMES | Hdmd | S.R |#Ref | Freq |Coding|RD-opt|Intra upd|8x8Tr| "
              "SNRY 1| SNRU 1| SNRV 1| SNRY N| SNRU N| SNRV N|#Bitr I|#Bitr "
              "P|#Bitr B|#Bitr IPB|     Total Time   |      Me Time     |\n");
      fprintf(p_log,
              " ---------------------------------------------------------------"
              "----------------------------------------------------------------"
              "----------------------------------------------------------------"
  ...
  for (i = 0; i < 40; i++)
    name[i] =
        p_Inp->input_file1
            .fname[i +
                   imax(0, ((int)strlen(p_Inp->input_file1.fname)) -
                               40)];  // write last part of path, max 40 chars
  fprintf(p_log, "%40.40s|", name);

  fprintf(p_log, "%5d |  %d/%d  |", p_Inp->no_frames, p_Inp->PicInterlace,
          p_Inp->MbInterlace);
  fprintf(p_log, " %-3d| %-3d| %-3d|", p_Inp->qp[I_SLICE], p_Inp->qp[P_SLICE],
          p_Inp->qp[B_SLICE]);
  ...
  switch (p_Inp->SearchMode[0]) {
    case UM_HEX: fprintf(p_log, "  HEX |"); break;
    case UM_HEX_SIMPLE: fprintf(p_log, " SHEX |"); break;
    case EPZS: fprintf(p_log, " EPZS |"); break;
    case FAST_FULL_SEARCH: fprintf(p_log, "  FFS |"); break;
    default: fprintf(p_log, "  FS  |"); break;
  }
#if (MVC_EXTENSION_ENABLE)
  if (p_Inp->SepViewInterSearch) {
  ...
  fprintf(p_log, "  %1d%1d%1d |", p_Inp->MEErrorMetric[F_PEL],
          p_Inp->MEErrorMetric[H_PEL], p_Inp->MEErrorMetric[Q_PEL]);

#if (MVC_EXTENSION_ENABLE)
  if (p_Inp->SepViewInterSearch) {
    fprintf(p_log, " %3d (%3d) | %2d  |", p_Inp->search_range[0],
            p_Inp->search_range[1], p_Inp->num_ref_frames);
  } else
#endif
    fprintf(p_log, " %3d | %2d  |", p_Inp->search_range[0],
  ...
  fprintf(p_log, "%7.3f|%7.3f|%7.3f|", p_Dist->metric[PSNR].avslice[I_SLICE][0],
          p_Dist->metric[PSNR].avslice[I_SLICE][1],
          p_Dist->metric[PSNR].avslice[I_SLICE][2]);
  fprintf(p_log, "%7.3f|%7.3f|%7.3f|", p_Dist->metric[PSNR].average[0],
          p_Dist->metric[PSNR].average[1], p_Dist->metric[PSNR].average[2]);
  /*
  fprintf(p_log,"%-5.3f|%-5.3f|%-5.3f|",
  p_Dist->metric[PSNR].avslice[I_SLICE][0],
  p_Dist->metric[PSNR].avslice[I_SLICE][1],
  p_Dist->metric[PSNR].avslice[I_SLICE][2]);
  fprintf(p_log,"%-5.3f|%-5.3f|%-5.3f|",
  ...
  p_Dist->metric[PSNR].avslice[B_SLICE][2]);
  */
  fprintf(p_log, "%7.0f|%7.0f|%7.0f|%9.0f|", p_Stats->bitrate_st[I_SLICE],
          p_Stats->bitrate_st[P_SLICE], p_Stats->bitrate_st[B_SLICE],
          p_Stats->bitrate);
  fprintf(p_log, "   %12d   |   %12d   |", (int)p_Vid->tot_time,
          (int)p_Vid->me_tot_time);
  ...
  if ((p_Stats->NumberBFrames != 0) &&
      (p_Stats->frame_ctr[B_SLICE] != 0))  // B picture used
  {
    fprintf(p_log,
            "%3d %2d %2d %2.2f %2.2f %2.2f %5" FORMAT_OFF_T
            " "
            "%2.2f %2.2f %2.2f %5d "
            ...
            p_Dist->metric[PSNR].avslice[I_SLICE][1],
            p_Dist->metric[PSNR].avslice[I_SLICE][2],
            p_Stats->bit_counter[I_SLICE], 0.0, 0.0, 0.0, 0,
            p_Dist->metric[PSNR].average[0], p_Dist->metric[PSNR].average[1],
            p_Dist->metric[PSNR].average[2],
            (p_Stats->bit_counter[I_SLICE] + p_Stats->bit_ctr) /
                p_Stats->frame_counter,
    ...
    if (p_Inp->no_frames != 0)
      fprintf(p_log,
              ...
              (p_Stats->bit_counter[I_SLICE] + p_Stats->bit_ctr) /
                  p_Stats->frame_counter,
              0, (double)0.001 * p_Vid->tot_time / p_Stats->frame_counter);
  }
 ...
 ************************************************************************
 */
void report(VideoParameters *p_Vid, InputParameters *p_Inp,
            StatParameters *p_Stats) {
  DistortionParams *p_Dist = p_Vid->p_Dist;
  ...
  bit_use[P_SLICE][0] =
      imax(p_Stats->frame_ctr[P_SLICE] + p_Stats->frame_ctr[SP_SLICE], 1);
  ...
  for (j = 0; j < NUM_SLICE_TYPES; j++) {
    for (i = 0; i < MAXMODE; i++) bit_use[j][1] += p_Stats->bit_use_mode[j][i];
  ...
  //! Currently adding NVB bits on P rate. Maybe additional p_Stats info should
  //! be created instead and added in log file
  p_Stats->bitrate_st[I_SLICE] = (p_Stats->bit_counter[I_SLICE]) *
                                 (float)(p_Inp->output.frame_rate) /
                                 (float)(p_Stats->frame_counter);
  p_Stats->bitrate_st[P_SLICE] = (p_Stats->bit_counter[P_SLICE]) *
                                 (float)(p_Inp->output.frame_rate) /
                                 (float)(p_Stats->frame_counter);
  p_Stats->bitrate_st[B_SLICE] = (p_Stats->bit_counter[B_SLICE]) *
                                 (float)(p_Inp->output.frame_rate) /
                                 (float)(p_Stats->frame_counter);
  p_Stats->bitrate_st[SP_SLICE] = (p_Stats->bit_counter[SP_SLICE]) *
                                  (float)(p_Inp->output.frame_rate) /
                                  (float)(p_Stats->frame_counter);
  ...
  if (p_Inp->Verbose != 0) {
    ...
    float csnr_y =
        psnr(p_Vid->max_imgpel_value_comp_sq[0], impix, sse->average[0]);
    float csnr_u =
        psnr(p_Vid->max_imgpel_value_comp_sq[1], impix_cr, sse->average[1]);
    float csnr_v =
        psnr(p_Vid->max_imgpel_value_comp_sq[2], impix_cr, sse->average[2]);
    ...
    if (p_Inp->num_of_views == 2) {
      ...
      csnr_y_v[0] =
          psnr(p_Vid->max_imgpel_value_comp_sq[0], impix, sse_v[0]->average[0]);
      csnr_u_v[0] = psnr(p_Vid->max_imgpel_value_comp_sq[1], impix_cr,
                         sse_v[0]->average[1]);
      csnr_v_v[0] = psnr(p_Vid->max_imgpel_value_comp_sq[2], impix_cr,
                         sse_v[0]->average[2]);
      csnr_y_v[1] =
          psnr(p_Vid->max_imgpel_value_comp_sq[0], impix, sse_v[1]->average[0]);
      csnr_u_v[1] = psnr(p_Vid->max_imgpel_value_comp_sq[1], impix_cr,
                         sse_v[1]->average[1]);
      csnr_v_v[1] = psnr(p_Vid->max_imgpel_value_comp_sq[2], impix_cr,
                         sse_v[1]->average[2]);
    }
#endif

    fprintf(stdout,
            " Total encoding time for the seq.  : %7.3f sec (%3.2f fps)\n",
            (float)p_Vid->tot_time * 0.001,
            1000.0 * (float)(p_Stats->frame_counter) / (float)p_Vid->tot_time);
    ...
    if (p_Inp->num_of_views == 2) {
      fprintf(stdout, "\n");
      fprintf(stdout,
              " View0_Y { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, "
              "%9.5f }\n",
              snr_v[0]->average[0], csnr_y_v[0],
              sse_v[0]->average[0] / (float)impix);
      fprintf(stdout,
              " View0_U { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, "
              "%9.5f }\n",
              snr_v[0]->average[1], csnr_u_v[0],
              sse_v[0]->average[1] / (float)impix_cr);
      fprintf(stdout,
              " View0_V { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, "
              "%9.5f }\n",
              snr_v[0]->average[2], csnr_v_v[0],
              sse_v[0]->average[2] / (float)impix_cr);
      fprintf(stdout, "\n");
      fprintf(stdout,
              " View1_Y { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, "
              "%9.5f }\n",
              snr_v[1]->average[0], csnr_y_v[1],
              sse_v[1]->average[0] / (float)impix);
      fprintf(stdout,
              " View1_U { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, "
              "%9.5f }\n",
              snr_v[1]->average[1], csnr_u_v[1],
              sse_v[1]->average[1] / (float)impix_cr);
      fprintf(stdout,
              " View1_V { PSNR (dB), cSNR (dB), MSE }   : { %7.3f, %7.3f, "
              "%9.5f }\n",
              snr_v[1]->average[2], csnr_v_v[1],
              sse_v[1]->average[2] / (float)impix_cr);
    }
#endif

    if (p_Inp->DistortionYUVtoRGB == 1) {
      float csnr_r =
          psnr(p_Vid->max_imgpel_value_comp_sq[0], impix, sse_rgb->average[0]);
      float csnr_g =
          psnr(p_Vid->max_imgpel_value_comp_sq[1], impix, sse_rgb->average[1]);
      float csnr_b =
          psnr(p_Vid->max_imgpel_value_comp_sq[2], impix, sse_rgb->average[2]);
    ...
    if (p_Inp->Distortion[SSIM] == 1) {
      if (p_Inp->DistortionYUVtoRGB == 1) {
        fprintf(stdout,
                ...
                p_Dist->metric[SSIM_RGB].average[0]);
        fprintf(stdout,
                " SSIM {U, G}                       : { %5.4f, %5.4f }\n",
                p_Dist->metric[SSIM].average[1],
                p_Dist->metric[SSIM_RGB].average[1]);
        fprintf(stdout,
                " SSIM {V, B}                       : { %5.4f, %5.4f }\n",
                p_Dist->metric[SSIM].average[2],
                p_Dist->metric[SSIM_RGB].average[2]);
      } else {
    ...
    if (p_Inp->Distortion[MS_SSIM] == 1) {
      if (p_Inp->DistortionYUVtoRGB == 1) {
        fprintf(stdout,
                " MS-SSIM {Y, R}                    : { %5.4f, %5.4f }\n",
                p_Dist->metric[MS_SSIM].average[0],
                p_Dist->metric[MS_SSIM_RGB].average[0]);
        fprintf(stdout,
                " MS-SSIM {U, G}                    : { %5.4f, %5.4f }\n",
                p_Dist->metric[MS_SSIM].average[1],
                p_Dist->metric[MS_SSIM_RGB].average[1]);
        fprintf(stdout,
                " MS-SSIM {V, B}                    : { %5.4f, %5.4f }\n",
                p_Dist->metric[MS_SSIM].average[2],
                p_Dist->metric[MS_SSIM_RGB].average[2]);
      } else {
    ...
    fprintf(stdout,
            " Total encoding time for the seq.  : %5.3f sec (%5.2f fps)\n\n",
            p_Vid->tot_time * 0.001,
            1000.0 * (p_Stats->frame_counter) / p_Vid->tot_time);

  total_bits = p_Stats->bit_ctr_parametersets;
  total_bits += p_Stats->bit_ctr_filler_data;
  for (i = 0; i < NUM_SLICE_TYPES; i++) total_bits += p_Stats->bit_counter[i];
#if (MVC_EXTENSION_ENABLE)
  if (p_Inp->num_of_views == 2) {
  ...
  if (p_Inp->num_of_views == 2 && p_Vid->fld_flag) {
    if (p_Stats->frame_ctr[B_SLICE] != 0) {
      fprintf(stdout,
              " Total bits                        : %" FORMAT_OFF_T
              " (I %" FORMAT_OFF_T ", P %" FORMAT_OFF_T ", B %" FORMAT_OFF_T
              " NVB %d) \n",
              total_bits_v[0] + total_bits_v[1],
              p_Stats->bit_counter_v[0][I_SLICE] +
                  p_Stats->bit_counter_v[1][I_SLICE],
              p_Stats->bit_counter_v[0][P_SLICE] +
                  p_Stats->bit_counter_v[1][P_SLICE],
              p_Stats->bit_counter_v[0][B_SLICE] +
                  p_Stats->bit_counter_v[1][B_SLICE],
              p_Stats->bit_ctr_parametersets_v[0] +
                  p_Stats->bit_ctr_parametersets_v[1]);
    } else {
      fprintf(stdout,
              " Total bits                        : %" FORMAT_OFF_T
...
#endif
  {
    if (p_Stats->frame_ctr[B_SLICE] != 0) {
      fprintf(stdout,
              ...
              " (I %" FORMAT_OFF_T ", P %" FORMAT_OFF_T ", B %" FORMAT_OFF_T
              " NVB %d) \n",
              total_bits, p_Stats->bit_counter[I_SLICE],
              p_Stats->bit_counter[P_SLICE], p_Stats->bit_counter[B_SLICE],
      ...
      fprintf(stdout,
              ...
              " (I %" FORMAT_OFF_T ", P %" FORMAT_OFF_T ", NVB %d) \n",
              total_bits, p_Stats->bit_counter[I_SLICE],
              p_Stats->bit_counter[P_SLICE], p_Stats->bit_ctr_parametersets);
    }
  }
#if (MVC_EXTENSION_ENABLE)
  if (p_Inp->num_of_views == 2) {
    ...
    for (view_id = 0; view_id < NUM_VIEWS; view_id++) {
      if (p_Stats->frame_ctr[B_SLICE] != 0) {
        fprintf(stdout,
                " View %1d Total-bits                 : %" FORMAT_OFF_T
                " (I %" FORMAT_OFF_T ", P %" FORMAT_OFF_T ", B %" FORMAT_OFF_T
                " NVB %d) \n",
                view_id, total_bits_v[view_id],
                p_Stats->bit_counter_v[view_id][I_SLICE],
                p_Stats->bit_counter_v[view_id][P_SLICE],
                p_Stats->bit_counter_v[view_id][B_SLICE],
                p_Stats->bit_ctr_parametersets_v[view_id]);
      } else {
        fprintf(stdout,
                " View %1d Total-bits                 : %" FORMAT_OFF_T
                " (I %" FORMAT_OFF_T ", P %" FORMAT_OFF_T ", NVB %d) \n",
  ...
  if (p_Inp->num_of_views == 2) {
    p_Stats->bitrate = ((float)total_bits * (float)p_Inp->output.frame_rate) /
                       ((float)(p_Stats->frame_counter >> 1));
  } else
#endif
    p_Stats->bitrate = ((float)total_bits * (float)p_Inp->output.frame_rate) /
                       ((float)(p_Stats->frame_counter));
  ...
  if (p_Inp->num_of_views == 2) {
    p_Stats->bitrate_v[0] =
        ((float)total_bits_v[0] * (float)p_Inp->output.frame_rate) /
        ((float)(p_Stats->frame_counter >> 1));
    p_Stats->bitrate_v[1] =
        ((float)total_bits_v[1] * (float)p_Inp->output.frame_rate) /
        ((float)(p_Stats->frame_counter >> 1));
  ...
  if (p_Vid->num_of_layers == 2) {
    for (i = 0; i < 5; i++) {
      p_Stats->bit_ctr_emulationprevention_v[p_Vid->dpb_layer_id] +=
          p_Stats->bit_use_stuffing_bits[i];
    }
  }
#endif
  ...
  if (p_Inp->ReportFrameStats) {
    if ((p_Vid->p_log = fopen("stat_frame.dat", "a")) ==
        NULL)  // append new statistic at the end
    {
      snprintf(errortext, ET_SIZE, "Error open file %s  \n",
               "stat_frame.dat.dat");
 ...
 ************************************************************************
 */
void information_init(VideoParameters *p_Vid, InputParameters *p_Inp,
                      StatParameters *p_Stats) {
  int i;
  static const char yuv_types[4][10] = { "YUV 4:0:0", "YUV 4:2:0", "YUV 4:2:2",
                                         "YUV 4:4:4" };
  switch (p_Inp->Verbose) {
    case 0:
    case 1:
  ...
  fprintf(stdout, " YUV Format                        : %s \n",
          &yuv_types[p_Vid->yuv_format]
                    [0]);  // p_Vid->yuv_format==YUV422?"YUV
                           // 4:2:2":(p_Vid->yuv_format==YUV444)?"YUV
                           // 4:4:4":"YUV 4:2:0");
  ...
  if (p_Inp->Verbose != 0) {
    ...
    fprintf(stdout, " Image format                      : %dx%d (%dx%d)\n",
            p_Inp->output.width[0], p_Inp->output.height[0], p_Vid->width,
            p_Vid->height);
    ...
    if (p_Stats->NumberBFrames > 0 && p_Inp->HierarchicalCoding) {
      ...
      if (p_Inp->BRefPictures)
        fprintf(stdout, " %s (QP: I %d, P %d, RB %d) \n", seqtype,
                ...
                iClip3(0, 51, p_Inp->qp[B_SLICE] + p_Inp->qpBRSOffset));
      else
        fprintf(stdout, " %s (QP: I %d, P %d, B %d) \n", seqtype,
                p_Inp->qp[I_SLICE], p_Inp->qp[P_SLICE], p_Inp->qp[B_SLICE]);
    } else if (p_Stats->NumberBFrames == 0 &&
               (p_Inp->intra_period == 1 || p_Inp->idr_period == 1))
      ...
      fprintf(stdout, " I-P-P-SP-P (QP: I %d, P %d, SP (%d, %d)) \n",
              p_Inp->qp[I_SLICE], p_Inp->qp[P_SLICE], p_Inp->qp[SP_SLICE],
              p_Inp->qpsp);
    ...
    if (p_Inp->full_search == 2)
      ...
      fprintf(stdout,
              " Search range restrictions         : older reference frames\n");
    else
      fprintf(stdout,
              " Search range restrictions         : smaller blocks and older "
              "reference frames\n");
    ...
    switch (p_Inp->of_mode) {
      case PAR_OF_ANNEXB:
        fprintf(stdout,
                " Output File Format                : H.264/AVC Annex B Byte "
                "Stream Format \n");
        break;
      case PAR_OF_RTP:
        fprintf(
    ...
    case 1:
      ...
      if (p_Inp->num_of_views == 2) {
        printf(
            ...
            "------------------\n");
        printf(
            "Frame     View Bit/pic    QP   SnrY    SnrU    SnrV    Time(ms) "
            "MET(ms) Frm/Fld Ref  \n");
        printf(
            "------------------------------------------------------------------"
            "------------------\n");
...
#endif
      {
        printf(
            ...
            "-------------\n");
        printf(
            "Frame     Bit/pic    QP   SnrY    SnrU    SnrV    Time(ms) "
            "MET(ms) Frm/Fld Ref  \n");
        printf(
            "------------------------------------------------------------------"
            "-------------\n");
      ...
      if (p_Inp->num_of_views == 2) {
        if (p_Inp->Distortion[SSIM] == 1) {
          printf(
              ...
              "--\n");
          printf(
              "Frame     View Bit/pic WP QP   QL   SnrY    SnrU    SnrV   "
              "SsimY   SsimU   SsimV    Time(ms) MET(ms) Frm/Fld   I D L0 L1 "
              "RDP Ref\n");
          printf(
              "----------------------------------------------------------------"
              "----------------------------------------------------------------"
          ...
          printf(
              "Frame     View Bit/pic WP QP   QL   SnrY    SnrU    SnrV    "
              "Time(ms) MET(ms) Frm/Fld   I D L0 L1 RDP Ref\n");
          printf(
              "----------------------------------------------------------------"
              "-----------------------------------------\n");
        }
...
#endif
      {
        if (p_Inp->Distortion[SSIM] == 1) {
          printf(
              ...
              "-----------------------------------------------------------\n");
          printf(
              "Frame     Bit/pic WP QP   QL   SnrY    SnrU    SnrV   SsimY   "
              "SsimU   SsimV    Time(ms) MET(ms) Frm/Fld   I D L0 L1 RDP "
              "Ref\n");
          printf(
              "----------------------------------------------------------------"
              "-----------------------------------------------------------\n");
          ...
          printf(
              ...
              "-----------------------------------\n");
          printf(
              "Frame     Bit/pic WP QP   QL   SnrY    SnrU    SnrV    Time(ms) "
              "MET(ms) Frm/Fld   I D L0 L1 RDP Ref\n");
          printf(
              "----------------------------------------------------------------"
              "-----------------------------------\n");
        }
      ...
      if (p_Inp->num_of_views == 2) {
        if (p_Inp->Distortion[SSIM] == 1) {
          printf(
              ...
              "-------\n");
          printf(
              "Frame      View Bit/pic NVB WP QP   QL   SnrY    SnrU    SnrV   "
              "SsimY   SsimU   SsimV    Time(ms) MET(ms) Frm/Fld   I D L0 L1 "
              "RDP Ref\n");
          printf(
              "----------------------------------------------------------------"
...
#endif
      {
        if (p_Inp->Distortion[SSIM] == 1) {
          printf(
              ...
              "\n");
          printf(
              "Frame      Bit/pic NVB WP QP   QL   SnrY    SnrU    SnrV   "
              "SsimY   SsimU   SsimV    Time(ms) MET(ms) Frm/Fld   I D L0 L1 "
              "RDP Ref\n");
          printf(
              "----------------------------------------------------------------"
      ...
      if (p_Inp->num_of_views == 2) {
        if (p_Inp->Distortion[SSIM] == 1) {
          printf(
              ...
              "-----------------\n");
          printf(
              "Frame     View Bit/pic   Filler NVB  WP QP   QL   SnrY    SnrU  "
              "  SnrV   SsimY   SsimU   SsimV    Time(ms) MET(ms) Frm/Fld   I "
              "D L0 L1 RDP Ref\n");
          printf(
              "----------------------------------------------------------------"
              "----------------------------------------------------------------"
          ...
          printf(
              "Frame     View Bit/pic   Filler NVB  WP QP   QL   SnrY    SnrU  "
              "  SnrV    Time(ms) MET(ms) Frm/Fld   I D L0 L1 RDP Ref\n");
          printf(
              "----------------------------------------------------------------"
              "---------------------------------------------------------\n");
        }
...
#endif
      {
        if (p_Inp->Distortion[SSIM] == 1) {
          printf(
              ...
              "----------\n");
          printf(
              "Frame      Bit/pic   Filler NVB  WP QP   QL   SnrY    SnrU    "
              "SnrV   SsimY   SsimU   SsimV    Time(ms) MET(ms) Frm/Fld   I D "
              "L0 L1 RDP Ref\n");
          printf(
              "----------------------------------------------------------------"
              "----------------------------------------------------------------"
...
void report_log_mode(VideoParameters *p_Vid, InputParameters *p_Inp,
  ...
  {
    if ((p_stat = fopen("log_mode.dat", "a")) ==
        NULL)  // append new statistic at the end
    {
      snprintf(errortext, ET_SIZE, "Error open file %s  \n", "log_mode.dat");
      error(errortext, 500);
    } else  // Create header for new log file
    {
      ...
      fprintf(p_stat,
              "|     ver     | Date  | Time  |    Sequence                  | "
              "QP |  I4  |  I8  | I16  | IC0  | IC1  | IC2  | IC3  | PI4  | "
              "PI8  | PI16 |  P0  |  P1  |  P2  |  P3  | P1*4*| P1*8*| P2*4*| "
              "P2*8*| P3*4*| P3*8*|  P8  | P8:4 | P4*4*| P4*8*| P8:5 | P8:6 | "
              "P8:7 | BI4  | BI8  | BI16 |  B0  |  B1  |  B2  |  B3  | B0*4*| "
              "B0*8*| B1*4*| B1*8*| B2*4*| B2*8*| B3*4*| B3*8*|  B8  | B8:0 "
              "|B80*4*|B80*8*| B8:4 | B4*4*| B4*8*| B8:5 | B8:6 | B8:7 |\n");
      fprintf(p_stat,
              " ---------------------------------------------------------------"
              "----------------------------------------------------------------"
              "----------------------------------------------------------------"
  ...
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[P_SLICE][1][0]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[P_SLICE][1][1]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[P_SLICE][2][0]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[P_SLICE][2][1]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[P_SLICE][3][0]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[P_SLICE][3][1]);

  fprintf(p_stat, " %5" FORMAT_OFF_T "|", p_Stats->mode_use[P_SLICE][P8x8]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|", p_Stats->mode_use[P_SLICE][4]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[P_SLICE][4][0]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[P_SLICE][4][1]);
  ...
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][0][0]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][0][1]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][1][0]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][1][1]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][2][0]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][2][1]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][3][0]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][3][1]);

  fprintf(p_stat, " %5" FORMAT_OFF_T "|", p_Stats->mode_use[B_SLICE][P8x8]);
  fprintf(p_stat, " %d|",
          (p_Stats->b8_mode_0_use[B_SLICE][0] +
           p_Stats->b8_mode_0_use[B_SLICE][1]));
  ...
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][4][0]);
  fprintf(p_stat, " %5" FORMAT_OFF_T "|",
          p_Stats->mode_use_transform[B_SLICE][4][1]);
