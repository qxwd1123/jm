...
static void DeblockMb(VideoParameters *p_Vid, imgpel **imgY, imgpel ***imgUV,
                      int MbQAddr);
...
#else
static void DeblockParallel(VideoParameters *p_Vid, imgpel **imgY,
                            imgpel ***imgUV, unsigned int column, int block,
                            int n_last) {
  int i, j;
...
void DeblockFrame(VideoParameters *p_Vid, imgpel **imgY, imgpel ***imgUV) {
  ...
  for (i = 0; i < k; i++) {
    ...
    int n_start =
        (i < p_Vid->PicWidthInMbs) ? 0 : ((i - p_Vid->PicWidthInMbs) >> 1) + 1;
...
static void DeblockMb(VideoParameters *p_Vid, imgpel **imgY, imgpel ***imgUV,
  ...
  int mvlimit =
      (p_Vid->structure != FRAME) || (p_Vid->mb_aff_frame_flag && MbQ->mb_field)
          ? 2
          : 4;
  ...
  if (MbQ->mb_type == I8MB) assert(MbQ->luma_transform_size_8x8_flag);
  ...
  if (MbQ->DFDisableIdc == 2) {
    ...
    // edge
    filterTopMbEdgeFlag =
        (p_Vid->mb_aff_frame_flag && !MbQ->mb_field && (MbQAddr & 0x01))
            ? 1
            : MbQ->mbAvailB;
  }
  ...
  for (edge = 0; edge < 4; ++edge) {
    ...
    if (MbQ->cbp == 0) {
      if (filterNon8x8LumaEdgesFlag[edge] == 0 &&
        ...
        continue;
      else if (edge > 0 && (currSlice->slice_type == P_SLICE ||
                            currSlice->slice_type == B_SLICE)) {
        if ((MbQ->mb_type == 0 && currSlice->slice_type == P_SLICE) ||
            (MbQ->mb_type == 1) || (MbQ->mb_type == 2))
          continue;
        else if ((edge & 0x01) && ((MbQ->mb_type == 3) ||
                                   ((edge & 0x01) && MbQ->mb_type == 0 &&
                                    currSlice->slice_type == B_SLICE &&
    ...
    if (edge || filterLeftMbEdgeFlag) {
      ...
      p_Vid->GetStrengthVer(Strength, MbQ, edge << 2,
                            mvlimit);  // Strength for 4 blks in 1 stripe

      if (Strength[0] != 0 || Strength[1] != 0 || Strength[2] != 0 ||
          Strength[3] != 0 || Strength[4] != 0 || Strength[5] != 0 ||
          Strength[6] != 0 || Strength[7] != 0 || Strength[8] != 0 ||
          Strength[9] != 0 || Strength[10] != 0 || Strength[11] != 0 ||
          Strength[12] != 0 || Strength[13] != 0 || Strength[14] != 0 ||
          Strength[15] != 0)  // only if one of the 16 Strength bytes is != 0
      {
  ...
  for (edge = 0; edge < 4; ++edge) {
    ...
    if (edge || filterTopMbEdgeFlag) {
      ...
      {
        if (filterNon8x8LumaEdgesFlag[edge]) {
          p_Vid->EdgeLoopLumaHor(PLANE_Y, imgY, Strength, MbQ, edge << 2,
                                 p_Vid->width_padded);
          if (p_Vid->P444_joined) {
            p_Vid->EdgeLoopLumaHor(PLANE_U, imgUV[0], Strength, MbQ, edge << 2,
                                   p_Vid->width_padded);
            p_Vid->EdgeLoopLumaHor(PLANE_V, imgUV[1], Strength, MbQ, edge << 2,
                                   p_Vid->width_padded);
          }
        }
        if (p_Vid->yuv_format == YUV420 || p_Vid->yuv_format == YUV422) {
          ...
          if ((imgUV != NULL) && (edge_cr >= 0)) {
            p_Vid->EdgeLoopChromaHor(imgUV[0], Strength, MbQ, edge_cr,
                                     p_Vid->width_cr + p_Vid->pad_size_uv_x * 2,
                                     0);
            p_Vid->EdgeLoopChromaHor(imgUV[1], Strength, MbQ, edge_cr,
                                     p_Vid->width_cr + p_Vid->pad_size_uv_x * 2,
                                     1);
          }
        }
      ...
      if (!edge && !MbQ->mb_field && p_Vid->mixedModeEdgeFlag) {
        // this is the extra horizontal edge between a frame macroblock pair and
        // a field above it
        MbQ->DeblockCall = 2;
        p_Vid->GetStrengthHor(Strength, MbQ, MB_BLOCK_SIZE,
                              mvlimit);  // Strength for 4 blks in 1 stripe
        // if( Strength[0] != 0 || Strength[1] != 0 || Strength[2] != 0 ||
        // Strength[3] != 0  )                      // only if one of the 4
        // Strength bytes is != 0
        {
          if (filterNon8x8LumaEdgesFlag[edge]) {
            p_Vid->EdgeLoopLumaHor(PLANE_Y, imgY, Strength, MbQ, MB_BLOCK_SIZE,
                                   p_Vid->width_padded);
            if (p_Vid->P444_joined) {
              p_Vid->EdgeLoopLumaHor(PLANE_U, imgUV[0], Strength, MbQ,
                                     MB_BLOCK_SIZE, p_Vid->width_padded);
          ...
          if (p_Vid->yuv_format == YUV420 || p_Vid->yuv_format == YUV422) {
            edge_cr = chroma_edge[1][edge][p_Vid->yuv_format];
            if ((imgUV != NULL) && (edge_cr >= 0)) {
              p_Vid->EdgeLoopChromaHor(
                  imgUV[0], Strength, MbQ, MB_BLOCK_SIZE,
                  p_Vid->width_cr + p_Vid->pad_size_uv_x * 2, 0);
              p_Vid->EdgeLoopChromaHor(
                  imgUV[1], Strength, MbQ, MB_BLOCK_SIZE,
                  p_Vid->width_cr + p_Vid->pad_size_uv_x * 2, 1);
            }
          }
        }
        MbQ->DeblockCall = 1;
