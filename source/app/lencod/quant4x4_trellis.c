 ...
 ************************************************************************
 */
int quant_4x4_trellis(Macroblock *currMB, int **tblock,
    ...
    if (*m7 != 0) {
      ...
      */
      ...
      if (level != 0) {
        ...
        *m7 = rshift_rnd_sf(
            ((level * q_params_4x4[j][i].InvScaleComp) << qp_per), 4);
        *ACL++ = level;
        *ACR++ = run;
...
************************************************************************
*/
void rdoq_4x4_CAVLC(Macroblock *currMB, int **tblock,
                    struct quant_methods *q_method, int levelTrellis[]) {
  ...
  init_trellis_data_4x4_CAVLC(currMB, tblock, block_x, qp_per, qp_rem,
                              q_params_4x4, p_scan, &levelData[0], type);
  est_RunLevel_CAVLC(currMB, levelData, levelTrellis, LUMA, b8, b4, 16,
                     lambda_md);
}
/*!
************************************************************************
...
************************************************************************
*/
void rdoq_4x4_CABAC(Macroblock *currMB, int **tblock,
                    struct quant_methods *q_method, int levelTrellis[]) {
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  noCoeff =
      init_trellis_data_4x4_CABAC(currMB, tblock, q_method, p_scan,
                                  &levelData[0], &kStart, &kStop, LUMA_4x4);
  estBits = est_write_and_store_CBP_block_bit(currMB, LUMA_4x4);
  est_writeRunLevel_CABAC(currMB, levelData, levelTrellis, LUMA_4x4, lambda_md,
                          kStart, kStop, noCoeff, estBits);
}

/*!
 ************************************************************************
 * \brief
 *    Quantization process for All coefficients for a 4x4 block (LUMA_16AC or
 *CHROMA_AC)
 *
 ************************************************************************
 */
int quant_ac4x4_trellis(Macroblock *currMB, int **tblock,
                        struct quant_methods *q_method) {
  int block_x = q_method->block_x;
      ...
      */
      ...
      if (level != 0) {
        ...
        *m7 = rshift_rnd_sf(
        ...
        // *m7 = (qp_per<4) ?
        // rshift_rnd_sf((level*q_params_4x4[j][i].InvScaleComp),4-qp_per) :
        // (level*q_params_4x4[j][i].InvScaleComp)<<(qp_per-4);
        *ACL++ = level;
        *ACR++ = run;
        // reset zero level counter
...
************************************************************************
*/
void rdoq_ac4x4_CAVLC(Macroblock *currMB, int **tblock,
                      struct quant_methods *q_method, int levelTrellis[]) {
  VideoParameters *p_Vid = currMB->p_Vid;
  int block_x = q_method->block_x;
  int block_y = q_method->block_y;
  ...
  init_trellis_data_4x4_CAVLC(currMB, tblock, block_x, qp_per, qp_rem,
                              q_params_4x4, p_scan, &levelData[0], type);
  est_RunLevel_CAVLC(currMB, levelData, levelTrellis, block_type, b8, b4, 15,
                     lambda_md);
}
/*!
************************************************************************
...
************************************************************************
*/
void rdoq_ac4x4_CABAC(Macroblock *currMB, int **tblock,
                      struct quant_methods *q_method, int levelTrellis[]) {
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  noCoeff = init_trellis_data_4x4_CABAC(currMB, tblock, q_method, p_scan,
                                        &levelData[0], &kStart, &kStop, type);
  estBits = est_write_and_store_CBP_block_bit(currMB, type);
  est_writeRunLevel_CABAC(currMB, levelData, levelTrellis, type, lambda_md,
                          kStart, kStop, noCoeff, estBits);
}
 ...
 ************************************************************************
 */
int quant_dc4x4_trellis(Macroblock *currMB, int **tblock, int qp, int *DCLevel,
  ...
  currSlice->rdoq_dc(currMB, tblock, qp_per, qp_rem, q_params_4x4, pos_scan,
                     levelTrellis, LUMA_16DC);
...
************************************************************************
*/
void rdoq_dc_CAVLC(Macroblock *currMB, int **tblock, int qp_per, int qp_rem,
                   LevelQuantParams *q_params_4x4, const byte (*pos_scan)[2],
                   int levelTrellis[], int type) {
  VideoParameters *p_Vid = currMB->p_Vid;
  const byte *p_scan = &pos_scan[0][0];
  levelDataStruct levelData[16];
  double lambda_md = p_Vid->lambda_rdoq[p_Vid->type][p_Vid->masterQP];

  init_trellis_data_DC_CAVLC(currMB, tblock, qp_per, qp_rem, q_params_4x4,
                             p_scan, &levelData[0]);
  est_RunLevel_CAVLC(currMB, levelData, levelTrellis, LUMA_INTRA16x16DC, 0, 0,
                     16, lambda_md);
}
...
************************************************************************
*/
void rdoq_dc_CABAC(Macroblock *currMB, int **tblock, int qp_per, int qp_rem,
                   LevelQuantParams *q_params_4x4, const byte (*pos_scan)[2],
                   int levelTrellis[], int type) {
  VideoParameters *p_Vid = currMB->p_Vid;
  const byte *p_scan = &pos_scan[0][0];
  ...
  noCoeff =
      init_trellis_data_DC_CABAC(currMB, tblock, qp_per, qp_rem, q_params_4x4,
                                 p_scan, &levelData[0], &kStart, &kStop);
  estBits = est_write_and_store_CBP_block_bit(currMB, type);
  est_writeRunLevel_CABAC(currMB, levelData, levelTrellis, type, lambda_md,
                          kStart, kStop, noCoeff, estBits);
}
