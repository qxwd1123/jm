 ...
 ************************************************************************
 */
int quant_4x4_around(Macroblock *currMB, int **tblock,
  ...
  for (coeff_ctr = 0; coeff_ctr < 16; ++coeff_ctr) {
    ...
    if (*m7 != 0) {
      ...
      if (level != 0) {
        ...
        *padjust4x4 = rshift_rnd_sf(
            (AdaptRndWeight * (scaled_coeff - (level << q_bits))), q_bits + 1);
        ...
        *m7 = rshift_rnd_sf(((level * q_params->InvScaleComp) << qp_per), 4);
        ...
        // *m7 = (qp_per<4) ?
        // rshift_rnd_sf((level*q_params->InvScaleComp),4-qp_per) :
        // (level*q_params->InvScaleComp)<<(qp_per-4);
        *ACL++ = level;
        *ACR++ = run;
...
int quant_ac4x4_around(Macroblock *currMB, int **tblock,
                       struct quant_methods *q_method) {
  int block_x = q_method->block_x;
  ...
  for (coeff_ctr = 1; coeff_ctr < 16; ++coeff_ctr) {
    ...
    if (*m7 != 0) {
      ...
      if (level != 0) {
        ...
        *padjust4x4 =
            rshift_rnd_sf((AdaptRndWeight * (scaled_coeff - (level << q_bits))),
                          (q_bits + 1));
 ...
 ************************************************************************
 */
int quant_dc4x4_around(Macroblock *currMB, int **tblock, int qp, int *DCLevel,
