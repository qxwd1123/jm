...
static const MotionVector Diamond_SubPelSearch[4] = {
  { -1, 0 }, { 1, 0 }, { 0, -1 }, { 0, 1 }
};
static const MotionVector Hexagon[6] = { { -8, 0 }, { 8, 0 },  { -4, -8 },
                                         { 4, 8 },  { -4, 8 }, { 4, -8 } };
static const short Big_Hexagon_X[16] = { -16, 16, 0,   0,  -16, 16, -16, 16,
                                         -16, 16, -16, 16, -8,  8,  -8,  8 };
static const short Big_Hexagon_Y[16] = { 0,  0, -16, 16, -4,  4,  4,  -4,
                                         -8, 8, 8,   -8, -12, 12, 12, -12 };

static const short block_type_shift_factor[8] = {
  0, 0, 1, 1, 2, 3, 3, 1
};  // last one relaxed to 1 instead 4
...
#define SEARCH_ONE_PIXEL_BIPRED                                             \
  if ((iabs(cand.mv_x - center2.mv_x) >> 2) <= search_range &&              \
    ...
    if (mcost < min_mcost) {                                                \
      mcost +=                                                              \
          mv_block->computeBiPredFPel(ref_picture1, ref_picture2, mv_block, \
                                      min_mcost - mcost, &center1, &cand);  \
      if (mcost < min_mcost) {                                              \
        best = cand;                                                        \
        min_mcost = mcost;                                                  \
      }                                                                     \
...
int smpUMHEX_get_mem(VideoParameters *p_Vid) {
  ...
  int memory_size = 0;
  if (NULL ==
      (p_UMHexSMP->flag_intra = calloc((p_Vid->width >> 4) + 1, sizeof(byte))))
    no_mem_exit("smpUMHEX_get_mem: flag_intra");

  memory_size += get_mem3Ddistblk(&p_UMHexSMP->l0_cost, 9, p_Vid->height >> 2,
                                  p_Vid->width >> 2);
  memory_size += get_mem3Ddistblk(&p_UMHexSMP->l1_cost, 9, p_Vid->height >> 2,
                                  p_Vid->width >> 2);
...
smpUMHEXIntegerPelBlockMotionSearch(
  ...
  if (min_mcost <
      (p_UMHexSMP->ConvergeThreshold >> block_type_shift_factor[blocktype])) {
    for (m = 0; m < 4; m++) {
  ...
  // If distortion is large, use large shapes. Otherwise, compact shapes are
  // faster
  if ((blocktype == 1 &&
       min_mcost > (p_UMHexSMP->SymmetricalCrossSearchThreshold1 >>
                    block_type_shift_factor[blocktype])) ||
      (min_mcost > (p_UMHexSMP->SymmetricalCrossSearchThreshold2 >>
                    block_type_shift_factor[blocktype]))) {
    iMinNow = best;
  ...
  for (j = (mv_block->pos_y2); j < (mv_block->pos_y2) + (blocksize_y >> 2);
       j++) {
    for (i = (mv_block->pos_x2); i < (mv_block->pos_x2) + (blocksize_x >> 2);
         i++) {
      if (mv_block->list == 0) {
 ...
 */
distblk  //  ==> minimum motion cost after search
smpUMHEXFullSubPelBlockMotionSearch(
    Macroblock *currMB,  // <--  current Macroblock
    MotionVector
        *pred_mv,  // <--  motion vector predictor (x|y) in sub-pel units
    MEBlock *mv_block,
  ...
  int check_position0 =
      (!p_Inp->rdopt && currSlice->slice_type != B_SLICE && ref == 0 &&
       blocktype == 1 && mv->mv_x == 0 && mv->mv_y == 0);

  int max_pos2 =
      (!p_Vid->start_me_refinement_hp ? imax(1, mv_block->search_pos2)
  ...
  for (best_pos = 0, pos = p_Vid->start_me_refinement_hp; pos < max_pos2;
       pos++) {
    cand.mv_x =
        mv->mv_x + p_Vid->spiral_hpel_search[pos].mv_x;  // quarter-pel units
    cand.mv_y =
        mv->mv_y + p_Vid->spiral_hpel_search[pos].mv_y;  // quarter-pel units
    ...
    if (min_mcost <
        (p_UMHexSMP->SubPelThreshold3 >> block_type_shift_factor[blocktype])) {
      break;
    }
  }
  ...
  if ((mv->mv_x == 0) && (mv->mv_y == 0) &&
      (pred_mv->mv_x == 0 && pred_mv->mv_y == 0) &&
      (min_mcost <
       (p_UMHexSMP->SubPelThreshold1 >> block_type_shift_factor[blocktype]))) {
    best_pos = 0;
    return min_mcost;
  }
  if (!p_Vid->start_me_refinement_qp) min_mcost = DISTBLK_MAX;
  /************************************
   *****                          *****
  ...
  //===== loop over search positions =====
  for (best_pos = 0, pos = p_Vid->start_me_refinement_qp;
       pos < mv_block->search_pos4; pos++) {
...
smpUMHEXSubPelBlockMotionSearch(
  ...
  memset(p_UMHexSMP->SearchState[0], 0,
  ...
  if (pred_frac_mv_x || pred_frac_mv_y) {
    ...
    p_UMHexSMP->SearchState[cand.mv_y - mv->mv_y + dynamic_search_range]
  ...
  for (i = 0; i < dynamic_search_range; i++) {
    ...
    for (m = 0; m < 4; m++) {
      cand.mv_x =
          iMinNow.mv_x + Diamond_SubPelSearch[m].mv_x;  // Diamond[m].mv_x;
      cand.mv_y =
          iMinNow.mv_y + Diamond_SubPelSearch[m].mv_y;  // Diamond[m].mv_y;
      ...
      if (iabs(cand.mv_x - mv->mv_x) <= dynamic_search_range &&
        ...
        if (!p_UMHexSMP
          ...
          if (min_mcost < (p_UMHexSMP->SubPelThreshold3 >>
                           block_type_shift_factor[blocktype])) {
            *mv = currmv;
            return min_mcost;
          }
...
smpUMHEXSubPelBlockME(Macroblock *currMB,     // <--  current Macroblock
                      MotionVector *pred_mv,  // <--  motion vector predictor
  ...
  if (mv_block->blocktype > 1) {
    min_mcost = smpUMHEXSubPelBlockMotionSearch(
        currMB, pred_mv, mv_block, min_mcost, lambda_factor[Q_PEL]);
  } else {
    min_mcost = smpUMHEXFullSubPelBlockMotionSearch(
        currMB, pred_mv, mv_block, min_mcost, lambda_factor[Q_PEL]);
  }
 ...
 ************************************************************************
 * \brief
 *    smpUMHEXBipredIntegerPelBlockMotionSearch: fast pixel block motion search
 *for bipred mode
 *
 ************************************************************************
 */
...
smpUMHEXBipredIntegerPelBlockMotionSearch(
  ...
  pred1.mv_x = mv_block->pos_x_padded +
               pred_mv1->mv_x;  // predicted position x (in sub-pel units)
  pred1.mv_y = mv_block->pos_y_padded +
               pred_mv1->mv_y;  // predicted position y (in sub-pel units)
  pred2.mv_x = mv_block->pos_x_padded +
               pred_mv2->mv_x;  // predicted position x (in sub-pel units)
  pred2.mv_y = mv_block->pos_y_padded +
               pred_mv2->mv_y;  // predicted position y (in sub-pel units)

  center2.mv_x = mv_block->pos_x_padded +
                 mv1->mv_x;  // center position x (in sub-pel units)
  center2.mv_y = mv_block->pos_y_padded +
                 mv1->mv_y;  // center position y (in sub-pel units)
  center1.mv_x = mv_block->pos_x_padded +
                 mv2->mv_x;  // mvx of second pred (in sub-pel units)
  center1.mv_y = mv_block->pos_y_padded +
                 mv2->mv_y;  // mvy of second pred (in sub-pel units)
  ...
  iMinNow = best;
  if (0 != pred_mv1->mv_x || 0 != pred_mv1->mv_y || 0 != pred_mv2->mv_x ||
      0 != pred_mv2->mv_y) {
    cand.mv_x = pic_pix_x;
    cand.mv_y = pic_pix_y;
  ...
  // faster
  if ((blocktype == 1 &&
       (min_mcost << 2) > (p_UMHexSMP->SymmetricalCrossSearchThreshold1 >>
                           block_type_shift_factor[blocktype])) ||
      ((min_mcost << 2) > (p_UMHexSMP->SymmetricalCrossSearchThreshold2 >>
                           block_type_shift_factor[blocktype]))) {
...
void smpUMHEX_decide_intrabk_SAD(Macroblock *currMB) {
  ...
  if (currSlice->slice_type != I_SLICE && currSlice->slice_type != SI_SLICE) {
    if (currMB->pix_x == 0 && currMB->pix_y == 0) {
      ...
      p_UMHexSMP->flag_intra_SAD =
          p_UMHexSMP->flag_intra[((currMB->pix_x) >> 4) - 1];
    } else {
      p_UMHexSMP->flag_intra_SAD =
          ((p_UMHexSMP->flag_intra[(currMB->pix_x) >> 4]) ||
           (p_UMHexSMP->flag_intra[((currMB->pix_x) >> 4) - 1]) ||
           (p_UMHexSMP->flag_intra[((currMB->pix_x) >> 4) + 1]));
    }
  }
 ...
 ************************************************************************
 */
void smpUMHEX_setup(Macroblock *currMB, short ref, int list, int block_y,
  ...
  if (blocktype > 1) {
    if (blocktype > 6) {
      p_UMHexSMP->pred_SAD_uplayer =
          (list == 1) ? (p_UMHexSMP->l1_cost[5][(currMB->block_y) + block_y]
                                            [(currMB->block_x) + block_x])
                      : (p_UMHexSMP->l0_cost[5][(currMB->block_y) + block_y]
                                            [(currMB->block_x) + block_x]);
      p_UMHexSMP->pred_SAD_uplayer /= 2;
    } else if (blocktype > 4) {
      p_UMHexSMP->pred_SAD_uplayer =
          (list == 1) ? (p_UMHexSMP->l1_cost[4][(currMB->block_y) + block_y]
                                            [(currMB->block_x) + block_x])
                      : (p_UMHexSMP->l0_cost[4][(currMB->block_y) + block_y]
                                            [(currMB->block_x) + block_x]);
      p_UMHexSMP->pred_SAD_uplayer /= 2;
    } else if (blocktype == 4) {
      p_UMHexSMP->pred_SAD_uplayer =
          (list == 1) ? (p_UMHexSMP->l1_cost[2][(currMB->block_y) + block_y]
                                            [(currMB->block_x) + block_x])
                      : (p_UMHexSMP->l0_cost[2][(currMB->block_y) + block_y]
                                            [(currMB->block_x) + block_x]);
      p_UMHexSMP->pred_SAD_uplayer /= 2;
    } else {
      p_UMHexSMP->pred_SAD_uplayer =
          (list == 1) ? (p_UMHexSMP->l1_cost[1][(currMB->block_y) + block_y]
                                            [(currMB->block_x) + block_x])
                      : (p_UMHexSMP->l0_cost[1][(currMB->block_y) + block_y]
                                            [(currMB->block_x) + block_x]);
      p_UMHexSMP->pred_SAD_uplayer /= 2;
    }

    p_UMHexSMP->pred_SAD_uplayer =
        p_UMHexSMP->flag_intra_SAD ? 0 : p_UMHexSMP->pred_SAD_uplayer;
  }
}
