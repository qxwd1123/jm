...
struct umhex_struct {
  ...
  distblk Threshold_DSR_MB[8];  //!<  Threshold for usage of DSR. DSR refer to
                                //!<  JVT-Q088
  byte **McostState;            //!< state for integer pel search
  byte **SearchState;           //!< state for fractional pel search
  distblk ****fastme_ref_cost;  //!< store SAD information needed for forward
                                //!< ref-frame prediction
  distblk ***fastme_l0_cost;    //!< store SAD information needed for forward
                                //!< median and uplayer prediction
  distblk ***fastme_l1_cost;    //!< store SAD information needed for backward
                                //!< median and uplayer prediction
  ...
  int pred_MV_ref[2],
      pred_MV_uplayer[2];  //!< pred motion vector by space or temporal
...
#define SEARCH_ONE_PIXEL                                                       \
  if ((iabs(cand.mv_x - center.mv_x) >> 2) < search_range &&                   \
    ...
    if (!p_UMHex                                                               \
             ->McostState[((cand.mv_y - center.mv_y) >> 2) + search_range]     \
                         [((cand.mv_x - center.mv_x) >> 2) + search_range]) {  \
      mcost = mv_cost(p_Vid, lambda_factor, &cand, &pred);                     \
      if (mcost < min_mcost) {                                                 \
        mcost += mv_block->computePredFPel(ref_picture, mv_block,              \
                                           min_mcost - mcost, &cand);          \
        p_UMHex->McostState[((cand.mv_y - center.mv_y) >> 2) + search_range]   \
                           [((cand.mv_x - center.mv_x) >> 2) + search_range] = \
            1;                                                                 \
        if (mcost < min_mcost) {                                               \
          best = cand;                                                         \
...
#define SEARCH_ONE_PIXEL_BIPRED                                                \
  if ((iabs(cand.mv_x - center2.mv_x) >> 2) < search_range &&                  \
    ...
    if (!p_UMHex                                                               \
             ->McostState[((cand.mv_y - center2.mv_y) >> 2) + search_range]    \
                         [((cand.mv_x - center2.mv_x) >> 2) + search_range]) { \
...
extern void UMHEX_DefineThresholdMB(VideoParameters *p_Vid,
...
extern void UMHEX_setup(Macroblock *currMB, short ref, int list, int block_y,
...
extern distblk
UMHEXSubPelBlockMotionSearch(  //  ==> minimum motion cost after search
    Macroblock *currMB,        // <--  current Macroblock
    MotionVector
...
extern distblk UMHEXBipredIntegerPelBlockMotionSearch(
    Macroblock *, int, MotionVector *, MotionVector *, MotionVector *,
    MotionVector *, MEBlock *, int, distblk, int);

extern void UMHEXSetMotionVectorPredictor(Macroblock *currMB, MotionVector *pmv,
                                          struct pic_motion_params **mv_info,
                                          short ref_frame, int list, int mb_x,
                                          int mb_y, int bl_x, int bl_y,
                                          MEBlock *mv_block);
