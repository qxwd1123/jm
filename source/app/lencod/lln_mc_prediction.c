 ...
 ************************************************************************
 */
static inline void mc_prediction(imgpel **mb_pred, int block_size_y,
                                 int block_size_x, int ioff,
                                 imgpel block[MB_BLOCK_SIZE][MB_BLOCK_SIZE]) {
  int jj;

  if (block_size_x == MB_BLOCK_SIZE) {
    memcpy(&(mb_pred[0][ioff]), &(block[0][0]),
           block_size_x * block_size_y * sizeof(imgpel));
  } else {
    for (jj = 0; jj < block_size_y; jj++) {
      memcpy(&(mb_pred[jj][ioff]), &(block[jj][0]),
             block_size_x * sizeof(imgpel));
    }
  }
}
 ...
 ************************************************************************
 */
static inline void weighted_mc_prediction(
    imgpel **mb_pred, int block_size_y, int block_size_x, int ioff,
    imgpel block[MB_BLOCK_SIZE][MB_BLOCK_SIZE], int wp_scale, int wp_offset,
 ...
 ************************************************************************
 */
static inline void weighted_bi_prediction(
    imgpel **mb_pred, imgpel block_l0[MB_BLOCK_SIZE][MB_BLOCK_SIZE],
    imgpel block_l1[MB_BLOCK_SIZE][MB_BLOCK_SIZE], int block_size_y,
    int block_size_x, int ioff, int wp_scale_l0, int wp_scale_l1, int wp_offset,
    int weight_denom, int color_clip) {
  int ii, jj;
  imgpel *mpr, *b0, *b1;
  ...
  for (jj = 0; jj < block_size_y; jj++) {
    ...
    for (ii = 0; ii < block_size_x; ii++)
      *(mpr++) = (imgpel)iClip1(
          color_clip,
          (rshift_rnd((wp_scale_l0 * *(b0++) + wp_scale_l1 * *(b1++)),
                      weight_denom) +
           wp_offset));
  }
}

void get_block_luma(Macroblock *currMB, int decoder, ColorPlane pl,
                    StorablePicture *dec_picture, StorablePicture *curr_ref,
                    int x_pos, int y_pos, int block_size_x, int block_size_y,
                    imgpel block[MB_BLOCK_SIZE][MB_BLOCK_SIZE]) {
  ...
  if ((p_Inp->separate_colour_plane_flag != 0)) {
    cur_imgY =
        curr_ref->de_mem->p_dec_img[(short)p_Vid->colour_plane_id][decoder];
  } else {
  ...
  if ((y_pos > 1) && (y_pos < maxold_y - 2 - block_size_y) && (x_pos > 1) &&
      (x_pos < maxold_x - 2 - block_size_x)) {
    cur_imgY = &cur_imgY[y_pos];
    if (dx == 0 && dy == 0) { /* fullpel position */
      for (j = 0; j < block_size_y; j++) {
        memcpy(&(block[j][0]), &(cur_imgY[j][x_pos]),
               block_size_x * sizeof(imgpel));
      }
      ...
      if (dy == 0) { /* No vertical interpolation */
        for (j = 0; j < block_size_y; j++) {
          ...
          for (i = 0; i < block_size_x; i++) {
            ...
            *orig_line++ =
                (imgpel)iClip1(max_imgpel_value, ((result + 16) >> 5));
          }
        }
        ...
        for (j = 0; j < block_size_y; j++) {
          ...
          for (i = 0; i < block_size_x; i++) {
            result = (*x0++ + *x5++) * COEF[0] + (*x1++ + *x4++) * COEF[1] +
                     (*x2++ + *x3++) * COEF[2];

            *(orig_line++) =
                (imgpel)iClip1(max_imgpel_value, ((result + 512) >> 10));
          }
        }
        ...
        if ((dy & 1) == 1) {
          ...
          for (j = 0; j < block_size_y; j++) {
            ...
            orig_line = block[j];
            for (i = 0; i < block_size_x; i++) {
              *orig_line = (imgpel)((*orig_line +
                                     iClip1(max_imgpel_value,
                                            ((*(tmp_line++) + 16) >> 5)) +
                                     1) >>
                                    1);
              orig_line++;
      ...
      if (dy == 0) { /* No vertical interpolation */
        ...
        if ((dy & 1) == 1) {
          for (j = 0; j < block_size_y; j++) {
            ...
            for (i = 0; i < block_size_x; i++) {
              *orig_line =
                  (imgpel)((*orig_line +
                            cur_lineY[iClip3(0, maxold_x, x_pos + i)] + 1) >>
                           1);
              orig_line++;
            }
          }
        ...
        for (i = 0; i < block_size_x; i++) {
          ...
          for (j = 0; j < block_size_y + 5; j++) {
            ...
            tmp_res[j][i] +=
                (cur_lineY[ipos_m1] + cur_lineY[ipos_p2]) * COEF[1];
        ...
        for (i = 0; i < block_size_x; i++) {
          ...
          for (j = 0; j < block_size_y; j++) {
            cur_lineY = cur_imgY[iClip3(0, maxold_y,
                                        (dy == 1 ? y_pos + j : y_pos + j + 1))];
...
void get_block_chroma(Macroblock *currMB, int decoder, int uv,
                      StorablePicture *dec_picture, StorablePicture *curr_ref,
                      int x_pos, int y_pos, int block_size_x, int block_size_y,
                      imgpel block[MB_BLOCK_SIZE][MB_BLOCK_SIZE]) {
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  if ((y_pos >= 0) && (y_pos < maxold_y - block_size_y) && (x_pos >= 0) &&
      (x_pos < maxold_x - block_size_x)) {
    if (dx == 0 && dy == 0) { /* fullpel position */
      for (j = 0; j < block_size_y; j++) {
        memcpy(&(block[j][0]), &(cur_img[y_pos + j][x_pos]),
               block_size_x * sizeof(imgpel));
      }
    } else if (dx == 0) {
      for (j = 0; j < block_size_y; j++) {
        ...
        for (i = 0; i < block_size_x; i++) {
          ...
          *(blk_line++) = (imgpel)iClip1(max_imgpel_value,
                                         rshift_rnd_sf(result, total_scale));
        }
      }
    } else if (dy == 0) {
      for (j = 0; j < block_size_y; j++) {
    ...
    if (dx == 0 && dy == 0) { /* fullpel position */
      ...
      for (j = 0; j < block_size_y; j++) {
        ...
        for (i = 0; i < block_size_x; i++) {
          ...
          result = (w00 * cur_line[ipos] + w10 * cur_line[ipos_p1] +
                    w01 * cur_line_p1[ipos] + w11 * cur_line_p1[ipos_p1]);
          *(blk_line++) = (imgpel)iClip1(max_imgpel_value,
                                         rshift_rnd_sf(result, total_scale));
        }
      }
...
void perform_mc(Macroblock *currMB, int decoder, ColorPlane pl,
                StorablePicture *dec_picture, int pred_dir, int l0_mode,
                int l1_mode, PicMotionParams **mv_info, int i, int j,
                int block_size_x, int block_size_y, short bipred_me) {
  ...
  int max_imgpel_value = p_Vid->max_pel_value_comp[pl];
  int apply_weights = ((p_Vid->active_pps->weighted_pred_flag &&
                        (currSlice->slice_type == P_SLICE ||
                         currSlice->slice_type == SP_SLICE)) ||
                       (p_Vid->active_pps->weighted_bipred_idc &&
  ...
  if (pred_dir != 2) {
    ...
    get_block_luma(currMB, decoder, pl, dec_picture, list[ref_idx], vec1_x,
                   vec1_y, block_size_x, block_size_y, tmp_block_l0);
    ...
    if (apply_weights) {
      ...
      weighted_mc_prediction(&p_Vid->p_decs->dec_mb_pred[decoder][joff],
                             block_size_y, block_size_x, ioff, tmp_block_l0,
                             alpha_l0, wp_off, currSlice->luma_log_weight_denom,
                             max_imgpel_value);
    } else {
      mc_prediction(&p_Vid->p_decs->dec_mb_pred[decoder][joff], block_size_y,
                    block_size_x, ioff, tmp_block_l0);
    }
    ...
    MotionVector **l0_mv_array =
        (bipred_me
             ? currSlice->bipred_mv[bipred_me - 1][LIST_0][l0_ref][l0_mode]
             : currSlice->all_mv[LIST_0][l0_ref][l0_mode]);
    MotionVector **l1_mv_array =
        (bipred_me
             ? currSlice->bipred_mv[bipred_me - 1][LIST_1][l1_ref][l1_mode]
             : currSlice->all_mv[LIST_1][l1_ref][l1_mode]);
    ...
    get_block_luma(currMB, decoder, pl, dec_picture,
                   currSlice->listX[LIST_0 + currMB->list_offset][l0_ref],
                   vec1_x, vec1_y, block_size_x, block_size_y, tmp_block_l0);
    get_block_luma(currMB, decoder, pl, dec_picture,
                   currSlice->listX[LIST_1 + currMB->list_offset][l1_ref],
                   vec2_x, vec2_y, block_size_x, block_size_y, tmp_block_l1);

    if (apply_weights) {
      int wt_list_offset = (p_Vid->active_pps->weighted_bipred_idc == 2)
                               ? currMB->list_offset
                               : 0;

      // This code existed in the original. Seems pointless but copying it here
      // for reference and in case temporal direct breaks. if (mv_mode==0 &&
      // p_Vid->direct_spatial_mv_pred_flag==0 ) l1_ref_idx=0;
      if (currMB->mb_field) {
        l0_ref >>= 1;
        l1_ref >>= 1;
      }
      ...
      alpha_l0 =
          currSlice->wbp_weight[LIST_0 + wt_list_offset][l0_ref][l1_ref][0];
      alpha_l1 =
          currSlice->wbp_weight[LIST_1 + wt_list_offset][l0_ref][l1_ref][0];
      wp_off =
          ((currSlice->wp_offset[LIST_0 + wt_list_offset][l0_ref][0] +
            currSlice->wp_offset[LIST_1 + wt_list_offset][l1_ref][0] + 1) >>
           1);

      weighted_bi_prediction(
          &p_Vid->p_decs->dec_mb_pred[decoder][joff], tmp_block_l0,
          tmp_block_l1, block_size_y, block_size_x, ioff, alpha_l0, alpha_l1,
          wp_off, (currSlice->luma_log_weight_denom + 1), max_imgpel_value);
    } else {
      bi_prediction(&p_Vid->p_decs->dec_mb_pred[decoder][joff], tmp_block_l0,
                    tmp_block_l1, block_size_y, block_size_x, ioff);
    }
#if 0
    if ((dec_picture->chroma_format_idc != YUV400) && (dec_picture->chroma_format_idc != YUV444) ) 
    {
...
void perform_mc_concealment(Macroblock *currMB, int decoder, ColorPlane pl,
                            StorablePicture *dec_picture, int pred_dir,
                            int l0_mode, int l1_mode, PicMotionParams **mv_info,
                            int i, int j, int block_size_x, int block_size_y) {
  imgpel tmp_block_l0[MB_BLOCK_SIZE][MB_BLOCK_SIZE];  //!< l0 prediction
  imgpel tmp_block_l1[MB_BLOCK_SIZE][MB_BLOCK_SIZE];  //!< l1 prediction
  ...
  if (pred_dir != 2) {
    ...
    if (apply_weights) {
      ...
      weighted_mc_prediction(
          &p_Vid->p_decs->dec_mb_pred[decoder][joff], block_size_y,
          block_size_x, ioff, tmp_block_l0, alpha_l0, wp_off,
          p_Vid->currentSlice->luma_log_weight_denom, max_imgpel_value);
    } else {
      mc_prediction(&p_Vid->p_decs->dec_mb_pred[decoder][joff], block_size_y,
                    block_size_x, ioff, tmp_block_l0);
    }
    ...
    if (apply_weights) {
      ...
      alpha_l0 = p_Vid->currentSlice
                     ->wbp_weight[LIST_0 + wt_list_offset][l0_ref][l1_ref][0];
      alpha_l1 = p_Vid->currentSlice
                     ->wbp_weight[LIST_1 + wt_list_offset][l0_ref][l1_ref][0];
      wp_off =
          ((p_Vid->currentSlice->wp_offset[LIST_0 + wt_list_offset][l0_ref][0] +
            p_Vid->currentSlice->wp_offset[LIST_1 + wt_list_offset][l1_ref][0] +
            1) >>
           1);

      weighted_bi_prediction(&p_Vid->p_decs->dec_mb_pred[decoder][joff],
                             tmp_block_l0, tmp_block_l1, block_size_y,
                             block_size_x, ioff, alpha_l0, alpha_l1, wp_off,
                             (p_Vid->currentSlice->luma_log_weight_denom + 1),
                             max_imgpel_value);
    } else {
      bi_prediction(&p_Vid->p_decs->dec_mb_pred[decoder][joff], tmp_block_l0,
                    tmp_block_l1, block_size_y, block_size_x, ioff);
