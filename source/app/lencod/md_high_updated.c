  ...
  for (currMB->c_ipred_mode = chroma_pred_mode_range[0];
    ...
    for (index = 0; index < max_index; index++) {
      ...
      if (enc_mb.valid[mode]) {
        ...
        //# 0: Enable always
        //# 1: Enable only when best mode is 16x16 with skip motion vectors and
        // no residual (natural mode) # 2: Enable only when best mode is 16x16
        // with skip motion vectors # 3: Enable only when best mode is 16x16 and
        // mvs are close to skip motion vectors (+-8) # 4: Enable only when best
        // mode is inter with no residual # 5: Enable only when best mode is
        // inter with no residual or 16x16 with mvs close to skip (+-8)
          ...
          switch (p_Inp->PSliceSkipDecisionMethod) {
            ...
            case 3:
              if (!(currMB->best_mode == 1 &&
                    motion[currMB->block_y][currMB->block_x].ref_idx[LIST_0] ==
                        0 &&
                    iabs(motion[currMB->block_y][currMB->block_x]
                             .mv[LIST_0]
                             .mv_x -
                         allmvs->mv_x) < 8 &&
                    iabs(motion[currMB->block_y][currMB->block_x]
        ...
        // Skip intra modes in inter slices if best mode is inter <P8x8 with cbp
        // equal to 0
     ...
     // currMB->c_ipred_mode<=chroma_pred_mode_range[1]; currMB->c_ipred_mode++)
