 ...
 ***********************************************************************
 */
void initialize_fast_full_search(VideoParameters *p_Vid,
                                 InputParameters *p_Inp) {
  int i, j, k, list;
  int search_range = p_Inp->SearchMode[0] == FAST_FULL_SEARCH
  ...
  if ((p_me_ffast->BlockSAD =
           (distpel *****)malloc(2 * sizeof(distpel ****))) == NULL)
    no_mem_exit("initialize_fast_full_search: p_me_ffast->BlockSAD");

  for (list = 0; list < 2; list++) {
    if ((p_me_ffast->BlockSAD[list] = (distpel ****)malloc(
             (p_Vid->max_num_references) * sizeof(distpel ***))) == NULL)
      no_mem_exit("initialize_fast_full_search: p_me_ffast->BlockSAD");
    for (i = 0; i < p_Vid->max_num_references; i++) {
      if ((p_me_ffast->BlockSAD[list][i] =
               (distpel ***)malloc(8 * sizeof(distpel **))) == NULL)
        no_mem_exit("initialize_fast_full_search: p_me_ffast->BlockSAD");
      for (j = 1; j < 8; j++) {
        if ((p_me_ffast->BlockSAD[list][i][j] =
                 (distpel **)malloc(16 * sizeof(distpel *))) == NULL)
          no_mem_exit("initialize_fast_full_search: p_me_ffast->BlockSAD");
        for (k = 0; k < 16; k++) {
          if ((p_me_ffast->BlockSAD[list][i][j][k] =
                   (distpel *)malloc(max_pos * sizeof(distpel))) == NULL)
            no_mem_exit("initialize_fast_full_search: p_me_ffast->BlockSAD");
        }
      }
    }
  ...
  if ((p_me_ffast->search_setup_done = (int **)malloc(2 * sizeof(int *))) ==
      NULL)
    no_mem_exit("initialize_fast_full_search: p_me_ffast->search_setup_done");
  if ((p_me_ffast->search_center =
           (MotionVector **)malloc(2 * sizeof(MotionVector *))) == NULL)
    no_mem_exit("initialize_fast_full_search: p_me_ffast->search_center");
  if ((p_me_ffast->search_center_padded =
           (MotionVector **)malloc(2 * sizeof(MotionVector *))) == NULL)
    no_mem_exit(
        "initialize_fast_full_search: p_me_ffast->search_center_padded");

  if ((p_me_ffast->pos_00 = (int **)malloc(2 * sizeof(int *))) == NULL)
    no_mem_exit("initialize_fast_full_search: p_me_ffast->pos_00");
  if ((p_me_ffast->max_search_range = (int **)malloc(2 * sizeof(int *))) ==
      NULL)
    no_mem_exit("initialize_fast_full_search: p_me_ffast->max_search_range");

  for (list = 0; list < 2; list++) {
    if ((p_me_ffast->search_setup_done[list] =
             (int *)malloc((p_Vid->max_num_references) * sizeof(int))) == NULL)
      no_mem_exit("initialize_fast_full_search: p_me_ffast->search_setup_done");
    if ((p_me_ffast->search_center[list] = (MotionVector *)malloc(
             (p_Vid->max_num_references) * sizeof(MotionVector))) == NULL)
      no_mem_exit("initialize_fast_full_search: p_me_ffast->search_center");
    if ((p_me_ffast->search_center_padded[list] = (MotionVector *)malloc(
             (p_Vid->max_num_references) * sizeof(MotionVector))) == NULL)
      no_mem_exit(
          "initialize_fast_full_search: p_me_ffast->search_center_padded");

    if ((p_me_ffast->pos_00[list] =
             (int *)malloc((p_Vid->max_num_references) * sizeof(int))) == NULL)
      no_mem_exit("initialize_fast_full_search: p_me_ffast->pos_00");
    if ((p_me_ffast->max_search_range[list] =
             (int *)malloc((p_Vid->max_num_references) * sizeof(int))) == NULL)
      no_mem_exit("initialize_fast_full_search: p_me_ffast->max_search_range");
  }
  ...
  if (p_Inp->full_search == 2) {
    ...
    for (list = 0; list < 2; list++) {
      ...
      for (i = 1; i < p_Vid->max_num_references; i++)
        p_me_ffast->max_search_range[list][i] = search_range / 2;
    }
  }
}
 ...
 ***********************************************************************
 */
void clear_fast_full_search(VideoParameters *p_Vid) {
  int i, j, k, list;
...
void reset_fast_full_search(VideoParameters *p_Vid) {
  int list;
  for (list = 0; list < 2; list++)
    memset(&p_Vid->p_ffast_me->search_setup_done[list][0], 0,
           p_Vid->max_num_references * sizeof(int));
}
/*!
 ***********************************************************************
 * \brief
 ...
 ***********************************************************************
 */
void update_full_search_large_blocks(MEFullFast *p_ffast_me, int list,
                                     int refindex, int max_pos) {
#define ADD_UP_BLOCKS() \
  _o = *_bo;            \
  _i = *_bi;            \
  _j = *_bj;            \
 ...
 ***********************************************************************
 */
void setup_fast_full_search(
    Macroblock *currMB, MEBlock *mv_block,
    int list)  // <--  reference frame parameter, list0 or 1
{
  ...
  int *imgpel_dist =
      p_Inp->MEErrorMetric[0] ? p_Vid->imgpel_quad : p_Vid->imgpel_abs;
#endif
  MotionVector pmv;
  ...
  currMB->GetMVPredictor(currMB, block, &pmv, ref, p_Vid->enc_picture->mv_info,
                         list, 0, 0, 16, 16);
  ...
  if (!p_Inp->rdopt) {
    ...
    p_Vid->p_ffast_me->search_center[list][ref].mv_x =
        (short)iClip3(-search_range, search_range,
                      p_Vid->p_ffast_me->search_center[list][ref].mv_x);
    p_Vid->p_ffast_me->search_center[list][ref].mv_y =
        (short)iClip3(-search_range, search_range,
                      p_Vid->p_ffast_me->search_center[list][ref].mv_y);
  }

  p_Vid->p_ffast_me->search_center[list][ref].mv_x = (short)iClip3(
      p_Vid->MaxHmvR[4] + search_range, p_Vid->MaxHmvR[5] - search_range,
      p_Vid->p_ffast_me->search_center[list][ref].mv_x);
  p_Vid->p_ffast_me->search_center[list][ref].mv_y = (short)iClip3(
      p_Vid->MaxVmvR[4] + search_range, p_Vid->MaxVmvR[5] - search_range,
      p_Vid->p_ffast_me->search_center[list][ref].mv_y);

  p_Vid->p_ffast_me->search_center_padded[list][ref] =
      pad_MVs(p_Vid->p_ffast_me->search_center[list][ref], mv_block);
  ...
  for (y = currMB->opix_y; y < currMB->opix_y + MB_BLOCK_SIZE; y++) {
    memcpy(srcptr, &p_Vid->pCurImg[y][currMB->pix_x],
  ...
  if (mv_block->ChromaMEEnable) {
    for (k = 1; k < 3; k++) {
      for (y = currMB->opix_c_y; y < currMB->opix_c_y + p_Vid->mb_cr_size_y;
           y++) {
        memcpy(srcptr, &p_Vid->pImgOrg[k][y][currMB->pix_c_x],
  ...
  if (!p_Inp->rdopt) {
    ...
    for (pos = 0; pos < max_pos; pos++) {
      if (ref_x == p_Vid->spiral_qpel_search[pos].mv_x &&
          ref_y == p_Vid->spiral_qpel_search[pos].mv_y) {
  ...
  if (apply_weights) {
    ...
    for (pos = 0; pos < max_pos; pos++) {
      ...
      for (blky = 0; blky < 4; blky++) {
        ...
        for (y = 0; y < 4; y++) {
          weighted_pel = iClip1(p_Vid->max_imgpel_value,
                                ((weight_luma * *refptr++ + wp_luma_round) >>
              ...
              for (x = 0; x < p_Vid->mb_cr_size_x; x += BLOCK_SIZE) {
                weighted_pel =
                    iClip1(max_imgpel_value_uv,
                           ((weight_cr[k] * *refptr++ + wp_chroma_round) >>
...
fast_full_search_motion_estimation(
  ...
  int pos;
  int search_range =
      imax(mv_block->searchRange.max_x, mv_block->searchRange.max_y) >> 2;
  ...
  MotionVector cand = { 0, 0 },
               *offset = &p_Vid->p_ffast_me->search_center[list][ref];
  int max_mvd = p_Vid->max_mvd - 1;

  block_index = (mv_block->block_y << 2) +
                (mv_block->block_x);  // block index for indexing SAD array
  block_sad = p_me_ffast->BlockSAD[list][ref][mv_block->blocktype]
                                  [block_index];  // pointer to SAD array
  ...
  //===== cost for (0,0)-vector: it is done before, because MVCost can be
  // negative =====
