...
extern void SbSMuxBasic(ImageData *imgOut, ImageData *imgIn0, ImageData *imgIn1,
                        int offset);
...
static void process_picture_in_dpb_s(VideoParameters *p_Vid,
                                     StorablePicture *p_pic);
static StorablePicture *clone_storable_picture(VideoParameters *p_Vid,
                                               StorablePicture *p_pic);
...
int getDpbSize(VideoParameters *p_Vid, seq_parameter_set_rbsp_t *active_sps) {
  int pic_size_mb = (active_sps->pic_width_in_mbs_minus1 + 1) *
                    (active_sps->pic_height_in_map_units_minus1 + 1) *
                    (active_sps->frame_mbs_only_flag ? 1 : 2);
...
void init_dpb(VideoParameters *p_Vid, DecodedPictureBuffer *p_Dpb) {
  ...
  if (p_Dpb->size < (unsigned int)(p_Vid->p_Inp)->num_ref_frames) {
    error(
        "DPB size at specified level is smaller than the specified number of "
  ...
  if (NULL == p_Dpb->fs) no_mem_exit("init_dpb: p_Dpb->fs");

  p_Dpb->fs_ref = calloc(p_Dpb->size, sizeof(FrameStore *));
  if (NULL == p_Dpb->fs_ref) no_mem_exit("init_dpb: p_Dpb->fs_ref");

  p_Dpb->fs_ltref = calloc(p_Dpb->size, sizeof(FrameStore *));
  if (NULL == p_Dpb->fs_ltref) no_mem_exit("init_dpb: p_Dpb->fs_ltref");
  ...
  if (NULL == p_Dpb->fs_ilref) no_mem_exit("init_dpb: p_Dpb->fs_ilref");
...
FrameStore *alloc_frame_store(void) {
  ...
  if (NULL == f) no_mem_exit("alloc_frame_store: f");
...
StorablePicture *alloc_storable_picture(VideoParameters *p_Vid,
  ...
  if (NULL == s) no_mem_exit("alloc_storable_picture: s");
  ...
  if ((p_Vid->nal_reference_idc != NALU_PRIORITY_DISPOSABLE) ||
      ((p_Inp->num_of_views == 2) &&
       p_Vid->view_id ==
           0))  // p_Vid->inter_view_flag[structure?structure-1: structure]))
#else
  if (p_Vid->nal_reference_idc != NALU_PRIORITY_DISPOSABLE)
#endif
  {
    ...
    {
      get_mem4Dpel_pad(&(s->imgY_sub), 4, 4, size_y, size_x, IMG_PAD_SIZE_Y,
                       IMG_PAD_SIZE_X);
      s->imgY = s->imgY_sub[0][0];

      if (p_Inp->ChromaMCBuffer || p_Vid->P444_joined ||
          (p_Inp->yuv_format == YUV444 && !p_Vid->P444_joined)) {
        // UV components
        if (p_Vid->yuv_format != YUV400) {
          if (p_Vid->yuv_format == YUV420) {
            get_mem5Dpel_pad(&(s->imgUV_sub), 2, 8, 8, size_y_cr, size_x_cr,
                             p_Vid->pad_size_uv_y, p_Vid->pad_size_uv_x);
          } else if (p_Vid->yuv_format == YUV422) {
            get_mem5Dpel_pad(&(s->imgUV_sub), 2, 4, 8, size_y_cr, size_x_cr,
                             p_Vid->pad_size_uv_y, p_Vid->pad_size_uv_x);
          } else {  // YUV444
            get_mem5Dpel_pad(&(s->imgUV_sub), 2, 4, 4, size_y_cr, size_x_cr,
                             p_Vid->pad_size_uv_y, p_Vid->pad_size_uv_x);
          }
        ...
        get_mem3Dpel_pad(&(s->imgUV), 2, size_y_cr, size_x_cr,
                         p_Vid->pad_size_uv_y, p_Vid->pad_size_uv_x);
      }
    } else if (p_Inp->OnTheFlyFractMCP == OTF_L1)  // OTF L1
    {
      get_mem4Dpel_pad(&(s->imgY_sub), 2, 2, size_y, size_x, IMG_PAD_SIZE_Y,
                       IMG_PAD_SIZE_X);
      s->imgY = s->imgY_sub[0][0];
      ...
      if (p_Inp->ChromaMCBuffer || p_Vid->P444_joined ||
        ...
        if (p_Vid->yuv_format != YUV400) {
          if (p_Vid->yuv_format == YUV420) {
            ...
            get_mem5Dpel_pad(&(s->imgUV_sub), 2, 2, 4, size_y_cr, size_x_cr,
                             p_Vid->pad_size_uv_y, p_Vid->pad_size_uv_x);
          } else {  // YUV444
            get_mem5Dpel_pad(&(s->imgUV_sub), 2, 2, 2, size_y_cr, size_x_cr,
                             p_Vid->pad_size_uv_y, p_Vid->pad_size_uv_x);
          }
          s->p_img_sub[1] = s->imgUV_sub[0];
          s->p_img_sub[2] = s->imgUV_sub[1];
    ...
    {
      get_mem2Dpel_pad(&(s->imgY), size_y, size_x, IMG_PAD_SIZE_Y,
                       IMG_PAD_SIZE_X);
      get_mem3Dpel_pad(&(s->imgUV), 2, size_y_cr, size_x_cr,
                       p_Vid->pad_size_uv_y, p_Vid->pad_size_uv_x);
    }
    ...
    get_mem2Dpel_pad(&(s->imgY), size_y, size_x, IMG_PAD_SIZE_Y,
                     IMG_PAD_SIZE_X);
    get_mem3Dpel_pad(&(s->imgUV), 2, size_y_cr, size_x_cr, p_Vid->pad_size_uv_y,
                     p_Vid->pad_size_uv_x);
  }
  ...
  alloc_pic_motion(&s->motion, (size_y >> BLOCK_SHIFT),
                   (size_x >> BLOCK_SHIFT));

  if ((p_Inp->separate_colour_plane_flag != 0)) {
    for (nplane = 0; nplane < MAX_PLANE; nplane++) {
      get_mem2Dmp(&s->JVmv_info[nplane], (size_y >> BLOCK_SHIFT),
                  (size_x >> BLOCK_SHIFT));
      alloc_pic_motion(&s->JVmotion[nplane], (size_y >> BLOCK_SHIFT),
                       (size_x >> BLOCK_SHIFT));
    }
  }

  if (p_Inp->rdopt == 3) {
    errdo_alloc_storable_picture(s, p_Vid, p_Inp, size_x, size_y, size_x_cr,
                                 size_y_cr);
  }
  ...
  if (p_Inp->HMEEnable &&
    ...
    AllocHMEMemory(&s->pHmeImage, p_Vid, size_y, size_x, IMG_PAD_SIZE_Y,
                   IMG_PAD_SIZE_X, 1);
  }
  ...
  s->size_x_pad =
      size_x + 2 * IMG_PAD_SIZE_X - 1 - MB_BLOCK_SIZE - IMG_PAD_SIZE_X;
  s->size_y_pad =
      size_y + 2 * IMG_PAD_SIZE_Y - 1 - MB_BLOCK_SIZE - IMG_PAD_SIZE_Y;
  s->size_x_cr = size_x_cr;
  s->size_y_cr = size_y_cr;
  s->size_x_cr_pad = (int)(size_x_cr - 1) + (p_Vid->pad_size_uv_x << 1) -
                     (p_Vid->mb_cr_size_x) - p_Vid->pad_size_uv_x;
  s->size_y_cr_pad = (int)(size_y_cr - 1) + (p_Vid->pad_size_uv_y << 1) -
                     (p_Vid->mb_cr_size_y) - p_Vid->pad_size_uv_y;
  ...
  if (picture) {
    if (picture->imgY_sub) {
      if (bFreeImage) {
        ...
        {
          free_mem4Dpel_pad(picture->imgY_sub, 4, IMG_PAD_SIZE_Y,
                            IMG_PAD_SIZE_X);
        } else {
          free_mem4Dpel_pad(picture->imgY_sub, 16, IMG_PAD_SIZE_Y,
                            IMG_PAD_SIZE_X);
        }
        ...
        for (k = 1; k < K; k++) {
          if (picture->imgY_sub[k >> 2][k & 3]) {
            free_mem2Dpel_pad(picture->imgY_sub[k >> 2][k & 3], IMG_PAD_SIZE_Y,
                              IMG_PAD_SIZE_X);
    ...
    if (picture->imgUV_sub) {
      ...
      if (bFreeImage) {
        ...
        free_mem5Dpel_pad(picture->imgUV_sub, 2 * iUVResY * iUVResX,
                          picture->pad_size_uv_y, picture->pad_size_uv_x);
        ...
        for (k = 1; k < iUVResY * iUVResX; k++) {
          ...
          if (picture->imgUV_sub[0][j][i]) {
            free_mem2Dpel_pad(picture->imgUV_sub[0][j][i],
                              picture->pad_size_uv_y, picture->pad_size_uv_x);
            picture->imgUV_sub[0][j][i] = NULL;
          }
          if (picture->imgUV_sub[1][j][i]) {
            free_mem2Dpel_pad(picture->imgUV_sub[1][j][i],
                              picture->pad_size_uv_y, picture->pad_size_uv_x);
        ...
        if ((p_Dpb->fs_ref[i]->frame->used_for_reference) &&
          ...
          if (p_Dpb->fs_ref[i]->frame_num > currSlice->frame_num) {
            p_Dpb->fs_ref[i]->frame_num_wrap =
                p_Dpb->fs_ref[i]->frame_num - max_frame_num;
          } else {
...
void init_lists_p_slice(Slice *currSlice) {
  ...
  if (currSlice->structure == FRAME) {
    ...
    if (NULL == fs_list0) no_mem_exit("init_lists: fs_list0");
    fs_listlt = calloc(p_Dpb->size, sizeof(FrameStore *));
    if (NULL == fs_listlt) no_mem_exit("init_lists: fs_listlt");
  ...
  if (p_Inp->num_of_views == 2 && p_Vid->view_id == 1) {
    ...
    if (currSlice->structure == FRAME) {
      ...
      currSlice->listXsize[1] = (char)imin(
    ...
    } else  // FIELD
    {
      ...
      list0idx = currSlice->listXsize[0];
      /*
      interview_pos = (currSlice->structure != BOTTOM_FIELD) ? p_Dpb->used_size
      - 1: p_Dpb->used_size - 2; // top field is at the last position base_ref =
      currSlice->structure!=BOTTOM_FIELD?p_Dpb->fs[interview_pos]->top_field:p_Dpb->fs[interview_pos]->bottom_field;
      */
      interview_pos = 0;
      base_ref = currSlice->structure != BOTTOM_FIELD
                     ? p_Dpb->fs_ilref[interview_pos]->top_field
                     : p_Dpb->fs_ilref[interview_pos]->bottom_field;
      if (base_ref->used_for_reference ||
          base_ref->inter_view_flag[cur_inter_view_flag]) {
        // p_Dpb->fs[interview_pos]->frame_num_wrap = -99;
        p_Dpb->fs_ilref[interview_pos]->frame_num_wrap = -99;
        currSlice->listX[0][list0idx++] = base_ref;
        currSlice->listXsize[0] = (char)list0idx;
  ...
  if (p_Vid->is_hme == 0) {
#if (MVC_EXTENSION_ENABLE)
    if (p_Vid->num_of_layers == 2 && p_Inp->MVCInterViewReorder &&
        p_Vid->view_id == 1) {
      currSlice->listXsize[0] = (char)imin(
          currSlice->listXsize[0], currSlice->num_ref_idx_active[LIST_0] + 1);
      currSlice->listXsize[1] = (char)imin(
          currSlice->listXsize[1], currSlice->num_ref_idx_active[LIST_1]);
    } else
#endif
...
void init_lists_b_slice(Slice *currSlice) {
...

  {
    ...
    if (currSlice->structure == FRAME) {
      ...
      if (NULL == fs_list1) no_mem_exit("init_lists: fs_list1");
      fs_listlt = calloc(p_Dpb->size, sizeof(FrameStore *));
      if (NULL == fs_listlt) no_mem_exit("init_lists: fs_listlt");
  ...
  if (p_Inp->num_of_views == 2 && p_Vid->view_id == 1) {
    ...
    } else  // FIELD
    {
      ...
      list1idx = currSlice->listXsize[1];
      /*
      interview_pos = currSlice->structure!=BOTTOM_FIELD ? p_Dpb->used_size - 1:
      p_Dpb->used_size - 2; // top field is at the last position base_ref =
      currSlice->structure != BOTTOM_FIELD ?
      p_Dpb->fs[interview_pos]->top_field:p_Dpb->fs[interview_pos]->bottom_field;
      */
      interview_pos = 0;
      base_ref = currSlice->structure != BOTTOM_FIELD
                     ? p_Dpb->fs_ilref[interview_pos]->top_field
...
************************************************************************
*/
StorablePicture *get_short_term_pic(Slice *currSlice,
                                    DecodedPictureBuffer *p_Dpb, int picNum) {
  unsigned i;
 ...
 ************************************************************************
 */
void reorder_short_term(Slice *currSlice, DecodedPictureBuffer *p_Dpb,
                        int cur_list, int picNumLX, int *refIdxLX) {
  ...
  for (cIdx = *refIdxLX; cIdx <= currSlice->num_ref_idx_active[cur_list];
       cIdx++) {
    if (RefPicListX[cIdx])
 ...
 ************************************************************************
 */
static void reorder_long_term(Slice *currSlice, DecodedPictureBuffer *p_Dpb,
                              StorablePicture **RefPicListX, int cur_list,
                              int frame_no, int *refIdxLX) {
  int cIdx, nIdx;
...
void reorder_ref_pic_list(Slice *currSlice, int cur_list) {
  ...
  for (i = 0; modification_of_pic_nums_idc[i] != 3; i++) {
    ...
    {
      reorder_long_term(currSlice, p_Dpb, currSlice->listX[cur_list], cur_list,
                        i, &refIdxLX);
    }
  }
...
static void idr_memory_management(DecodedPictureBuffer *p_Dpb,
                                  StorablePicture *p, FrameFormat *output) {
  unsigned i;
  VideoParameters *p_Vid = p_Dpb->p_Vid;
...
static void sliding_window_memory_management(DecodedPictureBuffer *p_Dpb,
  ...
  if (p_Dpb->ref_frames_in_buffer ==
      imax(1, p_Vid->active_sps->num_ref_frames) -
...
static void adaptive_memory_management(DecodedPictureBuffer *p_Dpb,
    ...
    switch (tmp_drpm->memory_management_control_operation) {
      case 0:
        if (tmp_drpm->Next != NULL) {
          error(
              "memory_management_control_operation = 0 not last operation in "
              "buffer",
              500);
        }
        break;
  ...
  if (p_Vid->last_has_mmco_5) {
    ...
    if (p_Vid->num_of_layers > 1) flush_dpb(p_Vid->p_Dpb_layer[1], output);
  }
}
...
void store_picture_in_dpb(DecodedPictureBuffer *p_Dpb, StorablePicture *p,
  ...
  if ((!p_Vid->currentPicture->idr_flag) &&
      (p->used_for_reference && (!p_Vid->adaptive_ref_pic_buffering_flag))) {
  ...
  while (p_Dpb->used_size == p_Dpb->size) {
    ...
    if (!p->used_for_reference) {
      ...
      if (p_Vid->p_Inp->num_of_views == 2) {
        if (((-1 == pos) || (p->poc < poc))) {
          direct_output(p_Vid, p, output,
                        (p_Dpb->layer_id ? p_Vid->p_dec2 : p_Vid->p_dec));
          return;
        }
      } else
#endif
 ...
 ************************************************************************
 */
void replace_top_proc_pic_with_frame(DecodedPictureBuffer *p_Dpb,
                                     StorablePicture *p) {
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  ...
  proc_picture->used_for_reference =
      (p_Vid->nal_reference_idc != NALU_PRIORITY_DISPOSABLE);
  ...
  if ((p_Vid->p_Inp->separate_colour_plane_flag != 0)) {
    ...
    if (p_Vid->p_Inp->HMEEnable) GenerateHMELayers(p_Vid, proc_picture);
  }

  found = 0;

  if (p_Dpb->fs_ilref[0] &&
      (p_Dpb->fs_ilref[0]->frame_num == p_Vid->frame_num) &&
      (p_Dpb->fs_ilref[0]->is_used == 1)) {
    found = 1;
    fs = p_Dpb->fs_ilref[0];
  }
  ...
  if (!found) {
    printf(
        "replace_top_proc_pic_with_frame: Warning! Reference processed field "
        "not found. Processed frame not inserted in DPB\n");
 ...
 ************************************************************************
 */
void replace_top_pic_with_frame(DecodedPictureBuffer *p_Dpb, StorablePicture *p,
                                FrameFormat *output) {
  VideoParameters *p_Vid = p_Dpb->p_Vid;
  FrameStore *fs = NULL;
  unsigned i, found;
...
#if (MVC_EXTENSION_ENABLE)
  if (p->used_for_reference ||
      (p_Vid->p_Inp->num_of_views == 2 && p_Vid->view_id == 0))
#else
  if (p->used_for_reference)
#endif
  {
    if ((p_Vid->p_Inp->separate_colour_plane_flag != 0)) {
      ...
      if (p_Vid->p_Inp->HMEEnable) GenerateHMELayers(p_Vid, p);
    }
  }
  ...
  for (i = 0; i < p_Dpb->used_size; i++) {
    if ((p_Dpb->fs[i]->frame_num == p_Vid->frame_num) &&
        (p_Dpb->fs[i]->is_used == 1)) {
      found = 1;
      fs = p_Dpb->fs[i];
      break;
    }
  ...
  if (!found) {
    // this should only happen for non-reference pictures when the dpb is full
    // of reference pics
#if (MVC_EXTENSION_ENABLE)
    if (p_Vid->p_Inp->num_of_views == 2) {
...
#if (MVC_EXTENSION_ENABLE)
  else if (p_Vid->p_Inp->num_of_views == 2 &&
           p->inter_view_flag[p->structure >> 1]) {
    UnifiedOneForthPix(p_Vid, p);
    if (p_Vid->p_Inp->HMEEnable) GenerateHMELayers(p_Vid, p);
  }
#endif
...
void remove_frame_from_dpb(DecodedPictureBuffer *p_Dpb, int pos) {
  ...
  switch (fs->is_used) {
    ...
    case 1:
      ...
      break;
    case 0: break;
    default: error("invalid frame store type", 500);
  }
  fs->is_used = 0;
  fs->is_long_term = 0;
  fs->is_reference = 0;
...
static void output_one_frame_from_dpb(DecodedPictureBuffer *p_Dpb,
  ...
  //  printf ("output frame with frame_num #%d, poc %d (p_Dpb-> p_Dpb->size=%d,
...
void dpb_split_field(VideoParameters *p_Vid, FrameStore *fs) {
  ...
  if (!p_Vid->active_sps->frame_mbs_only_flag) {
    fs_top = fs->top_field = alloc_storable_picture(
        p_Vid, TOP_FIELD, frame->size_x, frame->size_y >> 1, frame->size_x_cr,
        frame->size_y_cr >> 1);
    fs_btm = fs->bottom_field = alloc_storable_picture(
        p_Vid, BOTTOM_FIELD, frame->size_x, frame->size_y >> 1,
    ...
    for (k = 0; k < 2; k++) {
      for (i = 0; i < (frame->size_y_cr >> 1); i++) {
        memcpy(fs_top->imgUV[k][i], frame->imgUV[k][i * 2],
               frame->size_x_cr * sizeof(imgpel));
        memcpy(fs_btm->imgUV[k][i], frame->imgUV[k][i * 2 + 1],
               frame->size_x_cr * sizeof(imgpel));
      }
    }
    ...
    fs->top_field->bottom_poc = fs->bottom_field->bottom_poc =
        frame->bottom_poc;
    ...
    fs->long_term_frame_idx = fs->top_field->long_term_frame_idx =
        fs->bottom_field->long_term_frame_idx = frame->long_term_frame_idx;
    ...
    fs->top_field->chroma_format_idc = fs->bottom_field->chroma_format_idc =
        frame->chroma_format_idc;
    fs->top_field->chroma_mask_mv_x = fs->bottom_field->chroma_mask_mv_x =
        frame->chroma_mask_mv_x;
    fs->top_field->chroma_mask_mv_y = fs->bottom_field->chroma_mask_mv_y =
        frame->chroma_mask_mv_y;
    fs->top_field->chroma_shift_x = fs->bottom_field->chroma_shift_x =
        frame->chroma_shift_x;
    fs->top_field->chroma_shift_y = fs->bottom_field->chroma_shift_y =
        frame->chroma_shift_y;
    ...
    for (j = 0; j < frame->size_y >> 3; j++) {
      ...
      for (i = 0; i < (frame->size_x >> 2); i++) {
        ...
        if (fs->frame->motion.mb_field[currentmb]) {
          fs->bottom_field->mv_info[j][i].field_frame =
              fs->top_field->mv_info[j][i].field_frame = 1;
          fs->frame->mv_info[2 * j][i].field_frame =
    ...
    for (j = 0; j < fs->frame->size_y >> 3; j++) {
      ...
      for (i = 0; i < fs->frame->size_x >> 2; i++) {
        ...
        if (!fs->frame->mb_aff_frame_flag ||
            !fs->frame->motion.mb_field[currentmb]) {
          fs->frame->mv_info[2 * j + 1][i].field_frame =
              fs->frame->mv_info[2 * j][i].field_frame = 0;

          fs->top_field->mv_info[j][i].field_frame =
              fs->bottom_field->mv_info[j][i].field_frame = 0;

          fs->top_field->mv_info[j][i].mv[LIST_0] =
              fs->bottom_field->mv_info[j][i].mv[LIST_0] =
                  fs->frame->mv_info[jj][ii].mv[LIST_0];
          fs->top_field->mv_info[j][i].mv[LIST_1] =
              fs->bottom_field->mv_info[j][i].mv[LIST_1] =
                  fs->frame->mv_info[jj][ii].mv[LIST_1];

          // Scaling of references is done here since it will not affect spatial
          // direct (2*0 =0)
          if (fs->frame->mv_info[jj][ii].ref_idx[LIST_0] == -1)
            fs->top_field->mv_info[j][i].ref_idx[LIST_0] =
                fs->bottom_field->mv_info[j][i].ref_idx[LIST_0] = -1;
          else {
            fs->top_field->mv_info[j][i].ref_idx[LIST_0] =
                fs->bottom_field->mv_info[j][i].ref_idx[LIST_0] =
                    fs->frame->mv_info[jj][ii].ref_idx[LIST_0];
          }

          if (fs->frame->mv_info[jj][ii].ref_idx[LIST_1] == -1)
            fs->top_field->mv_info[j][i].ref_idx[LIST_1] =
                fs->bottom_field->mv_info[j][i].ref_idx[LIST_1] = -1;
          else {
            fs->top_field->mv_info[j][i].ref_idx[LIST_1] =
                fs->bottom_field->mv_info[j][i].ref_idx[LIST_1] =
                    fs->frame->mv_info[jj][ii].ref_idx[LIST_1];
          }
        } else {
          fs->frame->mv_info[2 * j + 1][i].field_frame =
              fs->frame->mv_info[2 * j][i].field_frame =
...
void dpb_combine_field(VideoParameters *p_Vid, FrameStore *fs) {
  ...
  if (fs->frame->used_for_reference) {
    if ((p_Vid->p_Inp->separate_colour_plane_flag != 0)) {
      ...
      if (p_Vid->p_Inp->HMEEnable) GenerateHMELayers(p_Vid, fs->frame);
    }
  }
...
void fill_frame_num_gap(VideoParameters *p_Vid, FrameFormat *output) {
  ...
  while (CurrFrameNum != UnusedShortTermFrameNum) {
    ...
    UnusedShortTermFrameNum =
        (UnusedShortTermFrameNum + 1) % p_Vid->max_frame_num;
  }
  ...
  if (currSlice->direct_spatial_mv_pred_flag == 0) {
    for (j = 0; j < 2 + (currSlice->mb_aff_frame_flag * 4); j += 2) {
      for (i = 0; i < currSlice->listXsize[j]; i++) {
        ...
        if (j == 0) {
          iTRb = iClip3(-128, 127,
                        p_Vid->enc_picture->poc - listX[LIST_0 + j][i]->poc);
        } else if (j == 2) {
          iTRb =
              iClip3(-128, 127,
                     p_Vid->enc_picture->top_poc - listX[LIST_0 + j][i]->poc);
        } else {
          iTRb = iClip3(
              -128, 127,
              p_Vid->enc_picture->bottom_poc - listX[LIST_0 + j][i]->poc);
        }
...
static void process_picture_in_dpb_s(VideoParameters *p_Vid,
  ...
  img_in.frm_stride[1] = img_in.frm_stride[2] =
      p_pic->size_x_cr + (p_pic->pad_size_uv_x << 1);
  ...
  if (p_pic->structure == FRAME) {
    ...
    img_in.top_stride[1] = img_in.bot_stride[1] = img_in.top_stride[2] =
        img_in.bot_stride[2] = img_in.frm_stride[1] << 1;
    d_img = p_img_out->frm_data;
  } else  // If reference picture is a field, then frm_data will actually
          // contain field data and therefore top/bottom stride is set
          // accordingly.
  {
    img_in.top_stride[0] = img_in.bot_stride[0] = img_in.frm_stride[0];
    img_in.top_stride[1] = img_in.bot_stride[1] = img_in.top_stride[2] =
        img_in.bot_stride[2] = img_in.frm_stride[1];
  ...
  if (p_Vid->yuv_format != YUV400) {
    for (i = 0; i < p_pic->size_y_cr; i++)
      memcpy(d_img[1][i], p_pic->imgUV[0][i],
             p_pic->size_x_cr * sizeof(imgpel));
    for (i = 0; i < p_pic->size_y_cr; i++)
      memcpy(d_img[2][i], p_pic->imgUV[1][i],
             p_pic->size_x_cr * sizeof(imgpel));
  }
}
 ...
 ************************************************************************
 */
static void copy_storable_picture(VideoParameters *p_Vid, StorablePicture *s,
                                  StorablePicture *d) {
  InputParameters *p_Inp = p_Vid->p_Inp;
  int nplane;
  int i, j;
  ...
  if (p_Vid->yuv_format != YUV400) {
    ...
    for (i = 0; i < size_y_cr; i++)
      memcpy((void *)d->imgUV[0][i], (void *)s_img[1][i],
             size_x_cr * sizeof(imgpel));
    for (i = 0; i < size_y_cr; i++)
      memcpy((void *)d->imgUV[1][i], (void *)s_img[2][i],
             size_x_cr * sizeof(imgpel));
...
static StorablePicture *clone_storable_picture(VideoParameters *p_Vid,
  ...
  p_stored_pic = alloc_storable_picture(
      p_Vid, (PictureStructure)p_Vid->structure, p_pic->size_x, p_pic->size_y,
      p_pic->size_x_cr, p_pic->size_y_cr);
  ...
  p_stored_pic->mb_aff_frame_flag = p_Vid->mb_aff_frame_flag =
