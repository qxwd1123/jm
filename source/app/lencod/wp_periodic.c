  ...
  for (clist = 0; clist < 2 + list_offset; clist++) {
    for (n = 0; n < currSlice->listXsize[clist]; n++) {
      for (comp = 0; comp < 3; comp++) {
        cur_weight = (int)((127.0 * cos(p_Vid->number * PI / 30)) +
                           (4.0 * (((float)rand()) / RAND_MAX)) - 4.0);
        weight[clist][n][comp] = sClip3(-128, 127, cur_weight);
        cur_offset = (int)((127.0 * cos((p_Vid->number + 15) * PI / 45)) +
                           (4.0 * (((float)rand()) / RAND_MAX)) - 4.0);
        offset[clist][n][comp] =
            (sClip3(-128, 127, cur_offset) << bitdepth[comp]);
      }
    }
  }
    ...
    for (clist = 0; clist < 2 + list_offset; clist++) {
      for (n = 0; n < currSlice->listXsize[clist]; n++) {
        for (comp = 0; comp < 3; comp++) {
          ...
          cur_offset = (int)((127.0 * cos((p_Vid->number + 45) * PI / 45)) +
                             (4.0 * (((float)rand()) / RAND_MAX)) - 4.0);
