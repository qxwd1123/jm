...
extern void UpdateDecoders(VideoParameters *p_Vid, InputParameters *p_Inp,
...
static void field_picture(VideoParameters *p_Vid, Picture *top,
                          Picture *bottom);
static void prepare_enc_frame_picture(VideoParameters *p_Vid,
...
static void writeout_picture(VideoParameters *p_Vid, Picture *pic);
#endif
static byte picture_structure_decision(VideoParameters *p_Vid, Picture *frame,
                                       Picture *top, Picture *bot);
static void distortion_fld(VideoParameters *p_Vid, InputParameters *p_Inp,
                           Picture *field_pic, ImageData *imgData);
    ...
    for (i = 0; i < (int)p_Vid->PicSizeInMbs; i += 2) {
      if (p_Enc_Pic->motion.mb_field[i]) {
        ...
        for (y = 0; y < MB_BLOCK_SIZE; y++) {
          memcpy(&imgY[y0 + (2 * y)][x0], temp[y],
                 MB_BLOCK_SIZE * sizeof(imgpel));
          memcpy(&imgY[y0 + (2 * y + 1)][x0], temp[y + MB_BLOCK_SIZE],
                 MB_BLOCK_SIZE * sizeof(imgpel));
        }
        ...
        for (uv = 0; uv < 2; uv++) {
          for (y = 0; y < (2 * p_Vid->mb_cr_size_y); y++)
            memcpy(&temp[y], &imgUV[uv][y0 + y][x0],
                   p_Vid->mb_cr_size_x * sizeof(imgpel));

          for (y = 0; y < p_Vid->mb_cr_size_y; y++) {
            memcpy(&imgUV[uv][y0 + (2 * y)][x0], temp[y],
                   p_Vid->mb_cr_size_x * sizeof(imgpel));
            memcpy(&imgUV[uv][y0 + (2 * y + 1)][x0],
                   temp[y + p_Vid->mb_cr_size_y],
                   p_Vid->mb_cr_size_x * sizeof(imgpel));
          }
        }
      }
 ...
 ************************************************************************
 */
void set_slice_type(VideoParameters *p_Vid, InputParameters *p_Inp,
                    int slice_type) {
...
static void code_a_plane(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  while (NumberOfCodedMBs < p_Vid->PicSizeInMbs)  // loop over slices
  {
    ...
    while (!FmoSliceGroupCompletelyCoded(p_Vid, SliceGroup)) {
      ...
      if (!p_Vid->mb_aff_frame_flag)
        NumberOfCodedMBs +=
            encode_one_slice(p_Vid, SliceGroup, NumberOfCodedMBs);
      else
        NumberOfCodedMBs +=
            encode_one_slice_MBAFF(p_Vid, SliceGroup, NumberOfCodedMBs);
  ...
  if ((p_Inp->SkipDeBlockNonRef == 0) || (p_Vid->nal_reference_idc != 0)) {
    ...
    DeblockFrame(
        p_Vid, p_Vid->enc_picture->imgY,
        p_Vid->enc_picture->imgUV);  // comment out to disable deblocking filter
...
static void code_a_picture(VideoParameters *p_Vid, Picture *pic) {
  ...
  RandomIntraNewPicture(
      p_Vid);  //! Allocates forced INTRA MBs (even for fields!)
  ...
  if (p_Vid->mb_aff_frame_flag) MbAffPostProc(p_Vid);
}
...
byte get_idr_flag(VideoParameters *p_Vid) {
  ...
  switch (p_Vid->structure) {
    default:
    case FRAME: idr_flag = p_cur_frm->p_frame_pic->idr_flag; break;
    case TOP_FIELD: idr_flag = p_cur_frm->p_top_fld_pic->idr_flag; break;
    case BOTTOM_FIELD: idr_flag = p_cur_frm->p_bot_fld_pic->idr_flag; break;
  }
...
byte get_random_access_flag(VideoParameters *p_Vid) {
  ...
  switch (p_Vid->structure) {
    default:
    case FRAME: random_access = p_cur_frm->p_frame_pic->random_access; break;
    case TOP_FIELD:
 ...
 ************************************************************************
 */
void update_global_stats(InputParameters *p_Inp, StatParameters *gl_stats,
  ...
  if (p_Inp->skip_gl_stats == 0) {
    ...
    for (i = 0; i < 5; i++) {
      ...
      for (j = 0; j < 15; j++) {
        ...
        for (k = 0; k < 2; k++)
          gl_stats->mode_use_transform[i][j][k] +=
              cur_stats->mode_use_transform[i][j][k];
      }
    }
  }
}
...
static void storeRedundantFrame(VideoParameters *p_Vid) {
  ...
  if (p_Vid->key_frame) {
    for (j = 0; j < p_Vid->height; j++) {
      memcpy(p_Vid->imgY_tmp[j], p_Vid->enc_frame_picture[0]->imgY[j],
             p_Vid->width * sizeof(imgpel));
    }

    for (k = 0; k < 2; k++) {
      for (j = 0; j < p_Vid->height_cr; j++) {
        memcpy(p_Vid->imgUV_tmp[k][j], p_Vid->enc_frame_picture[0]->imgUV[k][j],
               p_Vid->width_cr * sizeof(imgpel));
      }
    }
  }
  ...
  if (p_Vid->redundant_coding) {
    for (j = 0; j < p_Vid->height; j++) {
      memcpy(p_Vid->enc_frame_picture[0]->imgY[j], p_Vid->imgY_tmp[j],
             p_Vid->width * sizeof(imgpel));
    }
    for (k = 0; k < 2; k++) {
      for (j = 0; j < p_Vid->height_cr; j++) {
        memcpy(p_Vid->enc_frame_picture[0]->imgUV[k][j], p_Vid->imgUV_tmp[k][j],
               p_Vid->width_cr * sizeof(imgpel));
      }
    }
  }
...
static int read_input_data_32pulldown(VideoParameters *p_Vid) {
  ...
  if (p_Inp->num_of_views == 2 && p_Vid->view_id == 1) {
    ...
    file_read = read_one_frame(p_Vid, &p_Inp->input_file2, frm_no_in_file_first,
                               p_Inp->infile_header, &p_Inp->source,
                               &p_Inp->output, p_Vid->imgData0.frm_data);
    if (!file_read) {
    ...
    pad_borders(p_Inp->output, p_Vid->width, p_Vid->height, p_Vid->width_cr,
                p_Vid->height_cr, p_Vid->imgData0.frm_data);

    // Read frame data (second frame)
    file_read = read_one_frame(
        p_Vid, &p_Inp->input_file2, frm_no_in_file_second, p_Inp->infile_header,
        &p_Inp->source, &p_Inp->output, p_Vid->imgData4.frm_data);
    if (!file_read) {
      // end of file or stream found: trigger error handling
      get_number_of_frames(p_Inp, &p_Inp->input_file2);
      fprintf(stdout,
    ...
    pad_borders(p_Inp->output, p_Vid->width, p_Vid->height, p_Vid->width_cr,
                p_Vid->height_cr, p_Vid->imgData4.frm_data);
  } else
#endif
  {
    frm_no_in_file_first = ((p_Vid->frm_no_in_file * 4 + pull_down_offset) / 5);
    ...
    // Read frame data (first frame)
    file_read = read_one_frame(p_Vid, &p_Inp->input_file1, frm_no_in_file_first,
                               p_Inp->infile_header, &p_Inp->source,
                               &p_Inp->output, p_Vid->imgData0.frm_data);
    if (!file_read) {
      // end of file or stream found: trigger error handling
    ...
    // Read frame data (second frame)
    file_read = read_one_frame(
        p_Vid, &p_Inp->input_file1, frm_no_in_file_second, p_Inp->infile_header,
        &p_Inp->source, &p_Inp->output, p_Vid->imgData4.frm_data);
    if (!file_read) {
      // end of file or stream found: trigger error handling
      get_number_of_frames(p_Inp, &p_Inp->input_file1);
...
static int read_input_data(VideoParameters *p_Vid) {
  ...
  if (p_Inp->num_of_views == 2 && p_Vid->view_id == 1) {
    file_read = read_one_frame(
        p_Vid, &p_Inp->input_file2, p_Vid->frm_no_in_file, p_Inp->infile_header,
        &p_Inp->source, &p_Inp->output, p_Vid->imgData0.frm_data);
    if (!file_read) {
      // end of file or stream found: trigger error handling
      get_number_of_frames(p_Inp, &p_Inp->input_file2);
      fprintf(stdout,
  ...
  {
    file_read = read_one_frame(
        p_Vid, &p_Inp->input_file1, p_Vid->frm_no_in_file, p_Inp->infile_header,
        &p_Inp->source, &p_Inp->output, p_Vid->imgData0.frm_data);
    if (!file_read) {
      // end of file or stream found: trigger error handling
      get_number_of_frames(p_Inp, &p_Inp->input_file1);
...
void perform_encode_frame(VideoParameters *p_Vid) {
  ...
  // Rate control
  if (p_Inp->RCEnable) rc_init_frame(p_Vid, p_Inp);
  ...
  if (p_Inp->HMEEnable) {
    if (p_Vid->type != I_SLICE && p_Vid->type != SI_SLICE) invoke_HME(p_Vid, 0);
  }

#if (MVC_EXTENSION_ENABLE)
  if (p_Vid->view_id != 1 || !p_Vid->sec_view_force_fld) {
#endif
    if ((p_Vid->type == B_SLICE || p_Vid->type == P_SLICE ||
         p_Vid->type == I_SLICE) &&
        p_Inp->RDPictureDecision) {
    ...
    tmpFrameQP = p_Vid->SumFrameQP;  // call it here since rd_picture_coding
                                     // buffers it and may modify it
    ...
    if (p_Vid->p_curr_frm_struct->type == SI_SLICE) {
      // once the picture has been encoded as a primary SP frame encode as an SI
      // frame
    ...
    if ((p_Vid->type == SP_SLICE) && (p_Inp->sp_output_indicator)) {
      // output the transformed and quantized coefficients (useful for switching
      // SP frames)
  ...
  if ((p_Inp->PicInterlace == ADAPTIVE_CODING) &&
      (!is_MVC_profile(p_Vid->active_sps->profile_idc) ||
       (/*(p_Vid->active_sps->profile_idc == SBSPLUS_HIGH) &&*/ (
           (p_Vid->view_id == 0) ||
           (p_Vid->view_id == 1 && p_Vid->sec_view_force_fld)))))
#else
  if (p_Inp->PicInterlace == ADAPTIVE_CODING)
#endif
  {
    ...
    if (p_Inp->num_of_views == 1 ||
        p_Vid->view_id == 0)  // first view will make the decision
    {
      if (p_Vid->rd_pass == 0)
        p_Vid->fld_flag = picture_structure_decision(p_Vid, p_Vid->frame_pic[0],
                                                     p_Vid->field_pic_ptr[0],
                                                     p_Vid->field_pic_ptr[1]);
      else if (p_Vid->rd_pass == 1)
        p_Vid->fld_flag = picture_structure_decision(p_Vid, p_Vid->frame_pic[1],
                                                     p_Vid->field_pic_ptr[0],
                                                     p_Vid->field_pic_ptr[1]);
      else
        p_Vid->fld_flag = picture_structure_decision(p_Vid, p_Vid->frame_pic[2],
                                                     p_Vid->field_pic_ptr[0],
                                                     p_Vid->field_pic_ptr[1]);

      p_Vid->sec_view_force_fld =
          p_Vid->fld_flag;  // store 1st view decision, same coding structure to
                            // be used for second view
    } else {
    ...
    if (p_Vid->rd_pass == 0)
      p_Vid->fld_flag = picture_structure_decision(
          p_Vid, p_Vid->frame_pic[0], p_Vid->field_pic[0], p_Vid->field_pic[1]);
    else if (p_Vid->rd_pass == 1)
      p_Vid->fld_flag = picture_structure_decision(
          p_Vid, p_Vid->frame_pic[1], p_Vid->field_pic[0], p_Vid->field_pic[1]);
    else
      p_Vid->fld_flag = picture_structure_decision(
          p_Vid, p_Vid->frame_pic[2], p_Vid->field_pic[0], p_Vid->field_pic[1]);
#endif
...
void free_slice_data(VideoParameters *p_Vid) {
...
#if (MVC_EXTENSION_ENABLE)
  if (p_Vid->field_pic_ptr &&
      (p_Vid->p_Inp->num_of_views == 1 || p_Vid->view_id == 1)) {
    for (i = 0; i < 2; i++) {
      if (p_Vid->field_pic1[i]) free_slice_list(p_Vid->field_pic1[i]);
      if (p_Vid->p_Inp->num_of_views == 2 && p_Vid->field_pic2) {
        if (p_Vid->field_pic2[i]) free_slice_list(p_Vid->field_pic2[i]);
      }
    }
  }
#else
  if (p_Vid->field_pic) {
    for (i = 0; i < 2; i++) {
      if (p_Vid->field_pic[i]) free_slice_list(p_Vid->field_pic[i]);
    }
  }
#endif
}
...
#if (MVC_EXTENSION_ENABLE)
  if ((p_Inp->PicInterlace == ADAPTIVE_CODING) &&
      ((p_Dpb->layer_id == 0) || !is_MVC_profile(profile_idc)))
#else
  if (p_Inp->PicInterlace == ADAPTIVE_CODING)
#endif
  {
    if (p_Vid->fld_flag) {
      update_global_stats(p_Inp, p_Vid->p_Stats,
                          &p_Vid->enc_field_picture[0]->stats);
      update_global_stats(p_Inp, p_Vid->p_Stats,
                          &p_Vid->enc_field_picture[1]->stats);
      // store bottom field
      store_picture_in_dpb(p_Dpb, p_Vid->enc_field_picture[1], &p_Inp->output);
#if (MVC_EXTENSION_ENABLE)
      if (p_Vid->proc_picture && is_MVC_profile(profile_idc)) {
        store_proc_picture_in_dpb(p_Vid->p_Dpb_layer[1], p_Vid->proc_picture,
                                  &p_Inp->output);
      ...
      update_global_stats(p_Inp, p_Vid->p_Stats,
                          &p_Vid->enc_frame_picture[p_Vid->rd_pass]->stats);
      // replace top with frame
      if (p_Vid->rd_pass == 2) {
        replace_top_pic_with_frame(p_Dpb, p_Vid->enc_frame_picture[2],
                                   &p_Inp->output);
        free_storable_picture(p_Vid, p_Vid->enc_frame_picture[0]);
        free_storable_picture(p_Vid, p_Vid->enc_frame_picture[1]);
      } else if (p_Vid->rd_pass == 1) {
        replace_top_pic_with_frame(p_Dpb, p_Vid->enc_frame_picture[1],
                                   &p_Inp->output);
        free_storable_picture(p_Vid, p_Vid->enc_frame_picture[0]);
        free_storable_picture(p_Vid, p_Vid->enc_frame_picture[2]);
      } else {
        if (p_Inp->redundant_pic_flag == 0 || (p_Vid->key_frame == 0)) {
          replace_top_pic_with_frame(p_Dpb, p_Vid->enc_frame_picture[0],
                                     &p_Inp->output);
          free_storable_picture(p_Vid, p_Vid->enc_frame_picture[1]);
          free_storable_picture(p_Vid, p_Vid->enc_frame_picture[2]);
        }
...
void update_bitcounter_stats(VideoParameters *p_Vid) {
  ...
  if (p_Vid->p_Inp->num_of_views == 2) {
    if (p_Vid->fld_flag) {
      if (p_Vid->view_id == 1) {
        p_Vid->p_Stats->bit_ctr_parametersets_v[0] +=
            p_Vid->p_Stats->bit_ctr_parametersets_n_v[0];
        p_Vid->p_Stats->bit_ctr_parametersets_v[1] +=
        ...
        p_Vid->p_Stats->bit_ctr_filler_data_n_v[0] =
            p_Vid->p_Stats->bit_ctr_filler_data_v[0];
        p_Vid->p_Stats->bit_ctr_filler_data_n_v[1] =
            p_Vid->p_Stats->bit_ctr_filler_data_v[1];
      }
    } else  // this seems to be enough and the check above seems
            // redundant...check later...
    {
      p_Vid->p_Stats->bit_ctr_parametersets_v[p_Vid->view_id] +=
          p_Vid->p_Stats->bit_ctr_parametersets_n_v[p_Vid->view_id];
      p_Vid->p_Stats->bit_ctr_n_v[p_Vid->view_id] =
          p_Vid->p_Stats->bit_ctr_v[p_Vid->view_id];
      p_Vid->p_Stats->bit_ctr_parametersets_n_v[p_Vid->view_id] = 0;
      p_Vid->p_Stats->bit_ctr_filler_data_n_v[p_Vid->view_id] =
          p_Vid->p_Stats->bit_ctr_filler_data_v[p_Vid->view_id];
    }
  }
#endif
}
...
void update_idr_order_stats(VideoParameters *p_Vid) {
  ...
  {
    // top field picture
    if (p_Vid->p_curr_frm_struct->p_top_fld_pic->p_Slice[0].type == I_SLICE &&
        p_Vid->p_curr_frm_struct->p_top_fld_pic->nal_ref_idc) {
      ...
      if (p_Vid->p_curr_frm_struct->p_top_fld_pic->idr_flag) {
        p_Vid->last_idr_disp_order =
            imax(p_Vid->frame_no, p_Vid->last_idr_disp_order);
        p_Vid->last_idr_code_order = p_Vid->curr_frm_idx;
      }
    }
    // bottom field picture
    if (p_Vid->p_curr_frm_struct->p_bot_fld_pic->p_Slice[0].type == I_SLICE &&
        p_Vid->p_curr_frm_struct->p_bot_fld_pic->nal_ref_idc) {
      p_Vid->lastINTRA = imax(p_Vid->lastINTRA, p_Vid->frame_no);
      p_Vid->lastIntraNumber = p_Vid->curr_frm_idx;
    ...
    // frame picture
    if (p_Vid->p_curr_frm_struct->p_frame_pic->p_Slice[0].type == I_SLICE &&
        p_Vid->p_curr_frm_struct->p_frame_pic->nal_ref_idc) {
      p_Vid->lastINTRA = imax(p_Vid->lastINTRA, p_Vid->frame_no);
      p_Vid->lastIntraNumber = p_Vid->curr_frm_idx;
...
int update_video_stats(VideoParameters *p_Vid) {
  ...
  if (p_Inp->RCEnable) {
    if ((!p_Inp->PicInterlace) && (!p_Inp->MbInterlace)) {
      bits = (int)(p_Vid->p_Stats->bit_ctr - p_Vid->p_Stats->bit_ctr_n) +
             (int)(p_Vid->p_Stats->bit_ctr_filler_data -
                   p_Vid->p_Stats->bit_ctr_filler_data_n);
    } else if (p_Inp->RCUpdateMode <= MAX_RC_MODE) {
      bits =
          (int)(p_Vid->p_Stats->bit_ctr - (p_Vid->p_rc_quad->Pprev_bits)) +
          (int)(p_Vid->p_Stats->bit_ctr_filler_data -
                p_Vid->p_Stats
                    ->bit_ctr_filler_data_n);  // used for rate control update
      p_Vid->p_rc_quad->Pprev_bits =
          p_Vid->p_Stats->bit_ctr + p_Vid->p_Stats->bit_ctr_filler_data;
    } else {
      bits = (int)(p_Vid->p_Stats->bit_ctr - p_Vid->p_Stats->bit_ctr_n) +
             (int)(p_Vid->p_Stats->bit_ctr_filler_data -
                   p_Vid->p_Stats->bit_ctr_filler_data_n);
  ...
  p_Vid->p_Stats->bit_counter[p_Vid->type] +=
      p_Vid->p_Stats->bit_ctr - p_Vid->p_Stats->bit_ctr_n;
  ...
  if (p_Inp->num_of_views == 2) {
    if (p_Vid->fld_flag) {
      if (p_Vid->view_id == 1) {
        int prev_type =
            (p_Vid->p_pred->p_frm_mvc +
             ((p_Vid->curr_frm_idx << 1) % p_Vid->p_pred->num_frames_mvc))
                ->type;

        p_Vid->p_Stats->bit_counter_v[0][prev_type] +=
            p_Vid->p_Stats->bit_ctr_v[0] - p_Vid->p_Stats->bit_ctr_n_v[0];
        p_Vid->p_Stats->bit_counter_v[1][p_Vid->type] +=
            p_Vid->p_Stats->bit_ctr_v[1] - p_Vid->p_Stats->bit_ctr_n_v[1];
      }
    } else {
      p_Vid->p_Stats->bit_counter_v[p_Vid->view_id][p_Vid->type] +=
          p_Vid->p_Stats->bit_ctr_v[p_Vid->view_id] -
          p_Vid->p_Stats->bit_ctr_n_v[p_Vid->view_id];
    }
  }
...
int encode_one_frame(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  for (i = 0; i < 6; i++) p_Vid->enc_frame_picture[i] = NULL;
  ...
  */
  ...
  // Following code should consider optimal coding mode. Currently also does not
  // support multiple slices per frame.
  ...
  if (p_Vid->type == SP_SLICE) {
    if (p_Inp->sp2_frame_indicator) {  // switching SP frame encoding
  ...
  // Here, p_Vid->structure may be either FRAME or BOTTOM FIELD depending on
  // whether AFF coding is used The picture structure decision changes really
  // only the fld_flag
  ...
  if (p_Inp->PicInterlace == FRAME_CODING) {
    if ((p_Inp->rdopt == 3) && (p_Inp->de == LLN) &&
        (p_Vid->nal_reference_idc != 0)) {
      UpdateDecoders(p_Vid, p_Inp,
                     p_Vid->enc_picture);  // simulate packet losses and move
                                           // decoded image to reference buffers
    }

    if (p_Inp->RestrictRef) UpdatePixelMap(p_Vid, p_Inp);
  }
...
#if OUTPUT_REF_LIST
  {
    ...
    for (i = 0; i < p_Vid->currentSlice->listXsize[0]; i++) {
      printf("%d ", p_Vid->currentSlice->listX[0][i]->poc /
                        2);  // may not work with field
    }
    printf(" [L1]: ");
    for (i = 0; i < p_Vid->currentSlice->listXsize[1]; i++) {
      printf("%d ", p_Vid->currentSlice->listX[1][i]->poc /
                        2);  // may not work with field
    }
    printf("\n");
  }
  ...
  if (p_Inp->RCEnable) {
    ...
    if (p_Inp->RCUpdateMode <= MAX_RC_MODE)
      p_Vid->rc_update_pict_frame_ptr(p_Vid, p_Inp, p_Vid->p_rc_quad,
                                      p_Vid->p_rc_gen, bits);
  }

  compute_distortion(p_Vid, &p_Vid->imgData);

  // redundant pictures: save reconstruction to calculate SNR and replace
  // reference picture
  ...
  // (int)p_Vid->enc_picture->frame_num, p_Vid->enc_picture->poc);
  store_coded_picture(p_Vid->p_Dpb_layer[p_Vid->view_id]);

  p_Vid->AverageFrameQP =
      isign(p_Vid->SumFrameQP) *
      ((iabs(p_Vid->SumFrameQP) + (int)(p_Vid->FrameSizeInMbs >> 1)) /
       (int)p_Vid->FrameSizeInMbs);

  if (p_Inp->RCEnable && p_Inp->RCUpdateMode <= MAX_RC_MODE &&
      p_Vid->type != B_SLICE && p_Inp->basicunit < p_Vid->FrameSizeInMbs)
    p_Vid->p_rc_quad->CurrLastQP =
        p_Vid->AverageFrameQP + p_Vid->p_rc_quad->bitdepth_qp_scale;

#ifdef _LEAKYBUCKET_
  // Store bits used for this frame and increment counter of no. of coded frames
  if (!p_Vid->redundant_coding) {
    p_Vid->Bit_Buffer[p_Vid->total_frame_buffer++] =
        (long)(p_Vid->p_Stats->bit_ctr - p_Vid->p_Stats->bit_ctr_n) +
        (long)(p_Vid->p_Stats->bit_ctr_filler_data -
               p_Vid->p_Stats->bit_ctr_filler_data_n);
  }
#endif

  // POC200301: Verify that POC coding type 2 is not used if more than one
  // consecutive non-reference frame is requested or if decoding order is
  // different from output order
  if (p_Vid->pic_order_cnt_type == 2) {
    ...
    if (p_Vid->frame_no < p_Vid->prev_frame_no ||
        p_Vid->consecutive_non_reference_pictures > 1)
      error(
          "POC type 2 cannot be applied for the coding pattern where the "
  ...
  if (p_Vid->curr_frm_idx == 0)
    ...
    switch (p_Vid->type) {
      case I_SLICE:
      case SI_SLICE: ReportI(p_Vid, tmp_time); break;
      case B_SLICE: ReportB(p_Vid, tmp_time); break;
      case SP_SLICE: ReportP(p_Vid, tmp_time); break;
      case P_SLICE:
      default: ReportP(p_Vid, tmp_time);
    }
  }
  ...
  // Rate control
  if (p_Inp->RCEnable) p_Vid->rc_update_picture_ptr(p_Vid, p_Inp, bits);
...
void copy_params(VideoParameters *p_Vid, StorablePicture *enc_picture,
  ...
  if (active_sps->frame_cropping_flag) {
    ...
    enc_picture->frame_crop_bottom_offset =
...
static void prepare_enc_frame_picture(VideoParameters *p_Vid,
                                      StorablePicture **stored_pic) {
  InputParameters *p_Inp = p_Vid->p_Inp;
  (*stored_pic) = alloc_storable_picture(
      p_Vid, (PictureStructure)p_Vid->structure, p_Vid->width, p_Vid->height,
  ...
  (*stored_pic)->mb_aff_frame_flag = p_Vid->mb_aff_frame_flag =
      (Boolean)(p_Inp->MbInterlace != FRAME_CODING);

  p_Vid->get_mb_block_pos = p_Vid->mb_aff_frame_flag ? get_mb_block_pos_mbaff
                                                     : get_mb_block_pos_normal;
  p_Vid->getNeighbour =
      p_Vid->mb_aff_frame_flag ? getAffNeighbour : getNonAffNeighbour;
...
static void calc_picture_bits(Picture *frame) {
  ...
  for (i = 0; i < frame->no_slices; i++) {
    ...
    for (j = 0; j < thisSlice->max_part_nr; j++)
      frame->bits_per_picture +=
          8 * ((thisSlice->partArr[j]).bitstream)->byte_pos;
  }
}
 ...
 ************************************************************************
 */
void frame_picture(VideoParameters *p_Vid, Picture *frame, ImageData *imgData,
                   int rd_pass) {
  int nplane;
  InputParameters *p_Inp = p_Vid->p_Inp;
  ...
  if (rd_pass == 0 && p_Vid->wp_parameters_set == 0) ResetWP(p_Vid, p_Inp);
...
static void field_picture(VideoParameters *p_Vid, Picture *top,
  ...
  p_Vid->enc_field_picture[0] = alloc_storable_picture(
      p_Vid, (PictureStructure)p_Vid->structure, p_Vid->width, p_Vid->height,
      p_Vid->width_cr, p_Vid->height_cr);
  ...
  p_Vid->enc_field_picture[0]->mb_aff_frame_flag = p_Vid->mb_aff_frame_flag =
      FALSE;
  ...
  store_picture_in_dpb(p_Vid->p_Dpb_layer[p_Vid->view_id],
                       p_Vid->enc_field_picture[0], &p_Inp->output);
  ...
  //  Bottom field
  p_Vid->enc_field_picture[1] = alloc_storable_picture(
      p_Vid, (PictureStructure)p_Vid->structure, p_Vid->width, p_Vid->height,
      p_Vid->width_cr, p_Vid->height_cr);
  ...
  p_Vid->enc_field_picture[1]->mb_aff_frame_flag = p_Vid->mb_aff_frame_flag =
      FALSE;
  p_Vid->get_mb_block_pos = get_mb_block_pos_normal;
  p_Vid->getNeighbour = getNonAffNeighbour;
...
static void combine_field(VideoParameters *p_Vid) {
  ...
  for (i = 0; i < (p_Vid->height >> 1); i++) {
    memcpy(p_Vid->imgY_com[i * 2], p_Vid->enc_field_picture[0]->imgY[i],
           p_Vid->width * sizeof(imgpel));  // top field
    memcpy(p_Vid->imgY_com[i * 2 + 1], p_Vid->enc_field_picture[1]->imgY[i],
           p_Vid->width * sizeof(imgpel));  // bottom field
  }

  if (p_Vid->yuv_format != YUV400) {
    for (k = 0; k < 2; k++) {
      for (i = 0; i < (p_Vid->height_cr >> 1); i++) {
        memcpy(p_Vid->imgUV_com[k][i * 2],
               p_Vid->enc_field_picture[0]->imgUV[k][i],
               p_Vid->width_cr * sizeof(imgpel));
        memcpy(p_Vid->imgUV_com[k][i * 2 + 1],
               p_Vid->enc_field_picture[1]->imgUV[k][i],
               p_Vid->width_cr * sizeof(imgpel));
      }
    }
  }
 ...
 ************************************************************************
 */
static byte decide_fld_frame(float snr_frame_Y, float snr_field_Y,
                             int bit_field, int bit_frame,
...
static byte picture_structure_decision(VideoParameters *p_Vid, Picture *frame,
  ...
  lambda_picture =
      0.68 *
      pow(2, p_Vid->bitdepth_lambda_scale + ((p_Vid->qp - SHIFT_QP) / 3.0)) *
      ((p_Vid->type == B_SLICE) ? 1 : 1);

  sse_frame = frame->distortion.value[0] + frame->distortion.value[1] +
              frame->distortion.value[2];
  //! all distrortions of a field picture are accumulated in the top field
  sse_field = top->distortion.value[0] + top->distortion.value[1] +
              top->distortion.value[2];

  bit_field = top->bits_per_picture + bot->bits_per_picture;
  bit_frame = frame->bits_per_picture;
  return decide_fld_frame(sse_frame, sse_field, bit_field, bit_frame,
                          lambda_picture);
}
...
static void frame_mode_buffer(VideoParameters *p_Vid) {
  ...
  if ((p_Inp->PicInterlace != FRAME_CODING) ||
      (p_Inp->MbInterlace != FRAME_CODING)) {
...
static inline void init_fixed_qp_i_slice(VideoParameters *p_Vid,
                                         InputParameters *p_Inp,
                                         FrameUnitStruct *p_cur_frm) {
...
static inline void init_fixed_qp_pb_slice(VideoParameters *p_Vid,
                                          InputParameters *p_Inp,
                                          FrameUnitStruct *p_cur_frm,
                                          int type) {
  // QP oscillation for secondary SP frames
...
static inline void init_fixed_qp_sp_slice(VideoParameters *p_Vid,
                                          InputParameters *p_Inp,
                                          FrameUnitStruct *p_cur_frm) {
  p_Vid->qp = p_cur_frm->qp;
...
static void init_fixed_qp(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_cur_frm->layer == 0) {
    switch (p_cur_frm->type) {
      default:
      case I_SLICE: init_fixed_qp_i_slice(p_Vid, p_Inp, p_cur_frm); break;
      case P_SLICE:
      case B_SLICE:
        init_fixed_qp_pb_slice(p_Vid, p_Inp, p_cur_frm, p_cur_frm->type);
        break;
      case SP_SLICE:
      case SI_SLICE: init_fixed_qp_sp_slice(p_Vid, p_Inp, p_cur_frm); break;
    }

    p_Vid->mb_y_intra = p_Vid->mb_y_upd;  //  p_Vid->mb_y_intra indicates which
                                          //  GOB to intra code for this frame

    if (p_Inp->intra_upd > 0)  // if error robustness, find next GOB to update
    {
      p_Vid->mb_y_upd = (p_Vid->curr_frm_idx / p_Inp->intra_upd) %
                        (p_Vid->height / MB_BLOCK_SIZE);
    }
  } else {
    p_Vid->qp = p_cur_frm->qp;
  }
  ...
  if (p_Inp->qp2frame && p_Inp->qp2frame < p_Vid->frame_no) {
    p_Vid->p_curr_frm_struct->qp = p_Vid->qp =
        iClip3(-p_Vid->bitdepth_luma_qp_scale, MAX_QP,
               p_Vid->qp + p_Inp->qp2off[p_cur_frm->type]);
  }
}
...
static void init_mb_line_intra_up(VideoParameters *p_Vid,
                                  InputParameters *p_Inp) {
  FrameUnitStruct *p_cur_frm = p_Vid->p_curr_frm_struct;
...
static void init_frame(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  // The 'slice_nr' of each macroblock is set to -1 here, to guarantee the
  // correct encoding with FMO (if no FMO, encoding is correct without following
  // assignment), for which MBs may not be encoded with scan order
  if ((p_Inp->separate_colour_plane_flag != 0)) {
  ...
  if (p_Inp->WPIterMC) p_Vid->frameOffsetAvail = 0;

  // set parameters for direct mode and deblocking filter
  // currently selection is done at the frame level instead of slice level. This
  // needs to be changed.
  ...
  {
    p_Vid->DFDisableIdc =
        (char)p_Inp->DFDisableIdc[p_Vid->nal_reference_idc > 0][p_Vid->type];
    p_Vid->DFAlphaC0Offset =
        (char)p_Inp->DFAlpha[p_Vid->nal_reference_idc > 0][p_Vid->type];
    p_Vid->DFBetaOffset =
        (char)p_Inp->DFBeta[p_Vid->nal_reference_idc > 0][p_Vid->type];
  }
#if (MVC_EXTENSION_ENABLE)
  else {
    p_Vid->DFDisableIdc =
        (char)p_Inp
            ->EnhLayerDFDisableIdc[p_Vid->nal_reference_idc > 0][p_Vid->type];
    p_Vid->DFAlphaC0Offset =
        (char)p_Inp->EnhLayerDFAlpha[p_Vid->nal_reference_idc > 0][p_Vid->type];
    p_Vid->DFBetaOffset =
        (char)p_Inp->EnhLayerDFBeta[p_Vid->nal_reference_idc > 0][p_Vid->type];
  }
#endif
...
void GenerateImagePyramid(VideoParameters *p_Vid, int size_x, int size_y,
                          imgpel ***p_hme_img, int offset_x, int offset_y) {
  ...
  for (i = 1; i < pHMEInfo->iPyramidLevels; i++) {
    ...
    PyrDownG5x5_U8CnR(
        (const imgpel *)*(p_hme_img[i - 1]),
        (int)((p_hme_img[i - 1][1] - p_hme_img[i - 1][0]) * sizeof(imgpel)),
        iPrevWidth, iPrevHeight, *(p_hme_img[i]),
        (int)((p_hme_img[i][1] - p_hme_img[i][0]) * sizeof(imgpel)), 1);
...
void GenerateHMELayers(VideoParameters *p_Vid, StorablePicture *s) {
  ...
  GenerateImagePyramid(p_Vid, s->size_x, s->size_y, s->pHmeImage,
                       IMG_PAD_SIZE_X, IMG_PAD_SIZE_Y);
...
void UnifiedOneForthPix(VideoParameters *p_Vid, StorablePicture *s) {
  ...
  if ((!p_Inp->OnTheFlyFractMCP) ||
      (p_Inp->OnTheFlyFractMCP == OTF_L1))  // JLT : on-the-fly
  {
    ...
    // perform  padding ( copying borders) that is implicitly done above if
    // p_Inp->OnTheFlyFractMCP=0
    OtfCompatibility_copyWithPadding(s->imgY, s->imgY, s->size_x, s->size_y,
                                     IMG_PAD_SIZE_X, IMG_PAD_SIZE_Y);
    OtfCompatibility_copyWithPadding(s->imgUV[0], s->imgUV[0], s->size_x_cr,
                                     s->size_y_cr, p_Vid->pad_size_uv_x,
                                     p_Vid->pad_size_uv_y);
    OtfCompatibility_copyWithPadding(s->imgUV[1], s->imgUV[1], s->size_x_cr,
                                     s->size_y_cr, p_Vid->pad_size_uv_x,
                                     p_Vid->pad_size_uv_y);
  }
}
 ...
 *
 ************************************************************************/
void UnifiedOneForthPix_JV(VideoParameters *p_Vid, int nplane,
                           StorablePicture *s) {
  InputParameters *p_Inp = p_Vid->p_Inp;
  ...
  if ((!p_Inp->OnTheFlyFractMCP) || (p_Inp->OnTheFlyFractMCP == OTF_L1)) {
    ...
    // perform  padding ( copying borders) that is implicitly done above if
    // p_Inp->OnTheFlyFractMCP=0
    OtfCompatibility_copyWithPadding(s->p_img[nplane], s->p_img[nplane],
                                     s->size_x, s->size_y, IMG_PAD_SIZE_X,
                                     IMG_PAD_SIZE_Y);
  }
}
 ...
 ************************************************************************
 */
Boolean dummy_slice_too_big(int bits_slice) { return FALSE; }

static void ReportSimple(VideoParameters *p_Vid, char *pic_type, int cur_bits,
                         DistMetric *metric, int tmp_time) {
#if (MVC_EXTENSION_ENABLE)
  if (p_Vid->p_Inp->num_of_views == 2) {
    if (p_Vid->fld_flag && p_Vid->view_id == 0) {
      ...
      printf(
          ...
          p_Vid->prev_cs.frm_no_in_file, p_Vid->prev_cs.pic_type,
          p_Vid->prev_cs.view_id,
          (int)(p_Vid->p_Stats->bit_ctr_v[0] - p_Vid->p_Stats->bit_ctr_n_v[0]) +
              (int)(p_Vid->p_Stats->bit_ctr_filler_data_v[0] -
                    p_Vid->p_Stats->bit_ctr_filler_data_n_v[0]),
          p_Vid->prev_cs.AverageFrameQP, p_Vid->prev_cs.psnr_value[0],
          p_Vid->prev_cs.psnr_value[1], p_Vid->prev_cs.psnr_value[2],
      ...
      printf(
          ...
          (int)(p_Vid->p_Stats->bit_ctr_v[1] - p_Vid->p_Stats->bit_ctr_n_v[1]) +
              (int)(p_Vid->p_Stats->bit_ctr_filler_data_v[1] -
                    p_Vid->p_Stats->bit_ctr_filler_data_n_v[1]),
          p_Vid->AverageFrameQP, metric->value[0], metric->value[1],
          metric->value[2], tmp_time, (int)p_Vid->me_time,
          p_Vid->fld_flag ? "FLD" : "FRM", p_Vid->nal_reference_idc);
    } else {
      printf(
          "%05d(%3s)  %1d  %8d   %2d %7.3f %7.3f %7.3f %9d %7d    %3s    %d\n",
    ...
    printf("%05d(%3s)%8d   %2d %7.3f %7.3f %7.3f %9d %7d    %3s    %d\n",
           p_Vid->frm_no_in_file, pic_type, cur_bits, p_Vid->AverageFrameQP,
           metric->value[0], metric->value[1], metric->value[2], tmp_time,
           (int)p_Vid->me_time, p_Vid->fld_flag ? "FLD" : "FRM",
           p_Vid->nal_reference_idc);
}

static void ReportVerbose(VideoParameters *p_Vid, char *pic_type, int cur_bits,
                          int wp_method, int lambda, DistMetric *mPSNR,
                          int tmp_time, int direct_mode) {
#if (MVC_EXTENSION_ENABLE)
  if (p_Vid->p_Inp->num_of_views == 2) {
    if (p_Vid->fld_flag && p_Vid->view_id == 0) {
      p_Vid->prev_cs.frm_no_in_file = p_Vid->frm_no_in_file;
      ...
      printf(
          "%05d(%3s)  %1d  %8d %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    %3s "
          "%5d %1d %2d %2d  %d   %d\n",
          p_Vid->prev_cs.frm_no_in_file, p_Vid->prev_cs.pic_type,
          p_Vid->prev_cs.view_id,
          (int)(p_Vid->p_Stats->bit_ctr_v[0] - p_Vid->p_Stats->bit_ctr_n_v[0]) +
              (int)(p_Vid->p_Stats->bit_ctr_filler_data_v[0] -
                    p_Vid->p_Stats->bit_ctr_filler_data_n_v[0]),
          p_Vid->prev_cs.wp_method, p_Vid->prev_cs.AverageFrameQP,
          p_Vid->prev_cs.lambda, p_Vid->prev_cs.psnr_value[0],
          p_Vid->prev_cs.psnr_value[1], p_Vid->prev_cs.psnr_value[2],
          p_Vid->prev_cs.tmp_time, (int)p_Vid->prev_cs.me_time,
          p_Vid->prev_cs.fld_flag ? "FLD" : "FRM", p_Vid->prev_cs.intras,
          p_Vid->prev_cs.direct_mode, p_Vid->prev_cs.num_ref_idx_l0_active,
          p_Vid->prev_cs.num_ref_idx_l1_active, p_Vid->prev_cs.rd_pass,
          p_Vid->prev_cs.nal_reference_idc);
      printf(
          "%05d(%3s)  %1d  %8d %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    %3s "
          "%5d %1d %2d %2d  %d   %d\n",
          ...
          (int)(p_Vid->p_Stats->bit_ctr_v[1] - p_Vid->p_Stats->bit_ctr_n_v[1]) +
              (int)(p_Vid->p_Stats->bit_ctr_filler_data_v[1] -
                    p_Vid->p_Stats->bit_ctr_filler_data_n_v[1]),
          wp_method, p_Vid->AverageFrameQP, lambda, mPSNR->value[0],
          mPSNR->value[1], mPSNR->value[2], tmp_time, (int)p_Vid->me_time,
          p_Vid->fld_flag ? "FLD" : "FRM", p_Vid->intras, direct_mode,
          p_Vid->num_ref_idx_l0_active, p_Vid->num_ref_idx_l1_active,
          p_Vid->rd_pass, p_Vid->nal_reference_idc);
    } else {
      printf(
          "%05d(%3s)  %1d  %8d %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    %3s "
          "%5d %1d %2d %2d  %d   %d\n",
          p_Vid->frm_no_in_file, pic_type, p_Vid->view_id, cur_bits, wp_method,
          p_Vid->AverageFrameQP, lambda, mPSNR->value[0], mPSNR->value[1],
          mPSNR->value[2], tmp_time, (int)p_Vid->me_time,
          p_Vid->fld_flag ? "FLD" : "FRM", p_Vid->intras, direct_mode,
          p_Vid->num_ref_idx_l0_active, p_Vid->num_ref_idx_l1_active,
          p_Vid->rd_pass, p_Vid->nal_reference_idc);
    }
  ...
  {
    printf(
        "%05d(%3s)%8d %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    %3s %5d %1d %2d "
        "%2d  %d   %d\n",
...
static void ReportVerboseNVB(VideoParameters *p_Vid, char *pic_type,
                             int cur_bits, int nvb_bits, int wp_method,
                             int lambda, DistMetric *mPSNR, int tmp_time,
                             int direct_mode) {
#if (MVC_EXTENSION_ENABLE)
  if (p_Vid->num_of_layers == 2) {
    if (p_Vid->fld_flag && p_Vid->view_id == 0) {
      ...
      printf(
          "%05d(%3s)  %1d  %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    "
          "%3s %5d %1d %2d %2d  %d   %d\n",
          p_Vid->prev_cs.frm_no_in_file, p_Vid->prev_cs.pic_type,
          p_Vid->prev_cs.view_id,
          (int)(p_Vid->p_Stats->bit_ctr_v[0] - p_Vid->p_Stats->bit_ctr_n_v[0]) +
      ...
      printf(
          "%05d(%3s)  %1d  %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    "
          "%3s %5d %1d %2d %2d  %d   %d  %d\n",
          p_Vid->frm_no_in_file, pic_type, p_Vid->view_id, cur_bits, nvb_bits,
          wp_method, p_Vid->AverageFrameQP, lambda, mPSNR->value[0],
          mPSNR->value[1], mPSNR->value[2], tmp_time, (int)p_Vid->me_time,
          p_Vid->fld_flag ? "FLD" : "FRM", p_Vid->intras, direct_mode,
          p_Vid->num_ref_idx_l0_active, p_Vid->num_ref_idx_l1_active,
          p_Vid->rd_pass, p_Vid->nal_reference_idc, p_Vid->iInterViewMBs);
    }
  } else
#endif
  {
    printf(
        "%05d(%3s)%8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    %3s %5d "
        "%1d %2d %2d  %d   %d\n",
...
static void ReportVerboseFDN(VideoParameters *p_Vid, char *pic_type,
                             int cur_bits, int fdn_bits, int nvb_bits,
                             int wp_method, int lambda, DistMetric *mPSNR,
                             int tmp_time, int direct_mode) {
#if (MVC_EXTENSION_ENABLE)
  if (p_Vid->p_Inp->num_of_views == 2) {
    if (p_Vid->fld_flag && p_Vid->view_id == 0) {
      ...
      printf(
          "%05d(%3s)  %1d  %8d %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d  "
          "  %3s %5d %1d %2d %2d  %d   %d\n",
          p_Vid->prev_cs.frm_no_in_file, p_Vid->prev_cs.pic_type,
          p_Vid->prev_cs.view_id,
          (int)(p_Vid->p_Stats->bit_ctr_v[0] - p_Vid->p_Stats->bit_ctr_n_v[0]) +
      ...
      printf(
          "%05d(%3s)  %1d  %8d %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d  "
          "  %3s %5d %1d %2d %2d  %d   %d\n",
          p_Vid->frm_no_in_file, pic_type, p_Vid->view_id, cur_bits, fdn_bits,
          nvb_bits, wp_method, p_Vid->AverageFrameQP, lambda, mPSNR->value[0],
          mPSNR->value[1], mPSNR->value[2], tmp_time, (int)p_Vid->me_time,
          p_Vid->fld_flag ? "FLD" : "FRM", p_Vid->intras, direct_mode,
          p_Vid->num_ref_idx_l0_active, p_Vid->num_ref_idx_l1_active,
          p_Vid->rd_pass, p_Vid->nal_reference_idc);
  ...
  {
    printf(
        "%05d(%3s)%8d %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %9d %7d    %3s "
        "%5d %1d %2d %2d  %d   %d\n",
        p_Vid->frm_no_in_file, pic_type, cur_bits, fdn_bits, nvb_bits,
        wp_method, p_Vid->AverageFrameQP, lambda, mPSNR->value[0],
        mPSNR->value[1], mPSNR->value[2], tmp_time, (int)p_Vid->me_time,
        p_Vid->fld_flag ? "FLD" : "FRM", p_Vid->intras, direct_mode,
        p_Vid->num_ref_idx_l0_active, p_Vid->num_ref_idx_l1_active,
...
static void ReportVerboseSSIM(VideoParameters *p_Vid, char *pic_type,
                              int cur_bits, int wp_method, int lambda,
                              DistMetric *mPSNR, DistMetric *mSSIM,
                              int tmp_time, int direct_mode) {
#if (MVC_EXTENSION_ENABLE)
  if (p_Vid->p_Inp->num_of_views == 2) {
    printf(
        "%05d(%3s)  %1d  %8d %1d %2d %4d %7.3f %7.3f %7.3f %7.4f %7.4f %7.4f "
        "%9d %7d    %3s %5d %1d %2d %2d  %d   %d\n",
        p_Vid->frm_no_in_file, pic_type, p_Vid->view_id, cur_bits, wp_method,
        p_Vid->AverageFrameQP, lambda, mPSNR->value[0], mPSNR->value[1],
        mPSNR->value[2], mSSIM->value[0], mSSIM->value[1], mSSIM->value[2],
        tmp_time, (int)p_Vid->me_time, p_Vid->fld_flag ? "FLD" : "FRM",
        p_Vid->intras, direct_mode, p_Vid->num_ref_idx_l0_active,
        p_Vid->num_ref_idx_l1_active, p_Vid->rd_pass, p_Vid->nal_reference_idc);
  } else
#endif
  {
    printf(
        "%05d(%3s)%8d %1d %2d %4d %7.3f %7.3f %7.3f %7.4f %7.4f %7.4f %9d %7d  "
        "  %3s %5d %1d %2d %2d  %d   %d\n",
        p_Vid->frm_no_in_file, pic_type, cur_bits, wp_method,
        p_Vid->AverageFrameQP, lambda, mPSNR->value[0], mPSNR->value[1],
        mPSNR->value[2], mSSIM->value[0], mSSIM->value[1], mSSIM->value[2],
...
static void ReportVerboseNVBSSIM(VideoParameters *p_Vid, char *pic_type,
                                 int cur_bits, int nvb_bits, int wp_method,
                                 int lambda, DistMetric *mPSNR,
                                 DistMetric *mSSIM, int tmp_time,
                                 int direct_mode) {
#if (MVC_EXTENSION_ENABLE)
  if (p_Vid->p_Inp->num_of_views == 2) {
    printf(
        "%05d(%3s)  %1d  %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %7.4f %7.4f "
        "%7.4f %9d %7d    %3s %5d %1d %2d %2d  %d   %d\n",
        p_Vid->frm_no_in_file, pic_type, p_Vid->view_id, cur_bits, nvb_bits,
        wp_method, p_Vid->AverageFrameQP, lambda, mPSNR->value[0],
        mPSNR->value[1], mPSNR->value[2], mSSIM->value[0], mSSIM->value[1],
        mSSIM->value[2], tmp_time, (int)p_Vid->me_time,
        p_Vid->fld_flag ? "FLD" : "FRM", p_Vid->intras, direct_mode,
        p_Vid->num_ref_idx_l0_active, p_Vid->num_ref_idx_l1_active,
        p_Vid->rd_pass, p_Vid->nal_reference_idc);
  } else
  ...
  {
    printf(
        "%05d(%3s)%8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %7.4f %7.4f %7.4f %9d "
        "%7d    %3s %5d %1d %2d %2d  %d   %d\n",
        p_Vid->frm_no_in_file, pic_type, cur_bits, nvb_bits, wp_method,
        p_Vid->AverageFrameQP, lambda, mPSNR->value[0], mPSNR->value[1],
        mPSNR->value[2], mSSIM->value[0], mSSIM->value[1], mSSIM->value[2],
        tmp_time, (int)p_Vid->me_time, p_Vid->fld_flag ? "FLD" : "FRM",
...
static void ReportVerboseFDNSSIM(VideoParameters *p_Vid, char *pic_type,
                                 int cur_bits, int fdn_bits, int nvb_bits,
                                 int wp_method, int lambda, DistMetric *mPSNR,
                                 DistMetric *mSSIM, int tmp_time,
                                 int direct_mode) {
  ...
  if (p_Vid->p_Inp->num_of_views == 2) {
    printf(
        "%05d(%3s)  %1d  %8d %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %7.4f "
        "%7.4f %7.4f %9d %7d    %3s %5d %1d %2d %2d  %d   %d\n",
        p_Vid->frm_no_in_file, pic_type, p_Vid->view_id, cur_bits, fdn_bits,
        nvb_bits, wp_method, p_Vid->AverageFrameQP, lambda, mPSNR->value[0],
        mPSNR->value[1], mPSNR->value[2], mSSIM->value[0], mSSIM->value[1],
        mSSIM->value[2], tmp_time, (int)p_Vid->me_time,
  ...
  {
    printf(
        "%05d(%3s)%8d %8d %3d  %1d %2d %4d %7.3f %7.3f %7.3f %7.4f %7.4f %7.4f "
        "%9d %7d    %3s %5d %1d %2d %2d  %d   %d\n",
        p_Vid->frm_no_in_file, pic_type, cur_bits, fdn_bits, nvb_bits,
        wp_method, p_Vid->AverageFrameQP, lambda, mPSNR->value[0],
        mPSNR->value[1], mPSNR->value[2], mSSIM->value[0], mSSIM->value[1],
...
static void ReportNALNonVLCBits(VideoParameters *p_Vid, int64 tmp_time) {
  ...
  if (p_Inp->num_of_views == 2) {
    if (p_Inp->Verbose != 0)
      printf("%05d(NVB)     %8d \n", p_Vid->frame_no,
             p_Stats->bit_ctr_parametersets_n);
  } else
#endif
  {
    if (p_Inp->Verbose != 0)
...
static void ReportFirstframe(VideoParameters *p_Vid, int64 tmp_time) {
  ...
  if (p_Inp->Verbose == 1) {
    ReportSimple(p_Vid, "IDR", cur_bits, &p_Vid->p_Dist->metric[PSNR],
                 (int)tmp_time);
  } else if (p_Inp->Verbose == 2) {
    int lambda = (int)p_Vid->lambda_me[I_SLICE][p_Vid->masterQP][F_PEL];
    if (p_Inp->Distortion[SSIM] == 1)
      ReportVerboseSSIM(p_Vid, "IDR", cur_bits, 0, lambda,
                        &p_Vid->p_Dist->metric[PSNR],
                        &p_Vid->p_Dist->metric[SSIM], (int)tmp_time, 0);
    else
      ReportVerbose(p_Vid, "IDR", cur_bits, 0, lambda,
                    &p_Vid->p_Dist->metric[PSNR], (int)tmp_time, 0);
  } else if (p_Inp->Verbose == 3) {
    int lambda = (int)p_Vid->lambda_me[I_SLICE][p_Vid->masterQP][F_PEL];
    if (p_Inp->Distortion[SSIM] == 1)
      ReportVerboseNVBSSIM(p_Vid, "IDR",
                           cur_bits + stats->bit_ctr_parametersets_n,
                           stats->bit_ctr_parametersets_n, 0, lambda,
                           &p_Vid->p_Dist->metric[PSNR],
                           &p_Vid->p_Dist->metric[SSIM], (int)tmp_time, 0);
    else
      ReportVerboseNVB(p_Vid, "IDR", cur_bits + stats->bit_ctr_parametersets_n,
                       stats->bit_ctr_parametersets_n, 0, lambda,
                       &p_Vid->p_Dist->metric[PSNR], (int)tmp_time, 0);
  } else if (p_Inp->Verbose == 4) {
    int lambda = (int)p_Vid->lambda_me[I_SLICE][p_Vid->masterQP][F_PEL];
    if (p_Inp->Distortion[SSIM] == 1)
      ReportVerboseFDNSSIM(
          p_Vid, "IDR", cur_bits + stats->bit_ctr_parametersets_n,
          (int)(stats->bit_ctr_filler_data - stats->bit_ctr_filler_data_n),
          stats->bit_ctr_parametersets_n, 0, lambda,
          &p_Vid->p_Dist->metric[PSNR], &p_Vid->p_Dist->metric[SSIM],
          (int)tmp_time, 0);
    else
      ReportVerboseFDN(
          p_Vid, "IDR", cur_bits + stats->bit_ctr_parametersets_n,
...
static void ReportI(VideoParameters *p_Vid, int64 tmp_time) {
  ...
  if (p_Inp->Verbose == 1) {
    ReportSimple(p_Vid, pic_type, cur_bits, &p_Vid->p_Dist->metric[PSNR],
                 (int)tmp_time);
  } else if (p_Inp->Verbose == 2) {
    int lambda = (int)p_Vid->lambda_me[I_SLICE][p_Vid->masterQP][F_PEL];
    if (p_Inp->Distortion[SSIM] == 1) {
      ReportVerboseSSIM(p_Vid, pic_type, cur_bits, 0, lambda,
                        &p_Vid->p_Dist->metric[PSNR],
                        &p_Vid->p_Dist->metric[SSIM], (int)tmp_time, 0);
    } else {
      ReportVerbose(p_Vid, pic_type, cur_bits, 0, lambda,
                    &p_Vid->p_Dist->metric[PSNR], (int)tmp_time, 0);
    }
  } else if (p_Inp->Verbose == 3) {
    int lambda = (int)p_Vid->lambda_me[I_SLICE][p_Vid->masterQP][F_PEL];
    if (p_Inp->Distortion[SSIM] == 1) {
      ReportVerboseNVBSSIM(p_Vid, pic_type,
                           cur_bits + stats->bit_ctr_parametersets_n,
                           stats->bit_ctr_parametersets_n, 0, lambda,
                           &p_Vid->p_Dist->metric[PSNR],
                           &p_Vid->p_Dist->metric[SSIM], (int)tmp_time, 0);
    } else {
      ReportVerboseNVB(p_Vid, pic_type,
                       cur_bits + stats->bit_ctr_parametersets_n,
                       stats->bit_ctr_parametersets_n, 0, lambda,
                       &p_Vid->p_Dist->metric[PSNR], (int)tmp_time, 0);
    }
    ...
    if (p_Inp->Distortion[SSIM] == 1) {
      ReportVerboseFDNSSIM(
          p_Vid, pic_type, cur_bits + stats->bit_ctr_parametersets_n,
          (int)(stats->bit_ctr_filler_data - stats->bit_ctr_filler_data_n),
          stats->bit_ctr_parametersets_n, 0, lambda,
          &p_Vid->p_Dist->metric[PSNR], &p_Vid->p_Dist->metric[SSIM],
          (int)tmp_time, 0);
...
static void ReportB(VideoParameters *p_Vid, int64 tmp_time) {
  ...
  if (p_Inp->Verbose == 1) {
    ReportSimple(p_Vid, " B ", cur_bits, &p_Vid->p_Dist->metric[PSNR],
                 (int)tmp_time);
  } else if (p_Inp->Verbose == 2) {
    int lambda = (int)p_Vid->lambda_me[B_SLICE][p_Vid->masterQP][F_PEL];
    if (p_Inp->Distortion[SSIM] == 1)
      ReportVerboseSSIM(
          p_Vid, " B ", cur_bits, p_Vid->active_pps->weighted_bipred_idc,
          lambda, &p_Vid->p_Dist->metric[PSNR], &p_Vid->p_Dist->metric[SSIM],
          (int)tmp_time, p_Vid->direct_spatial_mv_pred_flag);
    else
      ReportVerbose(p_Vid, " B ", cur_bits,
                    p_Vid->active_pps->weighted_bipred_idc, lambda,
                    &p_Vid->p_Dist->metric[PSNR], (int)tmp_time,
                    p_Vid->direct_spatial_mv_pred_flag);
  } else if (p_Inp->Verbose == 3) {
    int lambda = (int)p_Vid->lambda_me[B_SLICE][p_Vid->masterQP][F_PEL];
    if (p_Inp->Distortion[SSIM] == 1)
      ReportVerboseNVBSSIM(
          p_Vid, " B ", cur_bits + stats->bit_ctr_parametersets_n,
          stats->bit_ctr_parametersets_n,
          p_Vid->active_pps->weighted_bipred_idc, lambda,
          &p_Vid->p_Dist->metric[PSNR], &p_Vid->p_Dist->metric[SSIM],
          (int)tmp_time, p_Vid->direct_spatial_mv_pred_flag);
    else
      ReportVerboseNVB(p_Vid, " B ", cur_bits + stats->bit_ctr_parametersets_n,
                       stats->bit_ctr_parametersets_n,
                       p_Vid->active_pps->weighted_bipred_idc, lambda,
                       &p_Vid->p_Dist->metric[PSNR], (int)tmp_time,
                       p_Vid->direct_spatial_mv_pred_flag);
...
static void ReportP(VideoParameters *p_Vid, int64 tmp_time) {
  ...
  if (p_Inp->Verbose == 1) {
    ...
    if (p_Inp->Distortion[SSIM] == 1)
      ReportVerboseSSIM(p_Vid, pic_type, cur_bits,
                        p_Vid->active_pps->weighted_pred_flag, lambda,
                        &p_Vid->p_Dist->metric[PSNR],
                        &p_Vid->p_Dist->metric[SSIM], (int)tmp_time, 0);
    else
      ReportVerbose(p_Vid, pic_type, cur_bits,
                    p_Vid->active_pps->weighted_pred_flag, lambda,
                    &p_Vid->p_Dist->metric[PSNR], (int)tmp_time, 0);
  } else if (p_Inp->Verbose == 3) {
    int lambda = (int)p_Vid->lambda_me[P_SLICE][p_Vid->masterQP][F_PEL];
    if (p_Inp->Distortion[SSIM] == 1)
      ReportVerboseNVBSSIM(
          p_Vid, pic_type, cur_bits + stats->bit_ctr_parametersets_n,
          stats->bit_ctr_parametersets_n, p_Vid->active_pps->weighted_pred_flag,
          lambda, &p_Vid->p_Dist->metric[PSNR], &p_Vid->p_Dist->metric[SSIM],
          (int)tmp_time, 0);
    else
      ReportVerboseNVB(
          p_Vid, pic_type, cur_bits + stats->bit_ctr_parametersets_n,
          stats->bit_ctr_parametersets_n, p_Vid->active_pps->weighted_pred_flag,
          lambda, &p_Vid->p_Dist->metric[PSNR], (int)tmp_time, 0);
  } else if (p_Inp->Verbose == 4) {
    int lambda = (int)p_Vid->lambda_me[P_SLICE][p_Vid->masterQP][F_PEL];
    if (p_Inp->Distortion[SSIM] == 1)
      ReportVerboseFDNSSIM(
...
void output_SP_coefficients(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  for (k = 0; k < 2; k++) {
    for (i = 0; i < p_Vid->height_cr; i++) {
      ret = (int)fwrite(p_Vid->lrec_uv[k][i], sizeof(int), p_Vid->width_cr,
                        SP_coeff_file);
...
void read_SP_coefficients(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_Inp->sp_switch_period > 0 &&
      (p_Vid->number_sp2_frames % (p_Inp->sp_switch_period << 1)) >=
          p_Inp->sp_switch_period) {
  ...
  if (0 != fseek(SP_coeff_file,
                 p_Vid->size * 3 / 2 * p_Vid->number_sp2_frames * sizeof(int),
                 SEEK_SET)) {
  ...
  for (i = 0; i < p_Vid->height; i++) {
    if (p_Vid->width !=
        (int)fread(p_Vid->lrec[i], sizeof(int), p_Vid->width, SP_coeff_file)) {
  ...
  for (k = 0; k < 2; k++) {
    for (i = 0; i < p_Vid->height_cr; i++) {
      if (p_Vid->width_cr != (int)fread(p_Vid->lrec_uv[k][i], sizeof(int),
                                        p_Vid->width_cr, SP_coeff_file)) {
        printf("Fatal: cannot read in SP input file, exit (-1)\n");
        exit(-1);
      }
    }
...
void select_plane(VideoParameters *p_Vid, ColorPlane color_plane) {
  ...
  p_Vid->enc_picture->p_curr_img_sub =
      p_Vid->enc_picture->p_img_sub[color_plane];
...
static int is_gop_first_unit(VideoParameters *p_Vid, InputParameters *p_Inp) {
  return (get_idr_flag(p_Vid) ||
          (p_Vid->type == I_SLICE && p_Inp->EnableOpenGOP &&
           get_random_access_flag(p_Vid)));
}
...
void write_non_vcl_nalu(VideoParameters *p_Vid) {
  ...
  // SPS + PPS + AUD
  if (p_Inp->ResendSPS == 3 && is_gop_first_unit(p_Vid, p_Inp) &&
      p_Vid->number) {
    p_Vid->p_Stats->bit_slice = rewrite_paramsets(p_Vid);
  } else if (p_Inp->ResendSPS == 2 && get_idr_flag(p_Vid) && p_Vid->number) {
    p_Vid->p_Stats->bit_slice = rewrite_paramsets(p_Vid);
  } else if (p_Inp->ResendSPS == 1 && p_Vid->type == I_SLICE &&
             p_Vid->curr_frm_idx != 0) {
    p_Vid->p_Stats->bit_slice = rewrite_paramsets(p_Vid);
  }
  // PPS + AUD
  ...
  UpdateSceneInformation(
      p_Vid->p_SEI, FALSE, 0, 0,
      -1);  // JVT-D099, scene information SEI, nothing included by default
  ...
  p_Vid->p_Stats->bit_ctr_parametersets +=
      p_Vid->p_Stats->bit_ctr_parametersets_n;
}
...
void write_non_vcl_nalu_bot_fld(VideoParameters *p_Vid) {
  ...
  // update seq NVB counter
  p_Vid->p_Stats->bit_ctr_parametersets +=
      (p_Vid->p_Stats->bit_ctr_parametersets_n - init_param_set_bits);
}

void OtfCompatibility_copyWithPadding(imgpel **dstImg, imgpel **srcImg,
                                      int size_x, int size_y, int padding_x,
                                      int padding_y) {
  int i, j;
  ...
  wBufSrc = srcImg[0];
  // left IMG_PAD_SIZE
  for (i = 0; i < padding_x; ++i) *(wBufDst++) = wBufSrc[0];
  // center 0-(size_x)
  memcpy(wBufDst, wBufSrc, size_x * sizeof(imgpel));
  wBufDst += size_x;
  // right IMG_PAD_SIZE
  for (i = 0; i < padding_x; ++i) *(wBufDst++) = wBufSrc[size_x_minus1];

  // Now copy remaining pad lines
  for (j = -padding_y + 1; j < 0; ++j) {
    memcpy(dstImg[j] - padding_x, dstImg[j - 1] - padding_x,
           (size_x + 2 * padding_x) * sizeof(imgpel));
  }
...
void write_non_vcl_nalu_mvc(VideoParameters *p_Vid) {
  ...
  } else  // view_id == 0
  {
    // SPS + PPS + AUD
    if (p_Inp->ResendSPS == 3 && is_gop_first_unit(p_Vid, p_Inp) &&
        p_Vid->number && p_Vid->structure != BOTTOM_FIELD) {
      p_Vid->p_Stats->bit_slice = rewrite_paramsets(p_Vid);
    } else if (p_Inp->ResendSPS == 2 && get_idr_flag(p_Vid) && p_Vid->number &&
               p_Vid->structure != BOTTOM_FIELD) {
      p_Vid->p_Stats->bit_slice = rewrite_paramsets(p_Vid);
    } else if (p_Inp->ResendSPS == 1 && p_Vid->type == I_SLICE &&
               p_Vid->curr_frm_idx != 0 && p_Vid->structure != BOTTOM_FIELD) {
      p_Vid->p_Stats->bit_slice = rewrite_paramsets(p_Vid);
    }
    // PPS + AUD
    else if (p_Inp->ResendPPS && p_Vid->curr_frm_idx != 0) {
      ...
      if (p_Inp->SendAUD) {
        ...
        p_Vid->p_Stats->bit_ctr_parametersets_n_v[p_Vid->view_id] += bits;
        for (i = 0; i < total_pps; i++) {
          p_Vid->p_Stats->bit_ctr_parametersets_n += bits =
              write_PPS(p_Vid, 0, i);
          p_Vid->p_Stats->bit_ctr_parametersets_n_v[p_Vid->view_id] += bits;
        }
      } else {
