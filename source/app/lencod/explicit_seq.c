...
static int ReadTextField(FILE *expSeqFile, char *keyword) {
  ...
  do {
    ...
    if (err == -1) break;
    if (strcasecmp(word, keyword) != 0) err = -1;
...
static void ParseSliceType(char *slice_type, ExpFrameInfo *info,
                           int coding_index) {
...
static void ParseReferenceIDC(int reference_idc, int coding_index) {
  if (reference_idc < NALU_PRIORITY_DISPOSABLE ||
      reference_idc > NALU_PRIORITY_HIGHEST) {
  ...
  if (coding_index == 0 && reference_idc == NALU_PRIORITY_DISPOSABLE) {
    printf(
        "ReadExplicitSeqFile : First coded picture needs to be a reference "
        "picture.\n");
    report_stats_on_error();
  }
}

static void ParseSeqNumber(int seq_number, ExpSeqInfo *seq_info,
                           int coding_index) {
  int i;
  for (i = 0; i < imin(coding_index, seq_info->no_frames); i++) {
    if (seq_info->info[i].seq_number == seq_number) {
      printf(
          "ReadExplicitSeqFile : SeqNumber used for current frame already "
          "used. Terminating\n");
      report_stats_on_error();
    }
  }
...
void ReadFrameData(FILE *expSeqFile, ExpSeqInfo *seq_info, int coding_index) {
  ...
  do {
    ...
    } else if (err == 2)  // parse different parameters. Input method is
                          // flexible and does not fix input order.
    {
  ...
  if (slice_type_present == FALSE || seq_number_present == FALSE) {
    printf(
        "Sequence info file does not contain all mandatory info (SeqNumber or "
        "SliceType). Terminating.\n");
    report_stats_on_error();
  }
}
 ...
 ************************************************************************
 */
void ReadExplicitSeqFile(ExpSeqInfo *seq_info, FILE *expSFile,
                         int coding_index) {
...
void OpenExplicitSeqFile(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (ReadTextField(p_Vid->expSFile, "Sequence") == -1) {
    ...
    if (frm_count > 0) {
      ...
      p_Vid->expSeq->info =
          (ExpFrameInfo *)calloc(frm_count, sizeof(ExpFrameInfo));
    } else {
...
void CloseExplicitSeqFile(VideoParameters *p_Vid) {
  if (p_Vid->expSFile != NULL) fclose(p_Vid->expSFile);
