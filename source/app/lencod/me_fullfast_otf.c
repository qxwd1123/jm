...
*
* \brief
*    Motion Estimation using Full Search Fast (extention for on-the-fly
*interpolation)
*
*
*************************************************************************************
 ...
 ***********************************************************************
 */
void SetupFastFullPelSearch_otf(
    ...
    int list)  // <--  reference frame parameter, list0 or 1
{
  ...
  int (*dist_method)(int x) =
      p_Inp->MEErrorMetric[0 + ((p_Vid->dpb_layer_id >= 1) ? 3 : 0)] ? iabs2
                                                                     : iabs;
#if (JM_MEM_DISTORTION)
  int *imgpel_dist = p_Inp->MEErrorMetric[0 + p_Vid->layer_offset]
                         ? p_Vid->imgpel_quad
                         : p_Vid->imgpel_abs;
#endif
  MotionVector pmv;
  ...
  if (apply_weights) {
    ...
    for (pos = 0; pos < max_pos; pos++) {
      ...
      bindex = 0;
      refptr = data;
      p_Dpb->pf_get_block_luma(p_Vid, refptr, tmp_data, cand.mv_x, cand.mv_y,
                               MB_BLOCK_SIZE, MB_BLOCK_SIZE, ref_picture, 0);
      ...
      if (mv_block->ChromaMEEnable) {
        ...
        for (k = 0; k < 2; k++) {
          ...
          p_Dpb->pf_get_block_chroma[OTF_ME](
              p_Vid, refptr, tmp_data, cand.mv_x, cand.mv_y,
              p_Vid->mb_cr_size_x, p_Vid->mb_cr_size_y, ref_picture, k + 1);
