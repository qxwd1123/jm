...
*************************************************************************************
*/
void submacroblock_mode_decision_p_slice(Macroblock *currMB, RD_PARAMS *enc_mb,
                                         RD_8x8DATA *dataTr, int ****cofACtr,
                                         int block, distblk *cost) {
  ...
  *partition = best;
  ...
  for (j = 0; j <= 1; j++) {
    for (i = 0; i <= 1; i++)
      best_nz_coeff[i][j] = p_Vid->nz_coeff[currMB->mbAddrX][i1 + i][j1 + j] =
          0;
  }
#endif
  ...
  for (min_cost8x8 = DISTBLK_MAX, min_rdcost = DISTBLK_MAX, index = 1;
    ...
    *cost = 0;
    ...
    if (enc_mb->valid[mode] && (!(transform8x8 == 1 && mode > 4)) &&
        (transform8x8 == 0 || mode != 0)) {
      if (transform8x8) {
...

      {
        ...
        list_prediction_cost(currMB, LIST_0, block, mode, enc_mb, bmcost,
                             best.ref);
        ...
        motion[block_y][block_x].ref_pic[LIST_0] =
            currSlice->listX[currMB->list_offset][b_ref];
        motion[block_y][block_x].mv[LIST_0] =
            currSlice->all_mv[LIST_0][b_ref][mode][j1][i1];
        motion[block_y][block_x].ref_idx[LIST_0] = (char)b_ref;
        motion[block_y][block_x + 1].ref_pic[LIST_0] =
            currSlice->listX[currMB->list_offset][b_ref];
        motion[block_y][block_x + 1].mv[LIST_0] =
            currSlice->all_mv[LIST_0][b_ref][mode][j1][i1 + 1];
        motion[block_y][block_x + 1].ref_idx[LIST_0] = (char)b_ref;
        motion[block_y + 1][block_x].ref_pic[LIST_0] =
            currSlice->listX[currMB->list_offset][b_ref];
        motion[block_y + 1][block_x].mv[LIST_0] =
            currSlice->all_mv[LIST_0][b_ref][mode][j1 + 1][i1];
        motion[block_y + 1][block_x].ref_idx[LIST_0] = (char)b_ref;
        motion[block_y + 1][block_x + 1].ref_pic[LIST_0] =
            currSlice->listX[currMB->list_offset][b_ref];
        motion[block_y + 1][block_x + 1].mv[LIST_0] =
            currSlice->all_mv[LIST_0][b_ref][mode][j1 + 1][i1 + 1];
      ...
      rdcost = rdcost_for_8x8blocks(currMB, dataTr, &cnt_nonz, &curr_cbp_blk,
                                    enc_mb->lambda_mdfp, block, (short)mode,
                                    &best, min_rdcost);
        ...
        *partition = best;
        ...
        if (cnt_nonz) {
          for (i = 0; i <= 1; i++) {
            ...
            best_nz_coeff[i][1] =
                p_Vid->nz_coeff[currMB->mbAddrX][i1 + i][j1 + 1];
          }
        } else {
          for (i = 0; i <= 1; i++) {
            best_nz_coeff[i][0] = 0;
        ...
        dataTr->cbp_blk8x8 &=
            (~(0x33 << (((block >> 1) << 3) +
                        ((block & 0x01) << 1))));  // delete bits for block
        dataTr->cbp_blk8x8 |= curr_cbp_blk;

        //--- store coefficients ---
        memcpy(&cofACtr[0][0][0][0], &currSlice->cofAC[block][0][0][0],
               4 * 2 * 65 * sizeof(int));

        if (currSlice->P444_joined) {
          //--- store coefficients ---
          memcpy(&cofACtr[1][0][0][0], &currSlice->cofAC[block + 4][0][0][0],
                 4 * 2 * 65 * sizeof(int));
          memcpy(&cofACtr[2][0][0][0], &currSlice->cofAC[block + 8][0][0][0],
                 4 * 2 * 65 * sizeof(int));
        }

        //--- store reconstruction and prediction ---
        copy_image_data_8x8(&dataTr->rec_mbY8x8[j0],
                            &p_Vid->enc_picture->imgY[currMB->pix_y + j0], i0,
                            currMB->pix_x + i0);
        copy_image_data_8x8(&dataTr->mpr8x8[j0], &currSlice->mb_pred[0][j0], i0,
                            i0);
        ...
        if (currSlice->slice_type == SP_SLICE) {
          for (j = j0; j < j0 + BLOCK_SIZE_8x8; j++) {
            memcpy(&dataTr->lrec[j][i0],
                   &p_Vid->lrec[currMB->pix_y + j][currMB->pix_x + i0],
                   BLOCK_SIZE_8x8 * sizeof(int));
          }
        }

        if (currSlice->P444_joined) {
          copy_image_data_8x8(&dataTr->rec_mb8x8_cr[0][j0],
                              &p_Vid->enc_picture->imgUV[0][currMB->pix_y + j0],
                              i0, currMB->pix_x + i0);
          copy_image_data_8x8(&dataTr->mpr8x8CbCr[0][j0],
                              &currSlice->mb_pred[1][j0], i0, i0);

          copy_image_data_8x8(&dataTr->rec_mb8x8_cr[1][j0],
                              &p_Vid->enc_picture->imgUV[1][currMB->pix_y + j0],
                              i0, currMB->pix_x + i0);
          copy_image_data_8x8(&dataTr->mpr8x8CbCr[1][j0],
                              &currSlice->mb_pred[2][j0], i0, i0);
        }
      ...
      //--- re-set coding state as it was before coding with current mode was
      // performed ---
      if (index != maxindex - 1) {
    ...
    }  // if ((enc_mb->valid[mode] && (transform8x8 == 0 || mode != 0 || (mode
  ...
  if (valid_8x8 == TRUE) {
    ...
    if (!transform8x8) {
      if (block < 3) {
        ...
        copy_image_data_8x8(&p_Vid->enc_picture->imgY[currMB->pix_y + j0],
                            &dataTr->rec_mbY8x8[j0], currMB->pix_x + i0, i0);
        ...
        if (currSlice->slice_type == SP_SLICE) {
          for (j = j0; j < j0 + BLOCK_SIZE_8x8; j++) {
            memcpy(&p_Vid->lrec[currMB->pix_y + j][currMB->pix_x],
                   dataTr->lrec[j],
                   2 * BLOCK_SIZE *
                       sizeof(int));  // reset the coefficients for SP slice
          }
        }
        ...
        if (currSlice->P444_joined) {
          for (k = 0; k < 2; k++) {
            copy_image_data_8x8(
                &p_Vid->enc_picture->imgUV[k][currMB->pix_y + j0],
                &dataTr->rec_mb8x8_cr[k][j0], currMB->pix_x + i0, i0);
          }
        }
      }  // if (block<3)
    } else {
...
*************************************************************************************
*/
void submacroblock_mode_decision_b_slice(Macroblock *currMB, RD_PARAMS *enc_mb,
                                         RD_8x8DATA *dataTr, int ****cofACtr,
                                         int block, distblk *cost) {
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  ...
  *partition = best;
  ...
  //=====  LOOP OVER POSSIBLE CODING MODES FOR 8x8 SUB-PARTITION  =====
  for (min_cost8x8 = DISTBLK_MAX, min_rdcost = DISTBLK_MAX, index = 0;
       index < maxindex; index++) {
    mode = b8_mode_table[index];
    ...
    *cost = 0;
    ...
    if (enc_mb->valid[mode] && (!(transform8x8 == 1 && mode > 4)) &&
        (transform8x8 == 0 || mode != 0 ||
         (mode == 0 && p_Vid->active_sps->direct_8x8_inference_flag))) {
      if (transform8x8) {
        currMB->valid_8x8 = TRUE;
      } else
        currMB->valid_4x4 = TRUE;
      ...
      if (mode == 0)  //--- Direct8x8 Mode ---
      {
        ...
        list_prediction_cost(currMB, LIST_1, block, mode, enc_mb, bmcost,
                             best.ref);

        // Compute bipredictive cost between best list 0 and best list 1
        // references
        list_prediction_cost(currMB, BI_PRED, block, mode, enc_mb, bmcost,
                             best.ref);

        // currently Bi prediction ME is only supported for modes 1, 2, 3 and
        // only for ref 0 and only for ref 0
        ...
        for (k = LIST_0; k <= LIST_1; k++) {
          ...
          if (best.bipred) {
            motion[block_y][block_x].mv[k] =
                currSlice->bipred_mv[best.bipred - 1][k][(short)best.ref[k]]
                                    [mode][j1][i1];
            motion[block_y][block_x + 1].mv[k] =
                currSlice->bipred_mv[best.bipred - 1][k][(short)best.ref[k]]
                                    [mode][j1][i1 + 1];
            motion[block_y + 1][block_x].mv[k] =
                currSlice->bipred_mv[best.bipred - 1][k][(short)best.ref[k]]
                                    [mode][j1 + 1][i1];
            motion[block_y + 1][block_x + 1].mv[k] =
                currSlice->bipred_mv[best.bipred - 1][k][(short)best.ref[k]]
                                    [mode][j1 + 1][i1 + 1];
          } else {
            motion[block_y][block_x].mv[k] =
                currSlice->all_mv[k][(short)best.ref[k]][mode][j1][i1];
            motion[block_y][block_x + 1].mv[k] =
                currSlice->all_mv[k][(short)best.ref[k]][mode][j1][i1 + 1];
            motion[block_y + 1][block_x].mv[k] =
                currSlice->all_mv[k][(short)best.ref[k]][mode][j1 + 1][i1];
            motion[block_y + 1][block_x + 1].mv[k] =
                currSlice->all_mv[k][(short)best.ref[k]][mode][j1 + 1][i1 + 1];
          }
        }
      }  // if (mode!=0)
...
*************************************************************************************
*/
void submacroblock_mode_decision_low(Macroblock *currMB, RD_PARAMS *enc_mb,
                                     RD_8x8DATA *dataTr, int ****cofACtr,
                                     int *have_direct, int block,
                                     distblk *cost_direct, distblk *cost,
  ...
  if (transform8x8)
    currMB->luma_transform_size_8x8_flag = TRUE;  // switch to transform size
                                                  // 8x8
  ...
  //=====  LOOP OVER POSSIBLE CODING MODES FOR 8x8 SUB-PARTITION  =====
  for (min_cost8x8 = DISTBLK_MAX,
      index = (currSlice->slice_type == B_SLICE ? 0 : 1);
       index < maxindex; index++) {
    mode = b8_mode_table[index];
    best.mode = (char)mode;
    *cost = 0;
    ...
    if (enc_mb->valid[mode] && (!(transform8x8 == 1 && mode > 4)) &&
      ...
      if (mode == 0) {
        ...
        if ((direct4x4_tmp == DISTBLK_MAX) || (*cost_direct == DISTBLK_MAX)) {
          *cost_direct = DISTBLK_MAX;
          if (transform8x8) *cost8x8_direct = DISTBLK_MAX;
        } else {
          *cost_direct += direct4x4_tmp;
          if (transform8x8) *cost8x8_direct += direct8x8_tmp;
        }
        (*have_direct)++;
        ...
        if (transform8x8) {
          switch (p_Inp->Transform8x8Mode) {
            case 1:  // Mixture of 8x8 & 4x4 transform
              if ((direct8x8_tmp < direct4x4_tmp) ||
                  !(enc_mb->valid[5] && enc_mb->valid[6] && enc_mb->valid[7]))
                *cost = direct8x8_tmp;
              else
                *cost = direct4x4_tmp;
              break;
          ...
          if (p_Inp->Transform8x8Mode == 2) *cost = DISTBLK_MAX;
        } else {
          *cost = direct4x4_tmp;
        }
        ...
        if (currSlice->slice_type == B_SLICE) {
          ...
          // only for ref 0 and only for ref 0
          if (is_bipred_enabled(p_Vid, mode)) {
            list_prediction_cost(currMB, BI_PRED_L0, block, mode, enc_mb,
                                 bmcost, 0);
            list_prediction_cost(currMB, BI_PRED_L1, block, mode, enc_mb,
                                 bmcost, 0);
          } else {
            bmcost[BI_PRED_L0] = DISTBLK_MAX;
            bmcost[BI_PRED_L1] = DISTBLK_MAX;
      ...
      if (*cost != DISTBLK_MAX)
        *cost +=
            (ref_cost(currSlice, enc_mb->lambda_mf[Q_PEL],
                      (short)B8Mode2Value(currSlice, (short)mode, best.pdir),
                      (best.pdir < 1 ? currMB->list_offset
                                     : currMB->list_offset + LIST_1)) -
             1);
      ...
      if (*cost < min_cost8x8) {
        ...
        if (cnt_nonz) {
          ...
          best_nz_coeff[1][1] =
              p_Vid->nz_coeff[currMB->mbAddrX][i1 + 1][j1 + 1];
        } else {
          best_nz_coeff[0][0] = 0;
          best_nz_coeff[0][1] = 0;
          best_nz_coeff[1][0] = 0;
      ...
      currSlice->reset_coding_state(currMB, currSlice->p_RDO->cs_cm);
       ...
       // == 0 && active_sps->direct_8x8_inference_flag)))
  }  // for (min_rdcost=1e30, index=(currSlice->slice_type == B_SLICE ? 0 : 1);
     // index<6; index++)
  ...
  if (valid_8x8 == TRUE) {
    ...
    best_cnt_nonz = currSlice->luma_residual_coding_8x8(
        currMB, &dummy, &curr_cbp_blk, block, pdir, list_mode,
        dataTr->part[block].ref);

    if (currSlice->P444_joined) {
      best_cnt_nonz +=
          currSlice->coeff_cost_cr[1] + currSlice->coeff_cost_cr[2];
    }
    ...
    //--- store coefficients ---
    memcpy(cofACtr[0][0][0], currSlice->cofAC[block][0][0],
           4 * 2 * 65 * sizeof(int));

    if (currSlice->P444_joined) {
      //--- store coefficients ---
      memcpy(cofACtr[1][0][0], currSlice->cofAC[block + 4][0][0],
             4 * 2 * 65 * sizeof(int));
      memcpy(cofACtr[2][0][0], currSlice->cofAC[block + 8][0][0],
             4 * 2 * 65 * sizeof(int));
    }
    ...
    if (currSlice->slice_type == SP_SLICE) {
      for (j = j0; j < j0 + BLOCK_SIZE_8x8; j++) {
        memcpy(&dataTr->lrec[j][i0],
               &p_Vid->lrec[currMB->pix_y + j][currMB->pix_x + i0],
               BLOCK_SIZE_8x8 *
                   sizeof(int));  // store coefficients for primary SP slice
      }
    }
    if (currSlice->P444_joined) {
      copy_image_data_8x8(&dataTr->rec_mb8x8_cr[0][j0],
                          &p_Vid->enc_picture->imgUV[0][currMB->pix_y + j0], i0,
                          currMB->pix_x + i0);
      copy_image_data_8x8(&dataTr->mpr8x8CbCr[0][j0],
                          &currSlice->mb_pred[1][j0], i0, i0);
      copy_image_data_8x8(&dataTr->rec_mb8x8_cr[1][j0],
                          &p_Vid->enc_picture->imgUV[1][currMB->pix_y + j0], i0,
                          currMB->pix_x + i0);
      copy_image_data_8x8(&dataTr->mpr8x8CbCr[1][j0],
                          &currSlice->mb_pred[2][j0], i0, i0);
    }
    ...
    if (!transform8x8) {
      if (block < 3) {
        ...
        if (currSlice->P444_joined) {
          copy_image_data_8x8(&p_Vid->enc_picture->imgUV[0][currMB->pix_y + j0],
                              &dataTr->rec_mb8x8_cr[0][j0], currMB->pix_x + i0,
                              i0);
          copy_image_data_8x8(&p_Vid->enc_picture->imgUV[1][currMB->pix_y + j0],
                              &dataTr->rec_mb8x8_cr[1][j0], currMB->pix_x + i0,
                              i0);
        }
      }  // if (block<3)
    } else {
      //======= save motion data for 8x8 partition for transform size 8x8
      //========
      currSlice->store_8x8_motion_vectors(currSlice, 0, block,
                                          &dataTr->part[block]);
    }

    //===== set motion vectors and reference frames (prediction) =====
    currSlice->set_ref_and_motion_vectors(currMB, motion, &dataTr->part[block],
                                          block);
