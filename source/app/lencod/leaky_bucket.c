...
int get_LeakyBucketRate(InputParameters *p_Inp,
                        unsigned long NumberLeakyBuckets, unsigned long *Rmin) {
  FILE *f;
  unsigned long i, buf;

  if ((f = fopen(p_Inp->LeakyBucketRateFile, "r")) == NULL) {
    printf(
        " LeakyBucketRate File does not exist. Using rate calculated from avg. "
        "rate \n");
    return 0;
  }
...
void write_buffer(InputParameters *p_Inp, unsigned long NumberLeakyBuckets,
                  unsigned long Rmin[], unsigned long Bmin[],
                  unsigned long Fmin[]) {
  FILE *outf;
...
void calc_buffer(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (!buffer_frame) no_mem_exit("init_buffer: buffer_frame");
  Rmin = calloc(NumberLeakyBuckets, sizeof(unsigned long));
  if (!Rmin) no_mem_exit("init_buffer: Rmin");
  Bmin = calloc(NumberLeakyBuckets, sizeof(unsigned long));
  if (!Bmin) no_mem_exit("init_buffer: Bmin");
  Fmin = calloc(NumberLeakyBuckets, sizeof(unsigned long));
  if (!Fmin) no_mem_exit("init_buffer: Fmin");
  ...
  if (1 != get_LeakyBucketRate(p_Inp, NumberLeakyBuckets,
                               Rmin)) { /* if rate file is not present, use
                                           default calculated from avg.rate */
    for (iBucket = 0; iBucket < NumberLeakyBuckets; iBucket++) {
      if (iBucket == 0)
        Rmin[iBucket] = (unsigned long)((float)AvgRate *
                                        p_Vid->framerate); /* convert bits/frame
                                                              to bits/second */
      else
        Rmin[iBucket] = (unsigned long)((float)Rmin[iBucket - 1] +
                                        (AvgRate / 4) * (p_Vid->framerate));
    }
  }
  ...
  for (iBucket = 0; iBucket < NumberLeakyBuckets; iBucket++) {
    iChannelRate =
        (long)(Rmin[iBucket] /
               (p_Vid->framerate)); /* converts bits/second to bits/frame */
