 ...
 *
 * \brief
 *    Compute structural similarity (SSIM) index using the encoded image and the
 *reference image
 *
 * \author
...
// Computes the product of the contrast and structure componenents of the
// structural similarity metric.
float compute_structural_components(VideoParameters *p_Vid,
                                    InputParameters *p_Inp, imgpel **refImg,
                                    imgpel **encImg, int height, int width,
                                    int win_height, int win_width, int comp) {
  ...
  max_pix_value_sqd = (float)(p_Vid->max_pel_value_comp[comp] *
                              p_Vid->max_pel_value_comp[comp]);
  ...
  for (j = 0; j <= height - win_height; j += overlapSize) {
    for (i = 0; i <= width - win_width; i += overlapSize) {
      ...
      covOrgEnc =
          ((float)icovOrgEnc - ((float)imeanOrg) * meanEnc) / win_pixels_bias;
  ...
  if (cur_distortion >= 1.0 &&
      cur_distortion <
          1.01)  // avoid float accuracy problem at very low QP(e.g.2)
    cur_distortion = 1.0;

  return cur_distortion;
}

float compute_luminance_component(VideoParameters *p_Vid,
                                  InputParameters *p_Inp, imgpel **refImg,
                                  imgpel **encImg, int height, int width,
                                  int win_height, int win_width, int comp) {
...
static void imgpel_to_padded_int(imgpel **src, int **buffer, int width,
                                 int height) {
  int i, j;
  int *tmpDst;
  imgpel *tmpSrc;
...
void downsample(imgpel **src, imgpel **out, int height, int width) {
  ...
  for (j = 0; j < height; j++) {
    ...
    for (i = 0; i < width2; i++, iDst++) {
      ...
      tmpDst[iDst] = tmpSrc[ii - 2] + tmpSrc[ii + 3] + (tmp1 << 1) + tmp1 +
                     (tmp2 << 5) - (tmp2 << 2);
      tmpDst[iDst] >>= 6;
    }
  }
  ...
  for (i = 0; i < width2; i++) {
    ...
    for (j = 0; j < height2; j++) {
      ...
      tmp = dest[jj - 2][ii] + dest[jj + 3][ii] + (tmp1 << 1) + tmp1 +
            (tmp2 << 5) - (tmp2 << 2);
      out[j][i] =
          (unsigned char)(tmp >>
                          6);  // Note: Should change for different bit depths
    }
  }
  free_mem2Dint(itemp);
  free_mem2Dint(dest);
...
float compute_ms_ssim(VideoParameters *p_Vid, InputParameters *p_Inp,
                      imgpel **refImg, imgpel **encImg, int height, int width,
                      int win_height, int win_width, int comp) {
  ...
  static const float exponent[5] = { (float)MS_SSIM_BETA0, (float)MS_SSIM_BETA1,
                                     (float)MS_SSIM_BETA2, (float)MS_SSIM_BETA3,
                                     (float)MS_SSIM_BETA4 };
  ...
  structural[0] = compute_structural_components(
      p_Vid, p_Inp, refImg, encImg, height, width, win_height, win_width, comp);
  ...
  for (m = 1; m < MAX_SSIM_LEVELS; m++) {
    height >>= 1;
    width >>= 1;
    structural[m] = compute_structural_components(
        p_Vid, p_Inp, dsRef, dsEnc, height, width, imin(win_height, height),
        imin(win_width, width), comp);
    ...
    if (m < max_ssim_levels_minus_one) {
      ...
      luminance = compute_luminance_component(
          p_Vid, p_Inp, dsRef, dsEnc, height, width, imin(win_height, height),
          imin(win_width, width), comp);
 ...
 ************************************************************************
 */
void find_ms_ssim(VideoParameters *p_Vid, InputParameters *p_Inp,
                  ImageStructure *ref, ImageStructure *src,
                  DistMetric *metricSSIM) {
  DistortionParams *p_Dist = p_Vid->p_Dist;
  FrameFormat *format = &ref->format;

  metricSSIM->value[0] = compute_ms_ssim(
      p_Vid, p_Inp, ref->data[0], src->data[0], format->height[0],
      format->width[0], BLOCK_SIZE_8x8, BLOCK_SIZE_8x8, 0);
  // Chroma.
  if (format->yuv_format != YUV400) {
    metricSSIM->value[1] = compute_ms_ssim(
        p_Vid, p_Inp, ref->data[1], src->data[1], format->height[1],
        format->width[1], p_Vid->mb_cr_size_y, p_Vid->mb_cr_size_x, 1);
    metricSSIM->value[2] = compute_ms_ssim(
        p_Vid, p_Inp, ref->data[2], src->data[2], format->height[1],
        format->width[1], p_Vid->mb_cr_size_y, p_Vid->mb_cr_size_x, 2);
  }

  {
    accumulate_average(metricSSIM, p_Dist->frame_ctr);
    accumulate_avslice(metricSSIM, p_Vid->type,
