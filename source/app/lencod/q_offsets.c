 ...
 *
 * \brief
 *    read Quantization Offset matrix parameters from input file:
 *q_OffsetMatrix.cfg
 *
 *************************************************************************************
 */
...
static const char OffsetType4x4[15][24] = {
  "INTRA4X4_LUMA_INTRA",  "INTRA4X4_CHROMAU_INTRA",  "INTRA4X4_CHROMAV_INTRA",
  "INTRA4X4_LUMA_INTERP", "INTRA4X4_CHROMAU_INTERP", "INTRA4X4_CHROMAV_INTERP",
  "INTRA4X4_LUMA_INTERB", "INTRA4X4_CHROMAU_INTERB", "INTRA4X4_CHROMAV_INTERB",
  "INTER4X4_LUMA_INTERP", "INTER4X4_CHROMAU_INTERP", "INTER4X4_CHROMAV_INTERP",
  "INTER4X4_LUMA_INTERB", "INTER4X4_CHROMAU_INTERB", "INTER4X4_CHROMAV_INTERB"
};

static const char OffsetType8x8[15][24] = {
  "INTRA8X8_LUMA_INTRA",     "INTRA8X8_LUMA_INTERP",
  "INTRA8X8_LUMA_INTERB",    "INTER8X8_LUMA_INTERP",
  "INTER8X8_LUMA_INTERB",    "INTRA8X8_CHROMAU_INTRA",
  "INTRA8X8_CHROMAU_INTERP", "INTRA8X8_CHROMAU_INTERB",
  "INTER8X8_CHROMAU_INTERP", "INTER8X8_CHROMAU_INTERB",
  "INTRA8X8_CHROMAV_INTRA",  "INTRA8X8_CHROMAV_INTERP",
  "INTRA8X8_CHROMAV_INTERB", "INTER8X8_CHROMAV_INTERP",
  "INTER8X8_CHROMAV_INTERB"
};

static const short Offset_intra_flat_intra[16] = { 1024, 1024, 1024, 1024,
                                                   1024, 1024, 1024, 1024,
                                                   1024, 1024, 1024, 1024,
                                                   1024, 1024, 1024, 1024 };

static const short Offset_intra_flat_chroma[16] = { 1024, 1024, 1024, 1024,
                                                    1024, 1024, 1024, 1024,
                                                    1024, 1024, 1024, 1024,
                                                    1024, 1024, 1024, 1024 };
...
static void allocate_QOffsets(QuantParameters *p_Quant,
  ...
  int max_bitdepth =
      imax(p_Inp->output.bit_depth[0], p_Inp->output.bit_depth[1]);
...
static inline void update_q_offset4x4(LevelQuantParams **q_params,
                                      short *offsetList, int q_bits) {
  int i, j;
...
static inline void update_q_offset8x8(LevelQuantParams **q_params,
                                      short *offsetList, int q_bits) {
  int i, j;
  short *p_list;
  for (j = 0; j < 8; j++) {
  ...
  for (i = 0; i < item; i += cnt) {
    ...
    if (0 > (MapIdx = CheckOffsetParameterName(items[i + cnt], &type))) {
      snprintf(errortext, ET_SIZE,
               " Parsing error in quantization offset config file: Parameter "
    ...
    if (strcmp("=", items[i + cnt])) {
      snprintf(errortext, ET_SIZE,
               " Parsing error in quantization offset config file: '=' "
               "expected as the second token in each item.");
      error(errortext, 300);
    }
    cnt++;
    ...
    for (j = 0; j < range; j++) {
      if (1 != sscanf(items[i + cnt + j], "%d", &IntContent)) {
        snprintf(errortext, ET_SIZE,
                 " Parsing error in quantization offset file: Expected "
                 "numerical value for Parameter of %s, found '%s'.",
...
void init_qoffset(VideoParameters *p_Vid) {
  ...
  //! Now set up all offset p_Inp. This process could be reused if we wish to
  //! re-init offsets
...
void InitOffsetParam(QuantParameters *p_Quant, InputParameters *p_Inp) {
  ...
  for (i = 0;
       i < (p_Inp->AdaptRoundingFixed ? 1 : imax(max_qp_luma, max_qp_cr));
       i++) {
    if (p_Inp->OffsetMatrixPresentFlag) {
      memcpy(&(p_Quant->OffsetList4x4[i][0][0]),
             &(p_Quant->OffsetList4x4input[0][0]),
             400 * sizeof(short));  // 25 * 16
      memcpy(&(p_Quant->OffsetList8x8[i][0][0]),
             &(p_Quant->OffsetList8x8input[0][0]),
             960 * sizeof(short));  // 15 * 64
    } else {
      if (p_Inp->OffsetMatrixFlat == 1) {
        // 0 (INTRA4X4_LUMA_INTRA)
        memcpy(&(p_Quant->OffsetList4x4[i][0][0]),
               &(Offset_intra_flat_intra[0]), 16 * sizeof(short));
        for (k = 1; k < 3; k++)  // 1,2 (INTRA4X4_CHROMA_INTRA)
          memcpy(&(p_Quant->OffsetList4x4[i][k][0]),
                 &(Offset_intra_flat_chroma[0]), 16 * sizeof(short));
        for (k = 3; k < 9;
             k++)  // 3,4,5,6,7,8 (INTRA4X4_LUMA/CHROMA_INTERP/INTERB)
          memcpy(&(p_Quant->OffsetList4x4[i][k][0]),
                 &(Offset_intra_flat_inter[0]), 16 * sizeof(short));
        for (k = 9; k < 25; k++)  // 9,10,11,12,13,14 (INTER4X4)
          memcpy(&(p_Quant->OffsetList4x4[i][k][0]), &(Offset_inter_flat[0]),
                 16 * sizeof(short));

        // 0 (INTRA8X8_LUMA_INTRA)
        memcpy(&(p_Quant->OffsetList8x8[i][0][0]),
               &(Offset8_intra_flat_intra[0]), 64 * sizeof(short));
        for (k = 1; k < 3; k++)  // 1,2 (INTRA8X8_LUMA_INTERP/INTERB)
          memcpy(&(p_Quant->OffsetList8x8[i][k][0]),
                 &(Offset8_intra_flat_inter[0]), 64 * sizeof(short));
        for (k = 3; k < 5; k++)  // 3,4 (INTER8X8_LUMA_INTERP/INTERB)
          memcpy(&(p_Quant->OffsetList8x8[i][k][0]), &(Offset8_inter_flat[0]),
                 64 * sizeof(short));

        // 5 (INTRA8X8_CHROMAU_INTRA)
        memcpy(&(p_Quant->OffsetList8x8[i][5][0]),
               &(Offset8_intra_flat_chroma[0]), 64 * sizeof(short));
        ...
        memcpy(&(p_Quant->OffsetList8x8[i][10][0]),
               &(Offset8_intra_flat_chroma[0]), 64 * sizeof(short));
        ...
        // 0 (INTRA4X4_LUMA_INTRA)
        memcpy(&(p_Quant->OffsetList4x4[i][0][0]),
               &(Offset_intra_default_intra[0]), 16 * sizeof(short));
        for (k = 1; k < 3; k++)  // 1,2 (INTRA4X4_CHROMA_INTRA)
          memcpy(&(p_Quant->OffsetList4x4[i][k][0]),
                 &(Offset_intra_flat_chroma[0]), 16 * sizeof(short));
        memcpy(&(p_Quant->OffsetList4x4[i][3][0]),
               &(Offset_intra_default_inter[0]), 16 * sizeof(short));
        for (k = 4; k < 6; k++)  // 4,5 (INTRA4X4_CHROMA_INTERP)
          memcpy(&(p_Quant->OffsetList4x4[i][k][0]),
                 &(Offset_intra_flat_inter[0]), 16 * sizeof(short));
        memcpy(&(p_Quant->OffsetList4x4[i][6][0]),
               &(Offset_intra_default_inter[0]), 16 * sizeof(short));
        for (k = 7; k < 9; k++)  // 7,8 (INTRA4X4_CHROMA_INTERB)
          memcpy(&(p_Quant->OffsetList4x4[i][k][0]),
                 &(Offset_intra_flat_inter[0]), 16 * sizeof(short));
        for (k = 9; k < 25; k++)  // 9,10,11,12,13,14 (INTER4X4)
          memcpy(&(p_Quant->OffsetList4x4[i][k][0]), &(Offset_inter_default[0]),
                 16 * sizeof(short));

        // 0 (INTRA8X8_LUMA_INTRA)
        memcpy(&(p_Quant->OffsetList8x8[i][0][0]),
               &(Offset8_intra_default_intra[0]), 64 * sizeof(short));
        for (k = 1; k < 3; k++)  // 1,2 (INTRA8X8_LUMA_INTERP/INTERB)
          memcpy(&(p_Quant->OffsetList8x8[i][k][0]),
                 &(Offset8_intra_default_inter[0]), 64 * sizeof(short));
        for (k = 3; k < 5; k++)  // 3,4 (INTER8X8_LUMA_INTERP/INTERB)
          memcpy(&(p_Quant->OffsetList8x8[i][k][0]),
                 &(Offset8_inter_default[0]), 64 * sizeof(short));
        ...
        for (k = 1; k < 3; k++)  // 1,2 (INTRA4X4_CHROMA_INTRA)
          memcpy(&(p_Quant->OffsetList4x4[i][k][0]),
                 &(Offset_intra_default_chroma[0]), 16 * sizeof(short));
        for (k = 3; k < 9;
             k++)  // 3,4,5,6,7,8 (INTRA4X4_LUMA/CHROMA_INTERP/INTERB)
          memcpy(&(p_Quant->OffsetList4x4[i][k][0]),
                 &(Offset_intra_default_inter[0]), 16 * sizeof(short));
        ...
        // 5 (INTRA8X8_CHROMAU_INTRA)
        memcpy(&(p_Quant->OffsetList8x8[i][5][0]),
               &(Offset8_intra_default_chroma[0]), 64 * sizeof(short));
        for (k = 6; k < 8; k++)  // 6,7 (INTRA8X8_CHROMAU_INTERP/INTERB)
          memcpy(&(p_Quant->OffsetList8x8[i][k][0]),
                 &(Offset8_intra_default_inter[0]), 64 * sizeof(short));
        for (k = 8; k < 10; k++)  // 8,9 (INTER8X8_CHROMAU_INTERP/INTERB)
...
void CalculateOffset4x4Param(VideoParameters *p_Vid) {
  ...
  int k;
  int qp_per, qp;
  int img_type = ((p_Vid->type == SI_SLICE)
                      ? I_SLICE
                      : (p_Vid->type == SP_SLICE ? P_SLICE : p_Vid->type));
  ...
  p_Vid->AdaptRndWeight =
      p_Inp->AdaptRndWFactor[p_Vid->nal_reference_idc != 0][img_type];
  p_Vid->AdaptRndCrWeight =
      p_Inp->AdaptRndCrWFactor[p_Vid->nal_reference_idc != 0][img_type];
  ...
  if (img_type == I_SLICE) {
    for (qp = 0; qp < max_qp + 1; qp++) {
      ...
      // Intra4x4 luma
      update_q_offset4x4(p_Quant->q_params_4x4[0][1][qp],
                         p_Quant->OffsetList4x4[k][0], qp_per);
      // Intra4x4 chroma u
      update_q_offset4x4(p_Quant->q_params_4x4[1][1][qp],
                         p_Quant->OffsetList4x4[k][1], qp_per);
      // Intra4x4 chroma v
      update_q_offset4x4(p_Quant->q_params_4x4[2][1][qp],
                         p_Quant->OffsetList4x4[k][2], qp_per);
    }
    ...
    for (qp = 0; qp < max_qp + 1; qp++) {
      ...
      // Inter4x4 luma
      update_q_offset4x4(p_Quant->q_params_4x4[0][0][qp],
                         p_Quant->OffsetList4x4[k][12], qp_per);
      // Intra4x4 luma
      update_q_offset4x4(p_Quant->q_params_4x4[0][1][qp],
                         p_Quant->OffsetList4x4[k][6], qp_per);
      // Inter4x4 chroma u
      update_q_offset4x4(p_Quant->q_params_4x4[1][0][qp],
                         p_Quant->OffsetList4x4[k][13], qp_per);
      // Intra4x4 chroma u
      update_q_offset4x4(p_Quant->q_params_4x4[1][1][qp],
                         p_Quant->OffsetList4x4[k][7], qp_per);
      // Inter4x4 chroma v
      update_q_offset4x4(p_Quant->q_params_4x4[2][0][qp],
                         p_Quant->OffsetList4x4[k][14], qp_per);
      // Intra4x4 chroma v
      update_q_offset4x4(p_Quant->q_params_4x4[2][1][qp],
                         p_Quant->OffsetList4x4[k][8], qp_per);
    }
  } else {
    for (qp = 0; qp < max_qp + 1; qp++) {
      k = p_Quant->qp_per_matrix[qp];
      ...
      // Inter4x4 luma
      update_q_offset4x4(p_Quant->q_params_4x4[0][0][qp],
                         p_Quant->OffsetList4x4[k][9], qp_per);
      // Intra4x4 luma
      update_q_offset4x4(p_Quant->q_params_4x4[0][1][qp],
                         p_Quant->OffsetList4x4[k][3], qp_per);
      // Inter4x4 chroma u
      update_q_offset4x4(p_Quant->q_params_4x4[1][0][qp],
                         p_Quant->OffsetList4x4[k][10], qp_per);
      // Intra4x4 chroma u
      update_q_offset4x4(p_Quant->q_params_4x4[1][1][qp],
                         p_Quant->OffsetList4x4[k][4], qp_per);
      // Inter4x4 chroma v
      update_q_offset4x4(p_Quant->q_params_4x4[2][0][qp],
                         p_Quant->OffsetList4x4[k][11], qp_per);
      // Intra4x4 chroma v
      update_q_offset4x4(p_Quant->q_params_4x4[2][1][qp],
                         p_Quant->OffsetList4x4[k][5], qp_per);
    }
  }
}
...
void CalculateOffset8x8Param(VideoParameters *p_Vid) {
  ...
  if (p_Vid->type == I_SLICE || p_Vid->type == SI_SLICE) {
    for (qp = 0; qp < max_qp + 1; qp++) {
      ...
      k = p_Inp->AdaptRoundingFixed ? 0 : qp;
      // INTRA8X8
      update_q_offset8x8(p_Quant->q_params_8x8[0][1][qp],
                         p_Quant->OffsetList8x8[k][0], q_bits);
      // INTRA8X8 CHROMAU
      update_q_offset8x8(p_Quant->q_params_8x8[1][1][qp],
                         p_Quant->OffsetList8x8[k][5], q_bits);
      // INTRA8X8 CHROMAV
      update_q_offset8x8(p_Quant->q_params_8x8[2][1][qp],
                         p_Quant->OffsetList8x8[k][10], q_bits);
    }
    ...
    for (qp = 0; qp < max_qp + 1; qp++) {
      ...
      // INTER8X8
      update_q_offset8x8(p_Quant->q_params_8x8[0][0][qp],
                         p_Quant->OffsetList8x8[k][3], q_bits);
      // INTRA8X8
      update_q_offset8x8(p_Quant->q_params_8x8[0][1][qp],
                         p_Quant->OffsetList8x8[k][1], q_bits);
      // INTER8X8 CHROMAU
      update_q_offset8x8(p_Quant->q_params_8x8[1][0][qp],
                         p_Quant->OffsetList8x8[k][8], q_bits);
      // INTRA8X8 CHROMAU
      update_q_offset8x8(p_Quant->q_params_8x8[1][1][qp],
                         p_Quant->OffsetList8x8[k][6], q_bits);
      // INTER8X8 CHROMAV
      update_q_offset8x8(p_Quant->q_params_8x8[2][0][qp],
                         p_Quant->OffsetList8x8[k][13], q_bits);
      // INTRA8X8 CHROMAV
      update_q_offset8x8(p_Quant->q_params_8x8[2][1][qp],
                         p_Quant->OffsetList8x8[k][11], q_bits);
    }
  } else {
    for (qp = 0; qp < max_qp + 1; qp++) {
      q_bits = Q_BITS_8 + p_Quant->qp_per_matrix[qp] - OffsetBits;
      ...
      // INTER8X8
      update_q_offset8x8(p_Quant->q_params_8x8[0][0][qp],
                         p_Quant->OffsetList8x8[k][4], q_bits);
      // INTRA8X8
      update_q_offset8x8(p_Quant->q_params_8x8[0][1][qp],
                         p_Quant->OffsetList8x8[k][2], q_bits);
      // INTER8X8 CHROMAU
      update_q_offset8x8(p_Quant->q_params_8x8[1][0][qp],
                         p_Quant->OffsetList8x8[k][9], q_bits);
      // INTRA8X8 CHROMAU
      update_q_offset8x8(p_Quant->q_params_8x8[1][1][qp],
                         p_Quant->OffsetList8x8[k][7], q_bits);
      // INTER8X8 CHROMAV
      update_q_offset8x8(p_Quant->q_params_8x8[2][0][qp],
                         p_Quant->OffsetList8x8[k][14], q_bits);
      // INTRA8X8 CHROMAV
      update_q_offset8x8(p_Quant->q_params_8x8[2][1][qp],
                         p_Quant->OffsetList8x8[k][12], q_bits);
    }
  }
}
