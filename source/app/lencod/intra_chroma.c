...
void get_difference_4x4(imgpel **src, imgpel **prd, short *diff, int pos_x,
                        int block_x) {
  int i, j;
  short *p_diff = diff;
  ...
  for (j = 0; j < BLOCK_SIZE; j++) {
    ...
    for (i = 0; i < BLOCK_SIZE; i++) *p_diff++ = img_src[i] - img_prd[i];
  }
}
 ...
 ************************************************************************
 */
static inline void get_iChroma_vertical(imgpel **cur_pred, imgpel *PredPel,
                                        int cr_MB_x, int cr_MB_y) {
  int j;
  for (j = 0; j < cr_MB_y; j++)
 ...
 ************************************************************************
 */
static inline void get_iChroma_horizontal(imgpel **cur_pred, imgpel *PredPel,
                                          int cr_MB_x, int cr_MB_y) {
  int i, j;
 ...
 ************************************************************************
 */
static inline void get_iChroma_dc(imgpel **cur_pred, imgpel *PredPel,
                                  int left_available, int up_available) {
  int i, j, s0 = 0, s1 = 0, s2 = 0;

  if (up_available) {
    for (i = 1; i < MB_BLOCK_SIZE + 1; ++i) s1 += PredPel[i];
  }

  if (left_available) {
    for (i = 17; i < 33; ++i) s2 += PredPel[i];  // sum vert pix
  }
  ...
  for (j = 0; j < MB_BLOCK_SIZE; j++) {
    for (i = 0; i < MB_BLOCK_SIZE; i++) cur_pred[j][i] = (imgpel)s0;
  }
}
 ...
 ************************************************************************
 */
static inline void get_iChroma_plane(imgpel **cur_pred, imgpel *PredPel,
  ...
  for (j = 0; j < MB_BLOCK_SIZE; ++j) {
    for (i = 0; i < MB_BLOCK_SIZE; ++i) {
      cur_pred[j][i] = (imgpel)iClip1(
          max_imgpel_value, rshift_rnd_sf((iaa + (i - 7) * ib + (j - 7) * ic),
                                          5));  // store plane prediction
    }
  }
}
 ...
 ************************************************************************
 */
void set_intrapred_chroma(Macroblock *currMB, int *left_available,
                          int *up_available, int *all_available) {
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;

  int i;
  imgpel *PredPelU =
      currMB->intra16x16_pred[PLANE_U];  // array of predictor pels
  imgpel *PredPelV =
      currMB->intra16x16_pred[PLANE_V];  // array of predictor pels
  ...
  // form predictor pels
  if (*up_available) {
    memcpy(&PredPelU[1], &img_enc_u[pix_b.pos_y][pix_b.pos_x],
           cr_MB_x * sizeof(imgpel));
    memcpy(&PredPelV[1], &img_enc_v[pix_b.pos_y][pix_b.pos_x],
           cr_MB_x * sizeof(imgpel));
  } else {
 ...
 ************************************************************************
 */
void set_intrapred_chroma_mbaff(Macroblock *currMB, ColorPlane pl,
                                int *left_available, int *up_available,
                                int *all_available) {
  VideoParameters *p_Vid = currMB->p_Vid;
  InputParameters *p_Inp = currMB->p_Inp;
  ...
  if (p_Inp->UseConstrainedIntraPred == 0) {
    ...
    *up_available = pix_b.available ? p_Vid->intra_block[pix_b.mb_addr] : 0;
    for (i = 1, *left_available = 1; i < 17; ++i)
      *left_available &=
          pix_a[i].available ? p_Vid->intra_block[pix_a[i].mb_addr] : 0;
    *all_available =
        pix_a[0].available ? p_Vid->intra_block[pix_a[0].mb_addr] : 0;
  }
 ...
 ************************************************************************
 */
void get_intrapred_chroma(Macroblock *currMB, ColorPlane pl, int iChroma_mode,
                          int left_available, int up_available) {
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  switch (iChroma_mode) {
    case VERT_PRED_8:
      get_iChroma_vertical(curr_mpr_16x16[VERT_PRED_16], PredPel, cr_MB_x,
                           cr_MB_y);
      break;
    case HOR_PRED_8:
      get_iChroma_horizontal(curr_mpr_16x16[HOR_PRED_16], PredPel, cr_MB_x,
                             cr_MB_y);
      break;
    case DC_PRED_8:
      get_iChroma_dc(curr_mpr_16x16[DC_PRED_16], PredPel, left_available,
                     up_available);
      break;
    case PLANE_8:
      get_iChroma_plane(curr_mpr_16x16[PLANE_16], PredPel,
                        currMB->p_Vid->max_imgpel_value);
      break;
    default: printf("invalid prediction mode \n"); break;
  }
}

void rdo_low_intra_chroma_decision_mbaff(
    Macroblock *currMB, int mb_available_up, int mb_available_left[2],
    int mb_available_up_left) {  // since ipredmodes could be overwritten =>
  ...
  int mode;
  int best_mode =
      DC_PRED_8;  // just an initilaization here, should always be overwritten
  ...
  for (mode = DC_PRED_8; mode <= PLANE_8; mode++) {
    if (((currSlice->slice_type != I_SLICE &&
        ...
        p_Inp->ChromaIntraDisable == 1 && mode != DC_PRED_8)
      continue;

    if ((mode == VERT_PRED_8 && !mb_available_up) ||
        (mode == HOR_PRED_8 &&
         (!mb_available_left[0] || !mb_available_left[1])) ||
        (mode == PLANE_8 && (!mb_available_left[0] || !mb_available_left[1] ||
                             !mb_available_up || !mb_available_up_left)))
      continue;
...
void rdo_low_intra_chroma_decision(
    Macroblock *currMB, int mb_available_up, int mb_available_left[2],
    int mb_available_up_left) {  // since ipredmodes could be overwritten =>
                                 // encoder-decoder-mismatches
  Slice *currSlice = currMB->p_Slice;
  ...
  int mode;
  int best_mode =
      DC_PRED_8;  // just an initialization here, should always be overwritten
    ...
    for (uv = 1; uv < 3; uv++) {
      ...
      for (block_y = 0; block_y < cr_MB_y; block_y += BLOCK_SIZE) {
        for (block_x = 0; block_x < cr_MB_x; block_x += BLOCK_SIZE) {
          get_difference_4x4(&image[pos_y + block_y],
                             &curr_mpr_16x16[mode][block_y], diff, pos_x,
                             block_x);
          cost += p_Vid->distortion4x4(diff, min_cost);
          if (cost > min_cost) break;
        }
 ...
 ************************************************************************
 */
void intra_chroma_prediction(Macroblock *currMB, int *mb_up, int *mb_left,
                             int *mb_up_left) {
  int s, i, j;
  ...
  if (p_Inp->UseConstrainedIntraPred) {
    ...
    mb_available_left[0] = mb_available_left[1] =
        pix_a.available ? p_Vid->intra_block[pix_a.mb_addr] : 0;
    mb_available_up_left =
        pix_d.available ? p_Vid->intra_block[pix_d.mb_addr] : 0;
  }

  if (mb_up) *mb_up = mb_available_up;
  if (mb_left) *mb_left = mb_available_left[0];
  ...
  for (uv = 0; uv < 2; uv++) {
        ...
        switch (block_pos[yuv][b8][b4]) {
          case 0:  //===== TOP LEFT =====
          {
            ...
            if (mb_available_up) {
              ...
              for (i = 0; i < BLOCK_SIZE; i++) s0 += image[pos_y][pos_x++];
            }
            if (mb_available_left[0]) {
              int pos_x = pix_a.pos_x;
              int pos_y = pix_a.pos_y + block_y;

              for (i = 0; i < BLOCK_SIZE; i++) s2 += image[pos_y++][pos_x];
            }
          ...
          case 1:  //===== TOP RIGHT =====
          {
            ...
            if (mb_available_up) {
              int pos_x = pix_c.pos_x + blk_x;
              int pos_y = pix_c.pos_y;
              for (i = 0; i < BLOCK_SIZE; i++) s1 += image[pos_y][pos_x++];
            } else if (mb_available_left[0]) {
              int pos_x = pix_a.pos_x;
              int pos_y = pix_a.pos_y + block_y;
          ...
          case 2:  //===== BOTTOM LEFT =====
            if (mb_available_left[0]) {
              ...
              for (i = 0; i < BLOCK_SIZE; i++) s3 += image[pos_y++][pos_x];
              s = (s3 + 2) >> 2;
            } else if (mb_available_up) {
              int pos_x = pix_c.pos_x + blk_x;
              int pos_y = pix_c.pos_y;
    ...
    if (mb_available_left[0]) {
      ...
      for (i = 0; i < cr_MB_y; i++) vline[i] = image[pos_y++][pos_x];

      for (j = 0; j < cr_MB_y; j++) {
        int predictor = vline[j];
        for (i = 0; i < cr_MB_x; i++) cur_pred[j][i] = (imgpel)predictor;
      }
    }
    ...
    if (mb_available_left[0] && mb_available_up && mb_available_up_left) {
      ...
      for (j = 0; j < cr_MB_y; j++)
        for (i = 0; i < cr_MB_x; i++)
          cur_pred[j][i] = (imgpel)iClip1(max_imgpel_value_uv,
                                          (iaa + i * ib + j * ic + 16) >> 5);
    }
  }

  if (!p_Inp->rdopt)  // the rd-opt part does not work correctly (see
                      // encode_one_macroblock)
  {  // since ipredmodes could be overwritten => encoder-decoder-mismatches
    currSlice->rdo_low_intra_chroma_decision(
        currMB, mb_available_up, mb_available_left, mb_available_up_left);
  }
}
 ...
 ************************************************************************
 */
void intra_chroma_prediction_mbaff(Macroblock *currMB, int *mb_up, int *mb_left,
                                   int *mb_up_left) {
  int s, i, j;
  ...
  if (p_Inp->UseConstrainedIntraPred == 0) {
    ...
    for (i = 0, mb_available_left[0] = 1; i < (cr_MB_y >> 1); i++)
      mb_available_left[0] &=
          pix_a[i + 1].available ? p_Vid->intra_block[pix_a[i + 1].mb_addr] : 0;
  ...
  if (mb_up) *mb_up = mb_available_up;
  if (mb_left) *mb_left = mb_available_left[0] && mb_available_left[1];
  if (mb_up_left) *mb_up_left = mb_available_up_left;
  ...
  for (uv = 0; uv < 2; uv++) {
    ...
    // plane prediction
    if (mb_available_left[0] && mb_available_left[1] && mb_available_up &&
        mb_available_up_left) {
      int cr_x = (cr_MB_x >> 1);
      int cr_y = (cr_MB_y >> 1);
      if (cr_y > 8) cr_y = 8;
      ...
      int iaa, iv, ib, ic;
      int ih =
          cr_x * (hline[cr_MB_x - 1] - image[pix_a[0].pos_y][pix_a[0].pos_x]);
      for (i = 0; i < cr_x - 1; i++)
        ih += (i + 1) * (hline[cr_x + i] - hline[cr_x - 2 - i]);
 ...
 **************************************************************************************
 * \brief
 *    RD Decision for Intra prediction mode of the chrominance layers of one
 *macroblock
 **************************************************************************************
 */
void intra_chroma_RD_decision(Macroblock *currMB, RD_PARAMS *enc_mb) {
  ...
  if (p_Inp->UseConstrainedIntraPred) {
    ...
    mb_available_up_left =
        pix_b.available ? p_Vid->intra_block[pix_b.mb_addr] : 0;
  }
  ...
  for (mode = DC_PRED_8; mode <= PLANE_8; ++mode) {
    if ((mode == VERT_PRED_8 && !mb_available_up) ||
        ...
        (mode == PLANE_8 &&
         (!mb_available_left || !mb_available_up || !mb_available_up_left)))
      continue;

    cost = weight_cost(
        enc_mb->lambda_mf[Q_PEL],
        p_Vid->mvbits[mode]);  // exp golomb coding cost for mode signaling
...
void intra_chroma_RD_decision_mbaff(Macroblock *currMB, RD_PARAMS *enc_mb) {
  ...
  for (mode = DC_PRED_8; mode <= PLANE_8; ++mode) {
    ...
    // cost += (int) (enc_mb->lambda_me[Q_PEL] * p_Vid->mvbits[ mode ]); // exp
    // golomb coding cost for mode signaling
    cost += weight_cost(
        enc_mb->lambda_mf[Q_PEL],
        p_Vid->mvbits[mode]);  // exp golomb coding cost for mode signaling
