...
void JMHelpExit(void) {
  fprintf(
      ...
      "   -d :  use <defenc.cfg> as default file for parameter "
      "initializations.\n"
      ...
      "         See default encoder.cfg file for description of all "
      "parameters.\n\n"
      ...
      "   lencod  -f curenc1.cfg -p "
      "InputFile=\"e:\\data\\container_qcif_30.yuv\" -p SourceWidth=176 -p "
      "SourceHeight=144\n"
      "   lencod  -f curenc1.cfg -p FramesToBeEncoded=30 -p QPISlice=28 -p "
      "QPPSlice=28 -p QPBSlice=30\n");
...
void get_number_of_frames(InputParameters *p_Inp, VideoDataFile *input_file) {
  ...
  int maxBitDepth =
      imax(p_Inp->source.bit_depth[0], p_Inp->source.bit_depth[1]);

  isize <<= (maxBitDepth > 8) ? 1 : 0;
  p_Inp->no_frames =
      (int)(((fsize - p_Inp->infile_header) / isize) - p_Inp->start_frame);
}
...
static void updateOutFormat(InputParameters *p_Inp) {
  ...
  if (p_Inp->yuv_format == YUV400)  // reset bitdepth of chroma for 400 content
  {
    ...
    source->width[1] =
        (source->width[0] * mb_width_cr[output->yuv_format]) >> 4;
    source->width[2] = source->width[1];
    source->height[1] =
        (source->height[0] * mb_height_cr[output->yuv_format]) >> 4;
    source->height[2] = source->height[1];
    output->width[1] =
        (output->width[0] * mb_width_cr[output->yuv_format]) >> 4;
    output->width[2] = output->width[1];
    output->height[1] =
        (output->height[0] * mb_height_cr[output->yuv_format]) >> 4;
  ...
  source->pic_unit_size_on_disk =
      (imax(source->bit_depth[0], source->bit_depth[1]) > 8) ? 16 : 8;
 ...
 ***********************************************************************
 */
void Configure(VideoParameters *p_Vid, InputParameters *p_Inp, int ac,
               char *av[]) {
  char *content = NULL;
  ...
  if (p_Inp->DisplayEncParams) DisplayEncoderParams(Map);
}
...
void read_slice_group_info(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if ((p_Inp->slice_group_map_type != 0) &&
      (p_Inp->slice_group_map_type != 2) &&
      (p_Inp->slice_group_map_type != 6)) {
    // nothing to do
    return;
  }
  ...
  if (NULL == sgfile) {
    snprintf(errortext, ET_SIZE, "Error opening slice group file %s",
             p_Inp->SliceGroupConfigFileName);
    error(errortext, 500);
  }

  switch (p_Inp->slice_group_map_type) {
    case 0:
      p_Inp->run_length_minus1 =
          (int *)malloc(sizeof(int) * (p_Inp->num_slice_groups_minus1 + 1));
      ...
      for (i = 0; i <= p_Inp->num_slice_groups_minus1; i++) {
        ...
        if (1 != ret) {
          fclose(sgfile);
          snprintf(errortext, ET_SIZE,
                   "Error while reading slice group config file (line %d)",
                   i + 1);
          error(errortext, 500);
        }
        // scan remaining line
    ...
    case 2:
      ...
      PicSizeInMapUnits =
          (p_Inp->output.width[0] >> 4) * (p_Inp->output.height[0] >> 4);
      if (p_Inp->MbInterlace || p_Inp->PicInterlace) PicSizeInMapUnits >>= 1;

      p_Inp->top_left =
          (unsigned *)malloc(sizeof(unsigned) * p_Inp->num_slice_groups_minus1);
      p_Inp->bottom_right =
          (unsigned *)malloc(sizeof(unsigned) * p_Inp->num_slice_groups_minus1);
      ...
      for (i = 0; i < p_Inp->num_slice_groups_minus1; i++) {
        ...
        if (p_Inp->top_left[i] > PicSizeInMapUnits) {
          fprintf(stderr,
                  "Warning: slice group # %d top_left exceeds picture size "
                  "(will be clipped)\n",
                  i);
        }
        // scan remaining line
        ...
        if (p_Inp->bottom_right[i] > PicSizeInMapUnits) {
          fprintf(stderr,
                  "Warning: slice group # %d bottom_right exceeds picture size "
                  "(will be clipped)\n",
                  i);
        }
        // scan remaining line
    ...
    case 6: {
      ...
      p_Inp->slice_group_id =
          (byte *)malloc(sizeof(byte) * mapunit_height * mb_width);
      ...
      for (i = 0; i < mapunit_height * mb_width; i++) {
        ...
        if (*(p_Inp->slice_group_id + i) > p_Inp->num_slice_groups_minus1) {
          fclose(sgfile);
          snprintf(errortext, ET_SIZE,
                   "Error while reading slice group config file: "
                   "slice_group_id not allowed (line %d)",
                   i + 1);
          error(errortext, 500);
        }
        // scan remaining line
...
static void PatchInp(VideoParameters *p_Vid, InputParameters *p_Inp) {
  ...
  if (p_Inp->source.width[0] == 0 || p_Inp->source.height[0] == 0) {
    if (ParseSizeFromString(&p_Inp->input_file1, &(p_Inp->source.width[0]),
                            &(p_Inp->source.height[0]),
                            &(p_Inp->source.frame_rate)) == 0) {
      snprintf(errortext, ET_SIZE,
  ...
  if ((p_Inp->PicInterlace || p_Inp->MbInterlace) && p_Inp->symbol_mode) {
    snprintf(errortext, ET_SIZE,
             "Recompile with ENABLE_FIELD_CTX set to one to enable interlaced "
             "coding with CABAC.");
    error(errortext, 500);
  }
#endif
  ...
  if ((p_Inp->ProfileIDC == FREXT_Hi444 || p_Inp->ProfileIDC == NO_PROFILE) &&
      p_Inp->symbol_mode) {
    snprintf(errortext, ET_SIZE,
             "Recompile with ENABLE_HIGH444_CTX set to one to enable the High "
             "4:4:4 Profile with CABAC.");
    error(errortext, 500);
  }
#endif
  ...
  if (p_Inp->idr_period && p_Inp->intra_delay &&
      p_Inp->idr_period <= p_Inp->intra_delay) {
    snprintf(errortext, ET_SIZE,
             " IntraDelay cannot be larger than or equal to IDRPeriod.");
    error(errortext, 500);
  }

  // some contraints for IntraPeriod, IDRPeriod, EnableIDRGOP, and NumberBFrames
  if (p_Inp->idr_period > 0 &&
      (p_Inp->NumberBFrames + 1 + p_Inp->intra_delay) > p_Inp->idr_period) {
    snprintf(errortext, ET_SIZE,
  ...
  if (p_Inp->intra_period > 0 &&
      (p_Inp->NumberBFrames + 1 + p_Inp->intra_delay) > p_Inp->intra_period) {
    snprintf(errortext, ET_SIZE,
  ...
  if (p_Inp->SEIVUI32Pulldown && p_Inp->enable_32_pulldown) {
    snprintf(errortext, ET_SIZE,
             "SEIVUI32Pulldown and Enable32Pulldown cannot be set at the same "
             "time.");
    error(errortext, 500);
  }
  ...
  if (p_Inp->no_frames < 1) {
    snprintf(errortext, ET_SIZE, "Not enough frames to encode (%d)",
             p_Inp->no_frames);
    error(errortext, 500);
  }
  ...
  if (p_Inp->IntraProfile) {
    if (p_Inp->NumberBFrames) {
      snprintf(errortext, ET_SIZE,
               "Use of B slices not supported with Intra Profiles. Set "
               "NumberBFrames to 0.");
      error(errortext, 400);
    }
    ...
    if (p_Inp->PocMemoryManagement) {
      printf(
          "Warning: PocMemoryManagement not supported for Intra Profiles. "
          "Process Disabled.\n");
      p_Inp->PocMemoryManagement = 0;
    }
    if (p_Inp->ReferenceReorder) {
      printf(
          "Warning: ReferenceReorder not supported for Intra Profiles. Process "
          "Disabled.\n");
  ...
  if (p_Inp->NumberBFrames &&
      p_Inp->direct_spatial_mv_pred_flag != DIR_SPATIAL &&
      p_Inp->direct_spatial_mv_pred_flag != DIR_TEMPORAL) {
    snprintf(errortext, ET_SIZE,
             "Unsupported direct mode=%d, use TEMPORAL=0 or SPATIAL=1",
             p_Inp->direct_spatial_mv_pred_flag);
    error(errortext, 400);
  }
...
#if TRACE
  if ((int)strlen(p_Inp->TraceFile) > 0 &&
      (p_Enc->p_trace = fopen(p_Inp->TraceFile, "w")) == NULL) {
  ...
  if (p_Inp->WPMCPrecision &&
      (p_Inp->RDPictureDecision != 1 || p_Inp->GenerateMultiplePPS != 1)) {
    snprintf(errortext, ET_SIZE,
             "WPMCPrecision requires both RDPictureDecision=1 and "
  ...
  if (p_Inp->WPMCPrecision && p_Inp->WPMCPrecFullRef &&
    ...
    snprintf(errortext, ET_SIZE,
             "WPMCPrecFullRef requires NumberReferenceFrames < 16.\n");
    error(errortext, 400);
  }
  ...
  if (p_Inp->PicInterlace) {
    if (p_Inp->ReferenceReorder == 2) {
      snprintf(errortext, ET_SIZE,
               "ReferenceReorder = 2 are not supported with field encoding\n");
      error(errortext, 400);
    }
    if (p_Inp->PocMemoryManagement == 2) {
      snprintf(
          errortext, ET_SIZE,
          "PocMemoryManagement = 2 is not supported with field encoding\n");
      error(errortext, 400);
    }
  }
  ...
  if (p_Inp->ReferenceReorder && p_Inp->MbInterlace) {
    snprintf(errortext, ET_SIZE,
             "ReferenceReorder is not supported with MBAFF\n");
    error(errortext, 400);
  }

  if (p_Inp->SetFirstAsLongTerm && (p_Inp->ReferenceReorder != 0)) {
    printf(
        "SetFirstAsLongTerm is set. ReferenceReorder is not supported and "
        "therefore disabled. \n");
    p_Inp->ReferenceReorder = 0;
  }
  ...
  if (p_Inp->PocMemoryManagement && p_Inp->MbInterlace) {
    snprintf(errortext, ET_SIZE,
             "PocMemoryManagement is not supported with MBAFF\n");
    error(errortext, 400);
  }

  if (p_Inp->MbInterlace && p_Inp->RDPictureDecision &&
      p_Inp->GenerateMultiplePPS) {
    snprintf(errortext, ET_SIZE,
             "RDPictureDecision+GenerateMultiplePPS not supported with MBAFF. "
             "RDPictureDecision therefore disabled\n");
    p_Inp->RDPictureDecision = 0;
  }

  if ((!p_Inp->rdopt) && (p_Inp->MbInterlace == 2)) {
    snprintf(errortext, ET_SIZE,
             "MB AFF is not compatible with non-rd-optimized coding.");
    error(errortext, 500);
  }
  ...
  if (p_Inp->NumFramesInELSubSeq > p_Inp->num_ref_frames ||
      p_Inp->NumFramesInELSubSeq < 0) {
    snprintf(errortext, ET_SIZE,
             "NumFramesInELSubSeq (%d) is out of range [0,%d).",
             p_Inp->NumFramesInELSubSeq, p_Inp->num_ref_frames);
    error(errortext, 500);
  }
  ...
  if (p_Inp->of_mode != PAR_OF_RTP && p_Inp->SparePictureOption == TRUE) {
    snprintf(errortext, ET_SIZE,
             "Only RTP output mode is compatible with spare picture features.");
    error(errortext, 500);
  }
  ...
  if ((p_Inp->WeightedPrediction > 0 || p_Inp->WeightedBiprediction > 0) &&
    ...
    snprintf(
        errortext, ET_SIZE,
        "Weighted prediction coding is not supported for MB AFF currently.");
    error(errortext, 500);
  }
  if ((p_Inp->WeightedPrediction > 0 || p_Inp->WeightedBiprediction > 0) &&
      (p_Inp->yuv_format == 3)) {
    snprintf(errortext, ET_SIZE,
             "Weighted prediction coding is not supported for 4:4:4 encoding.");
    error(errortext, 500);
  }
  if (p_Inp->NumFramesInELSubSeq > 0 && p_Inp->WeightedPrediction > 0) {
    snprintf(errortext, ET_SIZE,
             "Enhanced GOP is not supported in weighted prediction coding mode "
             "yet.");
    error(errortext, 500);
  }
  ...
  if (p_Inp->RCEnable) {
    if (p_Inp->RCUpdateMode == RC_MODE_1 &&
        !((p_Inp->intra_period == 1 || p_Inp->idr_period == 1 ||
           p_Inp->BRefPictures == 2) &&
          !p_Inp->NumberBFrames)) {
      snprintf(
          errortext, ET_SIZE,
          "Use RCUpdateMode = 1 only for all intra or all B-slice coding.");
      error(errortext, 500);
    }
    if ((p_Inp->RCUpdateMode == RC_MODE_0 || p_Inp->RCUpdateMode == RC_MODE_2 ||
         p_Inp->RCUpdateMode == RC_MODE_3) &&
        (p_Inp->PReplaceBSlice && p_Inp->NumberBFrames)) {
      snprintf(errortext, ET_SIZE,
               "PReplaceBSlice is not supported with RCUpdateMode=0,2,3.");
      error(errortext, 500);
    }

    if (p_Inp->BRefPictures == 2 && p_Inp->intra_period == 0 &&
        p_Inp->RCUpdateMode != RC_MODE_1) {
      snprintf(errortext, ET_SIZE,
               "Use RCUpdateMode = 1 for all B-slice coding.");
      error(errortext, 500);
    }

    if (p_Inp->HierarchicalCoding && p_Inp->RCUpdateMode != RC_MODE_2 &&
        p_Inp->RCUpdateMode != RC_MODE_3) {
      snprintf(errortext, ET_SIZE,
    ...
    if ((p_Inp->RCUpdateMode != RC_MODE_1) && (p_Inp->intra_period == 1)) {
      snprintf(errortext, ET_SIZE,
               "Use RCUpdateMode = 1 for all intra coding.");
      error(errortext, 500);
    }
  }

  if ((p_Inp->NumberBFrames) && (p_Inp->BRefPictures) && (p_Inp->idr_period) &&
      (p_Inp->pic_order_cnt_type != 0)) {
    error(
        "Stored B pictures combined with IDR pictures only supported in "
        "Picture Order Count type 0\n",
        -1000);
  }

  if (!p_Inp->direct_spatial_mv_pred_flag && p_Inp->num_ref_frames < 2 &&
      p_Inp->NumberBFrames > 0)
    error("temporal direct needs at least 2 ref frames\n", -1000);

#if (MVC_EXTENSION_ENABLE)
  if (p_Inp->SepViewInterSearch && p_Inp->SearchMode[1] == FAST_FULL_SEARCH &&
      p_Inp->MEErrorMetric[F_PEL] > ERROR_SSE) {
    snprintf(errortext, ET_SIZE,
             "\nOnly SAD and SSE distortion computation supported with Fast "
             "Full Search.");
    error(errortext, 500);
  } else
#endif
  ...
  if (p_Inp->rdopt == 0) {
    if (p_Inp->DisableSubpelME[0] || p_Inp->DisableSubpelME[1]) {
      if (p_Inp->MEErrorMetric[F_PEL] != p_Inp->ModeDecisionMetric) {
        snprintf(errortext, ET_SIZE,
                 "\nLast refinement level (FPel) distortion not the same as "
                 "Mode decision distortion.\nPlease update MEDistortionFPel "
                 "(%d) and/or  MDDistortion(%d).",
                 p_Inp->MEErrorMetric[F_PEL], p_Inp->ModeDecisionMetric);
        error(errortext, 500);
      }
    } else if (p_Inp->MEErrorMetric[Q_PEL] != p_Inp->ModeDecisionMetric) {
      snprintf(errortext, ET_SIZE,
               "\nLast refinement level (QPel) distortion not the same as Mode "
               "decision distortion.\nPlease update MEDistortionQPel (%d) "
               "and/or  MDDistortion(%d).",
               p_Inp->MEErrorMetric[Q_PEL], p_Inp->ModeDecisionMetric);
      error(errortext, 500);
    }
  }
  // frext
  ...
  if (p_Inp->DisableIntra4x4 == 1 && p_Inp->DisableIntra16x16 == 1 &&
      p_Inp->EnableIPCM == 0 && p_Inp->Transform8x8Mode == 0) {
    snprintf(errortext, ET_SIZE,
  ...
  if (p_Inp->NumberBFrames &&
      ((p_Inp->BiPredMotionEstimation) &&
       (p_Inp->search_range[0] < p_Inp->BiPredMESearchRange[0]))) {
    snprintf(errortext, ET_SIZE,
  ...
  if (p_Inp->SepViewInterSearch) {
    if (p_Inp->NumberBFrames &&
        ((p_Inp->BiPredMotionEstimation) &&
         (p_Inp->search_range[1] < p_Inp->BiPredMESearchRange[1]))) {
      snprintf(errortext, ET_SIZE,
               "\nView1BiPredMESearchRange must be smaller or equal "
               "View1SearchRange.");
      error(errortext, 500);
    }
  }
  ...
  if (p_Inp->BiPredMotionEstimation) {
    ...
    for (i = 0; i < 4; i++) p_Inp->BiPredSearch[i] = 0;
  }
  ...
  if (p_Inp->ChromaMEEnable && p_Inp->yuv_format == YUV400) {
    fprintf(stderr,
            "Warning: ChromaMEEnable cannot be used with monochrome color "
            "format, disabling ChromaMEEnable.\n");
    p_Inp->ChromaMEEnable = FALSE;
  }

  if ((p_Inp->ChromaMCBuffer == 0) &&
      ((p_Inp->yuv_format == YUV444) && (!p_Inp->separate_colour_plane_flag))) {
    fprintf(
        stderr,
  ...
  if (p_Inp->redundant_pic_flag) {
    if (p_Inp->PicInterlace || p_Inp->MbInterlace) {
      snprintf(errortext, ET_SIZE,
               "Redundant pictures cannot be used with interlaced tools.");
      error(errortext, 500);
    }
    if (p_Inp->RDPictureDecision) {
      snprintf(errortext, ET_SIZE,
               "Redundant pictures cannot be used with RDPictureDecision.");
      error(errortext, 500);
    }
    if (p_Inp->NumberBFrames) {
      snprintf(errortext, ET_SIZE,
               "Redundant pictures cannot be used with B frames.");
      error(errortext, 500);
    }
    if (p_Inp->PrimaryGOPLength < (1 << p_Inp->NumRedundantHierarchy)) {
      snprintf(errortext, ET_SIZE,
               "PrimaryGOPLength must be equal or greater than "
               "2^NumRedundantHierarchy.");
      error(errortext, 500);
    }
    if (p_Inp->num_ref_frames < p_Inp->PrimaryGOPLength) {
      snprintf(errortext, ET_SIZE,
               "NumberReferenceFrames must be greater than or equal to "
               "PrimaryGOPLength.");
      error(errortext, 500);
    }
  }
  ...
  if (p_Inp->num_ref_frames == 1 && p_Inp->NumberBFrames) {
    fprintf(stderr,
            "\nWarning: B slices used but only one reference allocated within "
            "reference buffer.\n");
  ...
  if ((p_Inp->HierarchicalCoding || p_Inp->BRefPictures) &&
      p_Inp->NumberBFrames) {
    fprintf(stderr,
  ...
  if (p_Inp->UseRDOQuant == 1) {
    if (p_Inp->rdopt == 0) {
      snprintf(errortext, ET_SIZE,
               "RDO Quantization not supported with low complexity RDO.");
      error(errortext, 500);
    }

    if (p_Inp->MbInterlace != 0) {
      printf(
          "RDO Quantization currently not supported with MBAFF. Option "
          "disabled.\n");
      p_Inp->UseRDOQuant = 0;
      p_Inp->RDOQ_QP_Num = 1;
      p_Inp->RDOQ_CP_MV = 0;
  ...
  if (p_Inp->num_slice_groups_minus1 > 0 &&
      (p_Inp->GenerateMultiplePPS == 1 && p_Inp->RDPictureDecision == 1)) {
    printf(
        "Warning: Weighted Prediction may not function correctly for multiple "
        "slices\n");
  }
  ...
  if (p_Inp->BIdenticalList > 0 && p_Inp->ReferenceReorder != 1) {
    snprintf(errortext, ET_SIZE,
             "Set two lists of B picture identical can only be used when "
             "ReferenceReorder is 1");
    error(errortext, 500);
  }
#endif
  ...
  if ((p_Inp->useCRA == 1) && (p_Inp->PicInterlace > 0)) {
    snprintf(errortext, ET_SIZE,
             "HM5 like configuration options (CRA) cannot be used together "
             "with PicInterlace");
    error(errortext, 500);
  }
#endif
  ...
  if (p_Inp->HM50RefStructure == 1) {
    printf(
        "Warning: HM50RefStructure can only be used for random access case "
        "with GOP size equal to 8 and HM-5.0 coding order\n");
    printf(
        "         If it used for other coding structure, the performance may "
        "loss\n");
  }
  if (p_Inp->HM50RefStructure == 1 && p_Inp->NumberBFrames != 7) {
    printf(
        "Warning: HM50RefStructure can only be used for random access case "
        "with NumberBFrames equal to 7, turn it off\n");
    p_Inp->HM50RefStructure = 0;
  }
  if (p_Inp->HM50RefStructure == 1 && p_Inp->LowDelay == 1) {
    printf(
        "Warning: HM50RefStructure cannot be used for low delay, turn it "
        "off\n");
    p_Inp->HM50RefStructure = 0;
  }
  if ((p_Inp->HM50RefStructure == 1) && (p_Inp->PicInterlace > 0)) {
    snprintf(errortext, ET_SIZE,
             "HM5 like configuration options (HM50RefStructure) cannot be used "
             "together with PicInterlace");
    error(errortext, 500);
  }
#endif
  ...
  if (p_Inp->LDRefSetting == 1) {
    printf(
        "Warning: LDRefSetting can only be used for low delay case with GOP "
        "size equal to 4 and 4 reference frames as HM-5.0 low delay\n");
    printf(
        "         If other GOP size or number of reference frames are used, "
        "the performance may be loss with LDRefSetting\n");
  }
  ...
  if ((p_Inp->LDRefSetting == 1) && (p_Inp->PicInterlace > 0)) {
    snprintf(errortext, ET_SIZE,
             "HM5 like configuration options (LDRefSetting) cannot be used "
             "together with PicInterlace");
    error(errortext, 500);
  }
#endif
  ...
  if ((p_Inp->BLevel0MoreRef == 1) && (p_Inp->PicInterlace > 0)) {
    snprintf(errortext, ET_SIZE,
             "HM5 like configuration options (BLevel0MoreRef) cannot be used "
             "together with PicInterlace");
    error(errortext, 500);
  }
#endif
  ...
  if ((p_Inp->BIdenticalList == 1) && (p_Inp->PicInterlace > 0)) {
    snprintf(errortext, ET_SIZE,
             "HM5 like configuration options (BIdenticalList) cannot be used "
             "together with PicInterlace");
    error(errortext, 500);
  }
#endif
...
static void SetVUIScaleAndTicks(InputParameters *p_Inp, double frame_rate) {
  ...
  if (frame_rate_integer != frame_rate) {
    ...
    // check whether multiplying it with 1001/1000=1.001 brings it closer to
    // frame_rate_integer
    double new_frame_rate = 1.001 * frame_rate;

    if (dabs(new_frame_rate - frame_rate_integer) <
        dabs(frame_rate - frame_rate_integer)) {
      p_Inp->VUI.num_units_in_tick = 1001;
      p_Inp->VUI.time_scale =
          (int)floor(frame_rate_integer * (1000 << 1));  // two ticks per frame
    } else {
      p_Inp->VUI.num_units_in_tick = 1000;
      p_Inp->VUI.time_scale =
          (int)floor(frame_rate * (p_Inp->VUI.num_units_in_tick << 1) +
                     0.5);  // two ticks per frame
    }
  } else {
    ...
    p_Inp->VUI.time_scale =
        (int)floor(frame_rate *
                   (p_Inp->VUI.num_units_in_tick << 1));  // two ticks per frame
  }
}
