...
int RBSPtoNALU(unsigned char *rbsp, NALU_t *nalu, int rbsp_size,
               int nal_unit_type, int nal_reference_idc,
  ...
  if (nal_unit_type == NALU_TYPE_PREFIX || nal_unit_type == NALU_TYPE_SLC_EXT) {
    ...
    // nalu->non_idr_flag       = (nal_reference_idc==NALU_PRIORITY_HIGHEST) ?
    // 0:1;
...
int Write_AUD_NALU(VideoParameters *p_Vid) {
  ...
  switch (p_Vid->type) {
    case I_SLICE: p_Vid->primary_pic_type = 0; break;
    case P_SLICE: p_Vid->primary_pic_type = 1; break;
    case B_SLICE: p_Vid->primary_pic_type = 2; break;
  }
  RBSPlen = 1;
...
int Write_Filler_Data_NALU(VideoParameters *p_Vid, int num_bytes) {
  ...
  num_bytes = imax(2, num_bytes);  // one byte for the NAL header and one for
                                   // the rbsp trailing byte
