...
static void fast_mode_intra_decision(Macroblock *currMB, short *intra_skip) {
  ...
  if ((currMB->mb_y != (int)p_Vid->FrameHeightInMbs - 1) &&
      (currMB->mb_x != (int)p_Vid->PicWidthInMbs - 1) && mb_available_left &&
      mb_available_up) {
    for (i = 0; i < MB_BLOCK_SIZE; i++) {
      SBE +=
          iabs(p_Vid->pCurImg[currMB->opix_y][currMB->pix_x + i] -
               p_Vid->enc_picture->imgY[currMB->pix_y - 1][currMB->pix_x + i]);
      SBE +=
          iabs(p_Vid->pCurImg[currMB->opix_y + i][currMB->pix_x] -
               p_Vid->enc_picture->imgY[currMB->pix_y + i][currMB->pix_x - 1]);
    }

    for (i = 0; i < 8; i++) {
      SBE += iabs(p_Vid->pImgOrg[1][currMB->opix_c_y][currMB->pix_c_x + i] -
                  p_Vid->enc_picture
                      ->imgUV[0][currMB->pix_c_y - 1][currMB->pix_c_x + i]);
      SBE += iabs(p_Vid->pImgOrg[1][currMB->opix_c_y + i][currMB->pix_c_x] -
                  p_Vid->enc_picture
                      ->imgUV[0][currMB->pix_c_y + i][currMB->pix_c_x - 1]);
      SBE += iabs(p_Vid->pImgOrg[2][currMB->opix_c_y][currMB->pix_c_x + i] -
                  p_Vid->enc_picture
                      ->imgUV[1][currMB->pix_c_y - 1][currMB->pix_c_x + i]);
      SBE += iabs(p_Vid->pImgOrg[2][currMB->opix_c_y + i][currMB->pix_c_x] -
                  p_Vid->enc_picture
                      ->imgUV[1][currMB->pix_c_y + i][currMB->pix_c_x - 1]);
    }
    ABE = 1.0 / 64 * SBE;
  } else  // Image boundary
  {
...
void encode_one_macroblock_highfast(Macroblock *currMB) {
  ...
  MotionVector *allmvs = (currSlice->slice_type == I_SLICE)
                             ? NULL
  ...
  if (!intra) {
    ...
    for (mode = 1; mode < 4; mode++) {
      ...
      if (enc_mb.valid[mode] && !inter_skip) {
        ...
        if (mode == 1) {
          if (pslice) currMB->min_rdcost = max_rdcost;

          currMB->c_ipred_mode = DC_PRED_8;
          compute_mode_RD_cost(currMB, &enc_mb, (short)mode, &inter_skip);

          if (pslice) {
            // Get SKIP motion vector and compare SKIP_MV with best motion
            // vector of 16x16
            ...
            if (p_Inp->EarlySkipEnable) {
              //===== check for SKIP mode =====
              if (currMB->cbp == 0 &&
                  motion[currMB->block_y][currMB->block_x].ref_idx[LIST_0] ==
                      0 &&
    ...
    if ((!inter_skip) && enc_mb.valid[P8x8]) {
      ...
      if (p_Inp->Transform8x8Mode) {
        ...
        currMB->luma_transform_size_8x8_flag =
            TRUE;  // switch to 8x8 transform size
        //===========================================================
        ...
        for (block = 0; block < 4; block++) {
          currSlice->submacroblock_mode_decision(currMB, &enc_mb, p_RDO->tr8x8,
                                                 p_RDO->cofAC8x8ts[block],
                                                 block, &cost);
          if (!currMB->valid_8x8) break;
      ...
      if (p_Inp->Transform8x8Mode != 2) {
        currMB->luma_transform_size_8x8_flag =
            FALSE;  // switch to 8x8 transform size
        ...
        // Decision) =====
        for (block = 0; block < 4; block++) {
          currSlice->submacroblock_mode_decision(currMB, &enc_mb, p_RDO->tr4x4,
                                                 p_RDO->coefAC8x8[block], block,
                                                 &cost);
          if (!currMB->valid_4x4) break;
  ...
  //-------------------------------------------------------------------------
  {
    if (!inter_skip) {
      ...
      // if Fast High mode, compute  inter modes separate process for
      // inter/intra
      ...
      for (currMB->c_ipred_mode = chroma_pred_mode_range[0];
        ...
        if ((p_Vid->yuv_format != YUV400) &&
            (((!intra || !p_Inp->IntraDisableInterOnly) &&
              p_Inp->ChromaIntraDisable == 1 &&
              currMB->c_ipred_mode != DC_PRED_8) ||
             (currMB->c_ipred_mode == VERT_PRED_8 && !mb_available[0]) ||
             (currMB->c_ipred_mode == HOR_PRED_8 && !mb_available[1]) ||
             (currMB->c_ipred_mode == PLANE_8 &&
              (!mb_available[1] || !mb_available[0] || !mb_available[2]))))
          continue;
        ...
        for (index = 0; index < max_index; index++) {
          ...
          if (enc_mb.valid[mode]) {
            ...
            // Skip intra modes in inter slices if best mode is inter <P8x8 with
            // cbp equal to 0
            if (currSlice->P444_joined) {
              ...
              if (p_Inp->SkipIntraInInterSlices && !intra && mode >= I4MB &&
                  currMB->best_mode <= 3 && currMB->best_cbp == 0 &&
                  (currMB->min_rdcost < weighted_cost(enc_mb.lambda_mdfp, 5)))
                continue;
            }
      ...
      if ((currSlice->slice_type != I_SLICE &&
           currSlice->slice_type != SI_SLICE) &&
          p_Inp->SelectiveIntraEnable && !is_FREXT_profile(p_Inp->ProfileIDC)) {
          ...
          for (currMB->c_ipred_mode = chroma_pred_mode_range[0];
            ...
            if ((p_Vid->yuv_format != YUV400) &&
                (((!intra || !p_Inp->IntraDisableInterOnly) &&
                 ...
                 (currMB->c_ipred_mode == HOR_PRED_8 && !mb_available[1]) ||
                 (currMB->c_ipred_mode == PLANE_8 &&
                  (!mb_available[0] || !mb_available[1] || !mb_available[2]))))
              continue;
            ...
            for (index = 5; index < max_index; index++) {
              ...
              if (p_Vid->yuv_format != YUV400) {
                currMB->i16mode = 0;
                // RDcost of mode 1 in P-slice and mode 0, 1 in B-slice are
                // already available
                if (((bslice && mode == 0) || (!islice && mode == 1))) continue;
              }
