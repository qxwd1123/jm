 ...
 *
 * \brief
 *    Support for Flexible Macroblock Ordering for different Slice Group Modes:
 *MBAmap handling
 *
 * \date
 ...
 *  The functions for generating map are very similar to that in decoder, but
 *have a little difference.
 *
 *  The MB map is calculated at the beginning of coding of each picture (frame
 *or field).
 *
 *  'slice_group_change_cycle' in structure 'VideoParameters' is the syntax in
 *the slice header. It's set to be 1 before the initialization of FMO in
 *function code_a_picture(). It can be changed every time if needed.
 *
 ****************************************************************************
 */
...
static int FmoGenerateMapUnitToSliceGroupMap(VideoParameters *p_Vid,
                                             pic_parameter_set_rbsp_t *pps);
static int FmoGenerateMBAmap(VideoParameters *p_Vid,
...
static int FmoGenerateMapUnitToSliceGroupMap(VideoParameters *p_Vid,
  ...
  if (p_Vid->MapUnitToSliceGroupMap) free(p_Vid->MapUnitToSliceGroupMap);

  if ((p_Vid->MapUnitToSliceGroupMap =
           malloc((p_Vid->PicSizeInMapUnits) * sizeof(byte))) == NULL) {
    printf(
  ...
  {
    memset(p_Vid->MapUnitToSliceGroupMap, 0,
           p_Vid->PicSizeInMapUnits * sizeof(byte));
    return 0;
  }

  switch (pps->slice_group_map_type) {
    case 0: FmoGenerateType0MapUnitMap(p_Vid, pps); break;
    case 1: FmoGenerateType1MapUnitMap(p_Vid, pps); break;
    case 2: FmoGenerateType2MapUnitMap(p_Vid, pps); break;
    case 3: FmoGenerateType3MapUnitMap(p_Vid, pps); break;
    case 4: FmoGenerateType4MapUnitMap(p_Vid, pps); break;
    case 5: FmoGenerateType5MapUnitMap(p_Vid, pps); break;
    case 6: FmoGenerateType6MapUnitMap(p_Vid, pps); break;
    default:
...
static int FmoGenerateMBAmap(VideoParameters *p_Vid,
  ...
  if (p_Vid->MBAmap) free(p_Vid->MBAmap);
 ...
 ************************************************************************
 */
int FmoInit(VideoParameters *p_Vid, pic_parameter_set_rbsp_t *pps,
            seq_parameter_set_rbsp_t *sps) {
  ...
  int k;
  for (k = 0; k < MAXSLICEGROUPIDS; k++) p_Vid->FirstMBInSlice[k] = -1;
  ...
  for (j = 0; j < p_Vid->PicHeightInMapUnits; j++) {
    for (i = 0; i < p_Vid->PicWidthInMbs; i++) {
      printf("%d ",
             p_Vid->MapUnitToSliceGroupMap[i + j * p_Vid->PicWidthInMbs]);
    }
    printf("\n");
  }
  printf("\n");
  ...
  if (sps->mb_adaptive_frame_field_flag == 0) {
    ...
    for (j = 0; j < (p_Vid->PicSizeInMbs / p_Vid->PicWidthInMbs); j++) {
      for (i = 0; i < p_Vid->PicWidthInMbs; i++) {
        bottom = (j & 0x01);
        printf("%d ", p_Vid->MBAmap[(j - bottom) * p_Vid->PicWidthInMbs +
                                    i * 2 + bottom]);
      }
      printf("\n");
    }
...
static void FmoGenerateType0MapUnitMap(VideoParameters *p_Vid,
  ...
  do {
    for (iGroup = 0; (iGroup <= pps->num_slice_groups_minus1) &&
                     (i < p_Vid->PicSizeInMapUnits);
         i += pps->run_length_minus1[iGroup++] + 1) {
      for (j = 0; j <= pps->run_length_minus1[iGroup] &&
                  i + j < p_Vid->PicSizeInMapUnits;
           j++)
...
static void FmoGenerateType3MapUnitMap(VideoParameters *p_Vid,
  ...
  unsigned mapUnitsInSliceGroup0 =
      imin((pps->slice_group_change_rate_minus1 + 1) *
               p_Vid->slice_group_change_cycle,
           p_Vid->PicSizeInMapUnits);
...
static void FmoGenerateType4MapUnitMap(VideoParameters *p_Vid,
  ...
  unsigned sizeOfUpperLeftGroup =
      pps->slice_group_change_direction_flag
          ? (p_Vid->PicSizeInMapUnits - mapUnitsInSliceGroup0)
  ...
  for (i = 0; i < p_Vid->PicSizeInMapUnits; i++)
    if (i < sizeOfUpperLeftGroup)
      p_Vid->MapUnitToSliceGroupMap[i] =
          (byte)pps->slice_group_change_direction_flag;
    else
      p_Vid->MapUnitToSliceGroupMap[i] =
          (byte)(1 - pps->slice_group_change_direction_flag);
}
...
int FmoGetNextMBNr(VideoParameters *p_Vid, int CurrentMbNr) {
  ...
  while (++CurrentMbNr < (int)p_Vid->PicSizeInMbs &&
         p_Vid->MBAmap[CurrentMbNr] != SliceGroupID)
    ;
...
int FmoGetFirstMBOfSliceGroup(VideoParameters *p_Vid, int SliceGroupID) {
  int i = 0;
  while ((i < (int)p_Vid->PicSizeInMbs) &&
         (FmoMB2SliceGroup(p_Vid, i) != SliceGroupID))
    i++;
...
int FmoGetLastCodedMBOfSliceGroup(VideoParameters *p_Vid, int SliceGroupID) {
  ...
  for (i = 0; i < (int)p_Vid->PicSizeInMbs; i++)
    if (FmoMB2SliceGroup(p_Vid, i) == SliceGroupID) LastMB = i;
  return LastMB;
}
...
void FmoSetLastMacroblockInSlice(VideoParameters *p_Vid, int mb) {
  ...
  assert(mb >= 0);
  mb = FmoGetNextMBNr(
      p_Vid, mb);  // The next (still uncoded) MB, or -1 if SG is finished
...
int FmoSliceGroupCompletelyCoded(VideoParameters *p_Vid, int SliceGroupID) {
  if (FmoGetFirstMacroblockInSlice(p_Vid, SliceGroupID) <
      0)  // slice group completelty coded or not present
    return TRUE;
  else
    return FALSE;
}
