...
float compute_ssim(VideoParameters *p_Vid, InputParameters *p_Inp,
  ...
  for (j = 0; j <= height - win_height; j += overlapSize) {
    for (i = 0; i <= width - win_width; i += overlapSize) {
      ...
      mb_ssim =
          (float)((2.0 * meanOrg * meanEnc + C1) * (2.0 * covOrgEnc + C2));
 ...
 ************************************************************************
 */
void find_ssim(VideoParameters *p_Vid, InputParameters *p_Inp,
