...
void InitWP(VideoParameters *p_Vid, InputParameters *p_Inp,
  ...
  } else if (p_Inp->WPIterMC && p_Vid->nal_reference_idc) {
...
void ComputeImplicitWeights(
    Slice *currSlice, short default_weight[3],
  ...
  for (i = 0; i < currSlice->listXsize[LIST_0]; i++) {
    for (j = 0; j < currSlice->listXsize[LIST_1]; j++) {
      ...
      tb = (short)iClip3(-128, 127,
                         (p_Vid->ThisPOC - currSlice->listX[LIST_0][i]->poc));
...
static int ComputeBlockSum(imgpel **CurrentImage, int height_in_blk,
                           int width_in_blk, int blk_size_y, int blk_size_x,
                           int cur_blk) {
  int x, y;
  int sum = 0;
...
void ComputeImgSumBlockBased(imgpel **CurrentImage, int height_in_blk,
                             int width_in_blk, int blk_size_y, int blk_size_x,
  ...
  for (cur_blk = start_blk; cur_blk < end_blk; cur_blk++) {
    block_sum = ComputeBlockSum(CurrentImage, height_in_blk, width_in_blk,
                                blk_size_y, blk_size_x, cur_blk);
    slice_sum += block_sum;
  }
  (*dc) = (double)slice_sum;
}

int64 ComputeSumBlockBased(imgpel **CurrentImage, int height_in_blk,
                           int width_in_blk, int blk_size_y, int blk_size_x,
  ...
  for (cur_blk = start_blk; cur_blk < end_blk; cur_blk++) {
    slice_sum += ComputeBlockSum(CurrentImage, height_in_blk, width_in_blk,
                                 blk_size_y, blk_size_x, cur_blk);
  }
  return slice_sum;
}
    ...
    for (clist = 0; clist < 2 + list_offset; clist++) {
      for (n = 0; n < currSlice->listXsize[clist]; n++) {
        if (wpxDetermineWP(currSlice, clist, n)) {
          ...
          // To simplify these computations we may wish to perform these after a
          // reference is stored in the reference buffer and attach them to the
          ...
          if (dc_ref[clist][n] != 0.0)
            wf_weight =
                (short)(default_weight[0] * dc_org / dc_ref[clist][n] + 0.5);
          else
            wf_weight =
                default_weight[0];  // only used when reference picture is black
  ...
  for (clist = 0; clist < 2 + list_offset; clist++) {
    for (n = 0; n < currSlice->listXsize[clist]; n++) {
      ...
      if (select_offset == 0) {
        ...
        offset[clist][n][0] =
            (short)((dc_org - dc_ref[n]) / (p_Vid->size) + 0.5);
        offset[clist][n][0] =
...
int TestWPBSliceAlg0(Slice *currSlice, int select_method) {
  ...
  if (select_method == 1)  //! implicit mode
  {
    ...
    for (i = 0; i < currSlice->listXsize[LIST_0]; i++) {
      for (j = 0; j < currSlice->listXsize[LIST_1]; j++) {
        if (im_weight[1][i][j][0] != default_weight[0] ||
            im_weight[1][i][j][0] != default_weight[0]) {
          perform_wp = 1;
          break;
        }
      }
