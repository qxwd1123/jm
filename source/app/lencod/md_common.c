 ...
 *************************************************************************************
 */
static inline void CopyMVBlock16(PicMotionParams **mv_info,
                                 MotionVector **rdo_mv, int list, int block_x,
                                 int block_y, int start, int end) {
  int j, i;
  for (j = start; j < end; j++) {
 ...
 *************************************************************************************
 */
static inline void ResetMotionBlock16(PicMotionParams **mv_info, int list,
                                      int block_x, int block_y, int start,
                                      int end) {
  int j, i;
 ...
 *************************************************************************************
 */
static inline void CopyMVBlock8(PicMotionParams **mv_info,
                                MotionVector **rdo_mv, int list, int block_x,
 ...
 *************************************************************************************
 */
static inline void ResetMotionBlock8(PicMotionParams **mv_info, int list,
                                     int block_x, int start, int end) {
  int j;
...
void SetMotionVectorsMBPSlice(Macroblock *currMB) {
  ...
  if (currSlice->mb_aff_frame_flag ||
    ...
    memcpy(&rdopt->all_mv[LIST_0][0][0][0][0], &all_mv[0][0][0][0],
           p_Vid->max_num_references * 9 * MB_BLOCK_PARTITIONS *
               sizeof(MotionVector));
  }

  if (currMB->mb_type == PSKIP)  // Skip mode
  {
    CopyMVBlock16(mv_info, all_mv[0][PSKIP], LIST_0, currMB->block_x,
                  currMB->block_y, 0, 4);
  } else if (currMB->mb_type == P16x16)  // 16x16
  {
    l0_ref = mv_info[currMB->block_y][currMB->block_x].ref_idx[LIST_0];
    CopyMVBlock16(mv_info, all_mv[l0_ref][P16x16], LIST_0, currMB->block_x,
                  currMB->block_y, 0, 4);
  } else if (currMB->mb_type == P16x8)  // 16x8
  {
    l0_ref = mv_info[currMB->block_y][currMB->block_x].ref_idx[LIST_0];
    CopyMVBlock16(mv_info, all_mv[l0_ref][P16x8], LIST_0, currMB->block_x,
                  currMB->block_y, 0, 2);
 ...
 *************************************************************************************
 */
static void SetMVBSlice16x8(Slice *currSlice, PicMotionParams **motion,
                            Macroblock *currMB, int pos) {
  int l0_ref, l1_ref;
  int pdir = currMB->b8x8[pos].pdir;
  if (pdir == LIST_0) {
    l0_ref = motion[currMB->block_y + pos][currMB->block_x].ref_idx[LIST_0];
    CopyMVBlock16(motion, currSlice->all_mv[LIST_0][l0_ref][P16x8], LIST_0,
                  currMB->block_x, currMB->block_y, pos, pos + 2);
    ResetMotionBlock16(motion, LIST_1, currMB->block_x, currMB->block_y, pos,
                       pos + 2);
  } else if (pdir == LIST_1) {
    ResetMotionBlock16(motion, LIST_0, currMB->block_x, currMB->block_y, pos,
                       pos + 2);
    l1_ref = motion[currMB->block_y + pos][currMB->block_x].ref_idx[LIST_1];
    CopyMVBlock16(motion, currSlice->all_mv[LIST_1][l1_ref][P16x8], LIST_1,
                  currMB->block_x, currMB->block_y, pos, pos + 2);
  } else {
    int bipred_me = currMB->b8x8[pos].bipred;
    MotionVector *****all_mv =
        bipred_me ? currSlice->bipred_mv[bipred_me - 1] : currSlice->all_mv;
    l0_ref = motion[currMB->block_y + pos][currMB->block_x].ref_idx[LIST_0];
    CopyMVBlock16(motion, all_mv[LIST_0][l0_ref][P16x8], LIST_0,
                  currMB->block_x, currMB->block_y, pos, pos + 2);

    l1_ref = motion[currMB->block_y + pos][currMB->block_x].ref_idx[LIST_1];
    CopyMVBlock16(motion, all_mv[LIST_1][l1_ref][P16x8], LIST_1,
                  currMB->block_x, currMB->block_y, pos, pos + 2);

    if (bipred_me && (currSlice->mb_aff_frame_flag ||
                      (currSlice->UseRDOQuant && currSlice->RDOQ_QP_Num > 1))) {
      memcpy(currSlice->rddata->all_mv[LIST_0][l0_ref][P16x8][pos],
             all_mv[LIST_0][l0_ref][P16x8][pos],
             2 * BLOCK_MULTIPLE * sizeof(MotionVector));
      memcpy(currSlice->rddata->all_mv[LIST_1][l1_ref][P16x8][pos],
             all_mv[LIST_1][l1_ref][P16x8][pos],
             2 * BLOCK_MULTIPLE * sizeof(MotionVector));
    }
  }
}
 ...
 *************************************************************************************
 */
static void SetMVBSlice8x16(Slice *currSlice, PicMotionParams **motion,
                            Macroblock *currMB, int pos) {
  int l0_ref, l1_ref;
  int pdir = currMB->b8x8[pos >> 1].pdir;
  if (pdir == LIST_0) {
    l0_ref = motion[currMB->block_y][currMB->block_x + pos].ref_idx[LIST_0];
    CopyMVBlock8(&motion[currMB->block_y],
                 currSlice->all_mv[LIST_0][l0_ref][P8x16], LIST_0,
                 currMB->block_x + pos, 0, 4, pos);
    ResetMotionBlock8(&motion[currMB->block_y], LIST_1, currMB->block_x + pos,
                      0, 4);
  } else if (pdir == LIST_1) {
    ResetMotionBlock8(&motion[currMB->block_y], LIST_0, currMB->block_x + pos,
                      0, 4);
    l1_ref = motion[currMB->block_y][currMB->block_x + pos].ref_idx[LIST_1];
    CopyMVBlock8(&motion[currMB->block_y],
                 currSlice->all_mv[LIST_1][l1_ref][P8x16], LIST_1,
                 currMB->block_x + pos, 0, 4, pos);
  } else {
    ...
    l0_ref = motion[currMB->block_y][currMB->block_x + pos].ref_idx[LIST_0];
    CopyMVBlock8(&motion[currMB->block_y], all_mv[LIST_0][l0_ref][P8x16],
                 LIST_0, currMB->block_x + pos, 0, 4, pos);
    l1_ref = motion[currMB->block_y][currMB->block_x + pos].ref_idx[LIST_1];
    CopyMVBlock8(&motion[currMB->block_y], all_mv[LIST_1][l1_ref][P8x16],
                 LIST_1, currMB->block_x + pos, 0, 4, pos);

    if (bipred_me && (currSlice->mb_aff_frame_flag ||
                      (currSlice->UseRDOQuant && currSlice->RDOQ_QP_Num > 1))) {
      int j;
      for (j = 0; j < BLOCK_MULTIPLE; j++) {
        memcpy(&currSlice->rddata->all_mv[LIST_0][l0_ref][P8x16][j][pos],
               &all_mv[LIST_0][l0_ref][P8x16][j][pos],
               2 * sizeof(MotionVector));
      }
      for (j = 0; j < BLOCK_MULTIPLE; j++) {
        memcpy(&currSlice->rddata->all_mv[LIST_1][l1_ref][P8x16][j][pos],
               &all_mv[LIST_1][l1_ref][P8x16][j][pos],
               2 * sizeof(MotionVector));
      }
    }
  }
 ...
 *************************************************************************************
 */
static void SetMVBSlice8x8(Slice *currSlice, PicMotionParams **motion,
                           Macroblock *currMB, int pos_y, int pos_x) {
  int l0_ref, l1_ref;
  ...
  if (pdir == LIST_0) {
    ...
    CopyMVBlock8(&motion[currMB->block_y],
                 currSlice->all_mv[LIST_0][l0_ref][mode], LIST_0,
                 currMB->block_x + pos_x, pos_y, pos_y + 2, pos_x);
    ResetMotionBlock8(&motion[currMB->block_y], LIST_1, currMB->block_x + pos_x,
                      pos_y, pos_y + 2);
  } else if (pdir == LIST_1) {
    ResetMotionBlock8(&motion[currMB->block_y], LIST_0, currMB->block_x + pos_x,
                      pos_y, pos_y + 2);
    l1_ref = motion[block_y][block_x].ref_idx[LIST_1];
    CopyMVBlock8(&motion[currMB->block_y],
                 currSlice->all_mv[LIST_1][l1_ref][mode], LIST_1,
                 currMB->block_x + pos_x, pos_y, pos_y + 2, pos_x);
    ...
    MotionVector *****all_mv =
        bipred_me ? currSlice->bipred_mv[bipred_me - 1] : currSlice->all_mv;
    l0_ref = motion[block_y][block_x].ref_idx[LIST_0];
    CopyMVBlock8(&motion[currMB->block_y], all_mv[LIST_0][l0_ref][mode], LIST_0,
                 currMB->block_x + pos_x, pos_y, pos_y + 2, pos_x);

    l1_ref = motion[block_y][block_x].ref_idx[LIST_1];
    CopyMVBlock8(&motion[currMB->block_y], all_mv[LIST_1][l1_ref][mode], LIST_1,
                 currMB->block_x + pos_x, pos_y, pos_y + 2, pos_x);

    if (bipred_me && (currSlice->mb_aff_frame_flag ||
                      (currSlice->UseRDOQuant && currSlice->RDOQ_QP_Num > 1))) {
      memcpy(&currSlice->rddata->all_mv[LIST_0][l0_ref][mode][pos_y][pos_x],
             &all_mv[LIST_0][l0_ref][mode][pos_y][pos_x],
             2 * sizeof(MotionVector));
      memcpy(&currSlice->rddata->all_mv[LIST_0][l0_ref][mode][pos_y + 1][pos_x],
             &all_mv[LIST_0][l0_ref][mode][pos_y + 1][pos_x],
             2 * sizeof(MotionVector));
      memcpy(&currSlice->rddata->all_mv[LIST_1][l1_ref][mode][pos_y][pos_x],
             &all_mv[LIST_1][l1_ref][mode][pos_y][pos_x],
             2 * sizeof(MotionVector));
      memcpy(&currSlice->rddata->all_mv[LIST_1][l1_ref][mode][pos_y + 1][pos_x],
             &all_mv[LIST_1][l1_ref][mode][pos_y + 1][pos_x],
             2 * sizeof(MotionVector));
    }
  } else  // Invalid direct modes (out of range mvs). Adding this here for
          // precaution purposes.
  {
    ResetMotionBlock8(&motion[currMB->block_y], LIST_0, currMB->block_x + pos_x,
                      pos_y, pos_y + 2);
...
void SetMotionVectorsMBBSlice(Macroblock *currMB) {
  ...
  if (currSlice->mb_aff_frame_flag ||
      (currSlice->UseRDOQuant && currSlice->RDOQ_QP_Num > 1)) {
    memcpy(&currSlice->rddata->all_mv[LIST_0][0][0][0][0],
           &currSlice->all_mv[LIST_0][0][0][0][0],
           2 * p_Vid->max_num_references * 9 * MB_BLOCK_PARTITIONS *
               sizeof(MotionVector));
  }
  ...
  if (currMB->mb_type == P16x16)  // 16x16
  {
    ...
    if (pdir == LIST_0) {
      ...
      CopyMVBlock16(motion, currSlice->all_mv[LIST_0][l0_ref][P16x16], LIST_0,
                    currMB->block_x, currMB->block_y, 0, 4);
      ResetMotionBlock16(motion, LIST_1, currMB->block_x, currMB->block_y, 0,
                         4);
    } else if (pdir == LIST_1) {
      l1_ref = motion[currMB->block_y][currMB->block_x].ref_idx[LIST_1];
      ResetMotionBlock16(motion, LIST_0, currMB->block_x, currMB->block_y, 0,
                         4);
      CopyMVBlock16(motion, currSlice->all_mv[LIST_1][l1_ref][P16x16], LIST_1,
                    currMB->block_x, currMB->block_y, 0, 4);
    } else {
      ...
      MotionVector *****all_mv =
          bipred_me ? currSlice->bipred_mv[bipred_me - 1] : currSlice->all_mv;
      l0_ref = motion[currMB->block_y][currMB->block_x].ref_idx[LIST_0];
      CopyMVBlock16(motion, all_mv[LIST_0][l0_ref][P16x16], LIST_0,
                    currMB->block_x, currMB->block_y, 0, 4);
      l1_ref = motion[currMB->block_y][currMB->block_x].ref_idx[LIST_1];
      CopyMVBlock16(motion, all_mv[LIST_1][l1_ref][P16x16], LIST_1,
                    currMB->block_x, currMB->block_y, 0, 4);
      ...
      if (bipred_me &&
          (currSlice->mb_aff_frame_flag ||
           (currSlice->UseRDOQuant && currSlice->RDOQ_QP_Num > 1))) {
        memcpy(&currSlice->rddata->all_mv[LIST_0][l0_ref][P16x16][0][0],
               &all_mv[LIST_0][l0_ref][P16x16][0][0],
               MB_BLOCK_PARTITIONS * sizeof(MotionVector));
        memcpy(&currSlice->rddata->all_mv[LIST_1][l1_ref][P16x16][0][0],
               &all_mv[LIST_1][l1_ref][P16x16][0][0],
               MB_BLOCK_PARTITIONS * sizeof(MotionVector));
      }
    }
  } else if (currMB->mb_type == P16x8)  // 16x8
  ...
  } else if (currMB->mb_type == P8x16)  // 16x8
  {
    ...
    SetMVBSlice8x16(currSlice, motion, currMB, 2);
             ...
             currMB->mb_type == BSKIP_DIRECT)  // 8x8 & Direct/SKIP
  {
...
void copy_image_data_16x16(imgpel **imgBuf1, imgpel **imgBuf2, int off1,
  ...
  for (j = 0; j < MB_BLOCK_SIZE; j++) {
    memcpy(&imgBuf1[j][off1], &imgBuf2[j][off2],
...
*************************************************************************************
*/
void set_chroma_pred_mode(Macroblock *currMB, RD_PARAMS enc_mb,
                          int *mb_available, char chroma_pred_mode_range[2]) {
  VideoParameters *p_Vid = currMB->p_Vid;
  ...
  if ((p_Vid->yuv_format != YUV400) &&
    ...
    currSlice->intra_chroma_prediction(currMB, &mb_available[0],
                                       &mb_available[1], &mb_available[2]);
