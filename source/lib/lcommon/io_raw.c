...
static inline int ReadData(int vfile, FrameFormat *source, unsigned char *buf) {
  ...
  for (i = 0; i < source->height[0]; i++) {
    if (read(vfile, cur_buf, read_size) != read_size) {
      printf(
          "read_one_frame: cannot read %d bytes from input file, unexpected "
          "EOF!\n",
          source->width[0]);
      return 0;
    }
  ...
  if (source->yuv_format != YUV400) {
    ...
    for (j = 0; j < 2; j++) {
      for (i = 0; i < source->height[1]; i++) {
        if (read(vfile, cur_buf, read_size) != read_size) {
          printf(
              "read_one_frame: cannot read %d bytes from input file, "
              "unexpected EOF!\n",
              source->width[1]);
          return 0;
        }
...
static inline int ReadDataV210(int vfile, int framesize_in_bytes,
                               unsigned char *buf) {
  if (read(vfile, buf, (int)framesize_in_bytes) != (int)framesize_in_bytes) {
    printf(
        "read_one_frame: cannot read %d bytes from input file, unexpected "
...
int ReadFrameConcatenated(InputParameters *p_Inp, VideoDataFile *input_file,
  ...
  Boolean is_v210 =
      (input_file->is_interleaved && source->pixel_format == V210);
  ...
  const int64 framesize_in_bytes = (is_v210 == TRUE)
                                       ? 16 * (source->size_cmp[0] / 6)
                                       : bytes_y + 2 * bytes_uv;
  ...
  if ((source->pic_unit_size_on_disk & 0x07) == 0) {
    ...
    printf(
        "read_one_frame (NOT IMPLEMENTED): pic unit size on disk must be "
        "divisible by 8");
    exit(-1);
  }
  return file_read;
