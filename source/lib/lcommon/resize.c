...
********************************************************/
int PyrDownG5x5_U8CnR(const imgpel *src, int srcstep,
                      int width,   // width of source;
                      int height,  // height of source;
                      imgpel *dst, int dststep, int Cs) {
  worktype *buf = NULL;
  ...
  // alloc buffer;
  buf = malloc((width & -2) * 2 * Cs * sizeof(worktype) *
               (PD_SZ + 1));  // buf size is 2 times larger than required;
  buffer = buf;
  ...
  for (y = 0; y < height; y += 2, dst += dststep) {
    ...
    if (Cs == 1) {
      if (width > PD_SZ / 2) {
        for (y1 = fst; y1 < lst; y1++, src += srcstep) {
          ...
          row[Wd - 1] = PD_RB(src[Wd * 2 - 4], src[Wd * 2 - 3], src[Wd * 2 - 2],
                              src[Wd * 2 - 1]);
          /* other points (even) */
          for (x = 1; x < Wd - 1; x++) {
      ...
      for (y1 = fst; y1 < lst; y1++, src += srcstep) {
        ...
        if (width > PD_SZ / 2) {
          ...
          for (c = 0; c < 3; c++) {
            ...
            row[Wdn - 3 + c] =
                PD_RB(src[Wdn * 2 - 12 + c], src[Wdn * 2 - 9 + c],
          ...
          for (x = 3; x < Wdn - 3; x += 3) {
            row[x] = PD_FILTER(src[2 * x - 6], src[2 * x - 3], src[2 * x],
                               src[2 * x + 3], src[2 * x + 6]);
            row[x + 1] =
                PD_FILTER(src[2 * x - 5], src[2 * x - 2], src[2 * x + 1],
                          src[2 * x + 4], src[2 * x + 7]);
            row[x + 2] =
                PD_FILTER(src[2 * x - 4], src[2 * x - 1], src[2 * x + 2],
                          src[2 * x + 5], src[2 * x + 8]);
          }
    ...
    if (y > 0) {
      if (y < height - PD_SZ / 2) {
        for (x = 0; x < Wdn; x++, x1++) {
          dst[x] = (imgpel)PD_SCALE_INT(
              PD_FILTER(row01[x], row01[x1], row23[x], row23[x1], row4[x]));
        }
