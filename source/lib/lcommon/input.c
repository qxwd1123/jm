...
static void deinterleave_yuv420(
  ...
  unsigned char *ocmp1 =
      ocmp0 + symbol_size_in_bytes * source->size_cmp[Y_COMP];
  unsigned char *ocmp2 =
      ocmp1 + symbol_size_in_bytes * source->size_cmp[U_COMP];
...
static void deinterleave_yuv444(
    unsigned char **input,    //!< input buffer
    unsigned char **output,   //!< output buffer
    FrameFormat *source,      //!< format of source buffer
    int symbol_size_in_bytes  //!< number of bytes per symbol
...
static void deinterleave_v210(
  ...
  for (i = 0; i < source->size_cmp[U_COMP] / 3; i++) {
    ...
    *(ui16cmp0 + 4) = (*(ui32cmp++) & 0x3FF);         // Y 1
    ...
    // printf("luma: %d %d %d %d %d %d\n", *(ui16cmp0 + 0), *(ui16cmp0 + 1),
    // *(ui16cmp0 + 2), *(ui16cmp0 + 3), *(ui16cmp0 + 4), *(ui16cmp0 + 5));
...
void buf2img_bitshift(
  ...
  if (testEndian()) {
    ...
    if (size_x == o_size_x && size_y == o_size_y) {
      ...
      if (o_size_x >= size_x) dst_offset_x = (o_size_x - size_x) >> 1;

      if (o_size_y >= size_y) dst_offset_y = (o_size_y - size_y) >> 1;
    ...
    if (size_x == o_size_x && size_y == o_size_y) {
      ...
      for (j = 0; j < iminheight; j++) {
        memcpy(&imgX[j + dst_offset_y][dst_offset_x],
               ...
               iminwidth * symbol_size_in_bytes);
      }
      for (j = 0; j < iminheight; j++) {
        j_pos = (j + offset_y) * size_x + offset_x;
        for (i = 0; i < iminwidth; i++) {
...
void buf2img_endian(
  ...
  switch (symbol_size_in_bytes) {
    ...
    default: {
      error(
          "reading only from formats of 8, 16 or 32 bit allowed on big endian "
          "architecture",
          500);
      break;
    }
...
void AllocateFrameMemory(VideoParameters *p_Vid, InputParameters *p_Inp,
                         FrameFormat *source) {
  // Note that size seems to be ok even for v210 formats (wasteful yes, but
  // should not create any issues with how we manage that format.
  if (NULL ==
...
void DeleteFrameMemory(VideoParameters *p_Vid) {
  if (p_Vid->buf != NULL) free(p_Vid->buf);
  if (p_Vid->ibuf != NULL) free(p_Vid->ibuf);
}
