...
static void getIntArray(Tiff *t, uint32 offset, TiffType type, uint32 a[],
  ...
  switch (type) {
    ...
    case T_RATIONAL:
      ...
      break;
    default: assert(type == T_SHORT || type == T_LONG || type == T_RATIONAL);
  }
...
static int readFileIntoMemory(Tiff *t, const char *path) {
  long cnt, result;
  uint16 byteOrder;
  int endian = 1, machineLittleEndian = (*((char *)(&endian)) == 1) ? 1 : 0, fd;
...
static int readTiff(Tiff *t, char *path) {
  ...
  if (readFileIntoMemory(t, path)) goto Error;
  if (readImageFileHeader(t)) goto Error;
  if (readImageFileDirectory(t)) goto Error;
  if (readImageData(t)) goto Error;
  return 0;
...
static int RGB_YUV_initialize(
  ...
  T->offuv =
      (int)(INTEGER_SCALE * (0.5 * pixScale + 0.5));  // offset + rounding
 ...
 *****************************************************************************
 */
static void RGB_YUV_rgb_to_yuv(
    RGB_YUV *T,
...
int ReadTIFFImage(InputParameters *p_Inp, VideoDataFile *input_file,
  ...
  assert((width & 1) == 0 &&
         (height & 1) == 0);  // width & height must be even.

  if (source->color_model == CM_RGB && !input_file->is_interleaved) {
    fprintf(stderr,
            "ReadTIFFImage:  RGB input file has not been declared as "
  ...
  if (source->color_model == CM_YUV) {
    ...
    switch (source->yuv_format) {
      case YUV420:
        ...
        // U
        horizontal_half_1chan_cosite(img + 1, width, height, 3, temp2, 1,
                                     65535);
        vertical_half_1chan(temp2, width / 2, height, 1, up, 1, 65535);
        // V
        horizontal_half_1chan_cosite(img + 2, width, height, 3, temp2, 1,
                                     65535);
